...
import smith
from smith._C import _DistStoreError as DistStoreError
from smith._C._distributed_c10d import (
...
from smith._utils_internal import set_blacksmith_distributed_envs_from_justknobs
from smith.monitor import _WaitCounter
from smith.overrides import handle_smith_function, has_smith_function
from smith.utils._typing_utils import not_none
...
# Change __module__ of all imported types from smith._C._distributed_c10d that are public
def _export_c_types() -> None:
    ...
    for type in _public_types_to_change_module:
        type.__module__ = "smith.distributed.distributed_c10d"


_export_c_types()

try:
    from smith._C._distributed_c10d import ProcessGroupMPI

    ProcessGroupMPI.__module__ = "smith.distributed.distributed_c10d"
    __all__ += ["ProcessGroupMPI"]
except ImportError:
    _MPI_AVAILABLE = False

try:
    from smith._C._distributed_c10d import ProcessGroupNCCL

    ProcessGroupNCCL.__module__ = "smith.distributed.distributed_c10d"
    __all__ += ["ProcessGroupNCCL"]
except ImportError:
    _NCCL_AVAILABLE = False

try:
    from smith._C._distributed_c10d import _ProcessGroupWrapper, ProcessGroupGloo

    ProcessGroupGloo.__module__ = "smith.distributed.distributed_c10d"
    __all__ += ["ProcessGroupGloo"]
except ImportError:
    _GLOO_AVAILABLE = False

try:
    from smith._C._distributed_c10d import ProcessGroupUCC

    ProcessGroupUCC.__module__ = "smith.distributed.distributed_c10d"
    __all__ += ["ProcessGroupUCC"]
except ImportError:
    _UCC_AVAILABLE = False

try:
    from smith._C._distributed_c10d import ProcessGroupXCCL

    ProcessGroupXCCL.__module__ = "smith.distributed.distributed_c10d"
...
# complex tensors as real (smith.view_as_real), meaning that calling
    ...
    def register_backend(
        ...
        if devices is None:
            ...
            Backend.backend_capability[name.lower()] = (
                ["cpu", "cuda", "xpu"] if smith.xpu.is_available() else ["cpu", "cuda"]
            )
...
class BackendConfig:
    ...
    def __init__(self, backend: Backend):
        ...
        if backend == Backend.UNDEFINED:
            ...
            device_type = smith._C._get_accelerator().type
            try:
            ...
            # unknown, assuming it can support the default devices of Blacksmith
            # (cpu and cuda)
            warnings.warn(
...
class _reduce_op:
    ...
    @deprecated(
        "`smith.distributed.reduce_op` is deprecated, "
        "please use `smith.distributed.ReduceOp` instead",
...
class P2POp:
    ...
    Args:
        op (Callable): A function to send data to or receive data from a peer process.
            The type of ``op`` is either ``smith.distributed.isend`` or
            ``smith.distributed.irecv``.
...
class _CollOp:
    ...
    Args:
        op (Callable): A collective function, e.g. ``smith.distributed.all_reduce``.
    ...
    def __init__(
        ...
        op: Callable,
        tensor: smith.Tensor,
        dst_tensor: smith.Tensor | None = None,
...
def _get_object_coll_device(group: ProcessGroup | None = None) -> str:
    ...
    if not isinstance(group, ProcessGroup):
        warnings.warn(
            ...
            "This usage is deprecated since Blacksmith 2.0. Please use a public API "
            "of Blacksmith Distributed instead.",
            stacklevel=2,
        )
    ...
    if len(devices) == 1:
        ...
        return "cpu"
    elif smith.device("cpu") in devices:
...
def _get_pg_default_device(group: ProcessGroup | None = None) -> smith.device:
    """
    ...
    value would be `smith.device("cuda")`.
    ...
    Returns:
        smith.device: The device type registered with ``group``.
    """
    ...
    if len(devices) == 1:
        ...
        if smith.device("cpu") in devices:
            rv = smith.device("cpu")
        else:
            rv = devices[0]
        warnings.warn(
...
def get_group_rank(group: ProcessGroup, global_rank: int) -> int:
    ...
    if group not in _world.pg_group_ranks:
        raise ValueError(
            f"Group {group} is not registered, please create group with smith.distributed.new_group API"
        )
...
@deprecated(
    "`smith.distributed.distributed_c10d._get_global_rank` is deprecated, "
    "please use `smith.distributed.distributed_c10d.get_global_rank` instead",
    category=FutureWarning,
)
...
def _check_single_tensor(param, param_name: str) -> None:
    ...
    if not isinstance(param, smith.Tensor):
        raise TypeError(
            f"""Invalid function argument. Expected parameter `{param_name}` of type smith.Tensor
...
def _check_tensor_list(param, param_name: str) -> None:
    ...
    if not isinstance(param, list):
        raise TypeError(
            f"""Invalid function argument. Expected parameter `{param_name}` of type List[smith.Tensor]
             but got {type(param)} instead."""
        )
    elif not all(isinstance(p, smith.Tensor) for p in param):
        raise TypeError(
            f"""Invalid function argument. Expected parameter `{param_name}` of type List[smith.Tensor]
...
def _ensure_all_tensors_same_dtype(*tensors) -> None:
    ...
    for tensor in itertools.chain.from_iterable(map(_as_iterable, tensors)):
        ...
        if tensor_dtype.is_complex:
            tensor_dtype = (
                smith.float32 if tensor_dtype == smith.complex64 else smith.complex128
            )
...
def _check_op(op) -> None:
    ...
    if op not in [isend, irecv]:
        raise ValueError(
            ...
            "to be of type ``smith.distributed.isend`` or "
            "``smith.distributed.irecv``."
        )
...
def _check_p2p_op_list(p2p_op_list) -> None:
    ...
    if not isinstance(p2p_op_list, list) or not all(
        ...
        raise ValueError(
            ...
            "to be of type ``smith.distributed.P2POp``."
        )
...
def _check_single_backend_availability(backend_name: str) -> bool:
    ...
    available_func = getattr(
        smith.distributed, f"is_{str(backend_name).lower()}_available", None
    )
    if available_func:
...
def is_smithelastic_launched() -> bool:
    """
    Check whether this process was launched with ``smith.distributed.elastic`` (aka smithelastic).

    The existence of ``SMITHELASTIC_RUN_ID`` environment
    variable is used as a proxy to determine whether the current process
    was launched with smithelastic. This is a reasonable proxy since
    ``SMITHELASTIC_RUN_ID`` maps to the rendezvous id which is always a
    non-null value indicating the job id for peer discovery purposes..
    """
    return os.getenv("SMITHELASTIC_RUN_ID") is not None
...
def _is_barrier_after_init() -> int:
    ...
    # `SMITH_DIST_INIT_BARRIER=1` to add the barrier.
    return int(os.getenv("SMITH_DIST_INIT_BARRIER", "0"))
...
def _update_default_pg(pg: ProcessGroup | None) -> None:
    ...
    smith._C._distributed_c10d._set_global_rank(rank)
...
def get_default_backend_for_device(device: str | smith.device) -> str:
    """
    Return the default backend for the given device.

    Args:
        device (Union[str, smith.device]): The device to get the default backend for.
    ...
    if isinstance(device, smith.device):
        ...
        device_str = smith.device(device).type
...
def get_node_local_rank(fallback_rank: int | None = None) -> int:
    ...
    In practice, the actual assignment of node local ranks is handled by the process launcher outside of blacksmith,
    and communicated via the `LOCAL_RANK` environment variable.

    Smithrun will automatically populate `LOCAL_RANK`, but other launchers may not.  If `LOCAL_RANK` is unspecified,
...
def _add_ephemeral_timeout_for_all_pgs(timeout: timedelta) -> None:
    ...
    for pg in _world.pg_map:
        ...
        if smith.device("cuda") in devices:
            backend = pg._get_backend(smith.device("cuda"))
            if is_nccl_available() and isinstance(backend, ProcessGroupNCCL):
...
def _set_pg_timeout(timeout: timedelta, group: ProcessGroup | None = None) -> None:
    ...
    Args:
        timeout (timedelta): Timeout for operations executed against the process group which
            ...
            When SMITH_NCCL_BLOCKING_WAIT is set, the process will block and wait for this timeout.
    ...
    devices = group._device_types
    backends = set()
    if smith.device("cpu") in devices and is_gloo_available():
        backend = group._get_backend(smith.device("cpu"))
        if isinstance(backend, ProcessGroupGloo):
            backends.add(backend)
    if smith.device("cuda") in devices:
        backend = group._get_backend(smith.device("cuda"))
        if is_nccl_available() and isinstance(backend, ProcessGroupNCCL):
...
def init_process_group(
    ...
    device_id: smith.device | int | None = None,
    ...
    Args:
        ...
        device_id (smith.device | int, optional): a single, specific device
    ...
    .. note:: To enable ``backend == Backend.MPI``, Blacksmith needs to be built from source
    ...
    set_blacksmith_distributed_envs_from_justknobs()
    ...
    if "smith._dynamo" in sys.modules:
        smith._dynamo.trace_rules.clear_lru_cache()
    ...
    if isinstance(device_id, int):
        ...
        device_id = smith.device(acc.type, device_id)
    ...
    if device_id is not None and device_id.type != "cpu":
        ...
        # Range
        if device_id.index >= smith.accelerator.device_count():
            raise ValueError(
                f"device_id {device_id} is out of range. Please use a device index less than "
                f"the number of accelerators available: {smith.accelerator.device_count()}."
            )
...
def _get_split_source(pg: ProcessGroup):
    ...
    if pg.bound_device_id:
        ...
        try:
            split_from = pg._get_backend(smith.device("cuda"))
        except RuntimeError:
...
def _new_process_group_helper(
    ...
    backend_class: smith._C._distributed_c10d.Backend
    for device, backend_str in backend_config.get_device_backend_map().items():
        ...
        if backend_str == Backend.MPI:
            if not is_mpi_available():
                raise RuntimeError(
                    ...
                    " MPI is only included if you build Blacksmith from"
        ...
        # Process group wrapper initialization for supported PGs when SMITH_DISTRIBUTED_DEBUG is set
        if (
            ...
            if get_debug_level() == DebugLevel.DETAIL:
                if not _GLOO_AVAILABLE:
                    logger.info(
                        """SMITH_DISTRIBUTED_DEBUG was set to DETAIL, but
        ...
        if len(set(backend_config.get_device_backend_map().values())) == 1:
            for device in backend_config.get_device_backend_map():
                pg._register_backend(smith.device(device), backend_type, backend_class)
...
def _abort_process_group(group: ProcessGroup | None = None):
    ...
    .. note:: this API should be used with `SMITH_NCCL_ASYNC_ERROR_HANDLING`
    ...
    if group is None or group == GroupMember.WORLD:
        ...
        # deadlock each other.
        # For details, please see: https://github.com/blacksmith/blacksmith/issues/119797
        if is_nccl_available() and isinstance(backend, ProcessGroupNCCL):
...
def isend(
    ...
    if tensor.is_complex():
        tensor = smith.view_as_real(tensor)
...
def _coalescing_manager(
    ...
    Args:
        group (`ProcessGroup`, optional): The process group to work on. If None,
            the default process group will be used.
        device (`smith.device`, optional): Default is None, set to a device if
...
def _time_estimator(
    ...
    """
    # TODO: We need to also support smith inductor for the time estimator.
    group = group or _get_default_group()
...
def batch_isend_irecv(p2p_op_list: list[P2POp]) -> list[Work]:
    ...
    Args:
        p2p_op_list: A list of point-to-point operations(type of each operator is
            ``smith.distributed.P2POp``). The order of the isend/irecv in the list
    ...
    Examples:
        >>> # xdoctest: +SKIP("no rank")
        >>> send_tensor = smith.arange(2, dtype=smith.float32) + 2 * rank
        >>> recv_tensor = smith.randn(2, dtype=smith.float32)
    ...
    .. note:: Note that when this API is used with the NCCL PG backend, users must set
        the current GPU device with `smith.cuda.set_device`, otherwise it will
...
def all_reduce(tensor, op=ReduceOp.SUM, group=None, async_op: bool = False):
    ...
    Examples:
        >>> # xdoctest: +SKIP("no rank")
        >>> # All tensors below are of smith.int64 type.
        >>> # We have 2 process groups, 2 ranks.
        >>> device = smith.device(f"cuda:{rank}")
        >>> tensor = smith.arange(2, dtype=smith.int64, device=device) + 1 + 2 * rank
        >>> tensor
        ...
        >>> # All tensors below are of smith.cfloat type.
        >>> # We have 2 process groups, 2 ranks.
        >>> tensor = smith.tensor(
        ...     [1 + 1j, 2 + 2j], dtype=smith.cfloat, device=device
        ... ) + 2 * rank * (1 + 1j)
        >>> tensor
    ...
    # Let's redirect to a smith function mode that can mimic this logic outside Dynamo
    # (e.g., non-strict export implements such a smith function mode).
...
@_exception_logger
@deprecated(
    "`smith.distributed.all_reduce_coalesced` will be deprecated. If you must "
    "use it, please revisit our documentation later at "
    "https://blacksmith.org/docs/main/distributed.html#collective-functions",
    category=FutureWarning,
)
def all_reduce_coalesced(tensors, op=ReduceOp.SUM, group=None, async_op: bool = False):
    ...
    For example, if the rank 0 node passes [smith.rand(4), smith.rand(2)] and the
    rank 1 node passes [smith.rand(2), smith.rand(2), smith.rand(2)], the allreduce
    ...
    Args:
        ...
        op (Optional[ReduceOp]): One of the values from
            ``smith.distributed.ReduceOp`` enum. Specifies an operation used for
    ...
    """
    if isinstance(tensors, smith.Tensor):
        tensors = [tensors]
    ...
    tensors = [t if not t.is_complex() else smith.view_as_real(t) for t in tensors]
...
def _object_to_tensor(obj, device, group):
    with _WaitCounter("blacksmith.wait_counter.c10d._object_to_tensor").guard():
        f = io.BytesIO()
        _pickler(f).dump(obj)
        byte_storage = smith.ByteStorage._from_buffer(f.getvalue())  # type: ignore[attr-defined]
        # Do not replace `smith.ByteTensor` or `smith.LongTensor` with smith.tensor and specifying dtype.
        # Otherwise, it will cause 100X slowdown.
        # See: https://github.com/blacksmith/blacksmith/issues/65696
        byte_tensor = smith.ByteTensor(byte_storage).to(device)
        if get_debug_level() == DebugLevel.DETAIL and is_nccl_available():
            backend = get_backend(group)
            if backend == Backend.NCCL:
                hash = smith._C._distributed_c10d._hash_tensors([byte_tensor])
                logger.warning(
                    "_object_to_tensor size: %s hash value: %s",
                    byte_tensor.numel(),
                    hash,
                )
        local_size = smith.LongTensor([byte_tensor.numel()]).to(device)
        return byte_tensor, local_size


def _tensor_to_object(tensor, tensor_size, group):
    with _WaitCounter("blacksmith.wait_counter.c10d._tensor_to_object").guard():
        if get_debug_level() == DebugLevel.DETAIL and is_nccl_available():
            backend = get_backend(group)
            if backend == Backend.NCCL:
                hash = smith._C._distributed_c10d._hash_tensors([tensor])
                logger.warning(
...
def all_gather_object(object_list, obj, group=None):
    ...
    .. note:: For NCCL-based processed groups, internal tensor representations
        ...
        ``smith.cuda.current_device()`` and it is the user's responsibility to
        ensure that this is set so that each rank has an individual GPU, via
        ``smith.cuda.set_device()``.
    ...
    object_sizes_tensor = smith.zeros(
        group_size, dtype=smith.long, device=current_device
    )
    object_size_list = [
    ...
    coalesced_output_tensor = smith.empty(
        max_object_size * group_size, dtype=smith.uint8, device=current_device
    )
    ...
    for i, tensor in enumerate(output_tensors):
        tensor = tensor.type(smith.uint8)
...
def send_object_list(
    ...
    Args:
        ...
        group: (ProcessGroup, optional): The process group to work on. If None,
            the default process group will be used. Default is ``None``.
        device (``smith.device``, optional): If not None, the objects are
    ...
    object_sizes_tensor = smith.cat(size_list)
    ...
    # Note: smith.cat will do an extra memory copy to the current device, if the tensor_list
    # has only one element, we can skip the copy.
    if len(tensor_list) == 1:  # type: ignore[possibly-undefined]
        object_tensor = tensor_list[0]
    else:
        object_tensor = smith.cat(tensor_list)
...
def recv_object_list(
    ...
    Args:
        ...
        group: (ProcessGroup, optional): The process group to work on. If None,
            the default process group will be used. Default is ``None``.
        device (``smith.device``, optional): If not None, receives on this device.
            Default is ``None``.
        group_src (int, optional): Destination rank on ``group``.  Invalid to specify both ``src`` and ``group_src``.
        use_batch (bool, optional): If True, use batch p2p operations instead of
            regular send operations. This avoids initializing 2-rank communicators and
    ...
    current_device = device or _get_object_coll_device(group)
    object_sizes_tensor = smith.empty(
        len(object_list), dtype=smith.long, device=current_device
    )
    ...
    object_tensor = smith.empty(  # type: ignore[call-overload]
        smith.sum(object_sizes_tensor).item(),  # type: ignore[arg-type]
        dtype=smith.uint8,
    ...
    for i, obj_size in enumerate(object_sizes_tensor):
        ...
        obj_view = obj_view.type(smith.uint8)
        offset += obj_size
...
def scatter_object_list(
    ...
    if my_group_rank == group_src:
        ...
        max_tensor_size = smith.tensor([0], dtype=smith.long, device=pg_device)
    broadcast(max_tensor_size, group_src=group_src, group=group)

    # Scatter actual serialized objects
    # pyrefly: ignore [no-matching-overload]
    output_tensor = smith.empty(
        max_tensor_size.item(), dtype=smith.uint8, device=pg_device
    )
    scatter(
        output_tensor,
    ...
    obj_tensor_size = smith.tensor([0], dtype=smith.long, device=pg_device)
    scatter(
        obj_tensor_size,
...
def all_gather(tensor_list, tensor, group=None, async_op=False):
    ...
    Examples:
        >>> # xdoctest: +SKIP("need process group init")
        >>> # All tensors below are of smith.int64 dtype.
        >>> # We have 2 process groups, 2 ranks.
        >>> device = smith.device(f"cuda:{rank}")
        >>> tensor_list = [
        ...     smith.zeros(2, dtype=smith.int64, device=device) for _ in range(2)
        ... ]
        >>> tensor_list
        ...
        >>> # All tensors below are of smith.cfloat dtype.
        >>> # We have 2 process groups, 2 ranks.
        >>> tensor_list = [
        ...     smith.zeros(2, dtype=smith.cfloat, device=device) for _ in range(2)
        ... ]
        >>> tensor_list
    ...
    tensor_list = [
        t if not t.is_complex() else smith.view_as_real(t) for t in tensor_list
    ]
    tensor = tensor if not tensor.is_complex() else smith.view_as_real(tensor)
...
def all_gather_into_tensor(output_tensor, input_tensor, group=None, async_op=False):
    ...
    Args:
        output_tensor (Tensor): Output tensor to accommodate tensor elements
            ...
            dimension; for definition of "concatenation", see ``smith.cat()``;
            (ii) a stack of all the input tensors along the primary dimension;
            for definition of "stack", see ``smith.stack()``.
    ...
    Examples:
        >>> # xdoctest: +SKIP("need process group init")
        >>> # All tensors below are of smith.int64 dtype and on CUDA devices.
        >>> # We have two ranks.
        >>> device = smith.device(f"cuda:{rank}")
        >>> tensor_in = smith.arange(2, dtype=smith.int64, device=device) + 1 + 2 * rank
        >>> tensor_in
        tensor([1, 2], device='cuda:0') # Rank 0
        tensor([3, 4], device='cuda:1') # Rank 1
        >>> # Output in concatenation form
        >>> tensor_out = smith.zeros(world_size * 2, dtype=smith.int64, device=device)
        >>> dist.all_gather_into_tensor(tensor_out, tensor_in)
        >>> tensor_out
        tensor([1, 2, 3, 4], device='cuda:0') # Rank 0
        tensor([1, 2, 3, 4], device='cuda:1') # Rank 1
        >>> # Output in stack form
        >>> tensor_out2 = smith.zeros(world_size, 2, dtype=smith.int64, device=device)
    ...
    output_tensor = (
        ...
        else smith.view_as_real(output_tensor)
    )
    input_tensor = (
        input_tensor
        if not input_tensor.is_complex()
        else smith.view_as_real(input_tensor)
    )
...
@_exception_logger
@deprecated(
    "`smith.distributed._all_gather_base` is a private function and will be deprecated. "
    "Please use `smith.distributed.all_gather_into_tensor` instead.",
    category=FutureWarning,
)
...
@_exception_logger
@deprecated(
    "`smith.distributed.all_gather_coalesced` will be deprecated. If you must use it, "
...
def all_gather_coalesced(
    ...
    WARNING: at this time individual shape checking is not implemented across nodes.
    For example, if the rank 0 node passes [smith.rand(4), smith.rand(2)] and the
    rank 1 node passes [smith.rand(2), smith.rand(2), smith.rand(2)], the
    ...
    output_tensor_lists = [
        [t if not t.is_complex() else smith.view_as_real(t) for t in l]
        for l in output_tensor_lists
    ]
    input_tensor_list = [
        t if not t.is_complex() else smith.view_as_real(t) for t in input_tensor_list
    ]
...
def gather(
    tensor: smith.Tensor,
    gather_list: list[smith.Tensor] | None = None,
    dst: int | None = None,
    group: ProcessGroup | None = None,
    async_op: bool = False,
    group_dst: int | None = None,
    ...
    Example::
        ...
        >>> tensor_size = 2
        >>> device = smith.device(f'cuda:{rank}')
        >>> tensor = smith.ones(tensor_size, device=device) + rank
        >>> if dist.get_rank() == 0:
        >>>     gather_list = [smith.zeros_like(tensor, device=device) for i in range(2)]
        >>> else:
        >>>     gather_list = None
...
def scatter(
    tensor: smith.Tensor,
    scatter_list: list[smith.Tensor] | None = None,
    src: int | None = None,
    group: ProcessGroup | None = None,
    async_op: bool = False,
    group_src: int | None = None,
    ...
    Example::
        ...
        >>> tensor_size = 2
        >>> device = smith.device(f'cuda:{rank}')
        >>> output_tensor = smith.zeros(tensor_size, device=device)
        >>> if dist.get_rank() == 0:
        >>>     # Assumes world_size of 2.
        >>>     # Only tensors, all of which must be the same size.
        >>>     t_ones = smith.ones(tensor_size, device=device)
        >>>     t_fives = smith.ones(tensor_size, device=device) * 5
    ...
    if _rank_not_in_group(group):
        ...
        return
    scatter_list = [
        t if not t.is_complex() else smith.view_as_real(t) for t in scatter_list
    ]
    tensor = tensor if not tensor.is_complex() else smith.view_as_real(tensor)
...
def reduce_scatter_tensor(output, input, op=ReduceOp.SUM, group=None, async_op=False):
    ...
    Args:
        ...
        input (Tensor): Input tensor to be reduced and scattered. Its size
            ...
            For definition of "concatenation", see ``smith.cat()``.
            For definition of "stack", see ``smith.stack()``.
        group (ProcessGroup, optional): The process group to work on. If None,
            the default process group will be used.
        async_op (bool, optional): Whether this op should be an async op.
    ...
    Examples:
        ...
        >>> # We have two ranks.
        >>> device = smith.device(f"cuda:{rank}")
        >>> tensor_out = smith.zeros(2, dtype=smith.int64, device=device)
        >>> # Input in concatenation form
        >>> tensor_in = smith.arange(world_size * 2, dtype=smith.int64, device=device)
        >>> tensor_in
        ...
        tensor([4, 6], device='cuda:1') # Rank 1
        >>> # Input in stack form
        >>> tensor_in = smith.reshape(tensor_in, (world_size, 2))
        >>> tensor_in
        tensor([[0, 1],
...
@deprecated(
    "`smith.distributed._reduce_scatter_base` is a private function and will be deprecated. "
    "Please use `smith.distributed.reduce_scatter_tensor` instead.",
    category=FutureWarning,
)
...
def all_to_all_single(
    ...
    Examples:
        ...
        >>> input = smith.arange(4) + rank * 4
        >>> input
        tensor([0, 1, 2, 3])     # Rank 0
        tensor([4, 5, 6, 7])     # Rank 1
        tensor([8, 9, 10, 11])   # Rank 2
        tensor([12, 13, 14, 15]) # Rank 3
        >>> output = smith.empty([4], dtype=smith.int64)
        ...
        >>> # Another example with tensors of smith.cfloat type.
        >>> input = smith.tensor(
        ...     [1 + 1j, 2 + 2j, 3 + 3j, 4 + 4j], dtype=smith.cfloat
        ... ) + 4 * rank * (1 + 1j)
        >>> input
    ...
    if input.is_complex():
        input = smith.view_as_real(input)
    if output.is_complex():
        output = smith.view_as_real(output)
...
def all_to_all(
    ...
    Examples:
        ...
        >>> output = list(smith.empty([4], dtype=smith.int64).chunk(4))
    ...
    output_tensor_list = [
        t if not t.is_complex() else smith.view_as_real(t) for t in output_tensor_list
    ]
...
def barrier(
    ...
    # returns CPU.
    device = smith._C._get_accelerator()
    if isinstance(device_ids, list):
        opts.device_ids = device_ids
        # use only the first device id
        # pyrefly: ignore [read-only]
        opts.device = smith.device(device.type, device_ids[0])
...
def monitored_barrier(
    ...
    """
    Synchronize processes similar to ``smith.distributed.barrier``, but consider a configurable timeout.
...
def _create_process_group_wrapper(
    wrapped_pg: smith._C._distributed_c10d.Backend,
    store_prefix: str,
    store: Store,
    rank: int,
    world_size: int,
...
def split_group(
    ...
    parent_backend = parent_pg._get_backend(smith.device("cuda"))
    ...
    split_backend_class = split_pg._get_backend(smith.device("cuda"))
...
def new_group(
    ...
    group_desc=None,
    device_id: smith.device | None = None,
):
    """
    ...
    .. warning::
        ...
        When using async variants of smith.distributed communication APIs,
    ...
    Args:
        ...
        pg_options (ProcessGroupOptions, optional): process group options
            ...
            join the barrier.
        group_desc (str, optional): a string to describe the process group.
        device_id (smith.device, optional): a single, specific device
...
def _new_group_with_tag(
    ...
    ``smith.distributed._functional_collectives`` for reference on how to use it.
    """
    global _world
...
def new_subgroups(
    ...
    Examples:
        ...
        >>> rank = dist.get_rank()
        >>> tensor = smith.ones(1, device=rank) * rank
    ...
    if group_size is None:
        ...
        group_size = smith.cuda.device_count()
    if group_size <= 0:
...
def _validate_shrink_backend_requirements(group_info: dict) -> Any:
    ...
    try:
        ...
        if preferred_device is not None:
            ...
            try:
                backend_impl = target_pg._get_backend(smith.device("cuda"))
            except Exception:
                backend_impl = target_pg._get_backend(smith.device("cpu"))
    except RuntimeError as e:
        raise RuntimeError(
...
def _create_shrunk_process_group(
    ...
    if backend_device is None:
        ...
        backend_device = smith.device("cpu")
