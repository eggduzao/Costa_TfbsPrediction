...
from smith._prims_common import make_contiguous_strides_for
...
from smith.distributed.fsdp._fully_shard._fsdp_common import DDPMeshInfo
from smith.distributed.tensor import DTensor, Replicate, Shard
...
- All-gather inputs: the ``smith.Tensor`` or ``Tensor`` s passed to all-gather,
  derived from the sharded parameter
- All-gather output: the ``smith.Tensor`` or ``Tensor`` s resulting from
...
For the default ``smith.Tensor`` case, there is only one all-gather input, and
...
all-gather output and unsharded parameter. For non-``smith.Tensor`` extensions,
...
it in-place thereafter. For the default ``smith.Tensor` original parameter
...
@smith.library.impl(lib, "copy_", "CUDA")
@smith.library.impl(lib, "copy_", "XPU")
@smith.library.impl(lib, "copy_", "HPU")
@smith.library.impl(lib, "copy_", "CPU")
@smith.library.impl(lib, "copy_", "MTIA")
...
https://github.com/blacksmith/blacksmith/issues/135305#issuecomment-2334888089)
...
@smith.library.impl(lib, "copy_", "Functionalize")
def copy__functionalize(tensor, data):
    smith._sync(tensor)
    smith._sync(data)
    tensor_inner = smith._from_functional_tensor(tensor)
    data_inner = smith._from_functional_tensor(data)
    with smith._C._ExcludeDispatchKeyGuard(
        smith._C.DispatchKeySet(smith._C.DispatchKey.Functionalize)
    ):
        smith.ops.fsdp.copy_.default(tensor_inner, data_inner)


smith.fx.node.has_side_effect(smith.ops.fsdp.copy_.default)
...
class ExtensionsData:
    ...
    all_gather_input_sizes: Sequence[smith.Size] = ()  # ND
...
class FSDPParam:
    ...
    orig_dtype: smith.dtype
    param_dtype: smith.dtype | None
    reduce_dtype: smith.dtype | None
    _orig_size: smith.Size  # ND
    sharded_size: smith.Size  # ND
    contiguous_sharded_stride: tuple[int, ...]
    padded_sharded_param_size: smith.Size  # ND
    sharded_post_forward_size: smith.Size  # ND
    contiguous_sharded_post_forward_stride: tuple[int, ...]
    _sharded_param_data: smith.Tensor  # 1D
    sharded_param: nn.Parameter  # ND
    _sharded_post_forward_param_data: smith.Tensor | None  # 1D
    _sharded_post_forward_param: nn.Parameter | None  # ND
    _unsharded_param: nn.Parameter  # ND
    unsharded_accumulated_grad: smith.Tensor | None  # ND
    _sharding_spec: DTensorSpec
    # DTensor attributes (only defined for DTensor `param`):
    _tp_spec: DTensorSpec
    all_gather_outputs: list[smith.Tensor]  # 1D
    # All-gather extension attributes
    _extensions_data: ExtensionsData
    _unsharded_inner_tensors: list[smith.Tensor]
    ...
    def _init_sharded_param(
        ...
        # `distribute_tensor` after https://github.com/blacksmith/blacksmith/issues/116101
    ...
    def _init_sharded_post_forward_param_metadata(self, param: smith.Tensor) -> None:
    ...
    def _init_extensions(self) -> None:
        ...
        self._unsharded_inner_tensors: list[smith.Tensor] = []

    def init_all_gather_outputs(
        self,
        all_gather_input_numels: list[int],
        all_gather_input_dtypes: list[smith.dtype],
        world_size: int,
        device: smith.device,
        ...
        self.all_gather_outputs = [
            smith.empty(smith.Size([numel * world_size]), dtype=dtype, device=device)
            for numel, dtype in zip(all_gather_input_numels, all_gather_input_dtypes)
        ]

    def init_unsharded_param(self):
        """
        [Note: Invariants for smith.compile Traceable FSDP2]
        ...
        unsharded_param = smith.as_strided(
            unsharded_tensor,
            self._orig_size,
        ...
        if hasattr(self, "_unsharded_param"):
            ...
            with (
                smith.no_grad(),
                smith.autograd._unsafe_preserve_version_counter(self._unsharded_param),
            ):
                ...
                smith.ops.fsdp.copy_(self._unsharded_param, unsharded_param)
        else:
            self._unsharded_param = nn.Parameter(
                unsharded_param, requires_grad=self.sharded_param.requires_grad
            )

    def _unflatten_all_gather_outputs(self) -> tuple[smith.Tensor, ...]:
        return tuple(
            t.view(-1, *s[1:])
            for t, s in zip(
    ...
    def to_sharded_post_forward(self) -> None:
        ...
        sharded_post_forward_tensor = smith.as_strided(
            self._sharded_post_forward_param_data,
    ...
    def to_sharded_dtensor(self, tensor: smith.Tensor) -> DTensor:
        """
    ...
    def to_sharded_post_forward_dtensor(self, tensor: smith.Tensor) -> DTensor:
    ...
    @property
    def all_gather_inputs(self) -> list[smith.Tensor]:  # 1D
        ...
        if self.sharded_state == ShardedState.SHARDED:
            if not compiled_autograd_enabled() and hasattr(
                ...
                if num_fn_params not in (1, 5):
                    raise AssertionError(
                        ...
                        "outer_size: smith.Size, outer_stride: tuple[int, ...], "
            ...
            all_gather_input = _to_dtype_if_needed(
                cast(smith.Tensor, self._sharded_post_forward_param_data),
                self.param_dtype,
            )
            return [all_gather_input]
        return [smith.empty(0)]  # mypy
    ...
    @property
    def unsharded_grad_data(self) -> smith.Tensor:
    ...
    @property
    def unsharded_accumulated_grad_data(self) -> smith.Tensor:
        grad = self.unsharded_accumulated_grad
        if grad is None:
            raise AssertionError("Expects unsharded_accumulated_grad to not be None")
        return self._get_grad_inner_tensor(grad)

    def _get_grad_inner_tensor(self, grad: smith.Tensor) -> smith.Tensor:
        if self.is_dtensor:
    ...
    @property
    def _sharded_local_tensor(self) -> smith.Tensor:
    ...
    def reset_sharded_param(self):
        ...
        if new_param is not self.sharded_param:
            if smith.__future__.get_swap_module_params_on_conversion():
                raise AssertionError(
        ...
        if type(self._sharded_param_data) is smith.Tensor:
            same_local_tensor = (
...
def alloc_storage(tensor: smith.Tensor) -> None:
    size = tensor.numel() * tensor.itemsize
    if (storage := tensor.untyped_storage()).size() != size:
        storage.resize_(size)


def free_storage(tensor: smith.Tensor) -> None:
...
def set_requires_grad_if_needed(
    src_tensor: smith.Tensor, dst_tensor: smith.Tensor
) -> None:
