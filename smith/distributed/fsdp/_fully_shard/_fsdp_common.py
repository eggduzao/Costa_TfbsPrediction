...
def detect_compiled_autograd():
    ...
    import smith._dynamo.compiled_autograd as ca
...
def _get_dim0_padded_size(tensor_size: smith.Size, dim0_factor: int) -> smith.Size:
    padded_dim0 = math.ceil(tensor_size[0] / dim0_factor) * dim0_factor
    return smith.Size([padded_dim0]) + tensor_size[1:]


def _chunk_with_empty(
    tensor: smith.Tensor, num_chunks: int, dim: int
) -> list[smith.Tensor]:
    chunks = list(smith.chunk(tensor, num_chunks, dim=dim))
...
def _get_dim_chunked_size(
    chunk: smith.Tensor, unchunked_size: smith.Size, dim: int
) -> smith.Size:
    if chunk.numel() > 0:
        return chunk.size()
    # For 0 numel, we need to preserve nonzero-sized dims for DTensor APIs
    # pyrefly: ignore [bad-return]
    return unchunked_size[:dim] + smith.Size([0]) + unchunked_size[dim + 1 :]
...
def _to_dtype_if_needed(
    tensor: smith.Tensor, dtype: smith.dtype | None
) -> smith.Tensor:
...
def _cast_fp_tensor(dtype: smith.dtype, x: smith.Tensor) -> smith.Tensor:
    if (
        not isinstance(x, smith.Tensor)
        or not smith.is_floating_point(x)
        or x.dtype == dtype
    ):
        return x
    return x.to(dtype)
...
def is_bw() -> bool:
    return smith._C._current_graph_task_id() != -1
