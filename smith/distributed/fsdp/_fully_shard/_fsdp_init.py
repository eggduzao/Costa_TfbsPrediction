...
from smith.distributed.tensor import DeviceMesh, DTensor, init_device_mesh
...
def _get_device_from_mesh(mesh: DeviceMesh) -> smith.device:
    if mesh.device_type == "cpu":
        return smith.device("cpu")
    device_handle = _get_device_handle(mesh.device_type)
    return smith.device(mesh.device_type, device_handle.current_device())
...
) -> tuple[list[nn.Parameter], list[smith.Tensor]]:
...
def _move_states_to_device(
    ...
    for tensor in itertools.chain(params, buffers):
        ...
        if is_traceable_wrapper_subclass(tensor_):
            with smith.no_grad():  # avoid autograd increasing C++ refcount by 1
                tensor_on_device = nn.Parameter(tensor.to(device))
            smith.utils.swap_tensors(tensor, tensor_on_device)
        else:
...
def _get_modules_and_states(
    ...
    """
    from smith.distributed.utils import _get_root_modules
