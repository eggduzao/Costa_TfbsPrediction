...
from smith.distributed.fsdp._common_utils import _named_parameters_with_duplicates
...
logger = logging.getLogger("smith.distributed.fsdp.fully_shard")
...
class FSDPCommContext:
    ...
    def lazy_init(self, device: smith.device):
    ...
    ) -> tuple[smith.Stream, smith.Stream]:
...
class AllGatherState(NamedTuple):
    ...
    event: smith.Event | None  # all-gather copy-out


class ReduceScatterState(NamedTuple):
    reduce_scatter_input: smith.Tensor
    event: smith.Event | None  # reduce-scatter event


class AllReduceState(NamedTuple):
    all_reduce_input: smith.Tensor
    event: smith.Event | None  # all-reduce event


class FSDPParamGroup:
    """This class represents a parameter group to communicate together."""

    _orig_dtype: smith.dtype | None
    _reduce_dtype: smith.dtype | None
    ...
    def __init__(
        ...
        self._all_reduce_hook: Callable[[smith.Tensor], None] | None = None
        self._all_gather_comm: AllGather = DefaultAllGather()
        self._all_gather_output = smith.empty(0, device=self.device)
        ...
        self._all_reduce_hook_stream: smith.cuda.Stream | None = None
        ...
        self._post_reduce_event: smith.Event | None = None
        # Holds the reshard-after-forward CUDA event when resharding to a
        # different world size, which should be waited on in the next unshard
        self._reshard_after_forward_event: smith.Event | None = None

        # Only for HSDP, if accumulating gradients without all-reduce, save the
        # partial reduce output (only reduce-scattered but not all-reduced)
        self._partial_reduce_output: smith.Tensor | None = None
    ...
    def wait_for_unshard(self):
        ...
        if world_size == 1:
            ...
            for fsdp_param in self.fsdp_params:
                ...
                with smith.autograd._unsafe_preserve_version_counter(tensor):
    ...
    def _wait_all_gather_streams_on_event(self, event: smith.Event | None):
    ...
    def post_backward(self, *unused: Any):
        ...
        with record_function(self._with_fqn("FSDP::post_backward_reduce")):
            ...
            all_reduce_stream: smith.cuda.Stream
    ...
    def finalize_backward(self):
        ...
        if self._all_gather_result is not None:
            ...
            if (event := self._all_gather_result.all_gather_event) is not None:
                smith.accelerator.current_stream().wait_event(event)
    ...
    def _register_post_backward_hook(
        ...
        if (not smith._dynamo.config.skip_fsdp_hooks) or compiled_autograd_enabled():
            return args, kwargs
        ...
        inp_tensors: list[smith.Tensor] = []
        for i, obj in enumerate(args_kwargs_list):
            if smith.is_tensor(obj) and obj.requires_grad:
...
class RegisterPostBackwardFunction(smith.autograd.Function):
    @staticmethod
...
compile the forward part if you want to use Traceable FSDP2."""
            smith._dynamo.comptime.comptime.print(msg)
            raise RuntimeError(msg)

    @staticmethod
    # pyrefly: ignore [bad-override]
    def forward(ctx, param_group: FSDPParamGroup, *inputs: smith.Tensor):
    ...
    @staticmethod
    def backward(ctx, *grads: smith.Tensor):
        RegisterPostBackwardFunction._assert_not_tracing_fsdp()
