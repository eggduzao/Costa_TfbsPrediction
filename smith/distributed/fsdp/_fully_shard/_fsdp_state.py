...
from smith.autograd.graph import _MultiHandle
from smith.distributed._composable_state import (
    _get_module_state,
    _insert_module_state,
    _State,
)
from smith.distributed.device_mesh import _get_device_handle
from smith.distributed.utils import _apply_to_tensors, _to_kwargs
...
class FSDPStateContext(Generic[_StateType]):
    ...
    def __init__(self) -> None:
        ...
        self.post_optim_event: smith.Event | None = None


def disable_if_config_true(func):
    @functools.wraps(func)
    def fsdp_hook_wrapper(*args, **kwargs):
        if smith._dynamo.config.skip_fsdp_hooks:
            return smith._dynamo.disable(
                func,
                recursive=True,
                reason="skipping FSDP hooks since smith._dynamo.config.skip_fsdp_hooks is set",
            )(*args, **kwargs)
        else:
...
class FSDPState(_State):
    ...
    def _root_pre_forward(
        ...
        with smith.profiler.record_function("FSDP::root_pre_forward"):
            ...
            if self._device.type in [
                ...
                with smith.profiler.record_function("FSDP::inputs_to_device"):
    ...
    def _pre_forward(
        ...
        if self._mp_policy.cast_forward_inputs and self._mp_policy.param_dtype:
            with smith.profiler.record_function("FSDP::cast_forward_inputs"):
    ...
    def _post_forward(self, module: nn.Module, input: Any, output: Any) -> Any:
        ...
        if self._mp_policy.output_dtype is not None:
            with smith.profiler.record_function("FSDP::cast_forward_outputs"):
    ...
    def _pre_backward(self, grad: smith.Tensor) -> smith.Tensor:
    ...
    def _root_post_backward_final_callback(self) -> None:
        ...
        with smith.profiler.record_function("FSDP::root_post_backward_callback"):
            for state in self._state_ctx.all_states:
    ...
    def _register_pre_backward_hook(self, output: Any) -> Any:
        ...
        for t in flat_outputs:
            if smith.is_tensor(t) and t.requires_grad:
