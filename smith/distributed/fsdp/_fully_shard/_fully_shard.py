...
from smith.distributed._composable import contract
...
def fully_shard(
    ...
    *,
    ...
    parameters on ``module`` (e.g. :class:`smith.Tensor` if originally
    :class:`smith.Tensor`). A module
    `forward pre-hook <https://blacksmith.org/docs/main/generated/smith.nn.Module.html#smith.nn.Module.register_forward_pre_hook>`_
    on ``module`` all-gathers the parameters, and a module
    `forward hook <https://blacksmith.org/docs/main/generated/smith.nn.Module.html#smith.nn.Module.register_forward_hook>`_
    ...
    Args:
        ...
        reshard_after_forward (Optional[Union[bool, int]]): This controls the parameter
            ...
            - If an ``int``, then this represents the world size to reshard to
              ...
              choice may be the intra-node size (e.g. ``smith.cuda.device_count()``).
    ...
    """
    smith._C._log_api_usage_once("smith.distributed.fsdp.fully_shard")
...
class FSDPModule:
    ...
    def set_all_reduce_hook(
        self,
        hook: Callable[[smith.Tensor], None],
        *,
        stream: smith.cuda.Stream | None = None,
    ):
        """
        Args:
            hook (Callable[[smith.Tensor], None]): User-defined all-reduce hook
                with expected signature ``hook(reduce_output: smith.Tensor) -> None``
                where ``reduce_output`` is the reduce-scatter output if only
                using FSDP or the all-reduce output if using native HSDP.
            stream (Optional[smith.cuda.Stream]): Stream to run the all-reduce
    ...
    def set_post_optim_event(self, event: smith.Event) -> None:
        """
        ...
        Args:
            event (smith.Event): Event recorded after the optimizer step
...
def share_comm_ctx(modules: list[FSDPModule]) -> None:
    ...
    Example usage:
        from smith.distributed.fsdp import share_comm_ctx
