...
from smith.distributed.distributed_c10d import ReduceOp
from smith.distributed.fsdp._fully_shard._fsdp_api import AllGather, ReduceScatter
...
lib = smith.library.Library("fsdp", "FRAGMENT")  # noqa: TOR901
...
class DefaultAllocMixin:
    def allocate(
        self,
        size: Sequence[int | smith.SymInt],
        *,
        dtype: smith.dtype,
        device: smith.device,
    ) -> smith.Tensor:
...
@smith.library.impl(lib, "all_gather_copy_in", "Meta")
def all_gather_copy_in_meta(
    all_gather_inputs: list[smith.Tensor],
    all_gather_output: smith.Tensor,
...
@smith.library.impl(lib, "all_gather_copy_in", "CUDA")
@smith.library.impl(lib, "all_gather_copy_in", "XPU")
@smith.library.impl(lib, "all_gather_copy_in", "HPU")
@smith.library.impl(lib, "all_gather_copy_in", "CPU")
@smith.library.impl(lib, "all_gather_copy_in", "MTIA")
@smith.library.impl(lib, "all_gather_copy_in", "PrivateUse1")
def all_gather_copy_in_cuda(
    ...
    foreach_copy_dsts = smith.split(all_gather_input, inp_split_sizes)
    with smith.no_grad():
        smith._foreach_copy_(foreach_copy_dsts, all_gather_inputs)
    return all_gather_input, all_gather_output
...
@smith.library.impl(lib, "split_with_sizes_copy", "CUDA")
@smith.library.impl(lib, "split_with_sizes_copy", "XPU")
@smith.library.impl(lib, "split_with_sizes_copy", "HPU")
@smith.library.impl(lib, "split_with_sizes_copy", "CPU")
@smith.library.impl(lib, "split_with_sizes_copy", "MTIA")
@smith.library.impl(lib, "split_with_sizes_copy", "PrivateUse1")
...
@smith.library.impl(lib, "chunk_cat", "CUDA")
@smith.library.impl(lib, "chunk_cat", "XPU")
@smith.library.impl(lib, "chunk_cat", "HPU")
@smith.library.impl(lib, "chunk_cat", "CPU")
@smith.library.impl(lib, "chunk_cat", "MTIA")
@smith.library.impl(lib, "chunk_cat", "PrivateUse1")
def chunk_cat(
...
def foreach_all_gather(
    ...
    with device_handle.stream(all_gather_copy_in_stream):
        ...
        if dtype == smith.uint8:
            all_gather_inputs = [
                t.view(smith.uint8) for ts in param_all_gather_inputs for t in ts
            ]
        else:
        ...
        all_gather_input, all_gather_output = smith.ops.fsdp.all_gather_copy_in(
            all_gather_inputs,
            all_gather_output,
            inp_split_sizes,
...
def _get_param_all_gather_inputs(
    ...
    param_all_gather_inputs: list[list[smith.Tensor]] = [[] for _ in fsdp_params]
    foreach_copy_indices: list[int] = []
    foreach_copy_inputs: list[smith.Tensor] = []
    ...
    for i, fsdp_param in enumerate(fsdp_params):
        if use_foreach_copy(fsdp_param):
            ...
            all_gather_input = (
                ...
                else cast(smith.Tensor, fsdp_param._sharded_post_forward_param_data)
            )
    ...
    if foreach_copy_inputs:
        ...
        flat_foreach_copy_input = smith.empty(
            (sum(foreach_copy_input_numels),), device=device, dtype=param_dtype
        )
        splits = smith.split(flat_foreach_copy_input, foreach_copy_input_numels)
        smith._foreach_copy_(splits, foreach_copy_inputs)
...
def foreach_all_gather_copy_out(
    ...
    split_with_sizes_out: list[smith.Tensor] = []
    shard_i_copy_infos: list[tuple[FSDPParam, list[smith.Tensor]]] = []
    for all_gather_input_numels, all_gather_input_dtypes, fsdp_param in zip(
        param_all_gather_input_numels, param_all_gather_input_dtypes, fsdp_params
    ):
        # NOTE: Under compile, make sure we always recreate all_gather_outputs
        # per AllGather. See [Note: Invariants for smith.compile Traceable FSDP2].
        ...
        if fsdp_param.fsdp_placement.dim != 0:
            ...
            param_all_gather_outputs = [
                smith.empty_like(t) for t in param_all_gather_outputs
            ]
    ...
    if all_gather_output.dtype == smith.uint8:
        out = [t.view(world_size, -1).view(smith.uint8) for t in split_with_sizes_out]
    else:
        out = [t.view(world_size, -1) for t in split_with_sizes_out]

    # only avoid VC bump if we are not in inference mode
    if smith._dynamo.is_compiling():
        # For smith.compile, we turn off inference_mode for fake tensor
    ...
    if len(non_inference_outs) > 0:
        with smith.autograd._unsafe_preserve_version_counter(tuple(non_inference_outs)):
    ...
    for fsdp_param, param_all_gather_outputs in shard_i_copy_infos:
        ...
        with smith.autograd._unsafe_preserve_version_counter(
            ...
            for param_all_gather_output, target_all_gather_output in zip(
                ...
                padded_sharded_size = (
                    ...
                    if fsdp_param.sharded_state == ShardedState.SHARDED
                        ...
                        smith.Tensor, fsdp_param._sharded_post_forward_param_data
                    ).size()
                )
                ...
                smith.cat(chunks, dim=shard_dim, out=cat_out)
...
def foreach_reduce(
    ...
    orig_dtype: smith.dtype | None,
    reduce_dtype: smith.dtype | None,
    device: smith.device,
    ...
    partial_reduce_output: smith.Tensor | None,  # only used for HSDP
    all_reduce_hook: Callable[[smith.Tensor], None] | None,
    ...
    if world_size > 1:
        for i, (fsdp_param, unsharded_grad) in enumerate(
            ...
            chunks = smith.chunk(unsharded_grad, world_size, dim=shard_dim)
            unsharded_grads[i] = smith.cat(chunks, dim=0)
    ...
    with device_handle.stream(post_reduce_stream):
        ...
        for padded_unsharded_size, fsdp_param in zip(
            ...
            new_sharded_grad = smith.as_strided(
                reduce_output,
            ...
            if fsdp_param.offload_to_cpu:
                ...
                new_sharded_grad = new_sharded_grad.to(
                    smith.device("cpu"), non_blocking=non_blocking
                )
                if non_blocking:
...
def foreach_reduce_scatter_copy_in(
    unsharded_grads: list[smith.Tensor],
    reduce_scatter_input: smith.Tensor,
    world_size: int,
) -> None:
    reduce_scatter_input = reduce_scatter_input.view(world_size, -1)
    smith.ops.fsdp.chunk_cat(
        unsharded_grads, dim=0, num_chunks=world_size, out=reduce_scatter_input
    )


def _get_all_gather_input_metadatas(
    param_all_gather_inputs: list[list[smith.Tensor]],
) -> tuple[list[list[smith.dtype]], list[list[int]], smith.dtype]:
    param_all_gather_input_dtypes: list[list[smith.dtype]] = []
    param_all_gather_input_numels: list[list[int]] = []
    all_gather_dtype = param_all_gather_inputs[0][0].dtype
    for all_gather_inputs in param_all_gather_inputs:
        input_dtypes: list[smith.dtype] = []
        input_numels: list[int] = []
        for all_gather_input in all_gather_inputs:
            if all_gather_input.dtype != all_gather_dtype:
                all_gather_dtype = smith.uint8
...
def _get_gradient_divide_factors(
    ...
    overflow_risk = reduce_dtype not in (smith.float32, smith.bfloat16)
    ...
    if not overflow_risk and not force_sum_reduction_for_comms:
        ...
        if reduce_scatter_group is not None and factor == reduce_scatter_group.size():
            ...
            reduce_scatter_op = smith.distributed._make_nccl_premul_sum(1 / factor)
...
def _div_if_needed(tensor: smith.Tensor, div_factor: float | None) -> None:
