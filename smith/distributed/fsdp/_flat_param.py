...
from smith.nn.parameter import _ParameterMeta  # type: ignore[attr-defined]
from smith.testing._internal.distributed.fake_pg import FakeProcessGroup
...
class FlatParamShardMetadata(NamedTuple):
    ...
    Attributes:
        ...
        param_shapes (Tuple[smith.Size, ...]): Parameter shapes of this rank's
            shard of the parameters; see :class:`FlatParameter`.
        param_strides (Tuple[smith.Size, ...]): Parameter strides of this rank's
            shard of the parameters; see :class:`FlatParameter`.
...
class _FlatParameterMeta(_ParameterMeta):
    ...
    def __instancecheck__(self, instance):
        ...
        return isinstance(instance, smith.Tensor) and getattr(
...
class FlatParameter(nn.Parameter, metaclass=_FlatParameterMeta):
    ...
    Attributes:
        _unpadded_unsharded_size (smith.Size): Unsharded flat parameter's size
        ...
        _padded_unsharded_size (smith.Size): Unsharded flat parameter's size
            ...
            the all-gather.
        _sharded_size (smith.Size): Sharded flat parameter's size with padding.
        ...
        _shapes (Tuple[smith.Size, ...]): Each parameter's original shape.
        _strides (Tuple[smith.Size, ...]): Each parameter's original stride.
        ...
        _numels_with_padding (Tuple[int, ...]): Each parameter's numel
            ...
            into the flat parameter via ``smith.split()``. This may have length
    ...
    _unpadded_unsharded_size: smith.Size
    _padded_unsharded_size: smith.Size
    _sharded_size: smith.Size
    _num_params: int
    _param_infos: tuple[ParamInfo, ...]
    _shapes: tuple[smith.Size, ...]
...
class FlatParamHandle:
    ...
    Args:
        ...
        device (smith.device): The compute and communication device, which
        ...
        mp_param_dtype (Optional[smith.dtype]): Parameter mixed precision
            setting passed to the FSDP constructor.
        mp_reduce_dtype (Optional[smith.dtype]): Gradient reduction mixed
    ...
    def __init__(
        ...
        offload_params: bool,
        mp_param_dtype: smith.dtype | None,
        mp_reduce_dtype: smith.dtype | None,
    ...
    def flatten_tensors(
        ...
        if aligned_numel > 0:
            ...
            for tensor in tensors:
                ...
                if numel_to_pad > 0 and numel_to_pad < aligned_numel:
                    ...
                    total_numel += numel_to_pad
                flat_tensors.append(
                    smith.flatten(_detach_if_needed(tensor))
        ...
        return smith.cat(flat_tensors, dim=0)
    ...
    def shard(self):
        ...
        if not self.uses_sharded_strategy:
            ...
            if not smith.distributed._functional_collectives.is_smithdynamo_compiling():
    ...
    def _get_unpadded_shard(
        ...
        """
        chunks = (
            smith.flatten(tensor).chunk(world_size)
            if _is_truly_contiguous(tensor)
    ...
    @staticmethod
    def _get_sharded_size(tensor: Tensor, rank: int, world_size: int) -> smith.Size:
        """
        ...
        return smith.Size([unpadded_sharded_size[0] + numel_to_pad])
    ...
    def init_flat_param_attributes(self) -> None:
        ...
        if self._offload_params:
            ...
            flat_param._cpu_grad = smith.zeros_like(
        ...
        if self._uses_param_mixed_precision:
            ...
            # computation.
            flat_param._mp_shard = smith.empty_like(
        ...
        if self.uses_sharded_strategy:
            ...
            flat_param._full_param_padded = smith.empty(
            ...
            if self._uses_param_mixed_precision:
                ...
                flat_param._full_prec_full_param_padded = smith.empty(
                    padded_unsharded_numel,
                    device=self.device,
    ...
    def _get_padded_unsharded_flat_param(self) -> smith.Tensor:
        """
    ...
    def _use_unsharded_flat_param(
        self,
        padded_unsharded_flat_param: smith.Tensor,
    ) -> None:
        """
    ...
    def unshard_grad(self):
        ...
        num_grad_none = smith.zeros(1, dtype=smith.int32, device=self.device)
        ...
        if flat_param.grad is None:
            ...
            flat_param._saved_grad_shard = None  # type: ignore[assignment]
            sharded_grad = smith.zeros(flat_param._sharded_size, device=self.device)  # type: ignore[attr-defined]
        else:
        ...
        padded_unsharded_grad = smith.empty(
    ...
    def to_cpu(self):
        ...
        self.flat_param_to(smith.device("cpu"))
    ...
    def _get_unflat_views_unaligned(
        ...
        views = (
            ...
            for (subtensor, shape, stride, contiguous, param_extension) in zip(
                smith.split(tensor, flat_param._numels, dim=0),
                flat_param._shapes,
                flat_param._strides,
    ...
    def _get_unflat_views_aligned(
        ...
        if tensor is None:
            tensor = flat_param
        splits: list[Tensor] = smith.split(
    ...
    def _use_unsharded_views(self, as_params: bool) -> None:
        ...
        Note:
            ...
            annotated with `@smith.no_grad()`
            `@smith.enable_grad()` ensures non-empty `view.grad_fn`
        ...
        for i, (view, (param_name, module, _)) in enumerate(
            ...
            if (
                ...
                if param.grad is None:
                    param.grad = smith.empty_like(param)
    ...
    def _use_sharded_views(self) -> None:
        ...
        size_0_empty_tensor = smith.empty(
            0,
    ...
    def _writeback_orig_params(self) -> bool:
        ...
        for i, (
            ...
            if needs_param_writeback:
                expected_shape = smith.Size([numel_in_shard])
            ...
            if param.grad is None and flat_param.grad is not None:
                ...
                if needs_grad_writeback:
                    if flat_param_grad is None:
                        flat_param_grad = smith.zeros_like(flat_param)
                    expected_shape = smith.Size([numel_in_shard])
                    src = (
                        param.grad
                        if self.uses_sharded_strategy
    ...
    def _check_on_cpu(self, tensor: Tensor):
        _p_assert(
            tensor.device == smith.device("cpu"),
...
def _convert_to_params(
    tensors: list[smith.Tensor | nn.Parameter],
) -> list[nn.Parameter]:
    return [t if isinstance(t, nn.Parameter) else nn.Parameter(t) for t in tensors]


def _is_truly_contiguous(x: Tensor) -> bool:
    # Special case: Blacksmith thinks that 1x1 channels_last convolution weights are
...
def _get_aligned_numel(unsharded_dtype: smith.dtype):
    # NOTE: This alignment constraint comes from SmithInductor.
    ALIGNMENT = 16  # bytes
...
def _construct_padding_tensor(
    padding_numel: int, dtype: smith.dtype, requires_grad: bool, device: smith.device
):
...
def _same_storage_size(a: smith.Tensor, b: int):
