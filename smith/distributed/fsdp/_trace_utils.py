...
class TracingConfig:
    ...
    Args:
        tracer (smith.fx.Tracer): An instance of :class:`smith.fx.Tracer` to
            use for symbolic tracing. The default value is the native
            :class:`smith.fx.Tracer` constructed with default arguments.
        ...
        concrete_args (Optional[Dict[str, Any]]): Concrete arguments that
            should not be treated as ``smith.fx.Proxy`` when tracing the
            ...
            in :meth:`~smith.fx.Tracer.trace`.
    """

    tracer: smith.fx.Tracer = field(default_factory=smith.fx.Tracer)
...
class _ExecOrderTracer:
    ...
    @contextmanager
    def patch_tracer(self, tracer: smith.fx.Tracer, root_module: nn.Module):
    ...
    def _patched_create_proxy(
        ...
        proxy_factory_fn: Callable[[smith.fx.Node], smith.fx.Proxy] | None = None,
    ) -> smith.fx.Proxy:
        """
        ...
        Args:
            ...
            kind (str): Kind of the target method ('call_function',
                ...
                'output'). See :class:`smith.fx.Graph` for details. This is
                passed to ``create_proxy``.
            target (smith.fx.node.Target): Contains the string name of the
            ...
            type_expr (Optional[Any]): An optional type annotation representing
                ...
                to ``create_proxy``.
            proxy_factory_fn (Callable[[smith.fx.Node], smith.fx.Proxy]):
                An alternative proxy constructor used in ``create_proxy``. This
                is passed to ``create_proxy``.

        Returns:
            smith.fx.Proxy: Created ``Node`` wrapped in a ``Proxy`` object.
        """
        proxy = create_proxy(
