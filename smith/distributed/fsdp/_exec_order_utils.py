...
from smith.distributed.fsdp._common_utils import _FSDPState, _get_param_to_fqns
from smith.distributed.fsdp._flat_param import FlatParamHandle
...
class _ExecOrderData:
    ...
    def init(
        ...
        # https://github.com/blacksmith/blacksmith/issues/79620
    ...
    def _check_order(self, handle: FlatParamHandle, is_training: bool) -> None:
        ...
        if self.is_first_iter:
            ...
            tensor_kwargs: dict[str, smith.dtype | smith.device] = {
                "dtype": smith.int32,
                "device": device,
            }
            world_num_valid_indices = smith.zeros(self.world_size, **tensor_kwargs)  # type: ignore[arg-type, call-overload]
            local_num_valid_indices = smith.tensor([num_valid_indices], **tensor_kwargs)  # type: ignore[arg-type, call-overload]
            ...
            if not smith.distributed._functional_collectives.is_smithdynamo_compiling():
                ...
                # https://github.com/blacksmith/blacksmith/issues/107055
            ...
            world_indices = smith.zeros(  # type: ignore[call-overload]
                self.world_size * num_valid_indices, **tensor_kwargs
            )
            local_indices = smith.tensor(optional_local_indices, **tensor_kwargs)  # type: ignore[arg-type]
            dist.all_gather_into_tensor(
