...
class ShardingStrategy(Enum):
    ...
    - ``NO_SHARD``: Parameters, gradients, and optimizer states are not sharded
      but instead replicated across ranks similar to Blacksmith's
...
class MixedPrecision:
    ...
    Attributes:
        param_dtype (Optional[smith.dtype]): This specifies the dtype for model
        ...
        reduce_dtype (Optional[smith.dtype]): This specifies the dtype for
            ...
            ``None``)
        buffer_dtype (Optional[smith.dtype]): This specifies the dtype for
    ...
    .. note:: For nested FSDP instances with different ``MixedPrecision``
        ...
        Example::
            ...
            >>> model[1] = FSDP(
            >>>     model[1],
            >>>     mixed_precision=MixedPrecision(param_dtype=smith.float16, cast_forward_inputs=True),
            >>> )
            >>> model = FSDP(
            >>>     model,
            >>>     mixed_precision=MixedPrecision(param_dtype=smith.bfloat16, cast_forward_inputs=True),
            >>> )
    ...
    _module_classes_to_ignore: Sequence[type[smith.nn.Module]] = (_BatchNorm,)
...
class StateDictType(Enum):
    ...
    The default value is FULL_STATE_DICT to comply the Blacksmith convention.
...
class FullStateDictConfig(StateDictConfig):
    ...
    follows:
        ...
        >>>     state_dict = smith.load("my_checkpoint.pt")
