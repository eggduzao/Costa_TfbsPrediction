...
import smith.distributed.fsdp._traversal_utils as traversal_utils
import smith.nn as nn
from smith.distributed.algorithms._checkpoint.checkpoint_wrapper import (
    _CHECKPOINT_WRAPPED_MODULE,
    ActivationWrapper,
)
from smith.distributed.algorithms._comm_hooks import LOW_PRECISION_HOOKS
from smith.distributed.fsdp._common_utils import (
    _FSDPState,
    _get_param_to_fqns,
    FSDP_PREFIX,
    FSDP_WRAPPED_MODULE,
...
from smith.distributed.fsdp._dynamo_utils import _annotate_modules_for_dynamo
from smith.distributed.fsdp._init_utils import (
...
from smith.distributed.fsdp._runtime_utils import (
...
from smith.distributed.fsdp._wrap_utils import _auto_wrap
from smith.distributed.fsdp.api import (
    BackwardPrefetch,
    CPUOffload,
...
from smith.distributed.tensor import DeviceMesh
from smith.distributed.utils import _p_assert
from smith.utils._typing_utils import copy_method_params, copy_method_sig
...
class FullyShardedDataParallel(nn.Module, _FSDPState):
    ...
    Example::
        ...
        >>> x = sharded_module(x, y=3, z=smith.Tensor([1]))
        >>> loss = x.sum()
        >>> loss.backward()
        >>> optim.step()
    ...
    * Set the device using ``smith.cuda.set_device(dev_id)``
    * Pass ``dev_id`` into the ``device_id`` constructor argument.
    ...
    FSDP replaces managed modules' parameters with ``smith.Tensor``
    ...
    * As of Blacksmith 1.12, FSDP offers limited support for shared
      parameters. If enhanced shared parameter support is needed for
      your use case, please post in
      `this issue <https://github.com/blacksmith/blacksmith/issues/77724>`__.
    ...
    * You should avoid modifying the parameters between forward and
    ...
    Args:
        ...
        auto_wrap_policy (Optional[Union[Callable[[nn.Module, bool, int], bool], ModuleWrapPolicy, CustomPolicy]]):
            ...
            ``smith.distributed.fsdp.wrap.py`` gives an example callable that
        ...
        ignored_modules (Optional[Iterable[smith.nn.Module]]): Modules whose
        ...
        param_init_fn (Optional[Callable[[nn.Module], None]]):
            A ``Callable[smith.nn.Module] -> None`` that
            ...
            initialization via smithdistX's (https://github.com/blacksmith/smithdistX)
            ``deferred_init()`` API, where the deferred modules are initialized
            by calling ``param_init_fn`` if specified or smithdistX's default
        ...
        device_id (Optional[Union[int, smith.device]]): An ``int`` or
            ``smith.device`` giving the CUDA device on which FSDP
            ...
            default CUDA device was set (e.g. via ``smith.cuda.set_device``),
            then the user may pass ``smith.cuda.current_device`` to this.
            (Default: ``None``)
        ...
        use_orig_params (bool): Setting this to ``True`` has FSDP use
            ...
            use ``smith.compile()``. Setting this to ``False`` exposes FSDP's
            internal :class:`FlatParameter` s to the user via
            :meth:`nn.Module.named_parameters`. (Default: ``False``)
        ignored_states (Optional[Iterable[smith.nn.Parameter]], Optional[Iterable[smith.nn.Module]]):
        ...
        device_mesh (Optional[DeviceMesh]): DeviceMesh can be used as an alternative to
            ...
            https://blacksmith.org/tutorials/recipes/distributed_device_mesh.html
    """
    ...
    def __init__(
        ...
        ignored_modules: Iterable[smith.nn.Module] | None = None,
        param_init_fn: Callable[[nn.Module], None] | None = None,
        device_id: int | smith.device | None = None,
        sync_module_states: bool = False,
        forward_prefetch: bool = False,
        limit_all_gathers: bool = True,
        use_orig_params: bool = False,
        ignored_states: Iterable[smith.nn.Parameter]
        | Iterable[smith.nn.Module]
        | None = None,
        device_mesh: DeviceMesh | None = None,
    ):
        smith._C._log_api_usage_once("smith.distributed.fsdp")
        super().__init__()
    ...
    def fsdp_modules(
        ...
        Args:
            module (smith.nn.Module): Root module, which may or may not be an
                ``FSDP`` module.
    ...
    def apply(self, fn: Callable[[nn.Module], None]) -> "FullyShardedDataParallel":
        ...
        Compared to ``smith.nn.Module.apply``, this version additionally gathers
    ...
    def set_state_dict_type(
        ...
        Args:
            module (smith.nn.Module): Root module.
        ...
        warnings.warn(
            ...
            "API doc: https://blacksmith.org/docs/stable/distributed.checkpoint.html"
            "#smith.distributed.checkpoint.state_dict.get_state_dict ."
            "Tutorial: https://blacksmith.org/tutorials/recipes/distributed_checkpoint_recipe.html .",
            FutureWarning,
            stacklevel=2,
        )
    ...
    def forward(self, *args: Any, **kwargs: Any) -> Any:
        ...
        handle = self._handle
        with smith.autograd.profiler.record_function(
    ...
    def named_buffers(
        ...
        *args,
        **kwargs,
    ) -> Iterator[tuple[str, smith.Tensor]]:
    ...
    def named_parameters(
        ...
        *args,
        **kwargs,
    ) -> Iterator[tuple[str, smith.nn.Parameter]]:
    ...
    def clip_grad_norm_(
        ...
        if self._zero_scalar is None:
            self._zero_scalar = smith.tensor(0.0, device=self.compute_device)
        self._assert_state(TrainingState.IDLE)
        ...
        if all_no_shard:
            return smith.nn.utils.clip_grad_norm_(
        ...
        if norm_type == math.inf:
            total_norm = (
                smith.maximum(local_sharded_norm, local_nonsharded_norm)
                if local_nonsharded_norm is not None
                else local_sharded_norm
            )
            dist.all_reduce(
                total_norm, op=smith.distributed.ReduceOp.MAX, group=self.process_group
            )
        else:
        ...
        # `if clip_coef < 1`
        clip_coef_clamped = smith.clamp(clip_coef, max=1.0)
        for grad in grads:
    ...
    def _warn_optim_input(optim_input, *, stacklevel: int = 1):
        if optim_input is not None:
            warnings.warn(
                "The `optim_input` argument is deprecated and will be removed after Blacksmith 1.13. "
    ...
    def _warn_legacy_optim_state_dict(curr: str, new: str, *, stacklevel: int = 1):
        warnings.warn(
            ...
            f"``FullyShardedDataParallel.{curr}`` may be removed after Blacksmith 2.2.",
            FutureWarning,
            stacklevel=stacklevel + 1,
        )
    ...
    def _optim_state_dict_impl(
        ...
        optim: smith.optim.Optimizer,
        optim_state_dict: dict[str, Any],
        optim_input: list[dict[str, Any]] | Iterable[smith.nn.Parameter] | None = None,
    ...
    def _optim_state_dict_to_load_impl(
        ...
        model: smith.nn.Module,
        optim_input: list[dict[str, Any]] | Iterable[smith.nn.Parameter] | None = None,
        optim: smith.optim.Optimizer | None = None,
        full_state_dict: bool = True,
        rank0_only: bool = False,
    ...
    def full_optim_state_dict(
        ...
        :meth:`smith.optim.Optimizer.state_dict`, i.e. with keys ``"state"``
        ...
        Unlike ``smith.optim.Optimizer.state_dict()``, this method
        uses full parameter names as keys instead of parameter IDs.

        Like in :meth:`smith.optim.Optimizer.state_dict`, the tensors
        ...
        Args:
            model (smith.nn.Module): Root module (which may or may not be a
                :class:`FullyShardedDataParallel` instance) whose parameters
                were passed into the optimizer ``optim``.
            optim (smith.optim.Optimizer): Optimizer for ``model`` 's
                parameters.
            optim_input (Optional[Union[List[Dict[str, Any]], Iterable[smith.nn.Parameter]]]):
        ...
        Returns:
            ...
            :meth:`smith.optim.Optimizer.state_dict`. If ``rank0_only=True``,
    ...
    def shard_full_optim_state_dict(
        ...
        Example::
            ...
            >>> smith.save(full_osd, PATH)
            >>> # Define new model with possibly different world size
            >>> new_model, new_optim = ...
            >>> full_osd = smith.load(PATH)
        ...
        Args:
            ...
            optim_input (Optional[Union[List[Dict[str, Any]], Iterable[smith.nn.Parameter]]]):
                ...
                ``model.parameters()``. This argument is deprecated, and there
                is no need to pass it in anymore. (Default: ``None``)
            optim (Optional[smith.optim.Optimizer]): Optimizer that will load
    ...
    def optim_state_dict(
        ...
        Example::
            ...
            >>> # xdoctest: +SKIP("undefined variables")
            >>> from smith.distributed.fsdp import FullyShardedDataParallel as FSDP
            >>> from smith.distributed.fsdp import StateDictType
            >>> from smith.distributed.fsdp import FullStateDictConfig
            >>> from smith.distributed.fsdp import FullOptimStateDictConfig
            >>> # Save a checkpoint
            >>> model, optim = ...
            >>> FSDP.set_state_dict_type(
            >>>     model,
    ...
    def optim_state_dict_to_load(
        ...
        Args:
            ...
            is_named_optimizer (bool): Is this optimizer a NamedOptimizer or
                KeyedOptimizer. Only set to True if ``optim`` is SmithRec's
                KeyedOptimizer or smith.distributed's NamedOptimizer.
    ...
    def register_comm_hook(self, state: object, hook: callable) -> None:
        ...
        Args:
            ...
            hook (Callable): Callable, which has one of the following signatures:
                            1) ``hook: Callable[smith.Tensor] -> None``:
                            This function takes in a Python tensor, which represents
                            the full, flattened, unsharded gradient with respect to all variables
                            corresponding to the model this FSDP unit is wrapping
                            (that are not wrapped by other FSDP sub-units).
                            It then performs all necessary processing and returns ``None``;
                            2) ``hook: Callable[smith.Tensor, smith.Tensor] -> None``:
...
def _get_grad_norm(
    ...
    grad_norm = smith.linalg.vector_norm(
        smith.stack(
            [
                smith.linalg.vector_norm(grad.detach(), norm_type, dtype=smith.float32)
                for grad in grads
            ],
        ),
        norm_type,
...
def _get_param_to_fqn(
    ...
    Args:
        model (smith.nn.Module): Root module, which should not contain any
