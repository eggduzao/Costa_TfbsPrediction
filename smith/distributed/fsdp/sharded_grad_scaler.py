...
from smith.amp.grad_scaler import _MultiDeviceReplicator, GradScaler, OptState
from smith.distributed.distributed_c10d import ProcessGroup
...
def _is_supported_device(tensor: smith.Tensor) -> bool:
...
class ShardedGradScaler(GradScaler):
    ...
    * Supports Blacksmith DDP and FSDP implementations
    ...
    * Sync inf/nan for scaled gradient tensors on any smith.device (where tensors are placed) across
    nodes
    ...
    Args:
        ...
        enabled (bool, optional):  If ``False``, disables gradient scaling. :meth:`step` simply
            ...
            Default: ``True``
        process_group (ProcessGroup, optional, default=smith.distributed.group.WORLD):
    ...
    def scale(
        self, outputs: smith.Tensor | Iterable[smith.Tensor]
    ) -> smith.Tensor | Iterable[smith.Tensor]:
        ...
        def apply_scale(val: smith.Tensor | Iterable[smith.Tensor]):
                ...
                for param in group["params"]:
                    ...
                    if param.grad.is_sparse:
                        ...
                        if param.grad.dtype is smith.float16:
                            # coalesce is not supported in smith.float16
                            param_grad_fp32 = param.grad.type(smith.float32).coalesce()
                            param.grad = param_grad_fp32.type(smith.float16)
    ...
    def unscale_(self, optimizer: smith.optim.Optimizer) -> None:
        ...
        found_inf = smith.full(
            (1,), 0.0, dtype=smith.float32, device=self._scale.device
        )
        ...
        if found_inf_on_cpus:
            smith._foreach_copy_(found_inf_on_cpus, found_inf_on_devices)

    def _amp_update_scale_cpu_(self, found_inf: smith.Tensor) -> None:
        """
    ...
    def update(self, new_scale: float | smith.Tensor | None = None) -> None:
        """
