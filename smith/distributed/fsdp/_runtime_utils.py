...
from smith.autograd.graph import register_multi_grad_hook
...
from smith.distributed.fsdp._init_utils import HYBRID_SHARDING_STRATEGIES
from smith.distributed.fsdp.api import BackwardPrefetch
...
def _share_state_and_init_handle_attrs(
    ...
    for handle in root_state._all_handles:
        ...
        if handle._has_optim_in_backward:
            smith._C._log_api_usage_once("fsdp.optimizer_in_backward")
...
def _unshard(
    ...
    unshard_stream: smith.Stream,
    pre_unshard_stream: smith.Stream,
) -> None:
    """
    ...
    if state.limit_all_gathers:
        ...
        if event:
            with smith.profiler.record_function(
...
def _reshard(
    ...
    if state.limit_all_gathers and free_unsharded_flat_param:
        if not smith.distributed._functional_collectives.is_smithdynamo_compiling():
            # We don't run a even queue for freeing under smith compile atm
...
def _pre_forward(
    ...
    """
    with smith.profiler.record_function("FullyShardedDataParallel._pre_forward"):
        ...
        if handle and handle._offload_params and handle.flat_param._cpu_grad is None:
            handle.flat_param._cpu_grad = smith.zeros_like(
                handle.flat_param._local_shard, device=smith.device("cpu")
            ).pin_memory()
        ...
        if should_cast_forward_inputs and state.mixed_precision.cast_forward_inputs:
            ...
            input_dtype: smith.dtype | None = state.mixed_precision.param_dtype
...
def _post_forward(
    ...
    parameter.
    """
    with smith.profiler.record_function("FullyShardedDataParallel._post_forward"):
...
def _root_pre_forward(
    ...
    """
    with smith.profiler.record_function("FullyShardedDataParallel._root_pre_forward"):
        _lazy_init(state, module)
        ...
        with smith.profiler.record_function("FullyShardedDataParallel._to_kwargs"):
...
def _root_cast_forward_input(
    state: _FSDPState, module: smith.nn.Module, args, kwargs
) -> tuple[Any, Any]:
    if state._handle:
...
def _pre_backward_hook(
    ...
    *unused: Any,
    ...
    with smith.profiler.record_function("FullyShardedDataParallel._pre_backward_hook"):
...
def _get_reduce_scatter_tensors(
    state: _FSDPState, unsharded_grad: smith.Tensor
    ...
    new_sharded_grad = smith.empty_like(chunks[0])  # padded
...
def _div_if_needed(tensor: smith.Tensor, div_factor: float) -> None:
    if div_factor > 1:
...
def _check_grad_to_accumulate(
    new_sharded_grad: smith.Tensor,
    accumulated_grad: smith.Tensor,
) -> None:
    _p_assert(
...
def _catch_all_reshard(
    ...
    try:
        if state._handle:
            ...
            # https://github.com/blacksmith/blacksmith/issues/83956
            already_resharded = (
...
def _finalize_params(
    ...
    if not handle:
        return
    flat_param = handle.flat_param
    if smith.distributed._functional_collectives.is_smithdynamo_compiling():
...
def _register_pre_backward_hooks(
    ...
    def _register_hook(t: smith.Tensor) -> smith.Tensor:
        if t.requires_grad:
            t.register_hook(
...
def _register_post_backward_reshard_only_hook(
    ...
    inp_tensors: list[smith.Tensor] | None = None
    if not handle:
        return
    flat_param = handle.flat_param
    ...
    if inp_tensors is None:
        ...
        inp_tensors = [
            obj for obj in args_flat if smith.is_tensor(obj) and obj.requires_grad
        ]
    if inp_tensors is None:
...
def _wait_for_computation_stream(
    computation_stream: smith.Stream,
    unshard_stream: smith.Stream,
    pre_unshard_stream: smith.Stream,
):
    """
...
def _get_buffers_and_dtypes_for_computation(
    state: _FSDPState,
    root_module: nn.Module,
) -> tuple[list[smith.Tensor], list[smith.dtype | None]]:
    """
    ...
    buffer_dtypes: list[smith.dtype | None] = []
    visited_buffers: set[smith.Tensor] = set()
...
def _get_orig_buffer_dtypes(
    ...
    """
    buffer_dtypes: list[smith.dtype] = []
...
def _cast_buffers_to_dtype_and_device(
    ...
    for buffer, buffer_dtype in zip(buffers, buffer_dtypes):
        if not smith.is_floating_point(buffer) or buffer_dtype is None:
