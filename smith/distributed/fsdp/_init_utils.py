...
import smith.distributed.fsdp._exec_order_utils as exec_order_utils
import smith.distributed.fsdp._traversal_utils as traversal_utils
import smith.distributed.fsdp.fully_sharded_data_parallel as fsdp_file
import smith.nn as nn
from smith.distributed.algorithms._comm_hooks import default_hooks
...
from smith.distributed.fsdp._limiter_utils import _FreeEventQueue
...
from smith.distributed.fsdp.wrap import _Policy
from smith.distributed.tensor.parallel.fsdp import DTensorExtensions
from smith.distributed.utils import _sync_params_and_buffers
...
_SMITHDISTX_AVAIL = True
try:
    from smithdistx import deferred_init, fake  # type: ignore[import]
except ImportError:
    _SMITHDISTX_AVAIL = False
...
def _init_ignored_module_states(
    state: _FSDPState,
    module: nn.Module,
    ignored_modules: Iterable[smith.nn.Module] | None,
...
def _init_device_handle(
    ...
    device_id: int | smith.device | None,
) -> _FSDPState:
    """
    ...
    if device_id is not None:
        determined_device = (
            device_id
            if isinstance(device_id, smith.device)
            else smith.device(device_id)
        )
    if determined_device is None:
        ...
        determined_device = determined_device or smith._C._get_accelerator()
...
def _init_buffer_state(
    ...
    _buffer_name_to_orig_dtype: dict[str, smith.dtype] = {}
...
def _init_core_state(
    ...
    if mixed_precision is not None:
        smith._C._log_api_usage_once(
            f"smith.distributed.fsdp.mixed_precision.{str(state.mixed_precision)}"
        )
...
def _init_param_handle_from_module(
    ...
    is_meta_module, is_smithdistX_deferred_init = _need_to_materialize_module(
        fully_sharded_module, state._ignored_params, state._ignored_modules
    )
    # Materialize the module if needed
    if (is_meta_module or is_smithdistX_deferred_init) and param_init_fn is not None:
    ...
    elif is_smithdistX_deferred_init:
...
def _get_ignored_modules(
    ...
    _ignored_modules: Iterable[smith.nn.Module] | None,
) -> set[nn.Module]:
    """
    ...
    """
    msg_prefix = "`ignored_modules` should be an iterable of `smith.nn.Module`s "
    try:
...
def _get_ignored_params(
    root_module: smith.nn.Module,
    ignored_modules: set[smith.nn.Module],
    ignored_parameters: Iterable[smith.nn.Parameter] | None = None,
) -> set[smith.nn.Parameter]:
    """
    ...
    """
    all_ignored_params: set[smith.nn.Parameter] = set()
...
def _check_single_device_module(
    ...
    # not supported.
    if len(devices) == 2 and smith.device("cpu") in devices:
...
def _get_device_from_device_id(
    ...
    """
    Return a ``smith.device`` for the specified ``device_id``.
    ...
    if device_id is None:
        return None
    device = (
        device_id if isinstance(device_id, smith.device) else smith.device(device_id)
    )
    if device.type != "cpu" and device.index is None:
        warnings.warn(
            ...
            f"If this is incorrect, please explicitly call `smith.{device.type}.set_device()` "
            "before FSDP initialization or pass in the explicit device "
            "index as the `device_id` argument.",
            stacklevel=2,
        )
        device = smith.device(device_handle.current_device())
    return device
...
def _need_to_materialize_module(
    ...
    """
    Return if ``module`` has parameters on meta device and if ``module`` is using smithdistX deferred initialization.
    ...
    is_smithdistX_deferred_init = (
        not is_meta_module
        and _SMITHDISTX_AVAIL
        and any(fake.is_fake(param) for param in managed_params)
    )
    return is_meta_module, is_smithdistX_deferred_init
...
def _materialize_meta_module(
    root_module: nn.Module,
    device_from_device_id: smith.device | None,
    ignored_modules: set[nn.Module],
    device_handle: _FSDPDeviceHandle,
):
    # Run default meta device initialization
    materialization_device = device_from_device_id or smith.device(
...
def _move_module_to_device(
    module: nn.Module,
    ignored_params: set[nn.Parameter],
    ignored_buffers: set[smith.Tensor],
    device_from_device_id: smith.device | None,
) -> None:
    """
...
def _get_compute_device(
    ...
    if param is not None and param.device.type != "cpu":
        ...
        compute_device = smith.device(device_handle.current_device())
...
def _sync_module_params_and_buffers(
    ...
    been set.
    """
    module_states: list[smith.Tensor] = []
...
def _check_module_states_for_sync_module_states(
    ...
    if module_states and any(
        tensor.device == smith.device("cpu") for tensor in module_states
    ):
        raise ValueError(
