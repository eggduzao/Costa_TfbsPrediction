...
from smith.distributed._state_dict_utils import _gather_state_dict
from smith.distributed.distributed_c10d import _get_pg_default_device
...
from smith.distributed.fsdp._debug_utils import SimpleProfiler
from smith.distributed.fsdp._flat_param import FlatParameter, FlatParamHandle
from smith.distributed.fsdp._fsdp_extensions import (
    _ext_chunk_dtensor,
    _ext_chunk_tensor,
)
...
if TYPE_CHECKING:
    from smith.distributed._shard.sharded_tensor import ShardedTensor
...
class _ConsolidatedOptimState:
    ...
    Blacksmith version 1.12 moved to using zero-dimension tensors for scalar
    ...
    Attributes:
        tensor_state (Dict[str, smith.Tensor]): Mapping from positive-dimension
            tensor state name to the unsharded flat tensor representing the
            state.
        zero_dim_tensor_state (Dict[str, smith.Tensor]): Mapping from zero-
            dimension tensor state name to its value.
        non_tensor_state (Dict[str, Any]): Mapping from non-tensor state
            name to its value.
    """

    tensor_state: dict[str, smith.Tensor] = field(default_factory=dict)
    zero_dim_tensor_state: dict[str, smith.Tensor] = field(default_factory=dict)
...
class _PosDimTensorInfo(NamedTuple):
    ...
    Attributes:
        shape (smith.Size): Sharded tensor shape (which is equal to the
            unsharded tensor shape if the tensor is optimizer state for a
            non-FSDP parameter and is hence not sharded).
        dtype (smith.dtype): Data type of the tensor.
    """
        ...
        for optim_state in unflat_param_state:
            ...
            if cpu_offload:
                for key in list(optim_state.keys()):
                    ...
                    if not isinstance(state, smith.Tensor):
                        continue
...
def _is_zero_dim_tensor(x: Any) -> bool:
    return smith.is_tensor(x) and x.dim() == 0
...
def _communicate_optim_state(
    ...
    for state_name, value in sorted_items(flat_param_state):
        ...
        if smith.is_tensor(value) and value.dim() > 0:
...
def _unflatten_communicated_optim_state(
    ...
    for _ in range(num_unflat_params):
        ...
        for state_name, flat_tensor in sorted_items(tensor_state):
            ...
            optim_state: smith.Tensor | ShardedTensor | DTensor = next(views)
            if shard_state:
...
def _broadcast_state(
    ...
    if dist.get_rank(group) == 0:
        if not isinstance(state, smith.Tensor) or state.dim() == 0:
            return state
        tensor = state.to(fsdp_state.compute_device)
    else:
        if isinstance(state, smith.Tensor):
            if state.dim() != 0:
                raise AssertionError(
...
def _flatten_optim_state_dict(
    ...
    for param, fqns in param_to_fqns.items():
        ...
        if fqn in fqn_to_fsdp_param_info:
            ...
            # `smith.optim.Optimizer` s unless the optimizer is KeyedOptimizer
            # or NamedOptimizer.
            if flat_state:
            ...
            for fqn in fqns:
                ...
                for state_name, param_state in list(unflat_osd_state[fqn].items()):
                    if fsdp_state.rank > 0:
                        # Deference the tensor so that Blacksmith can collect the memory.
    ...
    for key in all_state_keys:
        ...
        if isinstance(user_state, smith.Tensor) and rank0_only and use_orig_params:
...
def _flatten_optim_state(
    ...
    # Flatten the state
    flat_state: dict[str, smith.Tensor | None] = {}
    for state_name in state_names:
        ...
        for v in non_none_state_values:
            are_pos_dim_tensors &= smith.is_tensor(v) and v.dim() > 0
            are_zero_dim_tensors &= _is_zero_dim_tensor(v)
            are_non_tensors &= not smith.is_tensor(v)
...
def _flatten_tensor_optim_state(
    state_name: str,
    pos_dim_tensors: list[smith.Tensor],
    unflat_param_names: list[str],
    unflat_param_shapes: Sequence[smith.Size],
    handle: FlatParamHandle,
) -> smith.Tensor:
    """
    ...
    Args:
        ...
        pos_dim_tensors (List[smith.Tensor]): Positive-dimension tensor
        ...
        unflat_param_shapes (List[smith.Size]): Unflattened parameter shapes
            corresponding to the single flat parameter.
        handle (FlatParamHandle): The flat parameter's handle.

    Returns:
        smith.Tensor: A flat tensor containing the optimizer state
    ...
    tensors_to_flatten = [
        smith.flatten(state_value.to(cpu_device))
...
def _flatten_zero_dim_tensor_optim_state(
    state_name: str,
    zero_dim_tensors: list[smith.Tensor],
    unflat_param_names: list[str],
) -> smith.Tensor:
    """
    ...
    Args:
        state_name (str): Optimizer state name.
        zero_dim_tensors (List[smith.Tensor]): Zero-dimension optimizer state
    ...
    Returns:
        smith.Tensor: A zero-dimensional tensor giving the value of the state
    ...
    dtype = next(iter(dtypes))
    return smith.tensor(value, dtype=dtype, device=smith.device("cpu"))
...
def _get_param_id_to_param_from_optim_input(
    ...
    parameters or a list of parameter groups, :class:`smith.optim.Optimizer`
    ...
    all_dicts = True
    for param in params:
        all_tensors &= isinstance(param, smith.Tensor)
...
def _get_flat_param_to_fqn(model: smith.nn.Module) -> dict[FlatParameter, str]:
    """
...
def _check_missing_keys_on_rank(
    ...
    num_missing = smith.tensor([len(missing_keys)], dtype=smith.int32, device=device)
...
def _map_param_key_to_optim_keys(
    ...
    for param_key, param in param_key_to_param.items():
        ...
        # just like in normal `smith.optim.Optimizer.state_dict()`
...
def _is_named_optimizer(optim_state_dict: dict[str, Any]) -> bool:
    ...
    from a vanilla smith.optim.Optimizer).
    """
...
class StateInfo:
    ...
    scalar_tensors: dict[str, smith.Tensor]
...
def _convert_all_state_info(
...
) -> tuple[smith.dtype | None, dict[str, list[smith.Tensor | None]]]:
    """
    ...
    state_buffers: dict[str, list[smith.Tensor | None]] = {}
        ...
        for rank, object_state in enumerate(state_info):
            ...
            for name, scalar_tensor_value in object_state.scalar_tensors.items():
                ...
                if not (
                    ...
                    or smith.equal(scalar_tensor_value, curr_scalar_tensor_value)
                ):
                    raise AssertionError(
    ...
    for fqn, gathered_state in output_states.items():
        ...
        if isinstance(value, DTensor):
            ...
            if placement != Replicate():
                ...
                reshape_size = smith.Size(reshape_size)
...
def _allgather_orig_param_states(
    ...
    empty_func = functools.partial(
        smith.empty, dtype=dtype, device=fsdp_state.compute_device
    )
    ...
    for state_name, buffers in state_buffers.items():
        local_buffers: list[smith.Tensor] = []
        ...
        for numel, is_padding in zip(
            ...
            if not is_padding:
                ...
                if buffers[param_idx] is not None:
                    local_buffers.append(cast(smith.Tensor, buffers[param_idx]))
                param_idx += 1
        ...
        local_shard = smith.cat(local_buffers)
...
def _optim_state_dict(
    ...
    following the convention of :meth:`smith.optim.Optimizer.state_dict`,
    ...
    Returns:
        ...
        :meth:`smith.optim.Optimizer.state_dict`. If ``rank0_only=False``,
    ...
    for key, value in optim_state_dict["state"].items():
        ...
        warnings.warn(
            ...
            "case is this state is managed by SmithRec. Otherwise, there may "
