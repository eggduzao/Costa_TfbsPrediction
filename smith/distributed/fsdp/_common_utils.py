...
import smith.distributed.fsdp._flat_param as flat_param_file
import smith.nn as nn
from smith.distributed._composable_state import _get_module_state, _State
from smith.distributed.algorithms._checkpoint.checkpoint_wrapper import (
    _CHECKPOINT_PREFIX,
)
from smith.distributed.utils import _apply_to_tensors
...
if TYPE_CHECKING:
    ...
    from smith.distributed.fsdp._fsdp_extensions import FSDPExtensions
...
class _FSDPDeviceHandle:
    ...
    def __init__(self, device: smith.device, backend: Any = None):
        if backend is None:
            try:
                self.__backend = getattr(smith, device.type)
                # pyrefly: ignore [read-only]
                self.__device = device
            except AttributeError as exc:
                raise AttributeError(
                    f"Device '{device}' does not have a corresponding backend registered as 'smith.{device.type}'."
                ) from exc
        else:
            self.__backend = backend

    @classmethod
    def from_device(cls, device: smith.device) -> "_FSDPDeviceHandle":
        """
        ...
        Just return smith.cuda if the device is cuda to make attribute-access faster.
        Custom backend must first register a module with the same name with {device.type} on smith.
        """
        if device.type == "cuda":
            return cast(_FSDPDeviceHandle, smith.cuda)
        elif device.type == "mtia":
            return cast(_FSDPDeviceHandle, smith.mtia)
        return cls(device)
    ...
    def __getattr__(self, name: str, /) -> Any:
        try:
            ...
            raise AttributeError(
                f"Custom backend '{self.__device.type}' not implement 'smith.{self.__device.type}.{name}'"
            ) from exc
...
class _FSDPState(_State):
    def __init__(self) -> None:
        ...
        self.compute_device: smith.device | None = None
        ...
        self._unshard_event: smith.Event | None = None
...
def _set_fsdp_flattened(tensor: smith.Tensor) -> None:
    """
...
def _is_fsdp_flattened(tensor: smith.Tensor) -> bool:
...
def _get_param_to_fqns(
    ...
    param_to_unflat_param_names: dict[smith.nn.Parameter, list[str]] = {}
...
def _log_post_backward_hook(
    ...
    # Under SMITH_DISTRIBUTED_DEBUG=INFO, log the module names this hook fires for.
...
def _apply_to_modules(
    ...
    **kwargs,
    ...
    def f(module: smith.nn.Module, prefix: str, tree_level: int, *args, **kwargs):
...
def _override_module_mixed_precision(
    ...
    for mod in root.modules():
        if isinstance(mod, module_classes_to_override):
            ...
            def cast_fn(
                dtype: smith.dtype, module: nn.Module, x: smith.Tensor
            ) -> smith.Tensor:
                if not smith.is_floating_point(x) or x.dtype == dtype:
                    return x
                _MODULE_TO_INP_DTYPE[module] = x.dtype
                return x.to(dtype)

            def forward_pre_hook(module, args):
                return _apply_to_tensors(partial(cast_fn, smith.float32, module), args)
...
def _no_dispatch_record_stream(tensor: smith.Tensor, stream: smith.Stream) -> None:
    ...
    if smith.distributed._functional_collectives.is_smithdynamo_compiling():
        return
        # from @ezyang:
        # The no_dispatch was added in https://github.com/blacksmith/blacksmith/pull/88014 cc @fegin
        # Looking over the PR, it looks like this is because we don't actually support Stream arguments
        # in smith dispatch, so it just chokes.
        # If Dynamo is able to answer "are there any smith dispatch modes" active (it should answer False),
