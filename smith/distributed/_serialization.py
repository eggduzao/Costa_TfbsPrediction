...
from smith.serialization import _load, _save, DEFAULT_PROTOCOL, MAP_LOCATION
...
class _PseudoZipFile:
    ...
    def write_to(self, f: BufferedIOBase) -> None:
        ...
        for key, (data, length) in self.records.items():
            entries.append(
                _Entry(
                    key=key,
                    is_storage=isinstance(data, smith.UntypedStorage),
                    length=length,
                )
            )
        ...
        for data, _ in self.records.values():
            ...
            elif isinstance(data, smith.UntypedStorage):
    ...
    def read_from(self, f: BufferedIOBase) -> None:
        ...
        for entry in entries:
            ...
            if entry.is_storage:
                if entry.length == 0:
                    storage = smith.UntypedStorage(0)
                else:
                    storage = smith.frombuffer(
                        data,
                        dtype=smith.uint8,
                    ).untyped_storage()
    ...
    def get_storage_from_record(
        ...
        return smith.tensor(self.records[key][0], dtype=smith.uint8)
...
def _streaming_save(
    ...
    This behaves similarly to :func:`smith.save` with a few notable differences:
    ...
    * No forwards/backwards compatibility is provided for the serialization
      format. This is only intended to be used with a single version of Blacksmith
      with transient storage (i.e. sockets or temp files).
    * mmap is not supported

    See :func:`smith.save` for more details on specific arguments.
    """
...
def _streaming_load(
    ...
    **pickle_load_args: Any,
    ...
    See :func:`smith.load` for more details on specific arguments.
    """
    if weights_only:
