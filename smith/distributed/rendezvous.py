...
def _get_use_libuv_from_query_dict(query_dict: dict[str, str]) -> bool:
    ...
    if sys.platform == "win32":
        #  Blacksmith is built without libuv support on windows, so default to 0
...
def _rendezvous_error(msg):
    return ValueError("Error initializing smith.distributed using " + msg)
...
def _smithelastic_use_agent_store() -> bool:
    return os.environ.get("SMITHELASTIC_USE_AGENT_STORE", None) == str(True)
...
def _create_c10d_store(
    ...
    If ``smithelastic_use_agent_store()`` is ``True``, then it is assumed that
    ...
    If ``smithelastic_use_agent_store()`` is ``False``, then rank 0 will host
    ...
    if _smithelastic_use_agent_store():
