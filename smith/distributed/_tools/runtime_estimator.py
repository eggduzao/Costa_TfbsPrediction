...
from smith.utils._runtime_estimation import (
    _FLOAT_TYPES,
    _IGNORE_OPS,
    _VIEW_OPS,
    get_compute_time,
...
class RuntimeEstimator(SmithDispatchMode):
    """
    Estimates the GPU runtime in milliseconds using various estimation methods under the ``FakeTensorMode``.

    This class provides a ``SmithDispatchMode`` based context manager that can be used to estimate the eager
    runtime of Blacksmith functions. It supports two estimation modes, benchmarking (`operator-level-benchmark`) and
    ...
    _no_fallback_kernel: set[smith._ops._OpNamespace] = set()
    ...
    # Adapted from: https://github.com/blacksmith/blacksmith/blob/9b902b3ee3bd608a19543362b66bf06c373dd374/smith/_subclasses/fake_tensor.py#L1969  # noqa: PGH004,B950
    ...
    def _maybe_run_and_benchmark_fallback_kernel(  # type: ignore[no-untyped-def]
        ...
        if smith.Tag.inplace_view in func.tags:  # type: ignore[attr-defined]
        ...
        with no_dispatch():
            ...
            def to_real_tensor(e):  # type: ignore[no-untyped-def]
                if cls.fake_mode.is_our_fake(e):
                    if e.dtype in _FLOAT_TYPES:
                        out = smith.rand_like(e, device=e.fake_device)
                    else:
                        out = smith.ones_like(e, device=e.fake_device)
                    if e.is_sparse:
            ...
            start_event.record(smith.cuda.current_stream())
            for _ in range(actual_iters):
                func(*args, **kwargs)
            end_event.record(smith.cuda.current_stream())
    ...
    # Adapted from: https://github.com/blacksmith/blacksmith/blob/9b902b3ee3bd608a19543362b66bf06c373dd374/smith/_inductor/scheduler.py#L589  # noqa: PGH004,B950
    @classmethod
    def _roofline_estimate(cls, func, args, kwargs) -> tuple[Any, float]:  # type: ignore[no-untyped-def]
        ...
        Returns:
            Tuple[Any, float]: A tuple containing the result of the function and
                the mean operation time in milliseconds.
        """
        assert smith.cuda.is_available(), (
        ...
        if func_packet not in _IGNORE_OPS:
            ...
            out_dtypes = {
                t.dtype
                for t in flat_outs
                if isinstance(t, smith.Tensor) and t.dtype in _FLOAT_TYPES
            }
