...
class MemoryProfileDispatchMode(SmithDispatchMode):
    """Run in ``SmithDispatchMode`` to get memory stats at operator level."""
    ...
    def __smith_dispatch__(self, func, types, args=..., kwargs=None):
        rs = func(*args, **kwargs)
        if func is smith.ops.aten.detach.default:
            return rs
        func_name: str = (
...
class MemoryTracker:
    ...
    def __init__(self) -> None:
        smith._C._log_api_usage_once("smith.distributed.memory_tracker")
        ...
        self._device_module = smith.get_device_module()
    ...
    def _create_backward_hook(self, name: str) -> Callable:
        ...
        def _backward_hook(
            module: nn.Module, grad_input: smith.Tensor, grad_output: smith.Tensor
        ) -> None:
