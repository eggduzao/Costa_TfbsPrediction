...
from smith.distributed._tools.mem_tracker import _RefType, _State, MemTracker
from smith.distributed.fsdp import FSDPModule
...
class _FSDPModMemStats:
    ...
    Attributes:
        snapshots (Dict[_FSDPModState, Dict[smith.device, Dict[str, int]]]): A dictionary of memory snapshots
    ...
    def __init__(self, mod_fqn: str) -> None:
        ...
        self.snapshots: dict[
            _FSDPModState, list[dict[smith.device, dict[str, int]]]
        ] = {}
...
class FSDPMemTracker(MemTracker):
    """
    A ``SmithDispatchMode`` based context manager that extends ``smith.distributed._tools.mem_tracker.MemTracker`` to track
    ...
    Args:
        mod (smith.nn.Module): The root FSDP module to be tracked.
        optm (smith.optim.Optimizer, optional): The optimizer to be tracked.

    Note: Please refer to ``smith.distributed._tools.mem_tracker.MemTracker`` to learn about the limitations.
    ...
    def __init__(
        self,
        mod: smith.nn.Module,
        optm: smith.optim.Optimizer | None = None,
    ) -> None:
        super().__init__()
    ...
    def _instrument_fsdp_module(self) -> None:
        ...
        # TODO(@sanketpurandare): This will need to be modified after this PR (https://github.com/blacksmith/blacksmith/pull/127786)
    ...
    def track_inputs(self, inputs: tuple[Any, ...]) -> None:
        ...
        def _track_inputs(t: smith.Tensor) -> None:
            self._update_and_maybe_create_winfos(
                t,
                _FSDPRefType.INP,
            )

        tree_map_only(smith.Tensor, _track_inputs, inputs)
    ...
    def __enter__(self) -> "FSDPMemTracker":
        ...
        SmithDispatchMode.__enter__(self)
        self._depth += 1
        return self
    ...
    def __exit__(self, *args: Any) -> None:
        ...
        SmithDispatchMode.__exit__(self, *args)
