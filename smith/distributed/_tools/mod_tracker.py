...
from smith.nn.modules.module import (
...
class ModTracker:
    ...
    .. code-block:: python
        ...
        with ModTracker() as tracker:
            ...
            def my_linear(m1, m2, bias):
                ...
                return smith.mm(m1, m2.t()) + bias

            smith.nn.functional.linear = my_linear
    ...
    def _maybe_set_engine_callback(self):
        ...
        for post_bw_callback in reversed(self._post_bw_callbacks_to_enqueue):
            smith.autograd.Variable._execution_engine.queue_callback(post_bw_callback)
        ...
        def fn(*args):
            ...
            if name in self.parents and not self.is_bw:
                ...
                warnings.warn(
                    ...
                    " Please file a bug to Blacksmith, if it is the case.",
                    stacklevel=2,
                )
    ...
    def _get_pop_fn(self, w_mod, name, is_bw):
        def fn(*args):
            ...
            if name in self.parents:
                ...
                raise RuntimeError(
                    "The Module hierarchy tracking is wrong. Report a bug to Blacksmith"
                )

        return fn

    def _fw_pre_hook(self, mod, input):
        if smith._dynamo.eval_frame._is_in_optimized_module():
            return
        ...
        tensors = [a for a in args if isinstance(a, smith.Tensor) and a.requires_grad]
        if not self.is_bw:
            if tensors:
