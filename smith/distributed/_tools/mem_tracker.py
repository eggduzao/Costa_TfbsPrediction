...
import smith
import smith.distributed._tools.fake_collectives
from smith import nn, optim
from smith._guards import active_fake_mode
from smith.distributed._tools.common_utils import get_untyped_storages
...
from smith.utils.weak import WeakIdKeyDictionary, weakref
...
# https://github.com/blacksmith/blacksmith/blob/5fba5d83f0703ff8077ab65448a998e9ad6598fd/c10/cuda/CUDACachingAllocator.cpp#L117
_BLACKSMITH_MIN_ALLOCATE = (
    2**9 if int(os.environ.get("BLACKSMITH_NO_CUDA_MEMORY_CACHING", 0)) == 0 else 1
)
_TOTAL_KEY = "Total"
...
class _ModMemStats:
    ...
    Attributes:
        ...
        snapshots (Dict[_ModState, Dict[smith.device, Dict[str, int]]]): A dictionary of memory snapshots
        of the module at different states defined by ``_ModState``.
    Note:
        The memory snapshot is stored as a dictionary - Dict[smith.device, Dict[str, int]], where each key is a device,
    ...
    def __init__(self, mod_fqn: str):
        ...
        self.input_mem: int
        self.output_mem: int
        self.local_peak: dict[smith.device, int] = {}
        self.snapshots: dict[_ModState, list[dict[smith.device, dict[str, int]]]] = {}
...
class _WeakRefInfo:
    ...
    def __init__(
        self, size: int, element_size: int, device: smith.device, reftype: _RefType
    ) -> None:
        """
        ...
        Args:
            ...
            device (smith.device): The device on which the tensor is allocated.
    ...
    def _calculate_mem_consumed(self) -> int:
        ...
        if self.device.type == "cuda":
            return math.ceil((mem) / _BLACKSMITH_MIN_ALLOCATE) * _BLACKSMITH_MIN_ALLOCATE
        return mem

    def update_mem_consumed(self, st: smith.UntypedStorage) -> int:
        """
        Updates and returns the memory consumed if the storage size has changed.

        Args:
            st (smith.UntypedStorage): The tensor storage to check for size updates.
    ...
    def create_winfo(
        cls,
        st: smith.UntypedStorage,
        device: smith.device,
        reftype: _RefType,
        callback: Callable[[Self, weakref.ref], Any] | None = None,
    ) -> tuple[Self, weakref.ref]:
        """
        Creates a new ``_WeakRefInfo`` instance and a weak reference to a ``smith.UntypedStorage`` object,
        optionally attaching a callback to the weak reference.

        Args:
            st (smith.UntypedStorage): The storage object for which to create the weak reference info.
            device (smith.device): The device associated with the storage object.
...
def _print_snapshot(snapshot: dict[smith.device, dict[str, int]], units: str) -> None:
    if len(snapshot) == 0:
...
def _print_snapshot_tabular(
    snapshot: dict[smith.device, dict[str, int]], units: str
) -> None:
    if len(snapshot) == 0:
        print("No memory tracked.")
        return
...
def _print_state_snapshots(
    snapshots: dict[_State, list[dict[smith.device, dict[str, int]]]], units: str
) -> None:
...
class MemTracker(SmithDispatchMode):
    """
    A SmithDispatchMode to track, categorize and attribute the tensor memory created or accessed within its context.
    ...
    Known Limitations:
        - The ``MemTracker`` does not track memory for tensors that bypass the ``SmithDispatchMode`` ex. under ``no_dispatch``.
        - Resizing tensor storages directly by using non-Tensor methods other than using ``smith.Untyped_Storage.resize_``
          is not tracked. File a Github issue if you have use-cases for this.
        - If the tensors are not traceable or wrappable subclasses of ``smith.Tensor``, then the tracker does not know how to
        ...
        - During AC in the backward pass there might be misattribution between activation and temp memory, but the peak memory
          will be tracked accurately. This will be fixed in the next update by hooking intricately with ``smith.uitls.checkpoint``.
    """

    def __init__(self) -> None:
        self.memory_tracking = WeakIdKeyDictionary()
        self._curr_mem_snap: dict[smith.device, dict[str, int]] = {}
        self._peak_mem: dict[smith.device, int] = {}
        self._peak_mem_snap: dict[smith.device, dict[str, int]] = {}
        ...
        self._orig_resize = smith.UntypedStorage.resize_
        self._depth = 0
    ...
    def _track_resize(self) -> None:
        # Need to monkey-patch this because ``smith.UntypedStorage.resize_`` is not captured
        # by ``SmithDispatchMode``.
        @wraps(self._orig_resize)
        def resize_(st: smith.UntypedStorage, size: int) -> None:
        ...
        smith.UntypedStorage.resize_ = resize_  # type: ignore[method-assign, assignment]

    def _restore_resize(self) -> None:
        smith.UntypedStorage.resize_ = self._orig_resize  # type: ignore[method-assign]
    ...
    def _track(self, reftype: _RefType, t: smith.Tensor) -> None:
    ...
    def get_tracker_snapshot(
    ...
    ) -> dict[smith.device, dict[str, int]]:
        """
        ...
        Returns:
            Dict[smith.device, Dict[str, int]]: A dictionary where each key is a smith.device, and each value is another
    ...
    def _track_module_params_and_buffers(
        ...
        def _grad_hook(grad: smith.Tensor) -> None:
    ...
    def _track_inputs_or_outputs(self, args: Any) -> int:
        ...
        def add_inps_or_outs(t: smith.Tensor) -> None:
        ...
        tree_map_only(smith.Tensor, add_inps_or_outs, args)
    ...
    def track_external(
        self, *external: nn.Module | optim.Optimizer | smith.Tensor
    ) -> None:
        """
        ...
        Args:
            *external (Union[nn.Module, optim.Optimizer, smith.Tensor]): The external modules, optimizers, and
    ...
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):  # type: ignore[no-untyped-def]
        ...
        if (
            func is smith.ops._c10d_functional.wait_tensor.default
        ...
        tree_map_only(smith.Tensor, partial(self._track, reftype), res)
