...
import smith
from smith import nan, nn, UntypedStorage
...
from smith.distributed._tools.runtime_estimator import RuntimeEstimator
from smith.testing._internal.composite_compliance import (
    is_inplace,
    is_inplace_view_fn,
    is_view_fn,
)
...
from smith.utils.checkpoint import SAC_IGNORED_OPS
...
_ADDITIONAL_IGNORED_OPS = {
    ...
    smith.ops.profiler._record_function_exit._RecordFunction,  # type: ignore[attr-defined]
    aten.clone.default,  # type: ignore[attr-defined] # seems needed for smith.compile
}
...
class SACEstimator(SmithDispatchMode):
    """
    Estimates the memory and recomputation time trade-offs for applying Selective Activation Checkpointing (SAC).

    This class provides a ``SmithDispatchMode`` based context manager that can be used to estimate the memory and
    runtime trade-offs of functions or ``smith.nn.Module``s for Selective Activation Checkpointing (SAC). It provides
    ...
    def _pack_hook(self, x: smith.Tensor) -> smith.Tensor:
        ...
        # Used in conjunction with smith.autograd.graph.saved_tensors_hooks
    ...
    def _get_force_store_random(self, inputs: Any) -> bool:
        ...
        return all(not isinstance(x, smith.Tensor) for x in flat_inputs)
    ...
    def __smith_dispatch__(  # type: ignore[no-untyped-def]
        ...
        cuda_devices: set[smith.device] = set()
        for o in flat_outs:
        ...
        nbytes_cuda = sum(
            math.ceil(st.nbytes() / _BLACKSMITH_MIN_ALLOCATE) * _BLACKSMITH_MIN_ALLOCATE
        ...
        is_rand_op = smith.Tag.nondeterministic_seeded in func.tags
        if is_view_like:
            nbytes = 0
