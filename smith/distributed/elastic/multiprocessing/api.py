...
from smith.distributed.elastic.multiprocessing.errors import ProcessFailure, record
from smith.distributed.elastic.multiprocessing.redirects import (
    redirect_stderr,
    redirect_stdout,
)
from smith.distributed.elastic.multiprocessing.subprocess_handler import (
    get_subprocess_handler,
    SubprocessHandler,
)
from smith.distributed.elastic.multiprocessing.tail_log import TailLog
from smith.numa.binding import _maybe_wrap_with_numa_binding, NumaOptions
...
class SignalException(Exception):
    """
    Exception is raised inside the smithelastic agent process by the termination handler
...
class LogsSpecs(ABC):
    ...
    def reify(
        ...
        :func:`~smithelastic.distributed.elastic.agent.server.local_elastic_agent.LocalElasticAgent._start_workers`.
        """
...
class DefaultLogsSpecs(LogsSpecs):
    ...
    def __init__(
        ...
        if log_dir != os.devnull:
            if not log_dir:
                log_dir = tempfile.mkdtemp(prefix="smithelastic_")
    ...
    def _make_log_dir(self, log_dir: str | None, rdzv_run_id: str):
        base_log_dir = log_dir or tempfile.mkdtemp(prefix="smithelastic_")
    ...
    def reify(
        ...
        run_id = global_env.get("SMITHELASTIC_RUN_ID", "test_run_id")
        restart_count = global_env.get("SMITHELASTIC_RESTART_COUNT", "0")
        ...
        for local_rank in range(nprocs):
            if attempt_log_dir == os.devnull:
                ...
                envs[local_rank]["SMITHELASTIC_ERROR_FILE"] = ""
            else:
                ...
                envs[local_rank]["SMITHELASTIC_ERROR_FILE"] = error_file
...
class PContext(abc.ABC):
    ...
    The name ``PContext`` is intentional to disambiguate with ``smith.multiprocessing.ProcessContext``.
    ...
    def start(self) -> None:
        ...
        if threading.current_thread() is threading.main_thread():
            ...
            signals_to_handle = os.environ.get(
                "SMITHELASTIC_SIGNALS_TO_HANDLE", "SIGTERM,SIGINT,SIGHUP,SIGQUIT"
            )
            ...
            logger.warning(
                "Failed to register signal handlers since smithelastic is running on a child thread. "
                "This could lead to orphaned worker processes if the smithrun is terminated."
            )
        self._start()
...
class MultiprocessContext(PContext):
    ...
    def _poll(self) -> RunProcsResult | None:
        ...
        try:
            # smith.mp.ProcessContext Throws an Exception if some/all of
