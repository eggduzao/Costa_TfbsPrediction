...
For functions, it uses ``smith.multiprocessing`` (and therefore python
...
::
 ...
 from smith.distributed.elastic.multiprocessing import Std, start_processes
...
Just like ``smith.multiprocessing``, the return value of the function
...
from smith.distributed.elastic.multiprocessing.api import (  # noqa: F401
    _validate_full_rank,
    DefaultLogsSpecs,
    LogsDest,
    LogsSpecs,
...
def start_processes(
    ...
    .. note:: When the ``entrypoint`` is a binary (``str``), ``args`` can only be strings.
              ...
              ``smith.distributed.elastic.multiprocessing.errors.record``. For function launches,
