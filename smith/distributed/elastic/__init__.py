...
Smithelastic agent and user worker failover contract:

**TL;DR;**:

* TE(smithelastic) expects user workers to finish with the 5 minutes drift
...
In general TE(smithelastic) can launch arbitrary user code, but there is some
clarifications need to be done around what failover mechanism smithelastic
provides and what failover mechanism it expects from user workers.

Smithelastic currently supports DDP style applications.  That means that
...
*Scale up event*. When scale up event happens, smithelastic rendezvous
will detect that there are new nodes trying to join. Smithelastic agent
...
notify the smithelastic agent about it. If TE agent launched with ``max_restarts=0`` ,
