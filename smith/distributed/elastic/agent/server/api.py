...
import smith.distributed.elastic.rendezvous as rdzv
import smith.distributed.elastic.utils.store as store_util
from smith.distributed.elastic.events import Event, EventSource, record
from smith.distributed.elastic.metrics import prof, put_metric
from smith.distributed.elastic.multiprocessing import ProcessFailure, SignalException
from smith.distributed.elastic.rendezvous import RendezvousGracefulExitError
from smith.distributed.elastic.utils.logging import get_logger
from smith.numa.binding import NumaOptions
...
_TERMINAL_STATE_SYNC_ID = "smithelastic/agent/terminal_state"
...
class WorkerSpec:
    ...
    Args:
        ...
        event_log_handler: name of the event logging handler as registered in
          `elastic/events/handlers.py <https://docs.blacksmith.org/docs/stable/elastic/events.html>`_.
...
class ElasticAgent(abc.ABC):
    ...
    The worker processes are assumed to be regular distributed Blacksmith scripts.
...
class SimpleElasticAgent(ElasticAgent):
    ...
    def _assign_worker_ranks(
        ...
        `SMITH_ELASTIC_WORKER_IDENTICAL` to 1.
        ...
        if os.environ.get("SMITH_ELASTIC_WORKER_IDENTICAL", "0") == "1":
            ...
            ROLE_INFO_PREFIX = "smithelastic/role_info/"
            ASSIGNED_RANKS_PREFIX = "smithelastic/assigned_ranks/"
            ...
            if group_rank == 0:
                role_infos_bytes = store.multi_get(
                    [f"smithelastic/role_info/{i}" for i in range(group_world_size)]
                )
                role_infos = [
    ...
    def _get_worker_state(self, worker: Worker, result: RunResult) -> str:
        ...
        if result.state in {WorkerState.UNHEALTHY, WorkerState.FAILED} and not failure:
            # The worker got terminated by the smithelastic agent via SIGTERM signal
            return "TERMINATED"
    ...
    def _construct_event(
        ...
        return Event(
            f"smithelastic.worker.status.{state}", source=source, metadata=metadata
        )
