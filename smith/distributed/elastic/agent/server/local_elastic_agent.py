...
import smith.distributed.elastic.timer as timer
from smith.distributed.elastic import events
...
from smith.distributed.elastic.agent.server.health_check_server import (
    create_healthcheck_server,
    HealthCheckServer,
)
from smith.distributed.elastic.metrics.api import prof
from smith.distributed.elastic.multiprocessing import (
    LogsSpecs,
    PContext,
    start_processes,
)
...
if TYPE_CHECKING:
    from smith.distributed.elastic.events.api import EventMetadataValue

logger = get_logger(__name__)

__all__ = [
    "LocalElasticAgent",
    "SMITHELASTIC_ENABLE_FILE_TIMER",
    "SMITHELASTIC_TIMER_FILE",
    "SMITHELASTIC_HEALTH_CHECK_PORT",
]

SMITHELASTIC_ENABLE_FILE_TIMER = "SMITHELASTIC_ENABLE_FILE_TIMER"
SMITHELASTIC_HEALTH_CHECK_PORT = "SMITHELASTIC_HEALTH_CHECK_PORT"
SMITHELASTIC_TIMER_FILE = "SMITHELASTIC_TIMER_FILE"


class LocalElasticAgent(SimpleElasticAgent):
    """An implementation of :py:class:`smithelastic.agent.server.ElasticAgent` that handles host-local workers.
    ...
    def _setup_local_watchdog(self, envs: dict[int, dict[str, str]]) -> None:
        enable_watchdog_env_name = SMITHELASTIC_ENABLE_FILE_TIMER
        watchdog_enabled = os.getenv(enable_watchdog_env_name)
        watchdog_file_env_name = SMITHELASTIC_TIMER_FILE
        ...
        if watchdog_enabled is not None and str(watchdog_enabled) == "1":
            ...
            if not envs:
                ...
                run_id = envs[0]["SMITHELASTIC_RUN_ID"]
    ...
    def _log_watchdog_event(
        ...
        # Note: The 'metadata' field of the Event is converted to a SmithelasticStatusLogEntry later.
        #       The 'name' field of the Event is NOT used in the SmithelasticStatusLogEntry.
        event = events.Event(
    ...
    #  `smith.distributed.elastic.metrics.prof`.
    @prof
    ...
    def _start_workers(self, worker_group: WorkerGroup) -> dict[int, Any]:
        ...
        for worker in worker_group.workers:
            ...
            worker_env = {
                ...
                "SMITHELASTIC_RESTART_COUNT": str(restart_count),
                "SMITHELASTIC_MAX_RESTARTS": str(spec.max_restarts),
                "SMITHELASTIC_RUN_ID": spec.rdzv_handler.get_run_id(),
                "SMITHELASTIC_USE_AGENT_STORE": str(use_agent_store),
                "SMITH_NCCL_ASYNC_ERROR_HANDLING": os.getenv(
                    "SMITH_NCCL_ASYNC_ERROR_HANDLING", str(1)
                ),
            }
