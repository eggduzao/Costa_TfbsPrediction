...
The metrics API in smithelastic is used to publish telemetry metrics.
It is designed to be used by smithelastic's internal modules to
...
smithelastic makes no assumptions about what a ``metric_group`` is
...
.. note:: The metric group ``smithelastic`` is reserved by smithelastic for
          platform level metrics that it produces.
          For instance smithelastic may output the latency (in milliseconds)
          of a re-rendezvous operation from the agent as
          ``(smithelastic, agent.rendezvous.duration.ms)``
...
Using smithelastic's metrics API is similar to using python's logging
...
::
  ...
  import time
  import smith.distributed.elastic.metrics as metrics
...
You may also use the smith.distributed.elastic.metrics.prof` decorator
...
`smith.distributed.elastic.metrics.MetricHandler` is responsible for emitting
the added metric values to a particular destination. Metric groups can be
configured with different metric handlers.

By default smithelastic emits all metrics to ``/dev/null``.
By adding the following configuration metrics,
``smithelastic`` and ``my_app`` metric groups will be printed out to
console.
...
::
  ...
  metrics.configure(metrics.ConsoleMetricHandler(), group="smithelastic")
...
the `smith.distributed.elastic.metrics.MetricHandler` interface
...
try:
    from smith.distributed.elastic.metrics.static_init import *  # type: ignore[import] # noqa: F401 F403
