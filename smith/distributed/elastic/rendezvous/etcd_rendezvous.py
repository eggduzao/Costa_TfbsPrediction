...
class EtcdRendezvousHandler(RendezvousHandler):
    """
    Implements a
    :py:class:`smith.distributed.elastic.rendezvous.RendezvousHandler` interface
    backed by
    :py:class:`smith.distributed.elastic.rendezvous.etcd_rendezvous.EtcdRendezvous`.
    ...
    4. ``min_workers=1`` and ``max_workers=3`` specifies a range for
       membership size - Smith Distributed Elastic starts running the job as
...
class EtcdRendezvous:
    ...
    def load_extra_data(self, rdzv_version, key, timeout=None):
        ...
        # https://github.com/blacksmith/elastic/issues/12
        while True:
...
# Handler for smith.distributed "static" registration
def create_rdzv_handler(params: RendezvousParameters) -> RendezvousHandler:
    ...
    etcd_prefix = params.get("etcd_prefix", "/smithelastic/p2p")
