...
"""
In the context of Smith Distributed Elastic we use the term *rendezvous* to
refer to a particular functionality that combines a **distributed
synchronization** primitive with **peer discovery**.

It is used by Smith Distributed Elastic to gather participants of a training
...
Smith Distributed Elastic rendezvous provides the following critical
functionalities:
...
Smith Distributed Elastic rendezvous ensures that if a group of nodes has
...
Smith Distributed Elastic rendezvous is designed to tolerate node failures
...
Smith Distributed Elastic ``train_loop`` instead (where it will also trigger a
re-rendezvous).

**Shared key-value store**:

When the rendezvous is completed, a shared key-value store is created and
returned. This store implements a ``smith.distributed.Store`` API (see
`distributed communication docs
<https://blacksmith.org/docs/stable/distributed.html>`__).

This store is only shared by the members of the completed rendezvous. It
is intended to be used by Smith Distributed Elastic to exchange information
necessary to initialize job control and data-planes.

**Waiting workers and rendezvous closing**:

Smith Distributed Elastic rendezvous handler object provides additional
...
Smith Distributed Elastic comes with the :py:class:`.DynamicRendezvousHandler`
...
Smith distributed users can either implement their own backend type or use one
of the following implementations that come with Blacksmith:
