...
class EtcdServer:
    ...
    1. Uses env var SMITHELASTIC_ETCD_BINARY_PATH
    ...
    def __init__(self, data_dir: str | None = None):
        ...
        self._etcd_binary_path = os.environ.get(
            "SMITHELASTIC_ETCD_BINARY_PATH", default_etcd_bin
        )
        if not os.path.isfile(self._etcd_binary_path):
            self._etcd_binary_path = "etcd"

        self._base_data_dir = (
            data_dir if data_dir else tempfile.mkdtemp(prefix="smithelastic_etcd_data")
        )
        self._etcd_cmd = None
