...
from smith.distributed.debug._store import get_world_size, tcpstore_client
from smith.distributed.flight_recorder.components.builder import build_db
from smith.distributed.flight_recorder.components.config_manager import JobConfig
from smith.distributed.flight_recorder.components.types import (
    Collective,
    Group,
    Membership,
    NCCLCall,
...
<!doctype html>
<head>
    <title>{% block title %}{% endblock %} - Blacksmith Distributed</title>
    <link rel="shortcut icon" type="image/x-icon" href="https://blacksmith.org/favicon.ico?">
...
<nav>
    <h1>Smith Distributed Debug Server</h1>
    ...
    <a href="/profile">smith profiler</a> <!--@lint-ignore-->
...
{% extends "base.html" %}
{% block header %}
    <h1>{% block title %}smith.profiler{% endblock %}</h1>
{% endblock %}
...
class FrontendServer:
    ...
    def _handle_profiler(self, req: HTTPRequestHandler) -> bytes:
        ...
        addrs, resps = fetch_all("smith_profile", f"duration={duration}")
