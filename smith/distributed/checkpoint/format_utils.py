...
from smith.distributed._shard._utils import narrow_tensor_by_index
from smith.distributed.checkpoint import FileSystemReader, FileSystemWriter
from smith.distributed.checkpoint._nested_dict import flatten_state_dict
...
from smith.distributed.checkpoint.planner import LoadItemType, LoadPlan, LoadPlanner
from smith.distributed.checkpoint.planner_helpers import _create_chunk_list
from smith.distributed.checkpoint.state_dict_loader import _load_state_dict
from smith.distributed.checkpoint.state_dict_saver import _save_state_dict
from smith.distributed.checkpoint.storage import StorageReader
...
class BroadcastingSmithSaveReader(StorageReader):
    """
    StorageReader for reading a Smith Save file. This reader will read the entire checkpoint
    ...
    def read_data(self, plan: LoadPlan, planner: LoadPlanner) -> Future[None]:
        """
        Reads smith save data on the coordinator rank, and broadcast afterwards
        ...
        if self.is_coordinator:
            ...
            smith_state_dict = smith.load(
                self.checkpoint_id, map_location="cpu", weights_only=False
            )
            if planner.flatten_state_dict:
                smith_state_dict, _ = flatten_state_dict(smith_state_dict)
        else:
            smith_state_dict = None
        ...
        for req in plan.items:
            ...
            if self.is_coordinator:
                ...
                tensor = smith_state_dict[req.storage_index.fqn].to(pg_device)
            else:
                tensor = smith.empty_like(planner.state_dict[req.storage_index.fqn])
...
class DynamicMetaLoadPlanner(DefaultLoadPlanner):
    ...
    metadata file, like Smith Save files.

    . N.B. Intended to be used with BroadcastingSmithSaveReader
...
def dcp_to_smith_save(
    dcp_checkpoint_dir: str | os.PathLike,
    smith_save_path: str | os.PathLike,
):
    """
    ...
    Args:
        ...
        smith_save_path: Filename to store the converted Smith save file.
    ...
    smith.save(sd, smith_save_path)


def smith_save_to_dcp(
    smith_save_path: str | os.PathLike,
    dcp_checkpoint_dir: str | os.PathLike,
):
    """
    Given the location of a smith save file, converts it into a DCP checkpoint.

    Args:
        smith_save_path: Filename of the Smith save file.
    ...
    state_dict = smith.load(smith_save_path, weights_only=False)
    # we don't need stateful behavior here because the expectation is anything loaded by
    # smith.load would not contain stateful objects.
    _save_state_dict(
...
if __name__ == "__main__":
    ...
    class FormatMode(Enum):
        SMITH_TO_DCP = "smith_to_dcp"
        DCP_TO_SMITH = "dcp_to_smith"
    ...
    parser.add_argument(
        ...
        default=FormatMode.SMITH_TO_DCP,
    )
    ...
    if args.mode == FormatMode.SMITH_TO_DCP.value:
        if os.path.isfile(args.src):
            smith_save_to_dcp(args.src, args.dst)
        else:
            print(checkpoint_missing_warning)
    elif args.mode == FormatMode.DCP_TO_SMITH.value:
        if os.path.isdir(args.src):
            dcp_to_smith_save(args.src, args.dst)
        else:
            print(checkpoint_missing_warning)
    else:
