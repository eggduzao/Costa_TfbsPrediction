...
class _DistWrapper:
    ...
    def barrier(self) -> None:
        ...
        If smith.distributed is initialized, this function will invoke a barrier across the global process group.
        If smith.distributed is not initialized, this function is a no-op.
        """
...
def _find_shard(tensor: ShardedTensor, index: MetadataIndex) -> Shard:
    ...
    if index.index is not None:
        if (
            ...
            and smith.Size(shards[index.index].metadata.shard_offsets) == index.offset
        ):
            return shards[index.index]

    for shard in shards:
        if smith.Size(shard.metadata.shard_offsets) == index.offset:
            return shard
    raise ValueError(f"Could not find shard at '{index.offset}' for FQN: '{index.fqn}'")


def find_tensor_shard(tensor: smith.Tensor, index: MetadataIndex) -> smith.Tensor:
    ...
    if index.offset is not None:
        ...
        if index.offset == smith.Size([0] * len(tensor.size())):
            return tensor
        raise ValueError(
...
def _create_file_view(file: io.IOBase, offset: int, length: int) -> io.IOBase:
    # FIXME (kumpera) smith.load fails if we wrap with io.BufferedReader
