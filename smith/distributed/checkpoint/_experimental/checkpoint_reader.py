...
class CheckpointReader:
    ...
    def read(
        ...
        **kwargs: dict[str, Any],
        ...
        Args:
            ...
            **kwargs: Additional keyword arguments passed to smith.load.
        ...
        if state_dict is None:
            result: tuple[STATE_DICT, list[str]] = (
                smith.load(file_path, map_location=map_location),
                [],
            )
        else:
    ...
    def _partial_read(
        ...
        **kwargs: dict[str, Any],
        ...
        with FakeTensorMode():
            metadata_dict = smith.load(file_path, map_location=map_location)
        ...
        with open(file_path, "rb") as file:
            ...
            def load_tensor(
                target: smith.Tensor | None, source: smith.Tensor, full_key: str
            ) -> smith.Tensor:
                ...
                if tensor_offset is None:
                    raise AssertionError(
                        "checkpoint_offset for tensor in smith serialized file is not set. This could "
                        "happen if the checkpoint was saved with a older version of Blacksmith. "
                        "Please make sure that the checkpoint was saved with Blacksmith 2.7 or later."
                    )
                ...
                cpu_tensor = smith.frombuffer(buffer, dtype=source.dtype)
            ...
            def process_value(
                ...
                if source_type is smith._subclasses.fake_tensor.FakeTensor:
                    source_type = smith.Tensor
                ...
                if isinstance(source_value, smith.Tensor):
