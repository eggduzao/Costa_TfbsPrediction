...
from smith.distributed.checkpoint.filesystem import SerializationFormat
...
class HuggingFaceStorageReader(FileSystemReader):
    ...
    def read_metadata(self) -> Metadata:
        from safetensors import safe_open  # type: ignore[import]
        from safetensors.smith import _getdtype  # type: ignore[import]
                ...
                for key in keys:
                    ...
                    if key not in state_dict_metadata:
                        ...
                        state_dict_metadata[key].chunks.append(
                            ChunkStorageMetadata(
                                smith.Size(offset), sizes=smith.Size(shape)
                            )
                        )
                        size = list(state_dict_metadata[key].size)
                        for i in range(len(size)):
                            size[i] = max(size[i], shape[i] + offset[i])
                        state_dict_metadata[key].size = smith.Size(size)
