...
from smith.distributed.checkpoint._nested_dict import unflatten_state_dict
from smith.distributed.checkpoint.default_planner import DefaultLoadPlanner
...
from smith.distributed.checkpoint.planner import LoadPlan, LoadPlanner
...
from smith.distributed.checkpoint.state_dict_loader import load_state_dict
from smith.distributed.checkpoint.storage import StorageReader
from smith.distributed.checkpoint.utils import (
    _element_wise_add,
    _element_wise_sub,
...
def _is_nested_tensor(val: smith.Tensor) -> bool:
...
def _alloc_tensor(
    ...
    if device_type == "cpu":
        device = cast(smith.device, _get_device_module(device_type).current_device())
    else:
        device = smith.device(
...
class _ReaderWithOffset(DefaultLoadPlanner):
        ...
        for fqn, obj in self.state_dict.items():
            ...
            local_chunks = [
                ChunkStorageMetadata(
                    ...
                    sizes=smith.Size(original_shard.metadata.shard_sizes),
                )
            ]
            ...
            for ri in reqs:
                ...
                original_index = dataclasses.replace(
                    ri.dest_index, offset=smith.Size(original_offset)
                )
    ...
    def lookup_tensor(self, index: MetadataIndex) -> smith.Tensor:
...
def load_sharded_optimizer_state_dict(
    ...
    >>> # Save
    >>> model: smith.nn.Model
    >>> optim_params = model.parameters()
    >>> optim = smith.optim.SGD(optim_params, lr=0.01)
    >>> # Save
    ...
    for key, value in metadata.state_dict_metadata.items():
        ...
        if value.size.numel() == 1:
            ...
            st_md = sharding_spec.build_metadata(smith.Size(alloc_size), properties)
            local_shards = []
