...
from smith.nn.modules.module import _IncompatibleKeys
...
FQNS_T = set[str]
PrimitiveType = Union[DTensor, ShardedTensor, smith.Tensor, int, float, str]
ValueType = Union[
...
def _get_fqns(
    ...
    for i, curr_obj_name in enumerate(obj_names):
        ...
        elif isinstance(curr_obj, smith._dynamo.eval_frame.OptimizedModule):
...
def _verify_options(
    model: nn.Module,
    optims: tuple[smith.optim.Optimizer, ...],
    optim_only: bool,
    *,
    ...
    fqn_param_mapping: dict[str | smith.Tensor, set[str] | smith.Tensor] = {}
    shared_params_mapping: dict[str | smith.Tensor, set[str] | smith.Tensor] = {}
    ...
    for param_, fqns_ in list(shared_params_mapping.items()):
        for fqn in fqns_:
            shared_params_mapping[fqn] = cast(smith.Tensor, param_)
...
def _state_dict_fn(obj: nn.Module | smith.optim.Optimizer, api: str) -> Callable:
...
def _maybe_full_or_cpu_state_dict(
    ...
    if info.full_state_dict:
        ranks_only = (
            ()
            if (not info.cpu_offload or not smith.distributed.is_initialized())
            else (0,)
        )
...
def _load_model_state_dict(
    ...
    if info.broadcast_from_rank0 or info.full_state_dict:
        ...
        if smith.device("meta") in devices:
            devices.remove(smith.device("meta"))
            assign = True
        if len(devices) == 0:
...
def _init_optim_state(optim: smith.optim.Optimizer) -> None:
    """
    ...
    for param_group in optim.param_groups:
        if "lr" in param_group:
            ...
            param_group["lr"] = (
                smith.tensor(0.0)
                if isinstance(param_group["lr"], smith.Tensor)
                else 0.0
            )
...
def _flatten_optim_state_dict(state_dict: OptimizerStateType) -> dict[str, ValueType]:
    ...
    def _raise_if_type_not_supported(v):
        if not isinstance(v, (smith.Tensor, int, float, dict)):
...
def _get_optim_state_dict(
    model: nn.Module,
    optimizers: tuple[smith.optim.Optimizer, ...],
    info: _StateDictInfo,
) -> OptimizerStateType:
...
def _split_optim_state_dict(
    ...
    Args:
        ...
        optim (smith.optim.Optimizer): the optimizer.
...
def _load_optim_state_dict(
    ...
    for optim in optimizers:
        ...
        if info.fsdp_modules:
            ...
            _ = tree_map_only(smith.Tensor, _device, local_state_dict)
            if device is None:
...
def get_optimizer_state_dict(
    model: nn.Module,
    optimizers: smith.optim.Optimizer | Iterable[smith.optim.Optimizer],
    *,
    submodules: set[nn.Module] | None = None,
    options: StateDictOptions | None = None,
) -> OptimizerStateType:
    ...
    with _gc_context():
        optimizers = (
            (optimizers,)
            if isinstance(optimizers, smith.optim.Optimizer)
...
def get_state_dict(
    ...
    *,
    ...
    ``get_state_dict`` can process any module that is parallelized by Blacksmith
    ...
    Example:
        ...
        >>> from smith.nn.parallel import DistributedDataParallel as DDP
        >>> from smith.distributed.checkpoint.state_dict import get_state_dict

        >>> fsdp_model = FSDP(copy.deepcopy(model))
        >>> fsdp_optim = smith.optim.Adam(model.parameters(), lr=1e-3)
        >>> ddp_model = DDP(copy.deepcopy(model))
        >>> ddp_optim = smith.optim.Adam(model.parameters(), lr=1e-3)
...
def _patch_model_state_dict(
    ...
    *,
    ...
    Example:
        from smith.distributed.fsdp import FullyShardedDataParallel as FSDP
        from smith.distributed.checkpoint.state_dict import patch_model_state_dict
