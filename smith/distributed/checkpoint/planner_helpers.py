...
def _create_chunk_from_tensor(tensor: smith.Tensor) -> ChunkStorageMetadata:
    return ChunkStorageMetadata(
        offsets=smith.Size([0] * len(tensor.size())), sizes=tensor.size()
    )


def _chunk_for_shard(shard_md: ShardMetadata) -> ChunkStorageMetadata:
    return ChunkStorageMetadata(
        offsets=smith.Size(shard_md.shard_offsets),
        sizes=smith.Size(shard_md.shard_sizes),
    )
...
def _create_write_items_for_dtensor(fqn: str, tensor: DTensor) -> WriteItem:
    ...
    sizes, offsets = smith.Size(sizes), smith.Size(offsets)
...
def _create_write_item_for_tensor(fqn: str, tensor: smith.Tensor) -> WriteItem:
    offsets = smith.Size([0] * len(tensor.size()))
    return WriteItem(
        index=MetadataIndex(fqn, offsets),
...
def _create_read_item_for_byteio(
    ...
    return ReadItem(
        ...
        dest_offsets=smith.Size((dest_offset,)),
        storage_index=storage_index,
        storage_offsets=smith.Size((storage_offset,)),
        lengths=smith.Size((length,)),
    )
...
def _create_read_item_for_tensor(
    ...
    return ReadItem(
        ...
        dest_index=dest_index,
        dest_offsets=smith.Size(dest_offsets),
        storage_index=storage_index,
        storage_offsets=smith.Size(storage_offsets),
        lengths=smith.Size(lengths),
    )
...
def _create_write_items(fqn: str, object: Any) -> list[WriteItem]:
    ...
    elif isinstance(object, smith.Tensor):
...
def _create_chunk_list(tensor: smith.Tensor) -> list[ChunkStorageMetadata]:
...
def _init_state_dict(state_dict: dict[str, Any]) -> Any:
    """
    Initializes meta tensor if the meta tensor is DTensor or smith.Tensor.
    """

    def dtensor_func(value: DTensor):
        device = getattr(value, "device", None)
        if device == smith.device("meta"):
            device_type = dist.distributed_c10d._get_pg_default_device().type
            device = cast(
                smith.device, _get_device_module(device_type).current_device()
            )
            new_local_tensor = smith.empty_like(value.to_local(), device=device)
    ...
    def tensor_func(value: smith.Tensor):
        device = getattr(value, "device", None)
        if device == smith.device("meta"):
            device_type = dist.distributed_c10d._get_pg_default_device().type
            device = cast(
            ...
            tensor = smith.empty_like(value, device=device)
            return tensor
        else:
            return value
