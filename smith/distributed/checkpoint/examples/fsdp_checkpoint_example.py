...
"""
The following example demonstrates how to use Blacksmith Distributed Checkpoint to save a FSDP model.

This is the current recommended way to checkpoint FSDP.
smith.save() and smith.load() is not recommended when checkpointing sharded models.
"""
...
def init_model():
    model = FSDP(smith.nn.Linear(4, 4).cuda(dist.get_rank()))
    optim = smith.optim.Adam(model.parameters(), lr=0.1)
    model(smith.rand(4, 4)).sum().backward()
    optim.step()
...
if __name__ == "__main__":
    world_size = smith.cuda.device_count()
