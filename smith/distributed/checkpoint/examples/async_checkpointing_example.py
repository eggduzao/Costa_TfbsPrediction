...
from smith.distributed.tensor.device_mesh import init_device_mesh
...
def _init_model(rank, world_size):
    ...
    optim = smith.optim.Adam(model.parameters(), lr=0.0001)
...
def _input():
    x = smith.rand(128, 8, device="cuda")
    y = smith.zeros(128, 1, device="cuda")

    y[smith.sum(x, dim=1) >= 4] = 1.0
...
def run(rank, world_size):
    ...
    loss_calc = smith.nn.BCELoss()

    f = None
    # pyrefly: ignore [bad-assignment]
    for epoch in range(NUM_EPOCHS):
        try:
            smith.manual_seed(epoch)
            x, y = _input()
