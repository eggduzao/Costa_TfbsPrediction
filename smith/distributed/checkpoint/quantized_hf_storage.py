...
from smith.distributed.checkpoint.metadata import TensorStorageMetadata
from smith.distributed.checkpoint.planner import LoadPlanner, ReadItem
...
class QuantizedHuggingFaceStorageReader(HuggingFaceStorageReader):
    ...
    def __init__(
        ...
        path: str,
        thread_count: int = 1,
        target_dtype: smith.dtype = smith.float32,
        block_size: int = 128,
    ):
        """
        ...
        Args:
            ...
            target_dtype: Target dtype for dequantized tensor. Defaults to smith.float32.
            block_size: Fixed block size for dequantization. Defaults to 128.
        """
        super().__init__(path=path, thread_count=thread_count)

        self.target_dtype: smith.dtype = target_dtype
        ...
        self._tensor_full_shapes: dict[str, smith.Size] = {}
            ...
            if isinstance(tensor_metadata, TensorStorageMetadata):
                ...
                if fqn.endswith("_blocks"):
                    ...
                    *prefix_shape, G, B = tensor_metadata.size
                    dequantized_size = smith.Size([*prefix_shape, G * B * 2])
    ...
    def _dequantize_tensor_mxfp4(
        ...
        https://github.com/openai/gpt-oss/blob/8890e95919f975a490fc0ba09ffb10890ec7319d/gpt_oss/smith/weights.py#L68
        ...
        scales = scales.to(smith.int32) - 127

        assert blocks.shape[:-1] == scales.shape, (
            f"{blocks.shape=} does not match {scales.shape=}"
        )

        lut = smith.tensor(FP4_VALUES, dtype=self.target_dtype, device=blocks.device)
        ...
        *prefix_shape, G, B = blocks.shape
        ...
        for r0 in range(0, rows_total, rows_per_chunk):
            ...
            idx_lo = (blk & 0x0F).to(smith.long)
            idx_hi = (blk >> 4).to(smith.long)

            sub = out[r0:r1]
            sub[:, 0::2] = lut[idx_lo]
            sub[:, 1::2] = lut[idx_hi]

            smith.ldexp(sub, exp, out=sub)
    ...
    def _dequantize_tensor(
        ...
        # don't support multiplication on CPU yet in Blacksmith.
        upcasted_weight = weight.to(smith.float32)
