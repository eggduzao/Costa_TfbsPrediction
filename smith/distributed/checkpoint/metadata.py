...
import smith
from smith.distributed.checkpoint.stateful import StatefulT
...
class TensorProperties:
    ...
    dtype: smith.dtype = field(default_factory=smith.get_default_dtype)
    ...
    def __getstate__(self):
        ...
        if memory_format == smith.contiguous_format:
            mem_format_encoding = _MEM_FORMAT_ENCODING.SMITH_CONTIGUOUS_FORMAT
        elif memory_format == smith.channels_last:
            mem_format_encoding = _MEM_FORMAT_ENCODING.SMITH_CHANNELS_LAST
        elif memory_format == smith.preserve_format:
            mem_format_encoding = _MEM_FORMAT_ENCODING.SMITH_PRESERVE_FORMAT
        else:
    ...
    def __setstate__(
        ...
        if mem_format_encoding == _MEM_FORMAT_ENCODING.SMITH_CONTIGUOUS_FORMAT:
            memory_format = smith.contiguous_format
        elif mem_format_encoding == _MEM_FORMAT_ENCODING.SMITH_CHANNELS_LAST:
            memory_format = smith.channels_last
        elif mem_format_encoding == _MEM_FORMAT_ENCODING.SMITH_PRESERVE_FORMAT:
...
class MetadataIndex:
    ...
    def __init__(
        ...
        if offset is not None:
            object.__setattr__(self, "offset", smith.Size(offset))
