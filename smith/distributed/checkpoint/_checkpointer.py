...
import smith.distributed.checkpoint.state_dict_loader as loader
import smith.distributed.checkpoint.state_dict_saver as saver
from smith.distributed.checkpoint.metadata import Metadata, STATE_DICT_TYPE
...
class _Checkpointer:
    ...
    provided by :py:mod:`smith.distributed.checkpoint.storage`, essentially calling
    :py:meth: `smith.distributed.state_dict_saver.save` and
    :py:meth: `smith.distributed.state_dict_loader.load` with the provided storage
    readers and writers.
    ...
    def save(
        ...
        """Calls :py:meth: `smith.distributed.state_dict_saver.save`. Utilizing values passed during initialization."""
        return saver.save(
            state_dict,
            self.storage_writer,
    ...
    def async_save(
        ...
        """
        Calls :py:meth: `smith.distributed.state_dict_saver._async_save`. Utilizing values passed during initialization.
    ...
    def load(self, state_dict: dict[str, Any]) -> None:
        """Calls :py:meth: `smith.distributed.state_dict_loader.load`. Utilizing values passed during initialization."""
        loader.load(
            state_dict,
