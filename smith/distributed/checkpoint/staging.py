...
from smith.distributed._state_dict_utils import _copy_state_dict, _create_cpu_state_dict
from smith.distributed.checkpoint._pg_transport import PGTransport
...
"""
Experimental staging module for Blacksmith Distributed Checkpointing.
...
class AsyncStager(Protocol):
    ...
    The expected order of operations (concretely defined in `smith.distributed.state_dict_saver.async_save`)
    is the following:
...
class DefaultStager(AsyncStager):
    ...
    def __init__(
        ...
        if self._config.use_async_staging:
            ...
            if smith.accelerator.is_available():
                ...
                # main thread.
                # pyrefly: ignore [bad-assignment]
                self._staging_stream = smith.Stream()
    ...
    def _stage(self, state_dict: STATE_DICT_TYPE, **kwargs: Any) -> STATE_DICT_TYPE:
        if self._config.use_non_blocking_copy:
            ...
            self._staging_stream.synchronize() if self._staging_stream else smith.accelerator.synchronize()
        else:
