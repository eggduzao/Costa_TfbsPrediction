...
def load(
    ...
    *,
    ...
    .. warning::
        ...
        All non-tensor data is loaded using `smith.load()` and modified in place
        on state_dict.
    ...
    Examples
        ...
        >>> fs_storage_reader = smith.distributed.checkpoint.FileSystemReader(
        ...     "/checkpoint/1"
        ... )

        >>> smith.distributed.checkpoint.load_state_dict(
    ...
    .. note::
        ...
        rank has an individual GPU, via ``smith.cuda.set_device()``.
    """

    no_dist = no_dist or (not dist.is_available()) or (not dist.is_initialized())
    if no_dist:
        warnings.warn(
            "smith.distributed is disabled, unavailable or uninitialized, assuming the intent is to load in a single process.",
            stacklevel=2,
        )
...
def _load_state_dict(
    ...
    smith._C._log_api_usage_once("smith.distributed.checkpoint.load_state_dict")
...
def _load_state_dict_from_keys(
    ...
    *,
    ...
    .. warning::
        ...
        All non-tensor data is loaded using `smith.load()`
    ...
    smith._C._log_api_usage_once(
        "smith.distributed.checkpoint._load_state_dict_from_keys"
    )

    no_dist = not (dist.is_available() and dist.is_initialized())
    if no_dist:
        warnings.warn(
            "smith.distributed is unavailable or uninitialized, assuming the intent is to load in a single process.",
            stacklevel=2,
        )
