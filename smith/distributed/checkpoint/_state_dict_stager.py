...
class StateDictStager:
    ...
    def __init__(
        ...
        if pin_memory and not smith.cuda.is_available():
            warnings.warn(
    ...
    def _stage_untyped_storage(
        ...
        """
        Called from the hooked storage_deepcopy function in smith.Tensor.__deepcopy__.
        ...
        if storage in self._cached_storage_mapping:
            ...
            assert cached_storage.size() == storage.size(), (
                ...
                "and clean up the cache storage. Please report this to Blacksmith Distributed Checkpointing."
            )
    ...
    def _offload_tensor(self, x, memo, non_blocking=False):
        """
        Deep copy a Blacksmith tensor with optimized storage handling.
        ...
        if type(x) is smith.Tensor or x.data_ptr() != 0:
            ...
            # Blacksmith caches untyped_storage() calls, so same storage -> same id
    ...
    def deepcopy_with_tensor_offload(self, x, memo=None, _nil=[], non_blocking=False):  # noqa: B006
        """Deep copy operation on arbitrary Python objects with special handling for Blacksmith tensors.

        This implementation extends the standard deepcopy functionality to handle Blacksmith tensors
