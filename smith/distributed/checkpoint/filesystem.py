...
from smith._utils import _get_available_device_type, _get_device_module
from smith.distributed._shard._utils import narrow_tensor_by_index
from smith.distributed.checkpoint._extension import (
    ExtensionRegistry,
    StreamTransformExtension,
)
from smith.distributed.checkpoint._hf_utils import (
    CUSTOM_METADATA_KEY,
    DCP_VERSION_KEY,
    FORMAT_KEY,
    FORMAT_VALUE,
...
from smith.distributed.checkpoint.metadata import Metadata, STATE_DICT_TYPE, StorageMeta
...
from smith.distributed.checkpoint.staging import BlockingAsyncStager
from smith.distributed.checkpoint.storage import (
    StorageReader,
    StorageWriter,
    WriteResult,
)
from smith.distributed.checkpoint.utils import _create_file_view
...
class _TensorLoader(ABC):
    ...
    @abstractmethod
    def values(self) -> Iterator[tuple[smith.Tensor, object]]:
        pass
...
class _OverlappingCpuLoader(_TensorLoader):
    def __init__(
        ...
        self.stream = cast(
            smith.cuda.Stream, stream or self.device_module.current_stream()
        )
    ...
    def _drain(self) -> list[tuple[smith.Tensor, object]]:
        drained = []
    ...
    def _refill(self) -> None:
        with self.device_module.stream(self.stream):
            while not self._done and self.in_flight_data < self.inflight_threshhold:
                ...
                elif tensor.device == smith.device("cpu"):
                    if (
    ...
    def _finish(self) -> Iterable[tuple[smith.Tensor, object]]:
        if not self._done:
...
def _item_size(item: WriteItem) -> int:
    ...
    return size * smith._utils._element_size(dtype)
...
def _write_item(
    ...
    if write_item.type == WriteItemType.BYTE_IO:
        ...
        if not isinstance(data, smith.Tensor):
            raise AssertionError(
                "Data must be smith.Tensor for non-BYTE_IO write items"
            )
        if data.device != smith.device("cpu"):
            raise AssertionError("Tensor must be on CPU device")
        if serialization_format == SerializationFormat.SMITH_SAVE:
            smith.save(data, transform_to)

    transform_to.close()

    if serialization_format == SerializationFormat.SMITH_SAVE or isinstance(
        data, io.BytesIO
    ):
        ...
        while True:
            ...
            with create_stream(file_name, "wb") as stream:
                ...
                if serialization_format == SerializationFormat.SAFETENSORS:
                    from safetensors.smith import save  # type: ignore[import-not-found]
...
class _FileSystemWriter(StorageWriter):
    ...
    * File creation is atomic
    ...
    def __init__(
        ...
        serialization_format: SerializationFormat = SerializationFormat.SMITH_SAVE,
        *args: Any,
        **kwargs: Any,
    ) -> None:
        """
        ...
        if self._metadata_exists():
            if self.overwrite:
                warnings.warn(
                    ...
                    " Past version 2.5 of Blacksmith, `overwrite` will default to False. Set this variable to True to"
                ...
                for req in reqs:
                    ...
                    if req.type == LoadItemType.BYTE_IO:
                        ...
                        if transform_from.seekable():
                            ...
                            # smith.load requires a seekable input, so read the transform
