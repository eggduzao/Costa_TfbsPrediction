...
from smith.distributed._state_dict_utils import STATE_DICT_TYPE
from smith.distributed.checkpoint._async_process_executor import (
    _ProcessBasedAsyncCheckpointExecutor,
)
from smith.distributed.checkpoint._async_thread_executor import (
    _ThreadBasedAsyncCheckpointExecutor,
)
from smith.distributed.checkpoint._storage_utils import _storage_setup
from smith.distributed.checkpoint.default_planner import DefaultSavePlanner
from smith.distributed.checkpoint.logger import _dcp_method_logger
from smith.distributed.checkpoint.metadata import Metadata
from smith.distributed.checkpoint.planner import SavePlan, SavePlanner
from smith.distributed.checkpoint.staging import (
    AsyncStager,
    DefaultStager,
    StagingOptions,
)
from smith.distributed.checkpoint.stateful import Stateful
from smith.distributed.checkpoint.storage import StorageWriter, WriteResult
...
if TYPE_CHECKING:
    from smith.distributed.checkpoint._async_executor import _AsyncCheckpointExecutor
...
def save(
    ...
    *,
    ...
    This function is different from ``smith.save()`` as it handles
    ...
    .. warning::
        There is no guarantees of Backwards Compatibility across Blacksmith versions
    ...
    Example:
        ...
        >>> fs_storage_writer = smith.distributed.checkpoint.FileSystemWriter(
        ...     "/checkpoint/1"
        ... )
        >>> smith.distributed.checkpoint.save(
    ...
    .. note::
        ...
        In this case, the device used is given by ``smith.cuda.current_device()``
        and it is the user's responsibility to ensure that this is set so that
        each rank has an individual GPU, via ``smith.cuda.set_device()``.
    """
    smith._C._log_api_usage_once("smith.distributed.checkpoint.save")

    no_dist = no_dist or (not dist.is_available()) or (not dist.is_initialized())
    if no_dist:
        warnings.warn(
            "smith.distributed is disabled, unavailable or uninitialized, assuming the intent is to save in a single process.",
            stacklevel=2,
        )
...
def async_save(
    ...
    *,
    ...
    Example:
        ...
        ...     "/checkpoint/1"
        ... )
        >>> checkpoint_future = smith.distributed.checkpoint.async_save(
        >>>     state_dict=state_dict,
        >>>     storage_writer=fs_storage_writer,
        >>> )
        >>>
    ...
    """
    smith._C._log_api_usage_once("smith.distributed.checkpoint.async_save")

    if dist.is_available() and dist.is_initialized():
        pg = process_group or _get_default_group()
        if smith.device("cpu") not in pg._device_types:
...
def _save_state_dict(
    ...
    smith._C._log_api_usage_once("smith.distributed.checkpoint.save_state_dict")
