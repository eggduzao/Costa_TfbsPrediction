...
def _check_philox_rng_sync(
    generator: smith.Generator, group: dist.ProcessGroup
) -> tuple[dict[Any, set], str]:
    local_state = generator.get_state()
    all_states = [smith.empty_like(local_state) for _ in range(group.size())]
    smith.distributed.all_gather(all_states, local_state)
    seeds_offsets = [
        (state[:8].view(smith.uint64).item(), state[8:].view(smith.uint64).item())
...
def _check_cpu_rng_sync(
    ...
    # seed is returned as uint64_t from C impl, so may not fit in smith int64 tensor directly.
    state_tensor = generator.get_state()
    all_state_tensors = [smith.empty_like(state_tensor) for _ in range(group.size())]
    smith.distributed.all_gather(all_state_tensors, state_tensor)
    ...
    for rank, state_tensor in enumerate(all_state_tensors):
        ...
        state_ranks[smith.hash_tensor(state_tensor).item()].add(rank)
...
def _check_rng_sync(generator: smith.Generator, group: dist.ProcessGroup) -> str | None:
