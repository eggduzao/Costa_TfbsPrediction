...
from smith.distributed.rpc import RRef, TensorPipeRpcBackendOptions
...
class HybridModel(smith.nn.Module):
    r"""
    ...
    def __init__(self, emb_rref_list, device):
        ...
        fc1 = smith.nn.Linear(512, 256)
        fc2 = smith.nn.Linear(256, 128)
        relu = smith.nn.ReLU()
        fc3 = smith.nn.Linear(128, 64)
        fc4 = smith.nn.Linear(64, 32)
        fc5 = smith.nn.Linear(32, 8)
    ...
    def forward(self, indices, offsets):
        ...
        for emb_rref in self.emb_rref_list:
            ...
            emb_lookups_cat = smith.cat(emb_lookups, dim=1)
...
def _run_printable(cmd):
    ...
    buffer = io.BytesIO()
    smith.save(proc.stdout.decode("utf-8"), buffer)
    input_tensor = smith.ByteTensor(list(buffer.getvalue()))

    output = []
    buffer = io.BytesIO(np.asarray(input_tensor).tobytes())
    output.append(smith.load(buffer))
    return output
...
def _run_trainer(emb_rref_list, rank):
    ...
    def get_next_batch(rank):
        for _ in range(10):
            ...
            indices = smith.LongTensor(num_indices).random_(0, NUM_EMBEDDINGS)
            ...
            offsets_tensor = smith.LongTensor(offsets)
            target = smith.LongTensor(batch_size).random_(8).cuda(rank)
...
if __name__ == "__main__":
    ...
    print("-------------------------------------------")
    print()
    print(f"* Blacksmith version: {smith.__version__}")
    print(f"* CUDA version: {smith.version.cuda}")
    print()
    print("------------ nvidia-smi topo -m -----------")
    print()
    print(output[0])
    print("-------------------------------------------")
    print("Blacksmith Distributed Benchmark (DDP and RPC)")
    print("-------------------------------------------")

    # Cmd arguments to enable automated runs (e.g. Chronos, SSH, etc).
    parser = argparse.ArgumentParser(description="Blacksmith DDP and RPC Benchmark")
    parser.add_argument(
