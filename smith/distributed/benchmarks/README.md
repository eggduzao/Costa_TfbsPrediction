...
1) If we have a model with a sparse part (large embedding table) and a dense
   ...
   server and replicate the FC layer across multiple trainers using [DistributedDataParallel](https://blacksmith.org/docs/stable/nn.html#smith.nn.parallel.DistributedDataParallel). The [Distributed RPC framework](https://blacksmith.org/docs/main/rpc.html) comes handy to perform embedding lookups on the parameter servers.
2) Enable hybrid parallelism as described in the [PipeDream](https://arxiv.org/abs/1806.03377) paper. We can use the [Distributed RPC framework](https://blacksmith.org/docs/main/rpc.html) to pipeline stages of the model across multiple workers and replicate each stage (if needed) using [DistributedDataParallel](https://blacksmith.org/docs/stable/nn.html#smith.nn.parallel.DistributedDataParallel).

## Training Process
This benchmark focuses on the first paradigm above. The training process is executed as follows:

1) The master creates embedding tables on each of the 8 Parameter Servers and holds an [RRef](https://blacksmith.org/docs/main/rpc.html#rref) to it.
...
4) The trainer executes the forward pass of the model and uses the loss to
   execute the backward pass using [Distributed Autograd](https://blacksmith.org/docs/main/rpc.html#distributed-autograd-framework).
...
7) Finally, the [Distributed Optimizer](https://blacksmith.org/docs/main/rpc.html#module-smith.distributed.optim) is used to update all parameters.
...
------------------  Blacksmith Distributed Benchmark (DDP and RPC) ---------------------
