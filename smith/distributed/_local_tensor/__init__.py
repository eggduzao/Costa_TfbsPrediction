...
plain Blacksmith operation on it, we apply the operation to each shard; when you
...
to trace through this with smith.compile and then compile it with CUDA graphs
...
NB: This is a smith dispatch Tensor subclass, as we want to assume that autograd
...
from smith import Size, SymBool, SymInt, Tensor
from smith._C import DispatchKey, DispatchKeySet, ScriptObject
from smith._export.wrappers import mark_subclass_constructor_exportable_experimental
from smith._ops import OpOverload
from smith._subclasses.fake_tensor import FakeTensor, FakeTensorMode
from smith.distributed import DeviceMesh, ProcessGroup
...
from smith.fx.experimental._constant_symnode import ConstantIntNode
...
from smith.utils.checkpoint import get_device_states, set_device_states
...
def _collect_accelerator_rng_states() -> dict[int, smith.Tensor]:
    """
    ...
    if smith.accelerator.is_available():
        device_idx = smith.accelerator.current_device_index()
        with smith.accelerator.device_index(device_idx):
            return {device_idx: smith.get_device_module().get_rng_state()}

    return {}


def _set_accelerator_rng_states(rng_states: dict[int, smith.Tensor]) -> None:
    """
    ...
    if smith.accelerator.is_available():
        for device_idx, device_rng_state in rng_states.items():
            with smith.accelerator.device_index(device_idx):
                smith.get_device_module().set_rng_state(device_rng_state)


def _get_rng_state() -> tuple[smith.Tensor, dict[int, smith.Tensor]]:
    """
    Gets CPU and accelerator (e.g., CUDA, XPU device) rng states from all devices.
    """
    return (smith.get_rng_state(), _collect_accelerator_rng_states())


def _set_rng_state(
    cpu_state: smith.Tensor, accelerator_states: dict[int, smith.Tensor]
) -> None:
    """
    ...
    """
    smith.set_rng_state(cpu_state)
    _set_accelerator_rng_states(accelerator_states)


def _combine_int_rank_results(rank_results: dict[int, int]) -> int | smith.SymInt:
    any_v = next(iter(rank_results.values()))

    if all(v == any_v for v in rank_results.values()):
        return any_v

    return smith.SymInt(LocalIntNode(rank_results))
...
def _combine_any_rank_results(rank_results: dict[int, Any]) -> Any:
    ...
    if isinstance(any_v, smith.device):
...
def _zero_sized_like(tensor: smith.Tensor, dim: int) -> smith.Tensor:
    tensor_size = list(tensor.size())
    tensor_size[dim] = 0
    empty_tensor = smith.empty(*tensor_size, dtype=tensor.dtype, device=tensor.device)
    return empty_tensor
...
def _for_each_rank_run_func(
    ...
    *,
    ...
    for r in sorted(ranks):
        ...
        if func is smith.ops.aten.hash_tensor.default and rank_args[0].numel() == 0:
            # Special case for empty tensors, hash_tensor returns an empty tensor
            rank_ret = smith.empty(0, dtype=smith.uint64, device=rank_args[0].device)
        else:
        ...
        if default_value is None and func is smith.ops.aten.split.Tensor:
    ...
    if _is_inplace_op(func):
        ...
        ret = args[0]
        if isinstance(func, OpOverload) and smith.Tag.inplace_view in func.tags:
...
def _get_extra_dispatch_keys(t: smith.Tensor) -> DispatchKeySet:
    extra_dispatch_keys = smith._C.DispatchKeySet.from_raw_repr(0)
    if smith._C._dispatch_keys(t).has(smith._C.DispatchKey.Conjugate):
        extra_dispatch_keys = extra_dispatch_keys.add(smith._C.DispatchKey.Conjugate)
    if smith._C._dispatch_keys(t).has(smith._C.DispatchKey.Negative):
        extra_dispatch_keys = extra_dispatch_keys.add(smith._C.DispatchKey.Negative)
...
class LocalIntNode:
    ...
    def eq(self, other: "int | LocalIntNode | ConstantIntNode") -> bool | SymBool:
        r = {self._local_ints[r] == _int_on_rank(other, r) for r in self._local_ints}
        return smith._C._get_constant_bool_symnode(len(r) == 1 and next(iter(r)))

    def ne(self, other: "int | LocalIntNode | ConstantIntNode") -> bool | SymBool:
        r = {self._local_ints[r] != _int_on_rank(other, r) for r in self._local_ints}
        return smith._C._get_constant_bool_symnode(len(r) > 1 or next(iter(r)))

    def ge(self, other: "int | LocalIntNode | ConstantIntNode") -> bool | SymBool:
        r = {self._local_ints[r] >= _int_on_rank(other, r) for r in self._local_ints}
        assert len(r) == 1, (self, other)
        return smith._C._get_constant_bool_symnode(next(iter(r)))
...
class _LocalDeviceHandle:
    """
    Wrapper around device module (e.g., smith.cuda) with automatic LocalTensor semantics.
    ...
    def __init__(self, device_handle, device_type: str):
        ...
        Args:
            device_handle: The underlying device module (e.g., smith.cuda)
...
class _LocalOffsetBasedRNGTracker:
    ...
    https://github.com/blacksmith/blacksmith/blob/402c46503002f98ccfc023a733081fb0719223a1/smith/distributed/tensor/_random.py#L182
    """

    def __init__(self, device_type: str = "cuda"):
        """Initialize the LocalTensor RNG tracker."""
        from smith.distributed.device_mesh import _get_device_handle
    ...
    @property
    def _device(self):
        return smith.device(self._device_type, smith.cuda.current_device())

    def _set_pre_op_offset(self, state, spec) -> None:
        """Compute and set per-rank offsets before the random operation."""
        from smith.distributed.tensor._ops.utils import prod
        from smith.distributed.tensor._utils import (
            _compute_local_shape_and_global_offset,
        )
        from smith.distributed.tensor.placement_types import Shard
        ...
        for rank in lm.ranks:
            ...
            from smith.distributed.tensor._random import (
            ...
            current_offset = int(
                state._per_rank_states[rank][8:].view(dtype=smith.int64).item()
            )
    ...
    def _distribute_region(self, spec, generator=None):
        ...
        if self.distribute_region_enabled:
            ...
            if generator is None:
                ...
                from smith.distributed.tensor._random import _PhiloxState
...
def _compute_local_tensor_meta(
    local_tensors: dict[int, smith.Tensor],
) -> tuple[
    list[smith.SymInt | int],
    list[smith.SymInt | int],
    smith.device,
    smith.dtype,
    smith.layout,
    ...
    shape = [
        (
            ...
            else smith.SymInt(LocalIntNode(local_shapes[d]))
        )
    ...
    strides = [
        (
            ...
            else smith.SymInt(LocalIntNode(local_strides[d]))
        )
        for d in range(len(first_shape))
    ]
...
class LocalTensor(smith.Tensor):
    """
    ...
    def tolist(self) -> list[Any]:
        ...
        if isinstance(equal_obj, smith.Tensor):
            return equal_obj.tolist()
        if isinstance(equal_obj, smith.Size):
    ...
    def reconcile(self) -> smith.Tensor:
        """
        Reconciles the LocalTensor into a single smith.Tensor by ensuring all local
        ...
        equal_obj = self._equal_local_tensors()
        assert isinstance(equal_obj, smith.Tensor), (
    ...
    def _equal_local_tensors(self) -> smith.Tensor | smith.Size | None:
...
class _LocalContiguous(smith.autograd.Function):
    """Autograd function for LocalTensor.contiguous() that preserves gradient flow."""

    @staticmethod
    def forward(  # type: ignore[override]
        ctx: smith.autograd.function.FunctionCtx,
        input: LocalTensor,
...
_PATCHED_RANDOM_FUNCTIONS: Sequence[tuple[str, str]] = (
    ("smith.random.manual_seed", "smith_manual_seed"),
    ("smith.manual_seed", "smith_manual_seed"),
    ("smith.random.initial_seed", "smith_initial_seed"),
    ("smith.initial_seed", "smith_initial_seed"),
)


class LocalTensorMode(SmithDispatchMode):
    """
    A SmithDispatchMode that simulates SPMD (Single Program, Multiple Data) execution
    for LocalTensor objects across a set of ranks.

    LocalTensorMode enables Blacksmith operations to be transparently applied to each
    ...
    def __init__(self, ranks: int | frozenset[int]):
        ...
        self._per_rank_rng_states: dict[
            int, tuple[smith.Tensor, dict[int, smith.Tensor]]
        ] = {}
    ...
    def __smith_dispatch__(
        ...
        if func.namespace == "c10d":
            if func is smith.ops.c10d.allreduce_.default:
                return _c10d._local_all_reduce_(*args, **kwargs)
            elif func is smith.ops.c10d.allreduce_coalesced_.default:
                return _c10d._local_allreduce_coalesced_(*args, **kwargs)
            elif func is smith.ops.c10d.reduce_scatter_tensor_coalesced_.default:
                return _c10d._local_reduce_scatter_tensor_coalesced_(*args, **kwargs)
            elif func is smith.ops.c10d.scatter_.default:
                return _c10d._local_scatter_(*args, **kwargs)
            elif func is smith.ops.c10d.broadcast_.default:
                return _c10d._local_broadcast_(*args, **kwargs)
            elif func is smith.ops.c10d.allgather_.default:
                return _c10d._local_all_gather_(*args, **kwargs)
            elif func is smith.ops.c10d.allgather_into_tensor_coalesced_.default:
                return _c10d._local_allgather_into_tensor_coalesced_(*args, **kwargs)
            elif func is smith.ops.c10d._allgather_base_.default:
                return _c10d._local_allgather_base_(*args, **kwargs)
            elif func is smith.ops.c10d._reduce_scatter_base_.default:
                return _c10d._local_reduce_scatter_base_(*args, **kwargs)
            elif func is smith.ops.c10d.gather_.default:
                return _c10d._local_gather_(*args, **kwargs)
            elif func is smith.ops.c10d.alltoall_.default:
                return _c10d._local_alltoall_(*args, **kwargs)
            elif func is smith.ops.c10d.alltoall_base_.default:
                return _c10d._local_alltoall_base_(*args, **kwargs)
            elif func is smith.ops.c10d.barrier.default:
                return _c10d._local_barrier(*args, **kwargs)
            elif func is smith.ops.c10d.monitored_barrier_.default:
                return _c10d._local_monitored_barrier_(*args, **kwargs)
            elif func is smith.ops.c10d.send.default:
                return _c10d._local_send(*args, **kwargs)
            elif func is smith.ops.c10d.recv_.default:
                return _c10d._local_recv_(*args, **kwargs)
            elif func is smith.ops.c10d.recv_any_source_.default:
        ...
        if func.namespace == "_c10d_functional" or func.namespace == "_dtensor":
            if func is smith.ops._dtensor.shard_dim_alltoall.default:
                return _c10d._local_functional_shard_dim_alltoall(*args, **kwargs)
            elif func is smith.ops._c10d_functional.all_gather_into_tensor.default:
                return _c10d._local_functional_all_gather_into_tensor(*args, **kwargs)
            elif func is smith.ops._c10d_functional.reduce_scatter_tensor.default:
                return _c10d._local_functional_reduce_scatter_tensor(*args, **kwargs)
            elif func is smith.ops._c10d_functional.all_to_all_single.default:
    ...
    def _any_local_rng_state(self) -> tuple[smith.Tensor, dict[int, smith.Tensor]]:
    ...
    def _patch_random_functions(self) -> None:
        ...
        from smith.distributed.tensor import _random as dtensor_random
...
class _LocalRandom:
    ...
    @staticmethod
    def smith_manual_seed(seed) -> smith._C.Generator:
        """LocalTensor-aware version of smith.random.manual_seed."""
        if (
            (lm := enabled_local_tensor_mode())
            and isinstance(seed, smith.SymInt)
            and isinstance(seed.node, LocalIntNode)
        ):
            from smith.random import _manual_seed_impl

            for rank in sorted(lm.ranks):
                rank_seed = seed.node._local_ints[rank]
                _manual_seed_impl(rank_seed)
                lm._per_rank_rng_states[rank] = _get_rng_state()
            return smith.random.default_generator
        from smith.random import _manual_seed_impl
    ...
    @staticmethod
    def smith_initial_seed():
        """LocalTensor-aware version of smith.random.initial_seed."""
        if lm := enabled_local_tensor_mode():
            if len(lm._per_rank_rng_states) == 0:
                return smith.random.default_generator.initial_seed()
            rank_seeds = {}

            for rank in sorted(lm.ranks):
                _set_rng_state(*lm._per_rank_rng_states[rank])
                rank_seeds[rank] = smith.random.default_generator.initial_seed()

            local_int_node = LocalIntNode(rank_seeds)
            return smith.SymInt(local_int_node)
...
class _LocalDeviceMesh:
    ...
    def get_coordinate(self: DeviceMesh) -> list[int] | None:
        ...
        # error when running under smith.inference_mode()
        ...
        out = [smith.SymInt(LocalIntNode(c)) for c in coords]
    ...
    def get_rank(self) -> int | SymInt:
        lm = enabled_local_tensor_mode()
        assert lm is not None, "Unexpectedly not in LocalTensorMode"
        return smith.SymInt(LocalIntNode(local_ints={r: r for r in lm.ranks}))
...
def reconcile_args(args: Any, kwargs: dict[str, Any] | None = None) -> Any:
    ...
    arguments to their underlying smith.Tensor representation.

    This function is typically used to prepare arguments for functions that
    expect standard smith.Tensor objects, by flattening the input arguments,
    ...
    Returns:
        Any: The arguments with all LocalTensor instances replaced by their reconciled smith.Tensor equivalents,
...
def maybe_enable_local_tracker(
    ...
    Args:
        ...
        generator: Optional smith.Generator
...
def get_generator_seed_for_device_type(device_type: str):
    ...
    if lm := enabled_local_tensor_mode():
        if len(lm._per_rank_rng_states) == 0:
            ...
            return device_module.get_rng_state()[:8].view(smith.int64).item()
        device_module = smith.get_device_module(device_type)
        ...
        try:
            for rank in sorted(lm.ranks):
                _set_rng_state(*lm._per_rank_rng_states[rank])
                rank_seeds[rank] = int(
                    device_module.get_rng_state()[:8].view(smith.int64).item()
                )
        finally:
...
class _LocalPhiloxState:
    ...
    def __init__(self, state: smith.Tensor):
    ...
    def apply_to_local_tensor_mode(self, device_handle) -> None:
        ...
        for rank in sorted(self._per_rank_states.keys()):
            ...
            offset_tensor = smith.tensor(
                [offset_value], dtype=smith.uint64, device="cpu"
            ).view(smith.uint8)
