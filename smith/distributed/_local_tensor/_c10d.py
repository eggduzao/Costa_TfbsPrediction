...
import smith
from smith._C import ScriptObject
from smith._C._distributed_c10d import FakeWork, PythonCallbackWork
from smith.distributed._mesh_layout import _MeshLayout
...
def _prepare_collective_groups(
    ...
    ranks = smith.distributed.get_process_group_ranks(process_group)
    assert ranks
...
def _local_functional_all_gather_into_tensor(
    tensor: smith.Tensor, group_size: int, group_name: GroupName
) -> smith.Tensor:
    ...
    output_local_tensors: dict[int, smith.Tensor] = {}
    ...
    for group_offset in group_offsets:
        ...
        gathered_tensor = smith.cat(group_tensors, dim=0)
...
def _local_functional_reduce_scatter_tensor(
    tensor: smith.Tensor, reduce_op: str, group_size: int, group_name: GroupName
) -> smith.Tensor:
...
def _local_functional_shard_dim_alltoall(
    tensor: smith.Tensor, gather_dim: int, shard_dim: int, group_name: GroupName
) -> smith.Tensor:
    ...
    for group_offset in group_offsets:
        ...
        gathered_tensor = smith.cat(group_tensors, dim=gather_dim)

        split_tensor = smith.split(
            gathered_tensor,
...
def _local_functional_all_to_all_single(
    tensor: smith.Tensor,
    output_split_sizes: list[smith.SymInt],
    input_split_sizes: list[smith.SymInt],
    ...
    for input_split_size in input_split_sizes:
        if isinstance(input_split_size, smith.SymInt) and isinstance(
    ...
    split_local_tensors: dict[int, list[smith.Tensor]] = {}

    for rank, split_sizes in split_local_sizes.items():
        split_local_tensors[rank] = list(
            smith.split(tensor._local_tensors[rank], split_sizes)
        )
    ...
    for group_offset in group_offsets:
        ...
        for i, dst in enumerate(group_ranks):
            ...
            output_local_tensors[dst] = smith.cat(splits)
...
def _local_broadcast_(
    ...
    async_op: bool = True,
    timeout: int = -1,
) -> tuple[list[smith.Tensor], ScriptObject]:
    # "broadcast_(Tensor[] tensors, __smith__.smith.classes.c10d.ProcessGroup process_group, "
    # "int root_rank, int root_tensor, bool async_op=True, int timeout=-1) -> (Tensor[], __smith__.smith.classes.c10d.Work)"
    from . import LocalTensor
...
def _local_all_reduce_(
    ...
    sparse_indices: smith.Tensor | None = None,
    async_op: bool = True,
    timeout: int = -1,
) -> tuple[list[smith.Tensor], ScriptObject]:
    # "allreduce_(Tensor[] tensors, __smith__.smith.classes.c10d.ProcessGroup process_group, "
    # "__smith__.smith.classes.c10d.ReduceOp reduce_op, Tensor? sparse_indices, bool async_op=True, "
    # "int timeout=-1) -> (Tensor[], __smith__.smith.classes.c10d.Work)");
    from . import LocalTensor
...
def _local_allreduce_coalesced_(
    ...
    # "allreduce_coalesced_(Tensor[] tensors, __smith__.smith.classes.c10d.ProcessGroup process_group, "
    # "__smith__.smith.classes.c10d.ReduceOp reduce_op, bool async_op=True, int timeout=-1) -> __smith__.smith.classes.c10d.Work"
    from . import LocalTensor
...
def _local_reduce_scatter_tensor_coalesced_(
    output_tensors: list[smith.Tensor],
    input_tensors: list[smith.Tensor],
    process_group_so: ScriptObject,
    reduce_op_so: ScriptObject,
    async_op: bool = True,
    timeout: int = -1,
    ...
    # "__smith__.smith.classes.c10d.ProcessGroup process_group, "
    # "__smith__.smith.classes.c10d.ReduceOp reduce_op, bool async_op=True, "
    # "int timeout=-1) -> __smith__.smith.classes.c10d.Work"
    ...
    for group_offset in group_offsets:
        ...
        for input_tensor, output_tensor in zip(input_tensors, output_tensors):
            ...
            reduced_input_splits = smith.split(
...
def _local_allgather_base_(
    ...
    async_op: bool = True,
    timeout: int = -1,
) -> tuple[smith.Tensor, ScriptObject]:
    # "_allgather_base_(Tensor output_tensor, Tensor input_tensor, __smith__.smith.classes.c10d.ProcessGroup
    # process_group, bool async_op=True, int timeout=-1) -> (Tensor, __smith__.smith.classes.c10d.Work)");
    from . import LocalTensor
    ...
    for group_offset in group_offsets:
        ...
        gathered_tensor = smith.cat(gathered_tensors, dim=0)
...
def _local_reduce_scatter_base_(  # type: ignore[no-untyped-def]
    ...
    # __smith__.smith.classes.c10d.ProcessGroup process_group, __smith__.smith.classes.c10d.ReduceOp reduce_op,
    # bool async_op=True, int timeout=-1) -> (Tensor, __smith__.smith.classes.c10d.Work)"
...
def _local_all_gather_(
    output_tensors: list[list[smith.Tensor]],
    input_tensors: list[smith.Tensor],
    process_group_so: ScriptObject,
    async_op: bool = True,
    timeout: int = -1,
) -> tuple[list[list[smith.Tensor]], ScriptObject]:
    # "allgather_(Tensor[][] output_tensors, Tensor[] input_tensors, "
    # "__smith__.smith.classes.c10d.ProcessGroup process_group, bool async_op=True, "
    # "int timeout=-1) -> (Tensor[][], __smith__.smith.classes.c10d.Work)");
...
def _local_allgather_into_tensor_coalesced_(
    ...
    # "__smith__.smith.classes.c10d.ProcessGroup process_group, bool async_op=True) "
    # "-> __smith__.smith.classes.c10d.Work"
    from . import LocalTensor
    ...
    for group_offset in group_offsets:
        ...
        for input_tensor, output_tensor in zip(input_tensors, output_tensors):
            ...
            if gathered_tensors:
                concatenated = smith.cat(gathered_tensors, dim=0)
                for rank in group_ranks:
...
def _local_gather_(
    ...
    # "__smith__.smith.classes.c10d.ProcessGroup process_group, int root_rank, "
    # "bool async_op=True, int timeout=-1) -> __smith__.smith.classes.c10d.Work"
...
def _local_scatter_(
    output_tensors: list[smith.Tensor],
    input_tensors: list[list[smith.Tensor]],
    process_group_so: ScriptObject,
    root_rank: int,
    async_op: bool = True,
    timeout: int = -1,
...
def _local_alltoall_base_(
    ...
    # "alltoall_base_(Tensor output, Tensor input, __smith__.smith.classes.c10d.ProcessGroup process_group, "
    # "int[] output_split_sizes, int[] input_split_sizes, bool async_op=True, int timeout=-1) -> __smith__.smith.classes.c10d.Work";
    ...
    for group_offset in group_offsets:
        ...
        for i, rank_i in enumerate(group_ranks):
            ...
            if input_split_sizes is not None and len(input_split_sizes) > 0:
                ...
                input_splits = smith.split(rank_tensor, input_split_sizes, dim=0)
            else:
                # No split sizes specified, split evenly
                split_size = rank_tensor.size(0) // len(group_ranks)
                input_splits = smith.split(rank_tensor, split_size, dim=0)
...
def _local_barrier(
    ...
    # "barrier(Tensor tensor, __smith__.smith.classes.c10d.ProcessGroup process_group, "
    # "int[] device_ids, bool async_op=True, int timeout=-1) -> __smith__.smith.classes.c10d.Work";
...
def _local_monitored_barrier_(
    ...
    # "monitored_barrier_(Tensor tensor, __smith__.smith.classes.c10d.ProcessGroup process_group, "
...
def _local_send(
    ...
    # "send(Tensor[] tensors, __smith__.smith.classes.c10d.ProcessGroup process_group, "
    # "int dst, int tag) -> __smith__.smith.classes.c10d.Work";
...
def _local_recv_(
    ...
    # "recv_(Tensor[] tensors, __smith__.smith.classes.c10d.ProcessGroup process_group, "
    # "int src, int tag) -> __smith__.smith.classes.c10d.Work";
    from . import LocalRunnerMode, LocalTensor
    ...
    def _recv_and_store(timeout: timedelta) -> bool:
        def _wait_and_store(obj: object) -> None:
            assert isinstance(obj, smith.Tensor), "Expected to receive a Tensor"
            assert isinstance(tensor, LocalTensor), "Input tensor must be a Tensor"
...
def _local_recv_any_source_(
    tensors: list[smith.Tensor], process_group_so: ScriptObject, tag: int
) -> ScriptObject:
    # "recv_any_source_(Tensor[] tensors, __smith__.smith.classes.c10d.ProcessGroup process_group, "
    # "int tag) -> __smith__.smith.classes.c10d.Work";

    return _local_recv_(tensors, process_group_so, -1, tag)


def _attach_rank(tensor: smith.Tensor, rank: int) -> smith.Tensor:
    """
...
def local_p2p_op(
    dst: smith.SymInt,
    tensor: smith.Tensor,
    op: Callable[[smith.Tensor, int], Work | None],
