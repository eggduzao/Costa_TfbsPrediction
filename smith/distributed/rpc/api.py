...
import smith
from smith._C._distributed_rpc import (
    ...
    _invoke_remote_smithscript,
    _invoke_rpc_builtin,
    _invoke_rpc_python_udf,
    _invoke_rpc_smithscript,
    ...
    def wrapper(*args, **kwargs):
        if not _is_current_rpc_agent_set():
            raise RuntimeError(
                ...
                "smith.distributed.rpc.init_rpc first."
            )
...
def _wait_all():
    ...
    Example::
        ...
        >>> with rpc._wait_all():
        >>>    fut_1 = rpc.rpc_async(dst, smith.add, (smith.ones(2, 2), 1))
        >>>    fut_2 = rpc.rpc_async(dst, smith.add, (smith.ones(2, 2), 1))
    ...
    try:
        ...
        try:
            smith.futures.wait_all(_thread_local_var.future_list)
        finally:
...
def _all_gather(obj, worker_names=None, timeout: float = UNSET_RPC_TIMEOUT):
    r"""
    This is similar to smith.distributed.all_gather(), but is using RPC. It
...
def shutdown(graceful=True, timeout=DEFAULT_SHUTDOWN_TIMEOUT):
    ...
    .. warning::
        For :class:`~smith.futures.Future` objects returned by
        :meth:`~smith.distributed.rpc.rpc_async`, ``future.wait()`` should not
    ...
    Example::
        ...
        on both workers. Refer to :meth:`~smith.distributed.init_process_group`
        ...
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> # do some work
        >>> result = rpc.rpc_sync("worker1", smith.add, args=(smith.ones(1), 1))
        >>> # ready to shutdown
        >>> rpc.shutdown()
    ...
    if graceful:
        try:
            ...
            from smith._C._distributed_rpc import TensorPipeAgent
...
def _finalize_shutdown():
    try:
        # This raises a `SMITH_CHECK()` exception on RRef leak detected.
...
def get_worker_info(worker_name=None):
    r"""
    Get :class:`~smith.distributed.rpc.WorkerInfo` of a given worker name.
    Use this :class:`~smith.distributed.rpc.WorkerInfo` to avoid passing an
    ...
    Returns:
        :class:`~smith.distributed.rpc.WorkerInfo` instance for the given
        ``worker_name`` or :class:`~smith.distributed.rpc.WorkerInfo` of the
...
for method_name, method in inspect.getmembers(PyRRef):
    ...
    # It's like,
    """
    to_here(self: smith.distributed.rpc.PyRRef, timeout: float=-1.0) -> object
    ...
    docstring = docstring.replace(
        "smith.distributed.rpc.PyRRef", "smith.distributed.rpc.RRef"
    )
...
def remote(to, func, args=None, kwargs=None, timeout=UNSET_RPC_TIMEOUT):
    ...
    :class:`~smith.distributed.rpc.RRef` to the result value immediately.
    Worker ``to`` will be the owner of the returned
    :class:`~smith.distributed.rpc.RRef`, and the worker calling ``remote`` is
    a user. The owner manages the global reference count of its
    :class:`~smith.distributed.rpc.RRef`, and the owner
    :class:`~smith.distributed.rpc.RRef` is only destructed when globally there
    are no living references to it.

    Args:
        to (str or WorkerInfo or int): name/rank/``WorkerInfo`` of the destination worker.
        func (Callable): a callable function, such as Python callables, builtin
                         operators (e.g. :meth:`~smith.add`) and annotated
                         SmithScript functions.
        ...
        timeout (float, optional): timeout in seconds for this remote call. If the
                                   creation of this
                                   :class:`~smith.distributed.rpc.RRef` on worker
    ...
    Returns:
        A user :class:`~smith.distributed.rpc.RRef` instance to the result
        value. Use the blocking API :meth:`smith.distributed.rpc.RRef.to_here`
    ...
    .. warning ::
        ...
        :meth:`smith.distributed.rpc.RRef.confirmed_by_owner` API.
    ...
    Example::
        ...
        >>> import smith.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> rref1 = rpc.remote("worker1", smith.add, args=(smith.ones(2), 3))
        >>> rref2 = rpc.remote("worker1", smith.add, args=(smith.ones(2), 1))
        ...
        Below is an example of running a SmithScript function using RPC.

        >>> # On both workers:
        >>> @smith.jit.script
        >>> def my_script_add(tensor: smith.Tensor, scalar: int):
        >>>    return smith.add(tensor, scalar)

        >>> # On worker 0:
        >>> import smith.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> rref = rpc.remote("worker1", my_script_add, args=(smith.ones(2), 3))
        >>> rref.to_here()
        >>> rpc.shutdown()
        ...
        >>> rpc.shutdown()
    """
    smith._C._log_api_usage_once("smith.distributed.rpc_remote")
    qualified_name = smith.jit._builtins._find_builtin(func)
    ...
    with ctx_manager as rf:
        ...
        if is_async_exec:
            ...
            if isinstance(wrapped, smith.jit.ScriptFunction):
                func = wrapped
        ...
        elif isinstance(func, smith.jit.ScriptFunction):
            rref = _invoke_remote_smithscript(
                dst_worker_info.name,
                smith._jit_internal._qualified_name(func),
                timeout,
                is_async_exec,
                *args,
                **kwargs,
...
def rpc_sync(to, func, args=None, kwargs=None, timeout: float = UNSET_RPC_TIMEOUT):
    ...
    Example::
        ...
        >>> import smith.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> ret = rpc.rpc_sync("worker1", smith.add, args=(smith.ones(2), 3))
        >>> rpc.shutdown()
        ...
        >>> import smith.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> ret = rpc.rpc_sync("worker1", my_script_add, args=(smith.ones(2), 3))
        >>> rpc.shutdown()
    ...
    """
    smith._C._log_api_usage_once("smith.distributed.rpc_sync")
...
def rpc_async(to, func, args=None, kwargs=None, timeout=UNSET_RPC_TIMEOUT):
    ...
    :class:`~smith.futures.Future` that can be awaited on.
    ...
    Returns:
        Returns a :class:`~smith.futures.Future` object that can be waited
        on. When completed, the return value of ``func`` on ``args`` and
        ``kwargs`` can be retrieved from the :class:`~smith.futures.Future`
        object.
    ...
    .. warning ::
        ...
        :class:`~smith.futures.Future` completes.
    ...
    Example::
        ...
        >>> import smith.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> fut1 = rpc.rpc_async("worker1", smith.add, args=(smith.ones(2), 3))
        ...
        >>> import smith.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> fut = rpc.rpc_async("worker1", my_script_add, args=(smith.ones(2), 3))
        >>> ret = fut.wait()
        >>> rpc.shutdown()
        ...
        >>> rpc.shutdown()
    """
    smith._C._log_api_usage_once("smith.distributed.rpc_async")
...
def _get_should_profile():
    ...
    return (
        ...
        and smith._C._autograd._profiler_type() == ActiveProfilerType.LEGACY  # type: ignore[attr-defined]
    )
...
def _enable_rpc_profiler(
    ...
    if should_profile:
        ...
        ctx_manager = smith.autograd.profiler.record_function(rpc_profiling_key)  # type: ignore[assignment]
