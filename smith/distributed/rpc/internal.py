...
from smith._C._distributed_rpc import _get_current_rpc_agent


__all__ = ["RPCExecMode", "serialize", "deserialize", "PythonUDF", "RemoteException"]

# Thread local tensor tables to store tensors while pickling smith.Tensor
# objects
...
class _InternalRPCPickler:
    ...
    def __init__(self):
        ...
        self._dispatch_table[smith.Tensor] = self._tensor_reducer
    ...
    def _script_module_receiver(cls, script_module_serialized):
        """
        Given a serialized representation of a ScriptModule created with smith.jit.save,
    ...
    def _script_module_reducer(self, script_module):
        ...
        """
        f = io.BytesIO()
        smith.jit.save(script_module, f)
...
def _run_function(python_udf):
    ...
    See ``smith/csrc/distributed/rpc/python_rpc_handler.cpp``.
...
def _start_record_function(exec_type, func_name, current_worker_name, dest_worker_name):
    ...
    Returns:
        An instance of `smith.autograd._RecordFunction`.
    """
    assert smith.autograd._profiler_enabled(), "Autograd profiler should be enabled."
    profile_key = f"rpc_{exec_type.value}#{str(func_name)}({current_worker_name} -> {dest_worker_name})"
    rf = smith.autograd._RecordFunction()  # type: ignore[attr-defined]
    smith.autograd._run_before_callbacks(rf, profile_key)  # type: ignore[attr-defined]
    return rf
