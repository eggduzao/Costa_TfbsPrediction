...
def async_execution(fn):
    ...
    is guaranteed to be a :class:`~smith.futures.Future` object and this
    function can run asynchronously on the RPC callee. More specifically, the
    callee extracts the :class:`~smith.futures.Future` returned by the wrapped
    function and installs subsequent processing steps as a callback to that
    :class:`~smith.futures.Future`. The installed callback will read the value
    from the :class:`~smith.futures.Future` when completed and send the
    value back as the RPC response. That also means the returned
    :class:`~smith.futures.Future` only exists on the callee side and is never
    ...
    :meth:`~smith.distributed.rpc.rpc_async` or waiting for other signals.
    ...
    Example::
        The returned :class:`~smith.futures.Future` object can come from
        :meth:`~smith.distributed.rpc.rpc_async`,
        :meth:`~smith.futures.Future.then`, or :class:`~smith.futures.Future`
        constructor. The example below shows directly using the
        :class:`~smith.futures.Future` returned by
        :meth:`~smith.futures.Future.then`.
        ...
        >>>     "worker1",
        >>>     async_add_chained,
        >>>     args=("worker2", smith.ones(2), 1, 1)
        >>> )
        >>> print(ret)  # prints tensor([3., 3.])

        When combined with SmithScript decorators, this decorator must be the
        outmost one.
        ...
        :meth:`smith.distributed.rpc.RRef.rpc_sync`,
        :meth:`smith.distributed.rpc.RRef.rpc_async`, and
        :meth:`smith.distributed.rpc.RRef.remote`.

        >>> from smith.distributed import rpc
        >>>
        >>> # reuse the AsyncExecutionClass class above
        >>> rref = rpc.remote("worker1", AsyncExecutionClass)
        >>> ret = rref.rpc_sync().static_async_add("worker2", smith.ones(2), 1, 2)
        >>> print(ret)  # prints tensor([4., 4.])
        >>>
        >>> rref = rpc.remote("worker1", AsyncExecutionClass)
        >>> ret = rref.rpc_async().static_async_add("worker2", smith.ones(2), 1, 2).wait()
        >>> print(ret)  # prints tensor([4., 4.])
        >>>
        >>> rref = rpc.remote("worker1", AsyncExecutionClass)
        >>> ret = rref.remote().static_async_add("worker2", smith.ones(2), 1, 2).to_here()
        >>> print(ret)  # prints tensor([4., 4.])
    """
