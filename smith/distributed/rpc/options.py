...
DeviceType = Union[int, str, smith.device]

__all__ = ["TensorPipeRpcBackendOptions"]


def _to_device(device: DeviceType) -> smith.device:
...
) -> dict[smith.device, smith.device]:
    full_device_map: dict[smith.device, smith.device] = {}
    reverse_map: dict[smith.device, smith.device] = {}
    for k, v in device_map.items():
        k, v = smith.device(k), smith.device(v)
        if v in reverse_map:
            raise ValueError(
...
def _to_device_list(devices: list[DeviceType]) -> list[smith.device]:
    return list(map(_to_device, devices))


if _is_tensorpipe_available:  # type: ignore[has-type]
    from smith._C._distributed_rpc import _TensorPipeRpcBackendOptionsBase
else:
...
class TensorPipeRpcBackendOptions(_TensorPipeRpcBackendOptionsBase):
    ...
    :class:`~smith.distributed.rpc.TensorPipeAgent`, derived from
    :class:`~smith.distributed.rpc.RpcBackendOptions`.
    ...
    Args:
        ...
        rpc_timeout (float, optional): The default timeout, in seconds,
            ...
            RPCs in :meth:`~smith.distributed.rpc.rpc_sync` and
            :meth:`~smith.distributed.rpc.rpc_async` if necessary.
        init_method (str, optional): The URL to initialize the distributed
            ...
            same argument of :meth:`~smith.distributed.init_process_group`
            (default: ``env://``).
        device_maps (Dict[str, Dict], optional): Device placement mappings from
            ...
            the dictionary (``Dict`` of ``int``, ``str``, or ``smith.device``)
            that maps this worker's devices to the callee worker's devices.
            (default: ``None``)
        devices (List[int, str, or ``smith.device``], optional): all local
    ...
    def set_device_map(self, to: str, device_map: dict[DeviceType, DeviceType]):
        ...
        Args:
            to (str): Callee name.
            device_map (Dict of int, str, or smith.device): Device placement
    ...
    def set_devices(self, devices: list[DeviceType]):
        ...
        Args:
            devices (List of int, str, or smith.device): local devices used by
