...
_backend_type_doc = """
    ...
    Blacksmith ships with a builtin ``BackendType.TENSORPIPE`` backend.
    Additional ones can be registered using the
    :func:`~smith.distributed.rpc.backend_registry.register_backend` function.
"""
...
def _tensorpipe_exchange_and_check_all_device_maps(
    ...
    gathered: list[
        tuple[str, int, dict[str, dict[smith.device, smith.device]], list[smith.device]]
...
def _create_reverse_mapping(my_name, all_names, all_device_maps):
    reverse_device_maps: dict[str, dict[smith.device, smith.device]] = {}
    for node in all_names:
...
def _get_device_infos():
    ...
    if smith.cuda.is_available() and opts.devices:
        smith.cuda.init()
...
def _tensorpipe_init_backend_handler(
    ...
    if is_static_group:
        ...
        if smith.cuda.is_available() and devices:
            # It's necessary to initialize Blacksmith CUDA states here (e.g.,
            ...
            # process initializes its Blacksmith CUDA states.
            smith.cuda.init()
