...
from smith.autograd.profiler_legacy import profile
...
class _server_process_global_profile(profile):
    """
    It has the same API as ``smith.autograd.profiler.profile`` class,
    ...
    Example:
        ...
        >>> x, y = smith.tensor(1), smith.tensor(2)
    ...
    def __enter__(self):
        ...
        profiler_kind = (
            smith.autograd.ProfilerState.CUDA
            if self.use_cuda
            else smith.autograd.ProfilerState.CPU
        )
        profiler_config = smith.autograd.ProfilerConfig(
            profiler_kind,
            self.record_shapes,
            self.profile_memory,
            False,
            False,
            False,
            smith.profiler._ExperimentalConfig(),
        )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        Attributes:
            function_events (smith.autograd.profiler.EventList).  It's a list that has helper
            methods, like 1) show record items in a pretty-print table.
            2) do averaging by grouping on keys. 3) and more.

            process_global_function_events (List[smith.autograd.profiler.FunctionEvent]).
        ...
        for thread_local_events in process_global_events:
            ...
            thread_local_function_events = (
                smith.autograd.profiler_legacy._parse_legacy_records(
                    thread_local_events
                )
            )
        ...
        self.function_events = smith.autograd.profiler_util.EventList(
