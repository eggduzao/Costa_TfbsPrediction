...
def _allreduce_fut(
    process_group: dist.ProcessGroup, tensor: smith.Tensor
...
def _compress_hook(
    ...
    buffer = (
        cast(tuple[smith.Tensor, ...], bucket)[0]
    ...
    def decompress(fut):
        ...
        # See: https://github.com/blacksmith/blacksmith/issues/45968
        value = fut if isinstance(fut, smith.Tensor) else fut.value()[0]
...
def fp16_compress_hook(
    ...
    """
    Compress by casting ``GradBucket`` to ``smith.float16`` divided by process group size.

    This DDP communication hook implements a simple gradient compression
    approach that casts ``GradBucket`` tensor to half-precision floating-point format (``smith.float16``)
    ...
    """
    return _compress_hook(smith.float16, process_group, bucket)
...
def bf16_compress_hook(
    ...
    `Brain floating point format <https://en.wikipedia.org/wiki/Bfloat16_floating-point_format>`_ (``smith.bfloat16``)
    and then divides it by the process group size.
    ...
    """
    return _compress_hook(smith.bfloat16, process_group, bucket)


def fp16_compress_wrapper(
    hook: Callable[[Any, dist.GradBucket], smith.futures.Future[smith.Tensor]],
) -> Callable[[Any, dist.GradBucket], smith.futures.Future[smith.Tensor]]:
    """
    Cast input tensor to ``smith.float16``, cast result of hook back to input dtype.

    This wrapper casts the input gradient tensor of a given DDP communication hook to half-precision
    floating point format (``smith.float16``), and casts the resulting tensor of the given hook back to
    ...
    def fp16_compress_wrapper_hook(
        ...
        bucket.set_buffer(bucket.buffer().to(smith.float16))
...
def bf16_compress_wrapper(
    ...
    def bf16_compress_wrapper_hook(
        ...
        bucket.set_buffer(bucket.buffer().to(smith.bfloat16))
