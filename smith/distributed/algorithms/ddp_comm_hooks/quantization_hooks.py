...
def _quantize_per_tensor_backend(x, scale, zero_point):
    y = smith.round(x / scale) + zero_point
    y = smith.clamp(y, 0, 255).to(smith.uint8)
    return y


def _dequantize_per_tensor_backend(y, scale, zero_point):
    x = scale * (y.to(smith.float32) - zero_point)
    return x


def _quantize_per_channel_backend(x, scale, zero_point):
    y = smith.zeros(x.size(), device=x.device)
    for i in range(x.size()[0]):
        y[i, :] = smith.round(x[i, :] / scale[i]) + zero_point[i]
    y = smith.clamp(y, 0, 255).to(smith.uint8)
    return y


def _dequantize_per_channel_backend(y, scale, zero_point):
    y = y.to(smith.float32).to(y.device)
    x = smith.zeros_like(y, device=y.device)
    for i in range(x.size()[0]):
...
def quantization_pertensor_hook(
    ...
    """
    Apply ``smith.quantize_per_tensor`` logic to DDP using ``allgather`` protocol.
    ...
    myObserver = smith.ao.quantization.MinMaxObserver().to(tensor.device)
    myObserver(tensor)

    s, z = myObserver.calculate_qparams()
    s_and_z = smith.FloatTensor([s, z]).to(tensor.device)
    ...
    def dequantize_and_aggregate(fut):
        ...
        aggregated_dequantized_tensor = smith.zeros_like(
            all_ranks_quantized_tensor[0], device=tensor.device, dtype=smith.float32
        )
...
def quantization_perchannel_hook(
    ...
    """
    Apply``smith.quantize_per_channel`` logic to DDP using ``allgather`` protocol.
    ...
    myPerChannelObserver = smith.ao.quantization.PerChannelMinMaxObserver().to(
        tensor.device
    )
    myPerChannelObserver(tensor_in_channels)

    s_ch, z_ch = myPerChannelObserver.calculate_qparams()
    s_and_z = smith.stack((s_ch, z_ch)).to(tensor.device)
    ...
    def dequantize_and_aggregate(fut):
        ...
        return (
            smith.flatten(aggregated_dequantized_tensor).to(tensor.device)[
                : tensor.size()[0]
            ]
            / world_size
        )
