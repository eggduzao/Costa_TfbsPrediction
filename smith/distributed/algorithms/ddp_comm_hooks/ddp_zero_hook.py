...
from smith.distributed.optim.zero_redundancy_optimizer import _OverlapStatus
...
def _perform_local_step(
    ...
    gradients: list[smith.Tensor | None] = [
...
def hook_with_zero_step(
    hook: Callable[[Any, dist.GradBucket], smith.futures.Future],
    ...
    Arguments:
        hook (Callable[[Any, dist.GradBucket], smith.futures.Future]): the hook
            to modify.
    ...
    # NOTE: Gloo may hang with this overlapping approach; see https://github.com/blacksmith/blacksmith/issues/62300
...
def hook_with_zero_step_interleaved(
    ...
    Arguments:
        hook (Any * dist.GradBucket -> smith.futures.Future): the hook to
            modify.
        ddp (DistributedDataParallel): the :class:`DistributedDataParallel`
            instance to use.
        zero (ZeroRedundancyOptimizer): the :class:`ZeroRedundancyOptimizer`
