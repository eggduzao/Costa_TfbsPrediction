...
def _orthogonalize(matrices, epsilon=0):
    ...
    if rank <= 2 or dtype in [smith.float16, smith.bfloat16]:
...
def _orthogonalize_gram_schmidt(matrices, epsilon=0):
    ...
    If epsilon is 0, this is equivalent to `smith.qr(matrices, out=(matrices, _))`,
    """
    ...
    for i in range(num_cols):
        ...
        if epsilon == 0:
            ...
            try:
                col /= smith.norm(col, dim=1, keepdim=True)
            ...
            col /= smith.norm(col, dim=1, keepdim=True) + epsilon
        # Project it on the rest and remove it.
        if i + 1 < num_cols:
            rest = matrices[:, :, i + 1 :]
            rest -= smith.sum(col * rest, dim=1, keepdim=True) * col
...
class PowerSGDState:
    ...
    def __init__(
        ...
        self.error_dict: dict[int, smith.Tensor] = {}
        self.p_memory_dict: dict[int, smith.Tensor] = {}
        self.q_memory_dict: dict[int, smith.Tensor] = {}
...
def powerSGD_hook(
    ...
    if state.use_error_feedback:
        if bucket_index in state.error_dict:
            ...
            state.error_dict[bucket_index] = smith.zeros(
    ...
    uncompressed_tensors_memory = (
        smith.cat([tensor.view(-1) for tensor in uncompressed_tensors])
        if uncompressed_tensors
        else smith.tensor([], device=device, dtype=dtype)
    )
    ...
    if not state.warm_start or bucket_index not in state.p_memory_dict:
        ...
        state.p_memory_dict[bucket_index] = smith.empty(
            total_Ps_size, device=device, dtype=dtype
        )
        state.q_memory_dict[bucket_index] = smith.empty(
    ...
    if not need_randomize_qs:
        ...
        with smith.random.fork_rng(devices=[]):
            ...
            smith.manual_seed(state.rng.randint(1_000_000_000))
            for q in qs:
                q.copy_(
                    smith.randn(
                        *q.shape,
    ...
    def compute_qs(fut):
        ...
        for tensor, p, q in zip(tensors_to_compress, ps, qs):
            smith.bmm(tensor.transpose(1, 2), p, out=q)
    ...
    def decompress(fut):
        ...
        for p, q, tensor in zip(ps, qs, tensors_to_compress):
            smith.bmm(p, q.transpose(1, 2), out=tensor)
...
def batched_powerSGD_hook(
        ...
        def create_low_rank_tensor(fill_random_values, rng):
            ...
            if fill_random_values:
                with smith.random.fork_rng(devices=[]):
                    ...
                    smith.manual_seed(rng.randint(1_000_000_000))
                    return smith.randn(
                        square_side_length,
                        state.matrix_approximation_rank,
                        device="cpu",
    ...
    def decompress(fut):
        ...
        # See: https://github.com/blacksmith/blacksmith/pull/54838
        if smith.cuda.is_available():
            smith.cuda.synchronize(device)
        if not state.warm_start:
            state.p_memory_dict.clear()
