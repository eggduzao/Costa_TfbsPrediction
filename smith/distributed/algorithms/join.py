...
class Join:
    ...
    Example::
        ...
        >>> # xdoctest: +SKIP
        >>> import smith.nn.parallel.DistributedDataParallel as DDP
        >>> import smith.distributed.optim.ZeroRedundancyOptimizer as ZeRO
        >>> from smith.distributed.algorithms.join import Join
        >>>
        >>> # On each spawned worker
        >>> def worker(rank):
        >>>     dist.init_process_group("nccl", rank=rank, world_size=2)
        >>>     model = DDP(smith.nn.Linear(1, 1).to(rank), device_ids=[rank])
        >>>     optim = ZeRO(model.parameters(), smith.optim.Adam, lr=0.01)
        >>>     # Rank 1 gets one more input than rank 0
        >>>     inputs = [smith.tensor([1.]).to(rank) for _ in range(10 + rank)]
    ...
    def _get_num_nonjoined_procs(self):
        ...
        num_nonjoined_procs = smith.zeros(1, device=self._device)
    ...
    def _notify_procs_to_terminate(self):
        ...
        """
        ones = smith.ones(1, device=self._device)
    ...
    def notify_join_context(joinable: Joinable):
        ...
        ones = smith.ones(1, device=device)
        ...
        if join_config.throw_on_early_termination:
            ...
            zeros = smith.zeros(1, device=device)
