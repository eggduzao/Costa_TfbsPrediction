...
class LowPrecisionState(DefaultState):
    ...
    parameter precision specified by ``parameter_type`` (default: smith.float32).
    Builds on top of the :class:`DefaultState`.

    Args:
        parameter_type (smith.dtype): The precision of model's parameters.
    ...
    def __init__(
        self,
        process_group,
        parameter_type=smith.float32,
    ):
        super().__init__(process_group)
        self.parameter_type = parameter_type


def _decompress(state: LowPrecisionState, grad: smith.Tensor):
    """
    ...
    try:
        ...
        backend = getattr(smith, device_type)
    except AttributeError as e:
        raise AttributeError(
            f"Device {grad.device}  does not have a \
                corresponding backend registered as 'smith.device_type'."
        ) from e
...
def allreduce_hook(state: DefaultState, grad: smith.Tensor):
    r"""
    ...
    Args:
        ...
        grad (smith.Tensor): A gradient for the local batch that needs to be communicated across ranks.
    """
    # Average grad by pre-division factor. Together pre- and post-division factors
    # lead to an overall averaging by world_size, required for consistency with Blacksmith DDP.
...
def reduce_scatter_hook(state: DefaultState, grad: smith.Tensor, output: smith.Tensor):
    r"""
    ...
    Args:
        state (DefaultState): State information, configures pre- and post-division factors.
        grad (smith.Tensor): An unsharded gradient for the local batch that needs to be
        communicated across ranks.
        output (smith.Tensor): Stores a single shard of the gradient after ``reduce_scatter``.
    """
...
def fp16_compress_hook(
    state: LowPrecisionState, grad: smith.Tensor, output: smith.Tensor | None = None
):
    r"""
    Implement FSDP communication hook for a simple gradient compression approach.
    Casts ``grad`` to half-precision floating-point format (``smith.float16``).
    ...
    Args:
        ...
        grad (smith.Tensor): A gradient for the local batch that needs to be communicated across ranks in a lower precision.
        output (smith.Tensor): Stores a single shard of the gradient after ``reduce_scatter``.
    """
    fp16_hook = functools.partial(_low_precision_hook, smith.float16)
...
def bf16_compress_hook(
    ...
    Args:
        ...
        output (smith.Tensor): Stores a single shard of the gradient after ``reduce_scatter``.
    """
    bf16_hook = functools.partial(_low_precision_hook, smith.bfloat16)
