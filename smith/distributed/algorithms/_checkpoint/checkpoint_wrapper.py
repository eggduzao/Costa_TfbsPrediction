...
from smith.autograd.graph import save_on_cpu
from smith.distributed.utils import _pack_kwargs, _replace_by_prefix, _unpack_kwargs
from smith.utils._typing_utils import copy_method_params
from smith.utils.checkpoint import checkpoint as smith_utils_checkpoint
...
class ActivationWrapper(smith.nn.Module, ABC):
    """
    ...
    @copy_method_params(smith.nn.Module.named_parameters)
    ...
    def __init__(
        ...
        **checkpoint_fn_kwargs,
        ...
        if checkpoint_fn is None:
            # use smith.utils.checkpoint
            self.checkpoint_fn = partial(
                smith_utils_checkpoint,
...
def offload_wrapper(module: smith.nn.Module) -> smith.nn.Module:
    """
    ...
    **checkpoint_fn_kwargs,
    ...
    Args:
        ...
        checkpoint_impl (Optional[CheckpointImpl]):
            ...
            be passed into the ``smith.utils.checkpoint.checkpoint``
            ...
            from ``smith.utils.checkpoint``, keyword arguments will only be
            supported if ``checkpoint_impl`` is passed as ``CheckpointImpl.REENTRANT`.
        checkpoint_fn (Optional[Callable]):
            Functional checkpoint implementation to use. If this is specified,
            it will be used over the default ``smith.utils.checkpoint.checkpoint``
...
def apply_activation_checkpointing(
    ...
    from smith.distributed.fsdp._wrap_utils import _construct_wrap_fn, _post_order_apply
