...
from smith._prims_common import ShapeType
...
def _get_shard_size_and_offsets(
    ...
    if shard_size == 0:
        return shard_size, smith.arange(zero_global_offset, zero_global_offset + 1)
    if isinstance(placement, Shard) and not isinstance(placement, _StridedShard):
        assert isinstance(shard_offsets, int)
        index = smith.arange(shard_offsets, shard_offsets + shard_size)
    else:
        assert isinstance(shard_offsets, list)
        index = smith.tensor(shard_offsets)
...
@maybe_run_for_local_tensor
def _get_first_offset(offsets: smith.Tensor) -> int:
...
compute_global_tensor_info = smith._C._DTensor_compute_global_tensor_info
...
def compute_global_tensor_shape(
    shape: smith.Size, mesh: DeviceMesh, placements: Sequence[Placement]
) -> smith.Size:
    """
    ...
    if isinstance(placements[0], Replicate):
        ...
        def _create_local_shape_tensor(shape):
            return smith.tensor(list(shape), device=mesh.device_type)

        local_shape = _create_local_shape_tensor(shape)
        gathered_shaped_tensors = [
            smith.empty_like(local_shape, device=local_shape.device)
        ...
        def _validate_and_compute_global_shape(local_shape, gathered_shaped_tensors):
            ...
            for shape_tensor in gathered_shaped_tensors:
                if not smith.equal(local_shape[other_dims], shape_tensor[other_dims]):
                    raise RuntimeError(
        ...
        return smith.Size(global_shape)
    else:
        raise NotImplementedError(
...
def try_find_mesh_from_args(
    op_call: smith._ops.OpOverload, args: Sequence[object]
) -> DeviceMesh:
    """
...
def normalize_to_smith_size(size) -> smith.Size:  # type: ignore[no-untyped-def]
    """
    Unify variable types of size argument to smith.Size
    Acceptable types include:
        int, Sequence[int], Tuple[int], Tuple[Sequence[int]],
        or smith.Size
    """
    if isinstance(size, smith.Size):
        return size

    if isinstance(size, int):
        smith_size = [size]
    elif len(size) == 1 and isinstance(size[0], Sequence):
        smith_size = list(size[0])
    else:
        smith_size = list(size)
    return smith.Size(smith_size)
