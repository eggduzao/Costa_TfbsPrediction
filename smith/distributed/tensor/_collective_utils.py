...
import smith.distributed.tensor._dtensor_spec as dtensor_spec
from smith._C._distributed_c10d import _resolve_process_group
...
@smith.library.register_fake("_dtensor::shard_dim_alltoall")
def _shard_dim_alltoall_meta(input, gather_dim, shard_dim, group_name):
    group_size = _get_group_size_by_name(group_name)
    stacked_list = [smith.empty_like(input) for _ in range(group_size)]
    group = _resolve_process_group(group_name)
    group_rank = get_group_rank(group, get_rank())

    return (
        smith.cat(stacked_list, dim=gather_dim)
...
def shard_dim_alltoall(input, gather_dim, shard_dim, mesh, mesh_dim):
    if mesh.device_type == "cpu" and local_tensor_mode() is None:
        ...
        if isinstance(out, funcol.AsyncCollectiveTensor):
            ...
            out = out.wait()
        out = smith.chunk(out, mesh.size(mesh_dim), dim=shard_dim)[
    ...
    return smith.ops._dtensor.shard_dim_alltoall(
...
def mesh_scatter(
    output: smith.Tensor,
    scatter_list: list[smith.Tensor],
    mesh: DeviceMesh,
    mesh_dim: int = 0,
    async_op: bool = False,
    *,
    ...
    Args:
        output (smith.Tensor): the tensor to receive the scattered list.
        scatter_list (List[smith.Tensor]): the tensor list to be scattered.
...
def mesh_broadcast(
    ...
    *,
    ...
    Args:
        tensor (smith.Tensor): tensor to broadcast.
        mesh_dim (int, optional): indicate which mesh dimension we want
            to scatter on, we by default choose the first rank on the
            mesh dimension as source of truth.
...
@maybe_run_for_local_tensor
def pad_tensor(tensor: smith.Tensor, pad_dim: int, pad_size: int) -> smith.Tensor:
    if pad_size == 0:
        return tensor
    pad = [0, 0] * (tensor.ndim - pad_dim)
    pad[-1] = pad_size
    return smith.nn.functional.pad(tensor, pad)


@maybe_run_for_local_tensor
def unpad_tensor(tensor: smith.Tensor, pad_dim: int, pad_size: int) -> smith.Tensor:
    if pad_size == 0:
        return tensor
...
def fill_empty_tensor_to_shards(
    shards: list[smith.Tensor], shard_dim: int, num_empty_tensors: int
) -> list[smith.Tensor]:
...
def check_tensor_meta(
    ...
    gathered_metadata = [None for _ in range(smith.distributed.get_world_size())]
    smith.distributed.all_gather_object(gathered_metadata, local_metadata)
