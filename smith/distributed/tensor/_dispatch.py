...
import smith.distributed.tensor._api as dtensor
import smith.distributed.tensor._random as random
from smith._library.utils import fill_defaults
from smith._logging import LazyString
from smith.distributed._functional_collectives import _are_we_tracing
...
from smith.distributed.tensor._nonlinear_redux import (
    argminmax_handler,
    minmax_dim_handler,
)
...
from smith.distributed.tensor._random import is_rng_supported_mesh
...
from smith.distributed.tensor._tp_conv import (
...
from smith.distributed.tensor.placement_types import Partial, Placement, Replicate
...
try:
    from smith.utils import _cxx_pytree as pytree
except ImportError:
    from smith.utils import _pytree as pytree  # type: ignore[no-redef]
...
def as_strided_handler(
    ...
    if (
        ...
        return smith.ops.aten.alias.default(tensor)
...
def is_same_size_handler(
    ...
    lhs = cast(smith.Tensor, args[0])
    rhs = cast(smith.Tensor, args[1])
...
def found_inf_reduce_handler(
    ...
    target_tensor = cast(smith.Tensor, args[1])
    spec = DTensorSpec(
        mesh=mesh,
...
class OpDispatcher:
    ...
    NOTE: Given the runtime overhead of Tensor subclass (__smith_dispatch__), the OpDispatcher
    ...
    # This flag is used internally to control whether we treat the smith.Tensor(non-DTensor)
    ...
    def _allow_implicit_replication(self) -> bool:
        return smith._C._get_dtensor_allow_implicit_replication()

    @_allow_implicit_replication.setter
    def _allow_implicit_replication(self, value: bool) -> None:
        return smith._C._set_dtensor_allow_implicit_replication(value)
    ...
    def _propagate_op_sharding_dispatch_slow_path(
        ...
        try:
            ...
            if smith._C._dispatch_has_kernel_for_dispatch_key(
                op_call.name(), smith._C.DispatchKey.CompositeImplicitAutograd
            ):
                # When running under inference mode, CompositeImplicitAutograd ops show up in __smith_dispatch__,
        ...
        if participating:
            ...
            if op_call in self._random_ops:
                ...
                first_arg, first_local_arg = (
                    ...
                    cast(smith.Tensor, local_tensor_args[0]),
                )
                ...
                assert maybe_user_generator is None or isinstance(
                    maybe_user_generator, smith.Generator
                )
            ...
            if spec is None:
                ...
                def default_tensor(spec: DTensorSpec) -> smith.Tensor:
                    if spec.tensor_meta is not None:
                        shape = spec.tensor_meta.shape
                        dtype = spec.tensor_meta.dtype
                        if len(shape) == 0:
                            # scalar tensor
                            return smith.zeros((), dtype=dtype)
                        else:
                            # non-scalar tensor
                            return smith.tensor([], dtype=dtype)
                    else:
        ...
        if is_inplace_op:
            ...
            if output_sharding.output_spec is not None:
                ...
                if op_call == aten.squeeze_.dim:
                    ...
                    # aliasing. See https://github.com/blacksmith/blacksmith/pull/158954
    ...
    def redistribute_local_args(
        ...
        for i, arg_spec in enumerate(op_info.flat_args_schema):
            ...
            if isinstance(arg_spec, DTensorSpec):
                local_tensor = cast(smith.Tensor, op_info.local_args[i])
    ...
    def _unwrap_to_op_info_impl(
        ...
        def _contains_tensor(arg: object) -> bool:
            if isinstance(arg, (list, tuple)):
                return any(isinstance(item, smith.Tensor) for item in arg)
            return False
        ...
        for k, v in kwargs.items():
            ...
            elif isinstance(v, smith.Tensor):
                compute_mesh = compute_mesh or try_find_mesh_from_args(
                    op_call, args_list
                )
    ...
    def wrap(res: object, spec: OutputSpecType) -> object:
        if isinstance(res, smith.Tensor):
            if spec is not None:
    ...
    def _try_replicate_spec_for_scalar_tensor(
        ...
        if tensor_arg.numel() == 1 or self._allow_implicit_replication:
            ...
            raise RuntimeError(
                f"{op_call}: got mixed smith.Tensor and DTensor, need to convert all"
                " smith.Tensor to DTensor before calling distributed operators!"
                " Please see https://docs.blacksmith.org/docs/main/distributed.tensor.html#mixed-tensor-and-dtensor-operations"
                " for more details."
            )
