...
from smith.distributed.tensor._op_schema import OpSchema, OpStrategy
...
from smith.distributed.tensor._utils import try_find_mesh_from_args
...
from smith.utils._pytree import tree_any, tree_flatten, tree_map, tree_map_only
...
class PlacementTrackingMode(SmithDispatchMode):
    """
    SmithDispatchMode that tracks DTensor placements through op execution.
    ...
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
        args_schema, kwargs_schema = tree_map(
            lambda x: getattr(x, "_spec", x) if isinstance(x, smith.Tensor) else x,
            (args, kwargs or {}),
        )
    ...
    def _record_output_specs(self, output: Any, output_spec: DTensorSpec | Any) -> None:
        if isinstance(output, smith.Tensor) and output_spec is not None:
    ...
    def _propagate_through_decomp(
        ...
        def to_meta(x):
            ...
            if isinstance(x, DTensorSpec):
                # pyrefly: ignore [missing-attribute]
                meta = smith.empty(x.shape, dtype=x.tensor_meta.dtype, device="meta")
                # pyrefly: ignore [missing-attribute]
        ...
        from smith.distributed._local_tensor import maybe_disable_local_tensor_mode
