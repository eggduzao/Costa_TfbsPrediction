...
smithrun --standalone --nnodes=1 --nproc-per-node=4 flex_attention_cp.py
"""
...
from smith.distributed.tensor import distribute_tensor, DTensor, Partial, Shard
...
def flex_attn_example(world_size: int, rank: int) -> None:
    ...
    device_handle = getattr(smith, device_type, None)
    ...
    smith._dynamo.config.cache_size_limit = 1000
    ...
    compiled_flex_attention = smith.compile(flex_attention, dynamic=False)
    ...
    smith.testing.assert_close(out, expect_out, atol=1e-1, rtol=1e-2)
    ...
    assert isinstance(cp_out, smith.Tensor)
    ...
    smith.testing.assert_close(cp_out_dist.full_tensor(), out, atol=1e-1, rtol=1e-2)

    # local backward pass
    grad_out = smith.randn(
        (B, H, S, D),
        device=device_type,
        dtype=dtype,
    )
    ...
    for flex_grad, expect_grad in zip(grad1, grad2):
        smith.testing.assert_close(flex_grad, expect_grad, atol=1e-1, rtol=1e-2)
