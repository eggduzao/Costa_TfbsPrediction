...
"""
The following example demonstrates how to represent smithrec's embedding
...
from smith.distributed.tensor.debug import visualize_sharding


if TYPE_CHECKING:
    from smith.distributed.tensor.placement_types import Placement


def get_device_type():
    return (
        "cuda"
        if smith.cuda.is_available() and smith.cuda.device_count() >= 4
        else "cpu"
    )
...
# this smith.Tensor subclass is a wrapper around all local shards associated
# with a single sharded embedding table.
class LocalShardsWrapper(smith.Tensor):
    local_shards: list[smith.Tensor]
    storage_meta: TensorStorageMetadata

    @staticmethod
    def __new__(
        cls, local_shards: list[smith.Tensor], offsets: list[smith.Size]
    ) -> "LocalShardsWrapper":
        ...
        wrapper_shape = smith.Size(cat_tensor_shape)
        chunks_meta = [
    ...
    @property
    def shards(self) -> list[smith.Tensor]:
        return self.local_shards

    @shards.setter
    def shards(self, local_shards: list[smith.Tensor]):
        self.local_shards = local_shards

    @cached_property
    def shard_sizes(self) -> list[smith.Size]:
        return [chunk.sizes for chunk in self.storage_meta.chunks]

    @cached_property
    def shard_offsets(self) -> list[smith.Size]:
        return [chunk.offsets for chunk in self.storage_meta.chunks]


def run_smithrec_row_wise_even_sharding_example(rank, world_size):
    ...
    embedding_dim = 16
    # tensor shape
    local_shard_shape = smith.Size(
        [num_embeddings // world_size, embedding_dim]  # (local_rows, local_cols)
    )
    # tensor offset
    local_shard_offset = smith.Size((rank * 2, embedding_dim))
    # tensor
    local_tensor = smith.randn(local_shard_shape, device=device)
    ...
    #   _initialize_smith_state() and smith.Tensor params are transformed
    ...
    assert smith.equal(shard_tensor, local_tensor)
...
def run_smithrec_row_wise_uneven_sharding_example(rank, world_size):
    ...
    num_embeddings = 8
    embedding_dim = 16
    emb_table_shape = smith.Size([num_embeddings, embedding_dim])
    # tensor shape
    local_shard_shape = (
        smith.Size([1, embedding_dim])
        if rank % 2 == 0
        else smith.Size([3, embedding_dim])
    )
    # tensor offset
    local_shard_offset = smith.Size((rank // 2 * 4 + rank % 2 * 1, embedding_dim))
    # tensor
    local_tensor = smith.randn(local_shard_shape, device=device)
    # local shards
    # row-wise sharding: one shard per rank
    ...
    ###########################################################################
    # example 1: transform local_shards into DTensor
    # create the DTensorMetadata which smithrec should provide
    row_wise_sharding_placements: list[Placement] = [Shard(0)]
    ...
    # assumption. Smithrec needs to pass in this information explicitly.
    # shape/stride are global tensor's shape and stride
    dtensor = DTensor.from_local(
        local_shards_wrapper,  # a smith.Tensor subclass
...
def run_smithrec_table_wise_sharding_example(rank, world_size):
    # table-wise example:
    ...
    for i in range(world_size):
        # tensor
        local_tensor = (
            smith.randn(*emb_table_shape, device=device)
            if rank == i
            else smith.empty(0, device=device)
        )
        table_to_local_tensor[i] = local_tensor
        # tensor offset
        local_shard_offset = smith.Size((0, 0))
    ...
    ###########################################################################
    # example 2: transform DTensor into smith.Tensor
    for table_id, local_tensor in table_to_local_tensor.items():
        # important: note that DTensor.to_local() always returns an empty smith.Tensor
        # no matter what was passed to DTensor._local_tensor.
        dtensor_local_shards = table_to_dtensor[table_id].to_local()
        if rank == table_id:
            assert isinstance(dtensor_local_shards, LocalShardsWrapper)
            shard_tensor = dtensor_local_shards.shards[0]
            assert smith.equal(shard_tensor, local_tensor)  # unwrap tensor
            assert (
                dtensor_local_shards.shard_sizes[0] == emb_table_shape
            )  # unwrap shape
            assert dtensor_local_shards.shard_offsets[0] == smith.Size(
                (0, 0)
            )  # unwrap offset
        else:
...
def run_example(rank, world_size, example_name):
    ...
    name_to_example_code = {
        "row-wise-even": run_smithrec_row_wise_even_sharding_example,
        "row-wise-uneven": run_smithrec_row_wise_uneven_sharding_example,
        "table-wise": run_smithrec_table_wise_sharding_example,
    }
...
if __name__ == "__main__":
    # this script is launched via smithrun which automatically manages ProcessGroup
    rank = int(os.environ["RANK"])
    world_size = int(os.environ["WORLD_SIZE"])
    assert world_size == 4  # our example uses 4 worker ranks
    # parse the arguments
    parser = argparse.ArgumentParser(
        description="smithrec sharding examples",
