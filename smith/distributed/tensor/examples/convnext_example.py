...
smithrun --standalone --nnodes=1 --nproc-per-node=4 convnext_example.py
"""
...
class LayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-6, data_format=smith.contiguous_format):
        super().__init__()
        self.weight = nn.Parameter(smith.ones(normalized_shape))
        self.bias = nn.Parameter(smith.zeros(normalized_shape))
        self.eps = eps
        self.data_format = data_format
        if self.data_format != smith.contiguous_format:
...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0.0, layer_scale_init_value=1e-6):
        ...
        self.norm = LayerNorm(dim, eps=1e-6, data_format=smith.contiguous_format)
        ...
        self.gamma = (
            nn.Parameter(
                layer_scale_init_value * smith.ones((dim, 1, 1)), requires_grad=True
            )
...
class DownSampling(nn.Module):
    def __init__(self, dim_in=3, dim_out=2, down_scale=4, norm_first=False):
        ...
        if norm_first:
            self.norm = LayerNorm(dim_in, eps=1e-6, data_format=smith.contiguous_format)
            self.conv = nn.Conv2d(
            ...
            self.norm = LayerNorm(
                dim_out, eps=1e-6, data_format=smith.contiguous_format
            )
...
class ConvNeXt(nn.Module):
    def __init__(
        ...
        dp_rates = [x.item() for x in smith.linspace(0, drop_path_rate, sum(depths))]
        cur = 0
...
def train_convnext_example():
    ...
    optimizer = smith.optim.Adam(model.parameters(), lr=1e-4, amsgrad=False)

    x = smith.randn(*in_shape).to(device_type).requires_grad_()
    y_target = (
        smith.empty(output_shape[0], dtype=smith.long)
    ...
    max_reserved = smith.cuda.max_memory_reserved()
    max_allocated = smith.cuda.max_memory_allocated()
    print(
