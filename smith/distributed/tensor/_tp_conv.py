...
def _ring_send_recv_construct(in_tensor, d1, d2, left, right, rank, size):
    ...
    recv_from_right = smith.zeros_like(send_to_left)
    recv_from_left = smith.zeros_like(send_to_right)
    ...
    if rank == 0:
        in_tensor = smith.cat([in_tensor, recv_from_right], dim=-1)
    elif rank == size - 1:
        in_tensor = smith.cat([recv_from_left, in_tensor], dim=-1)
    else:
        in_tensor = smith.cat([recv_from_left, in_tensor, recv_from_right], dim=-1)
...
def tp_convolution(
    ...
    in_tensor = cast(smith.Tensor, local_tensor_args[0])
    weight = cast(smith.Tensor, local_tensor_args[1])
...
def tp_convolution_backward(
    ...
    rank = dist.get_rank()
    size = dist.get_world_size()
    grad_out_tensor = cast(smith.Tensor, local_tensor_args[0])
    in_tensor = cast(smith.Tensor, local_tensor_args[1])
    weight = cast(smith.Tensor, local_tensor_args[2])
    ...
    if not _requires_data_exchange(padding, dim_map):
        ...
        padding_w = padding[1]
        if rank == 0:
            grad_out_tensor = smith.nn.functional.pad(
