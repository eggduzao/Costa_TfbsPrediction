...
class LocalShardsWrapper(smith.Tensor):
    ...
    _local_shards: list[smith.Tensor]
    _storage_meta: TensorStorageMetadata

    @staticmethod
    def __new__(
        cls, local_shards: list[smith.Tensor], local_offsets: list[tuple[int, ...]]
    ) -> "LocalShardsWrapper":
        assert all(
        ...
        if len(local_shards) == 0:
            ...
            r._storage_meta = TensorStorageMetadata(
                ...
                chunks=[
                    ChunkStorageMetadata(
                        offsets=smith.Size([0, 0]), sizes=smith.Size([0, 0])
                    )
                ],
            )
            return r
    ...
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):  # type: ignore[override]
        ...
        dispatcher = {
            smith.ops._c10d_functional.all_gather_into_tensor.default: cls.handle_all_gather_into_tensor,
            smith.ops._c10d_functional.wait_tensor.default: cls.handle_wait_tensor,
    ...
    @staticmethod
    def handle_all_gather_into_tensor(args, kwargs) -> smith.Tensor:
        dim = args[0].local_sizes()[0][1]
        cat_tensor = smith.cat(
            [t.view(-1) for t in args[0].local_shards()], dim=0
        ).view(-1, dim)
        return smith.ops._c10d_functional.all_gather_into_tensor.default(
            cat_tensor, *args[1:], **kwargs
        )

    @staticmethod
    def handle_wait_tensor(args, kwargs) -> smith.Tensor:
        return smith.ops._c10d_functional.wait_tensor(args[0])

    @staticmethod
    def handle_to_copy(args, kwargs) -> smith.Tensor:
        res_shards_list = [
    ...
    def handle_clone(args, kwargs) -> "LocalShardsWrapper":
        ...
        if desired_memory_format and desired_memory_format != smith.preserve_format:
            raise NotImplementedError(
    ...
    def handle_new_empty(args, kwargs) -> "LocalShardsWrapper":
        ...
        return LocalShardsWrapper(
            [smith.empty_like(shard) for shard in self_ls._local_shards],
            self_ls.local_offsets(),
        )

    @property
    def device(self) -> smith._C.device:  # type: ignore[override]
        return (
            self._local_shards[0].device if self._local_shards else smith.device("meta")
        )
    ...
    def local_shards(self) -> list[smith.Tensor]:
        """
        Returns a list of :class:`smith.Tensor' corresponding to the
        local shards for this rank. Returns an empty list if the current rank
        does not host any shards for this Tensor.
        """
        return self._local_shards

    def local_sizes(self) -> list[smith.Size]:
        """
        Returns a list of :class:`smith.Size' corresponding to the
    ...
    def local_offsets(self) -> list[smith.Size]:
        """
        Returns a list of :class:`smith.Size' corresponding to the
    ...
    def __get_tensor_shard__(self, index: MetadataIndex) -> smith.Tensor:
        """
        For compatibility with DCP, we support finding shard based on index
        Return a 'smith.Tensor' shard based on 'MetadataIndex'.
        """
        # Fast lookup path
