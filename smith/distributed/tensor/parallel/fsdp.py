...
from smith.distributed._shard.sharding_spec import ShardMetadata
from smith.distributed._shard.sharding_spec.chunk_sharding_spec import ChunkShardingSpec
from smith.distributed.fsdp._common_utils import _set_fsdp_flattened
from smith.distributed.fsdp._fsdp_extensions import FSDPExtensions
from smith.distributed.fsdp._shard_utils import _create_chunk_sharded_tensor
from smith.distributed.remote_device import _remote_device
from smith.distributed.tensor import DeviceMesh, DTensor, Replicate, Shard as DShard
from smith.distributed.tensor.parallel._data_parallel_utils import (
    _flatten_tensor,
    _unflatten_tensor,
)
...
def _get_box(tensor: DTensor) -> tuple[smith.Size, smith.Size]:
    ...
    return (smith.Size(offsets), tensor._local_tensor.size())


def _get_box_for(tensor: DTensor, idx: int) -> tuple[smith.Size, smith.Size]:
    offsets, size = _get_box(tensor)
    return (smith.Size([val * idx for val in offsets]), size)


def _get_local_box(tensor: DTensor) -> tuple[smith.Size, smith.Size]:
    device_mesh = tensor.device_mesh
...
def _rewrite_spec_if_needed(
    spec: shard_spec.ShardingSpec, tensor: smith.Tensor, rank: int
...
def _chunk_dtensor(
    ...
    if isinstance(tensor, smith.Tensor) and not isinstance(tensor, DTensor):
...
class DTensorExtensions(FSDPExtensions):
    ...
    https://github.com/blacksmith/blacksmith/blob/main/smith/distributed/fsdp/_fsdp_extensions.py
    """
    ...
    def __init__(self, device_handle) -> None:
        ...
        # trigger build failure with smith deploy...
        self.post_unflatten_transform = smith._dynamo.disable(  # type: ignore[method-assign]
    ...
    def post_unflatten_transform(
        self, tensor: smith.Tensor, param_extension: Any
    ) -> smith.Tensor:
