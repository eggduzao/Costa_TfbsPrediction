...
class ColwiseParallel(ParallelStyle):
    ...
    Keyword Args:
        ...
        use_local_output (bool, optional):
            Whether to use local :class:`smith.Tensor` instead of :class:`DTensor` for the module output, default: True.
    Returns:
        A :class:`ParallelStyle` object that represents Colwise sharding of the nn.Module.

    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from smith.distributed.tensor.parallel import parallelize_module, ColwiseParallel
        ...
        >>> # and the output of "w1" will return :class:`smith.Tensor` that shards on the last dim.
        >>>
...
class RowwiseParallel(ParallelStyle):
    ...
    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from smith.distributed.tensor.parallel import parallelize_module, RowwiseParallel
        >>> from smith.distributed.device_mesh import init_device_mesh
        >>> ...
        ...
        >>> # and the output of "w2" will return a replicated :class:`smith.Tensor`.
        >>>
...
class SequenceParallel(ParallelStyle):
    ...
    If the input passed in to this ``nn.Module`` is a :class:`smith.Tensor`, it assumes that the input is already sharded
    ...
    Keyword Args:
        ...
        use_local_output (bool, optional):
            Whether to use local :class:`smith.Tensor` instead of :class:`DTensor` for the module output, default: False.
    Returns:
        A :class:`ParallelStyle` object that represents Sequence Parallel of the ``nn.Module``.

    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from smith.distributed.tensor.parallel import parallelize_module, SequenceParallel
        >>> from smith.distributed.device_mesh import init_device_mesh
        >>> ...
    ...
    def _replicate_module_fn(
        ...
        for p_name, param in module.named_parameters():
            ...
            replicated_param = smith.nn.Parameter(
    ...
    def _prepare_input_fn(sequence_sharding, mod, inputs, device_mesh):
        ...
        if isinstance(input_tensor, DTensor):
            ...
            return input_tensor
        elif isinstance(input_tensor, smith.Tensor):
            ...
            raise ValueError(
                f"expecting input of {mod} to be a smith.Tensor or DTensor, but got {input_tensor}"
            )
...
class PrepareModuleInput(ParallelStyle):
    ...
    Keyword Args:
        input_layouts (Union[Placement, Tuple[Optional[Placement]]]):
            ...
            DTensors. If some inputs are not smith.Tensor or no need to convert to DTensors, ``None`` need to be specified
        ...
        use_local_output (bool, optional):
            Whether to use local :class:`smith.Tensor` instead of :class:`DTensor` for the module inputs, default: False.
    Returns:
        A :class:`ParallelStyle` object that prepares the sharding layouts of the nn.Module's inputs.

    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from smith.distributed.tensor.parallel import parallelize_module, PrepareModuleInput
        >>> from smith.distributed.device_mesh import init_device_mesh
        >>> ...
        ...
        if input_layout is not None:
            if isinstance(input, DTensor):
                ...
                assert isinstance(input, smith.Tensor), (
                    "expecting input to be a smith.Tensor!"
                )
...
class PrepareModuleOutput(ParallelStyle):
    ...
    Keyword Args:
        output_layouts (Union[Placement, Tuple[Placement]]):
            ...
            DTensors if they are :class:`smith.Tensor`. If some outputs are not smith.Tensor or no need to convert to DTensors,
        ...
        use_local_output (bool, optional):
            Whether to use local :class:`smith.Tensor` instead of :class:`DTensor` for the module outputs, default: True.
    Returns:
        A ParallelStyle object that prepares the sharding layouts of the nn.Module's outputs.

    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from smith.distributed.tensor.parallel import parallelize_module, PrepareModuleOutput
        >>> from smith.distributed.device_mesh import init_device_mesh
        >>> ...
        >>> block = TransformerBlock(...)  # block is a nn.Module that contains an "attn" Attention submodule
        >>> tp_mesh = init_device_mesh("cuda", (8,))
...
class PrepareModuleInputOutput(ParallelStyle):
    ...
    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from smith.distributed.tensor.parallel import parallelize_module, PrepareModuleInputOutput
        >>> from smith.distributed.device_mesh import init_device_mesh
        >>> ...
        >>> block = TransformerBlock(...)  # block is a nn.Module that contains an "attn" Attention submodule
        >>> tp_mesh = init_device_mesh("cuda", (8,))
