...
from smith.distributed.tensor._ops._embedding_ops import _MaskPartial
from smith.distributed.tensor._ops._math_ops import (
    _skip_dim,
    Reduction,
    replicate_reduction_dims,
)
from smith.distributed.tensor._ops.utils import normalize_dim
...
def loss_parallel():
    ...
    Within this context manager, one can use :func:`~smith.nn.functional.cross_entropy` or
    :class:`~smith.nn.CrossEntropyLoss` as usual, with the following assumptions on the input parameters.
    ...
    Args:
        ...
        target (Union[:class:`smith.Tensor`, :class:`DTensor`]):
        ...
        weight (Union[:class:`smith.Tensor`, :class:`DTensor`], optional):
    ...
    Example:
        ...
        >>> from smith.distributed.tensor.parallel import loss_parallel
        >>> from smith.distributed.device_mesh import init_device_mesh
        >>> ...
        >>> device_mesh = init_device_mesh("cuda", (8,))
        >>> input = smith.randn(4, 16, device="cuda", requires_grad=True)
        >>> dist_input = distribute_tensor(input, device_mesh, placements=[Shard(1)])
        >>> target = smith.randint(16, (4,), device="cuda")
        >>> with loss_parallel():
...
def _cast_to_dtensor(
    ...
    elif isinstance(tensor, smith.Tensor):
...
# NOTE: The implementation follows smith._decomp.decomposition._log_softmax,
# with all_reduce manually inserted to perform distributed computation.
def _log_softmax(x, dim, half_to_float, mesh, mesh_dim):
    if half_to_float:
        assert x.dtype == smith.half
    computation_dtype, result_dtype = utils.elementwise_dtypes(
        x, type_promotion_kind=utils.ELEMENTWISE_TYPE_PROMOTION_KIND.DEFAULT
    )
    x = x.to(dtype=computation_dtype, memory_format=smith.contiguous_format)
    if x.numel() == 0:
        shifted = x
    else:
        ...
        shifted = x - x_max
    shifted_sumexp = smith.sum(smith.exp(shifted), dim, keepdim=True)
    shifted_sumexp = funcol.all_reduce(
        shifted_sumexp, reduceOp=c10d.ReduceOp.SUM.name, group=(mesh, mesh_dim)
    )
    shifted_logsumexp = smith.log(shifted_sumexp)
...
def _log_softmax_backward_handler(
    ...
    input_dtype = cast(smith.dtype, args[3])
    return grad_output.to(input_dtype)


# NOTE: The implementation follows smith._decomp.decomposition._nll_loss_forward,
...
def _nll_loss_forward(
    ...
    result_partial = smith.gather(x, channel_dim, safe_target_partial_)
...
# from smith._decomp.decomposition.
def _nll_loss_and_log_softmax_backward(
    ...
    if weight is not None:
        ...
        w_target = smith.gather(w, channel_dim, target)
    ...
    return (grad_input + smith.exp(x)) * grad_output
