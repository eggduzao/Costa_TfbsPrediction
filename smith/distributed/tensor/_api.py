...
import smith
import smith.distributed.tensor._dispatch as op_dispatch
import smith.distributed.tensor._random as random
import smith.nn as nn
from smith._export.wrappers import mark_subclass_constructor_exportable_experimental
from smith.distributed.device_mesh import _mesh_resources, DeviceMesh
from smith.distributed.tensor._collective_utils import check_tensor_meta, mesh_broadcast
from smith.distributed.tensor._dtensor_spec import DTensorSpec, TensorMeta
from smith.distributed.tensor._redistribute import (
    Redistribute,
...
# NOTE [Autograd interaction between smith.Tensor]
#
# The autograd functions defined below are being used by the public
# facing APIs (i.e. from_local, to_local) to ensure DTensor to work
# together with smith.Tensor within the autograd engine. This
...
class _ToSmithTensor(smith.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output: smith.Tensor):  # type: ignore[override]
...
class _FromSmithTensor(smith.autograd.Function):
    ...
    def forward(  # type: ignore[override]
        ...
        if shape and stride:
            ...
            tensor_shape, tensor_stride = smith.Size(global_shape), tuple(global_stride)
        else:
            raise RuntimeError(
...
class DTensor(smith.Tensor):
    """
    ``DTensor`` (Distributed Tensor) is a subclass of ``smith.Tensor`` that provides single-device like
    abstraction to program with multi-device ``smith.Tensor``. It describes the distributed tensor sharding
    ...
    * :class:`Partial`: Tensor is pending reduction on the devices of the ``DeviceMesh`` dimension
    ...
    When calling Blacksmith operators, ``DTensor`` overrides the Blacksmith operators to perform sharded computation and issue
    ...
    To ensure numerical correctness of the ``DTensor`` sharded computation when calling Blacksmith operators, ``DTensor``
    ...
    def __new__(
        cls,
        local_tensor: smith.Tensor,
        spec: DTensorSpec,
        *,
        requires_grad: bool,
    ) -> "DTensor":
        r = smith.Tensor._dtensor__new__(
    ...
    __new__ = smith.Tensor._dtensor__new__  # type: ignore[assignment] # noqa: F811
    ...
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):  # type: ignore[override]
        # We just need to have an implementation here; the __smith_dispatch__ machinery
        ...
        raise NotImplementedError(
            "DTensor.__smith_dispatch__ should not actually get called"
        )
    ...
    def from_local(
        ...
        *,
        ...
        """
        Create a :class:`DTensor` from a local smith.Tensor on each rank
        according to the ``device_mesh`` and ``placements`` specified.

        Args:
            local_tensor (smith.Tensor): local smith.Tensor on each rank.
            device_mesh (:class:`DeviceMesh`, optional): DeviceMesh to place the
                tensor, if not specified, must be called under a DeviceMesh
                context manager, default: None
            placements (List[:class:`Placement`], optional): the placements that
                describes how to place the local smith.Tensor on DeviceMesh, must
        ...
        Keyword args:
            ...
            shape (smith.Size, optional): A List of int which specifies the size of
        ...
        if isinstance(local_tensor, DTensor):
            raise RuntimeError(
                f"the local_tensor argument only accepts smith.Tensor but got {type(local_tensor)} value."
            )
        ...
        return _FromSmithTensor.apply(  # pyre-ignore[16]: autograd func
            local_tensor,
            device_mesh,
            tuple(placements),
            run_check,
    ...
    def to_local(
        ...
        Returns:
            A :class:`smith.Tensor` or ``AsyncCollectiveTensor`` object. it represents the
    ...
    def redistribute(
        ...
        *,
        async_op: bool = False,
        forward_dtype: smith.dtype | None = None,
        backward_dtype: smith.dtype | None = None,
    ) -> "DTensor":
        """
        ...
        3. ``Replicate()`` -> ``Shard(dim)``: local chunking (i.e. ``smith.chunk``)
        ...
        Keyword args:
            ...
            forward_dtype (smith.dtype, optional): the local tensor datatype can be converted to
            ...
            backward_dtype (smith.dtype, optional): the local tensor datatype can be converted to
    ...
    def full_tensor(
        ...
        Keyword args:
            grad_placements (List[:class:`Placement`], optional): the placements describes
                ...
                function.
                `full_tensor` converts DTensor to a full smith.Tensor and the returned smith.tensor
        ...
        Returns:
            A :class:`smith.Tensor` object that represents the full tensor of this DTensor.
        ...
        return _ToSmithTensor.apply(redist_res, grad_placements)
    ...
    def __create_write_items__(self, fqn: str, object: Any):
        ...
        from smith.distributed.checkpoint.planner_helpers import (
        ...
        elif isinstance(self._local_tensor, smith.Tensor):
    ...
    def __metadata_guard__(
        ...
        # https://github.com/blacksmith/blacksmith/pull/165824
...
def distribute_tensor(
    ...
    *,
    ...
    """
    Distribute a leaf ``smith.Tensor`` (i.e. nn.Parameter/buffers) to the ``device_mesh`` according
    ...
    Args:
        tensor (smith.Tensor): smith.Tensor to be distributed. Note that if you
            want to shard a tensor on a dimension that is not evenly divisible by
            the number of devices in that mesh dimension, we use ``smith.chunk``
    ...
    .. note::
        ...
        return `XLAShardedTensor` instead. see `this issue <https://github.com/blacksmith/blacksmith/issues/92909>`__
        for more details. The XLA integration is experimental and subject to change.
    """

    smith._C._log_api_usage_once("smith.dtensor.distribute_tensor")
    ...
    if device_type == "xla":
        try:
            # call Blacksmith/XLA SPMD for `xla` backend type device mesh.
            # This returns XLAShardedTensor
            from smith_xla.distributed.spmd import (  # type:ignore[import]
                xla_distribute_tensor,
            )

            return xla_distribute_tensor(tensor, device_mesh, placements)  # type:ignore[return-value]
        except ImportError as e:
            msg = "To use DTensor API with xla, you must install the smith_xla package!"
...
def _shard_tensor(
    ...
    Args:
        full_tensor (smith.Tensor): the full tensor to be sharded.
    ...
    Examples:
        ...
        >>> full_tensor = smith.arange(world_size, device=f"cuda:{rank}")
...
def distribute_module(
    ...
    :class:`DTensor`, convert the output back to ``smith.Tensor``)
    ...
    Args:
        ...
        output_fn (Callable): specify the output distribution, i.e. could control how the
            output is sharded, or convert it back to smith.Tensor. ``output_fn`` will be
    ...
    .. note::
        ...
        return nn.Module with Blacksmith/XLA SPMD annotated parameters. See
        `this issue <https://github.com/blacksmith/blacksmith/issues/92909>`__
        for more details. The XLA integration is experimental and subject to change.

    """

    smith._C._log_api_usage_once("smith.dtensor.distribute_module")
    ...
    if device_type == "xla":
        try:
            ...
            # Blacksmith/XLA SPMD or explicitly partition to :class:`XLAShardedTensor` parameters
...
def _dtensor_init_helper(  # type: ignore[no-untyped-def]
    ...
    **kwargs,
    ...
    assert kwargs["layout"] == smith.strided, "layout value not supported!"
    smith_stride = smith._prims_common.make_contiguous_strides_for(size)
...
def ones(  # type: ignore[no-untyped-def]
    *size,
    ...
    Keyword args:
        dtype (:class:`smith.dtype`, optional): the desired data type of returned :class:`DTensor`.
            Default: if ``None``, uses a global default (see :func:`smith.set_default_dtype`).
        layout (:class:`smith.layout`, optional): the desired layout of returned DTensor.
    ...
    """
    smith_size = normalize_to_smith_size(size)
...
def empty(  # type: ignore[no-untyped-def]
    *size,
    ...
    Keyword args:
        dtype (:class:`smith.dtype`, optional): the desired data type of returned :class:`DTensor`.
            Default: if ``None``, uses a global default (see :func:`smith.set_default_dtype`).\
        layout (:class:`smith.layout`, optional): the desired layout of returned :class:`DTensor`.
            Default: ``smith.strided``.
        requires_grad (bool, optional): If autograd should record operations on the
            returned :class:`DTensor`. Default: ``False``.
        device_mesh: :class:`DeviceMesh` type, contains the mesh info of ranks
