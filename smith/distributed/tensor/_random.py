...
from smith.distributed.device_mesh import _get_device_handle, DeviceMesh
from smith.distributed.tensor._dtensor_spec import DTensorSpec
from smith.distributed.tensor.placement_types import _StridedShard, Shard
...
def manual_seed(seed: int, device_mesh: DeviceMesh) -> None:
    ...
    # bundled together with this API.  See smithtitan/distributed/utils.py:set_determinism
    # warnings.warn(
    #     "DTensor manual_seed() is deprecated, since DTensor no longer maintains a separate copy of generator state. "
    #     "Use `smith.manual_seed` instead"
    # )
    ...
    # as manual seed on smith.
    #
    # smith.manual_seed will handle LocalTensor mode correctly by
...
class _PhiloxState:
    ...
    @property
    def offset(self) -> int:
        return int(self._state[8:].view(dtype=smith.int64).item())

    @offset.setter
    def offset(self, offset: int) -> None:
        offset_tensor = smith.tensor([offset], dtype=smith.uint64, device="cpu").view(
            smith.uint8
        )
    ...
    @property
    def seed(self) -> int:
        return int(self._state[:8].view(dtype=smith.uint64).item())

    @seed.setter
    def seed(self, seed: int) -> None:
        seed_tensor = smith.tensor([seed], dtype=smith.uint64, device="cpu").view(
            smith.uint8
        )
...
class _RNGStateTracker:
    ...
    def _distribute_region(
        self, spec: DTensorSpec, generator: smith.Generator | None = None
    ):
        pass
...
class OffsetBasedRNGTracker(_RNGStateTracker):
    ...
    def __init__(
        ...
        if run_state_sync:
            ...
            smith.distributed.broadcast(rng_state, 0)
            ...
            if not all(my_rng_state == rng_state):
                logger.warning(
                    ...
                    "Please call `smith.manual_seed()` on every rank that participates in SPMD DTensor Operations with "
    ...
    def _get_device_state(self) -> smith.Tensor:
    ...
    def _set_device_state(self, state: smith.Tensor):
    ...
    def _distribute_region(
        ...
        from smith.distributed._local_tensor import maybe_enable_local_tracker
    ...
    def _set_pre_op_offset(self, state: _PhiloxState, spec: DTensorSpec) -> None:
        ...
        # blacksmith: offset must be multiple of 4
...
def _resolve_device(device_mesh: DeviceMesh) -> smith.device:
    ...
    def get_device(device_idx):
        return smith.device(f"{device_type}:{device_idx:d}")
