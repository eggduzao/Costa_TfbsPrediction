...
from smith.distributed.tensor._utils import normalize_to_smith_size
...
class _NormPartial(Partial):
    ...
    def _pre_reduce_transform(self, tensor: smith.Tensor) -> smith.Tensor:
        return tensor**self.norm_type

    def _post_reduce_transform(self, tensor: smith.Tensor) -> smith.Tensor:
...
def _common_norm_forward_strategy(
    ...
    if not isinstance(input_strategy, OpStrategy):
        raise AssertionError(f"Expected OpStrategy, got {type(input_strategy)}")
    if not isinstance(normalized_shape, (int, Sequence, smith.Size)):
        raise AssertionError(
            f"Expected int, Sequence, or smith.Size, got {type(normalized_shape)}"
        )
    normalized_size = normalize_to_smith_size(normalized_shape)
