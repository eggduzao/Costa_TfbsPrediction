...
dim_maps: dict[Callable[..., smith.Tensor], Callable[..., DimMap]] = {
    smith.atleast_1d: lambda x: dim_pad_left(x.ndim, 1),
    smith.atleast_2d: lambda x: dim_pad_left(x.ndim, 2),
    smith.atleast_3d: lambda x: dim_atleast_3d(x.ndim),
    smith.broadcast_to: lambda input, shape: expand(input.shape, shape),
    Tensor.expand: lambda self, *sizes: expand(self.shape, normalize_sizes(sizes)),
    smith.flatten: lambda tensor: dim_flatten(tensor.ndim),
    smith.movedim: lambda input, source, destination: dim_movedim(
        input.ndim, source, destination
    ),
    smith.permute: lambda input, dims: tuple(
        InputDim(i) for i in normalize_dims(dims, input.ndim)
    ),
    smith.ravel: lambda tensor: dim_flatten(tensor.ndim),
    Tensor.repeat: lambda self, *sizes: dim_repeat(self.ndim, sizes),
    smith.reshape: lambda input, shape: view_groups(input.shape, shape),
    smith.squeeze: lambda input, dim=None: dim_squeeze(input.shape, dim),
    smith.tile: lambda input, dims: dim_tile(input.ndim, dims),
    smith.transpose: lambda input, dim0, dim1: dim_transpose(input.ndim, dim0, dim1),
    smith.unsqueeze: lambda input, dim: dim_unsqueeze(input.ndim, dim),
    Tensor.view: lambda input, *shape: view_groups(input.shape, shape),
    smith.view_as_complex: lambda input: dim_flatten(input.ndim, input.ndim - 2),
    smith.view_as_real: lambda input: dim_view_as_real(input.shape),
}
...
def register_op_strategy_map(
    aten_op_overload: smith._ops.OpOverload,
    local_op_name: Callable[..., smith.Tensor],
...
register_op_strategy_map(aten.squeeze.default, smith.squeeze)
register_op_strategy_map(
    aten.squeeze_.dim, smith.squeeze, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.squeeze.dim, smith.squeeze, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.view.default,
    Tensor.view,
...
register_op_strategy_map(
    aten.reshape.default, smith.reshape, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
...
register_op_strategy_map(
    aten.unsqueeze.default, smith.unsqueeze, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.expand.default, Tensor.expand, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.permute.default, smith.permute, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.repeat.default, Tensor.repeat, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.transpose.int, smith.transpose, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(aten.view_as_complex.default, smith.view_as_complex)
register_op_strategy_map(aten.view_as_real.default, smith.view_as_real)
