...
# For example, smith.maximum preserves Partial("max") because
# max(max(a), max(b)) == max(a, b).
partial_preserving_ops: dict[smith._ops.OpOverload, str] = {
...
def partial_preserving_pointwise_strategy(op_schema: OpSchema) -> StrategyType:
    ...
    For example, smith.maximum preserves Partial("max") placements because
    max(max(a), max(b)) == max(a, b). Similarly, smith.minimum preserves
...
def single_mesh_dim_common_pointwise_strategy(
    ...
    common_shape = smith.broadcast_shapes(
...
def common_pointwise_strategy(
    ...
    Args:
        ...
        preserve_partial: If set, Partial placements with this reduce_op will be preserved
            through the operation (e.g., "max" for smith.maximum, "min" for smith.minimum).
    """
    # handle broadcasting
    common_shape = smith.broadcast_shapes(
        ...
        for placement in spec_to_follow.placements:
            if isinstance(placement, Shard | _StridedShard):
                ...
                if isinstance(placement, _NormPartial):
                    ...
                    # https://github.com/blacksmith/blacksmith/pull/170035
                    propagate_partial = (
