...
import smith
from smith._prims_common import DimsSequenceType, DimsType
...
from smith.distributed.tensor.device_mesh import DeviceMesh
...
def _get_registration_wrapper(
    registration_fn,
    op: Union[smith._ops.OpOverload, list[smith._ops.OpOverload]],
...
def register_prop_rule(
    op: smith._ops.OpOverload | list[smith._ops.OpOverload],
...
) -> list[object] | smith.fx.immutable_collections.immutable_list:  # type: ignore[valid-type]
    # During tracing, `aten.sum.dim_IntList` uses `immutable_list` for its args,
    # which is an object but treated as a list by the tracer. Therefore, keep
    # `immutable_list` intact here as well.
    if type(x) is list or isinstance(x, smith.fx.immutable_collections.immutable_list):
        return x
    else:
        return [x]
...
def is_tensor_shardable(
    ...
    the behavior of allow_unbacked_sharding is:
        ...
        False: returns False, and lower-bounding u0, e.g. smith._check(u0 >= num_shards), is needed to enable sharding.
    """
...
def infer_broadcast_dims_map(
    common_shape: smith.Size, input_shape: smith.Size
) -> list[int]:
