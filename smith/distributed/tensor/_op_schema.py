...
distributed tensor operations in Blacksmith's DTensor system. It provides the foundational
...
try:
    from smith.utils._cxx_pytree import (
    ...
    from smith.utils._pytree import (  # type: ignore[no-redef, assignment]
        register_pytree_node,
        tree_leaves,
        tree_map_only,
        TreeSpec,
...
class TupleStrategy(StrategyType):
    ...
    For example, [out_a, out_b] = smith.foreach_add([a,  b], scalar): input a's sharding only affects out_a's sharding,
    independent of b and out_b.

    An example of an operator that should NOT use TupleStrategy is smith.split.  It produces a List[Tensor]
...
class OpSchema:
    ...
    def return_type_tuple_tensor_like(self) -> bool:
        ...
        return len(return_types) > 1 and isinstance(
            return_types[0].type, smith.TensorType
        )
    ...
    def return_type_list_tensor_like(self) -> bool:
        ...
        return_types = self.op._schema.returns
        return len(return_types) == 1 and isinstance(
            return_types[0].type, smith.ListType
        )
