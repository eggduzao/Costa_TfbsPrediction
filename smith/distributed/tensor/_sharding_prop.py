...
def _get_expected_num_tensor_outputs(op: OpOverload) -> int:
    ...
    if isinstance(first_return.type, smith.TensorType):
        # Could be single tensor or tuple of tensors
        return len(return_types)
    elif isinstance(first_return.type, smith.ListType):
...
def _select_min_cost_strategy(
    ...
    op_spec_costs: list[smith.types.FloatLikeType] = []
...
class ShardingPropagator:
    ...
    def _propagate_tensor_meta_non_cached(
        ...
        if isinstance(fake_out, smith.Tensor):
            return TensorMeta(
            ...
            for fake_out_item in fake_out:
                if isinstance(fake_out_item, smith.Tensor):
    ...
    def _wrap_with_op_strategy(self, op_schema: OpSchema) -> OpSchema:
        ...
        strategy propagation on blacksmith operators.
        """
    ...
    def propagate(self, op_info: OpInfo) -> None:
        ...
        # This is generally ok because this only happens during tracing in smith.compile,
    ...
    def propagate_op_sharding_non_cached(self, op_schema: OpSchema) -> OutputSharding:
        ...
        smith._C._log_api_usage_once(
            "smith.distributed.tensor._sharding_prop.ShardingPropagator.propogate_op_sharding_non_cached"
        )
        ...
        if single_dim_strategy is not None or op_strategy_func is not None:
            ...
            from smith.distributed.tensor._decompositions import DecompShardingStrategy
