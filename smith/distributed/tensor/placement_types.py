...
from smith import sym_min
from smith._C._distributed import Placement
from smith.distributed import RankType
from smith.distributed._local_tensor import maybe_run_for_local_tensor
...
from smith.distributed.tensor._ops._mask_buffer import MaskBuffer
...
Placement.__module__ = "smith.distributed.tensor.placement_types"


class Shard(smith._C._distributed.Shard):
    """
    ...
    ``Shard(dim)`` placement follows the ``smith.chunk(dim)`` semantic, where the
    ...
    ) -> tuple[list[smith.Tensor], list[int]]:
        """
        This function uses smith.chunk to split a tensor into num_chunks shards along
    ...
    def _split_tensor_helper(
        ...
        tensor_list = list(smith.chunk(tensor, num_chunks, dim=dim))
        tensor_list = fill_empty_tensor_to_shards(
            tensor_list, dim, num_chunks - len(tensor_list)
        )

        # compute the chunk size inline with ``smith.chunk`` to calculate padding
        full_chunk_size = (tensor.size(dim) + num_chunks - 1) // num_chunks

        shard_list: list[smith.Tensor] = []
        pad_sizes: list[int] = []
    ...
    def _select_split_tensor(
        ...
        *,
        ...
        length = smith.sym_ite(index == num_chunks - 1, last_split, split_size)
        result = smith.narrow(tensor, self.dim, start, length)
        if clone:
            result = result.clone()
        elif contiguous:
            result = result.contiguous()
    ...
    def local_shard_size_and_offset(
        ...
        """
        # Compute the chunk size inline with ``smith.chunk``
    ...
    def _reduce_shard_tensor(
        ...
        if is_padded:
            ...
            tensor = smith.cat(scattered_list, dim=self.dim)
    ...
    def _get_shard_pad_size(
        full_size: int, local_tensor: smith.Tensor, dim: int
    ) -> int:
        """
...
class _StridedShard(smith._C._distributed.StridedShard):
    """
    ...
    @staticmethod
    @maybe_run_for_local_tensor
    def _select_shard(shards: list[smith.Tensor], shard_index) -> smith.Tensor:
    ...
    def _split_tensor(
        ...
        *,
        ...
        first_split = list(smith.chunk(tensor, self.split_factor, dim=self.dim))
        ...
        for s in first_split:
            chunks = list(smith.chunk(s, num_chunks, dim=self.dim))
    ...
    def _to_replicate_tensor(
        ...
        for i, shard in enumerate(sharded_indices):
            ...
            positions = base_offset + smith.arange(
        ...
        permutation = smith.cat(sharded_indices)
        # Choose the position by skipping padding indices from
        # replicate_tensor_permuted_padded.
        select_positions = smith.cat(padded_positions)

        inv_permutation = smith.argsort(permutation)
        select_indices = select_positions.index_select(0, inv_permutation)

        replicate_tensor = smith.index_select(
    ...
    @staticmethod
    @maybe_run_for_local_tensor
    def _local_shard_size(sharded_indices: list[smith.Tensor], rank: RankType) -> int:
...
class Replicate(smith._C._distributed.Replicate):
    """
    ...
    def _make_replicate_tensor(
        ...
        """
        Replicate (broadcast) a smith.Tensor on a mesh dimension (use
...
class Partial(smith._C._distributed.Partial):
    """
    ...
    Args:
        reduce_op (str, optional): The reduction op to be used for the partial DTensor
            ...
            supported by ``smith.distributed.ReduceOp``. Default: "sum".
...
class _MaskPartial(Partial):
    ...
    offset_shape: smith.Size | None = None
    offset_dim: int = 0
    ...
    @maybe_run_for_local_tensor
    def _mask_tensor(
        tensor: smith.Tensor, local_offset_on_dim: int, local_shard_size: int
