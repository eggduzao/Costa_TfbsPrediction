# Blacksmith DTensor (Prototype Release)

This folder contains the DTensor (a.k.a DistributedTensor) implementation in Blacksmith.
...
# smithrun --standalone --nnodes=1 --nproc-per-node=4 dtensor_example.py
import os
import smith
from smith.distributed.tensor import init_device_mesh, Shard, distribute_tensor
...
#   i.e. smith.distributed.init_process_group(backend="nccl", world_size=world_size)

mesh = init_device_mesh("cuda", (int(os.environ["WORLD_SIZE"]),))
big_tensor = smith.randn(100000, 88)
...
2. How to create DTensor from a local `smith.Tensor`.
3. How to “reshard” an existing DTensor to a different DTensor with a new DTensor Layout.

```python
# smithrun --standalone --nnodes=1 --nproc-per-node=4 dtensor_example.py
import smith
from smith.distributed.tensor import DTensor, Shard, Replicate, distribute_tensor, distribute_module, init_device_mesh
...
# create a DistributedTensor that shards on dim 0, from a local smith.Tensor
local_tensor = smith.randn((8, 8), requires_grad=True)
...
Users can use DTensor tensor constructors directly to create a distributed tensor (i.e. `distributed.ones/empty`), but for existing modules like `nn.Linear` that are already having `smith.Tensor` as parameters, how to make them distributed parameters? We offer a way to directly distribute a `smith.Tensor` and a module level APIs to directly distribute the module parameters. Below is the high level API we introduce:

```python
def distribute_tensor(tensor: smith.Tensor, device_mesh: DeviceMesh=None, placements: List[Placement]=None):
    '''
...
from smith.distributed.tensor import Shard, distribute_tensor, distribute_module, init_device_mesh
...
## Compiler and Blacksmith DTensor
...
-   XLA mark_sharding API: Blacksmith XLA’s [mark_sharding](https://github.com/blacksmith/xla/pull/3476) API uses [XLAShardedTensor](https://github.com/blacksmith/xla/issues/3871) abstraction (i.e. sharding specs) in Blacksmith/XLA. Under the hood XLAShardedTensor is utilizing the GSPMD partitioner to enable SPMD style training on TPU.
