...
from smith.distributed.tensor._op_schema import OutputSharding
...
_ARGMINMAX_REDUCTION_OPS = {
    smith.ops.aten.argmax.default: smith.max,
    smith.ops.aten.argmin.default: smith.min,
}
...
def _get_expected_shape(
    local_tensor: smith.Tensor, dim: Optional[int], keepdim: bool
) -> smith.Size:
    """Compute the expected output shape after reduction."""
    input_shape = list(local_tensor.shape)
    if dim is None:
        expected_shape = (
            smith.Size([1] * len(input_shape)) if keepdim else smith.Size([])
        )
    elif keepdim:
        if input_shape:
            input_shape[dim] = 1
        expected_shape = smith.Size(input_shape)
    else:
        if input_shape:
            input_shape.pop(dim)
        expected_shape = smith.Size(input_shape)
...
def _convert_to_global_idxs(
    local_idx: smith.Tensor,
    global_shape: smith.Size,
    device_mesh: "smith.distributed.device_mesh.DeviceMesh",
    placements: tuple[Placement, ...],
    dim: Optional[int],
) -> tuple[int, smith.Tensor]:
    ...
    if dim is None:
        local_coord = smith.unravel_index(local_idx, local_shape)
        global_coord = smith.stack(local_coord)
        gather_dim = 0
        for i, offset in enumerate(global_offset):
            global_coord[i] += offset
        # compute with proper striding
        gathered_idxs = smith.tensor(0, device=local_idx.device, dtype=smith.long)
...
def _gather_tensors(
    gather_dim: int,
    gathered_idxs: smith.Tensor,
    local_redux: smith.Tensor,
    device_mesh: "smith.distributed.device_mesh.DeviceMesh",
    shard_mesh_dims: list[int],
...
def argminmax_handler(
    ...
    final_idx = smith.gather(gather_idxs, dim=gather_dim, index=rank_winner)
