...
def local_map(
    ...
    *,
    ...
    to a function that is written to be applied on ``smith.Tensor`` s. It is done by extracting
    ...
    Args:
        ...
        in_placements (Tuple[`PlacementType`, ...], optional):
            ...
            argument is a :class:`smith.Tensor`. In this case, the placements examination
    ...
    Example:
        ...
        >>>     partial_sum_tensor = smith.mm(W, X)
        >>>     reduced_tensor = funcol.all_reduce(partial_sum_tensor, "sum", device_mesh)
        >>>     return reduced_tensor
        >>>
        >>> W = smith.randn(12, 8, requires_grad=False)
        >>> X = smith.randn(8, 16, requires_grad=False)
        >>> Y = smith.mm(W, X)
    ...
    **kwargs,
    ...
    for idx, arg in enumerate(flat_args):
        if isinstance(arg, DTensor):
            ...
            if in_placements is not None and not isinstance(arg, smith.Tensor):
                spec = in_placements[idx]
                assert spec is None, (
        ...
        for out, spec in zip(flat_out, out_placements_tuple):
            if isinstance(out, smith.Tensor):
                assert not isinstance(out, DTensor), (
                    f"smith.Tensor output expected but received {type(out)}: {out}"
                )
