...
def register_sharding(op: OpOverload | list[OpOverload]):
    ...
    Returns:
        ...
        as the original op (except that if an arg is a :class:`smith.Tensor`, it will be
    ...
    def wrapper(custom_sharding_fn):
        def derive_schema_info(op):
            ...
            for i, arg in enumerate(op._schema.arguments):
                if isinstance(arg.type, smith.IntType) or (
                    isinstance(arg.type, smith.OptionalType)
                    and isinstance(arg.type.getElementType(), smith.IntType)
                ):
