...
from smith.distributed.tensor.parallel.style import ColwiseParallel, ParallelStyle
from smith.distributed.tensor.placement_types import Placement, Replicate, Shard
from smith.export import ExportedProgram
from smith.export.exported_program import ExportGraphSignature
...
class _TensorParallelTransformPass(PassBase):
    ...
    def __init__(
        ...
        super().__init__()
        self.rank = rank
        self.mesh = DeviceMesh(device_type, smith.arange(world_size))
        self.state_dict: dict[str, smith.Tensor] = state_dict
...
def _partitioner(gm: smith.fx.GraphModule) -> smith.fx.GraphModule:
    """
...
def _insert_reshard_gm(
    ...
    def reshard_fn(local_tensor: smith.Tensor) -> smith.Tensor:
...
def _clean_up_graph_metadata(gm: smith.fx.GraphModule) -> None:
    """
    ...
    for node in gm.graph.nodes:
        ...
        if "val" in node.meta and isinstance(node.meta["val"], smith.Tensor):
...
def _get_op_schema(node: Node, placement_strategies: dict[Node, OpSpec]) -> OpSchema:
    ...
    op_schema = OpSchema(
        op=cast(smith._ops.OpOverload, node.target),
...
def _shard_state_dict(
    ...
    for node, op_spec in placement_strategies.items():
        ...
        state_dict[fqn] = (
            smith.nn.Parameter(local_param)
            if isinstance(original_param, smith.nn.Parameter)
            else local_param
        )
