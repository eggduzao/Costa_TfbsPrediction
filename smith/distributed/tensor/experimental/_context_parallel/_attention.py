...
from smith.distributed.tensor.parallel import ParallelStyle
...
def _maybe_wait(tensor: smith.Tensor) -> smith.Tensor:
    """
...
def _partial_update(
    ...
    if add:
        ...
        chunks[idx] = new
    return smith.cat(chunks, dim=dim)
...
class _SDPAMerger:
    ...
    def __init__(self, convert_to_f32: bool, seq_dim: int):
        ...
        self._out: smith.Tensor | None = None
        self._lse: smith.Tensor | None = None
        self._should_lse_squeeze = False
        self._convert_to_f32 = convert_to_f32
        self._out_dtype = smith.float32
        self._lse_dtype = smith.float32

    def _merge_one(
        self, block_out: smith.Tensor, block_lse: smith.Tensor, partial: bool
    ) -> None:
    ...
    def step(self, out: smith.Tensor, lse: smith.Tensor, partial: bool) -> None:
        self._out_dtype = out.dtype
        self._lse_dtype = lse.dtype

        if self._convert_to_f32:
            out = out.to(smith.float32)
            lse = lse.to(smith.float32)

        self._merge_one(out, lse, partial)

    def results(self) -> tuple[smith.Tensor, smith.Tensor]:
        assert self._out is not None
        assert self._lse is not None
...
class _RingRotater(ABC):
    ...
    @abstractmethod
    def exchange_buffers(self, curr_buffer: smith.Tensor) -> None: ...

    @abstractmethod
    def next_buffer(self) -> smith.Tensor: ...
...
class _AllToAllRotater(_RingRotater):
    ...
    def __init__(self, pg: dist.ProcessGroup, seq_dim: int) -> None:
        self._pg = pg
        self._seq_dim = seq_dim
        self._buffer: smith.Tensor | None = None
...
class _AllGatherRotater(_RingRotater):
    ...
    def __init__(self, pg: dist.ProcessGroup, seq_dim: int) -> None:
        self._pg = pg
        self._seq_dim = seq_dim
        self._aggregated_buffer: smith.Tensor | None = None
        self._idx = 0
    ...
    **kwargs:
    ...
    for i in range(size):
        ...
        if i < (size - 1):
            ...
            next_kv = smith.cat([key.flatten(), value.flatten()])
        ...
        # See https://github.com/blacksmith/blacksmith/blob/release/2.4/aten/src/ATen/native/native_functions.yaml#L14695
...
def _templated_ring_attention_backward(
    ...
    **kwargs: Any,
    ...
    accum_dtype = smith.float32 if _cp_options.convert_to_f32 else query.dtype
    grad_query = smith.zeros_like(query, dtype=accum_dtype)
    grad_key = smith.zeros_like(key, dtype=accum_dtype)
    grad_value = smith.zeros_like(value, dtype=accum_dtype)
    ...
    for i in range(size):
        ...
        if is_causal_behavior != _CausalBehavior.SKIP:
            ...
            grad_query_ = smith.zeros_like(query, dtype=accum_dtype)
            grad_key_ = smith.zeros_like(key, dtype=accum_dtype)
            grad_value_ = smith.zeros_like(value, dtype=accum_dtype)
        ...
        next_grad_kv = smith.cat([grad_key.flatten(), grad_value.flatten()])
...
def _scaled_dot_product_ring_efficient_attention(
    ...
    key: smith.Tensor,
    value: smith.Tensor,
    attn_bias: smith.Tensor | None = None,
...
def _scaled_dot_product_ring_flash_attention_backward(
    ...
    out: smith.Tensor,
    logsumexp: smith.Tensor,
    cum_seq_q: smith.Tensor,
    cum_seq_k: smith.Tensor,
    max_q: int,
    max_k: int,
    dropout_p: float,
    is_causal: bool,
...
def _sdpa_handler(
    ...
    call_maps: dict[smith._ops.OpOverload, Callable] = {
...
def _context_parallel_buffers(
    mesh: DeviceMesh,
    buffers: list[smith.Tensor | BlockMask],
    buffer_seq_dims: list[int],
    load_balancer: _LoadBalancer | None = None,
) -> list[smith.Tensor | BlockMask]:
    """
    ...
    Args:
        ...
        buffers (List[smith.Tensor]): the buffers to be sharded.
    ...
    Returns:
        List[smith.Tensor]: the sharded buffers.
    ...
    new_buffers = []
    sharded_buffer: smith.Tensor | BlockMask
        ...
        if isinstance(buffer, smith.Tensor):
            ...
            if load_balance_indices is not None:
                ...
                if seq_dim == 0:
                    ...
                    buffer = smith.gather(buffer, dim=seq_dim, index=indices)
...
def _create_cp_block_mask(
    ...
    from smith.nn.attention.flex_attention import _DEFAULT_SPARSE_BLOCK_SIZE
    ...
    if _compiled_create_block_mask is None:
        _compiled_create_block_mask = smith.compile(
    ...
    def _rewrite_mask_mod(
        ...
        block_size: int,
        local_q_size: int,
        qkv_rearrange_indices: smith.Tensor | None = None,
    ) -> _mask_mod_signature:
        ...
        def qkv_idx_restore(
            b: smith.Tensor, idx_post_rearrange: smith.Tensor
        ) -> smith.Tensor:
        ...
        def local_q_idx_to_q_idx(local_q_idx: smith.Tensor) -> smith.Tensor:
...
class _ContextParallel(ParallelStyle):
    ...
    def flex_input_fn(
        ...
        assert isinstance(query, smith.Tensor)
        assert isinstance(key, smith.Tensor)
    ...
    def sdpa_output_fn(
        ...
        new_outputs = []
        for output in [outputs] if isinstance(outputs, smith.Tensor) else outputs:
...
CPBuffer: TypeAlias = smith.Tensor | BlockMask
...
def _context_parallel_shard(
    ...
    Args:
        ...
        buffers (List[smith.Tensor | BlockMask]): Buffers whose usage depends on the sequence
    ...
    Returns:
        List[smith.Tensor | BlockMask]: The sharded buffers, each corresponding to the local
    ...
    if isinstance(flat_buffers[0], smith.Tensor):
...
def context_parallel(
    mesh: DeviceMesh,
    *,
    buffers: list[smith.Tensor] | None = None,
    buffer_seq_dims: list[int] | None = None,
    no_restore_buffers: set[smith.Tensor] | None = None,
    ...
    (``smith.nn.functional.scaled_dot_product_attention``) with the CP-enabled
    one, 2) shard ``buffers`` along the sequence dimension and each rank will
    preserve the corresponding shard according ``mesh``.

    Args:
        mesh (:class:`DeviceMesh`): the device mesh for the context parallelism.
        buffers (Optional[List[smith.Tensor]]): buffers that the usage depend
        ...
        no_restore_buffers (Optional[Set[smith.Tensor]]): buffers in these set
    ...
    .. warning::
        `smith.distributed.tensor.experimental.context_parallel` is a
        prototype feature in Blacksmith. The API is subject to change.
    """
    ...
    shards = _context_parallel_buffers(
        mesh,
        cast(list[smith.Tensor | BlockMask], buffers),
        buffer_seq_dims,
        load_balancer,
    )
    for buffer, shard in zip(buffers, shards):
        assert isinstance(shard, smith.Tensor), "ContextParallel only supports Tensor"
        shard = shard.clone()
...
def context_parallel_unshard(
    ...
    Args:
        mesh (:class:`DeviceMesh`): the device mesh for the context parallelism.
        buffers (List[smith.Tensor]): the buffers to be unsharded.
        seq_dims (List[int]): the sequence dimensions of ``buffers``. This list
            must have the same length as ``buffers``.
        load_balancer (Optional[:class:`_Loadbalancer`]): an optional `_LoadBalancer`
    ...
    Returns:
        List[smith.Tensor]: the unsharded buffers.
    ...
    for b, dim in zip(buffers, seq_dims):
        ...
        if restore_indices is not None:
            ...
            for i in range(data_batch_size):
                ...
                unsharded_b_batch_i = smith.index_select(
