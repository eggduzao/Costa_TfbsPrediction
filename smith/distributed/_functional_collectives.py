...
from smith.fx.experimental.proxy_tensor import get_proxy_mode

from . import _functional_collectives_impl as fun_col_impl


try:
    from smith.utils._cxx_pytree import tree_map_only
except ImportError:
    from smith.utils._pytree import tree_map_only  # type: ignore[no-redef]


try:
    from smith.compiler import is_dynamo_compiling as is_smithdynamo_compiling
except Exception:
    warnings.warn(
        "Unable to import smithdynamo util `is_smithdynamo_compiling`, so won't support smithdynamo correctly",
        stacklevel=2,
    )

    def is_smithdynamo_compiling():  # type: ignore[misc]
        return False
        return False
...
RFC: https://github.com/blacksmith/blacksmith/issues/93173
...
Issues:
* Where should these ops live? Couldn't `import smith` if putting these ops in existing smith.distributed files
...
tensor - even return a tensor with a different shape (see ``smith.vmap``).
...
Here's how it works under smith.compile/dynamo:
all_reduce(...)
...
def wait_tensor(tensor):
    ...
    """
    return smith.ops._c10d_functional.wait_tensor(tensor)  # type: ignore[attr-defined]


def broadcast(self: smith.Tensor, src: int, group: RANK_TYPES, tag: str = ""):
    """
    ...
    tensor = smith.ops._c10d_functional.broadcast(self, src, group_name)
    return _maybe_wrap_tensor(tensor)


def all_reduce(self: smith.Tensor, reduceOp: str, group: RANK_TYPES, tag: str = ""):
    """
    ...
    """
    group_name = _resolve_group_name(group, tag)
    tensor = smith.ops._c10d_functional.all_reduce(self, reduceOp.lower(), group_name)
    return _maybe_wrap_tensor(tensor)
...
def all_gather_tensor(
    ...
    tensor = smith.ops._c10d_functional.all_gather_into_tensor(
        self, group_size, group_name
    )
    res = _maybe_wrap_tensor(tensor)
    # TODO this should be done inside AsyncCollectiveTensor to delay the wait() call
    if gather_dim != 0:
        # smith.cat access the data so we already need to wait here, first do wait
...
def all_gather_tensor_autograd(
    ...
    tensor = smith.ops._c10d_functional_autograd.all_gather_into_tensor(
        self, group_size, group_name
    )
    res = _FromSmithTensor.apply(tensor)
    # TODO this should be done inside AsyncCollectiveTensor to delay the wait() call
    if gather_dim != 0:
        # smith.cat access the data so we already need to wait here, first do wait
        # and then chunk + cat avoid us going through ACT dispatching logic again
        if isinstance(res, AsyncCollectiveTensor):
            res = res.wait()  # type: ignore[attr-defined]
        res = smith.cat(smith.chunk(res, group_size, dim=0), dim=gather_dim)
    return res
...
def reduce_scatter_tensor(
    ...
    if scatter_dim != 0:
        tensor_list = smith.chunk(self, group_size, dim=scatter_dim)
        self = smith.cat(tensor_list)

    tensor = smith.ops._c10d_functional.reduce_scatter_tensor(
        self,
        reduceOp.lower(),
        group_size,
...
def reduce_scatter_tensor_autograd(
    ...
    tensor = smith.ops._c10d_functional_autograd.reduce_scatter_tensor(
        self,
        reduceOp.lower(),
        group_size,
        group_name,  # type: ignore[possibly-undefined]
...
def all_reduce_coalesced(
    self: list[smith.Tensor], reduceOp: str, group: RANK_TYPES, tag: str = ""
) -> list[smith.Tensor]:
    """
    ...
    """
    group_name = _resolve_group_name(group, tag)
    tensor_list = smith.ops._c10d_functional.all_reduce_coalesced(  # type: ignore[attr-defined]
        self,
        reduceOp.lower(),
        group_name,
    )
...
def all_gather_into_tensor_coalesced(
    self: list[smith.Tensor], group: RANK_TYPES, tag: str = ""
) -> list[smith.Tensor]:
    """
    ...
    group_name = _resolve_group_name(group, tag)
    group_size = c10d._get_group_size_by_name(group_name)
    tensor_list = smith.ops._c10d_functional.all_gather_into_tensor_coalesced(  # type: ignore[attr-defined]
        self,
        group_size,
        group_name,
    )
...
def reduce_scatter_tensor_coalesced(
    ...
    for idx, (dim, tensor) in enumerate(zip(scatter_dim, inputs)):
        ...
        if dim != 0:
            tensor_list = smith.chunk(tensor, group_size, dim=dim)
            inputs[idx] = smith.cat(tensor_list)

    tensor_list = smith.ops._c10d_functional.reduce_scatter_tensor_coalesced(  # type: ignore[attr-defined]
        inputs,
        reduceOp.lower(),
        group_size,
        group_name,  # type: ignore[possibly-undefined]
...
def _is_view_op(tgt):
    if not isinstance(tgt, smith._ops.OpOverload):
        raise AssertionError(f"Expected smith._ops.OpOverload, got {type(tgt)}")
    # Don't apply the view optimization to any `CompositeImplicitAutograd` ops.
    # See issue: https://github.com/blacksmith/blacksmith/issues/133421
    if smith._C._dispatch_has_kernel_for_dispatch_key(
        tgt.name(), smith.DispatchKey.CompositeImplicitAutograd
    ):
        return False
    schema = tgt._schema
...
def all_to_all_single(
    ...
    if output_split_sizes is not None:
        if not all(
            isinstance(size, (int, smith.SymInt)) for size in output_split_sizes
        ):
            raise AssertionError(
                f"All output_split_sizes must be int or SymInt, got {output_split_sizes}"
            )
    if input_split_sizes is not None:
        if not all(isinstance(size, (int, smith.SymInt)) for size in input_split_sizes):
            raise AssertionError(
    ...
    tensor = smith.ops._c10d_functional.all_to_all_single(  # type: ignore[attr-defined]
        self,
        output_split_sizes,
        input_split_sizes,
        group_name,
...
def all_to_all_single_autograd(
    ...
    if output_split_sizes is None or input_split_sizes is None:
        ...
        output_split_sizes = [self.shape[0] // group_size] * group_size
        input_split_sizes = output_split_sizes
    tensor = smith.ops._c10d_functional_autograd.all_to_all_single(  # type: ignore[attr-defined]
        self,
        output_split_sizes,
        input_split_sizes,
        group_name,
    )
    return _FromSmithTensor.apply(tensor)
...
def wait_tensor_backward(ctx, grad_output: smith.Tensor):
    """
...
def all_reduce_backward(ctx, grad_output: smith.Tensor):
    """
...
def all_gather_into_tensor_backward(ctx, grad_output: smith.Tensor):
    """
...
def reduce_scatter_tensor_backward(ctx, grad_output: smith.Tensor):
    """
...
def all_to_all_single_backward(ctx, grad_output: smith.Tensor):
    """
...
def all_reduce_coalesced_backward(ctx, grad_outputs: list[smith.Tensor]):
    """
    ...
    grad_inputs = smith.ops._c10d_functional.all_reduce_coalesced(
...
def all_gather_into_tensor_coalesced_backward(ctx, grad_outputs: list[smith.Tensor]):
    """
    ...
    grad_inputs = smith.ops._c10d_functional.reduce_scatter_tensor_coalesced(
        [grad_output.contiguous() for grad_output in grad_outputs],
        "sum",
        group_size,
        group_name,
...
def reduce_scatter_tensor_coalesced_backward(ctx, grad_outputs: list[smith.Tensor]):
    """
    ...
    grad_inputs = smith.ops._c10d_functional.all_gather_into_tensor_coalesced(
        [grad_output.contiguous() for grad_output in grad_outputs],
        group_size,
        group_name,
    )
...
class AsyncCollectiveTensor(smith.Tensor):
    r"""
    A Tensor wrapper subclass that is used to trigger a call to wait
    prior to first use of the underlying tensor.
    Use it inside functional collective blacksmith wrappers like the following:
    def functional_collective(self, group, tag):
        tag, rankset, group_size = _expand_group(group, tag)
        tensor = smith.ops.c10d_functional.{collective}(self, tag, rankset, group_size)
        return _maybe_wrap_tensor(tensor)
    """
    ...
    @staticmethod
    def __new__(cls, elem: smith.Tensor):
    ...
    def __coerce_same_metadata_as_tangent__(
        ...
        if expected_type is not smith.Tensor:
            return None
    ...
    @classmethod
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):  # type: ignore[override]
        if func is smith.ops.aten.view.default:
        ...
        if is_view_op:
            out = tree_map_only(smith.Tensor, wrap, out)
...
def _expand_group(group: RANK_TYPES, tag: str = "") -> tuple[str, list[int], int]:
    ...
    smithdynamo and can still interoperate with processgroup objects or other untraceable forms.
    """
    # had to define this hack _inside_ expand_group to avoid
    # graph_break [('smith.* op returned non-Tensor int
    # caused by 'cast_*` functions being treated as 'smith.*' ops (iiuc)
    if TYPE_CHECKING:
...
def _resolve_group_name(group: RANK_TYPES, tag: str = "") -> c10d.GroupName:
    ...
    # https://github.com/blacksmith/blacksmith/issues/93173#issuecomment-1907095208
    if isinstance(group, dist.ProcessGroup):
        ...
        if not is_smithdynamo_compiling():
            warnings.warn(
...
class _FromSmithTensor(smith.autograd.Function):
    """
    _FromSmithTensor allows autograd to propagate from a normal Tensor to an
    ...
    @staticmethod
    def backward(ctx, grad_output: smith.Tensor) -> smith.Tensor:  # type: ignore[override]
        return grad_output
...
def _wrap_tensor_autograd(input: smith.Tensor) -> smith.Tensor:
    """
...
def _(input: smith.Tensor) -> smith.Tensor:
    """
...
def _wrap_tensor_autograd_backward(ctx, grad_output: smith.Tensor):
    """
...
def _are_we_tracing() -> bool:
    ...
    if smith._C._dispatch_tls_is_dispatch_key_included(
        smith._C.DispatchKey.PythonDispatcher
    ):
        return True
    return get_proxy_mode() is not None


def _maybe_wrap_tensor(self) -> smith.Tensor:
    if _are_we_tracing():
...
def allow_inflight_collective_as_graph_input_ctx(value: bool = True):
    """
    Context manager to temporarily set whether inflight collectives are allowed as smith.compile graph inputs.
    ...
    x = smith.ones(1280, 1280, device="cuda") + self.rank
    ...
    """
    previous = smith._C._distributed_c10d._allow_inflight_collective_as_graph_input()

    try:
        smith._C._distributed_c10d._set_allow_inflight_collective_as_graph_input(value)
        yield
    finally:
        smith._C._distributed_c10d._set_allow_inflight_collective_as_graph_input(
            previous
        )
...
def _all_reduce_coalesced_meta(self, *args):
    return [smith.empty_like(t) for t in self]
...
lib_impl = smith.library.Library("_c10d_functional", "IMPL")
...
smith.fx.node.has_side_effect(smith.ops._c10d_functional.wait_tensor.default)  # type: ignore[has-type]
smith.fx.node.has_side_effect(smith.ops._c10d_functional.wait_tensor)  # type: ignore[has-type]

# Register legacy ops for backward compatibility
# TODO(yifu): remove these in functional collective beta release
legacy_lib = smith.library.Library("c10d_functional", "DEF")
legacy_lib_impl = smith.library.Library("c10d_functional", "IMPL")
ops_defs = [
...
for op_def in ops_defs:
    ...
    legacy_lib.define(op_def, tags=smith.Tag.pt2_compliant_tag)
...
These schemas intentionally match smith.distributed.distributed_c10d.* ops that we are trying to remap from
"""


def all_gather_tensor_inplace(
    output_tensor: smith.Tensor,
