...
from smith.distributed import ProcessGroup
from smith.export import ExportedProgram
from smith.export.unflatten import (
    _assign_attr,
    _AttrKind,
    _sink_params,
    InterpreterModule,
)
from smith.fx.node import map_aggregate
...
def _find_loss_output(mod: smith.nn.Module, g: fx.Graph, output_loss_value_spec):
...
class PipeSequential(smith.nn.Sequential):
    @staticmethod
    def from_sequential(sequential_instance: smith.nn.Sequential):
...
class LossWrapper(smith.nn.Module):
    """
    ...
    be backpropagated by PiPPy. The above class would then be instantiated like::
        ...
        loss_fn = smith.nn.MSELoss()  # for the sake of demonstration
...
smith.library.define("pippy::_pipe_split", "() -> ()")


@smith.library.impl("pippy::_pipe_split", "BackendSelect")
def _pipe_split():
    return None


@smith.library.register_fake("pippy::_pipe_split")  # type: ignore[no-redef]
def _pipe_split():  # noqa: F811
    return None


# Add an alias for convenience
aten_pipe_split_alias = smith.ops.pippy._pipe_split.default

# Ask Export to preserve the `_pipe_split` op.
# See examples in blacksmith/smith/fx/node.py
...
def pipe_split():
    ...
    """
    return smith.ops.pippy._pipe_split()
...
class DetachExecutor(fx.Interpreter):
    ...
    def call_module(self, target, args, kwargs):
        ...
        def dont_traverse_size(a):
            return type(a) is not smith.Size
        """
...
class Pipe(smith.nn.Module):
    def __init__(
        ...
        params_to_users: dict[smith.nn.Parameter, dict[str, str]] = {}
    ...
    def get_stage_module(self, stage_idx: int) -> smith.nn.Module:
        """
    ...
    def _number_and_count_forward_stages(gm: fx.GraphModule):
        ...
        # Update: the following assert may fail against some smith versions >=
        # 2.2.0, as:
    ...
    def _from_traced(
        ...
        split_policy: Callable[[smith.fx.GraphModule], smith.fx.GraphModule]
        | None = None,
    ):
        """
        ...
        def move_param_to_callee(
            ...
            if not isinstance(param_val, smith.Tensor):
                raise AssertionError(
                    f"Expected '{param_fqn}' to be {smith.Tensor} but got {type(param_val)}."
                    + (
                        f" It might happen if module '{param_fqn}' was passed to some 'leaf function'"
                        f"(see https://blacksmith.org/docs/stable/fx.html#fx.wrap). Please inspect "
                        f"usages of '{param_fqn}' in the traced graph."
                        if isinstance(param_val, smith.nn.Module)
                        else ""
                    )
                )
    ...
    def _trace_with_export(
        ...
        try:
            ep = smith.export.export(mod, example_args, example_kwargs)
            ...
            raise RuntimeError(
                ...
                "graph breaks, see https://blacksmith.org/docs/stable/export.html"
            ) from e
    ...
    def build_stage(
        ...
        # `smith.ones`, `smith.zeros`, `smith.rand`, etc.
        if isinstance(stage_module, smith.fx.GraphModule):
            _modify_graph_op_device(stage_module, device)
        else:
            logger.warning(
                f"Expected a `smith.fx.GraphModule` but got {type(stage_module)}"  # noqa: G004
            )
...
def annotate_split_points(mod: smith.nn.Module, spec: dict[str, SplitPoint]):
