...
from smith.distributed._composable.replicate_with_fsdp import replicate, ReplicateModule
from smith.distributed.fsdp import FSDPModule, fully_shard
from smith.fx.node import Argument, map_aggregate
...
def _normalize_model_output_as_tuple(output: Any) -> tuple[Any]:
    ...
    smith.export tracing, compiled models may also return a list instead of a Tuple, which we will normalize back to a
...
def _make_tensor_from_meta(
    example: smith.Tensor | FakeTensor,
    device: smith.device,
) -> smith.Tensor:
    """
...
class _PipelineStageBase(ABC):
    ...
    def __init__(
        ...
        """
        Args:
            submodule (smith.nn.Module): The module to be executed in this stage.
            stage_index (int): The index of this stage.
            num_stages (int): The total number of stages in this pipeline.
            device (smith.device): The device to run this stage on.
        ...
        # Run time states
        self._outputs_meta: tuple[smith.Tensor, ...] | None = None
        # map microbatch ID to list of forward tensor args
        self.fwd_cache: dict[int, tuple[Any, list[smith.Tensor]]] = {}
        # map microbatch ID to list of backward grad tensor args
        self.bwd_cache: dict[int, tuple[smith.Tensor | None, ...]] = {}
    ...
    def _configure_outputs_meta(self, outputs_meta: tuple[smith.Tensor, ...]):
        """
    ...
    def get_outputs_meta(self) -> tuple[smith.Tensor, ...]:
    ...
    def set_local_bwd_input(
        self, next_stage_bwd_outputs: tuple[smith.Tensor | None, ...], mb_index: int
    ) -> None:
        """
    ...
    def get_bwd_send_ops(self, bwd_chunk_id: int) -> list[dist.P2POp]:
        ...
        for grad, grad_recv_stage in zip(grads_input, self.grad_send_info):
            if isinstance(grad, smith.Tensor) and grad_recv_stage is not None:
                logger.debug(
    ...
    def clear_runtime_states(self) -> None:
        ...
        # `smith.autograd.backward()` will accumulate gradients into leaf
        ...
        for recv_tuple in self.args_recv_info.values():  # iterate over all chunks
            for a in recv_tuple:  # iterate over all input args
                if isinstance(a, _RecvInfo):
                    ...
                    # See https://github.com/blacksmith/blacksmith/pull/92731
                    a.buffer.grad = None
    ...
    ) -> tuple[tuple[smith.Tensor | None, ...], list[dict[str, Any]] | None]:
        """
        ...
        if isinstance(self.submod, DistributedDataParallel):
            if last_backward:
                ...
                self.submod.reducer.prepare_for_backward(  # type: ignore[union-attr, operator]
                    list(
                        smith.nn.parallel.distributed._find_tensors(  # type: ignore[attr-defined]
                            bwd_kwargs["stage_output"]
                        )
                    )
                )
    ...
    def backward_one_chunk(
        ...
        if self.is_last:
            ...
            # `smith.autograd.backward` will accumulate the gradient into the
        ...
        grads_input: tuple[smith.Tensor | None, ...] = ()
        ...
        if self.dw_builder:
            ...
            if full_backward:
                ...
                if not self.is_first:
                    if isinstance(bwd_kwargs["stage_output"], smith.Tensor):
    ...
    def _validate_fwd_outputs(self, outputs: tuple[smith.Tensor, ...]):
    ...
    def _get_init_p2p_neighbors_ops(self) -> list[dist.P2POp]:
        ...
        recv_tensor = smith.zeros(1, device=self.device, dtype=smith.float32)
        send_tensor = smith.tensor(
            self.stage_index, device=self.device, dtype=smith.float32
        )
        # forward
        if not self.is_first:
            ops.append(
...
class _PipelineStage(_PipelineStageBase):
    def __init__(
        ...
        Args:
            stage_module (smith.nn.Module): the module to be wrapped by this stage
            ...
            device (smith.device): the device to be used by this stage
    ...
    def _create_act_send_info(self):
        ...
        output_vals: tuple[smith.Tensor] = tuple(
...
class PipelineStage(_PipelineStageBase):
    ...
    Args:
        submodule (nn.Module): The Blacksmith module wrapped by this stage.
        stage_index (int): The ID of this stage.
        num_stages (int): The total number of stages.
        device (smith.device): The device where this stage is located.
        input_args (Union[smith.Tensor, Tuple[smith.tensor]], optional): The input arguments for the submodule.
        output_args (Union[smith.Tensor, Tuple[smith.tensor]], optional): The output arguments for the submodule.
    ...
    def __init__(
        ...
        num_stages: int,
        device: smith.device,
        input_args: smith.Tensor | tuple[smith.Tensor, ...] | None = None,
        output_args: smith.Tensor | tuple[smith.Tensor, ...] | None = None,
        group: dist.ProcessGroup | None = None,
        dw_builder: Callable[[], Callable[..., None]] | None = None,
    ):
        super().__init__(submodule, stage_index, num_stages, device, group, dw_builder)
        self.inputs: list[smith.Tensor] | None = None
        self.inputs_meta: tuple[smith.Tensor, ...] | None = None
        ...
        if input_args is None:
            ...
            self.inputs_meta = (
                (input_args,) if isinstance(input_args, smith.Tensor) else input_args
            )
            if output_args is None:
                ...
                try:
                    ...
                    output_args = tree_map_only(
                        smith.Tensor, lambda x: x.to("meta"), output_args
                    )
                except Exception as e:
                    raise RuntimeError(
            ...
            self._configure_outputs_meta(
                (output_args,) if isinstance(output_args, smith.Tensor) else output_args
            )

        # these are the buffers used in backwards send/recv, they are allocated later
        self.outputs_grad: list[smith.Tensor] = []
    ...
    def _shape_inference(
        ...
        if (
            ...
            args = tree_map_only(smith.Tensor, lambda x: x.to("meta"), args)
        else:
            if len(args) != 0:
                raise AssertionError(
        ...
        args = tree_map_only(
            smith.Tensor, lambda x: smith.zeros_like(x, device=self.device), args
        )
        ...
        outputs_meta = tuple(
            tree_map_only(smith.Tensor, lambda x: x.to("meta"), outputs)
        )
        logger.debug(
