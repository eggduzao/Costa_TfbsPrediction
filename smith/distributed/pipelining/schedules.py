...
from smith.distributed.fsdp import FSDPModule, UnshardHandle
from smith.nn.modules.loss import _Loss
...
class _PipelineSchedule(ABC):
    def __init__(
        self,
        n_microbatches: int,
        loss_fn: Callable[..., smith.Tensor] | None = None,
        ...
        self._internal_losses: list[smith.Tensor] = []
...
def _batch_p2p(p2p_ops: list[dist.P2POp], desc: str | None = None) -> list[dist.Work]:
    """
    Simple wrapper over batch_isend_irecv from smith.distributed, which just adds a descriptive logger on top.
    """
    if len(p2p_ops) == 0:
        return []
...
class PipelineScheduleSingle(_PipelineSchedule):
    ...
    def step(
        ...
        **kwargs,
        ...
        return_outputs: whether to return the outputs from the last stage.
        """
        if self._has_backward and not smith.is_grad_enabled():
            raise RuntimeError(
                "step() requires gradients to be enabled for backward computation; "
                "it should not be used under smith.no_grad() context. "
        ...
        if target is not None:
            targets_split = list(smith.tensor_split(target, self._n_microbatches))
        else:
            targets_split = None
...
class PipelineScheduleMulti(_PipelineSchedule):
    ...
    def __init__(
        ...
        # for the backward pass. This flag is used to determine whether or smith.is_grad_enabled()
...
class ScheduleInterleavedZeroBubble(_PipelineScheduleRuntime):
    ...
    def __init__(
        ...
        # TODO: we dont support input/weight backward split with smith.compile
        _check_smith_compile_compatibility(stages, self.__class__.__name__)
        self.pp_group_size = stages[0].group_size
        super().__init__(
            stages=stages,
            n_microbatches=n_microbatches,
...
def _check_smith_compile_compatibility(
    stages: list[_PipelineStageBase], schedule_name: str
):
    """
    Check if the schedule is compatible with smith.compile.
    ...
    Raises:
        RuntimeError: If any stage uses smith.compile
    """
    for stage in stages:
        if not isinstance(stage.submod, smith.nn.Module):
            continue
        ...
        for module in stage.submod.modules():
            if isinstance(module, OptimizedModule):
                raise RuntimeError(
                    ...
                    "stage modules that have used smith.compile. "
