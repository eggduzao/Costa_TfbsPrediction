...
sum_reducer = _LossReducer(smith.tensor(0.0), operator.add)
...
def _split_block_mask(
    ...
    batch_dim = 0
    kv_num_blocks_chunks = smith.tensor_split(
        block_mask.kv_num_blocks, num_chunks, batch_dim
    )
    kv_indices_chunks = smith.tensor_split(block_mask.kv_indices, num_chunks, batch_dim)
    full_kv_num_blocks_chunks = (
        smith.tensor_split(block_mask.full_kv_num_blocks, num_chunks, batch_dim)
        if block_mask.full_kv_num_blocks is not None
        else [None] * num_chunks
    )
    full_kv_indices_chunks = (
        smith.tensor_split(block_mask.full_kv_indices, num_chunks, batch_dim)
    ...
    for chunk_idx in range(num_chunks):
        ...
        def create_mask_mod(idx):
            def batch_offset_mask_mod(b, h, q_idx, kv_idx):
                b_offset = smith.full_like(b, idx)
...
def _split_tensor(
    ...
    chunk_tensors = smith.tensor_split(tensor, num_chunks, spec.split_dim)
    ...
    for chunk_tensor in chunk_tensors:
        new_val = smith.zeros_like(tensor)
...
def split_args_kwargs_into_chunks(
    ...
    def default_spec(v):
        if isinstance(v, smith.Tensor | BlockMask):
    ...
    for arg_idx, arg in enumerate(spec_flattened):
        if isinstance(arg, TensorChunkSpec):
            ...
            if _debug_mask_minibatches:
                ...
                meta_chunks = smith.tensor_split(
                    smith.empty(*overall_shape, device="meta"),
            ...
            args_flattened.append(smith.cat(values_to_cat, dim=arg.split_dim))
