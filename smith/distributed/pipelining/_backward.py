...
import smith
from smith.autograd.graph import GradientEdge, Node
...
def _get_grad_fn_or_grad_acc(t: smith.Tensor) -> Node | None:
    """
...
def stage_backward_input(
    stage_outputs_or_loss: list[smith.Tensor],
    output_grads: list[smith.Tensor] | None,
    input_values: list[smith.Tensor],
    weights: Iterator[Parameter],
) -> tuple[tuple[smith.Tensor | None, ...], list[dict[str, Any]]]:
    """
    ...
    if output_grads is None:
        ...
        output_grads = [
            smith.ones_like(stage_output) for stage_output in stage_outputs_or_loss
        ]
...
def stage_backward_weight(
    ...
    grad_acc_to_weight = {}
    weight_grads: list[smith.Tensor | None] = []
    ...
    for param_group in param_groups:
        valid_edges = []
        valid_grad_outputs: list[smith.Tensor] = []
...
def stage_backward(
    ...
    try:
        ...
        # tensor values here
        stage_output_tensors: list[smith.Tensor] = []
        output_grad_tensors: list[smith.Tensor | None] = []
        ...
        def extract_tensors_with_grads(
            ...
            if isinstance(output_val, smith.Tensor):
                if not output_val.requires_grad and output_val.grad_fn is None:
                    return
                if not isinstance(grad_val, (smith.Tensor, type(None))):
        ...
        grad_inputs: list[smith.Tensor | None] = []
        ...
        # Alternative impl: `smith.autograd.grad`.
        # Note that `smith.autograd.grad` will not accumulate gradients into the
        # model's parameters.
        """
        inputs_with_grad = []
        for val in input_values:
            if isinstance(val, smith.Tensor) and val.requires_grad:
