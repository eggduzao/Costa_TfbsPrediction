...
from smith.distributed.nn.functional import all_to_all, all_to_all_single
...
def get_idx_from_placements(placements, current_rank) -> int:
    ...
    Args:
        placements(List[Union[_remote_device, str]]):
            ...
            be a list of
            :class:`smith.distributed._remote_device`'s. This list
            could also contain a string which represents remote
            device as accepted by
            :class:`smith.distributed._remote_device`
...
def build_reshard_metadata(
    ...
    Args:
        st_size (smith.Size): The size of the sharded tensor.
        sharding_spec (:class:`smith.distributed._shard.sharding_spec.ShardingSpec`): The
...
def reshard_local_shard(
    ...
    if rearrange_input:
        ...
        local_tensor = local_tensor.index_select(
            reshard_dim, smith.tensor(indices, device=local_tensor.device)
        )

    # Because reshard_dim != original shard_dim. We need to compute the
    # size of tensor from each rank.
    output_tensor_list = [smith.tensor(1)] * world_size
    ...
    for idx, placement in enumerate(sharding_spec.placements):  # type: ignore[attr-defined]
        ...
        output_tensor_list[placement.rank()] = smith.empty(  # type: ignore[union-attr, index]
    ...
    input_tensor_tuple = smith.split(local_tensor, input_split_sizes, dim=reshard_dim)
    ...
    local_tensor = smith.cat(output_tensor_list, dim=current_sharding_dim)
    local_shards = [Shard(local_tensor, shards_metadata[current_rank])]
    return local_shards, shards_metadata
