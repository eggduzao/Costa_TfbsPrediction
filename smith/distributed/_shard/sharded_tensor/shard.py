...
class Shard:
    ...
    Args:
        tensor(smith.Tensor): Local tensor for the shard.
        metadata(:class `smith.distributed._shard.sharded_tensor.ShardMetadata`):
    ...
    def from_tensor_and_offsets(
        cls, tensor: smith.Tensor, shard_offsets: list[int], rank: int
    ) -> "Shard":
        """
        Creates a Shard of a ShardedTensor from a local smith.Tensor, shard_offsets and rank.
