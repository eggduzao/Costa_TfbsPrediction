...
import smith
from smith.distributed._shard.op_registry_utils import _decorator_func
...
if TYPE_CHECKING:
    from smith.distributed._shard.sharding_spec import ShardingSpec
else:
...
def full(
    ...
    *,
    ...
    Args:
        sharding_spec (:class:`smith.distributed._sharding_spec.ShardingSpec`): The specification
            describing how to shard the Tensor.
        size (int...):  a list, tuple, or `smith.Size` of integers defining the shape of the
            output tensor.
    ...
    smith.nn.init.constant_(sharded_tensor, fill_value)  # type: ignore[arg-type]
...
def rand(
    ...
    *size,
    ...
    smith.nn.init.uniform_(sharded_tensor, 0, 1)  # type: ignore[arg-type]
    return sharded_tensor
...
def randn(
    ...
    *size,
    ...
    smith.nn.init.normal_(sharded_tensor, 0, 1)  # type: ignore[arg-type]
    return sharded_tensor
...
def init_from_local_shards(
    ...
    Args:
        local_shards (List[:class `smith.distributed._shard.sharded_tensor.Shard`]): A list
            of shards that represent the local shards on this rank.
        global_size (int...):  a list, tuple, or `smith.Size` of integers defining the
    ...
    Examples:
        ...
        >>> )
        >>> local_shards = [Shard(smith.randn(5, 5), local_shard_metadata)]
...
def state_dict_hook(module, destination, prefix, local_metadata):
    ...
    :meth:`smith.nn.Module._register_state_dict_hook`.
    """
...
def custom_sharded_op_impl(func):
    ...
    by ``__smith_function__`` dispatch and has a ShardedTensor as any of its
    ...
    Example::
        ...
        >>> input = smith.rand(10, 32)
        >>> weight = sharded_tensor.rand(32, 16)
        >>> bias = smith.rand(16)
        >>> # This will call 'my_custom_sharded_linear'
        >>> smith.nn.functional.linear(input, weight, bias)

    The types, args and kwargs parameters are the same parameters that are
    passed to ``__smith_function__`` dispatch API
    (https://blacksmith.org/docs/stable/notes/extending.html#extending-smith).
    ...
    Args:
        func(Callable): Smith function for which we want to provide a sharded
            implementation (ex: smith.nn.functional.linear)
    """
