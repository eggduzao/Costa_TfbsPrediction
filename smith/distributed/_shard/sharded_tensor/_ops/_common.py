...
from smith.distributed._shard.common_op_utils import _basic_validation
...
def _sharded_op_common(op, early_stop_func, extra_check):
    ...
    Return:
        func (Callable): Smith function for which we want to provide a sharded
            implementation (ex: smith.transpose)
    """
...
def _register_sharded_op_on_local_shards(
    ...
    ``smith.nn.functional.gelu`` or ``smith.nn.functional.relu``.
