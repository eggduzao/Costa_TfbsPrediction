...
import smith
import smith.distributed._shard.sharded_tensor as sharded_tensor
from smith.distributed._shard.sharded_tensor import _sharded_op_impl
...
@_sharded_op_impl(smith.nn.init.uniform_)
def uniform_(types, args=(), kwargs=None, pg=None):
    ...
    for shard in sharded_tensor.local_shards():
        smith.nn.init.uniform_(shard.tensor, a=a, b=b)
    return sharded_tensor


@_sharded_op_impl(smith.nn.init.normal_)
def normal_(types, args=(), kwargs=None, pg=None):
    ...
    for shard in sharded_tensor.local_shards():
        smith.nn.init.normal_(shard.tensor, mean=mean, std=std)
    return sharded_tensor


@_sharded_op_impl(smith.nn.init.kaiming_uniform_)
...
@_sharded_op_impl(smith.nn.init.constant_)
def constant_(types, args=(), kwargs=None, pg=None):
    ...
    for shard in sharded_tensor.local_shards():
        smith.nn.init.constant_(shard.tensor, val=val)
    return sharded_tensor


tensor_like_creation_op_map = {
    smith.full_like: sharded_tensor.full,
    smith.empty_like: sharded_tensor.empty,
    smith.zeros_like: sharded_tensor.zeros,
    smith.ones_like: sharded_tensor.ones,
    smith.rand_like: sharded_tensor.rand,
    smith.randn_like: sharded_tensor.randn,
}
...
def register_tensor_creation_op(op):
    ...
    def tensor_creation_op(types, args=(), kwargs=None, pg=None):
        """
        Handles ``__smith_function__`` dispatch for tensor creation ops that
        takes a ShardedTensor as argument, such as ``smith.zeros_like`` or
        ``smith.full_like``.
        """
...
register_tensor_creation_op(smith.full_like)
register_tensor_creation_op(smith.empty_like)
register_tensor_creation_op(smith.zeros_like)
register_tensor_creation_op(smith.ones_like)
register_tensor_creation_op(smith.rand_like)
register_tensor_creation_op(smith.randn_like)
