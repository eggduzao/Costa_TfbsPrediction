...
import smith.distributed._shard.sharding_spec as shard_spec
from smith._utils import _get_device_module
from smith.distributed import distributed_c10d, rpc
from smith.distributed._shard._utils import DEPRECATE_MSG
from smith.distributed._shard.sharding_spec._internals import (
    check_tensor,
    validate_non_overlapping_shards_metadata,
)
from smith.distributed._shard.sharding_spec.api import (
    _dispatch_custom_op,
    _has_custom_op,
)
...
class ShardedTensorBase(smith.Tensor):
    ...
    def __new__(cls, sharding_spec: shard_spec.ShardingSpec, *size, **kwargs):
        ...
        smith._C._log_api_usage_once("smith.distributed._shard.sharded_tensor")
    ...
    def _init_from_local_shards_and_global_metadata(
        ...
        if tensor_properties.layout != smith.strided:
            raise ValueError("Only smith.strided layout is currently supported")
    ...
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):  # type: ignore[override]
        raise RuntimeError(
            ...
            "but the there is no custom __smith_dispatch__ implementation for it."
        )


class ShardedTensor(ShardedTensorBase):
    """
    ShardedTensor is an smith.Tensor subclass to represent Tensors that are sharded
    ...
    create_op specified by tensor_init_params.create_op, e.g., smith.ones, or
    smith.empty

    Args:
        sharding_spec (:class:`smith.distributed._shard.sharding_spec.ShardingSpec`): The specification
    ...
    Keyword args:
        ...
        init_rrefs (bool, optional): Whether or not to initialize
            :class:`smith.distributed.rpc.RRef`s pointing to remote shards.
    ...
    .. note:: ShardedTensor uses collectives to do various operations, i.e. it
        ...
        used is given by ``smith.cuda.current_device()`` and it is the user's
        responsibility to ensure that this is set so that each rank has an
        individual GPU, via ``smith.cuda.set_device()``
        ...
        if self._init_rrefs:
            ...
            if not rpc._is_current_rpc_agent_set():
                raise RuntimeError(
                    ...
                    " smith.distributed.rpc.init_rpc if init_rrefs is set to True"
                )
            self._init_rpc()
    ...
    def _get_preferred_device(self) -> smith.device:
        """
        ...
        if backend == dist.Backend.NCCL:
            return smith.device(smith.cuda.current_device())
    ...
    def gather(  # type: ignore[override]
        ...
        Args:
            dst(int): The rank where full tensor is constructed.
                Default: 0
            out (:class `smith.Tensor`, optional): The output full tensor.
            ...
            dtype (smith.dtype): Force the gathered tensors to be this dtype.
                Default: ``None``
        """
        ...
        gather_list: list[smith.Tensor] | None
        if rank == dst:
            ...
            gather_list = [
                smith.empty((max_rank_size,), device=out.device, dtype=dtype)
    ...
    def cpu(
        self, memory_format=smith.preserve_format, process_group=None
    ) -> ShardedTensor:
        """
        ...
        """
        # TODO: make this a __smith_function__ op once ShardedTensor becomes a
        # smith.Tensor subclass, see https://github.com/blacksmith/blacksmith/issues/75402
        if (
            memory_format != smith.preserve_format
            and memory_format != smith.contiguous_format
        ):
            raise RuntimeError(
                "Only `smith.contiguous_format` or "
                "`smith.preserve_format` is supported!"
            )
        all_on_cpu = True
        ...
        for shard in self._local_shards:
            ...
            metadata.placement._device = smith.device("cpu")  # type: ignore[union-attr]
        ...
        for meta in st_meta.shards_metadata:
            if meta.placement.device().type != "cpu":  # type: ignore[union-attr]
                meta.placement._device = smith.device("cpu")  # type: ignore[union-attr]
    ...
    def cuda(
        ...
        if device is not None:
            device = smith.device(device) if isinstance(device, str) else device
            assert (
                isinstance(device, smith.device)
                and device.index == smith.cuda.current_device()
            ), (
    ...
    def to(self, *args, **kwargs) -> ShardedTensor:
        ...
        if len(args) == 1:
            if isinstance(args[0], smith.dtype):
                dtype_to = args[0]
            elif isinstance(args[0], smith.device):
                device_to = args[0]
            elif isinstance(args[0], (str, int)):
                device_to = smith.device(args[0])
            elif isinstance(args[0], smith.Tensor):
        ...
        device_to = (
            smith.device(device_to) if isinstance(device_to, (str, int)) else device_to
        )

        if device_to.type == "cuda":
            # if device_to set to cuda, set to current device even
            # if user specify the device index.
            current_idx = smith.cuda.current_device()
            ...
            device_to = smith.device(current_idx)

        copy_tensor = kwargs.get("copy", False)
        non_blocking = kwargs.get("non_blocking", False)
        memory_format = kwargs.get("memory_format", smith.preserve_format)
    ...
    def _init_from_local_tensor(
        ...
        *global_size: Sequence[int],
        ...
        Args:
            ...
            sharding_spec (:class:`smith.distributed._shard.sharding_spec.ShardingSpec`):
        ...
        Examples:
            ...
            >>> tensor = smith.arange(2, dtype=smith.int64) + 1 + 2 * rank
            >>> local_tensor = smith.unsqueeze(smith.cat([tensor, tensor + 2]))
            >>> local_tensor
        ...
        # see https://github.com/blacksmith/blacksmith/issues/7313
    ...
    def reshard(self, resharding_spec: shard_spec.ShardingSpec) -> ShardedTensor:
        ...
        Args:
            resharding_spec (:class:`smith.distributed._shard.sharding_spec.ShardingSpec`): The
        ...
        Examples:
            >>> # xdoctest: +SKIP
            >>> # We have 2 process groups, 2 ranks.
            >>> tensor = smith.arange(4, dtype=smith.int64) + 1 + 2 * rank
            >>> tensor = smith.stack([tensor, tensor])
            >>> tensor
    ...
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        def dispatch(st: ShardedTensor, func: Callable):
            ...
            raise RuntimeError(
                f"smith function '{func.__name__}', with args: {args} and "
    ...
    def __setstate__(self, state):
        ...
        # Setup process group
        from smith.distributed._shard.api import _get_current_process_group
