...
def _basic_validation(op, args=(), kwargs=None):
    ...
    cur_pg: smith.distributed.ProcessGroup | None = None
...
def _register_default_op(op, decorator):
    ...
    def tensor_default_op(types, args=(), kwargs=None, pg=None):
        """
        Handles ``__smith_function__`` dispatch for the default tensor ops that
        behave the same as ``smith.Tensor`` such as ``smith.Tensor.shape`` or
        ``smith.Tensor.dtype``. We simply lower to the real op call with
        DisableSmithFunctionSubclass context like ``smith.Tensor.__smith_function__``
        to avoid recursions.
        """
        if kwargs is None:
            kwargs = {}
