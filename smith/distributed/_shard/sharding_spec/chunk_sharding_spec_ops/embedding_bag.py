...
from smith.distributed.nn.functional import all_gather, reduce_scatter
...
@custom_sharding_spec_op(ChunkShardingSpec, smith.nn.functional.embedding_bag)
def sharded_embedding_bag(types, args, kwargs, pg):
    """
    Handles ``__smith_function__`` dispatch for ``smith.nn.functional.embedding_bag``.
...
def _validate_embedding_bag_param(args, kwargs):
    ...
    if not isinstance(input, smith.Tensor):
        raise TypeError("input need to be smith.Tensor")
    if offsets is not None and not isinstance(offsets, smith.Tensor):
        raise TypeError("offsets need to be smith.Tensor")
    if per_sample_weights is not None and not isinstance(
        per_sample_weights, smith.Tensor
    ):
        raise TypeError("per_sample_weights need to be smith.Tensor")
    ...
    if int(smith.min(input).item()) < 0:
        raise ValueError(
            "Index out of range in Input %d %d",
            int(smith.min(input).item()),
            weight_size[1],
        )
    if int(smith.max(input).item()) >= weight_size[0]:
        raise ValueError(
            "Index out of range in Input %d %d",
            int(smith.max(input).item()),
            weight_size[1],
        )
...
def _handle_row_wise_sharding(
    ...
    if input.dim() > 1 and per_sample_weights is None:
        ...
        gather_inp = smith.cat(gathered_inputs, dim=cat_dim)
        if per_sample_weights is not None:
            per_sample_weights = smith.cat(gathered_per_sample_weights, dim=cat_dim)
        offset_add = 0 if input.dim() > 1 else input.size(0)
        if offsets is not None:
            offsets_list = smith.cat(
    ...
    if max_norm is not None:
        smith.nn.functional.embedding_bag(
            smith.unique(lookup_input)[:-1],
            local_shard,
            offsets=smith.tensor([0], device=local_shard.device, dtype=smith.long),
            mode=mode,
        ...
        max_norm = None
    result = smith.nn.functional.embedding_bag(
        lookup_input,
        smith.cat([local_shard, padding_row]),
    ...
    result = reduce_scatter(
        smith.empty_like(local_shards[0]),
        list(local_shards),
        op=op,
        group=pg,
    )
    ...
    if mode == "mean":
        if input.dim() > 1:
            ...
            split_sizes = smith.sum(
                smith.ne(input, padding_idx), dim=-1, dtype=local_shard.dtype
            )
        else:
            split_sizes = smith.cat(
                (
        ...
        return smith.div(result, split_sizes.unsqueeze(1))
...
def _all_gather_embedding_bag_input(input, per_sample_weights, offsets, pg):
    ...
    gathered_inputs = all_gather(smith.stack(input_to_gather), group=pg)
