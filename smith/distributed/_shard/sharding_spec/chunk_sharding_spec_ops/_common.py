...
from smith.distributed._shard.sharded_tensor._ops._common import _sharded_op_common
...
from smith.distributed.nn.functional import (
    _all_gather_base,
    all_reduce,
    all_to_all_single,
)
...
def _register_sharded_op_on_local_tensor(
    ...
    """
    Handles ``__smith_function__`` dispatch for ops which are performed on
    the single local tensor of the sharded tensor such as op like
    ``smith.nn.functional.softmax`` or ``smith.Tensor.view``.
    ...
    def sharded_tensor_op_on_local_tensor(types, args=(), kwargs=None, pg=None):
        ...
        if len(st.local_shards()) != 1:
            raise TypeError(
                f"smith function '{op.__name__}', with args: {args} and "
...
def _handle_col_wise_sharding_base(
    ...
    for i, inp in enumerate(gathered_inputs):
        if op_func is smith.nn.functional.embedding_bag:
            result = op_func(
                inp,
                local_shard,
        ...
        elif op_func is smith.nn.functional.embedding:
            result = op_func(
                inp,
                local_shard,
                padding_idx=padding_idx,
        ...
        results.append(smith.transpose(result, 0, col_dim))
    ...
    return smith.transpose(output, 0, col_dim)
...
def _result_distribute_with_col_rearrange(results, input, world_size, weight, pg):
    ...
    combined_results = smith.cat(results)
    output = smith.empty(
    ...
    return output.index_select(0, smith.tensor(indices, device=output.device))
...
def _handle_max_norm_col_wise(
    ...
    unique_inp = smith.unique(smith.cat(gathered_inputs))
    local_shard_sum = smith.sum(
        smith.pow(smith.abs(local_shard), norm_type), dim=1, dtype=local_shard.dtype
    )
    ...
    local_shard_norm = smith.pow(local_shard_sum, 1.0 / norm_type)
    max_norm_tensor = smith.full(
        (local_shard.size(0),),
        float("inf"),
    ...
    local_shard_norm_renormed = (
        smith.div(smith.mul(local_shard_t, normalized_tensor), local_shard_norm)
        .t()
        .contiguous()
    )
...
def _all_gather_base_input(input, pg):
    ...
    gather_inp = smith.empty(gather_inp_size, device=input.device, dtype=input.dtype)
