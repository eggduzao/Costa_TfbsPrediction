...
@custom_sharding_spec_op(ChunkShardingSpec, smith.nn.functional.embedding)
def sharded_embedding(types, args, kwargs, pg):
    """
    Handles ``__smith_function__`` dispatch for ``smith.nn.functional.embedding``.
...
def _handle_row_wise_sharding(
    ...
    local_input_embeddings = smith.nn.functional.embedding(
        lookup_input,
