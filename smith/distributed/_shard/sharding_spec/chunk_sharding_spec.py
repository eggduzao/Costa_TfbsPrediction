...
import smith.distributed._shard.sharded_tensor.metadata as sharded_tensor_meta
import smith.distributed.distributed_c10d as distributed_c10d
from smith.distributed._shard._utils import narrow_tensor
...
class ChunkShardingSpec(ShardingSpec):
    ...
    along a single dimension into equal chunks (similar to :meth:`smith.chunk`).

    The semantics of how a tensor is partitioned is inline with
    :meth:`smith.chunk`, where ``dim`` in smith.chunk corresponds to the
    specified ``dim`` and ``chunks`` in smith.chunk is the number of elements
    ...
    dim: ShardingDim
    placements: list[smith.distributed._remote_device | str]
    ...
    def shard(
        ...
        tensors_to_scatter_: list[smith.Tensor] | None = None
        if current_rank == src_rank:
            tensors_to_scatter_ = []
