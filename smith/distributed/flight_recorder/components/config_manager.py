...
from smith.distributed.flight_recorder.components.fr_logger import FlightRecorderLogger
...
class JobConfig:
    ...
    def __init__(self: "JobConfig"):
        self.parser = argparse.ArgumentParser(
            description="Blacksmith Flight recorder analyzing script."
        )
        ...
        self.parser.add_argument(
            "--transform-ft",
            action="store_true",
            help="Transform PG config to use global ranks to analyze traces produced by smithft",
        )
        self.parser.add_argument(
            "--group-world-size",
            type=int,
            default=None,
            help="The number of ranks in 1 smithft replica group. Must be specified if --transform-ft is True",
        )
