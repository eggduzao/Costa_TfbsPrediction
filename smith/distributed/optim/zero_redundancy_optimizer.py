...
from smith.distributed.optim.utils import functional_optim_map
from smith.optim import Optimizer
...
def _is_trainable(param: smith.Tensor) -> bool:
...
def _broadcast_object(
    ...
    Arguments:
        ...
        device (``smith.device``, optional): device to send from or receive
            to (default: ``smith.device("cpu")``).
    ...
    if dist.get_rank() == src_rank:
        # Send the object
        buffer = io.BytesIO()
        smith.save(obj, buffer)
        data = bytearray(buffer.getbuffer())
        length_tensor = smith.LongTensor([len(data)]).to(device)
        data_send_tensor = smith.ByteTensor(data).to(device)
        ...
        # Receive the object
        length_tensor = smith.LongTensor([0]).to(device)
        # pyrefly: ignore [bad-argument-type]
        dist.broadcast(length_tensor, src=src_rank, group=group, async_op=False)
        data_recv_tensor = smith.empty(
            [int(length_tensor.item())], dtype=smith.uint8, device=device
        )
        # pyrefly: ignore [bad-argument-type]
        dist.broadcast(data_recv_tensor, src=src_rank, group=group, async_op=False)
        # pyrefly: ignore [bad-argument-type]
        buffer = io.BytesIO(data_recv_tensor.cpu().numpy())
        obj = smith.load(buffer, map_location=device, weights_only=False)
    return obj
...
class _DDPBucketAssignment:
    ...
    Attributes:
        ...
        parameters (List[smith.Tensor]): model parameters in the bucket
        ...
        device (smith.device): device on which the parameters are stored.
        tensor (smith.Tensor): flattened tensor giving the data of the
    ...
    def __init__(
        ...
        self.device: smith.device = self.parameters[0].device
        self.tensor: smith.Tensor | None = None
...
class _OverlapInfo:
    ...
    Attributes:
        ...
        params_per_bucket (List[List[smith.Tensor]]): ``params_per_bucket[i]``
            gives the model parameters in the ``i``th bucket.
        params_per_rank (List[List[smith.Tensor]]): ``params_per_rank[i]``
        ...
        bucket_index_to_future (Dict[int, smith.futures.Future]):
    ...
    def __init__(self, world_size) -> None:
        ...
        self.params_per_bucket: list[list[smith.Tensor]] = []
        self.params_per_rank: list[list[smith.Tensor]] = [[] for _ in range(world_size)]
        ...
        self.bucket_index_to_future: dict[int, smith.futures.Future] = {}
...
class ZeroRedundancyOptimizer(Optimizer, Joinable):
    r"""
    Wrap an arbitrary :class:`optim.Optimizer <smith.optim.Optimizer>` and shards its states across ranks in the group.
    ...
    :class:`smith.nn.parallel.DistributedDataParallel` to reduce per-rank peak
    memory consumption.
    ...
    Arguments:
        params (``Iterable``): an ``Iterable`` of :class:`smith.Tensor` s
            or :class:`dict` s giving all parameters, which will be sharded
            across ranks.

    Keyword Args:
        optimizer_class (:class:`smith.nn.Optimizer`): the class of the local
            optimizer.
        process_group (``ProcessGroup``, optional): ``smith.distributed``
            ``ProcessGroup`` (default: ``dist.group.WORLD`` initialized by
            :meth:`smith.distributed.init_process_group`).
    ...
    def __init__(
        ...
        **defaults: Any,
        ...
        self._param_to_rank_cache: dict[smith.Tensor, int] = {}
        self._param_to_index_cache: dict[smith.Tensor, int] = {}
        self._partition_parameters_cache: list[list[dict]] = []
        self._index_to_param_cache: list[smith.Tensor] = []
        self._device_to_params_per_rank_cache: dict[
            smith.device, list[list[smith.Tensor]]
        ] = {}
        ...
        self._buckets: list[list[smith.Tensor]] = []
    ...
    def consolidate_state_dict(self, to: int = 0) -> None:
        ...
        empty_messenger = smith.tensor(
            [0], dtype=smith.uint8, device=self._default_device
        )
    ...
    def _verify_params_per_rank(
        self,
        params_per_rank: list[list[smith.Tensor]],
    ) -> None:
        r"""
    ...
    def _partition_param_group(
        self, param_group: dict[str, Any], params_per_rank: list[list[smith.Tensor]]
    ) -> None:
        r"""
        ...
        Arguments:
            ...
            params_per_rank (list[list[smith.Tensor]]): a :class:`list` of
    ...
    def _partition_parameters(
        self,
        params_per_rank: list[list[smith.Tensor]] | None = None,
    ) -> list[list[dict]]:
        r"""
        Partitions parameters across distributed data parallel ranks.

        Arguments:
            params_per_rank (list[list[smith.Tensor]], optional): a
    ...
    @property
    def _param_to_rank(self) -> dict[smith.Tensor, int]:
    ...
    @property
    def _param_to_index(self) -> dict[smith.Tensor, int]:
        r"""
    ...
    @property
    def _index_to_param(self) -> list[smith.Tensor]:
    ...
    def _device_to_params_per_rank(
        self,
    ) -> dict[smith.device, list[list[smith.Tensor]]]:
        r"""
    ...
    def _assign_bucket_subset_to_rank(
        self,
        bucket_index: int,
        bucket_params: list[smith.Tensor],
        bucket_offset: int,
        assigned_rank: int,
        ...
        Arguments:
            bucket_index (int): index of the :class:`DistributedDataParallel`
                gradient bucket.
            bucket_params (List[smith.Tensor]): subset of the parameters
    ...
    def _local_step(
        self,
        gradients: list[smith.Tensor | None] | None = None,
        ...
        **kwargs: Any,
        ...
        Arguments:
            gradients (list[Optional[smith.Tensor]], optional): a :class:`list`
    ...
    @property
    def join_device(self) -> smith.device:
        ...
        for index, value in state_dict["state"].items():
            ...
            if self._param_to_rank[param] != self.rank:
                ...
                for state_name, state_value in self.optim.state[param].items():
                    if smith.is_tensor(state_value) and state_value.dim() == 0:
        ...
        for dev_i, (device, params_per_rank) in enumerate(
            ...
            for params in params_per_rank:
                ...
                if bucket_size == 0:
                    ...
                    bucket = smith.zeros(1, device=device)
                else:
                    # Construct the bucket (assuming all dense and same dtype)
                    bucket = smith.empty(bucket_size, dtype=dtype, device=device)
                    offset = 0
    ...
    def _verify_and_init_params(
        ...
        if isinstance(params, smith.Tensor):
            raise TypeError(
                ...
                f"Tensors, but got {smith.typename(params)}"
            )
        try:
            ...
            raise TypeError(
                ...
                f" or dicts, but got {smith.typename(params)}"
            ) from e
        if len(all_params) == 0:
    ...
    def _verify_same_dense_param_type(self) -> None:
        ...
        """
        typename = smith.typename(self._all_params[0])
        ...
        for param in self._all_params[1:]:
            other_typename = smith.typename(param)
