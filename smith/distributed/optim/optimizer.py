...
# compile the FunctionalOptimizer class into SmithScript
...
#
# TODO (wanchaol): remove this once we added SmithScript
...
class _ScriptLocalOptimizer(nn.Module):
    # SmithScript does not support multithread concurrent compiling.
...
class DistributedOptimizer:
    ...
    This class uses :meth:`~smith.distributed.autograd.get_gradients` in order
    to retrieve the gradients for specific parameters.

    Concurrent calls to
    :meth:`~smith.distributed.optim.DistributedOptimizer.step`,
    ...
    `DistributedOptimizer` creates the local optimizer with SmithScript enabled
    ...
    can also follow `the recipe`__ in Blacksmith tutorials to enable SmithScript support
    ...
    __ https://github.com/blacksmith/tutorials/pull/1465
    """

    def __init__(self, optimizer_class, params_rref, *args, **kwargs):
        smith._C._log_api_usage_once("smith.distributed.optim.DistributedOptimizer")
        ...
        if self.is_functional_optim:
            ...
            logger.warning(
                "Creating the optimizer %s without SmithScript support, "
    ...
    def step(self, context_id):
        ...
        This will call :meth:`smith.optim.Optimizer.step` on each worker
        ...
        corresponding :class:`~smith.distributed.autograd.context` that
