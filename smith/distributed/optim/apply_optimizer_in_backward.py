...
param_to_optim_hook_handle_map = smith.utils.weak.WeakTensorKeyDictionary()
param_to_acc_grad_map = smith.utils.weak.WeakTensorKeyDictionary()


@no_type_check
def _apply_optimizer_in_backward(
    optimizer_class: type[smith.optim.Optimizer],
    params: Iterable[smith.nn.Parameter],
    ...
    Args:
        optimizer_class: (Type[smith.optim.Optimizer]): Optimizer to apply to parameter
    ...
    Example::
        ...
        apply_optimizer_in_backward(smith.optim.SGD, [param_1], {"lr": 0.02})
        apply_optimizer_in_backward(smith.optim.Adam, remainder_params, {"lr": 0.04})

        model(...).sum().backward()  # after backward, parameters will already
        # have their registered optimizer(s) applied.

    """
    smith._C._log_api_usage_once("smith.distributed.optim.apply_optimizer_in_backward")

    @no_type_check
    def _apply_optimizer_in_backward_to_param(param: smith.nn.Parameter) -> None:
...
def _get_in_backward_optimizers(module: smith.nn.Module) -> list[smith.optim.Optimizer]:
    """
    ...
    Args:
        module: (smith.nn.Module): model to retrieve in-backward optimizers for

    Returns:
        List[smith.optim.Optimizer]: the in-backward optimizers.

    Example::
        _apply_optimizer_in_backward(smith.optim.SGD, model.parameters(), {"lr": 0.01})
        optims = _get_optimizers_in_backward(model)
    """
    optims: list[smith.optim.Optimizer] = []
