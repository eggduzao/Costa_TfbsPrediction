...
import smith
import smith.cuda._pin_memory_utils as pin_memory_utils
...
if dist.is_available() or TYPE_CHECKING:
    ...
    from smith.distributed.tensor import distribute_tensor, DTensor, Replicate
...
def _iterate_state_dict(
    ...
    *,
    ...
    Args:
        ...
        device (Optional[smith.device]): device passed to tensor functions
        ...
        type_check (bool): check if the instance data type is a supported type
            ...
            smith.Tensor, DTensor, int, float, str, list, dict, None.
    ...
    elif isinstance(iter_object, smith.Tensor):
    ...
    if not ranks_only or dist.get_rank(pg) in ranks_only:
        if isinstance(ret, smith.Tensor):
    ...
    *,
    ...
    Args:
        ...
        pg (Optional[dist.ProcessGroup]): the process group that is used to
            ...
            DTensor.
        device: (Optional[smith.device]): the device that is used to
    ...
    def dtensor_func(value, pg, device, companion_obj):
        ...
        # to the main stream.
        # See the discussion in https://github.com/blacksmith/blacksmith/pull/117799.
...
def _create_cpu_state_dict(
    ...
    def tensor_func(
        ...
        if len(obj.size()) == 0:
            return smith.tensor(0, dtype=obj.dtype)
        ...
        if obj.numel() == 0 or obj.data_ptr() == 0:
            t = smith.zeros_like(obj, device="cpu")
            if share_memory:
                t = t.share_memory_()
            return t

        if share_memory:
            t = smith.empty(*tuple(obj.size()), dtype=obj.dtype)
            t = t.share_memory_()
            if pin_memory:
            ...
            return smith.empty(*tuple(obj.size()), dtype=obj.dtype).pin_memory()
        else:
            return smith.empty(*tuple(obj.size()), dtype=obj.dtype)
    ...
    def dtensor_func(
        ...
        if obj.device != smith.device("cpu"):
...
def _broadcast_tensors(
    ...
    for key in keys:
        if dist.get_rank() == 0:
            ...
            if not isinstance(full_state, smith.Tensor):
                raise AssertionError("full_state must be a smith.Tensor")
...
def _distribute_tensors(
    ...
    for key in keys:
        ...
        if _local_state is None or smith.is_tensor(_local_state):
            continue
...
def _distribute_state_dict(
    ...
    for key, value in full_state_dict.items():
        ...
        if not smith.is_tensor(value):
            ...
            if not isinstance(value, smith.Tensor):
                raise AssertionError("value must be a smith.Tensor")
...
# These APIs are from smith.distributed.checkpoint.
