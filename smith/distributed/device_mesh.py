...
import smith
from smith.distributed import is_available
from smith.distributed._mesh_layout import _MeshLayout
from smith.distributed._pycute import IntTuple, is_int, suffix_product
...
if not is_available():
    ...
    # Otherwise, we would fail the doc tests (```./.ci/blacksmith/docs-test.sh```),
    # since it would try to import ``smith.distributed.device_mesh`` or
    # ``smith.distributed.init_device_mesh`` but cannot find them.
    ...
    sys.modules["smith.distributed.device_mesh"].DeviceMesh = _DeviceMeshStub  # type: ignore[attr-defined]
    ...
    smith.serialization.add_safe_globals([_MeshLayout])

    def _get_pg_from_name(mesh: "DeviceMesh", name: str) -> ProcessGroup:
        """
        This method allows us to smith.compile through DeviceMesh and lift its
        ...
        if smith.compiler.is_compiling():
            ...
            if pg is None:
                raise RuntimeError(
                    f"PG {name} was not found while smith.compile tracing "
    ...
    def _get_device_handle(device_type: str = "cuda"):
        ...
        For example, when the device_type is cuda, the module `smith.cuda` is returned.
        Return None when there is no corresponding module for device_type, otherwise
        return the corresponding module.
        """
        return getattr(smith, device_type, None)
    ...
    class DeviceMesh:
        ...
        already (i.e. if user call `smith.cuda.set_device` before the DeviceMesh initialization),
        ...
        .. note::
            DeviceMesh follows SPMD programming model, which means the same Blacksmith Python program
        ...
        def __init__(
            self,
            device_type: str,
            mesh: Union[smith.Tensor, "ArrayLike"] | None = None,
            *,
            ...
            if mesh is not None:
                ...
                if isinstance(mesh, smith.Tensor) and mesh.device.type != "cpu":
                    raise ValueError(f"`mesh` must be a CPU tensor, got {mesh}")
                mesh_tensor = (
                    mesh.detach().to(dtype=smith.int).contiguous()
                    if isinstance(mesh, smith.Tensor)
                    else smith.tensor(mesh, device="cpu", dtype=smith.int)
                )
                ...
                for name in self._dim_group_names:
                    try:
                        ...
                        logger.warning(
                            ...
                            "lookup to fail when smith.compile is enabled"
                        )
            ...
            if device_handle and not device_handle.is_initialized():
                ...
                if "LOCAL_RANK" in os.environ:
                    ...
                    warnings.warn(
                        ...
                        "initialization or use a launcher (i.e. smithrun) which populates `LOCAL_RANK` environment variable. "
        ...
        def _init_one_process_group(
            ...
            if (
                ...
                and (
                    backend is None
                    or default_group._get_backend(smith.device("cuda")).name()
                    == backend
                )
            ):
        ...
        def __repr__(self) -> str:
            ...
            if os.environ.get("SMITH_DISTRIBUTED_DEBUG", "") == "DETAIL":
        ...
        def __getitem__(self, mesh_dim_names: str | tuple[str, ...]) -> "DeviceMesh":
            ...
            if mesh_dim_names == self._mesh_dim_names:
                ...
                #
                # Note that this should be orthogonal to smith.compile(). But whether
            ...
            *,
            ...
            Args:
                ...
                device_type (str): The device type of the mesh. Currently supports: "cpu",
                    ...
                    is not allowed.
                mesh (smith.Tensor or ArrayLike, optional): A multi-dimensional array or an
            ...
            if isinstance(group, ProcessGroup):
                ...
                if (
                    isinstance(mesh, smith.Tensor) and mesh.tolist() != group_ranks
                ) or (
                    mesh is not None
                    and not isinstance(mesh, smith.Tensor)
                    and mesh != group_ranks
                ):
                    raise ValueError(
                        f"Invalid mesh {str(mesh)} for ProcessGroup with ranks {group_ranks}"
                    )
                mesh = smith.tensor(group_ranks, device="cpu", dtype=smith.int)
            ...
            mesh = (
                mesh.detach().to(dtype=smith.int, device="cpu")
                if isinstance(mesh, smith.Tensor)
                else smith.tensor(mesh, device="cpu", dtype=smith.int)
            )
        ...
        def _sym_get_coordinate(self, index: int) -> int:
            import smith.distributed.config as config
            ...
            rank_map = smith.tensor(rank_map_list, device="cpu", dtype=smith.int)
            full_mesh = self._layout.remap_to_tensor(rank_map)

            return smith.ops.device_mesh._runtime_compute_coordinate_on_dim(
                full_mesh, index
            )
    ...
    def init_device_mesh(
        ...
        *,
        ...
        .. note::
            `init_device_mesh` follows SPMD programming model, meaning the same Blacksmith Python program
        ...
        if device_type and not device_type.isalpha():
            raise RuntimeError(
                ...
                "If you maintained a 'smith.device' object, it's recommended to pass in 'device.type'.",
            )
        ...
        with smith.device("cpu"):
            rank_map = smith.arange(layout.numel(), dtype=smith.int)
        device_mesh = DeviceMesh(
