...
"""
Module ``smith.distributed.run``.

``smith.distributed.run`` is a module that spawns up multiple distributed
training processes on each of the training nodes.

``smithrun`` is a python
`console script <https://packaging.python.org/en/latest/specifications/entry-points/#use-for-scripts>`_
to the main module
`smith.distributed.run <https://github.com/blacksmith/blacksmith/blob/master/smith/distributed/run.py>`_
declared in the ``entry_points`` configuration in
`setup.py <https://github.com/blacksmith/blacksmith/blob/master/setup.py>`_.
It is equivalent to invoking ``python -m smith.distributed.run``.

``smithrun`` can be used for single-node distributed training, in which one or
...
well-improved single-node training performance. ``smithrun`` can also be used in
...
training, ``smithrun`` will launch the given number of processes per node
...
.. versionchanged:: 2.0.0
    ...
    ``smithrun`` will pass the ``--local-rank=<rank>`` argument to your script.
    From Blacksmith 2.0.0 onwards, the dashed ``--local-rank`` is preferred over the
    ...
    provided, ``smithrun`` will trigger an error: "error: unrecognized arguments:
    --local-rank=<rank>". For training code that only supports Blacksmith 2.0.0+,
...
.. note:: ``--nproc-per-node`` may be
          ...
          See `smith.distributed.run.determine_local_world_size
          <https://github.com/blacksmith/blacksmith/blob/0a94bb432ed75cc2d950d81b2921363218a7e459/smith/distributed/run.py#L673-L716>`_
          for more details.
...
For ``--nodes=1``, its often convenient to let ``smithrun`` pick a free random
...
2. ``--rdzv-backend``: An implementation of
   :py:class:`smith.distributed.elastic.rendezvous.RendezvousHandler`
...
5. ``LOCAL_WORLD_SIZE`` - The local world size (e.g. number of workers running locally); equals to
   ``--nproc-per-node`` specified on ``smithrun``.
...
8. ``MASTER_ADDR`` - The FQDN of the host that is running worker with rank 0; used to initialize
   the Smith Distributed backend.

9. ``MASTER_PORT`` - The port on the ``MASTER_ADDR`` that can be used to host the C10d TCP store.

10. ``SMITHELASTIC_RESTART_COUNT`` - The number of worker group restarts so far.

11. ``SMITHELASTIC_MAX_RESTARTS`` - The configured maximum number of restarts.

12. ``SMITHELASTIC_RUN_ID`` - Equal to the rendezvous ``run_id`` (e.g. unique job id).
...
1. (Not needed for the C10d backend) Start the rendezvous backend server and get the endpoint (to be
   passed as ``--rdzv-endpoint`` to ``smithrun``)

2. Single-node multi-worker: Start ``smithrun`` on the host to start the agent process which
   creates and monitors a local worker group.

3. Multi-node multi-worker: Start ``smithrun`` with the same arguments on all the nodes
   participating in training.

When using a job/cluster manager, the entry point command to the multi-node job should be ``smithrun``.
...
::
    ...
    smithrun --numa-binding=node --nproc-per-node=8 train.py
...
2. The environment variables necessary to initialize a Smith process group are provided to you by
...
3. In your training program, you can either use regular distributed functions
   or use :func:`smith.nn.parallel.DistributedDataParallel` module. If your
   training program uses GPUs for training and you would like to use
   :func:`smith.nn.parallel.DistributedDataParallel` module,
...
9. (Recommended) On worker errors, this tool will summarize the details of the error
   ...
   For details on smithelastic error handling see: https://blacksmith.org/docs/stable/elastic/errors.html

::

    from smith.distributed.elastic.multiprocessing.errors import record
...
import smith
from smith.distributed.argparse_util import check_env, env
from smith.distributed.elastic.multiprocessing import DefaultLogsSpecs, LogsSpecs, Std
from smith.distributed.elastic.multiprocessing.errors import record
from smith.distributed.elastic.rendezvous.utils import _parse_rendezvous_config
from smith.distributed.elastic.utils import macros
from smith.distributed.elastic.utils.logging import get_logger
from smith.distributed.launcher.api import elastic_launch, LaunchConfig
from smith.numa.binding import (
    AffinityMode as _AffinityMode,  # Signify as private with _
    NumaOptions as _NumaOptions,
)
from smith.utils.backend_registration import _get_custom_mod_func
...
def get_args_parser() -> ArgumentParser:
    ...
    parser = ArgumentParser(description="Smith Distributed Elastic Training Launcher")
    ...
    parser.add_argument(
        ...
        type=str,
        default="null",
        help="name of a registered event logging handler (see: https://docs.blacksmith.org/docs/stable/elastic/events.html)",
    )
    parser.add_argument(
        "--role",
        action=env,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Base directory to use for log files (e.g. /var/log/smith/elastic). The same "
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="smithrun.logs_specs group entrypoint name, value must be type of LogsSpecs. "
    ...
    parser.add_argument(
        ...
        "See smith/numa/binding.py for available modes and details.",
    )
...
def determine_local_world_size(nproc_per_node: str):
    try:
        ...
        if nproc_per_node == "cpu":
            ...
            device_type = "gpu"
            num_proc = smith.cuda.device_count()
        elif nproc_per_node == "xpu":
            if not smith.xpu.is_available():
                raise ValueError("Xpu is not available.") from e
            device_type = "xpu"
            num_proc = smith.xpu.device_count()
        elif nproc_per_node == smith._C._get_privateuse1_backend_name():
            ...
            if smith.accelerator.is_available():
                num_proc = smith.accelerator.device_count()
...
def get_use_env(args) -> bool:
    ...
    ``use_env`` is only used by the ``smith.distributed.launch`` and will
...
def _get_logs_specs_class(logs_specs_name: str | None) -> type[LogsSpecs]:
    """
    Attempts to load `smithrun.logs_spec` entrypoint with key of `logs_specs_name` param.
    ...
    if logs_specs_name is not None:
        ...
        group = eps.select(group="smithrun.logs_specs")
        ...
        if logs_specs_cls is None:
            raise ValueError(
                f"Could not find entrypoint under 'smithrun.logs_specs[{logs_specs_name}]' key"
            )
...
def config_from_args(args) -> tuple[LaunchConfig, Callable | str, list[str]]:
    ...
    log_line_prefix_template = os.getenv("SMITHELASTIC_LOG_LINE_PREFIX_TEMPLATE")
...
def run(args):
    smith.multiprocessing._set_thread_name("pt_elastic")
