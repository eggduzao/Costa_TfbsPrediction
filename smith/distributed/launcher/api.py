...
from smith._utils_internal import get_default_numa_options
from smith.distributed.elastic import events, metrics
from smith.distributed.elastic.agent.server.api import WorkerSpec
from smith.distributed.elastic.agent.server.local_elastic_agent import LocalElasticAgent
...
from smith.distributed.elastic.rendezvous.utils import parse_rendezvous_endpoint
...
class LaunchConfig:
    ...
    def __post_init__(self):
        ...
        # Post-processing to enable refactoring to introduce logs_specs due to non-smithrun API usage
        ...
        if (
            ...
            and smith.cuda.device_count() == self.nproc_per_node
        ):
...
class elastic_launch:
    """
    Launches an smithelastic agent on the container that invoked the entrypoint.
...
def launch_agent(
    ...
    os.environ["SMITHELASTIC_SIGNALS_TO_HANDLE"] = config.signals_to_handle
