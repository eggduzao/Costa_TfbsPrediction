...
from smith.distributed._composable_state import _State
...
class _ReplicateState(_State):
    ...
    def init(
        ...
        **kwargs,
        ...
        from smith.distributed.tensor.parallel.ddp import _localize_dtensor
        ...
        if "device_id" in kwargs:
            ...
            # user can pass in device_id as a Union[int, smith.device] even for
            ...
            if kwargs["device_id"] is not None:
                ...
                if isinstance(device_id, smith.device) and device_id.type == "cpu":
...
def replicate(
    ...
    **kwargs,
    ...
    Args:
        module (smith.nn.Module): module to replicate

    Example::
        >>> # xdoctest: +REQUIRES(module:smith._C._distributed_c10d)
        >>> module = nn.Linear(3, 3)
        >>> replicate(module)
    """
    smith._C._log_api_usage_once("smith.distributed.replicate")

    # TODO(fegin): using kwargs is not a good idea if we would like to make
    # replicate a formal API to replace DDP.
    if "device_id" in kwargs:
        if not isinstance(kwargs["device_id"], (int, smith.device)):
            raise RuntimeError(
                "Expected device_id to be int or smith.device, "
