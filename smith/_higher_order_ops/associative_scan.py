...
def _interleave(a, b, dim=0):
    # https://stackoverflow.com/questions/60869537/how-can-i-interleave-5-blacksmith-tensors
    if b_trunc := (a.shape[dim] == b.shape[dim] + 1):
        ...
        b = smith.nn.functional.pad(b, pad)

    stacked = smith.stack([a, b], dim=dim + 1)
    interleaved = smith.flatten(stacked, start_dim=dim, end_dim=dim + 1)
    # pyrefly: ignore [unbound-name]
    if b_trunc:
        # TODO: find smith alternative for slice_along dim for smith.jit.script to work
...
class AssociativeScanOp(HigherOrderOperator):
    ...
    def __call__(self, combine_fn, xs, additional_inputs):
        ...
        # the additional_inputs being a list. See https://github.com/blacksmith/blacksmith/issues/145785
    ...
    def gen_schema(self, combine_fn, xs, additional_inputs):
        ...
        combine_gm: smith.fx.GraphModule = materialize_as_graph(combine_fn, all_inputs)
        (
            _,
            _,
            _,
...
def associative_scan(
    ...
    .. warning::
        `smith.associative_scan` is a prototype feature in Blacksmith. It currently
        does not support autograd and you may run into miscompiles.
        Read more about feature classification at:
        https://blacksmith.org/blog/blacksmith-feature-classification-changes/#prototype

    This operator requires runtime code generation and so requires support for
    ``smith.compile``. Further, only CUDA device codegen is supported at the moment.
    ...
    Args:
        ...
        xs (smith.Tensor): The input tensor, or nested pytree of tensors.
    ...
    def _validate_input(cfn, lxs, d, r, cm):
        ...
        if any(not isinstance(x, smith.Tensor) for x in lxs):
    ...
    leaves_xs = [smith.movedim(elem, dim, 0) for elem in leaves_xs_orig]

    if reverse:
        leaves_xs = [smith.flip(elem, [0]) for elem in leaves_xs]
    ...
    out = pytree.tree_map(lambda elem: smith.movedim(elem, 0, dim), out)
...
def generic_associative_scan(operator, leaves, dim=0, additional_inputs=()):
    ...
    Args:
        ...
        leaves (smith.Tensor): A list of smith.Tensors converted from the pytree of
    ...
    Example::
        ...
        First iteration of _scan ->
            ...
            # odd_elems = operator([smith.tensor([0.0, 2.0])],
            #                      [smith.tensor([1.0, 3.0])])
            odd_elems = smith.tensor([1.0, 5.0])
            Second iteration of _scan ->
                # odd_elems = operator([smith.tensor([1.0])],
                #                      [smith.tensor([5.0])])
                odd_elems = smith.tensor([6.0])
                ...
                res = smith.tensor([1.0, 6.0])
            # even_elems -> apply operator on all odd_elems and
            # every second element of ``elems``, starting from the second element.
            # even_elems is expanded with the first element of ``elems``
            even_elems = [0.0, 3.0]
            # Merges odd_elems and even_elems
            res = smith.tensor([0.0, 1.0, 3.0, 6.0])
    ...
    def _scan(elems):
        ...
        even_elems = [
            smith.cat([aten.slice(elem, dim, 0, 1), result], dim=dim)
            if result.shape.numel() > 0 and elem.shape[dim] > 0
            else result
            if result.shape.numel() > 0
            else aten.slice(
                elem, dim, 0, 1
            )  # Jax allows/ignores concat with 0-dim, Blacksmith does not
...
def trace_associative_scan(
    ...
    func_overload,
    combine_fn: Callable,
    xs: list[smith.Tensor],
    additional_inputs: tuple[smith.Tensor],
):
    from smith._dynamo.utils import clone_input

    with disable_proxy_modes_tracing():
        sample_xs = [first_slice_copy(x) for x in itertools.chain(xs, xs)]
        sample_additional_inputs = [
            clone_input(x) if isinstance(x, smith.Tensor) else x
    ...
    xs_fake_tensors: list[smith.Tensor | smith.SymInt | int] = [
        first_slice_copy(x) for x in xs
    ]
    output_fake_tensors: list[smith.Tensor | smith.SymInt | int] = [
...
class AssociativeScanAutogradOp(smith.autograd.Function):
    r""" associative_scan
        Example::
            xs = smith.arange(1, 5) = [1, 2, 3, 4]

            def combine_fn(a: smith.Tensor, b: smith.Tensor):
                return a * b
    ...
    Thus, only [1, bwys21, bwys32, bwys43] are required to compute the y_mat.
        ...
        3.) Materialize the ``combine_fn_bw``
            This is required because smith.compile and smith.autograd.grad
    ...
    def backward(ctx, *gl_ys):
        ...
        Args:
            flat_grads (smith.Tensor): The tensor of upstream gradients, or a nested pytree of tensors.
        ...
        mapped_combine_fn_bw_gm = smith.vmap(combine_fn_bw_gm, 0, 0)
        ...
        dummy_upstream_grad = (smith.ones_like(x) for x in gl_ys)
        ...
        def compute_y_mat(bwys: smith.Tensor) -> smith.Tensor:
            ...
            def compute_helper_tril_mask(diagonal):
                ...
                tril_mask = smith.tril(
                    smith.ones(
                        scan_length, scan_length, device=bwys.device, dtype=smith.bool
                    ),
                    diagonal=diagonal,
                )
        ...
        def compute_grad(bwxs, bwys, gl_ys):
            ...
            smith.select(bwxs, dim, 0).fill_(1.0)
        ...
        bwxs_stacked_leaves = smith.stack(bwxs)
        bwys_stacked_leaves = smith.stack(bwys)
        gl_ys_stacked_leaves = smith.stack(gl_ys)
        ...
        # TODO: smith.vmap may create composability issues
        compute_grad_mapped = smith.vmap(compute_grad, 0, 0)
...
@associative_scan_op.py_impl(ProxySmithDispatchMode)
...
def _fake_associative_scan(combine_fn, xs, dim, reverse=False):
    ...
    results = [
        smith.stack([e[leave_ind] for e in op(result_flat)], dim)
        for leave_ind in range(num_leaves)
    ]
