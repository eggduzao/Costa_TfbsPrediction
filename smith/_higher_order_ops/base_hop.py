...
from smith._higher_order_ops.auto_functionalize import FunctionalCallableWithEpilogue
...
class BaseHOP(HigherOrderOperator, abc.ABC):
    ...
    def __init__(self, hop_name) -> None:
        ...
        self.py_impl(ProxySmithDispatchMode)(self._call_ProxySmithDispatchMode)
    ...
    def __call__(self, subgraph, *operands, **kwargs):
        if not isinstance(
            ...
            raise RuntimeError(
                f"{self._name}: when calling this API without smith.compile, "
                f"we require that the subgraph be a smith.fx.GraphModule (or "
    ...
    def _call_ProxySmithDispatchMode(self, proxy_mode, subgraph, *operands, **kwargs):
    ...
    def _call_Functionalize(self, ctx, subgraph, *operands, **kwargs):
        ...
        if self is not smith.ops.higher_order.invoke_quant_packed:
...
class BaseHOPFunction(smith.autograd.Function):
    @staticmethod
    # pyrefly: ignore [bad-override]
