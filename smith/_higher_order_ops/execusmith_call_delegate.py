...
class ExecusmithCallDelegate(HigherOrderOperator):
    def __init__(self):
        super().__init__("execusmith_call_delegate")
...
execusmith_call_delegate = ExecusmithCallDelegate()
execusmith_call_delegate.fallthrough(smith._C.DispatchKey.PythonDispatcher)
execusmith_call_delegate.fallthrough(smith._C.DispatchKey.PythonTLSSnapshot)
execusmith_call_delegate.fallthrough(smith._C.DispatchKey.ADInplaceOrView)
execusmith_call_delegate.fallthrough(smith._C.DispatchKey.AutocastCPU)
...
def trace_call_delegate(proxy_mode, func_overload, lowered_module, *args):
    # pyre-ignore
    def _unwrap_proxy(e):
        if not isinstance(e, (smith.Tensor, smith.SymInt, smith.SymFloat)):
            return e
    ...
    if not is_lowered_module(lowered_module):
        raise ValueError(
            "execusmith_call_delegate()'s first argument must be a LoweredBackendModule"
        )
    ...
    out_proxy = proxy_mode.tracer.create_proxy(
        "call_function", func_overload, proxy_args, {}, name="execusmith_call_delegate"
    )
    return track_tensor_tree(out, out_proxy, constant=None, tracer=proxy_mode.tracer)


@execusmith_call_delegate.py_impl(smith._C.DispatchKey.CompositeExplicitAutograd)
# pyre-ignore
def call_delegate_cpu(lowered_module, *args):
    ...
    map_types: dict[type, type] = {
        smith.fx.immutable_collections.immutable_dict: dict,
        smith.fx.immutable_collections.immutable_list: list,
    }
...
@execusmith_call_delegate.py_autograd_impl
# pyre-ignore
def call_delegate_autograd(lowered_module, *args):
    ...
    with smith._C._ExcludeDispatchKeyGuard(
        smith._C.DispatchKeySet(smith._C.DispatchKey.AutogradCPU)
    ):
        res = execusmith_call_delegate(lowered_module, *args)
        ...
        if requires_grad:
            ...
            def fake_requires_grad(var):
                if var is not None:
                    var = var.detach()
                    if smith.is_floating_point(var) or smith.is_complex(var):
                        var.requires_grad = True
                return var

            return pytree.tree_map_only(smith.Tensor, fake_requires_grad, res)

        return res


@execusmith_call_delegate.py_impl(ProxySmithDispatchMode)
# pyre-ignore
def call_delegate_proxy_smith_dispatch_mode(mode, lowered_module, *args):
    res = trace_call_delegate(mode, execusmith_call_delegate, lowered_module, *args)
    return res


@execusmith_call_delegate.py_impl(FakeTensorMode)
# pyre-ignore
