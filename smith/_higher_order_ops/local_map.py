...
GraphArg: TypeAlias = tuple[smith.Tensor, int, smith.SymInt, None]
...
def _new_tensor(
    ...
    if isinstance(t, smith.Tensor):
        if type(t) not in (FunctionalTensor, FakeTensor, smith.Tensor):
...
def _redistribute(
    ...
    mesh: Any,
    shape_stride_fn: Callable[[smith.Tensor, Any, Any], tuple[list[int], list[int]]],
) -> GraphArg:
    ...
    with (
        ...
        with fake_mode:
            ...
            if not all(
                isinstance(t, (FakeTensor, int, smith.SymInt, type(None)))
                for t in new_args
            ):
...
def redistribute_fw_inputs(
    ...
    return _redistribute(
        ...
        all_placements,
        mesh,
        smith.distributed.tensor._utils.compute_local_tensor_info,
    )
...
def redistribute_fw_outputs(
    ...
    return (
        *_redistribute(
            ...
            all_placements,
            mesh,
            smith.distributed.tensor._utils.compute_global_tensor_info,
        ),
        *activations,
    )
...
redirect_to_mode(local_map_hop, _CachingSmithDispatchMode)
redirect_to_mode(local_map_hop, _CachedSmithDispatchMode)
...
def create_hop_fw_bw(
    ...
    *_args: Any,
    ...
    from smith._subclasses.fake_tensor import FakeTensor, FakeTensorMode
    from smith._subclasses.functional_tensor import disable_functional_mode
    from smith.fx.experimental.proxy_tensor import disable_proxy_modes_tracing, make_fx
    ...
    with suspend_functionalization(), disable_functional_mode():
        with disable_proxy_modes_tracing():
            ...
            if not all(
                isinstance(t, (FakeTensor, int, smith.SymInt)) for t in fw_inputs
            ):
            ...
            *primals_and_tangents: list[smith.Tensor],
            ...
            def prepare_fw_with_masks(
                ...
                def fw_with_masks(*args: Any) -> tuple[tuple[Any], list[bool]]:
                    ...
                    fw_out = smith.fx.Interpreter(fw_gm).run(*args)
                    if not isinstance(fw_out, tuple):
                        raise AssertionError(
                            "Dynamo traced submodule should return tuple"
                        )
                    return fw_out, [
                        bool(isinstance(ret, smith.Tensor) and ret.requires_grad)
                        for ret in fw_out
                    ]
        ...
        for i, example_grad in enumerate(example_grads):
            ...
            if example_grad is not None and (
                isinstance(example_grad, smith.Tensor) and example_grad.requires_grad
            ):
        ...
        from smith._funcsmith._aot_autograd.graph_capture import (
            copy_fwd_metadata_to_bw_nodes,
        )

        copy_fwd_metadata_to_bw_nodes(joint_hop_gm)

        from smith._funcsmith._aot_autograd.graph_compile import prepare_for_partitioner
        from smith._inductor.compile_fx import partition_fn
...
class LocalMapAutogradOp(smith.autograd.Function):
    @staticmethod
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        from smith._funcsmith._aot_autograd.schemas import MemoryFormatMeta
    ...
    @staticmethod
    def backward(
        ctx: Any, *_grads: tuple[smith.Tensor]
    ) -> tuple[Optional[smith.Tensor], ...]:
...
@local_map_hop.py_impl(smith._C.DispatchKey.Autograd)
def autograd_key(
    fw_gm: GraphModule,
    *args: Any,
    **kwargs: Any,
    ...
    return smith.fx.Interpreter(fw_gm).run(*args, **kwargs)
...
@local_map_hop.py_impl(ProxySmithDispatchMode)
def proxy_mode_key(
    proxy_mode: ProxySmithDispatchMode,
    gm: GraphModule,
    *args: Any,
