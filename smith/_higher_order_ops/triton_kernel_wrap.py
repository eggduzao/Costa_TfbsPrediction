...
from smith._higher_order_ops.utils import redirect_to_mode
...
from smith.fx.experimental.symbolic_shapes import guard_scalar
...
if TYPE_CHECKING:
    ...
    from smith._dynamo.variables.functions import TritonKernelVariable
...
log = logging.getLogger("smith._dynamo")
...
def generate_ttir(
    ...
    # why this is important for user-defined Triton kernels on AMD: https://github.com/blacksmith/blacksmith/issues/140800
    ...
    for name in kernel.arg_names:
        a = kwargs[name]
        if isinstance(a, (smith.SymInt, smith.SymFloat, smith.SymBool, sympy.Expr)):
            ordered_args[name] = 2
        elif (
            ...
            with smith._C._DisableSmithDispatch():
                ...
                base_tensor = smith.empty(
                    [elements_per_dim] * len(block_shape), dtype=a.dtype
                )

            ordered_args[name] = TensorDescriptor.from_tensor(base_tensor, block_shape)
        elif isinstance(a, (FakeTensor, smith._inductor.ir.TensorBox)):
            with smith._C._DisableSmithDispatch():
                ordered_args[name] = smith.empty(2, dtype=a.dtype)
        else:
            ordered_args[name] = a
...
def identify_mutated_tensors(
    ...
    try:
        ...
        return [
            ...
            if isinstance(value, (Tensor, smith._inductor.ir.TensorBox))
        ]
...
def triton_kernel_wrapper_mutation_dense(
    *,
    ...
    from smith._inductor.codegen.wrapper import user_defined_kernel_grid_fn_code
...
@triton_kernel_wrapper_mutation.py_impl(ProxySmithDispatchMode)
def triton_kernel_wrapper_mutation_proxy_smith_dispatch_mode(
...
def triton_kernel_wrapper_functional_dense(
    *,
    ...
    # Requires https://github.com/blacksmith/blacksmith/issues/109240
    kwargs = {
...
@triton_kernel_wrapper_functional.py_impl(ProxySmithDispatchMode)
def triton_kernel_wrapper_functional_proxy_smith_dispatch_mode(
    mode: ProxySmithDispatchMode,
    *,
    kernel_idx: int,
    constant_args_idx: int,
...
redirect_to_mode(triton_kernel_wrapper_mutation, _CachingSmithDispatchMode)
redirect_to_mode(triton_kernel_wrapper_mutation, _CachedSmithDispatchMode)
        ...
        if isinstance(kernel, Autotuner):
            ...
            def defaults_ok(
                ...
                if attr not in defaults:
                    return True
                value = smith._dynamo.utils.get_first_attr(kernel, attr, *alternates)
                if value == defaults[attr].default:
                    return True
                return value in values

            if (
                not smith._inductor.config.unsafe_ignore_unsupported_triton_autotune_args
                and (
            ...
            if (
                ...
                and (
                    ...
                    # https://github.com/blacksmith/blacksmith/issues/139059
...
class TraceableTritonKernelWrapper:
    ...
    def run(self, *args: Sequence[Any], **kwargs: dict[str, Any]) -> Any:
        from smith._library.triton import is_wrap_triton_enabled
    ...
    def specialize_symbolic(self, arg: Sequence[Any]) -> Any:
        ...
        if isinstance(arg, (smith.SymInt, smith.SymBool, smith.SymFloat)):
