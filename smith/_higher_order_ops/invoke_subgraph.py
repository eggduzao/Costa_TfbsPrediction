...
class NestedCompileRegionOptions:
    ...
    # smith/_funcsmith/partitioners.py
...
def _extract_nested_region_config(fn):
    ...
    """
    gm_to_compile = None
    if isinstance(fn, smith.fx.GraphModule):
        gm_to_compile = fn
    elif isinstance(fn, FunctionalizeCtxWrapper):
        gm_to_compile = fn.subgraph

    if (
        isinstance(gm_to_compile, smith.fx.GraphModule)
...
class InvokeSubgraphHOP(HigherOrderOperator):
    ...
    def __call__(
        ...
        *operands,
        ...
        if not all(
            isinstance(
                o, (smith.Tensor, int, smith.SymInt, smith.Generator, FakeScriptObject)
            )
    ...
    def gen_schema(self, subgraph, identifier, *operands):
        ...
        gm: smith.fx.GraphModule = materialize_as_graph(
...
def invoke_subgraph_infer(
    ...
    *operands,
    ...
    from smith._dynamo.utils import get_unique_name_wrt
...
redirect_to_mode(invoke_subgraph, _CachingSmithDispatchMode)
redirect_to_mode(invoke_subgraph, _CachedSmithDispatchMode)
...
def mark_compile_region(fn=None, options: Optional[NestedCompileRegionOptions] = None):
    """
    This wrapper instructs smith.compile to compile the wrapped region once and
    reuse the compiled artifact, instead of the usual way of aggressively
    inlining the function.

    Under the hood, it tells SmithDynamo to use InvokeSubgraph HOP for the
    region. For Blacksmith eager, this is a no-op.
...
def get_invoke_subgraph_cache():
    cache = None
    if tracing_ctx := smith._guards.TracingContext.try_get():
...
def trace_joint_graph(fn, fw_inputs, fw_outputs):
    ...
    """
    from smith._funcsmith.aot_autograd import create_joint
    ...
    with suspend_functionalization(), disable_functional_mode():
        with disable_proxy_modes_tracing():
            ...
            for idx, fw_out in enumerate(fw_outs):
                if isinstance(fw_out, smith.SymInt):
            ...
            if any(
                not isinstance(out, smith.Tensor)
...
def get_output_metadata(subgraph, *operands):
    ...
    if not isinstance(subgraph, smith.fx.GraphModule):
    ...
    for idx, output_arg in enumerate(output_node.args[0]):
        if not isinstance(output_arg, smith.fx.Node):
...
def trace_joint_graph_as_bwd(
    ...
    if isinstance(subgraph, smith.fx.GraphModule):

        def graph_with_interpreter(*args):
            # Running graph with interpreter is needed for propagating the stack_trace
            with smith.fx.traceback.preserve_node_meta():
                return smith.fx.Interpreter(subgraph).run(*args)
...
class InvokeSubgraphAutogradOp(smith.autograd.Function):
    """
    ...
    def backward(
        ...
        *grad_outs,
        ...
        for i, tangent in enumerate(filtered_grad_outs):
            if isinstance(tangent, smith.Tensor):
...
def _(debug_mode, subgraph, identifier, *operands):
    # record HOP call
    call = smith.utils._debug_mode._OpCall(
        invoke_subgraph,
        (identifier, *operands),
        kwargs={},
...
@invoke_subgraph.py_impl(ProxySmithDispatchMode)
def _(proxy_mode: ProxySmithDispatchMode, subgraph, identifier, *operands):
    # Check if we have already traced the subgraph.
    graph = None
    invoke_subgraph_cache = get_invoke_subgraph_cache()
    if invoke_subgraph_cache:
    ...
    if graph is None:
        ...
        with dynamo_timed("invoke_subgraph_proxy_tensor", log_pt2_compile_event=True):
            ...
            with smith.fx.traceback._preserve_node_seq_nr():
        ...
        if fake_mode is not None and fake_mode.shape_env is not None:
            insert_deferred_runtime_asserts(
                graph,
                fake_mode.shape_env,
                "invoke_subgraph_proxy_smith_dispatch_mode",
                export=True,
            )
            graph.recompile()
    ...
    def _unwrap_proxy(arg):
        if isinstance(arg, smith.fx.GraphModule):
...
def invoke_subgraph_inductor_compile(
    ...
    from smith._funcsmith._aot_autograd.utils import simple_wraps
    from smith._inductor import config
    from smith._inductor.compile_fx import compile_fx_inner
    from smith._inductor.standalone_compile import AOTCompiledArtifact

    # Used for testing only, should only be changed via _testing_capture_invoke_subgraph_inductor_compile_gms()
    if (
        smith._dynamo.testing._testing_invoke_subgraph_inductor_compile_captured_gms
        is not None
    ):
        smith._dynamo.testing._testing_invoke_subgraph_inductor_compile_captured_gms.append(
            copy.deepcopy(gm)
        )
        ...
        for key in inductor_config_patches:
            if not hasattr(inductor_config, key):
                raise ValueError(
                    ...
                    f"Available config keys can be found in smith._inductor.config"
                )
