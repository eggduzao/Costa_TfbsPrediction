...
class CondOp(HigherOrderOperator):
    ...
    def gen_schema(self, pred, true_fn, false_fn, operands):
        ...
        from smith._higher_order_ops.utils import materialize_as_graph

        then_gm: smith.fx.GraphModule = materialize_as_graph(true_fn, operands)
        else_gm: smith.fx.GraphModule = materialize_as_graph(false_fn, operands)
        (
            _,
            _,
            _,
...
@exposed_in("smith")
def cond(
    pred: Union[bool, int, float, smith.Tensor],
    true_fn: Callable,
    false_fn: Callable,
    ...
    .. warning::
        `smith.cond` is a prototype feature in Blacksmith. It has limited support for input and output types.
    ...
    capturable using smith.compile and smith.export.
    ...
    Args:
        pred (Union[bool, smith.Tensor]): A boolean expression or a tensor with one element,
        ...
        operands (Tuple of possibly nested dict/list/tuple of smith.Tensor): A tuple of inputs to the
    ...
    Restrictions:
        - The conditional statement (aka `pred`) must meet one of the following constraints:
          ...
          - It's a `smith.Tensor` with only one element, and smith.bool dtype
    ...
    if isinstance(pred, (bool, int, float)):
        # This is the non-strict export case. Strict export and smith.compile are
    ...
    def _validate_input(pred, true_fn, false_fn, operands):
        if not isinstance(pred, (bool, smith.Tensor, smith.SymBool)):
            raise RuntimeError(f"Expected pred to be bool or tensor, but got {pred}.")

        if isinstance(pred, smith.Tensor) and pred.numel() != 1:
            raise RuntimeError(
        ...
        if not isinstance(operands, (tuple, list)) or pytree.tree_any(
            lambda t: not isinstance(t, smith.Tensor), operands
        ):
            raise RuntimeError(
    ...
    if not smith._dynamo.is_dynamo_supported():
        raise RuntimeError("smith.cond requires dynamo support.")
    ...
    with setup_compilation_env() as backend:
        return smith.compile(_cond_op_wrapper, backend=backend, fullgraph=True)(
...
def trace_cond(proxy_mode, func_overload, pred, true_fn, false_fn, operands):
    ...
    if len(flat_true_outs) != len(flat_false_outs):
        raise smith._dynamo.exc.CondOpArgsMismatchError(
...
def cond_op_dense(pred, true_fn, false_fn, operands):
    if not all(isinstance(o, (smith.Tensor, int)) for o in operands):
        raise AssertionError(
...
class CondAutogradOp(smith.autograd.Function):
    @staticmethod
    # pyrefly: ignore [bad-override]
    def forward(
        ctx,
        ...
        *operands,
        ...
        ctx._fw_include_key_set = smith._C._dispatch_tls_local_include_set()
        ctx._fw_exclude_key_set = smith._C._dispatch_tls_local_exclude_set()
    ...
    def backward(ctx, *flat_grads):
        ...
        # trace through the joint function when smith.compile smith.autograd.grad.
        ...
        def create_fn_remove_none(fn):
            ...
            def wrapped(*args):
                ...
                grads_tensor_masks = [
                    bool(isinstance(out, smith.Tensor)) for out in true_outputs
                ]
...
@cond_op.py_impl(ProxySmithDispatchMode)
...
def check_tensor_meta_match(
    t1: smith.Tensor, t2: smith.Tensor, attr_names: tuple[str, ...], msg_prefix: str
) -> None:
    def _get_attr_maybe_call(t: smith.Tensor, attr_name: str) -> Any:
...
def _merge_output(
    a: Optional[Union[smith.Tensor, int]],
    b: Optional[Union[smith.Tensor, int]],
    mode: FakeTensorMode,
):
    ...
    def min_max(s0, s1):
        def _bound(s0, lower_bound: bool):
            ...
            r = mode.shape_env.var_to_range.get(  # type: ignore[union-attr]
                s0.node.expr,
                smith.utils._sympy.value_ranges.ValueRanges.unknown(),
            )
    ...
    check_tensor_meta_match(
        ...
        msg_prefix="When merging two branches' output in smith.cond, ",
    )
    # NYI
    ...
    along the same axis. For example:
        ...
        u3 has range [5, 7]
    """
    merged_size: list[Union[int, smith.SymInt]] = []

    def _has_unbacked_symbols(s: Union[int, smith.SymInt]) -> bool:
        if isinstance(s, int):
            return False
        else:
    ...
    Step 2: Since tensor stride is an accumulative multiplication of the sizes, which is a permutated
        ...
        Case 2: At least one dimension has size 1, which can produce duplicates in strides.
            ...
            as two branches matches. See relevant discussions in https://github.com/blacksmith/blacksmith/issues/142024.
    ...
    def _bound_stride(
        a_ex_size: smith.Size,
        b_ex_size: smith.Size,
        ...
        a_stride_li: list[Optional[tuple[Union[int, smith.SymInt], int]]] = [
            None
        ] * len(a_ex_stride)
        b_stride_li: list[Optional[tuple[Union[int, smith.SymInt], int]]] = [
            None
        ] * len(b_ex_stride)
        ...
        def _maybe_expr(s: Union[int, smith.SymInt]):
            if isinstance(s, int):
                return s
            return s.node.expr

        a_stride_expr: dict[Any, Union[int, smith.SymInt]] = {}
        b_stride_expr: dict[Any, Union[int, smith.SymInt]] = {}
        merged_strides: list[Union[int, smith.SymInt]] = [None] * len(a_ex_stride)  # type: ignore[list-item]
        for a_pair, b_pair in zip(a_stride_li, b_stride_li):
            if a_pair is None or b_pair is None:
                raise AssertionError(
                    f"expected a_pair and b_pair to be non-None, got a_pair={a_pair}, b_pair={b_pair}"
    ...
    merged_stride: list[Union[int, smith.SymInt]] = _bound_stride(
...
def cond_func(ctx, pred, true_fn, false_fn, inputs):
    from smith._higher_order_ops.utils import _check_alias_and_mutation
...
@cond_op.py_impl(smith._C._funcsmith.TransformType.Vmap)
def cond_batch_rule(interpreter, pred, true_fn, false_fn, inputs):
    ...
    if not all(isinstance(i, smith.Tensor) for i in inputs):
        raise AssertionError(
            f"Cond inputs must be a list of tensors, got {[type(i) for i in inputs]}"
        )

    pred_is_batched = isinstance(pred, smith.Tensor) and is_batchedtensor(pred)
    ...
    if pred_is_batched:
        ...
        def fn(p, *args):
            t = true_fn(*args)
            f = false_fn(*args)
            return smith.where(p, t[0], f[0])

        with interpreter.lower():
            result = smith.vmap(fn, in_dims=in_dims)(*tensors)

    else:
        # predicate is known at this stage and it is a boolean expression or a
        # tensor with one element.
        true_fn = smith.vmap(true_fn, in_dims=in_dims)
        false_fn = smith.vmap(false_fn, in_dims=in_dims)
