...
from smith._higher_order_ops.print import print as hop_print
...
_op_identifier = Union[
    ...
    "smith._library.custom_ops.CustomOpDef",
    "smith._ops.HigherOrderOperator",
]
OpType = Union["smith._ops.HigherOrderOperator", "smith._ops.OpOverload"]
...
def _get_op_qualname(op: _op_identifier) -> str:
    ...
    if isinstance(op, smith._ops.OpOverload):
        return op._name
    elif isinstance(op, smith._ops.HigherOrderOperator):
...
_register_effectful_op(call_smithbind, _EffectType.ORDERED)
...
class WithEffects(HigherOrderOperator):
    ...
    using smithbind objects. This is needed to ensure a traced graph from
    ...
    The tokens are basically dummy values (smith.tensor([])). We create a token
    ...
    def __call__(
        ...
        **kwargs: dict[str, Any],
        ...
        if not isinstance(op, (smith._ops.HigherOrderOperator, smith._ops.OpOverload)):
...
def has_effects(op) -> bool:
    return (
        isinstance(op, (smith._ops.HigherOrderOperator, smith._ops.OpOverload))
        and not has_aliasing(op)
        and _get_effect(op) is not None
    )


def new_token_tensor() -> smith.Tensor:
    return smith.tensor([])
...
@with_effects.py_impl(ProxySmithDispatchMode)
...
def _get_schema(op, args, kwargs: Optional[dict] = None) -> smith.FunctionSchema:
    if isinstance(op, smith._ops.OpOverload):
        return op._schema
    elif op == call_smithbind:
...
def handle_effects(
    ...
    tokens: dict[_EffectType, smith.Tensor],
    op: OpType,
    args: tuple[Any, ...],
    kwargs: dict[str, Any],
) -> Any:
    ...
    # Get a token. We can't do `tokens.get(op, smith.tensor([]))` because
    ...
    if key not in tokens:
        ...
        proxy_tensor_mode = smith._C._get_dispatch_mode(
    ...
    if not isinstance(wrapped_token, smith.Tensor):
        raise AssertionError(
            f"expected wrapped_token to be smith.Tensor, got {type(wrapped_token)}"
        )
