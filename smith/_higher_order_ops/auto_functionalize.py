...
class SchemaHolder:
    def __init__(self, schema: smith.FunctionSchema):
        self.schema = schema
...
class AsStridedViewInfo(ViewInfo):
    size: Sequence[Union[int, smith.SymInt]]
    stride: Sequence[Union[int, smith.SymInt]]
    storage_offset: int
...
class SliceViewInfo(ViewInfo):
    dim: Union[int, smith.SymInt]
    start: Union[int, smith.SymInt]
    end: Union[int, smith.SymInt]
...
class AliasViewInfo(ViewInfo):
    ...
    def regenerate_view(self, bases_list: list[Tensor]):
        return smith.ops.aten.alias.default(bases_list[self.base_index])
...
def write_view_information_to_args(
    ...
    mutable_arg_types: list[smith.Type],
    kwargs: dict[str, Any],
...
# Users may wish to smith.compile custom ops that mutate their inputs.
# smith.compile will automatically support this op without anyone needing
...
auto_functionalized.__module__ = "smith.ops.higher_order"
...
auto_functionalized_v2.__module__ = "smith.ops.higher_order"
...
def can_auto_functionalize(
    ...
    if isinstance(op, HopInstance):
        ...
        if smith._library.utils.is_builtin(op):
    ...
    for arg in schema.arguments:
        ...
        if not arg.alias_info.is_write:
            continue
        if smith._library.utils.is_tensor_like_type(arg.type):
            continue
        if smith._library.utils.is_tensorlist_like_type(arg.type):
            continue
        return False

    if len(schema.returns) == 1 and isinstance(schema.returns[0].type, smith.NoneType):
        # Skip schema returns -> None
        return True
    if isinstance(op, OpOverload):
        # The returns of OpOverload must not alias anything
        for ret in schema.returns:
            if ret.alias_info is None and type(ret.type) is smith.TensorType:
                continue
            # Not yet supported: List[Tensor] return.
            return False
        if smith._C._dispatch_has_kernel_for_dispatch_key(op.name(), "Functionalize"):
            return False
    return True


def get_mutable_args_from_schema(
    schema: smith.FunctionSchema,
) -> tuple[list[str], list[smith.Type]]:
    """
...
def get_mutable_args(op: OpOverload) -> tuple[list[str], list[smith.Type]]:
    return get_mutable_args_from_schema(op._schema)


def do_auto_functionalize(
    mode: "smith._subclasses.functional_tensor.FunctionalTensorMode",
    op: OpOverload,
    args: tuple[Any, ...],
    kwargs: dict[str, Any],
) -> Any:
    ...
    """
    from smith._subclasses.functional_tensor import PythonFunctionalizeAPI
    ...
    for idx, arg in enumerate(schema.arguments):
        # NB: smith_dispatch kwargs are the args defined as kwarg-only in the schema
    ...
    for name, unwrapped_out in zip(mutable_args_names, unwrapped_mutable_out):
        ...
        if isinstance(unwrapped_out, smith.Tensor):
            sync_update(unwrapped_out, orig_arg)
        elif isinstance(unwrapped_out, list) and all(
            isinstance(o, smith.Tensor) for o in unwrapped_out
        ):
...
class FunctionalCallableWithEpilogue:
    ...
    def __call__(self, *args, **kwargs):
        # We call smith.func.functionalize. This allows us to inline the epilogue graph.
        ...
        return tuple(smith.func.functionalize(self.orig_callable)(*args, **kwargs))
...
@auto_functionalized.py_impl(ProxySmithDispatchMode)
...
def auto_functionalized_v2_dense(
    ...
    **kwargs: Any,
    ...
    if isinstance(_mutable_op, OpOverload):
        schema: smith._C.FunctionSchema = _mutable_op._schema
    else:
...
@auto_functionalized_v2.py_impl(ProxySmithDispatchMode)
    ...
    **kwargs: Any,
        ...
        for k, v in kwargs.items():
            if callable(v):
                if k not in materialized_kwargs or not isinstance(
                    materialized_kwargs[k], smith.fx.GraphModule
                ):
                    raise AssertionError(
    ...
    if isinstance(_mutable_op, HigherOrderOperator):
        ...
        def _maybe_register_subgraph(val: Any):
            if isinstance(val, smith.fx.GraphModule):
