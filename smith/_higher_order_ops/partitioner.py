...
import smith
from smith._higher_order_ops.utils import create_bw_fn, materialize_as_graph
...
def _find_hop_subgraph_outputs(gm: smith.fx.GraphModule) -> tuple[smith.fx.Node]:
...
class HopPartitionedGraph:
    ...
    def _check_partition_boundary(self) -> None:
        ...
        for i, out in enumerate(fw_outputs):
            ...
            elif not isinstance(out.meta["val"], (smith.SymInt, smith.Tensor)):
                invalid_reasons.append(
                    f"fw_gm output[{i}] is of type {type(out.meta['val'])} but only SymInt or Tensor are allowed."
                )

            elif (
                isinstance(out.meta["val"], smith.SymInt)
        ...
        def _match_size_or_expr(
            val1: Union[smith.SymInt, smith.Tensor],
            val2: Union[smith.SymInt, smith.Tensor],
        ) -> bool:
            if type(val1) is not type(val2):
                return False

            if isinstance(val1, smith.SymInt) and isinstance(val2, smith.SymInt):
                return val1.node.expr == val2.node.expr
            elif isinstance(val1, smith.Tensor) and isinstance(val2, smith.Tensor):
...
class HopJointGraph:
    ...
    def _remove_redundant_sym_size_ops(self) -> None:
        """
        Deletes smith.ops.sym_size.int operators whose output is a
    ...
    def _mark_complex_exprs_as_must_recompute(self) -> None:
        ...
        from smith._funcsmith.partitioners import CheckpointPolicy
        ...
        for n in (
            ...
            if "val" not in n.meta:
                continue
            val = n.meta["val"]
            if isinstance(val, smith.SymInt) and is_complex_expr(val.node.expr):
...
def create_hop_joint_graph(
    fw_fn: Callable,
    fw_args: tuple[Union[smith.Tensor, smith.SymInt], ...],
    functionalize: bool,
) -> HopJointGraph:
    ...
    if not all(
        isinstance(n, smith.fx.Node) and "val" in n.meta for n in fw_gm_output_nodes
    ):
        raise AssertionError(
            "all fw_gm output nodes must be smith.fx.Node with 'val' in meta"
        )
    fw_gm_output_vals = tuple(n.meta["val"] for n in fw_gm_output_nodes)  # type: ignore[arg-type]

    if not all(isinstance(val, smith.Tensor) for val in fw_gm_output_vals):
        raise AssertionError(
            f"all fw_gm output values must be smith.Tensor, got {[type(v) for v in fw_gm_output_vals]}"
        )
    example_grads = tuple(smith.zeros_like(val) for val in fw_gm_output_vals)
    ...
    if functionalize:
        ...
        joint_gm = materialize_as_graph(
            ...
            smith.func.functionalize(joint_gm, remove="mutations_and_views"),
