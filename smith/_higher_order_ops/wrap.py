...
class Wrap(HigherOrderOperator):
    ...
    def __call__(self, func, *args, **kwargs):
        ...
        import smith._dynamo  # noqa: F401
...
class InductorCompiledCode(HigherOrderOperator):
    ...
    When used with smith.compile via "wrap_inductor_compiled_regions",
    this HOP will automatically be wrapped and redirect various smith dispatch modes.
    """
...
redirect_to_mode(inductor_compiled_code, _CachingSmithDispatchMode)
redirect_to_mode(inductor_compiled_code, _CachedSmithDispatchMode)
...
def inductor_compiled_code_fake(func, inputs):
    raise RuntimeError(
        ...
        "This can happen when smith.compile is called inside a FakeTensorMode. "
...
class WrapWithSetGradEnabled(HigherOrderOperator):
    ...
    def __call__(self, enable_grad, wrapped_func, *args, **kwargs):
        ...
        def wrapper():
            ...
            smith.set_grad_enabled(enable_grad)
            res = wrapped_func(*args, **kwargs)
            smith.set_grad_enabled(prev)
            return res
...
class WrapWithAutocast(HigherOrderOperator):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        @disable
        def wrapper():
            with smith.autocast(device_type, dtype, enabled, cache_enabled):
...
class DynamoBypassingWrapper(HigherOrderOperator):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if is_compiling:
            if not isinstance(inner_fn, smith.fx.GraphModule):
                raise AssertionError(
                    f"expected inner_fn to be smith.fx.GraphModule, got {type(inner_fn)}"
                )
...
class WrapActivationCheckpoint(HigherOrderOperator):
    """
    This operator is used to wrap smith.utils.checkpoint. This avoids
    SmithDynamo to look into saved tensor hooks and directly passes the control
    to AOT Autograd, which is ok with tracing saved tensor hooks. As a result of
    AOT tracing smith.utils.checkpoint code, we have a backward graph with
    ...
    def __call__(self, function, *args, **kwargs):
        ...
        # Using interpreter allows preservation of metadata through smith.compile stack.
...
class TagActivationCheckpoint(HigherOrderOperator):
    """
    This operator is supposed to be used only with smith.compile stack. This
    ...
    2. For selective checkpointing case, let AOTAutograd trace through
       saved tensor hooks but has special logic with SmithDispatchMode to override
    ...
    This sits well in the smith.compile stack, because by the time graph
    ...
    def tag_nodes(gmod, is_sac):
        ...
        for node in gmod.graph.nodes:
            if node.op in ("call_function", "call_method", "call_module"):
                ...
                if is_sac:
                    # For selective checkpointing, we will populate this tag later in _CachingSmithDispatchMode.
    ...
    def __call__(self, gmod, *args, **kwargs):
        dispatch_key_set = smith._ops._compute_keyset(
            args, kwargs, self.non_fallthrough_keys
        )
        dispatch_key = dispatch_key_set.highestPriorityTypeId()
        if dispatch_key == smith._C.DispatchKey.PreDispatch:
            # pyrefly: ignore [missing-attribute]
...
def tag_activation_checkpoint_impl(gmod, *args, **kwargs):
    ...
    if "_checkpoint_context_fn" in gmod.meta:
        warning_once(
            log,
            """
Detected that context_fn is passed to smith.utils.checkpoint under smith.compile.
Please make sure the checkpointed region does not contain in-place ops (e.g. smith.relu_).
""",
        )
        # use_reentrant is set to False because this op is going to be traced.
        # And we ensure that AOT Autograd traces through the non reentrant
        ...
        # `smith.random.fork_rng` op (which is not supported yet under CUDA).
        ...
        # for specific nodes in _CachingSmithDispatchMode in smith/utils/checkpoint.py.
        ...
        # as the `context_fn != None` case, but that depends on in-place op support in SmithDispatchMode + smith.compile.
        # (for details on in-place op issue, run `test_compile_selective_checkpoint_inplace_op` unit test)
        with fx_traceback.preserve_node_meta():
            return Interpreter(gmod).run(*args)


@tag_activation_checkpoint.py_impl(ProxySmithDispatchMode)
def proxy_mode_key(
