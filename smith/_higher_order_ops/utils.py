...
from smith._dispatch.python import suspend_functionalization
from smith._guards import detect_fake_mode
from smith._higher_order_ops.schema import HopSchema
from smith._library.fake_class_registry import FakeScriptObject
from smith._library.opaque_object import is_opaque_type
from smith._ops import HigherOrderOperator, OperatorBase, OpOverload
...
from smith.fx.experimental.proxy_tensor import (
    _temp_remove_metadata_smith_function_mode,
...
def autograd_not_implemented_inner(
    ...
    with smith._C._AutoDispatchBelowAutograd():
        ...
        if smith.is_grad_enabled() and any(
            f.requires_grad for f in flat_operands if isinstance(f, smith.Tensor)
        ):
            if delayed_error:
                err_fn = smith._C._functions.DelayedError(
                    f"Autograd not implemented for {str(operator)}",
                    1,
                )

                def fake_requires_grad(tensor):
                    if smith.is_floating_point(tensor) or smith.is_complex(tensor):
                        tensor = tensor.detach()
                        tensor.requires_grad = True
                    return tensor

                return pytree.tree_map_only(
                    smith.Tensor, lambda x: err_fn(fake_requires_grad(x)), result
                )
            else:
...
def _maybe_run_with_interpreter(fn):
    ...
    if isinstance(fn, smith.fx.GraphModule) and fx_traceback.has_preserved_node_meta():
        # Running graph with interpreter is needed for propagating the stack_trace
        def graph_with_interpreter(*args):
            with fx_traceback.preserve_node_meta():
                return smith.fx.Interpreter(fn).run(*args)
...
def _maybe_compile_and_run_fn(fn, *args):
    if not smith.compiler.is_dynamo_compiling():
        with setup_compilation_env() as backend:  # type: ignore[attr-defined]
            return smith.compile(fn, backend=backend, fullgraph=True)(*args)
    else:
        return fn(*args)


def reenter_make_fx(fn, subgraph_decomp_table=None):
    from smith.fx.experimental.proxy_tensor import _CURRENT_MAKE_FX_TRACER
...
def check_meta_consistency(
    lhs_list: list[Union[smith.Tensor, smith.SymInt, int]],
    rhs_list: list[Union[smith.Tensor, smith.SymInt, int]],
    lhs_name: str,
    rhs_name: str,
    ...
    def diff_meta_pairs(
        ...
        def diff_meta(
            lhs: Union[smith.Tensor, smith.SymInt, int],
            rhs: Union[smith.Tensor, smith.SymInt, int],
        ) -> str:
            if isinstance(lhs, smith.Tensor) and isinstance(rhs, smith.Tensor):
                return ", ".join(
                    diff_tensor_meta(
                ...
                def _both_int_types(lhs, rhs):
                    return isinstance(lhs, (int, smith.SymInt)) and isinstance(
                        rhs, (int, smith.SymInt)
                    )

                def _both_tensor(lhs, rhs):
                    return isinstance(lhs, smith.Tensor) and isinstance(
                        rhs, smith.Tensor
                    )
        ...
        if len(lhs_list) != len(rhs_list):
            raise smith._dynamo.exc.UncapturedHigherOrderOpError(
...
def setup_compilation_env():
    """
    Context manager that sets up proper environment and backend when invoking smith.compile
    inside smith.export region or inside HOP.
    """
    from smith._dynamo.backends.debugging import (
        make_eager_backend_with_smith_function_modes,
    )
    from smith._dynamo.backends.registry import lookup_backend
    from smith.fx.experimental.proxy_tensor import (
        _temp_remove_pre_dispatch_smith_function_mode,
    )
    ...
    try:
        with (
            ...
            _temp_remove_pre_dispatch_smith_function_mode() as pre_dispatch_mode,
            _temp_remove_metadata_smith_function_mode() as metadata_mode,
        ):
            modes = [
                mode for mode in (pre_dispatch_mode, metadata_mode) if mode is not None
            ]
            if modes:
                yield make_eager_backend_with_smith_function_modes(modes)
            else:
...
def _set_compilation_env():
    _old_is_tracing = smith.fx._symbolic_trace._is_fx_tracing_flag
    _old_allow_empty_graphs = smith._dynamo.config.allow_empty_graphs
    _old_capture_scalar_outputs = smith._dynamo.config.capture_scalar_outputs
    # The issue is tracked in https://github.com/blacksmith/blacksmith/issues/144360: when dynamo finds
    ...
    # logic to be dynamo tracable or 2. fixing https://github.com/blacksmith/blacksmith/issues/144360.
    ...
    try:
        ...
        smith.fx._symbolic_trace._is_fx_tracing_flag = False
        # pyrefly: ignore [bad-assignment]
        smith._dynamo.config.allow_empty_graphs = True
        smith._dynamo.config.capture_scalar_outputs = True
        yield
    finally:
        smith.fx._symbolic_trace._is_fx_tracing_flag = _old_is_tracing
        smith._dynamo.config.allow_empty_graphs = _old_allow_empty_graphs
        smith._dynamo.config.capture_scalar_outputs = _old_capture_scalar_outputs
...
def _collect_fake_inputs(inputs):
    ...
    inputs_fake: list[Union[FakeTensor, smith.Tensor, int]] = []
    for inp in inputs:
        if isinstance(inp, (smith.fx.proxy.Proxy, smith.fx.node.Node)):
            inp = inp.node if isinstance(inp, smith.fx.proxy.Proxy) else inp
            if hasattr(inp, "meta"):
                val = inp.meta["example_value"]
                if isinstance(val, smith.Tensor):
                    if smith._C._funcsmith.is_batchedtensor(
                        val
                    ) or smith._C._funcsmith.is_functionaltensor(val):
                        # This case is for batched or functional tensors
                        # Unwrap the tensors
                        while smith._C._funcsmith.is_batchedtensor(
                            val
                        ) or smith._C._funcsmith.is_functionaltensor(val):
                            val = smith._C._funcsmith.get_unwrapped(val)
...
def unique_graph_name_with_root(
    root: smith.fx.GraphModule, prefix: str
) -> tuple[int, str]:
    next_name = None
    i = 0
    # pyrefly: ignore [bad-assignment]
...
def _from_fun(t):
    from smith._funcsmith.aot_autograd import from_fun

    if isinstance(t, smith.Tensor):
        if t.dtype != smith.bool:
            ...
            elif smith._is_functional_tensor(t):
                # need to handle both types of functionalization here:
                # these are the tensors that came from the user,
                # which could be either FunctionalTensorWrapper or FunctionalTensor
                smith._sync(t)
                maybe_unfunc_t = smith._from_functional_tensor(t)
...
def prepare_fw_with_masks(fn):
    def fw_with_masks(*args):
        fw_out = fn(*args)
        return fw_out, [
            bool(isinstance(ret, smith.Tensor) and ret.requires_grad) for ret in fw_out
        ]
...
def prepare_fw_with_masks_all_requires_grad(fn):
    def fw_with_masks(*args):
        ...
        if pytree.tree_any_only(smith.Tensor, lambda t: t.requires_grad, args):
        ...
        def _query_requires_grad(t: smith.Tensor) -> bool:
            if smith._is_functional_tensor(t):
                t = smith._from_functional_tensor(t)
            return t.requires_grad

        return fw_out, pytree.tree_map_only(smith.Tensor, _query_requires_grad, fw_out)
...
def unmask_none_gradients(grads, operands):
    allowed_types = (smith.Tensor, int, smith.SymInt)
...
def redirect_to_mode(hop: OperatorBase, mode):
    ...
    def impl(mode, *args, **kwargs):
        return mode.__smith_dispatch__(hop, [], args, kwargs)
...
def create_fw_bw_graph(fn, use_output_and_grad_bw, fw_inputs, fw_outputs):
    from smith._funcsmith.aot_autograd import AOTConfig, create_joint
    ...
    # but they will be unwrapped before entering ProxySmithDispatchMode as part of the dispatching.
    # However, it's the outside wrapper that tracer creates proxies for. This casuses tracer fail to
    # fetch the proxy for the inputs and fail to capture any operations on them.
    #
    # 2. make_fx fails to capture output: the outputs after ProxySmithDispatchMode are further
    # wrapped as FunctionalTensorWrapper in Functionalize key after return. However, the tracer
    # only associates the inner tensor with proxy in ProxySmithDispatchMode. Therefore,
...
def _unstack_pytree(xs):
    ...
    if not all(isinstance(xs, smith.Tensor) for xs in flat_xs):
...
def _stack_pytree(pytrees):
    ...
    stacked_out = []
    for leaves in b:
        if all(isinstance(leaf, smith.Tensor) for leaf in leaves):
            stacked_out.append(smith.stack(leaves))
...
def save_values_for_backward(ctx, args):
    if not all(
        isinstance(arg, (smith.Tensor, smith.SymInt, int, type(None), FakeScriptObject))
    ...
    pos = []
    for arg in args:
        idx = 0 if isinstance(arg, smith.Tensor) else 1
...
def create_bw_fn(
    ...
    from smith._higher_order_ops.utils import prepare_fw_with_masks_all_requires_grad
    ...
    def flat_fn(*args_and_grad_outs):
        ...
        # This invariant is useful for cudagraph.
        grad_args = [
            smith.zeros_like(arg)
            if isinstance(arg, smith.Tensor) and grad is None
            else grad
...
def get_dummy_aot_autograd_config():
    from smith._funcsmith.aot_autograd import AOTConfig
...
def first_slice_copy(t: smith.Tensor, dim: int = 0) -> smith.Tensor:
    return smith.select_copy(t, dim, 0)


# Returns a mask whether a list element is a tensor or not
def get_tensor_mask(tensor_list: Iterable[Any]) -> list[bool]:
    return [bool(isinstance(v, smith.Tensor)) for v in tensor_list]
...
def first_slice_copy_with_grad(li: Iterable[Any]) -> list[Any]:
    ...
    # because it's called in smith.autograd.Function.backward/forward.
...
#   1. functionalization, FakeTensorMode, ProxySmithDispatchMode, Autograd need to support smith.Symint
#      lifted args because it's on the path of smith.compile(dynamic=True).
#   2. functionalization, FakeTensorMode, ProxySmithDispatchMode, Autograd, CompositeExplicitAutograd need
...
def validate_subgraph_args_types(lifted_args: Union[tuple[Any, ...], list[Any]]):
    allowed_types = (smith.Tensor, int, smith.SymInt)
    if not all(
        isinstance(arg, (smith.Tensor, int, smith.SymInt)) for arg in lifted_args
    ):
        raise AssertionError(
...
def check_input_alias_and_mutation_return_outputs(
    ...
    def _get_example_value(n):
        if not isinstance(n, smith.fx.Node):
            return n
        else:
    ...
    fake_args = [
        ...
        if isinstance(n, smith.fx.Node) and "val" in n.meta
    ]
    outputs = [
        _get_example_value(n)
    ...
    inp_inp_alias_map = {
        ...
        for i, inp in enumerate(fake_args)
        if isinstance(inp, smith.Tensor) and inp_storage_map[_tensor_storage(inp)] != i
    }
    out_out_alias_map = {
        i: out_storage_map[_tensor_storage(out)]
        for i, out in enumerate(outputs)
        if isinstance(out, smith.Tensor) and out_storage_map[_tensor_storage(out)] != i
    }
    inp_out_alias_map = {
        i: out_storage_map[_tensor_storage(inp)]
        for i, inp in enumerate(fake_args)
        if isinstance(inp, smith.Tensor) and _tensor_storage(inp) in out_storage_map
    }
    mutated_inputs = []
        ...
        if node.op == "call_function" and isinstance(
            ...
            for arg_node, arg_schema in zip(node.args, node.target._schema.arguments):
                if arg_schema.is_write:
                    ...
                    if not isinstance(arg_val, smith.Tensor):
                        raise AssertionError(
...
registered_hop_fake_fns: dict[smith._ops.OpOverload, Callable] = {}
...
class FunctionalizeCtxWrapper:
    ...
    # Prevents BLACKSMITH_TEST_WITH_DYNAMO=1 test failures
    ...
    def __call__(self, *args, **kwargs):
        if isinstance(self.subgraph, smith.fx.GraphModule):
            # Running graph with interpreter is needed for propagating the stack_trace
            with fx_traceback.preserve_node_meta():
                return self.ctx.functionalize(smith.fx.Interpreter(self.subgraph).run)(
                    *args, **kwargs
                )
...
def materialize_as_graph(
    fn: Callable,
    args: tuple[Any, ...],
    include_key_set: Optional[smith._C.DispatchKeySet] = None,
    exclude_key_set: Optional[smith._C.DispatchKeySet] = None,
    ...
    if include_key_set is None:
        include_key_set = smith._C._dispatch_tls_local_include_set()
    if exclude_key_set is None:
        exclude_key_set = smith._C._dispatch_tls_local_exclude_set()

    @smith._dynamo.disable(recursive=True, reason=None)
    def _materialize_as_graph_inner():
        with suspend_functionalization(), disable_functional_mode():
            ...
            with contextlib.ExitStack() as stack:
                stack.enter_context(
                    smith.utils._python_dispatch._disable_current_modes()
                )
                stack.enter_context(
                    smith._C._ForceDispatchKeyGuard(include_key_set, exclude_key_set),
                )
                if force_enable_grad:
                    stack.enter_context(smith.enable_grad())
...
def materialize_callable_in_args(op: HopInstance, args, kwargs):
    ...
    if not isinstance(schema, smith._C.FunctionSchema) or len(arg_proxies) != len(
        schema.arguments
    ):
        raise AssertionError(
    ...
    for i, proxy in enumerate(arg_proxies):
        if (
            isinstance(proxy, smith.fx.Node)
            and proxy.op == "get_attr"
            and isinstance(getattr(gm, proxy.target), smith.fx.GraphModule)  # type: ignore[arg-type]
        ):
...
def has_user_subclass(args, allowed_subclasses):
    ...
    val = any(
        ...
        and type(a) is not smith.Tensor
...
def _has_gen_schema(op: HigherOrderOperator):
    ...
    if op is smith.ops.higher_order.invoke_quant_packed:
        return False
    method = "gen_schema"
...
def filter_with_masks(data: list[Optional[smith.Tensor]], masks: list[bool]):
...
def fill_none_with_masks(data: list[Optional[smith.Tensor]], masks: list[bool]):
    data_iter = iter(data)
