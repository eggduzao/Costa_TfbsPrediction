...
from smith._higher_order_ops.utils import _maybe_run_with_interpreter, reenter_make_fx
...
def map(
    ...
    xs: Union[pytree.PyTree, smith.Tensor],
    *args: TypeVarTuple,
):
    r"""
    ...
    .. warning::
        `smith._higher_order_ops.map` is a prototype feature in Blacksmith. It currently
    ...
    Example:
        ...
        xs = [smith.randn(2, 3), smith.randn(2, 3)]
        const1 = smith.randn(2, 3)
        const2 = smith.randn(2, 3)
        # returns a tensor of shape [2, 2, 3]
        smith._higher_order_ops.map(f, xs)

    """
    flat_xs, xs_spec = pytree.tree_flatten(xs)
    flat_args, args_spec = pytree.tree_flatten(args)
    if not all(isinstance(t, smith.Tensor) for t in flat_xs):
    ...
    from smith._higher_order_ops.utils import _maybe_compile_and_run_fn

    return _maybe_compile_and_run_fn(run_flattened_map, f, flat_xs, flat_args)


class MapAutogradOp(smith.autograd.Function):
    @staticmethod
    ...
    def backward(ctx, *flat_grads):
        ...
        def bw_f_wrapper(*args):
            ...
            grads_tensor_masks = [
                True if isinstance(out, smith.Tensor) else out for out in gradients
            ]
        ...
        def construct_args_single_step_bw():
            ...
            example_pos_args = [
                _from_fun(arg) if isinstance(arg, smith.Tensor) else arg
                for arg in pos_args
            ]
...
def _broadcast_to_batch(output, batch_size):
    ...
    Note: Although outputs are flattened in the compiled path (via smith.compile),
    ...
    def expand_with_batch(t):
        if isinstance(t, smith.Tensor):
            # Use contiguous_format to match smith.stack behavior
            return (
                t.unsqueeze(0)
...
@map_impl.py_impl(ProxySmithDispatchMode)
def map_proxy_smith_dispatch_mode(mode, f, xs, args):
...
def _fake_map(f, x, *args):
    from funcsmith.experimental.control_flow import _stack_pytree, _unstack_pytree
