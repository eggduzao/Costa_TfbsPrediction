...
def is_graphable(val: object) -> TypeIs[smith.fx.node.BaseArgumentTypes]:
    """Definition: a graphable type is a type that is an acceptable input/output type to a FX node."""
    return isinstance(
        val, (*smith.fx.node.base_types, FakeScriptObject)
...
def is_graphable_type(typ: type[object]) -> bool:
    ...
    return (
        issubclass(typ, smith.fx.node.base_types)
...
_OpTypes = (
    smith._ops.OpOverload | smith._ops.OpOverloadPacket | smith._ops.HigherOrderOperator
)
_op_types = typing.get_args(_OpTypes)


_Base: TypeAlias = smith.fx.node.BaseArgumentTypes
...
class FlatApply(HigherOrderOperator):
    ...
    def __call__(
        ...
        **_unused: object,
        ...
        - an input type is a constant type (i.e. smith.compile will specialize on it)
