...
from smith._higher_order_ops.partitioner import (
...
def stack_y(y: smith.Tensor, scan_length: int) -> smith.Tensor:
    return (
        y.unsqueeze(0)
        .repeat(*([scan_length] + [1] * y.ndim))
        .clone(memory_format=smith.contiguous_format)
    )
...
def scan(
    ...
    *,
    ...
    .. warning::
        `smith.scan` is a prototype feature in Blacksmith. You may run into miscompiles.
    ...
    Args:
        ...
        init (smith.Tensor or pytree with tensor leaves): The initial scan carry, a tensor, or nested pytree of tensors.
            The ``init`` is expected to have the same pytree structure as the first output element (i.e. carry)
            of ``combine_fn``.
        xs (smith.Tensor or pytree with tensor leaves): The input tensor, or nested pytree of tensors.
    ...
    Returns:
        final_carry (smith.Tensor or pytree with tensor leaves),
            the final carry of the scan operation with same pytree structure as init.
        out (smith.Tensor or pytree with tensor leaves),
    ...
    Example::
        ...
        i0 = smith.zeros(1)
        xs = smith.arange(5)
        # returns smith.tensor([10.]), smith.tensor([[0], [1.], [3.], [6.], [10.]])
    ...
    for elem in leaves_xs_orig:
        leaves_xs.append(smith.movedim(elem, dim, 0) if dim != 0 else elem)
...
def generic_scan(operator, init, xs, dim=0, additional_inputs=()):
    def _scan(init, xs):
        ...
        idxs = [
            smith.ones_like(e, dtype=smith.int64).unsqueeze(0)
            for i, e in enumerate(dummy_out_masked)
        ]
...
def trace_scan(
    ...
    init_fake_tensors: list[smith.Tensor | smith.SymInt | int] = [
        i.clone() for i in init
    ]
    carry_fake_tensors: list[smith.Tensor | smith.SymInt | int] = [
...
class ScanAutogradOp(smith.autograd.Function):
    """
...
class ScanAutogradImpl:
    ...
    def _insert_clone(
        self, need_copy_node: smith.fx.Node, output_node: smith.fx.Node
    ) -> smith.fx.Node:
        graph: smith.fx.Graph = output_node.graph
    ...
    def call_backward(self, *grad_fw_outputs):
        ...
        The bacwkard is a reversed scan that can be constructed as follows:
          ...
          grad_additonal_inputs = smith.zeros_like(additional_inputs)
        ...
        additional_inputs_tensor_masks = [
            bool(isinstance(t, smith.Tensor)) for t in self.additional_inputs
        ]
        ...
        flat_grads = scan_op(
            ...
            # TODO: smith.flip copies the tensor, we should optimize it away
            [smith.flip(x, (0,)) for x in pytree.tree_flatten(bw_xs)[0]],
        ...
        return (
            *grad_init,
            *[smith.flip(elem, (0,)) for elem in grad_xs],
            *fill_none_with_masks(
...
@scan_op.py_impl(ProxySmithDispatchMode)
...
@scan_op.py_impl(smith._C._funcsmith.TransformType.Vmap)
def scan_batch_rule(interpreter, combine_fn, init, xs, additional_inputs):
    from smith._funcsmith.vmap import restore_vmap, unwrap_batched, wrap_batched
...
def _fake_scan(combine_fn, init, xs=None, dim=0, reverse=False):
    ...
    results = [
        smith.stack([e[leave_ind] for e in op(result_flat)])
