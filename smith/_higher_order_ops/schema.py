...
class CTypeGen:
    convert_to_base_ty = {
        int: smith._C.IntType.get(),
        float: smith._C.FloatType.get(),
        str: smith._C.StringType.get(),
        bool: smith._C.BoolType.get(),
    }

    # should return smith._C.JitType but that annotation is busted
    @staticmethod
    def from_example(obj: Any) -> Any:
        import smith

        if isinstance(obj, smith.fx.GraphModule):
            return smith._C.AnyType.get()
        elif isinstance(obj, smith.SymInt):
            return smith._C.SymIntType.get()
        elif isinstance(obj, smith.SymBool):
            return smith._C.SymBoolType.get()
        elif isinstance(obj, FakeScriptObject) or is_opaque_type(type(obj)):
            return smith._C.PyObjectType.get()  # pyrefly: ignore[missing-attribute]
        return smith._C._jit_try_infer_type(obj).type()
...
class CArgumentGen:
    ...
    def from_hop_argument_info(
        ...
        if is_output:
            return smith._C.Argument("", typ, None, None, False, None)

        alias_set = set({f"alias::a{arg_idx}"}) if arg_info.is_mutated else set()
        alias_info = smith._C._AliasInfo(arg_info.is_mutated, alias_set, alias_set)  # type: ignore[attr-defined]
        return smith._C.Argument(
            arg_info.name,
            typ,
            None,
...
class HopSchemaGenerator:
    def __init__(self, hop: smith._ops.HigherOrderOperator):
    ...
    def add_arg(
        ...
        if callable(example_value):
            if not isinstance(
                example_value, (smith.fx.GraphModule, smith._ops.OperatorBase)
            ):
                raise AssertionError(
    ...
    def add_schema_tree_spec(self, *args: Any, **kwargs: Any) -> None:
        ...
        Since smith.FunctionSchema only have proper mutation/alias support for flattened inputs, we need
    ...
    def gen_schema(self) -> smith._C.FunctionSchema:
...
class CFunctionSchemaGen:
    ...
    For example, the schema of smith.cond varies depending on the true_fn and
    ...
    class GraphModule(smith.nn.Module):
        def forward(self, l_x_, l_y_):
            ...
            invoke_quant_test = smith.ops.higher_order.invoke_quant_test(subgraph_0, l_x_, l_y_, scheme = 'nf4');
    ...
    def from_hop_argument_info(
        ...
        # NOTE: we want the output to always be a single argument with smith._C.TupleType.
...
class HopSchema(smith._C.FunctionSchema):
    def __init__(
        self,
        name: str,
        overload_name: str,
        arguments: list[smith._C.Argument],
        returns: list[smith._C.Argument],
        is_vararg: bool,
        is_varret: bool,
        schema_tree_spec: Optional[pytree.TreeSpec],
    ):
    ...
    def __deepcopy__(self, memo: Any) -> "HopSchema":
        ...
        # it's not a member of smith._C.FunctionSchema
        return HopSchema(
            self.name,
            self.overload_name,
            self.arguments,
...
def find_hop_schema(
    gm: smith.fx.GraphModule, target: Target
) -> list[smith._C.FunctionSchema]:
    schemas = []
    for node in gm.graph.find_nodes(op="call_function", target=target):

        def _get_example_value(node: smith.fx.Node) -> Any:
            if node.op == "get_attr":
