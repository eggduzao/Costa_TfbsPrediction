...
from smith._higher_order_ops.utils import register_fake
from smith._ops import HigherOrderOperator
from smith.autograd.graph import get_gradient_edge
from smith.fx.experimental.proxy_tensor import ProxySmithDispatchMode, track_tensor_tree
from smith.nn.utils.stateless import _reparametrize_module
...
class LeafModuleState(NamedTuple):
    ...
    nn_module_index: int
    named_parameters: dict[str, smith.nn.Parameter]
...
class GradientInfo:
    ...
    edge: smith.autograd.graph.GradientEdge
    size: smith.Size
    stride: tuple[int, ...]
    dtype: smith.dtype
    device: smith.device
...
def _retrieve_module_by_index(nn_module_index: int) -> smith.nn.Module:
...
def check_escaped_gradients(
    ...
    Controlled by smith._dynamo.config.leaf_function_check_escaped_gradients.
    """
    ...
    input_nodes: set[smith.autograd.graph.Node] = set()
    for i, inp in enumerate(inputs):
        if (
            isinstance(inp, smith.Tensor)
    ...
    start_nodes: set[smith.autograd.graph.Node] = {
        out.grad_fn
    ...
    escaped: set[smith.autograd.graph.Node] = set()
...
def reconstruct_original_args(
    ...
    with contextlib.ExitStack() as stack:
        ...
        def process_module_state(state: LeafModuleState) -> smith.nn.Module:
...
def autograd_grad_with_gradient_info(
    ...
    # write fake_impl by returning smith.empty_like(input) to get the right shape
...
def _make_forward(
    ...
    def forward(*flat_args):
        ...
        flat_args = [
            arg.detach() if isinstance(arg, smith.Tensor) else arg for arg in flat_args
        ]
        ...
        inputs = [
            ...
            if isinstance(arg, smith.Tensor) and idx in requires_grad_indices
            else arg
        ...
        with smith._C._ForceDispatchKeyGuard(effective_keys, exclude_keys):
...
class InvokeLeafFunctionAutogradOp(smith.autograd.Function):
    @staticmethod
    # pyrefly: ignore [bad-override]
    def forward(ctx, real_fn_spec, fake_fn_spec, *flat_args):
        real_fn = unwrap_fn_spec(real_fn_spec)

        include_keys = smith._C._dispatch_tls_local_include_set()
        exclude_keys = smith._C._dispatch_tls_local_exclude_set()
...
@invoke_leaf_function.py_impl(ProxySmithDispatchMode)
...
def _validate_outputs_match(
    ...
    for i, (fake_val, real_val) in enumerate(zip(fake_flat, real_flat)):
        fake_is_tensor = isinstance(fake_val, smith.Tensor)
        real_is_tensor = isinstance(real_val, smith.Tensor)
...
def invoke_leaf_function_dense(real_fn_spec, fake_fn_spec, *flat_args):
    ...
    version_before = [
        arg._version if isinstance(arg, smith.Tensor) else 0 for arg in flat_args
    ]
