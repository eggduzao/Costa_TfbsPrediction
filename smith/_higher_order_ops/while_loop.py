...
class WhileLoopOp(HigherOrderOperator):
    ...
    def __call__(
        ...
        cond_fn: Callable,
        body_fn: Callable,
        carried_inputs: tuple[Union[smith.Tensor, int, float, bool]],
        additional_inputs: tuple[Union[smith.Tensor, smith.SymInt, int], ...],
        /,
    ):
    ...
    def gen_schema(self, cond_fn, body_fn, carried_inputs, additional_inputs):
        ...
        cond_gm: smith.fx.GraphModule = (
            cond_fn
            if isinstance(cond_fn, smith.fx.GraphModule)
            else materialize_as_graph(cond_fn, all_inputs)
        )
        body_gm: smith.fx.GraphModule = (
            body_fn
            if isinstance(body_fn, smith.fx.GraphModule)
        ...
        def _find_example_value(n, real_inp):
            if "val" in n.meta:
                ...
                if isinstance(real_inp, smith.Tensor):
...
def while_loop(cond_fn, body_fn, carried_inputs):
    ...
    .. warning::
        `smith.while_loop` is a prototype feature in Blacksmith. It has limited support for input and output types and
    ...
    `while_loop` is a structured control flow operator. It preserves the loop semantic across the smith.compile and smith.export.
    ...
    Example 1:
        ...
        while_loop(cond_fn, body_fn, (smith.zeros(1), smith.randn(3, 4)))
    ...
    Example 2:
        ...
        while_loop(cond,_fn, body_fn, (0, smith.randn(3, 4)))
    ...
    def _validate_input(cond_fn, body_fn, carried_inputs):
        from smith._higher_order_ops.utils import validate_subgraph_args_types
        ...
        if not pytree.tree_all(
            lambda t: isinstance(t, (smith.Tensor, smith.SymInt, int)), carried_inputs
        ):
            raise RuntimeError(
    ...
    with setup_compilation_env() as backend:
        return smith.compile(_while_loop_op_wrapper, backend=backend, fullgraph=True)(
...
def while_loop_dense(
    ...
    def _validate_cond_output(pred):
        if (
            isinstance(pred, smith.Tensor)
            and pred.size() == smith.Size([])
            and pred.dtype == smith.bool
    ...
    if not should_loop:
        if stack_output:
            return tuple(
                val.unsqueeze(0).clone() if isinstance(val, smith.Tensor) else val
                for val in carried_vals
            )
        else:
            return tuple(
                val.clone() if isinstance(val, smith.Tensor) else val
                for val in carried_vals
            )

    outputs: list[list[smith.Tensor]] = [[] for _ in carried_vals]
    ...
    if stack_output:
        outs: list[smith.Tensor] = []
        for out in outputs:
            outs.append(smith.stack(out, dim=0))
        return tuple(outs)
...
def _find_or_create_fake_mode() -> FakeTensorMode:
    ...
    fake_mode = smith._guards.detect_fake_mode()
...
@while_loop_op.py_impl(ProxySmithDispatchMode)
def while_loop_tracing(
    ...
    def _trace_while_loop(
        ...
        def _unspecialize_carried_inputs(x):
            ...
            # Here's the problem: when a user creates a constant tensor e.g. smith.tensor(0), Blacksmith calls aten.lift_fresh_copy
        ...
        with disable_proxy_modes_tracing():
            ...
            def produce_graph(fn):
                cloned_carried_inputs = pytree.tree_map_only(
                    smith.Tensor, lambda x: x.clone(), unspecialized_carried_inputs
                )
...
def while_loop_fake_tensor_mode(
    ...
    with mode:
        ...
        if stack_output:
            ...
            if not all(isinstance(x, smith.Tensor) for x in carried_inputs):
                raise AssertionError(
...
# respect to x. Note that bw can be computed from forward body_fn easily using smith.autograd.grad.
...
# init_grad_additional_inputs = smith.zeros_like(g_additional_inputs)
# fw_inps = smith.cat([ctx.fw_carried_inputs, fw_outputs[:-1]])
# while_loop(cond_fn, body_fn, (idx, init_grad_carries, init_grad_additional_inputs,), (fw_additional_inputs, fw_inps))


class WhileLoopAutogradOp(smith.autograd.Function):
    @staticmethod
    # pyrefly: ignore [bad-override]
    def forward(
        ctx,
        ...
        *carries_and_inputs,
        ...
        from smith._higher_order_ops.scan import split_into_chunks
        ...
        if (
            isinstance(loop_count, smith.SymInt)
    ...
    def backward(ctx, *grads):
        from smith._higher_order_ops.cond import create_bw_fn
        from smith._higher_order_ops.scan import split_into_chunks
        ...
        carries_tensor_masks = [
            bool(isinstance(t, smith.Tensor) and t.dtype.is_floating_point)
        ...
        init_idx = smith.zeros((), dtype=smith.int64)
        ...
        fw_carries = [
            smith.cat([carry.unsqueeze(0), carries[:-1]])
        ...
        def cond_fn(*flat_args):
            ...
            if not isinstance(fw_carries[0], smith.Tensor):
                raise AssertionError(
                    f"expected fw_carries[0] to be smith.Tensor, got {type(fw_carries[0])}"
                )
        ...
        def body_fn(*flat_args):
            ...
            if not all(isinstance(t, smith.Tensor) for t in cur_grad_carries):
                raise AssertionError(
...
while_loop_stack_output_op.py_impl(ProxySmithDispatchMode)(
