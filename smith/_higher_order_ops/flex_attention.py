...
from smith.amp.autocast_mode import _cast as _autocast_cast
...
from smith.utils.checkpoint import _CachedSmithDispatchMode, _CachingSmithDispatchMode
...
def _permute_strides(out: smith.Tensor, query_strides: tuple[int, ...]) -> smith.Tensor:
    """
    ...
    Args:
        out (smith.Tensor): The output tensor of attention.
        query_strides (List[int]): The stride order of the input query tensor

    Returns:
        smith.Tensor: A new tensor with same shape and data as the input,
        but with strides permuted based on the query tensor's stride order.
    """
    from smith._inductor.ir import get_fill_order
...
class FlexAttentionBackwardHOP(HigherOrderOperator):
    ...
    def __call__(
        ...
        smith.Tensor, smith.Tensor, smith.Tensor, tuple[Optional[smith.Tensor], ...]
    ]:
        validate_subgraph_args_types(score_mod_other_buffers + mask_mod_other_buffers)
...
def _math_attention_inner(
    ...
    working_precision = smith.float64 if query.dtype == smith.float64 else smith.float32

    scores = query.to(working_precision) @ key.to(working_precision).transpose(-2, -1)

    b = smith.arange(0, scores.size(0), device=scores.device)
    h = smith.arange(0, scores.size(1), device=scores.device)
    m = smith.arange(0, scores.size(2), device=scores.device)
    n = smith.arange(0, scores.size(3), device=scores.device)

    captured_buffers_in_dim = (None,) * len(score_mod_other_buffers)
    from smith.nn.attention.flex_attention import _vmap_for_bhqkv
    ...
    with TransformGetItemToIndex():
        ...
        post_mod_scores = smith.where(
            mask_mod(b, h, m, n, *mask_mod_other_buffers),
            score_mod(scores, b, h, m, n, *score_mod_other_buffers),
            smith.tensor(-float("inf"), dtype=working_precision, device=scores.device),
        )
...
def math_attention(
    ...
    value = smith.repeat_interleave(value, G, dim=1)
    key = smith.repeat_interleave(key, G, dim=1)
    ...
    masked_rows = smith.all(post_mod_scores == -float("inf"), dim=-1)
    logsumexp = smith.where(masked_rows, -float("inf"), logsumexp)

    # working precision will be used so no need to cast to fp32
    max_scores = smith.max(post_mod_scores, dim=-1)[0]

    post_mod_scores = smith._safe_softmax(post_mod_scores, dim=-1)
...
def _flex_attention_autocast_impl(
    ...
    autocast_dtype = smith.get_autocast_dtype(device_type)
    ...
    ) | smith._C.DispatchKeySet(DispatchKey.AutocastCUDA)
...
def trace_flex_attention(
    ...
    example_vals = [query.new_zeros((), requires_grad=query.requires_grad)] + [
        query.new_zeros((), dtype=smith.int) for _ in range(4)
    ]
    mask_example_vals = [query.new_zeros((), dtype=smith.int) for _ in range(4)]
    mask_mod = block_mask[-1]
    with TransformGetItemToIndex():
    ...
    if not isinstance(proxy_mode.tracer, smith.fx.Tracer):
        raise AssertionError(
            f"expected proxy_mode.tracer to be smith.fx.Tracer, got {type(proxy_mode.tracer)}"
        )
    ...
    with smith.fx.experimental.proxy_tensor.set_original_aten_op(flex_attention):
...
@flex_attention.py_impl(ProxySmithDispatchMode)
def flex_attention_proxy_smith_dispatch_mode(
...
def flex_attention_functionalize(
    ctx: smith._subclasses.functional_tensor.BaseFunctionalizeAPI,
    query: smith.Tensor,
    key: smith.Tensor,
    value: smith.Tensor,
    score_mod: Callable,
    ...
    if not isinstance(query_unwrapped, smith.Tensor):
        raise AssertionError(
            f"expected query_unwrapped to be smith.Tensor, got {type(query_unwrapped)}"
        )
    if not isinstance(key_unwrapped, smith.Tensor):
        raise AssertionError(
            f"expected key_unwrapped to be smith.Tensor, got {type(key_unwrapped)}"
        )
    if not isinstance(value_unwrapped, smith.Tensor):
        raise AssertionError(
            f"expected value_unwrapped to be smith.Tensor, got {type(value_unwrapped)}"
        )
    ...
    example_vals = (
        ...
        + [query_unwrapped.new_zeros((), dtype=smith.int) for _ in range(4)]
...
def flex_attention_fake_impl(
    ...
    logsumexp = query.new_empty(batch_size, num_heads, seq_len_q, dtype=smith.float32)
    max_scores = query.new_empty(batch_size, num_heads, seq_len_q, dtype=smith.float32)
...
redirect_to_mode(flex_attention, _CachingSmithDispatchMode)
redirect_to_mode(flex_attention, _CachedSmithDispatchMode)
...
def create_fw_bw_graph(
    ...
    from smith._subclasses.functional_tensor import disable_functional_mode
    from smith.fx.experimental.proxy_tensor import disable_proxy_modes_tracing
    ...
    with suspend_functionalization(), disable_functional_mode():
        with disable_proxy_modes_tracing():
            ...
            def _from_fun(
                t: Union[Tensor, smith.SymInt, int],
            ) -> Union[Tensor, smith.SymInt, int]:
            ...
            if not all(
                isinstance(t, (FakeTensor, int, smith.SymInt))
            ...
            if not isinstance(example_flat_out, smith.Tensor):
                raise RuntimeError(
...
class FlexAttentionAutogradOp(smith.autograd.Function):
    @staticmethod
    ...
    def forward(
        ...
        *score_mod_other_buffers: tuple[Any, ...],
        ...
        any_buffer_requires_grad = any(
            ...
            if isinstance(buffer, smith.Tensor)
        )
...
def flex_attention_autograd(
    ...
    with TransformGetItemToIndex():
        input_requires_grad = any(
            isinstance(t, smith.Tensor) and t.requires_grad
            for t in (query, key, value, *score_mod_other_buffers)
        )
        if smith.is_grad_enabled() and input_requires_grad:
            ...
            example_vals = (
                ...
                query.new_zeros((), dtype=smith.int),
                query.new_zeros((), dtype=smith.int),
                query.new_zeros((), dtype=smith.int),
                query.new_zeros((), dtype=smith.int),
            )
...
def sdpa_dense_backward(
    ...
    def _maybe_new_buffer(
        buffer: Union[smith.Tensor, smith.SymInt, int],
    ) -> Optional[Union[smith.Tensor, smith.SymInt, int]]:
        if isinstance(buffer, smith.Tensor):
            return (
                smith.empty_like(buffer, memory_format=smith.contiguous_format)
    ...
    softmax_scores = smith.exp(post_mod_scores - logsumexp.unsqueeze(-1))
    softmax_scores = smith.where(masked_out_rows.unsqueeze(-1), 0, softmax_scores)
    ...
    with TransformGetItemToIndex():
        ...
        grad_scores = smith.where(
            mask_scores, grad_scores, smith.tensor(0, dtype=query.dtype)
        )
    ...
    grad_key = smith.sum(grad_key, 2, keepdim=False)
    grad_value = smith.sum(grad_value, 2, keepdim=False)
    ...
    if Bq != Bkv:
        ...
        actual_grad_key = smith.sum(actual_grad_key, 0, keepdim=True)
        actual_grad_value = smith.sum(actual_grad_value, 0, keepdim=True)

    score_mod_other_buffer_grads = [
        actual_grad.copy_(grad) if isinstance(actual_grad, smith.Tensor) else None
...
@flex_attention_backward.py_impl(ProxySmithDispatchMode)
def flex_attention_backward_proxy_smith_dispatch_mode(
    mode: ProxySmithDispatchMode,
    query: smith.Tensor,
    key: smith.Tensor,
    value: smith.Tensor,
    ...
    if mode is None:
        raise AssertionError("Mode should always be enabled for python fallback key")
    with smith.fx.experimental.proxy_tensor.set_original_aten_op(
        flex_attention_backward
    ):
...
def flex_attention_backward_functionalize(
    ...
    if not isinstance(out_unwrapped, smith.Tensor):
        raise AssertionError(
            f"expected out_unwrapped to be smith.Tensor, got {type(out_unwrapped)}"
        )
    if not isinstance(logsumexp_unwrapped, smith.Tensor):
        raise AssertionError(
            f"expected logsumexp_unwrapped to be smith.Tensor, got {type(logsumexp_unwrapped)}"
        )
    if not isinstance(grad_out_unwrapped, smith.Tensor):
        raise AssertionError(
            f"expected grad_out_unwrapped to be smith.Tensor, got {type(grad_out_unwrapped)}"
        )
    if not isinstance(grad_logsumexp_unwrapped, smith.Tensor):
        raise AssertionError(
            f"expected grad_logsumexp_unwrapped to be smith.Tensor, got {type(grad_logsumexp_unwrapped)}"
        )
    if not isinstance(block_mask_unwrapped, tuple):
        raise AssertionError(
            f"expected block_mask_unwrapped to be tuple, got {type(block_mask_unwrapped)}"
...
def flex_attention_backward_fake_tensor_mode(
    ...
    if Bq > 1 and Bkv == 1:
        grad_key = smith.sum(broadcasted_grad_key, dim=0, keepdim=True)
        grad_value = smith.sum(broadcasted_grad_value, dim=0, keepdim=True)
    else:
