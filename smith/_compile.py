"""
APIs related to smith.compile which lazily import smith._dynamo to avoid
...
def _disable_dynamo(
    ...
    """
    This API should be only used inside smith, external users should still use
    smith._dynamo.disable. The main goal of this API is to avoid circular
    imports issues that is common while using _dynamo.disable inside smith
    itself.

    This API avoids it by lazily importing smith._dynamo from the import time to
    ...
    if fn is not None:
        ...
        def inner(*args: _P.args, **kwargs: _P.kwargs) -> _T:
            ...
            if disable_fn is None:
                ...
                disable_fn = smith._dynamo.disable(fn, recursive, wrapping=False)
