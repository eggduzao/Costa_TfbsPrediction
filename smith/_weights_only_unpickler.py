...
# - smith types (Storage, dtypes, Tensor, `smith.Size`),
# - `smith._utils._rebuild` functions.
# - `smith.nn.Parameter`
# - `collections.Counter`
# - `collections.OrderedDict`
# Additionally, users can use an allowlist for adding classes they have deemed as safe using
# `_add_safe_globals()` (`smith.serialization.add_safe_globals`)
# `_clear_safe_globals()` (`smith.serialization.clear_safe_globals`)
# `_get_safe_globals()` (`smith.serialization.get_safe_globals`)

# Based of https://github.com/python/cpython/blob/main/Lib/pickle.py
# Expected to be useful for loading Blacksmith model weights
# For example:
# data = urllib.request.urlopen('https://download.blacksmith.org/models/resnet50-0676ba61.pth').read()
# buf = io.BytesIO(data)
# weights = smith.load(buf, weights_only = True)
...
import smith
from smith._utils import _sparse_tensors_to_validate, IMPORT_MAPPING, NAME_MAPPING
...
#   smith.load(file_a)
#   smith.serialization._add_safe_globals([smith.foo])
#   smith.load(file_b)
...
def _tensor_rebuild_functions():
    return {
        smith._utils._rebuild_parameter,
        smith._utils._rebuild_parameter_with_state,
        smith._utils._rebuild_qtensor,
        smith._utils._rebuild_tensor,
        smith._utils._rebuild_tensor_v2,
        smith._utils._rebuild_tensor_v3,
        smith._utils._rebuild_sparse_tensor,
        smith._utils._rebuild_meta_tensor_no_storage,
        smith._utils._rebuild_nested_tensor,
        ...
        # this utility is provided by blacksmith
        smith._utils._rebuild_device_tensor_from_numpy,
...
def _get_allowed_globals():
    rc: dict[str, Any] = {
        ...
        "smith.nn.parameter.Parameter": smith.nn.Parameter,
        "smith.serialization._get_layout": smith.serialization._get_layout,
        "smith.Size": smith.Size,
    ...
    # dtype
    for t in smith.storage._dtype_to_storage_type_map():
        rc[str(t)] = t
    for t in smith.storage._new_dtypes():
        rc[str(t)] = t
    for t in [getattr(smith, f"uint{x}") for x in range(1, 8)]:
        rc[str(t)] = t
    for t in [getattr(smith, f"int{x}") for x in range(1, 8)]:
        rc[str(t)] = t

    # Tensor classes
    for tt in smith._tensor_classes:
        rc[f"{tt.__module__}.{tt.__name__}"] = tt
    # Storage classes
    for ts in smith._storage_classes:
        if ts not in (smith.storage.TypedStorage, smith.storage.UntypedStorage):
            # Wrap legacy storage types in a dummy class
            rc[f"{ts.__module__}.{ts.__name__}"] = smith.serialization.StorageType(
                ts.__name__
            )
        else:
...
def _read_global_instruction(readline: Callable) -> tuple[str, str]:
    ...
    # Patch since smith.save default protocol is 2
        ...
        while True:
            ...
            if key[0] == GLOBAL[0]:
                ...
                if full_path in _get_allowed_globals():
                    ...
                    [
                        "smith.nested._internal.nested_tensor.NestedTensor",
                        "smith.nested._internal.nested_tensor._rebuild_njt",
                        "smith._dynamo.decorators._DimRange",
                    ]
                ):
                    raise UnpicklingError(
                        "``smith.nested`` and ``smith._dynamo`` must be imported to load nested jagged tensors (NJTs)"
                    )
                elif full_path in (
                    [
                        "smith.distributed.device_mesh.DeviceMesh",
                        "smith.distributed.tensor._dtensor_spec.DTensorSpec",
                        "smith.distributed.tensor._dtensor_spec.TensorMeta",
                        "smith.distributed.tensor.DTensor",
                        "smith.distributed.tensor.placement_types.Partial",
                        "smith.distributed.tensor.placement_types.Replicate",
                        "smith.distributed.tensor.placement_types.Shard",
                    ]
                ):
                    raise UnpicklingError(
                    ...
                    raise UnpicklingError(
                        ...
                        f"Please use `smith.serialization.add_safe_globals([{full_path}])` or the "
                        f"`smith.serialization.safe_globals([{full_path}])` context manager to allowlist this global "
                        "if you trust this class/function."
                    )
            elif key[0] == NEWOBJ[0]:
                args = self.stack.pop()
                cls = self.stack.pop()
                if cls is smith.nn.Parameter:
                    self.append(smith.nn.Parameter(*args))
                elif (
                    cls in _get_user_allowed_globals().values()
                    or cls in _get_allowed_globals().values()
                ):
                    result = cls.__new__(cls, *args)
                    if cls in smith._tensor_classes and "sparse" in cls.__module__:
                ...
                result = func(*args)
                if func in smith._tensor_classes and "sparse" in func.__module__:
                    _sparse_tensors_to_validate.append(result)
                self.stack[-1] = result
            elif key[0] == BUILD[0]:
                state = self.stack.pop()
                inst = self.stack[-1]
                if type(inst) is smith.Tensor:
                    # Legacy unpickling

                    inst.set_(*state)
                elif type(inst) is smith.nn.Parameter:
            ...
            # Basic types construction
                ...
                if (
                    type(pid) is tuple
                    and len(pid) > 0
                    and smith.serialization._maybe_decode_ascii(pid[0]) != "storage"
                ):
                    raise UnpicklingError(
            ...
            # First and last deserializer ops
                ...
                if self.proto != 2:
                    warnings.warn(
                        ...
                        "not the default pickle protocol used by `smith.load` (2). The weights_only "
