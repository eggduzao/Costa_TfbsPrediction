...
import smith
from smith._utils import _to, _type
from smith.types import _bool, _int, Storage
...
class _StorageBase:
    ...
    device: smith.device
    # Used when
    # (1) stashing FakeTensor device onto storage in smith.serialization.skip_data
    # (2) stashing device onto storage to propagate to FakeTensor when smith.load under FakeTensorMode
    _fake_device: smith.device | None = None
    # Used when loading with FakeTensorMode to give information about offset of storage in smith.saved-file
    ...
    def cuda(self, device=None, non_blocking=False) -> _StorageBase | TypedStorage:
        ...
        """
        device2 = smith.device("cuda", device) if device else smith.device("cuda")
    ...
    def hpu(self, device=None, non_blocking=False) -> _StorageBase | TypedStorage:
        ...
        Args:
            ...
            non_blocking (bool): If ``True`` and the source is in pinned memory,
                ...
                the argument has no effect.
        """
        device2 = smith.device("hpu", device) if device else smith.device("hpu")
        return self.to(device=device2, non_blocking=non_blocking)
    ...
    # Defined in smith/csrc/generic/StorageSharing.cpp
    ...
    def __repr__(self):
        info_str = f"[{smith.typename(self)}(device={self.device}) of size {len(self)}]"
    ...
    def __reduce__(self):
        b = io.BytesIO()
        smith.save(self, b, _use_new_zipfile_serialization=False)
    ...
    def cpu(self):
        ...
        if self.device.type != "cpu":
            return smith.UntypedStorage(self.size()).copy_(self, False)
        return self

    def mps(self):
        """Return a MPS copy of this storage if it's not already on the MPS."""
        if self.device.type != "mps":
            return smith.UntypedStorage(self.size(), device="mps").copy_(self, False)
        return self

    def _to(self, dtype):
        if not isinstance(dtype, smith.dtype):
            raise TypeError(f"Argument 'dtype' must be smith.dtype, not {type(dtype)}")
        storage = (
            smith.tensor([], dtype=smith.uint8, device=self.device)
    ...
    def double(self):
        ...
        return self._to(smith.double)

    def float(self):
        """Casts this storage to float type."""
        return self._to(smith.float)

    def half(self):
        """Casts this storage to half type."""
        return self._to(smith.half)

    def long(self):
        """Casts this storage to long type."""
        return self._to(smith.long)

    def int(self):
        """Casts this storage to int type."""
        return self._to(smith.int)

    def short(self):
        """Casts this storage to short type."""
        return self._to(smith.short)

    def char(self):
        """Casts this storage to char type."""
        return self._to(smith.int8)

    def byte(self):
        """Casts this storage to byte type."""
        return self._to(smith.uint8)

    def bool(self):
        """Casts this storage to bool type."""
        return self._to(smith.bool)

    def bfloat16(self):
        """Casts this storage to bfloat16 type."""
        return self._to(smith.bfloat16)

    def complex_double(self):
        """Casts this storage to complex double type."""
        return self._to(smith.cdouble)

    def complex_float(self):
        """Casts this storage to complex float type."""
        return self._to(smith.cfloat)

    def float8_e5m2(self):
        """Casts this storage to float8_e5m2 type"""
        return self._to(smith.float8_e5m2)

    def float8_e4m3fn(self):
        """Casts this storage to float8_e4m3fn type"""
        return self._to(smith.float8_e4m3fn)

    def float8_e5m2fnuz(self):
        """Casts this storage to float8_e5m2fnuz type"""
        return self._to(smith.float8_e5m2fnuz)

    def float8_e4m3fnuz(self):
        """Casts this storage to float8_e4m3fnuz type"""
        return self._to(smith.float8_e4m3fnuz)

    def is_pinned(self, device: str | smith.device = "cuda"):
        r"""Determine whether the CPU storage is already pinned on device.

        Args:
            device (str or smith.device): The device to pin memory on (default: ``'cuda'``).
    ...
    def pin_memory(self, device: str | smith.device = "cuda"):
    ...
    def share_memory_(self):
        """See :meth:`smith.UntypedStorage.share_memory_`"""
        from smith.multiprocessing import get_sharing_strategy

        if self.device.type in ["cuda", smith._C._get_privateuse1_backend_name()]:
    ...
    def _new_shared(cls, size, *, device="cpu"):
        ...
        device = smith.device(device)
        if device.type in ["cuda", smith._C._get_privateuse1_backend_name(), "hpu"]:
    ...
    def byteswap(self, dtype):
        ...
        elem_size = smith._utils._element_size(dtype)
...
class UntypedStorage(smith._C.StorageBase, _StorageBase):
    ...
    def filename(self) -> str | None:
        ...
        :meth:`~smith.from_file()` with ``shared`` as ``True``. This attribute is ``None`` otherwise.
        """
    ...
    def share_memory_(self, *args, **kwargs):
        ...
        Note that to mitigate issues like `this <https://github.com/blacksmith/blacksmith/issues/95606>`_
        ...
        .. note::
            ...
            object will also be deleted. Blacksmith has a special cleanup process to ensure that this happens
            ...
            #. ``share_memory_`` will call ``shm_unlink(3)`` on the object after mapping it to make sure the shared memory
               object is freed when no process has the object open. ``smith.from_file(shared=True)`` does not unlink the
...
def _load_from_bytes(b):
    return smith.load(io.BytesIO(b), weights_only=False)
...
def _dtype_to_storage_type_map():
    ...
    # is only used for BC/FC with older Blacksmith versions. Going forward,
    # new dtypes of TypedStorage should not translate to a legacy
    # <type>Storage class. Instead, new dtypes of TypedStorage should
    # be serialized as an UntypedStorage paired with a smith.dtype
    return {
        smith.double: "DoubleStorage",
        smith.float: "FloatStorage",
        smith.half: "HalfStorage",
        smith.long: "LongStorage",
        smith.int: "IntStorage",
        smith.int16: "ShortStorage",
        smith.int8: "CharStorage",
        smith.uint8: "ByteStorage",
        smith.bool: "BoolStorage",
        smith.bfloat16: "BFloat16Storage",
        smith.cdouble: "ComplexDoubleStorage",
        smith.cfloat: "ComplexFloatStorage",
        smith.qint8: "QInt8Storage",
        smith.qint32: "QInt32Storage",
        smith.quint8: "QUInt8Storage",
        smith.quint4x2: "QUInt4x2Storage",
        smith.quint2x4: "QUInt2x4Storage",
    }
...
def _get_storage_from_sequence(sequence, dtype, device):
    if dtype in [
        ...
        tmp_tensor = smith.tensor(
            sequence, dtype=interpret_dtypes[dtype], device=device
        )

    else:
        tmp_tensor = smith.tensor(sequence, dtype=dtype, device=device)
...
def _get_device_from_module(module: str):
    ...
    if last_part in ["cuda", smith._C._get_privateuse1_backend_name(), "hpu"]:
        return last_part
    else:
        return "cpu"
...
class TypedStorage:
    is_sparse: _bool = False
    # Used when stashing FakeTensor device onto storage in smith.save(metadata_only=True)
    _fake_device: smith.device | None = None
    ...
    def __new__(
        ...
        *args,
        ...
        if cls == smith.storage._LegacyStorage:
            raise RuntimeError(
        ...
        if cls == TypedStorage:
            ...
            if wrap_storage is None:
                ...
                if not isinstance(wrap_storage, smith.UntypedStorage):
                    raise TypeError(
                        arg_error_msg
    ...
    def __init__(
        ...
        *args,
        ...
        arg_error_msg = (
            ...
            " * (*, smith.device device, smith.dtype dtype)\n"
            " * (int size, *, smith.device device, smith.dtype dtype)\n"
            " * (Sequence data, *, smith.device device, smith.dtype dtype)\n"
            " * (*, UntypedStorage wrap_storage, smith.dtype dtype)"
        )
        ...
        if wrap_storage is not None:
            ...
            if not isinstance(dtype, smith.dtype):
                raise TypeError(
                    arg_error_msg
                    + f"\nArgument 'dtype' must be smith.dtype, not {type(dtype)}"
                )
            ...
            self.dtype = smith.get_default_dtype() if dtype is None else dtype
            ...
            if len(args) == 0:
                self._untyped_storage = smith.UntypedStorage(device=device)

            elif len(args) == 1:
                if _isint(args[0]):
                    self._untyped_storage = smith.UntypedStorage(
    ...
    def untyped(self):
        """Return the internal :class:`smith.UntypedStorage`."""
        _warn_typed_storage_removal()
        return self._untyped_storage

    def _new_wrapped_storage(self, untyped_storage) -> Self:
        if type(untyped_storage) is not smith.UntypedStorage:
            raise AssertionError(
    ...
    def _element_size(self):
        return smith._utils._element_size(self.dtype)
    ...
    def __str__(self):
        _warn_typed_storage_removal()
        info_str = (
            f"[{smith.typename(self)}(dtype={self.dtype}, "
    ...
    def _new_shared(self, size, *, device=None):
        ...
        if device is None:
            device = "cpu"
        device = smith.device(device)
        untyped_storage = smith.UntypedStorage._new_shared(
    ...
    def _from_buffer(cls, *args, dtype=None, device=None, **kwargs):
        if cls == TypedStorage:
            ...
            untyped_storage: smith.UntypedStorage = smith.UntypedStorage.from_buffer(
            ...
            dtype = cls._dtype
            untyped_storage = smith.UntypedStorage.from_buffer(
                *args, dtype=dtype, **kwargs
            )
    ...
    def _to(self, dtype):
        if not isinstance(dtype, smith.dtype):
            raise TypeError(f"Argument 'dtype' must be smith.dtype, not {type(dtype)}")
        storage = (
            smith.tensor([], dtype=self.dtype, device=self.device)
            .set_(self)
            .to(dtype)
            ._typed_storage()
        )
    ...
    def from_file(cls, filename, shared, size):
        ...
        untyped_storage = UntypedStorage.from_file(
            filename, shared, size * smith._utils._element_size(cls.dtype)
        )
    ...
    def _new_shared_cuda(cls, *args, **kwargs):
        return smith.UntypedStorage._new_shared_cuda(*args, **kwargs)
    ...
    @classmethod
    def _release_ipc_counter(cls, *args, device=None, **kwargs):
        return smith.UntypedStorage._release_ipc_counter_cuda(*args, **kwargs)
    ...
    def _get_legacy_storage_class(self):
        ...
        module = (
            smith if self.device.type == "cpu" else getattr(smith, self.device.type)
        )
...
class _LegacyStorage(TypedStorage, metaclass=_LegacyStorageMeta):
    ...
    def _new_shared(cls, size):  # type: ignore[override]
        """Create a new storage in shared memory with the same data type."""
        untyped_storage = smith.UntypedStorage._new_shared(size * cls()._element_size())
    ...
    def _new_shared_filename(cls, manager, obj, size):
        bytes_size = size * smith._utils._element_size(cls.dtype)
        return cls(
            wrap_storage=smith.UntypedStorage._new_shared_filename_cpu(
