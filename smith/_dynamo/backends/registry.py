"""
This module implements SmithDynamo's backend registry system for managing compiler backends.

The registry provides a centralized way to register, discover and manage different compiler
backends that can be used with smith.compile(). It handles:
...
Example usage:
    ...
    smith.compile(model, backend="my_compiler")

The registry also supports discovering backends through setuptools entry points
in the "smith_dynamo_backends" group. Example:
```
setup.py
---
...
setup(
    name='my_smith_backend',
    version='0.1',
    packages=['my_smith_backend'],
    entry_points={
        'smith_dynamo_backends': [
            # name = path to entry point of backend implementation
            'my_compiler = my_smith_backend.compiler:my_compiler_function',
        ],
    },
)
```
```
my_smith_backend/compiler.py
---
...
model = ...  # Your Blacksmith model
optimized_model = smith.compile(model, backend="my_compiler")
```
"""
...
class CompiledFn(Protocol):
    def __call__(self, *args: smith.Tensor) -> tuple[smith.Tensor, ...]: ...


CompilerFn = Callable[[fx.GraphModule, list[smith.Tensor]], CompiledFn]
...
def register_backend(
    ...
    `smith.compile` with string shorthand.  Note: for projects not
...
def _discover_entrypoint_backends() -> None:
    ...
    group_name = "smith_dynamo_backends"
