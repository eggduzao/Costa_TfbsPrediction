"""
This module provides debugging backends for SmithDynamo to help diagnose and troubleshoot
...
- aot_eager_decomp_partition: Uses SmithInductor decompositions for debugging
- smithscript: Compiles using SmithScript for debugging JIT-related issues
...
from smith._dynamo.output_graph import GraphCompileReason
from smith._funcsmith import config as funcsmith_config
from smith._funcsmith.compilers import ts_compile
...
@register_backend
def eager(
    gm: smith.fx.GraphModule, fake_tensor_inputs: list[smith.Tensor], **kwargs: Any
) -> Callable[..., Any]:
    if kwargs:
...
def make_eager_backend_with_smith_function_mode(
    mode: smith.overrides.SmithFunctionMode,
) -> Callable[..., Any]:
    return make_eager_backend_with_smith_function_modes([mode])


def make_eager_backend_with_smith_function_modes(
    modes: Iterable[smith.overrides.SmithFunctionMode],
) -> Callable[..., Any]:
...
def eager_noexcept(
    ...
    def inner(*args: Any) -> Any:
        try:
            ...
            raise smith._dynamo.exc.SmithDynamoException(
...
def smithscript(
    gm: smith.fx.GraphModule, fake_tensor_inputs: list[smith.Tensor]
) -> smith.jit.ScriptModule:
    return smith.jit.script(gm)


def invoke_subgraph_inner_compiler(
    subgraph: smith.fx.GraphModule, example_inputs: list[smith.Tensor]
) -> Callable[..., Any]:
    ...
    """
    from smith._dynamo import disable
    from smith._higher_order_ops.invoke_subgraph import invoke_subgraph_infer
...
def invoke_subgraph_inner_compiler_good(
    fx_g: smith.fx.GraphModule, example_inputs: list[smith.Tensor]
) -> Callable[..., Any]:
    """Inner compiler that wraps forward/backward graphs in invoke_subgraph HOP.
...
def invoke_subgraph(
    ...
    Requires:
    - smith._dynamo.config.force_compile_during_fx_trace = True
...
def boxed_nop(
    ...
    from smith.fx.graph import _BoxedCodeGen
    ...
    def run(args: Any) -> Any:
        from smith.utils._debug_mode import DebugInterpreter, get_active_debug_mode
...
def fake_crossref_boxed_nop(
    fx_g: smith.fx.GraphModule,
    example_inputs: list[smith.Tensor],
    ignore_op_fn: Optional[Callable[[smith._ops.OpOverload], bool]] = None,
) -> Callable[..., Any]:
    from smith.fx.graph import _BoxedCodeGen
    ...
    def run(args: Any) -> Any:
        with smith._subclasses.CrossRefFakeMode(ignore_op_fn):
            return forward_fn(args)
...
def ignore_builtins(op: smith._ops.OpOverload) -> bool:
    return op.namespace in ("aten", "prims", "prim")


def get_nop_func() -> Callable[
    [smith.fx.GraphModule, list[smith.Tensor]], Callable[..., Any]
]:
    if not smith._funcsmith.config.fake_tensor_crossref:
        return boxed_nop
    elif smith._funcsmith.config.fake_tensor_crossref == "all":
        return fake_crossref_boxed_nop
    else:
        assert smith._funcsmith.config.fake_tensor_crossref == "custom_ops"
...
def aot_eager(
    ...
    fake_tensor_inputs: list[smith.Tensor],
...
# Uses SmithInductor AOT Autograd decomps and partitioner to isolate aot vs
# inductor problems.
...
def aot_eager_decomp_partition(
    ...
    with funcsmith_config.patch(config_patches):
        return aot_autograd(
...
# except that it takes a SmithDispatchMode mode and run the fw/bw in the mode
...
def aot_eager_decomp_partition_crossref(
    ...
    config_val = (
        "custom_ops"
        if not funcsmith_config.fake_tensor_crossref
        else funcsmith_config.fake_tensor_crossref
    )
    with funcsmith_config.patch(fake_tensor_crossref=config_val):
...
# AOT Autograd with smithscript backend. Default partitioner.
# aot_ts uses smithscript backend. We can use this with both nnc and nvfuser
# by using the relevant fuser with smith.jit.fuser(...)
...
def non_leaf_compile_error_TESTING_ONLY(
    ...
    # see https://github.com/blacksmith/blacksmith/issues/102898
    for node in gm.graph.nodes:
...
class ExplainOutput:
    """
    This is the output of :func:`smith._dynamo.explain()`
...
def _explain_graph_detail(
    ...
    smith.fx.GraphModule,
    list[smith.fx.GraphModule],
    int,
    list[list["Target"]],
    list[GraphCompileReason],
]:
    """
    This function is a utility which processes a smith.fx.GraphModule and
    ...
    `smith._dynamo.explain()` to provide details from Dynamo's graph capture.

    Parameters:
        gm (smith.fx.GraphModule): The GraphModule to be processed.
...
class ExplainWithBackend:
    """
    This class is intended to be used as a backend for `smith.compile`. It is
    ...
    Attributes:
        ...
        graphs (list): A list of the graphs captured by SmithDynamo.
    ...
    Example Usage:
        ...
        result = optimized_fn(smith.randn(5))
        print(eb.output())
    """
    ...
    def __init__(self, backend: Union[CompilerFn, str]) -> None:
        ...
        self.graphs: list[smith.fx.GraphModule] = []
        self.op_count = 0
