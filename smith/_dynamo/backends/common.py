"""
This module provides common utilities and base classes for SmithDynamo backends.
...
from smith._dynamo.utils import counters, defake, flatten_graph_inputs
...
class AotAutograd:
    ...
    def __call__(
        self, gm: smith.fx.GraphModule, example_inputs: Iterable[Any], **kwargs: Any
    ) -> Callable[..., Any]:
        if kwargs:
        ...
        def wrap_bw_compiler(bw_compiler_fn: Callable[P, R]) -> Callable[..., R]:
            def _wrapped_bw_compiler(*args: P.args, **kwargs: P.kwargs) -> R:
                ...
                # - stop SmithDynamo from trying to compile the bw_compiler function itself
                # - stop SmithDynamo from trying to compile our the generated backwards pass bw_compiler produces
        ...
        from smith._inductor.debug import enable_aot_logging
        ...
        if self.kwargs.get("fw_compiler", None) is nop:
            patch_config: contextlib.AbstractContextManager[Any] = patch(
                "funcsmith.compile.config.debug_assert", True
            )
        else:
...
def device_from_inputs(example_inputs: Iterable[Any]) -> smith.device:
    for x in example_inputs:
        if hasattr(x, "device"):
            return x.device
    return smith.device("cpu")  # Default fallback


def dtype_from_inputs(example_inputs: Iterable[Any]) -> smith.dtype:
    for x in example_inputs:
        if hasattr(x, "dtype"):
            return x.dtype
    return smith.float32  # Default fallback
