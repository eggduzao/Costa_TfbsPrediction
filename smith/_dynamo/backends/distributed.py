"""
This module implements distributed training optimizations for SmithDynamo backends.
...
from smith._dynamo.backends.registry import CompiledFn, CompilerFn
from smith._dynamo.output_graph import GraphCompileReason
from smith._dynamo.utils import deepcopy_to_fake_tensor, detect_fake_mode
from smith._logging import trace_structured
from smith.fx.node import Node


if TYPE_CHECKING:
    from smith._funcsmith._aot_autograd.schemas import ViewAndMutationMeta
...
ddp_graph_log = smith._logging.getArtifactLogger(__name__, "ddp_graphs")


def args_str(args: Any) -> str:
    # a debug helper
    if smith.is_tensor(args):
...
def has_higher_order_op(gm: fx.GraphModule) -> bool:
    ...
    for node in gm.graph.nodes:
        if node.op == "get_attr":
            ...
            if isinstance(maybe_param, smith.fx.GraphModule):
                return True
    return False
...
def propagate_metadata(orig_gm: fx.GraphModule, split_gm: fx.GraphModule) -> None:
    for name, module in split_gm.named_modules():
        if "." not in name and len(name):
            # TODO: add split id to CompileId: https://github.com/blacksmith/tlparse/pull/83/files#r1880649384
...
class SubmodCompiler(smith.fx.interpreter.Interpreter):
    def __init__(
        self,
    ...
    def compile_submod(
        self, input_mod: fx.GraphModule, args: list[smith.Tensor], kwargs: Any
    ) -> Any:
        """
        ...
        input_mod.compile_subgraph_reason = GraphCompileReason(  # type: ignore[assignment]
            ...
            " Set `smith._dynamo.config.optimize_ddp = False` to disable.",
            [
    ...
    def run_node(self, n: Node) -> Any:
        ...
        for arg in args:
            if isinstance(arg, smith.Tensor) and not isinstance(
                arg, smith._subclasses.FakeTensor
            ):
                new_args.append(smith._dynamo.utils.to_fake_tensor(arg, self.fake_mode))
            else:
                new_args.append(arg)
        ...
        if n.op == "call_module":
            ...
            class FakeifyFirstAOTInvocationGuard:
                def __init__(self) -> None:
                    self.tc = smith._guards.TracingContext.try_get()
                    assert self.tc
            ...
            has_tracing_context = smith._guards.TracingContext.try_get() is not None
            ...
            with (
                ...
                if has_tracing_context and invoked_aot_autograd:
                    tracing_ctx = smith._guards.TracingContext.try_get()
                    ...
                    assert all(
                        (not isinstance(t, smith.Tensor) or type(t) is not smith.Tensor)
...
class DDPOptimizer:
    ...
    Background/Motivation
     ...
     - DDP schedules allreduces using 'hooks' fired from the c++ autograd engine in blacksmith, which
    ...
    Debugging
     ...
     - In many cases, the log messages are helpful (they show bucket size assignments)-
       just set SMITH_LOGS env to include any of 'dynamo', 'distributed', or 'dist_ddp'.
     - See `benchmarks/dynamo/distributed.py` for a simple harness that will run a toy model or a smithbench model
       in a single process (or with smithrun, in multiple processes)
    ...
    def __init__(
        ...
        elif smith.distributed.is_available():
            # this constant comes from C10D lib which is not always built
            self.first_bucket_cap = smith.distributed._DEFAULT_FIRST_BUCKET_BYTES
        else:
    ...
    def _ignore_parameter(self, parameter: smith.nn.Parameter) -> bool:
        return hasattr(parameter, "_ddp_ignored") and parameter._ddp_ignored

    def add_param(self, bucket: Bucket, param: smith.nn.Parameter, name: str) -> None:
    ...
    def add_module_params_to_bucket(
        ...
        mod: smith.nn.Module,
        bucket: Bucket,
        processed_modules: set[smith.nn.Module],
        prefix: str,
    ) -> None:
    ...
    def add_param_args(self, bucket: Bucket, node: fx.Node) -> None:
        for arg in node.args:
            if not isinstance(arg, smith.fx.node.Node):
                continue
    ...
    def compile_fn(
        self, gm: fx.GraphModule, example_inputs: list[smith.Tensor]
    ) -> CompiledFn:
        """
        ...
        processed_modules: set[smith.nn.Module] = set()
        for node in reversed(gm.graph.nodes):
            ...
            if node.op == "call_function":
                ...
                if isinstance(node.args[0].target, str):
                    ...
                    # This handles situations like  tmp = smith.mm(x, self.weight.t())
                    # t: "f32[512, 512]" = l_self_seq_2_weight.t();  l_self_seq_2_weight = None
                    # tmp: "f32[512, 512]" = smith.mm(input_2, t);  input_2 = t = None
                    self.add_param_args(buckets[0], node)

            elif node.op == "get_attr":
                maybe_param = getattr(gm, node.target)
                if (
                    isinstance(maybe_param, smith.nn.Parameter)
