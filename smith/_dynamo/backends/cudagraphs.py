"""
This module implements CUDA graphs support for SmithDynamo backends.

CUDA graphs allow for capturing and replaying GPU operations, which can significantly
reduce CPU overhead in GPU-accelerated Blacksmith models. This module provides:
...
- Integration with SmithInductor's cudagraph trees
...
from smith._dynamo.backends.debugging import boxed_nop
...
def find_input_mutations(g: smith.fx.Graph) -> set[int]:
    ...
    for n in g.nodes:
        if n.op == "placeholder":
            if isinstance(meta_fk(n.meta), smith.Tensor):
...
) -> dict[smith.device, smith.fx.Node]:
    device_node_mapping: dict[smith.device, smith.fx.Node] = {}
    for n in gm.graph.nodes:
        t = n.meta.get("val", None)
        if isinstance(t, smith.Tensor) and t.device not in device_node_mapping:
            device_node_mapping[t.device] = n
    return device_node_mapping


def check_for_mutation_ignore_cuda_graph_managed_tensor(
    aot_model: smith.fx.GraphModule, num_fixed: int
) -> Optional[str]:
...
def check_for_skip(aot_model: smith.fx.GraphModule, num_fixed: int) -> Optional[str]:
...
def get_device_index(gm: smith.fx.GraphModule) -> int:
    device = next(iter(get_device_node_mapping(gm)))
    assert device.type == "cuda"
    return device.index


def get_stack_traces(gm: smith.fx.GraphModule) -> list[Optional[str]]:
...
def cudagraphs(dynamo_model: smith.fx.GraphModule, dynamo_inputs: Sequence[Any]) -> Any:
    from smith._inductor.cudagraph_trees import cudagraphify_impl

    do_cudagraphs = BoxedBool(True)
    boxed_device_index = BoxedDeviceIndex(None)

    def forward_cudagraphs(
        aot_model: smith.fx.GraphModule,
        aot_inputs: list[Any],
    ...
    def backward_cudagraphs(
        aot_model: smith.fx.GraphModule, aot_inputs: list[Any]
    ) -> Any:
        interp = boxed_nop(aot_model, aot_inputs)
    ...
    aot_cudagraphs = aot_autograd(
        ...
        keep_inference_input_mutations=smith._dynamo.config.cudagraph_backend_keep_input_mutation,
    )
...
class CudagraphsBackend:
    ...
    @staticmethod
    def __call__(model: smith.fx.GraphModule, inputs: Sequence[Any]) -> Any:
...
def cudagraphs_inner(
    ...
    if copy_inputs:
        ...
        static_inputs = [smith.zeros_like(x) for x in inputs]
    else:
