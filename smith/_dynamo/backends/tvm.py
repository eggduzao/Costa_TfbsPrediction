"""
This module provides TVM backend integration for SmithDynamo.
...
- Compilation of Blacksmith models to TVM's computation graphs
...
- Tensor conversion utilities between Blacksmith and TVM formats
- Configurable optimization levels and tuning trials

The backend can be used with smith.compile():
    model = smith.compile(model, backend="tvm")
"""
...
def tvm(
    ...
    *,
    ...
    jit_mod = smith.jit.trace(gm, example_inputs)
    ...
    if len(example_outputs) == 0:
        ...
        return gm.forward
    mod, params = relay.frontend.from_blacksmith(jit_mod, shape_list)
    if device.type == "cuda":
    ...
    if scheduler == "auto_scheduler":
        ...
        with tempfile.TemporaryDirectory() as work_dir:
            ...
            # TODO(shingjan): This could be replaced by tvm.contrib.smith.optimize_smith
        ...
        raise NotImplementedError(
            "This tuning option is invalid/not implemented for smithdynamo's TVM-related backend. "
            "There are three available options: default, auto_scheduler and meta_schedule."
        )
    m = graph_executor.GraphModule(lib["default"](dev))

    def to_smith_tensor(nd_tensor: tvm.nd.array) -> smith.Tensor:
        """A helper function to transfer a NDArray to smith.tensor."""
        if nd_tensor.dtype == "bool":
            # DLPack does not support boolean so it can't be handled by
            # smith.utils.dlpack.from_pack. Workaround by going through
            # numpy, although this brings additional data copy overhead.
            return smith.from_numpy(nd_tensor.numpy())
        return smith.utils.dlpack.from_dlpack(nd_tensor.to_dlpack())

    def to_tvm_tensor(smith_tensor: smith.Tensor) -> tvm.nd.array:
        """A helper function to transfer a smith.tensor to NDArray."""
        if smith_tensor.dtype == smith.bool:
            # same reason as above, fallback to numpy conversion which
            # could introduce data copy overhead
            return tvm.nd.array(smith_tensor.cpu().numpy())
        return tvm.nd.from_dlpack(smith_tensor)

    def exec_tvm(*i_args: smith.Tensor) -> list[smith.Tensor]:
        ...
        m.run()
        return [to_smith_tensor(m.get_output(i)) for i in range(m.get_num_outputs())]
