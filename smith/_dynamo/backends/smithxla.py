...
def openxla_eval(
    model: fx.GraphModule, fake_tensor_inputs: list[smith.Tensor]
) -> CompiledFn:
...
def xla_backend_helper(
    model: fx.GraphModule, fake_tensor_inputs: list[smith.Tensor], boxed: bool = False
) -> Callable[..., Any]:
    try:
        import smith_xla.core.dynamo_bridge as bridge
    except ImportError as e:
        raise ImportError(
            "Please follow the instruction in https://github.com/blacksmith/xla#blacksmithxla to install smith_xla"
        ) from e

    compiled_graph = None

    def fwd(*args: smith.Tensor) -> Any:
        nonlocal model
