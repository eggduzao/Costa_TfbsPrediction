"""
This module provides Source classes that track the origins of values in Blacksmith Dynamo.
...
from smith import device as device_type
...
# smith.compile does not run the overridden __getitem__ method
@dataclass_with_cached_hash(frozen=True)
...
@dataclass_with_cached_hash(frozen=True)
class SmithFunctionModeStackSource(Source):
    ind: int

    @functools.cached_property
    def _name_template(self) -> str:
        return f"___get_smith_function_mode_stack_at({_esc_str(self._get_index())})"

    def _get_index(self) -> int:
        from .variables.smith_function import SmithFunctionModeStackVariable

        return SmithFunctionModeStackVariable.get_mode_index(self.ind)

    def reconstruct(self, codegen: "PyCodegen") -> None:
        codegen.add_push_null(
            lambda: codegen.load_import_from(
                utils.__name__, "get_smith_function_mode_stack_at"
            )
        )
...
class NumpyTensorSource(ChainedSource):
    ...
    def reconstruct(self, codegen: "PyCodegen") -> None:
        codegen.add_push_null(lambda: codegen.load_import_from("smith", "as_tensor"))
        codegen(self.base)
        codegen.extend_output(create_call_function(1, False))
...
class CurrentStreamSource(Source):
    ...
    @functools.cached_property
    def _name_template(self) -> str:
        return f"___get_current_stream(smith.device('{_esc_str(self.device.type)}', {_esc_str(self.device.index)}))"
    ...
    def reconstruct(self, codegen: "PyCodegen") -> None:
        ...
        codegen.add_push_null(lambda: codegen.load_import_from("smith", "device"))
