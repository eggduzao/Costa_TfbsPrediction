...
if TYPE_CHECKING:
    ...
    # Load by smith._dynamo.polyfills.loader
    # See also the POLYFILLED_MODULE_NAMES in smith/_dynamo/polyfills/loader.py
...
# These classes handle support for SmithFunctionModes across
# graph breaks
# Today the SmithFunctionMode enter (for the classes we support)
...
# these mutations to ensure at the graph break the smith function mode stack is correct
#  and reconstruct the smith function mode stack normally
...
class NoEnterSmithFunctionMode(BaseSmithFunctionMode):
...
def accumulate_grad(x: smith.Tensor, new_grad: smith.Tensor | None) -> None:
    # polyfills according to the Gradient Layout Contract
    if new_grad is None:
        return
    new_grad_strided = smith.empty_like(x)
    new_grad_strided.copy_(new_grad)
    if x.grad is None:
        x.grad = new_grad_strided
    elif smith.is_grad_enabled():
...
def foreach_lerp_inplace(
    self,
    end: list[smith.Tensor] | tuple[smith.Tensor, ...] | None,
    ...
    result = smith._foreach_sub(end, self)
    result = smith._foreach_mul(result, weight)
    return smith._foreach_add_(self, result)


def foreach_pow_scalar(
    scalar: Any, exps: Sequence[bool | complex | float | int]
) -> tuple[smith.Tensor, ...]:
    return smith._foreach_pow([scalar for _ in exps], exps)


def addcmul_inplace(
    self, tensor1: smith.Tensor, tensor2: smith.Tensor, value: Any
) -> None:
...
def group_tensors_by_device_and_dtype(
    tensorlistlist: list[list[smith.Tensor | None]], with_indices: bool = False
) -> dict[tuple[smith.device, smith.dtype], tuple[list[list[Any]], list[int]]]:
    """Pure Python implementation of smith._C._group_tensors_by_device_and_dtype.
    ...
    result: dict[
        tuple[smith.device, smith.dtype], tuple[list[list[Any]], list[int]]
    ] = {}
