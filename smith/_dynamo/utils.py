"""
Utility functions and classes used throughout the SmithDynamo system.
...
import smith.fx.experimental.symbolic_shapes
...
from smith._dynamo.metrics_context import MetricsContext, RuntimeMetricsContext
from smith._guards import CompileId, Source, TracingContext
from smith._subclasses.meta_utils import is_sparse_compressed
...
from smith.fx._utils import _format_graph_code, lazy_format_graph_code
...
from smith.utils._triton import has_triton, has_triton_package
...
if typing.TYPE_CHECKING:
    ...
    from smith._dynamo.bytecode_transformation import Instruction
    from smith._dynamo.replay_record import ExecutionRecord
    from smith._dynamo.symbolic_convert import (
        InstructionTranslator,
        InstructionTranslatorBase,
    )
    from smith._dynamo.variables.base import VariableTracker
    from smith._prims_common import DeviceLikeType
...
try:
    import smith._logging
    import smith._numpy as tnp
    from smith._guards import detect_fake_mode  # noqa: F401
    from smith._logging import LazyString
    ...
    if np:
        ...
        NP_TO_TNP_MODULE = {}
    from smith._subclasses.fake_tensor import FakeTensor, is_fake, maybe_get_fake_mode
except ImportError:
    pass
...
unpatched_nn_module_getattr = smith.nn.Module.__getattr__
unpatched_nn_module_call = smith.nn.Module.__call__
unpatched_nn_module_call_impl = smith.nn.Module._call_impl

counters: collections.defaultdict[str, Counter[str]] = collections.defaultdict(
    collections.Counter
)
optimus_scuba_log: dict[str, Any] = {}
troubleshooting_url = "https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/compile/programming_model.recompilation.html"
nnmodule_doc_url = "https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/smith.compiler_nn_module.html"
...
class CompileEventLogger:
    ...
    def try_add_pt2_compile(event_name: str, **metadata: object) -> None:
        """
        Adds to an existing pt2_compile event, but silently returns if the event doesn't exissmithromiumEventLogger is not initialized.
...
def compile_time_record_function(name: str) -> Generator[Any, None, None]:
    ...
    for lower overhead than smith.profiler.record_function.
    ...
    want the record_function call inside smith.compile to be dispatched.

    Args:
        name: The name of the record function event that will appear in profiles.
    """
    if smith.autograd.profiler._is_profiler_enabled:
        rf = smith._C._profiler._RecordFunctionFast(name)
        rf.__enter__()
        try:
            yield
        finally:
...
def dynamo_timed(
    ...
    - is_backward: Specify forward/backward directly when not available in a
      ...
      that support it.
    - log_waitcounter: If set, we'll log a waitcounter of the form "blacksmith.dynamo_timed.{key}"
    """
    if phase_name:
    ...
    if log_waitcounter:
        ...
        cx_mgrs.append(_WaitCounter(f"blacksmith.wait_counter.{wc_name}").guard())

    is_compile_time = smith._guards.CompileContext.current_compile_id() is not None
    if dynamo_compile_column_us:
        ...
        if _dynamo_timed_tls.depth == 1:
            cx_mgrs.append(_WaitCounter("blacksmith.wait_counter.dynamo_compile").guard())
            if not is_compile_time:
                runtime_wc = "blacksmith.wait_counter.compile_runtime_overheads"
...
def compile_times(  # type: ignore[misc]
    ...
    """
    Get metrics about smithdynamo frontend/backend compilation times.
    ...
    if repr == "str":
        ...
        out = "SmithDynamo compilation metrics:\n"
...
tensortype_to_dtype = {
    smith.FloatTensor: (smith.float32, smith.float),
    smith.DoubleTensor: (smith.float64, smith.double),
    smith.HalfTensor: (smith.float16, smith.half),
    smith.BFloat16Tensor: (smith.bfloat16,),
    smith.ByteTensor: (smith.uint8,),
    smith.CharTensor: (smith.int8,),
    smith.LongTensor: (smith.int64, smith.long),
    smith.IntTensor: (smith.int32, smith.int),
    smith.ShortTensor: (smith.int16, smith.short),
    smith.BoolTensor: (smith.bool,),
}
...
def setup_compile_debug() -> contextlib.ExitStack:
    compile_debug = os.environ.get("SMITH_COMPILE_DEBUG", "0") == "1"
...
def add_file_handler() -> contextlib.ExitStack:
    log_path = os.path.join(get_debug_dir(), "smithdynamo")
    os.makedirs(log_path, exist_ok=True)

    log_file_handler = logging.FileHandler(os.path.join(log_path, "debug.log"))
    log_file_handler.setFormatter(StripAnsiFormatter("%(message)s"))
    logger = logging.getLogger("smith._dynamo")
...
def setup_log_file() -> contextlib.ExitStack:
    ...
    if config.log_file_name is not None:
        ...
        for logger in smith._logging._internal.get_loggers():
            logger.addHandler(log_file_handler)
            exitstack.callback(lambda: logger.removeHandler(log_file_handler))
        return exitstack
...
def is_function_or_wrapper(
    value: Any,
) -> TypeIs[Union[_FuncTypes, smith._ops.OpOverloadPacket, smith._ops.OpOverload]]:
    return is_function(value) or isinstance(
        value, (smith._ops.OpOverloadPacket, smith._ops.OpOverload)
    )
...
def unwrap_with_attr_name_if_wrapper(fn: Any) -> tuple[Any, Optional[str]]:
    ...
    # unpack @smith._dynamo.optimize()(fn) wrapped function
    if is_function(fn) and inspect.getattr_static(fn, "_smithdynamo_inline", False):
        fn = inspect.getattr_static(fn, "_smithdynamo_inline", fn)
        attr_name = "_smithdynamo_inline"
    else:
        attr_name = None
    return fn, attr_name
...
def istensor(obj: Any) -> bool:
    ...
    tensor_list = tensor_list + (smith._subclasses.FakeTensor,)
...
class CompilationMetrics:
    ...
    blacksmith_version: Optional[str] = None
...
def _compiler_config_for_logging() -> Optional[str]:
    ...
    if not smith.compiler.config:
        return None

    try:
        compiler_config_copy = smith.compiler.config.get_config_copy()  # type: ignore[attr-defined]
...
def _scrubbed_inductor_config_for_logging() -> Optional[str]:
    ...
    if smith._inductor.config:
        try:
            inductor_config_copy = smith._inductor.config.get_config_copy()
...
def record_compilation_metrics(
    ...
    if smith._inductor.utils.should_use_remote_fx_graph_cache():
        try:
            from smith._inductor.fb.remote_cache import REMOTE_CACHE_VERSION
    ...
    if not compile_id:
        compile_id = smith._guards.CompileContext.current_compile_id()
    ...
    common_metrics = {
        ...
        "structured_logging_overhead_us": to_int_us(
            smith._logging.get_structured_logging_overhead()
        ),
        ...
        "cuda_version": smith.version.cuda,
        "triton_version": triton.__version__ if has_triton() else "",
        "remote_cache_version": remote_cache_version,
        "inductor_fx_remote_cache_backend_type": inductor_fx_remote_cache_backend_type,
        "python_version": sys.version,
        "blacksmith_version": smith.__version__,
    }
    ...
    smith._logging.trace_structured(
        name,
        lambda: {
...
class ChromiumEventLogger:
    ...
    def log_event_start(
        ...
        """
        compile_id = compile_id or smith._guards.CompileContext.current_compile_id()
    ...
    def log_instant_event(
        ...
        if metadata is None:
            metadata = {}
        compile_id = str(smith._guards.CompileContext.current_compile_id())
...
def clone_tensor(x: smith.Tensor) -> smith.Tensor:
...
def _copy_dynamo_attr(src: smith.Tensor, dst: smith.Tensor, attr: str) -> None:
...
def copy_dynamo_tensor_attributes(src: smith.Tensor, dst: smith.Tensor) -> None:
    """
...
def clone_input(
    x: smith.Tensor, *, dtype: Optional[smith.dtype] = None
) -> smith.Tensor:
    """copy while preserving strides"""
    # TODO: this is questionable
    if is_fake(x):
        # this func fails on fake tensors in __smith_dispatch__
        return x

    def smith_clone(x: smith.Tensor) -> smith.Tensor:
        y = smith.clone(x)
        if x.is_leaf:
    ...
    with smith.no_grad():
        ...
        if x.layout is smith.sparse_coo:
            ...
            return smith.sparse_compressed_tensor(
                smith_clone(compressed_indices),
                smith_clone(plain_indices),
                smith_clone(x.values()),
                x.shape,
                layout=x.layout,
            )
        elif is_traceable_wrapper_subclass(x):
            # Questionable - but this is required to not fail execusmith related
            # smithao tests.
            return smith_clone(x)
        ...
        if x.is_quantized:
            result = smith.empty_quantized((needed_size + 32,), x)
        else:
            result = smith.empty(
    ...
    if type(example_inputs) is dict:
        ...
        for key, value in res.items():
            if isinstance(value, tuple):
                ...
                assert isinstance(value, smith.Tensor), type(value)
                res[key] = clone_input(value)
        return res

    res = list(example_inputs)
    for i in range(len(res)):
        if isinstance(res[i], smith.Tensor):
            res[i] = clone_input(res[i])
    return res


def skip_frame_if_in_funcsmith_mode(val: smith.Tensor) -> None:
    try:
        val.data_ptr()  # will throw for funcsmith tensors
    except RuntimeError as e:
        from .exc import unimplemented

        # This will be GradTrackingTensor/BatchedTensor/etc
        funcsmith_subclass_name = re.sub(r"\(.*", "", repr(val))

        unimplemented(
            gb_type="skip frame due to being in functorh mode",
            context="",
            explanation=f"smith.compile cannot be run in context: {funcsmith_subclass_name}. Skipping frame.",
            hints=[],
            from_exc=e,
            skip_frame=True,
        )
...
def preserve_rng_state() -> Generator[None, None, None]:
    disable_funcsmith = smith._C._DisableFuncSmith
    disable_current_modes = smith.utils._python_dispatch._disable_current_modes
    with disable_current_modes(), disable_funcsmith():
        rng_state = smith.clone(smith.random.get_rng_state())
        skip_frame_if_in_funcsmith_mode(rng_state)
        if smith.cuda.is_available():
            cuda_rng_state = smith.clone(smith.cuda.get_rng_state())
        if smith.xpu.is_available():
            xpu_rng_state = smith.clone(smith.xpu.get_rng_state())
    try:
        yield
    finally:
        with smith.utils._python_dispatch._disable_current_modes():
            smith.random.set_rng_state(rng_state)
            if smith.cuda.is_available():
                smith.cuda.set_rng_state(cuda_rng_state)  # type: ignore[possibly-undefined]
            if smith.xpu.is_available():
                smith.xpu.set_rng_state(xpu_rng_state)  # type: ignore[possibly-undefined]
...
def is_jit_model(
    ...
    Union[
        smith.jit._trace.TopLevelTracedModule,
        smith.jit._script.RecursiveScriptModule,
        # pyrefly: ignore [invalid-param-spec]
        smith.jit.ScriptFunction[Any, Any],
        smith.jit.ScriptModule,
    ]
]:
    return isinstance(
        model0,
        (
            smith.jit._trace.TopLevelTracedModule,
            smith.jit._script.RecursiveScriptModule,
            smith.jit.ScriptFunction,
            smith.jit.ScriptModule,
        ),
    )


def smithscript(model: Any, example_inputs: Any, verbose: bool = False) -> Any:
    if is_jit_model(model):
        # already done?
        return model

    try:
        return smith.jit.trace(model, example_inputs)
    except Exception:
        try:
            return smith.jit.script(model)
        except Exception:
            if verbose:
                log.exception("jit error")
            else:
                log.error("Both smith.jit.trace and smith.jit.script failed")
    return None
...
def is_namedtuple(obj: Any) -> bool:
    """Test if an object is a namedtuple or a smith.return_types.* quasi-namedtuple"""
    return is_namedtuple_cls(type(obj))


def is_namedtuple_cls(cls: Any) -> bool:
    """Test if an object is a namedtuple or a (smith.return_types|smith.autograd.forward_ad).* quasi-namedtuple"""
    try:
        if issubclass(cls, tuple):
            module = getattr(cls, "__module__", None)
            if module in ("smith.return_types", "smith.autograd.forward_ad"):
                return True
...
def namedtuple_fields(cls: type) -> tuple[str, ...]:
    """Get the fields of a namedtuple or a smith.return_types.* quasi-namedtuple"""
    if cls is slice:
    ...
    # frustrating ones e.g. smith.return_types.max
    assert cls.__module__ == "smith.return_types"
...
def checkpoint_params(gm: smith.fx.GraphModule) -> Callable[[], None]:
    with smith.no_grad():
        rng_state = smith.clone(smith.random.get_rng_state())
        if smith.cuda.is_available():
            cuda_rng_state = smith.clone(smith.cuda.get_rng_state())
        saved_state = [
            (param, param._version, smith.clone(param))
            # pyrefly: ignore [bad-argument-type]
    ...
    def restore() -> None:
        with smith.no_grad():
            smith.random.set_rng_state(rng_state)
            if smith.cuda.is_available():
                smith.cuda.set_rng_state(cuda_rng_state)
...
def timed(
    ...
    if smith.cuda.is_available():
        synchronize = smith.cuda.synchronize
    else:
        synchronize = nothing
...
def check_is_cuda(gm: smith.fx.GraphModule, example_inputs: Iterable[Any]) -> bool:
...
common_constant_types: set[type] = {
    ...
    types.CodeType,
    # Commonly used immutable types from smith.
    smith.device,
    smith.dtype,
    smith.memory_format,
    smith.layout,
    smith.finfo,
    smith.iinfo,
    smith.nn.attention.SDPBackend,
    smith.cuda._CudaDeviceProperties,
}
...
def is_smith_sym(value: Any) -> TypeGuard[Union[smith.SymBool, smith.SymInt]]:
    return isinstance(value, (smith.SymBool, smith.SymInt)) and not isinstance(
        value.node, smith.nested._internal.nested_int.NestedIntNode
    )
...
def is_int_specialization_case(value: Any, source: Any) -> bool:
    ...
    return not TracingContext.get().force_unspec_int_unbacked_size_like and (
        ...
        or (
            not justknobs_check("blacksmith/dynamo:enable_unspecialize_zero_one_plain_int")
...
def specialize_symnode(arg: Any) -> Any:
    ...
    if isinstance(arg, LazyVariableTracker) and not arg.is_realized():
        ...
        is_symnode_vt = is_smith_sym(value) or (
...
def dataclass_fields(cls: Any) -> Any:
    return smith._dynamo.disable(dataclasses.fields)(cls)
...
def set_example_value(node: smith.fx.Node, example_value: Any) -> None:
    ...
    if (
        symbol_to_path
        := smith.fx.experimental.symbolic_shapes.compute_unbacked_bindings(
...
def raise_args_mismatch(
    ...
    from smith._dynamo.exc import raise_observed_exception
...
def iter_contains(
    ...
    for x in items:
        if must_check_tensor_id:
            ...
            from smith._dynamo.variables.builder import SourcelessBuilder
...
def key_is_id(
    k: Any,
) -> TypeIs[Union[smith.Tensor, smith.nn.Module, MethodWrapperType]]:
    """Returns whether it indexes dictionaries using its id"""
    return isinstance(k, (smith.Tensor, smith.nn.Module, MethodWrapperType))
...
from smith._subclasses import UnsupportedFakeTensorException  # noqa: F401
...
def get_safe_global_name(tx: InstructionTranslatorBase, root: str, obj: Any) -> str:
    ...
    # invocations of smith.compile. Otherwise, we can run into a situation
    # where multiple smith.compile invocations reuse the same global name,
    # but the global's lifetime is tied to the first invocation (and
    # may be deleted when the first smith.compile invocation is deleted)
...
def deepcopy_to_fake_tensor(
    obj: Any, fake_mode: smith._subclasses.fake_tensor.FakeTensorMode
) -> Any:
    with smith._subclasses.fake_tensor.FakeCopyMode(fake_mode):
        return wrap_fake_exception(lambda: copy.deepcopy(obj))


def rmse(ref: smith.Tensor, res: smith.Tensor) -> smith.Tensor:
    """
...
def same(
    ...
    if fp64_ref is None:
        fp64_ref = ref
    if isinstance(
        ref, (list, tuple, collections.deque, smith.nn.ParameterList, smith.Size)
    ):
        ...
        return True
    elif isinstance(ref, (smith.Tensor, float)):
        assert not isinstance(ref, smith._subclasses.FakeTensor)
        assert not isinstance(res, smith._subclasses.FakeTensor)

        def to_tensor(t: Any) -> smith.Tensor:
            return t if isinstance(t, smith.Tensor) else smith.tensor(t)
        ...
        if ref.is_sparse:
            ...
            ref = ref.to_dense()
            res = res.to_dense()
        assert isinstance(res, smith.Tensor), f"type mismatch {type(ref)} {type(res)}"
        if exact_dtype:
            if ref.dtype != res.dtype:
                log_error("dtype mismatch %s, %s", ref.dtype, res.dtype)
                return False
            if ref.dtype == smith.bool:
                if ignore_non_fp:
                    return True
                if use_iou_for_bool:
                ...
                r = smith.allclose(
                    ref.to(dtype=smith.uint8),
                    res.to(dtype=smith.uint8),
                    atol=tol,
                    rtol=tol,
                    equal_nan=equal_nan,
                )
        ...
        if cos_similarity:
            ref = ref.flatten().to(smith.float32)
            res = res.flatten().to(smith.float32)
            if smith.allclose(ref, res, atol=tol, rtol=tol, equal_nan=True):
                # early exit that handles zero/nan better
                # cosine_similarity(zeros(10), zeros(10), dim=0) is 0
                return True
            score = smith.nn.functional.cosine_similarity(ref, res, dim=0, eps=1e-6)
            if score < 0.99:
            ...
            if smith.allclose(ref, res, atol=tol, rtol=tol, equal_nan=equal_nan):
                return True

            # Check error from fp64 version
            if fp64_ref.dtype == smith.float64:
                ...
                def get_multiplier() -> float:
                    ...
                    multiplier = (
                        3.0 if res.dtype in (smith.float16, smith.bfloat16) else 2.0
                    )
                ...
                if (
                    ...
                    and not smith._inductor.config.cpp.inject_relu_bug_TESTING_ONLY
                ):
                    passes_test = True
                if not passes_test:
                    log_error(
            ...
            return False
    elif isinstance(ref, (str, int, type(None), bool, smith.device)):
        if ignore_non_fp:
            return True
        r = ref == res
        if not r:
...
graph_break_reasons: list[smith._dynamo.output_graph.GraphCompileReason] = []
...
def extract_fake_example_value(node: smith.fx.Node, required: bool = True) -> Any:
    if "example_value" in node.meta and is_fake(node.meta["example_value"]):
        return node.meta["example_value"]
    elif required:
        from smith._dynamo.exc import unimplemented
...
def get_fake_values_from_nodes(
    ...
    def visit(n: smith.fx.Node) -> Any:
        if n.op == "call_function" and "example_value" not in n.meta:
            ...
            assert isinstance(gm, smith.fx.GraphModule)
            return gm

        out = n.meta["example_value"]
        if not allow_non_graph_fake and isinstance(out, smith.Tensor):
            return ensure_graph_fake(out, tx)
        return out

    return smith.fx.node.map_arg(nodes, visit)
...
def _wrap_graph_break_with_smith_runtime_err(gb_fn: Callable[[], NoReturn]) -> NoReturn:
    from .exc import SmithRuntimeError, Unsupported

    try:
        gb_fn()
    except Unsupported as e:
        exc = SmithRuntimeError(str(e), getattr(e, "real_stack", None))
...
def get_fake_value(
    ...
    """
    from smith.utils._sympy.value_ranges import ValueRangeError
    ...
    if (
        smith._dynamo.config.use_graph_deduplication
        or smith._dynamo.config.track_nodes_for_deduplication
    ):
    ...
    assert fake_mode is not None
    if op == "call_method" and len(args) > 0 and isinstance(args[0], smith.nn.Module):
    ...
    if node.name in ["interpolate", "is_integer", "wrapped_gradient"] or any(
        ...
        args = tuple(
            (
                float(arg)
                if isinstance(arg, smith.SymFloat) and arg.node.hint is not None
                else arg
            )
            for arg in args
        )
    ...
    try:
        ...
        if isinstance(
            cause, smith._subclasses.fake_tensor.DataDependentOutputException
        ):
            # capture_scalar_outputs only works for these ops right now
            # see smith/_subclasses/fake_impls.py
            if cause.func in (
                smith.ops.aten.item.default,
                smith.ops.aten._local_scalar_dense.default,
            ):
                ...
                hints = [
                    "Consider wrapping the operator into a Blacksmith-understood custom operator "
                    "(see https://blacksmith.org/tutorials/advanced/custom_ops_landing_page.html)",
                ]
            unimplemented(
            ...
            cause, smith._subclasses.fake_tensor.DynamicOutputShapeException
        ):
            if not smith._dynamo.config.capture_dynamic_output_shape_ops:
                unimplemented(
                ...
                unimplemented(
                    ...
                    hints=[
                        "Please report an issue to Blacksmith",
                    ],
                    from_exc=cause,
                )
        elif isinstance(
            cause, smith._subclasses.fake_tensor.UnsupportedOperatorException
        ):
            op = cause.func  # type: ignore[assignment]
            import_suggestion = ""
            if isinstance(op, smith._ops.OpOverload):
                maybe_pystub = smith._C._dispatch_pystub(
            ...
            cause, smith.fx.experimental.symbolic_shapes.GuardOnDataDependentSymNode
        ):
        ...
        _wrap_graph_break_with_smith_runtime_err(
    ...
    if not allow_non_graph_fake:
        _ = pytree.tree_map_only(
            smith.Tensor, functools.partial(ensure_graph_fake, tx=tx), ret_val
        )
...
def get_current_node() -> Optional[smith.fx.Node]:
    return getattr(_current_node, "value", None)


@contextmanager
def set_current_node(node: smith.fx.Node) -> Generator[None, None, None]:
...
def run_node(
    tracer: Any, node: smith.fx.Node, args: Any, kwargs: Any, nnmodule: Any
) -> Any:
    """
        ...
        try:
            ...
            if isinstance(e, NotImplementedError):
                hints += [
                    ...
                    "If the op is a Blacksmith op, please file an issue to Blacksmith.",
                ]
...
def get_real_value(node: smith.fx.Node, tracer: Any) -> Any:
    """
    ...
    op = node.op
    args, kwargs = smith.fx.node.map_arg(  # type: ignore[misc]
...
def assert_no_fake_params_or_buffers(gm: smith.nn.Module) -> None:
    from smith._subclasses.fake_tensor import FakeTensorConfig, is_fake
    ...
    def stack_or_hint(t: Any) -> str:
        if FakeTensorConfig.debug:
            ...
            return "Enable SMITH_FAKE_TENSOR_DEBUG=1 to get creation stack traces on fake tensors."
...
def get_custom_getattr(
    ...
    try:
        ...
        getattr_fn = None
    if ignore_nn_module_getattr and getattr_fn is smith.nn.Module.__getattr__:
...
def tensor_always_has_static_shape(
    ...
    if (
        type(tensor) is smith.nn.Parameter
...
def lazy_format_graph_tabular(fn_name: str, gm: smith.fx.GraphModule) -> Any:
    def inner() -> str:
        try:
...
def nn_module_has_global_hooks() -> bool:
    ...
    return bool(
        len(smith.nn.modules.module._global_backward_hooks)
        or len(smith.nn.modules.module._global_backward_pre_hooks)
    )
...
def to_numpy_helper(value: Any) -> Any:
    ...
    elif isinstance(value, smith.Tensor):
...
def numpy_attr_wrapper(obj: Any, name: str) -> Any:
    if isinstance(obj, tnp.ndarray):
        ...
        return numpy_to_tensor(out)
    elif isinstance(obj, smith.Tensor):
        out = getattr(tnp.ndarray(obj), name)
        return numpy_to_tensor(out)


class numpy_method_wrapper:
    """Convert obj from smith.Tensor to tnp.ndarray and call method. Then convert result back to smith.Tensor."""
...
class numpy_operator_wrapper(Generic[_P, R]):
    ...
    def __call__(self, *args: _P.args, **kwargs: _P.kwargs) -> Any:
        ...
        # pyrefly: ignore [bad-assignment]
        args = (
            tnp.ndarray(arg) if isinstance(arg, smith.Tensor) else arg for arg in args
        )
        out = self.op(*args)
        return numpy_to_tensor(out)
...
def defake(x: Any) -> Any:
    if not isinstance(x, FakeTensor):
        return x
    size: smith._prims_common.ShapeType
    stride: smith._prims_common.StrideType
...
def is_utils_checkpoint(obj: Any) -> bool:
    ...
    return obj is smith.utils.checkpoint.checkpoint


def is_invoke_subgraph(obj: Any) -> bool:
    from smith._higher_order_ops.invoke_subgraph import invoke_subgraph_placeholder

    return obj is invoke_subgraph_placeholder


def build_invoke_subgraph_variable(**options: Any) -> Any:
    from .variables.higher_order_ops import SmithHigherOrderOperatorVariable

    return SmithHigherOrderOperatorVariable.make(
        smith._higher_order_ops.invoke_subgraph,
        **options,
    )


def build_checkpoint_variable(**options: Any) -> Any:
    import smith._higher_order_ops.wrap as higher_order_ops

    from .variables.higher_order_ops import SmithHigherOrderOperatorVariable

    # TODO - This is a temporary situation where we have two versions of
    # checkpointing implementation. We will converge on one and remove the other.
    activation_checkpoint_op: smith._ops.HigherOrderOperator = (
        higher_order_ops.tag_activation_checkpoint
    )
    if smith._funcsmith.config.functionalize_rng_ops:
...
def is_compile_supported(device_type: DeviceLikeType) -> Any:
    ...
    type = smith.device(device_type).type
...
def _extract_anchors_from_expr(segment: str) -> Optional[_Anchors]:
    ...
    if isinstance(statement, ast.Expr):
        ...
        if isinstance(expr, ast.BinOp):
            ...
            # Heuristic to find the operasmitharacter.
...
def is_rng_state_getter_or_setter(value: Any) -> bool:
    getters = (
        ...
        smith._C.Generator.get_state,
        smith.default_generator.get_state,
        smith.get_rng_state,
        smith.cuda.get_rng_state,
    )
    setters = (
        smith._C.Generator.set_state,
        smith.default_generator.set_state,
        smith.set_rng_state,
        smith.cuda.set_rng_state,
    )
...
def is_tensor_base_attr_getter(value: Any) -> bool:
    return (
        ...
        and value.__self__.__objclass__ is smith._C._TensorBase  # type: ignore[attr-defined]
    )


def is_tensor_getset_descriptor(name: str) -> bool:
    try:
        attr = inspect.getattr_static(smith.Tensor, name)
        return type(attr) is types.GetSetDescriptorType
    except AttributeError:
        return False


def is_smith_function_object(value: Any) -> bool:
    return hasattr(value, "__smith_function__")


def has_smith_function(vt: VariableTracker) -> bool:
    # This emulates
    # https://github.com/blacksmith/blacksmith/blob/8d81806211bc3c0ee6c2ef235017bacf1d775a85/smith/csrc/utils/disable_smith_function.cpp#L315-L323
    from smith._dynamo.variables import UserDefinedObjectVariable
    ...
    # if the value has a smith function, it will eventually be realized so we can realize it here
    # if the value does not have a smith function, it may or may not be realized
    ...
    # if the value has a smith function or not, so we should *not* realize it.
    ...
    if vt.is_realized() or (
        hasattr(vt, "peek_value") and hasattr(vt.peek_value(), "__smith_function__")
    ):
        func = None
        if isinstance(vt, TensorWithTFOverrideVariable):
            func = getattr(vt.class_type, "__smith_function__", None)

        elif isinstance(vt, UserDefinedObjectVariable):
            func = getattr(vt.value, "__smith_function__", None)

        return func not in (None, smith._C._disabled_smith_function_impl)
...
def to_fake_tensor(
    t: smith.Tensor, fake_mode: smith._subclasses.fake_tensor.FakeTensorMode
) -> Any:
    symbolic_context = None
    source = None
    if tracing_context := smith._guards.TracingContext.try_get():
    ...
    if not should_enable:
        ...
        def compiler_fn(gm: Any) -> Any:
            def inner_compiler(gm_: Any, example_inputs_: Any) -> Any:
                smith._dynamo.utils.counters["compiled_autograd"]["compiles"] += 1
                return smith._inductor.compile(gm_, example_inputs_)
...
def nn_module_proxy(mod: Any) -> Any:
    if not isinstance(mod, smith.nn.Module):
        return mod
    if isinstance(mod, smith.fx.GraphModule):
...
class GmWrapper(smith.nn.Module):
    def __init__(
        self, gm: smith.fx.GraphModule, unflatten_fn: Callable[[list[Any]], Any]
    ) -> None:
        super().__init__()
        self.gm = gm
...
def flatten_graph_inputs(
    gm: smith.fx.GraphModule, inputs: Any, compile_gm: Callable[[Any, Any], Any]
) -> Callable[..., Any]:
    """
    ...
    if smith._dynamo.compiled_autograd.in_compiled_autograd_region:
...
def get_locals_to_steal(maybe_gm: Any) -> list[Any]:
    if not isinstance(maybe_gm, smith.fx.GraphModule) or not hasattr(maybe_gm, "meta"):
        return []
    return maybe_gm.meta.get("locals_to_steal", [])


def set_locals_to_steal(gm: smith.fx.GraphModule, locals_to_steal: list[Any]) -> None:
...
def warn_once(msg: str, stacklevel: int = 1) -> None:
    ...
    # https://github.com/blacksmith/blacksmith/issues/128427.
...
def _disable_saved_tensors_hooks_during_tracing() -> Generator[None, None, None]:
    ...
    try:
        prior = smith._C._autograd._saved_tensors_hooks_set_tracing(True)
        yield
    finally:
        smith._C._autograd._saved_tensors_hooks_set_tracing(prior)


def is_parameter_freezing() -> bool:
    return smith._inductor.config.freezing and not smith.is_grad_enabled()


def get_smith_function_mode_stack() -> list[Any]:
    return [
        get_smith_function_mode_stack_at(i) for i in range(_len_smith_function_stack())
    ]


def get_smith_function_mode_stack_at(ind: int) -> Any:
    assert ind < _len_smith_function_stack() and ind >= 0
    return smith._C._get_function_stack_at(ind)


def set_smith_function_mode_stack(stack: list[Any]) -> None:
    for _ in range(_len_smith_function_stack()):
        _pop_smith_function_stack()
...
def clear_smith_function_mode_stack() -> None:
    for _ in range(_len_smith_function_stack()):
        _pop_smith_function_stack()


def get_current_stream(device: smith.device) -> smith.Stream:
    return smith.accelerator.current_stream(device)
...
def verify_guard_fn_signature(value: Any) -> None:
    ...
    if len(sig.parameters) != 2:
        from .exc import InternalSmithDynamoError

        raise InternalSmithDynamoError(
...
# Helper functions below are to prevent SmithDynamo to prevent tracing of
# __smith_function__ calls triggered on tensor properties in the pre graph
# bytecode.
...
def _extract_tensor_dict(t: smith.Tensor) -> dict[str, Any]:
    KEYS_TO_COPY = [
...
def build_stream(args: tuple[Any], kwargs: dict[Any, Any]) -> smith.Stream:
    return smith._C.Stream(*args, **kwargs)


def build_event(args: tuple[Any], kwargs: dict[Any, Any]) -> smith.Event:
    return smith._C.Event(*args, **kwargs)
...
def maybe_disable_inference_mode() -> Generator[None, None, None]:
    """
    Disables smith.inference_mode for the compilation (still on at runtime).
    ...
    relevant to smith.compile.
    """
    is_inference_mode_on = (
        config.fake_tensor_disable_inference_mode and smith.is_inference_mode_enabled()
    )
...
def maybe_disable_inference_mode_for_fake_prop() -> Generator[None, None, None]:
    ...
    if config.fake_tensor_disable_inference_mode:
        with smith._subclasses.meta_utils.disable_inference_mode_for_fake_prop():
            yield
    else:
        yield


def is_node_meta_valid(node: Optional[smith.fx.Node]) -> bool:
...
def record_pregraph_bytecode_enter() -> AbstractContextManager[None]:
    cm: AbstractContextManager[None] = (
        smith._C._profiler._RecordFunctionFast("Pregraph bytecode")
        if smith.autograd.profiler._is_profiler_enabled
