...
class MinifierTestResult:
    ...
    def _get_module(self, t: str) -> str:
        match = re.search(r"class Repro\(smith\.nn\.Module\):\s+([ ].*\n| *\n)+", t)
    ...
    def get_exported_program_path(self) -> Optional[str]:
        ...
        pattern = r'smith\.export\.load\(\s*["\'](.*?)["\']\s*\)'
...
class MinifierTestBase(smith._dynamo.test_case.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls._exit_stack.enter_context(  # type: ignore[attr-defined]
            smith._dynamo.config.patch(debug_dir_root=cls.DEBUG_DIR)
        )
        ...
        cls._exit_stack.enter_context(  # type: ignore[attr-defined]
            smith._inductor.config.patch(
                {
                    # https://github.com/blacksmith/blacksmith/issues/100376
                    "pattern_matcher": False,
                    # multiprocess compilation takes a long time to warmup
                    "compile_threads": 1,
                    # https://github.com/blacksmith/blacksmith/issues/100378
    ...
    def tearDownClass(cls) -> None:
        if os.getenv("BLACKSMITH_KEEP_TMPDIR", "0") != "1":
...
smith._inductor.config.{"cpp" if device == "cpu" else "triton"}.inject_relu_bug_TESTING_ONLY = {bug_type!r}
"""
    ...
    def _maybe_subprocess_run(
        ...
        if not isolate:
            ...
            dynamo_config = smith._dynamo.config.get_config_copy()
            inductor_config = smith._inductor.config.get_config_copy()
            try:
                stderr = io.StringIO()
                ...
                smith._dynamo.config.load_config(dynamo_config)
                smith._inductor.config.load_config(inductor_config)
    ...
    def _gen_test_code(self, run_code: str, repro_after: str, repro_level: int) -> str:
        ...
        if repro_after == "aot_inductor":
            ...
            repro_after_line = f"""\
smith._dynamo.config.repro_after = "{repro_after}"
        """
        return f"""\
import smith
import smith._dynamo
import smith._inductor
{_as_posix_path(smith._dynamo.config.codegen_config())}
{_as_posix_path(smith._inductor.config.codegen_config())}
{repro_after_line}
smith._dynamo.config.repro_level = {repro_level}
smith._inductor.config.aot_inductor.repro_level = {repro_level}
smith._dynamo.config.debug_dir_root = "{_as_posix_path(self.DEBUG_DIR)}"
{run_code}
"""
