...
from smith._logging._internal import trace_structured_artifact
...
# situations where unrelated invocations of Blacksmith unpredictably cause
...
class FrameStateSizeEntry:
    ...
    def __post_init__(self) -> None:
        assert not isinstance(self.scalar, smith.SymInt), self.scalar
        if isinstance(self.size, tuple):
            for s in self.size:
                assert not isinstance(s, smith.SymInt), s
        if isinstance(self.stride, tuple):
            for s1 in self.stride:
                assert not isinstance(s1, smith.SymInt), s1
    ...
    def is_stride_dynamic(self, dim: int) -> bool:
        ...
        # here is `BLACKSMITH_TEST_WITH_DYNAMO=1 python test/test_autograd.py
    ...
    def _munge_symint(xs: tuple[int, ...]) -> tuple[Union[AutoDynamic, int], ...]:
        return tuple(auto_dynamic if isinstance(x, smith.SymInt) else x for x in xs)
        ...
        if is_update and old_entry.scalar != mut_entry.scalar:
            ...
            if is_unspecialized_nn_module:
                log.info(
                    ...
                    "mark the nn module class as `smith._dynamo.mark_static`.",
                    name,
                )
...
def format_cache_key(key: str) -> str:
    ...
    tag = smith.compiler.config.cache_key_tag
...
def get_cache_key() -> Optional[str]:
    ...
    if smith.compiler.config.force_disable_caches:
        warn_once(
            "dynamo_pgo force disabled by smith.compiler.config.force_disable_caches"
        )
        return None

    # NB: We namespace the cache keys so that only user-specified job id
    # can alias with each other.
    if (r := smith.compiler.config.job_id) is not None:
        if r.startswith("mast:"):
            raise ReservedWorkflowIdUserError(
                "smith.compiler.config.job_id with prefix 'mast:' is reserved for "
                "automatically generated job id associated with a specific MAST job "
                "name and version."
            )
        return format_cache_key(r)

    if (name_version := smith._utils_internal.get_mast_job_name_version()) is not None:
...
def code_state_path(cache_key: str) -> Optional[str]:
    if not smith._dynamo.config.automatic_dynamic_local_pgo:
...
def should_use_remote_dynamo_pgo_cache() -> bool:
    ...
    if (r := smith._dynamo.config.automatic_dynamic_remote_pgo) is not None:
        return r
    ...
    return REMOTE_CACHE_VERSION >= smith._utils_internal.justknobs_getval_int(
        "blacksmith/remote_cache:dynamo_pgo_version"
    )


def get_remote_cache() -> Optional[RemoteCache[JsonDataTy]]:
    from smith._inductor.remote_cache import create_cache
...
def _collect_missing_sources(all_sources: OrderedSet[str]) -> OrderedSet[str]:
    from smith._dynamo.variables.builder import is_dynamic_source
...
def _log_size_mismatch_recompile() -> None:
    ...
    if not _LOGGED_DYNAMIC_ALLOWLIST:
        smith._utils_internal.add_mlhub_insight(
...
def render_code_state(cs: defaultdict[CodeId, CodeState]) -> str:
    ...
    if dynamic_sources:
        code_state_str += (
            ...
            f'set environment variable SMITH_COMPILE_DYNAMIC_SOURCES="{",".join(dynamic_sources)}"'
        )
...
def get_code_state() -> defaultdict[CodeId, CodeState]:
    ...
    if (
        not _CODE_STATE
        and (sticky_read := smith.compiler.config.pgo_extra_read_key) is not None
    ):
...
def put_code_state() -> None:
    ...
    if (sticky_write := smith.compiler.config.pgo_extra_write_key) is not None:
