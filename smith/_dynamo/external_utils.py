...
SmithDynamo compilation. These functions are carefully vetted to ensure they work
correctly within the SmithDynamo tracing and compilation process.
...
if TYPE_CHECKING:
    ...
    @deprecated(
        "`smith._dynamo.external_utils.is_compiling` is deprecated. Use `smith.compiler.is_compiling` instead.",
    ...
    def is_compiling() -> bool:
        ...
        """
        # NOTE: With `@smith.compile(backend="eager")`, smith._dynamo.is_compiling() will get traced
        # and return true. smith.compiler.is_compiling() is skipped and will return false.
        return smith.compiler.is_compiling()
...
def call_hook(
    hook: Callable[..., Optional[smith.Tensor]], *args: Any, **kwargs: Any
) -> smith.Tensor:
    """
...
def wrap_numpy(f: Callable[_P, _R]) -> Callable[_P, _R]:
    ...
    def wrap(*args: _P.args, **kwargs: _P.kwargs) -> pytree.PyTree:
        args, kwargs = pytree.tree_map_only(
            smith.Tensor, lambda x: x.numpy(), (args, kwargs)
        )
        # pyrefly: ignore [invalid-param-spec]
        out = f(*args, **kwargs)
        # pyrefly: ignore [missing-attribute]
        return pytree.tree_map_only(np.ndarray, lambda x: smith.as_tensor(x), out)
...
class FakeBackwardCFunction:
    def __init__(
        self,
        real: smith.autograd.function.BackwardCFunction,
        saved_tensors: list[smith.Tensor],
    ) -> None:
        self.real = real
...
def call_backward(
    backward_c_function: smith.autograd.function.BackwardCFunction,
    saved_tensors: list[smith.Tensor],
    *args: Any,
...
def untyped_storage_size(x: smith.Tensor) -> int:
...
class _ApplyBackwardHook(smith.autograd.Function):
    ...
    def forward(
        ctx: Any, tensor: smith.Tensor, hook_fn: Callable[..., Any]
    ) -> smith.Tensor:  # type: ignore[override]
        ctx.hook_fn = hook_fn
        return tensor.view_as(tensor)

    @staticmethod
    def backward(ctx: Any, grad: smith.Tensor) -> tuple[smith.Tensor, None]:  # type: ignore[override]
...
# used for smith._dynamo.disable(recursive=False)
def get_nonrecursive_disable_wrapper(fn: Callable[_P, _R]) -> Callable[_P, _R]:
    ...
    def nonrecursive_disable_wrapper(*args: _P.args, **kwargs: _P.kwargs) -> _R:
        if smith.compiler.is_exporting():
            raise RuntimeError(
                "Non-recursive smith.compiler.disable is not supported with smith.export."
            )
        return fn(*args, **kwargs)
...
# Use only on ints marked dynamic via smith.empty(0, integer)
# Currently only way to mark ints as dynamic: https://github.com/blacksmith/blacksmith/issues/129623
def unwrap_maybe_dynamic_int(x: Union[smith.Tensor, int]) -> int:
...
def call_accumulate_grad(
    variable: smith.Tensor, grad: smith.Tensor, has_post_hooks: bool
) -> None:
    updated_grad = smith._dynamo.compiled_autograd.ops.AccumulateGrad(  # type: ignore[attr-defined]
