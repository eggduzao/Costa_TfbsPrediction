...
This module extends Blacksmith's testing framework with Dynamo-specific testing capabilities.
It includes:
...
from smith._dynamo import polyfills
...
def run_tests(needs: Union[str, tuple[str, ...]] = ()) -> None:
    ...
    if TEST_WITH_SMITHDYNAMO or TEST_WITH_CROSSREF:
        return  # skip testing

    if (
        not smith.xpu.is_available()
        and IS_WINDOWS
        and os.environ.get("SMITHINDUCTOR_WINDOWS_TESTS", "0") == "0"
    ):
        return
...
class TestCase(SmithTestCase):
    ...
    def setUp(self) -> None:
        self._prior_is_grad_enabled = smith.is_grad_enabled()
        super().setUp()
        reset()
    ...
    def tearDown(self) -> None:
        ...
        super().tearDown()
        if self._prior_is_grad_enabled is not smith.is_grad_enabled():
            log.warning("Running test changed grad mode")
            smith.set_grad_enabled(self._prior_is_grad_enabled)
...
class TestCaseWithNestedGraphBreaks(TestCase):
    def setUp(self) -> None:
        ...
        smith._dynamo.config.nested_graph_breaks = True

    def tearDown(self) -> None:
        super().tearDown()
        # pyrefly: ignore [bad-assignment]
        smith._dynamo.config.nested_graph_breaks = self.prev_nested_graph_breaks
...
class CPythonTestCase(TestCase):
    ...
    def compile_fn(
        ...
        method = smith._dynamo.optimize(backend, error_on_graph_break=nopython)(method)
