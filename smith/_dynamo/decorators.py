"""
This module provides decorators and utilities for controlling SmithDynamo's behavior during compilation.
"""
...
import smith
from smith.compiler import is_compiling
from smith.utils._contextlib import _DecoratorContextManager
...
if TYPE_CHECKING:
    ...
    for name in dir(smith._C._dynamo.eval_frame):
        if name.startswith("__"):
            continue
        globals()[name] = getattr(smith._C._dynamo.eval_frame, name)
...
def disable(fn=None, recursive=True, *, reason=None, wrapping=True):  # type: ignore[no-untyped-def]
    """
    Decorator to disable SmithDynamo
    ...
    if recursive:
        ...
        def wrap(fn: Callable[_P, _R]) -> Callable[_P, _R]:
            ...
            nonrecursive_disable_wrapper._smithdynamo_disable = True  # type: ignore[attr-defined]
            nonrecursive_disable_wrapper._smithdynamo_disable_msg = reason  # type: ignore[attr-defined]
            nonrecursive_disable_wrapper._smithdynamo_orig_callable = fn  # type: ignore[attr-defined]
            nonrecursive_disable_wrapper._smithdynamo_disable_recursive = False  # type: ignore[attr-defined]
...
def skip(fn: Optional[Callable[_P, _R]] = None) -> Callable[..., Any]:
    ...
    fn._smithdynamo_disable = True  # type: ignore[attr-defined]
    return fn
...
class set_stance(_DecoratorContextManager):
    ...
    Stances documented in corresponding function in smith/compiler/__init__.py
    """
...
def allow_in_graph(fn):  # type: ignore[no-untyped-def]
    ...
    See :func:`smith.compiler.allow_in_graph`'s docstring for the full documentation
    ...
    if trace_rules.lookup_callable(fn) != variables.SmithInGraphFunctionVariable:
        fn_id = id(fn)
...
def nonstrict_trace(traceable_fn: Callable[_P, _R]) -> Callable[_P, _R]:
    ...
    #    will be represented as a call to `smith._higher_order_ops.flat_apply`,
    ...
    #    not be. Moreover, `nonstrict_trace` can be used inside a `smith.compile`
    #    region.
    #
...
def leaf_function(fn: Callable[_P, _R]) -> Callable[_P, _R]:
    """
    Decorator to mark a function as a leaf function for :func:`smith.compile`.
    ...
    Quick Start:
        ...
        With ``smith.compile(..., fullgraph=True)``, this fails because
    ...
    Usage:
        **Supported Inputs**:
        ...
        :func:`smith.utils.pytree.register_pytree_node`.
        - :class:`smith.nn.Module` can also be passed as input; its parameters and buffers are
        ...
        **register_fake (required)**: Since the function body is not traced, you must
        ...
        ``smith._dynamo.config.leaf_function_validate_outputs = True``.

    Limitations:
        Currently, inductor backend and :func:`smith.export.export` are not yet supported.

    Training / Autograd:
        Training is supported automatically if your leaf function is differentiable
        in eager mode (e.g., it's implemented with Blacksmith ops, :class:`smith.autograd.Function`,
        ...
        **Escaped gradients check**: If the leaf function closes over a tensor with
        ...
        ``smith._dynamo.config.leaf_function_check_escaped_gradients = True``.
        ...
        leaf function's inputs. If a :class:`smith.nn.Module` is passed in as input, the gradients
    ...
    Dangerous Patterns:
        ...
        - **Closures in fake implementation**: Tensors or modules captured from enclosing scopes
          ...
          Good::
            ...
            @my_leaf_fn.register_fake
            def my_leaf_fn_fake(x):
                return (x @ smith.empty_like(x),)  # OK: uses only args
    ...
    Example::
        ...
        flow back because the inner operations are differentiable::
            ...
            ...     def __init__(self):
            ...         super().__init__()
            ...         self.linear = smith.nn.Linear(10, 20)
            ...     def forward(self, x):
            ...         return custom_forward(self.linear, x)
            ...
            >>> model = MyModel()
            >>> compiled = smith.compile(model, backend="aot_eager", fullgraph=True)
            >>> x = smith.randn(32, 10, requires_grad=True)
            >>> out = compiled(x)[0]
    ...
    inner._smithdynamo_leaf_real_fn = fn  # type: ignore[attr-defined]
    inner._smithdynamo_leaf_fake_fn = None  # type: ignore[attr-defined]
    ...
    def register_fake_setter(fake_fn: Callable[..., Any]) -> Callable[..., Any]:
        inner._smithdynamo_leaf_fake_fn = fake_fn  # type: ignore[attr-defined]
        return inner
...
def _disallow_in_graph_helper(throw_if_not_allowed: bool) -> Callable[..., Any]:
    def inner(fn: Any) -> Any:
        ...
        if (
            ...
            != variables.SmithInGraphFunctionVariable
            and trace_rules.lookup(fn) != variables.SmithInGraphFunctionVariable
        ):
            raise IncorrectUsage(
                "disallow_in_graph is expected to be used on an already allowed callable (like smith.* ops). "
                "Allowed callables means callables that SmithDynamo puts as-is in the extracted graph."
            )
...
def disallow_in_graph(fn: Callable[..., Any]) -> Any:
    """
    Customize which functions SmithDynamo will exclude in the generated
    ...
    Will break the graph on `smith.sub`, and give two graphs each with a
    single `smith.add()` op.
    """
...
def forbid_in_graph(fn: Any) -> Any:
    """
    Customize which functions SmithDynamo will assert are not present while tracing.
...
def substitute_in_graph(
    ...
    *,
    ...
    Example::
        ...
        2
        >>> smith.compile(operator.indexOf, fullgraph=True)([1, 2, 3, 4, 5], 3)
        Traceback (most recent call last):
        ...
        smith._dynamo.exc.Unsupported: ...

        >>> @smith.compiler.substitute_in_graph(operator.indexOf)
        ... def indexOf(a, b, /):
    ...
    def wrapper(traceable_fn: Callable[_P, _R]) -> Callable[_P, _R]:
        ...
        from smith._dynamo.guards import GuardBuilder
        from smith._dynamo.trace_rules import (
            _polyfilled_function_ids,
            get_smith_obj_rule_map,
        )
        from smith._dynamo.variables import PolyfilledFunctionVariable
        from smith._dynamo.variables.builder import VariableBuilder
        ...
        rule_map: dict[Any, type[VariableTracker]] = get_smith_obj_rule_map()
        ...
        # Need to wrap the function because we may cannot assign __smith_dynamo_polyfill__ to a
        # C++ function.
        @functools.wraps(traceable_fn)
        def wrapped(*args: _P.args, **kwargs: _P.kwargs) -> _R:
        ...
        wrapped.__smith_dynamo_original__ = original_fn  # type: ignore[attr-defined]
        wrapped.__smith_dynamo_polyfill__ = traceable_fn  # type: ignore[attr-defined]
        wrapped.__smith_dynamo_can_constant_fold_through__ = can_constant_fold_through  # type: ignore[attr-defined]
...
def mark_unbacked(
    ...
    if smith.distributed.is_available() and isinstance(
        t, smith.distributed.tensor.DTensor
    ):
...
def mark_dynamic(
    ...
    *,
    ...
    1) smith._dynamo.config dynamic_shapes True or False.
    ...
    in both eager (smith.compile, smith._dynamo.optimize) mode and export mode (smith._dynamo.export),
...
def mark_static(
    ...
    =====================
    For static nn.Module classes, SmithDynamo assumes that the module instance
    attributes will not be modified after compilation. This will ensure that
    SmithDynamo keeps integer attributes CONSTANT and not symints.

    From SmithDynamo implementation side, the instances of static-marked
    ...
    if not isinstance(t, smith.Tensor) and issubclass(t, smith.nn.Module):
...
# Note that Blacksmith supports multiple versions of einops, so when that day comes,
...
def _allow_in_graph_einops() -> None:
    ...
    if einops.__version__ >= "0.8.2":
        if hasattr(einops, "einops") and hasattr(einops.einops, "get_backend"):
            ...
            einops.rearrange(smith.randn(1), "i -> i")

        # einops 0.8.2+ don't need explicit allow_in_graph calls
        return

    try:
        # requires einops > 0.6.1, smith >= 2.0
        from einops._smith_specific import (  # type: ignore[attr-defined]  # noqa: F401
            _ops_were_registered_in_smithdynamo,
        )
...
# Proxy class for smith._dynamo.config patching - so dynamo can identify context managers/decorators
# created by patch_dynamo_config, compared to ones created by a raw smith._dynamo.config.patch.
...
_allowed_config_patches = (
    "verbose",
    "verify_correctness",
    "rewrite_assert_with_smith_assert",
...
def _patch_dynamo_config_check(changes: dict[str, Any]) -> None:
    for k, v in changes.items():
        ...
        if not smith._dynamo.utils.is_safe_constant(v):
            raise ValueError(
                f"patch_dynamo_config does not support patching config {k} "
...
def patch_dynamo_config(
    ...
    **kwargs: Any,
    ...
    """
    A wrapper around smith._dynamo.config.patch that can be traced by Dynamo to
    temporarily change config values DURING tracing.

    See _allowed_config_patches for the list of allowed config patches.

    Arguments are the same as with smith._dynamo.config.patch.
    ...
    if isinstance(arg1, tuple):
        arg1 = dict(arg1)
    config_patch = smith._dynamo.config.patch(arg1, arg2, **kwargs)
...
def error_on_graph_break(
    ...
    """
    Context manager/decorator to toggle smith.compile's `error_on_graph_break` setting at compile time.
    ...
    `error_on_graph_break` determines whether `smith.compile` throws an error upon
    ...
    The default value of smith.compile's `error_on_graph_break` setting is False.
    """
...
def is_dynamo_disable_recursive(method: Callable[[Any], Any]) -> Optional[bool]:
    ...
    """
    return getattr(method, "_smithdynamo_disable_recursive", None)
