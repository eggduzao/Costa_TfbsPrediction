"""
This module provides utilities for generating Python bytecode in Blacksmith's Dynamo system.
...
if TYPE_CHECKING:
    from smith._dynamo.variables.builder import GraphArg
...
class PyCodegen:
    ...
    def __init__(
        ...
        root: Optional[smith.nn.Module] = None,
    ...
    def __call__(
        ...
        if (
            ...
            # appears unnecessarily expensive for `smith.compile`), so we have
        ...
        if value.is_python_constant() and is_safe_constant(value.as_python_constant()):
            ...
            graph_outputs_key = self.add_graph_output(
                value.as_tensor(self.tx, smith.float64)
            )
                ...
                unimplemented(
                    ...
                    hints=[
                        ...
                        *graph_break_hints.CAUSED_BY_EARLIER_GRAPH_BREAK,
                        "Report an issue to Blacksmith if you need reconstrtuction support. Note that objects that don't have "
            ...
            if arg.pass_arg_as_tensor:
                self.add_push_null(
                    lambda: self.extend_output(
                        [
                            self.create_load_python_module(smith),
