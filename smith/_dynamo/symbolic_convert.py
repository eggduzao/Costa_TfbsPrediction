"""
Core module responsible for converting Python bytecode into SmithDynamo's symbolic execution format.

This module implements the bytecode-level tracing system that allows SmithDynamo to analyze
...
This is a core part of SmithDynamo's tracing system that enables ahead-of-time
optimization of Blacksmith programs.
"""
...
from smith._dynamo.dynamo_profiler import DynamoProfilerState, FunctionTraceTiming
from smith._dynamo.exc import ObservedException, TensorifyScalarRestartAnalysis
from smith._guards import InlinedCodeCache, tracing, TracingContext
from smith._logging.structured import dump_file
from smith.fx.experimental.symbolic_shapes import guard_bool
from smith.utils._functools import cache_method

from . import (
    config,
    exc,
    graph_break_hints,
    logging as smithdynamo_logging,
    trace_rules,
    variables,
)
...
from .variables.smith_function import (
    SymbolicSmithFunctionState,
...
trace_source_log = smith._logging.getArtifactLogger(__name__, "trace_source")
trace_bytecode_log = smith._logging.getArtifactLogger(__name__, "trace_bytecode")
tls = threading.local()
...
Otherwise, please submit a bug report, ideally including the contents of SMITH_LOGS=+dynamo
"""
            )
        self.index += 1
        return entry
...
def generic_jump(
        ...
        unimplemented(
            ...
            hints=[
                *graph_break_hints.FUNDAMENTAL,
                "Use `smith.cond` to express dynamic control flow.",
            ],
        )
    ...
    def inner(self: InstructionTranslatorBase, inst: Instruction) -> None:
        ...
        if (
            config.rewrite_assert_with_smith_assert
            ...
            if value.is_python_constant():
                if bool(value.as_python_constant()):
                    ...
                    unimplemented(
                        ...
                        hints=[
                            *graph_break_hints.FUNDAMENTAL,
                            "Use `smith._assert()` to raise a hard AssertionError when the check fails. "
            ...
            # Manually insert smith._assert_async instead of python assert and jump over
            ...
            if isinstance(value, SymNodeVariable):
                ...
                if not isinstance(sym_expr, smith.SymBool):
                    sym_expr = sym_expr != 0

                result = smith.fx.experimental.symbolic_shapes.expect_true(sym_expr)
                if not result:
                    unimplemented(
            ...
            scalar_to_tensor_proxy = self.output.create_proxy(
                "call_function", smith.scalar_tensor, *proxy_args_kwargs((value,), {})
            )
        ...
        if value.is_python_constant():
            ...
            try:
                ...
                if isinstance(value.sym_num, smith.SymBool):
...
def _reconstruct_block_stack(
    ...
    for tx in reversed(all_txes):
        for b in tx.block_stack:
            ...
            if isinstance(b.with_context, SmithFunctionModeVariable):
                cg.extend_output(
                    b.resume_fn().try_except_smith_function_mode(
...
class InstructionTranslatorBase(
    ...
    symbolic_smith_function_state: SymbolicSmithFunctionState
        ...
        try:
            ...
            if (
                ...
                if isinstance(e, StepUnsupported):
                    unimplemented(
                        gb_type="cannot resume from smith._dynamo.step_unsupported()",
                        context="",
                        explanation="traced smith._dynamo.step_unsupported(), but Dynamo is instructed "
            ...
            if self.current_speculation is None:
                ...
                if isinstance(e, StepUnsupported):
                    unimplemented(
                        gb_type="smith._dynamo.step_unsupported() with empty checkpoint",
                        context="",
                        explanation="traced smith._dynamo.step_unsupported(), but there is no checkpoint "
        ...
        with self.run_ctx_mgr():
            ...
            try:
                ...
                if hasattr(e, "msg") and "Data-dependent" in e.msg:
                    readable_graph = smith.fx.GraphModule(
    ...
    def nn_modules_globals_vt(self) -> VariableTracker:
        module_name = "smith.nn.modules.module"
    ...
    def import_source(self, module_name: str) -> GlobalSource:
        ...
        if "smith_package" in module_name:
            value = smith.package.package_importer._package_imported_modules[
                module_name
            ]
            alias = (
    ...
    def _raise_exception_variable(self, val: VariableTracker) -> NoReturn:
        ...
        if (
            ...
            val.python_stack = smith._guards.TracingContext.extract_stack()  # type: ignore[union-attr]
    ...
    def create_resume(
        ...
        # e.g. smith.set_grad_enabled(True) will be reconstructed as smith.set_grad_enabled
    ...
    def graph_break_on_leaf_function(self, inst: Instruction) -> None:
        if self.has_no_inlined_calls:
            unimplemented(
                ...
                hints=[
                    "Set smith._dynamo.config.debug_force_graph_break_on_leaf_return = False",
                ],
            )
    ...
    def _format_value(self, fmt_spec: VariableTracker, flags: int) -> None:
        value = self.pop()
        if isinstance(value, SymNodeVariable):
            from smith._dynamo.variables.lazy import (
    ...
    def unsupported_ctx_graph_break(ctx: VariableTracker) -> NoReturn:
        unimplemented(
            ...
            hints=[
                ...
                "If the context manager seems like it should be supported (e.g. smith.set_grad_enabled), then "
                ...
                "File an issue to Blacksmith. Simple context managers can potentially be supported, "
    ...
    def is_co_filename_from_nn_modules(self) -> bool:
        ...
        nn_modules_pattern = re.compile(r".*smith/nn/modules.*")
    ...
    def _handle_comprehension_graph_break(self, inst: Instruction) -> None:
        ...
        if self.f_code.co_name.startswith(SMITH_DYNAMO_RESUME_IN_PREFIX):
    ...
    def log_graph_break(
        ...
        if config.verbose:
            user_stack_trace += (
                ...
                "NOTE: the most recent `smith.compile` tracing attempt might not be where you applied `smith.compile`! "
        ...
        # smith._dynamo.explain() formats this a little nicer, and presents a slightly
    ...
    def raise_loop_graph_break(code: types.CodeType, exc: Unsupported) -> NoReturn:
        unimplemented(
            ...
            explanation="smith.compile detected a graph break in a for/while loop. "
    ...
    def __init__(
        ...
        self.symbolic_smith_function_state = symbolic_smith_function_state
...
class InstructionTranslator(InstructionTranslatorBase):
    ...
    def __init__(
        ...
        _step_logger()(
            logging.INFO,
            f"smithdynamo start tracing {f_code.co_name} {code_options['co_filename']}:{code_options['co_firstlineno']}",
        )
        super().__init__(
            output=OutputGraph(
                code_options,
                ...
                f_code=f_code,
                smith_function_mode_stack=smith_function_mode_stack,
                one_graph=one_graph,
                package=package,
            ),
            ...
            symbolic_globals={},
            symbolic_smith_function_state=None,  # type: ignore[arg-type] # set below
        ...
        with tracing(self.output.tracing_context), self.set_current_tx():
            ...
            self.symbolic_smith_function_state = SymbolicSmithFunctionState(
    ...
    def _throw_if_in_funcsmith(self) -> None:
        # Fallback to eager in case of a graph break inside vmap
        eager = smith._dynamo.lookup_backend("eager")
        ...
        forbidden_keys = (
            smith._C._funcsmith.TransformType.Vmap,
            smith._C._funcsmith.TransformType.Grad,
            smith._C._funcsmith.TransformType.Jvp,
        )
        ...
        if ci is not None and ci.key() in forbidden_keys and compiler_fn is not eager:
            ...
            msg = (
                ...
                # Calling a smith.compiled function
                f"- Calling smith.func.{name}(compiled_fn) function from eager mode is not supported. "
                f"Ensure that smith.func.{name} is also wrapped within a smith.compile function. "
                "For more information, see Blacksmith issue #128711.\n"
                # if it reaches here, it means Dynamo failed to inline a funcsmith function
                f"- smith.func.{name}(fn) requires the function to be inlined by dynamo"
            )
            unimplemented(
                gb_type="Unsupported funcsmith tracing attempt",
                context="",
                explanation=msg,
                hints=[],
            )
    ...
    def symbolic_locals_contain_module_class(self) -> bool:
        for v in self.symbolic_locals.values():
            if isinstance(v, UserDefinedClassVariable) and issubclass(
                v.as_python_constant(), smith.nn.Module
            ):
                return True
        return False
    ...
    def _return(self, inst: Instruction) -> None:
        ...
        if (
            ...
            raise exc.SkipFrame(
                "No ops traced for the FX graph. `smith.compile` will skip the frame and fall back to eager.\n"
        ...
        _step_logger()(
            logging.INFO,
            f"smithdynamo done tracing {self.f_code.co_name} ({inst.opname})",
        )
...
class InliningInstructionTranslator(InstructionTranslatorBase):
    ...
    def check_inlineable(
        ...
        if isinstance(func, UserFunctionVariable) and inspect.getattr_static(
            func.get_function(), "_smithdynamo_disable", False
        ):
            msg = inspect.getattr_static(
                func.get_function(), "_smithdynamo_disable_msg", None
            )
            unimplemented(
                gb_type="Skip inlining `smith.compiler.disable()`d function",
        ...
        if result.skipped:
            from smith._dynamo.variables.misc import produce_trampoline_autograd_apply
            ...
            if "_dynamo" not in func.get_filename():
                hints += [
                    f"Apply `@smith._dynamo.dont_skip_tracing` to the function `{fn_qualname}` "
    ...
    def build_inline_tracer(
        ...
        # with a single alias
        if smith._logging._internal.log_state.is_artifact_enabled("bytecode"):
        ...
        if is_generator(code):
            tracer = InliningGeneratorInstructionTranslator(
                ...
                parent.symbolic_smith_function_state,
    ...
    def __init__(
        ...
        super().__init__(
            ...
            symbolic_smith_function_state=symbolic_smith_function_state,
    ...
    def get_globals_source_and_value(
        ...
        if "__name__" in self.f_globals and not self.f_globals["__name__"].startswith(
            ...
            if "smith_package" in module_name:
                fglobals_value = (
                    smith.package.package_importer._package_imported_modules[
                        module_name
                    ]
