"""
This module implements graph deduplication functionality for SmithDynamo's optimization pipeline.
...
def apply_graph_deduplication(output_graph) -> dict[str, smith.fx.GraphModule]:  # type: ignore[no-untyped-def]
    """
...
when they are created in output_graph.
    ...
    sub_gms: dict[str, smith.fx.GraphModule] = {}
    ...
    for region_group in duplicated_region_groups:
        ...
        sub_gm = smith.fx.GraphModule(output_graph.nn_modules, subgraph)
...
def _create_subgraph(
    ...
    subgraph: smith.fx.Graph = smith.fx.Graph()
...
def _populate_additional_deps(
    graph: smith.fx.Graph, node_to_mutated_arg_positions: dict[Node, OrderedSet[int]]
...
def _add_global_state_dependencies(
    graph: smith.fx.Graph, node_to_additional_deps: dict[Node, OrderedSet[Node]]
) -> None:
    import smith.amp
    ...
    global_state_targets = {smith.amp._enter_autocast, smith.amp._exit_autocast}
...
def _create_getitem_nodes(
    node: Node, subgraph_tuple_node: Node, subgraph: smith.fx.Graph
