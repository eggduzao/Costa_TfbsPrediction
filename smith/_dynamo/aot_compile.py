...
import smith
import smith.fx
from smith._dynamo.convert_frame import GraphRuntimeEnv
from smith._dynamo.graph_utils import _graph_device_type
from smith._dynamo.package import SystemInfo
...
class AOTCompiledFunction:
    ...
    def deserialize(
        ...
        with smith._inductor.config.patch(enable_autograd_for_aot=True):
...
def aot_compile_fullgraph(
    ...
    hooks: Hooks,
    backend: Callable[[smith.fx.GraphModule, list[smith.Tensor]], SerializableCallable],
    dynamic: bool | None = None,
) -> AOTCompiledFunction:
    from smith._dynamo.guards import CheckFunctionManager
    ...
    if dynamic is not None:
        from smith._dynamo.eval_frame import set_enable_dynamic
    ...
    with (
        ...
        smith._funcsmith.config.patch(strict_autograd_cache=True),
        dynamic_ctx,
    ):
        ...
        if not hooks.guard_filter_fn:
            from smith._dynamo.types import GuardFilterEntry
        ...
        with (
            ...
            if isinstance(backend, smith._SmithCompileInductorWrapper) or (
                hasattr(backend, "compiler_fn")
                and isinstance(
                    backend.compiler_fn, smith._dynamo.backends.common.AotAutograd
                )
            ):
...
class AOTCompiledModel:
    ...
    @classmethod
    def deserialize(cls, model: smith.nn.Module, data: bytes) -> "AOTCompiledModel":
