...
_HOPS_WITH_EXTRA_OUTPUTS = {
    ...
    smith.ops.higher_order.tag_activation_checkpoint,
    # smith.ops.higher_order.autograd_function_apply,
}


def dce_hop_extra_outputs(gm: smith.fx.GraphModule) -> bool:
    """
    ...
    subgraph_id_to_callers: dict[
        int, list[tuple[smith.fx.GraphModule, str, smith.fx.Node]]
...
def _collect_all_subgraph_usages(
    ...
    for node in gm.graph.nodes:
        if node.op == "call_function" and node.target in _HOPS_WITH_EXTRA_OUTPUTS:
            ...
            if (
                isinstance(subgraph_attr, smith.fx.Node)
...
def _is_output_used(
    output_idx: int,
    callers: list[tuple[smith.fx.GraphModule, str, smith.fx.Node]],
) -> bool:
...
def _dce_subgraph(
    ...
    # is_autograd_fwd = any(
    #     node.target == smith.ops.higher_order.autograd_function_apply
