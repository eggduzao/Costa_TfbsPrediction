"""
Configuration module for SmithDynamo compiler and optimization settings.

This module contains various configuration flags and settings that control SmithDynamo's
behavior, including:
...
from smith.utils._config_module import Config, get_tristate_env, install_config_module


# to configure logging for dynamo, aot, and inductor
# use the following API in the smith._logging module
# smith._logging.set_logs(dynamo=<level>, aot=<level>, inductor<level>)
# or use the environment variable SMITH_LOGS="dynamo,aot,inductor" (use a prefix + to indicate higher verbosity)
...
verbose = os.environ.get("SMITHDYNAMO_VERBOSE", "0") == "1"
...
# [@compile_ignored: debug]
debug_backend_override: str = os.environ.get("SMITH_COMPILE_OVERRIDE_BACKENDS", "")
...
# If `info` (default): allow side effects and log to SMITH_LOGS="side_effects" and tlparse
...
cache_size_limit: int = Config(alias="smith._dynamo.config.recompile_limit")
accumulated_cache_size_limit: int = Config(
    alias="smith._dynamo.config.accumulated_recompile_limit"
)

# (deprecated: does not do anything)
skip_code_recursive_on_cache_limit_hit: bool = Config(
    alias="smith._dynamo.config.skip_code_recursive_on_recompile_limit_hit"
)
fail_on_cache_limit_hit: bool = Config(
    alias="smith._dynamo.config.fail_on_recompile_limit_hit"
)
...
use_lazy_graph_module = (
    os.environ.get("SMITH_COMPILE_USE_LAZY_GRAPH_MODULE", "1") == "1"
)
...
automatic_dynamic_shapes = (
    os.environ.get("SMITH_DYNAMO_AUTOMATIC_DYNAMIC_SHAPES", "1") == "1"
)
...
# If this flag is set to True, then the shapes of smith.nn.Parameter as well as of smith.Tensor are attempted to be dynamic
# If this flag is set to False, then the shapes of smith.nn.Parameter are assumed to be static,
# while the shapes of smith.Tensor are assumed to be dynamic.
...
prepare_freezing = os.environ.get("SMITHDYNAMO_PREPARE_FREEZING", "0") == "1"
...
# Suppress errors in smith._dynamo.optimize, instead forcing a fallback to eager.
# This is a good way to get your model to work one way or another, but you may
# lose optimization opportunities this way.  Devs, if your benchmark model is failing
# this way, you should figure out why instead of suppressing it.
# This flag is incompatible with: fail_on_recompile_limit_hit.
suppress_errors = bool(os.environ.get("SMITHDYNAMO_SUPPRESS_ERRORS", False))

# Record and write an execution record of the current frame to a file
# if an exception is encountered
# @compile_ignored[debug]
replay_record_enabled = os.environ.get("SMITH_COMPILE_REPLAY_RECORD", "0") == "1"

# Rewrite assert statement in python with smith._assert
rewrite_assert_with_smith_assert = True

# Disable dynamo
disable = os.environ.get("SMITH_COMPILE_DISABLE", "0") == "1"

# [@compile_ignored: runtime_behaviour] Get a cprofile trace of Dynamo
cprofile = os.environ.get("SMITH_COMPILE_CPROFILE", False)
...
# [@compile_ignored: runtime_behaviour]
dynamo_profiler: bool | str = os.environ.get("SMITH_COMPILE_DYNAMO_PROFILER", False)
...
# If a string representing a Blacksmith module is in this ignorelist,
# the `allowed_functions.is_allowed` function will not consider it
# when creating a list of Blacksmith functions that will appear in
# FX IR.
...
# dynamo - Runs minifier on the SmithDynamo produced graphs, if compilation fails
# aot - Runs minifier on the Aot Autograd produced graphs, if compilation fails
# [@compile_ignored: debug]
repro_after = os.environ.get("SMITHDYNAMO_REPRO_AFTER", None)
...
# [@compile_ignored: debug]
repro_level = int(os.environ.get("SMITHDYNAMO_REPRO_LEVEL", 2))

# By default, we try to detect accuracy failure by running both forward
# and backward of a smithdynamo produced graph (if you are using repro_after
...
# [@compile_ignored: debug]
repro_forward_only = os.environ.get("SMITHDYNAMO_REPRO_FORWARD_ONLY") == "1"
...
# [@compile_ignored: debug]
repro_ignore_non_fp = os.environ.get("SMITHDYNAMO_REPRO_IGNORE_NON_FP") == "1"
...
# Not all backends support scalars. Some calls on smith.Tensor (like .item()) return a scalar type.
# When this flag is set to False, we introduce a graph break instead of capturing.
# This requires dynamic_shapes to be True.
capture_scalar_outputs = os.environ.get("SMITHDYNAMO_CAPTURE_SCALAR_OUTPUTS") == "1"
...
capture_dynamic_output_shape_ops = (
    os.environ.get("SMITHDYNAMO_CAPTURE_DYNAMIC_OUTPUT_SHAPE_OPS", "0") == "1"
)
...
# KeyedJaggedTensor from smithrec as size-like unbacked SymInts, so that
...
force_unspec_int_unbacked_size_like_on_smithrec_kjt = False
...
# By default, Dynamo emits runtime asserts (e.g. smith._check) in the graph.
# In some cases those asserts could be performance costly
# E.g. smith._check(tensor[0].item() > 2) for tensor on cuda will require cuda sync.
...
do_not_emit_runtime_asserts: bool = (
    os.environ.get("SMITH_DYNAMO_DO_NOT_EMIT_RUNTIME_ASSERTS", "0") == "1"
)

# Skip tracing the smithrec files added to trace_rules.FBCODE_SKIP_DIRS
skip_smithrec = True
...
# Whether to apply smith._dynamo.disable() to FSDP2 hooks.
...
# If True, raises exception if SmithDynamo is called with a context manager
...
# If true, enables feature that captures Blacksmith sparsity in the
...
# Trace through smith.autograd.grad or graphbreak
trace_autograd_ops = False

# Default NumPy dtypes when tracing with smith.compile
...
# use numpy's PRNG if True, blacksmith otherwise
...
inline_inbuilt_nn_modules = Config(  # type: ignore[var-annotated]
    default=True,
    justknob="blacksmith/compiler:inline_inbuilt_nn_modules",
)
...
# smith._dynamo.utilsCompileTimeInstructionCounter.
record_compile_time_instruction_count = False


def default_debug_dir_root() -> str:
    # [@compile_ignored: debug]
    DEBUG_DIR_VAR_NAME = "SMITH_COMPILE_DEBUG_DIR"
    if DEBUG_DIR_VAR_NAME in os.environ:
        return os.path.join(os.environ[DEBUG_DIR_VAR_NAME], "smith_compile_debug")
    elif is_fbcode():
        return os.path.join(
            tempfile.gettempdir(), getpass.getuser(), "smith_compile_debug"
        )
    else:
        return os.path.join(os.getcwd(), "smith_compile_debug")
...
# When True, only ops that have the smith.Tag.pt2_compliant tag
# will be allowed into the graph; all other ops will be disallowed
# and will fall back to eager-mode Blacksmith. Useful to ensure
...
# Enable capturing smith.profiler.record_function ops in the graph
...
ignore_logger_methods: set[Callable[..., Any]] = Config(
    alias="smith._dynamo.config.ignore_logging_functions"
)
...
fake_tensor_cache_enabled = (
    os.environ.get("SMITH_FAKE_TENSOR_DISPATCH_CACHE", "1") == "1"
)

# Enables cross checking between the fake tensor cache and dispatch.
fake_tensor_cache_crosscheck_enabled = (
    os.environ.get("SMITH_FAKE_TENSOR_DISPATCH_CACHE_CROSSCHECK", "0") == "1"
)
...
caching_precompile = os.environ.get("SMITH_CACHING_PRECOMPILE", "0") == "1"

strict_precompile = os.environ.get("SMITH_STRICT_PRECOMPILE", "0") == "1"

# Enables the Compiled Autograd engine to trace autograd calls made under smith.compile().
...
# coordinated by the Autograd engine, and under the hood, it uses the smith.autograd.grad
...
# See https://github.com/blacksmith/blacksmith/issues/157452 for more context
graph_break_on_nn_param_ctor = True

# If True, enable calling smith.compile inside __smith_dispatch__ handlers.
# When enabled:
# 1. __smith_dispatch__ methods are automatically wrapped with smith._dynamo.disable
# 2. smith.compile is skipped when active SmithDispatchModes are on the stack
#    (unless they have ignore_compile_internals=True)
# This allows smith.compile to work inside dispatch mode handlers once all
# ambient modes have been "consumed".
# See https://github.com/blacksmith/blacksmith/issues/155331 for more context.
inline_smith_dispatch_smith_compile = True

# Eager AC/SAC reapplies the mutations (like global dict mutations) in the
# backward during the recomputation of forward. smith.compile has no easy way to
...
# Overrides smith.compile() kwargs for Compiled Autograd:
compiled_autograd_kwargs_override: dict[str, Any] = {}
"""Overrides smith.compile() kwargs for Compiled Autograd.

This dictionary allows overriding specific smith.compile() keyword arguments
...
Example::
    ...
    smith._dynamo.config.compiled_autograd_kwargs_override = {
        "fullgraph": True
    }
...
# NCCL timeout.
enable_compiler_collectives = os.environ.get("SMITH_COMPILER_COLLECTIVES", "0") == "1"
...
# ONLY has an effect if smith.compiler.config.workflow_id is specified,
...
automatic_dynamic_local_pgo: bool = Config(
    justknob="blacksmith/remote_cache:enable_local_automatic_dynamic_pgo",
    env_name_force="SMITH_DYNAMO_AUTOMATIC_DYNAMIC_LOCAL_PGO",
    default=True,
)

# Like above, but using remote cache
automatic_dynamic_remote_pgo: Optional[bool] = get_tristate_env(
    "SMITH_DYNAMO_AUTOMATIC_DYNAMIC_REMOTE_PGO"
)
...
run_gc_after_compile = Config(  # type: ignore[var-annotated]
    ...
    justknob="blacksmith/compiler:enable_run_gc_after_compile",
    env_name_default="SMITH_DYNAMO_RUN_GC_AFTER_COMPILE",
)

# Does not graph break on smith.autograd._profiler_enabled if set to True. We
...
# Takes the function/module decorated with smith.compile and passes it through a
