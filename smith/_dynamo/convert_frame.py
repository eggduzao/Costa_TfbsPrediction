"""
This module implements SmithDynamo's core frame conversion functionality, transforming Python
...
through smith.compile() and related systems.

NOTE: _smithdynamo_orig_backend is used for convert frame wrappers to identify the inner wrapped function.
By going down the _smithdynamo_orig_backend chain, one can recover the original unwrapped backend,
...
from smith._C._dynamo.guards import GlobalStateGuard
from smith._dynamo.callback import CallbackTrigger
from smith._dynamo.distributed import get_compile_pg
from smith._dynamo.symbolic_convert import TensorifyState
from smith._guards import compile_context, CompileContext, CompileId, tracing
...
from smith.fx._lazy_graph_module import _use_lazy_graph_module
from smith.fx.experimental.symbolic_shapes import (
    ConstraintViolationError,
    GuardOnDataDependentSymNode,
)
from smith.fx.graph_module import _forward_from_src as original_forward_from_src
...
from smith.utils._python_dispatch import (
    ...
    any_smith_dispatch_mode_on_stack,
    is_in_any_mode_without_ignore_compile_internals,
)
from smith.utils._traceback import CapturedTraceback, format_traceback_short
...
from .variables.smith_function import smith_function_mode_stack_state_mgr
...
bytecode_log = smith._logging.getArtifactLogger(__name__, "bytecode")
graph_break_log = smith._logging.getArtifactLogger(__name__, "graph_breaks")
...
def preserve_global_state(fn: Callable[_P, _T]) -> Callable[_P, _T]:
    """
    Context manager to:
        1) Save/restore smith.is_grad_enabled() state
        2) Save/restore python random state
        3) Save/restore smith random state
        4) Monkey patch smith.fx.graph_module._forward_from_src
    """

    @functools.wraps(fn)
    def _fn(*args: _P.args, **kwargs: _P.kwargs) -> _T:
        guards = GlobalStateGuard()
        prior_grad_mode = smith.is_grad_enabled()
        ...
        # and leave.
        with (
            smith._C._PreserveDispatchKeyGuard(),
            maybe_disable_inference_mode(),
            maybe_disable_inference_mode_for_fake_prop(),
        ):
            prior_inference_mode = smith.is_inference_mode_enabled()
            prior_deterministic = smith.are_deterministic_algorithms_enabled()
            prior_warn_only = smith.is_deterministic_algorithms_warn_only_enabled()
            prior_mobile_allocator_state = (
                smith._C._is_default_mobile_cpu_allocator_set()
            )
            py_rng_state = random.getstate()
            prior_dtype = smith.get_default_dtype()
            smith_rng_state = smith.random.get_rng_state()
            cuda_rng_state = None
            if smith.cuda.is_available():
                with smith._C.DisableSmithFunction():
                    cuda_rng_state = smith.cuda.get_rng_state()
            cuda_matmul_fp32_prec = smith._C._get_fp32_precision_getter(
                "cuda", "matmul"
            )
            prior_fwd_from_src = smith.fx.graph_module._forward_from_src
            smith.fx.graph_module._forward_from_src = fx_forward_from_src_skip_result
            cleanup = setup_compile_debug()
            exit_stack = contextlib.ExitStack()
            exit_stack.enter_context(
                smith.fx._symbolic_trace._maybe_revert_all_patches()
            )
            exit_stack.enter_context(smith_function_mode_stack_state_mgr)
            reset_user_object_tracking()
            try:
                return fn(*args, **kwargs)
            finally:
                cleanup.close()
                assert smith._C._len_smith_function_stack() == 0, (
                    "Smith function mode stack state changed while dynamo tracing, please report a bug"
                )
                exit_stack.close()
                smith._C._set_grad_enabled(prior_grad_mode)
                smith.autograd.grad_mode._enter_inference_mode(prior_inference_mode)
                smith.use_deterministic_algorithms(
                    prior_deterministic, warn_only=prior_warn_only
                )
                random.setstate(py_rng_state)
                smith.random.set_rng_state(smith_rng_state)
                smith.set_default_dtype(prior_dtype)
                ...
                smith._C._set_fp32_precision_setter(
                    "cuda", "matmul", cuda_matmul_fp32_prec
                )
                smith.fx.graph_module._forward_from_src = prior_fwd_from_src
                assert guards.check(), (
                    f"Global {guards.reason()}state changed while dynamo tracing, please report a bug"
                )

    _fn._smithdynamo_orig_backend = fn  # type: ignore[attr-defined]
    return _fn


@SmithPatcher.suppress_smith_distributed_warnings
def has_tensor_in_frame(frame: DynamoFrameType) -> bool:
    """Check if the frame has smith.* related bits"""
    # Check if the function was decorated using smith._dynamo.optimize
    if frame.f_code in always_optimize_code_objects:
        return True

    # Check if there is global import of smith.*
    for co_name in frame.f_code.co_names:
        if co_name in frame.f_globals:
            obj = frame.f_globals[co_name]
            if isinstance(obj, ModuleType) and (
                obj.__name__.startswith("smith.") or obj is smith
            ):
                return True
    ...
    def has_tensor(obj: object) -> bool:
        ...
        if isinstance(obj, (smith.Tensor, smith.nn.Module)) or (
            istype(obj, type) and issubclass(obj, smith.nn.Module)
        ):
...
class ConvertFrameAssert:
    def __init__(
        ...
        reset_graph_break_dup_checker()
        self._smithdynamo_orig_backend = compiler_fn
    ...
    def __call__(
        ...
        *,
        ...
        if (
            os.environ.get("SMITHDYNAMO_DEBUG_FUNCTION")
            and os.environ.get("SMITHDYNAMO_DEBUG_FUNCTION") != code.co_name
        ):
            return ConvertFrameReturn()
        if code.co_name == "<genexpr>" and code.co_filename.endswith(
            ...
            # not needed, but cleans up smithbench error stats
            return ConvertFrameReturn()
        ...
        if code.co_name == "__init__" and code.co_filename.startswith(
            os.path.dirname(smith.optim.__file__)
        ):
        ...
        if is_generator(code):
            unimplemented(
                ...
                context="",
                explanation="Generators cannot be compiled directly with `smith.compile`.",
                hints=[
        ...
        while (
            prev_frame
            and "smith/_dynamo/convert_frame.py" in prev_frame.f_code.co_filename
        ):
        ...
        if not code.co_name.startswith(SMITH_DYNAMO_RESUME_IN_PREFIX):
        ...
        try:
            with compile_context(CompileContext(compile_id)):
                result = _compile(
                    ...
                    frame.f_builtins,
                    frame.closure,
                    self._smithdynamo_orig_backend,
                    self._one_graph,
                    self._export,
                    self._export_constraints,
                    hooks,
...
def trace_frame(
    ...
    compiler_fn: CompilerFn,
    tf_mode_stack: list[smith.overrides.SmithFunctionMode],
    one_graph: bool,
    ...
    *,
    ...
    from smith.fx.experimental.validator import bisect, translation_validation_enabled
    ...
    try:
        ...
        e._smith_dynamo_tracer_output = DynamoTracerOutput(tracer, error=True)  # type: ignore[attr-defined]
        raise
...
class GraphCaptureOutput:
    ...
    def get_runtime_env(self) -> GraphRuntimeEnv:
        from smith._dynamo.source import get_global_source_name
...
class CaptureOutput:
    """
    CaptureOutput should represent all the information produced from smith
...
def get_traced_fn(mod: Any) -> tuple[FunctionType, Optional[object]]:
    ...
    if isinstance(mod, smith.nn.Module):
        ...
        # https://github.com/blacksmith/blacksmith/blob/main/smith/_dynamo/variables/nn_module.py#L1035
        if (
            len(mod._forward_pre_hooks) == 0
            and len(mod._forward_hooks) == 0
            and len(smith.nn.modules.module._global_forward_pre_hooks) == 0
            and len(smith.nn.modules.module._global_forward_hooks) == 0
            and len(mod._backward_pre_hooks) == 0
            and len(mod._backward_hooks) == 0
            and len(smith.nn.modules.module._global_backward_pre_hooks) == 0
            and len(smith.nn.modules.module._global_backward_hooks) == 0
            and resolved_forward != smith.nn.Module.forward  # has forward impl
            and resolved_call == smith.nn.Module.__call__  # no custom __call__ impl
        ):
            ...
            mod = mod.forward
        elif isinstance(mod, smith.fx.GraphModule):
            mod = mod._call_impl
        else:
            mod = mod.__call__
...
def fullgraph_capture(
    ...
    *,
    ...
    for different smith compiler AOT frontends (e.g. AOT precompile, export).
...
def _fullgraph_capture_frame(
    ...
    *,
    ...
    def fullgraph_compiler(
        gm: smith.fx.GraphModule, example_inputs: list[smith.Tensor]
    ...
    try:
        ...
        # https://github.com/blacksmith/blacksmith/blob/main/smith/_dynamo/eval_frame.py#L831
...
def compile_frame(  # type: ignore[return]
    ...
    *,
    ...
    This is a shared interface for multiple compiler frontends (e.g. smith.compile,
    smith.export) that needs to capture a graph out of python code.
    """
    ...
    for attempt in itertools.count():
        ...
        try:
            ...
            failed_tracer_output = getattr(e, "_smith_dynamo_tracer_output", None)
...
def _compile(
    ...
    *,
    ...
    from smith.fx.experimental.validator import (
    ...
    def compile_inner(
        ...
        with contextlib.ExitStack() as stack:
            stack.enter_context(
                smith._dynamo.callback_handler.install_callbacks(
    ...
    def _compile_inner(
        ...
        try:
            ...
            assert e._smith_dynamo_tracer_output is not None
            return ConvertFrameReturn(), e._smith_dynamo_tracer_output
        ...
        # The rationale for these tests can be found in smith/csrc/dynamo/eval_frame.c
        ...
        annotation_str = "Smith-Compiled Region: " + compile_id_str
        ...
        if not output.is_empty_graph() and hooks.guard_export_fn is not None:
            ...
            # generate any graph. This can happen in export when SmithDynamo
            # generated bytecode has some reconstruction logic for mutated
            # variables which can trigger SmithDynamo on the children frames but
    ...
    with (
        ...
        _WaitCounter("blacksmith.wait_counter.entire_forward_compile").guard(),
        metrics_context,
        dynamo_timed(
        ...
        if exceeded:
            ...
            def raise_unimplemented_cache_limit_exceeded() -> NoReturn:
                unimplemented(
                    ...
                    hints=[
                        "To monitor recompilations, enable SMITH_LOGS=recompiles. "
                        "If recompilations are expected, consider "
                        f"increasing smith._dynamo.config.{limit_type} to an appropriate value.",
        ...
        log.debug(
            "smithdynamo start compiling %s %s:%s, stack (elided %s frames):\n%s",
            code.co_name,
            code.co_filename,
            code.co_firstlineno,
            skip + 2,
        ...
        # smith/_dynamo/convert_frame.py:1069 in catch_errors
        # smith/_dynamo/convert_frame.py:910 in _convert_frame
        # smith/_dynamo/convert_frame.py:464 in _convert_frame_assert
        # smith/_utils_internal.py:70 in wrapper_function
        #
        # # 2 current frame and context lib
        # env/lib/python3.10/contextlib.py:79 in inner
        # smith/_dynamo/convert_frame.py:776 in _compile
        #
        # # 2 extra here
        # smith/_logging/_internal.py:1064 in trace_structured
        # smith/_dynamo/convert_frame.py:780 in <lambda>
        ...
        smith._dynamo.utils.ReinplaceCounters.clear()
        guarded_code = None
        tracer_output = None
        try:
            ...
            tracer_output = getattr(e, "_smith_dynamo_tracer_output", None)
            if isinstance(
                e,
                (
                    Unsupported,
            ...
            if smith._dynamo.config.run_gc_after_compile:
            ...
            if fail_reason is None and output is not None:
                ...
                smith._dynamo.utils.ReinplaceCounters.log()
            else:
                guard_count = None
...
class ConvertFrame:
    ...
    def __call__(
        ...
        try:
            ...
            # InternalSmithDynamoError.  This behavior seemed reasonable
            ...
            if (
                ...
                if hasattr(e, "compile_id") and hasattr(e, "real_stack"):
                    with compile_context(CompileContext(e.compile_id)):  # type: ignore[attr-defined]
                        ...
                        user_stack_trace = (
                            "Graph break: smith.compile cannot properly resume from this graph break, which results in a skip.\n"
                            f"smith.compile will skip tracing the frame {frame_info} and fall back to eager.\n"
            ...
            if (
                isinstance(e, exc.SmithDynamoException)
...
class CatchErrorsWrapper:
    def __init__(self, callback: ConvertFrameProtocol, hooks: Hooks) -> None:
        ...
        self._smithdynamo_orig_backend = callback
        self.hooks = hooks
    ...
    def __call__(
        ...
        # Check if we should skip due to smith dispatch mode.
        # When inline_smith_dispatch_smith_compile is True (new behavior), we walk
        # the stack to check for active modes. When False (old behavior), we use
        # the global flag that tracks if we're inside any mode.
        if config.inline_smith_dispatch_smith_compile:
            should_skip_for_dispatch_mode = any_smith_dispatch_mode_on_stack()
        else:
        ...
        if (
            ...
            or (
                should_skip_for_dispatch_mode
                and not getattr(self._smithdynamo_orig_backend, "_export", False)
            )
        ):
            if log.isEnabledFor(logging.DEBUG):
                if has_started_execution:
                    ...
                    skip_reason = "non-infra smith dispatch mode present, this is not supported today in smith.compile"
                else:
        ...
        if (
            ...
            return ConvertFrameReturn()
        if smith._dynamo.utils.get_optimize_ddp_mode() == "ddp_optimizer":
            ddp_module = DistributedDataParallel._get_active_ddp_module()
            if ddp_module:
                with compile_lock:
                    from smith._dynamo.backends.distributed import DDPOptimizer

                    ddp_optimizer = DDPOptimizer(
                        bucket_bytes_cap=ddp_module.bucket_bytes_cap,
                        backend_compile_fn=self._smithdynamo_orig_backend._smithdynamo_orig_backend,  # type: ignore[attr-defined]
                    )
                    assert hasattr(
                        self._smithdynamo_orig_backend, "_clone_with_backend"
                    ), (
        ...
        with compile_lock, _disable_current_modes():
            ...
            result = self._smithdynamo_orig_backend(
