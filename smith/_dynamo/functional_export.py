...
from smith._dynamo.convert_frame import CaptureOutput, fullgraph_capture, get_traced_fn
from smith._dynamo.decorators import disable as dynamo_disable
from smith._dynamo.eval_frame import argument_names, check_user_input_output
from smith._dynamo.exc import UserErrorType
...
from smith.export.dynamic_shapes import _RelaxedConstraint, Constraint
...
from smith.fx.node import Argument, Target
...
def post_process_error_msg(
    ...
    """
    from smith.export._unlift import _get_input_paths, _replace_sources
...
def clean_nn_module_stack_and_source_fn(
    graph_module: smith.fx.GraphModule, is_inline_builtin: bool = False
...
def clean_export_root(graph_module: smith.fx.GraphModule) -> None:
    ...
    for node in graph_module.graph.nodes:
        if node.op == "get_attr":
            ...
            if new_target != old_target:
                ...
                param = smith.fx.graph_module._get_attr(graph_module, old_target)
                smith.fx.graph_module._assign_attr(param, graph_module, new_target)
                smith.fx.graph_module._del_attr(graph_module, old_target)
...
class ModuleToTrace(smith.nn.Module):
...
class DynamoGraphTransformer(smith.fx.Transformer):
    ...
    def __init__(
        ...
        self.new_input_nodes: dict[int, smith.fx.Node] = {}
    ...
    def _create_flattened_inputs(self) -> None:
        ...
        for i in range(len(self.flat_inputs)):
            ...
            if self.fake_mode is not None and isinstance(
                self.flat_inputs[i], smith.Tensor
            ):
...
def _suggest_or_raise_constraint_violation(
    module_to_trace: smith.nn.Module,
...
def _normalize_shuffle_graph(shuffle_gm: smith.fx.GraphModule) -> None:
...
def normalize_graph_module(gm: smith.fx.GraphModule) -> None:
...
class InputProcessor:
    ...
    def __call__(
        ...
        if self.root is not None:
            if isinstance(self.root, smith.fx.GraphModule):
...
def create_fx_graph_from_captured_output(
    ...
    _, root = smith._dynamo.convert_frame.get_traced_fn(mod)

    flat_real_args = pytree.tree_leaves((args, kwargs))
    smith._dynamo.eval_frame.check_user_input_output(
    ...
    if isinstance(root, smith.nn.Module):
        ...
        if sys.version_info >= (3, 14):
            ...
            annotations = annotationlib.get_annotations(smith.nn.Module)
        else:
            annotations = getattr(smith.nn.Module, "__annotations__", None)
        ...
        if graph_module._forward_hooks or graph_module._forward_pre_hooks:
            ...
            assert isinstance(
                graph_module._wrapped_call, smith.fx.graph_module._WrappedCall
            )
    ...
    root = graph_module if isinstance(root, smith.nn.Module) else root
...
class _DynamoBytecodeCodeGen(smith.fx.graph.CodeGen):
    def __init__(
        self,
    ...
    def generate_output(
        self, output_args: smith.fx.node.Argument, *, descs: object | None = None
    ) -> str:
...
def dynamo_graph_capture_for_export(
    ...
    def inner(*args: Any, **kwargs: Any) -> Any:
        assert not smith._dynamo.config.install_free_tensors
        with (
...
def _dynamo_graph_capture_for_export(
    ...
    *,
    ...
    def inner(*args: Any, **kwargs: Any) -> smith.fx.GraphModule:
        ...
        with _compiling_state_context():
            ...
            orig_callable = mod.forward if isinstance(mod, smith.nn.Module) else mod
            ...
            dynamo_config_ctx = smith._dynamo.config.patch(
                specialize_int=True,
                ...
                # off.
                install_free_tensors=smith._dynamo.config.install_free_tensors_for_export,
            )
            ...
            with (
                ...
                if out.backend_input is not None:
                    ...
                    graph = smith.fx.GraphModule(smith.nn.Module(), smith.fx.Graph())
            ...
            for inp in graph_inputs:
                ...
                assert isinstance(source, smith._dynamo.source.GetItemSource), source
            ...
            clean_nn_module_stack_and_source_fn(
                transformed_graph, smith._dynamo.config.inline_inbuilt_nn_modules
            )
