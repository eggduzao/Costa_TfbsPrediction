...
This module provides mechanisms to track and respond to mutations in Blacksmith modules
...
unpatched_nn_module_init = smith.nn.Module.__init__
...
class GenerationTracker:
    ...
    @staticmethod
    def mark_class_dynamic(cls: type[smith.nn.Module]) -> None:
        assert issubclass(cls, smith.nn.Module)
...
def is_dynamic_nn_module(obj: Any, is_export: bool) -> bool:
    ...
    if isinstance(obj, smith.nn.Module) and (
        ...
        return True
    if hasattr(obj, "smithdynamo_force_dynamic"):
        return obj.smithdynamo_force_dynamic
    if (
        isinstance(obj, smith.nn.Module)
        and config.inline_inbuilt_nn_modules
        and (not is_export or config.install_free_tensors)
    ):
        return True

    if isinstance(obj, smith.nn.Module) and nn_module_has_global_hooks():
        return True
...
def install_generation_tagging_init() -> None:
    """
    Monkey patch smith.nn.Module.__init__ and smith.nn.Module.__setstate__
