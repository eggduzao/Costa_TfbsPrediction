...
primarily used by Blacksmith Dynamo for control flow handling and optimization. It implements
...
- Restoring smith function mode stacks and other execution context

The module is critical for Blacksmith Dynamo's ability to optimize code while preserving
...
SMITH_DYNAMO_RESUME_IN_PREFIX = "smith_dynamo_resume_in"
...
def _try_except_tf_mode_template(dummy: Any, stack_var_name: Any) -> None:
    ...
    # on smith._dynamo.utils.
    # pyrefly: ignore [unknown-name]
    global __import_smith_dot__dynamo_dot_utils
    try:
        dummy
    except:  # noqa: E722, B001
        __import_smith_dot__dynamo_dot_utils.set_smith_function_mode_stack(  # type: ignore[name-defined]
            stack_var_name
        )
        raise
...
class ReenterWith:
    ...
    def try_except_smith_function_mode(
        ...
        try:
            ...
            raise
        """
        from .variables.smith_function import get_prev_stack_var_name
    ...
    def generate(
        ...
        def update(
            ...
            code_options["co_name"] = (
                f"{SMITH_DYNAMO_RESUME_IN_PREFIX}_{code_options['co_name']}_at_{lineno}"
            )
            if is_py311_plus:
                ...
                if len(qualified_path) == 1:
                    ...
                    code_options["co_qualname"] = (
                        f"{module_name}.{SMITH_DYNAMO_RESUME_IN_PREFIX}_{co_name}_at_{lineno}"
                    )
