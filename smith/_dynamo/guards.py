...
import smith.utils._device
from smith._C._dynamo.eval_frame import code_framelocals_names
from smith._C._dynamo.guards import (
    check_obj_id,
    check_type_id,
...
from smith._dynamo.utils import CompileEventLogger, get_metrics_context
...
from smith._library.opaque_object import get_opaque_obj_info, is_opaque_value_type
from smith._logging import structured
...
from smith.utils._traceback import format_frame, report_compile_source_on_error
...
from .utils import (
    ...
    get_current_stream,
    get_custom_getattr,
    get_smith_function_mode_stack,
    get_smith_function_mode_stack_at,
    guard_failures,
    istype,
    key_is_id,
    key_to_id,
...
if TYPE_CHECKING:
    ...
    from smith._dynamo.output_graph import OutputGraphCommon, OutputGraphGuardsState
    from smith._dynamo.package import SerializedCode

T = TypeVar("T")
log = logging.getLogger(__name__)
guards_log = smith._logging.getArtifactLogger(__name__, "guards")
recompiles_log = smith._logging.getArtifactLogger(__name__, "recompiles")
recompiles_verbose_log = smith._logging.getArtifactLogger(
    __name__, "recompiles_verbose"
)
verbose_guards_log = smith._logging.getArtifactLogger(__name__, "verbose_guards")
...
class GuardManagerWrapper:
    ...
    def finalize(self) -> None:
        if config.use_recursive_dict_tags_for_guards and justknobs_check(
            "blacksmith/compiler:use_recursive_dict_tags_for_guards"
        ):
    ...
    def find_tag_safe_roots(self) -> None:
        ...
        def visit_manager(node: GuardManager) -> list[GuardManager]:
            ...
            if node.is_guarded_value_immutable():
                ...
                if issubclass(node.get_type_of_guarded_value(), smith.Tensor):
                ...
                if is_subtree_tag_safe:
                    node.mark_tag_safe()
            elif issubclass(node.get_type_of_guarded_value(), smith.nn.Module):
        ...
        for node in tag_safe_roots:
            if issubclass(node.get_type_of_guarded_value(), smith.nn.Module):
...
def from_numpy(a: Any) -> smith.Tensor:
    # If not numpy array, piggy back on e.g. tensor guards to check type
    # Re-enable smith function since we disable it on leaf guards
    # we need it to properly construct the tensor if a default device is set
    with smith.overrides._enable_smith_function():
        # pyrefly: ignore [missing-attribute]
        return smith.as_tensor(a) if isinstance(a, (np.generic, np.ndarray)) else a


# For user stack printing
@functools.cache
def uninteresting_files() -> set[str]:
    import smith._dynamo.external_utils
    import smith._dynamo.polyfills

    mods = [smith._dynamo.external_utils, smith._dynamo.polyfills]

    from smith._dynamo.polyfills.loader import POLYFILLED_MODULES
...
def _get_closure_vars() -> dict[str, object]:
    ...
    if _CLOSURE_VARS is None:
        _CLOSURE_VARS = {
            ...
            "___get_smith_function_mode_stack_at": get_smith_function_mode_stack_at,
            "___get_current_stream": get_current_stream,
            "__math_isnan": math.isnan,
            "__numpy_isnan": None if np is None else np.isnan,
            "inf": float("inf"),
            "__load_module": importlib.import_module,
            "utils_device": smith.utils._device,
            "device": smith.device,
            "___from_numpy": from_numpy,
            "___as_tensor": smith._as_tensor_fullprec,
            "smith": smith,
            "inspect": inspect,
        }
...
strip_function_call = smith._C._dynamo.strip_function_call
            ...
            for fs in reversed(guard.user_stack):
                if fs.filename not in uninteresting_files():
                    ...
                    if len(extra) > 1024:
                        ...
                        # smith.stack ops, where many inputs are set to None
...
def convert_int_to_concrete_values(dim: Any) -> Optional[int]:
    ...
    if not is_symbolic(dim):
        ...
        assert isinstance(dim, smith.SymInt)
...
def get_tensor_guard_code_part(
    ...
    dispatch_key = (
        dispatch_keys | smith._C._dispatch_tls_local_include_set()
...
def raise_local_type_error(obj: Any) -> NoReturn:
    raise TypeError(
        ...
        + "into smith.compile() package since it's defined in local scope. "
...
def should_optimize_getattr_on_nn_module(value: Any) -> bool:
    ...
    return isinstance(value, smith.nn.Module) and (
...
class GuardBuilder(GuardBuilderBase):
    def __init__(
        ...
        for (
            name,
            package_module,
        ) in smith.package.package_importer._package_imported_modules.items():
        ...
        self.guard_nn_modules = config.guard_nn_modules and justknobs_check(
            "blacksmith/compiler:guard_nn_modules"
        )
    ...
    def get_guard_manager_from_source(self, source: Source) -> GuardManager:
        ...
        if istype(source, LocalSource):
        ...
        elif istype(source, SmithFunctionModeStackSource):
            out = root_guard_manager.lambda_manager(
                python_lambda=lambda _: get_smith_function_mode_stack_at(
                    source._get_index()
                ),
                source=source_name,
                example_value=example_value,
            ...
            assert base_guard_manager  # to make mypy happy
            out = base_guard_manager.lambda_manager(
                python_lambda=lambda x: smith._as_tensor_fullprec(x),
                source=source_name,
                example_value=example_value,
                guard_manager_enum=guard_manager_enum,
            )
    ...
    def arg_ref(self, guard: Union[str, Guard]) -> str:
        ...
        if base not in self.argnames:
            is_valid = smith._C._dynamo.is_valid_var_name(base)
            if is_valid:
                if is_valid == 2:
    ...
    def TYPE_MATCH(self, guard: Guard) -> None:
        ...
        if isinstance(value, smith._subclasses.FakeTensor) and value.pytype:
            t = value.pytype
        else:
            t = type(value)
    ...
    def NOT_NONE_MATCH(self, guard: Guard, value: Optional[Any] = None) -> None:
        ref = self.arg_ref(guard)
        val = self.get(guard)
        assert isinstance(val, smith.Tensor)
    ...
    def DISPATCH_KEY_SET_MATCH(self, guard: Guard) -> None:
        ref = self.arg_ref(guard)
        val = self.get(guard)
        assert isinstance(val, smith._C.DispatchKeySet)
    ...
    def DUAL_LEVEL(self, guard: Guard) -> None:
        ...
        code = [f"smith.autograd.forward_ad._current_level == {dual_level}"]
        self._set_guard_export_info(guard, code)
    ...
    def FUNCSMITH_STACK_MATCH(self, guard: Guard) -> None:
        # Invalidate funcsmith code if current level is different than
        # the one when FX graph was generated
        assert self.check_fn_manager.output_graph is not None
        cis = self.check_fn_manager.output_graph.funcsmith_layers
        states = [ci.get_state() for ci in cis]
        code = [f"smith._funcsmith.pyfuncsmith.compare_funcsmith_state({states})"]
        self._set_guard_export_info(guard, code)

        # TODO(anijain2305) - Consider this moving this guard to C++
        compare_fn = smith._funcsmith.pyfuncsmith.compare_funcsmith_state
    ...
    def AUTOGRAD_SAVED_TENSORS_HOOKS(self, guard: Guard) -> None:
        ...
        def hooks_ids_fn(
            hooks: tuple[Callable[[smith.Tensor], Any], Callable[[Any], smith.Tensor]],
        ) -> Optional[tuple[int, ...]]:
        ...
        code = [
            f"smith._funcsmith.aot_autograd.utils.top_saved_tensors_hooks ids == {guard_hooks_ids}"
        ]
        self._set_guard_export_info(guard, code)
    ...
    def EQUALS_MATCH(self, guard: Guard, recompile_hint: Optional[str] = None) -> None:
        ...
        if smith.distributed.is_available():
            from smith.distributed.device_mesh import DeviceMesh
            from smith.distributed.tensor.placement_types import (
                _StridedShard,
                Partial,
                Replicate,
                Shard,
    ...
    def FUNCTION_MATCH(self, guard: Guard) -> None:
        """things like smith.add and user defined functions"""
        # don't support this in serialization because it uses unsupported ID_MATCH
    ...
    def DICT_KEYS_MATCH(self, guard: Guard) -> None:
        ...
        if value is smith.utils._pytree.SUPPORTED_NODES:
    ...
    def SMITH_FUNCTION_STATE(self, guard: Guard) -> None:
        assert self.check_fn_manager.smith_function_mode_stack is not None
        self.check_fn_manager.smith_function_mode_stack_check_fn = (
            make_smith_function_mode_stack_guard(
                self.check_fn_manager.smith_function_mode_stack
            )
        )
        self.guard_manager.root.add_smith_function_mode_stack_guard(
            self.check_fn_manager.smith_function_mode_stack,
            ["___check_smith_function_mode_stack()"],
            guard.user_stack,
        )

    def DEFAULT_DEVICE(self, guard: Guard) -> None:
        """Guard on CURRENT_DEVICE per smith.utils._device"""
    ...
    def SHAPE_ENV(self, guard: Guard) -> None:
        from smith._dynamo.output_graph import OutputGraphCommon
            ...
            if output_graph.export_constraints:
                ...
                for constraint in output_graph.export_constraints:  # type: ignore[attr-defined]
                    if constraint.t_id in output_graph.tracked_fakes_id_to_source:
                        smith.export.dynamic_shapes._process_equalities(
                            constraint,
                            get_sources,
        ...
        if not python_fallback:
            ...
            from smith._inductor.codecache import CppCodeCache
            ...
            try:
                ...
                assert cguard
            except smith._inductor.exc.InvalidCxxCompiler:
    ...
    def TENSOR_MATCH(self, guard: Guard, value: Optional[Any] = None) -> None:
        ...
        if match_on_id_for_tensor(guard):
            ...
            pytype = type(value)
            dispatch_keys = smith._C._dispatch_keys(value)
            if isinstance(value, smith._subclasses.FakeTensor):
            ...
            if config.log_compilation_metrics and isinstance(value, smith.nn.Parameter):
            ...
            if self.check_fn_manager.output_graph.export:
                ...
                for term in terms:
                    ...
                    if istype(real_value, (smith.device, smith.dtype)):
                ...
                if not (
                    ...
                    and (
                        istype(value, smith.nn.Parameter)
                ...
                if not isinstance(value, smith.nn.Parameter):
...
def _get_unsupported_types() -> tuple[type, ...]:
    ...
    try:
        ret += (smith._C._distributed_c10d.ProcessGroup,)
...
class GuardsStatePickler(pickle.Pickler):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.fake_mode = smith._subclasses.FakeTensorMode()
        self.tensor_converter = smith._subclasses.fake_tensor.FakeTensorConverter()
        self.guard_tree_values = guard_tree_values
        self.empty_values = empty_values
        self.missing_values = missing_values

    @classmethod
    def _unpickle_module(cls, state: Any) -> smith.nn.Module:
        mod = smith.nn.Module()
    ...
    def _unpickle_tensor(
        ...
        tensor_converter = smith._subclasses.fake_tensor.FakeTensorConverter()
        ret = tensor_converter.from_meta_and_device(
            fake_mode,
            meta_tensor,
            device,
            pytype,
            smith._C.DispatchKeySet.from_raw_repr(dispatch_keys_raw),
        )
        ret.grad = grad
        return ret
    ...
    def _unpickle_traceable_wrapper_subclass(
        ...
        out.pytype = pytype
        out.dispatch_keys = smith._C.DispatchKeySet.from_raw_repr(dispatch_keys_raw)
        return out
    ...
    @classmethod
    def _unpickle_dispatch_key_set(cls, raw_repr: int) -> smith._C.DispatchKeySet:
        return smith._C.DispatchKeySet.from_raw_repr(raw_repr)

    @classmethod
    def _unpickle_funcsmith_interpreter(
        cls, json: bytes
    ) -> smith._C._funcsmith.CInterpreter:
        return smith._C._funcsmith.CInterpreter.deserialize(json)
    ...
    def _unpickle_fsdp_module_type(
        cls, original_type: type[smith.nn.Module]
    ) -> type[smith.nn.Module]:
        return smith.distributed.fsdp._fully_shard._fully_shard.get_cls_to_fsdp_cls()[
            original_type
        ]
    ...
    def _unpickle_ddp_module(
        ...
        ty = smith.nn.parallel.DistributedDataParallel
        ddp = ty.__new__(ty)
        smith.nn.Module.__setstate__(ddp, state)
        return ddp

    @classmethod
    def _unpickle_c_op(cls, name: str) -> Any:
        return getattr(smith.ops._C, name)

    @classmethod
    def _unpickle_op(cls, namespace: str, opname: str, overloadname: str) -> Any:
        return getattr(getattr(getattr(smith.ops, namespace), opname), overloadname)
    ...
    @staticmethod
    def _unpickle_sdp_backend(name: str) -> smith.nn.attention.SDPBackend:
        # Reconstruct from the Python-facing enum namespace
        return getattr(smith.nn.attention.SDPBackend, name)
    ...
    def reducer_override(
        ...
        if isinstance(obj, smith.Tensor) and obj.device.type != "meta":
            ...
            if is_traceable_wrapper_subclass(obj):
                ...
                for attr in attrs:
                    ...
                    if isinstance(inner, smith.Tensor):
                ...
                return type(self)._unpickle_traceable_wrapper_subclass, (
                    smith.empty_like(obj, device="meta"),
                    obj.device,
                    type(obj),
                    smith._C._dispatch_keys(obj).raw_repr(),
                    ctx,
                    inner_data,
                )

            # For FakeTensors, use pytype if set, otherwise default to
            # smith.Tensor. This is important for cross-compilation where
            # we compile with fake tensors but run with real tensors.
            pytype = type(obj)
            if isinstance(obj, smith._subclasses.FakeTensor):
                pytype = obj.pytype if obj.pytype is not None else smith.Tensor

            return type(self)._unpickle_tensor, (
                smith.empty_like(obj, device="meta", requires_grad=obj.requires_grad),
                obj.device,
                pytype,
                smith._C._dispatch_keys(obj).raw_repr(),
                obj.grad,
        ...
        elif isinstance(obj, smith.nn.Module):
            if id(obj) not in self.guard_tree_values:
                return _Missing, ("module guard tree",)

            for attr in obj.__dict__.values():
                if isinstance(attr, (smith.Tensor, smith.nn.Module)):
                    continue
            ...
            if isinstance(obj, smith.nn.parallel.DistributedDataParallel):
                return type(self)._unpickle_ddp_module, (obj.__getstate__(),)

            if type(obj).__qualname__ == type(obj).__name__:
                return NotImplemented
            if obj.__class__.__getstate__ == smith.nn.Module.__getstate__:
                return type(self)._unpickle_module, (obj.__getstate__(),)

        elif inspect.ismodule(obj):
            return type(self)._unpickle_python_module, (obj.__name__,)

        elif isinstance(obj, smith._C.DispatchKeySet):
            return type(self)._unpickle_dispatch_key_set, (obj.raw_repr(),)

        elif isinstance(obj, smith._C._funcsmith.CInterpreter):
            return type(self)._unpickle_funcsmith_interpreter, (obj.serialize(),)

        elif (
            inspect.isclass(obj)
            and issubclass(obj, sympy.Function)
            and hasattr(obj, "_smith_handler_name")
        ):
            assert hasattr(obj, "_smith_unpickler")
            return obj._smith_unpickler, (obj._smith_handler_name,)
        ...
        if hasattr(smith.distributed, "distributed_c10d") and isinstance(
            obj, smith.distributed.distributed_c10d.Work
        ):
            if id(obj) not in self.guard_tree_values:
                return _Missing, ("distributed_c10d.Work",)

        if isinstance(obj, smith.nn.attention.SDPBackend):
        ...
        if (
            inspect.isclass(obj)
            and hasattr(smith.distributed, "fsdp")
            and issubclass(obj, smith.distributed.fsdp._fully_shard.FSDPModule)
        ):
            if obj is not smith.distributed.fsdp._fully_shard.FSDPModule:
                original_type = obj.__mro__[2]
                assert issubclass(original_type, smith.nn.Module)
                assert (
                    original_type
                    in smith.distributed.fsdp._fully_shard._fully_shard.get_cls_to_fsdp_cls()
                )
...
def pickle_guards_state(
    ...
    if all(
        smith.compiler.keep_portable_guards_unsafe(
            [
    ...
    try:
        ...
        raise smith._dynamo.exc.PackageError(str(e)) from e
    return buf.getvalue()
...
class CheckFunctionManager:
    def __init__(
        ...
        self.smith_function_mode_stack = (
            output_graph.smith_function_mode_stack if output_graph else None
        )
        ...
        self.global_state: Optional[smith._C._dynamo.guards.GlobalStateGuard] = None
        self.smith_function_mode_stack_check_fn: Optional[Callable[[], bool]] = None

        if not justknobs_check("blacksmith/compiler:guard_nn_modules"):
        ...
        if smith._dynamo.config.caching_precompile:
        ...
        if not output_graph.skip_guards_check and not output_graph.export:
            ...
            # here.  This is because, in scenarios where `smith._dynamo.reset`
        ...
        if save_guards:
            ...
            try:
                ...
                if smith._dynamo.config.strict_precompile or strict_error:
                    raise e
    ...
    def compile_check_fn(
        ...
        # Clear references to smith_function modes held in the list
        self.smith_function_mode_stack = None
        ...
        closure_vars = {
            ...
            "___check_smith_function_mode_stack": self.smith_function_mode_stack_check_fn,
            **SYMPY_INTERP,
            **_get_closure_vars(),
        }
...
def is_recompiles_enabled() -> bool:
    return smith._logging._internal.log_state.is_artifact_enabled("recompiles")


def is_recompiles_verbose_enabled() -> bool:
    return smith._logging._internal.log_state.is_artifact_enabled("recompiles_verbose")


# this will only be used if cpp guards are disabled
def make_smith_function_mode_stack_guard(
    initial_stack: list[smith.overrides.SmithFunctionMode],
) -> Callable[[], bool]:
    types = [type(x) for x in initial_stack]

    def check_smith_function_mode_stack() -> bool:
        cur_stack = get_smith_function_mode_stack()
    ...
    return check_smith_function_mode_stack
...
def get_guard_fail_reason_helper(
    ...
    if not guard_debug_info.result:
        ...
        reason = (
            "BACKEND_MATCH failure: smith.compile detected different backend callables."
            ...
            " More details: https://github.com/blacksmith/blacksmith/issues/168373"
        )
        ...
        reason = (
            ...
            " https://github.com/blacksmith/blacksmith"
        )
        reasons.append(reason)
