...
"""
This module implements the core frame evaluation handler for SmithDynamo's compilation system.
The eval frame handler intercepts Python bytecode execution at runtime to enable dynamic
compilation and optimization of Blacksmith code.
...
only execute at compile time should be placed in smith._dynamo.convert_frame.

The eval frame handler is the core mechanism that enables SmithDynamo to dynamically
intercept, analyze and optimize Blacksmith code during execution. It works by registering
a custom frame evaluation function that gets called for every Python frame, allowing
us to detect Blacksmith operations and trigger compilation as needed.
"""
...
from smith import _guards

# see discussion at https://github.com/blacksmith/blacksmith/issues/120699
from smith._C._dynamo.eval_frame import (  # noqa: F401
    _EvalFrameOverride,
    reset_code,
...
from smith._dynamo.types import ConvertFrameReturn, FrameAction, FrameExecStrategy
from smith._export.utils import _compiling_state_context
from smith._library.opaque_object import is_opaque_type
from smith._subclasses.fake_tensor import unset_fake_temporarily
from smith._utils_internal import DISABLE_JUSTKNOBS, justknobs_check, log_export_usage
...
from smith.fx import GraphModule, traceback as fx_traceback
from smith.fx.experimental._dynamism import (
...
from smith.fx.graph import _PyTreeCodeGen, _PyTreeInfo
...
if TYPE_CHECKING:
    ...
    from smith._dynamo.repro.after_dynamo import WrapBackendDebug
    from smith._subclasses import fake_tensor
    from smith.fx.node import Argument, Node, Target
...
if DISABLE_JUSTKNOBS:
    ...
    def _maybe_set_eval_frame(callback: DynamoCallback) -> DynamoCallback:
        ...
        # Users can disable smithDynamo by setting the JK to False.
        if not justknobs_check("blacksmith/compiler:enable_compiler_set_eval_frame"):
            smith._dynamo.utils.warn_once(
...
def _set_stance(stance: DynamoStance) -> DynamoStance:
    ...
    from smith._C._dynamo.eval_frame import get_eval_frame_callback

    callback = get_eval_frame_callback()

    if callback is not False and callback is not None:
        raise RuntimeError("attempted to set_stance in a smith.compile region")
...
def _callback_from_stance(callback: DynamoCallback) -> DynamoCallback:
    if _stance.stance == "default":
        ...
        def fail_callback(
            ...
            from smith._dynamo.guards import get_and_maybe_log_recompilation_reasons

            message = (
                "Detected recompile when smith.compile stance is 'fail_on_recompile'. "
        ...
        fail_callback._smithdynamo_orig_backend = callback  # type: ignore[attr-defined]

        return fail_callback
    else:
        raise RuntimeError(f"invalid smith.compile stance '{_stance}'")
...
def _create_delayed_compile_callback(
    ...
    def callback_fn(*args: Any, **kwargs: Any) -> convert_frame.ConvertFrameReturn:
        ...
        compiler_fn = callback._smithdynamo_orig_backend._smithdynamo_orig_backend  # type: ignore[union-attr]
        return _create_wrapped_callback(compiler_fn)(*args, **kwargs)

    # to prevent cache miss due to different backend
    callback_fn._smithdynamo_orig_backend = callback  # type: ignore[attr-defined]
...
def _debug_get_cache_entry_list(
    ...
    if callable(code):
        code = code.__code__
    return smith._C._dynamo.eval_frame._debug_get_cache_entry_list(code)


class OptimizedModule(smith.nn.Module):
    """
    Wraps the original nn.Module object and later patches its
    forward method to optimized self.forward method.
    """

    _smithdynamo_orig_callable: Callable[..., Any]
    get_compiler_config: Callable[[], Any]

    _opt_mod_attributes = {
        "_orig_mod",
        "dynamo_ctx",
        "_smithdynamo_orig_callable",
        "get_compiler_config",
        "forward",
        "_forward",
        "__dict__",
    ...
    def __init__(self, mod: smith.nn.Module, dynamo_ctx: _SmithDynamoContext) -> None:
    ...
    def _initialize(self) -> None:
        ...
        if isinstance(self.dynamo_ctx, DisableContext):
            ...
            # This may be a smith.nn.* instance in trace_rules.py which
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:
        if smith.nn.modules.module._has_any_global_hook():
            warnings.warn(
                "Using `smith.compile(module)` when there are global hooks on "
                "modules (e.g., from `register_module_forward_hook`); this will"
                " cause the hooks to fire an extra time for the "
                "`OptimizedModule` created by `smith.compile(module)`. If this "
    ...
    def _aot_compile(self, inputs: list[smith._dynamo.aot_compile.ModelInput]) -> None:
        """
        ...
        if not config.enable_aot_compile:
            raise RuntimeError(
                "AOT Compile is not enabled, please set smith._dynamo.config.enable_aot_compile=True"
            )
        if not self.dynamo_ctx.fullgraph:
            raise RuntimeError(
                "Graph breaks are not supported with aot compile. Please use smith.compile(fullgraph=True)."
            )
        ...
        backend = innermost_fn(
            self.dynamo_ctx.callback, unaltered_fn_attr="_smithdynamo_orig_backend"
        )
        from smith._dynamo.aot_compile import aot_compile_module
    ...
    def _save_aot_compiled_module(self, path: Optional[str] = None) -> bytes:
        ...
        from smith._dynamo.aot_compile import AOTCompiledModel
    ...
    def __reduce__(
        self,
    ) -> tuple[type[OptimizedModule], tuple[smith.nn.Module, _SmithDynamoContext]]:
...
def innermost_fn(
    fn: Callable[..., Any], unaltered_fn_attr: str = "_smithdynamo_orig_callable"
) -> Callable[..., Any]:
    """
    In case of nesting of _SmithDynamoContext calls, find the innermost
    function. SmithDynamo caches on fn.__code__ object, so its necessary to find
    the innermost function to pass on the optimize, run, disable etc.
    """
    # Don't unwrap bound methods. When a method is decorated at class definition
    # time, _smithdynamo_orig_callable is set on the wrapper function. Accessing
...
def _log_traced_frames() -> None:
    ...
    msg = f"SmithDynamo attempted to trace the following frames: [\n{msg}\n]"
    log.info(msg)
...
def _get_eval_frame_override() -> _EvalFrameOverride:
    if smith._dynamo.config.error_on_dynamo_callback_in_fullgraph_compiled_code:
        return _EvalFrameOverride.ERROR
    return _EvalFrameOverride.SKIP


class _SmithDynamoContext:
    def __init__(
        self,
        ...
        *,
        ...
        # Save the backends so that we can reset them during smith._dynamo.reset
        backend = innermost_fn(callback, unaltered_fn_attr="_smithdynamo_orig_backend")  # type: ignore[arg-type]
    ...
    def __enter__(self) -> None:
        if config.raise_on_ctx_manager_usage:
            raise RuntimeError(
                "smith._dynamo.optimize(...) is used with a context manager. "
                "Please refer to https://blacksmith.org/tutorials/intermediate/smith_compile_tutorial.html "
                "to use smith._dynamo.optimize(...) as an annotation/decorator. "
            )
    ...
    def __call__(self, fn: Any) -> Any:
        ...
        if config.caching_precompile:
            if self._package is not None and not self._package.is_initialized():
                fn_key = fn.forward if isinstance(fn, smith.nn.Module) else fn
        ...
        def aot_compile(example_inputs: tuple[tuple[Any, ...], dict[str, Any]]) -> Any:
            from smith._dynamo.aot_compile import aot_compile_fullgraph

            if smith._inductor.config.force_disable_caches:
                raise RuntimeError(
                    "Cannot precompile with smith._inductor.config.force_disable_caches=True; caching is required."
                )
            ...
            return aot_compile_fullgraph(
                ...
                hooks=self._hooks,
                backend=innermost_fn(
                    self.callback, unaltered_fn_attr="_smithdynamo_orig_backend"
                ),
        ...
        # Optimize the forward method of smith.nn.Module object
        if isinstance(fn, smith.nn.Module):
            mod = fn
            new_mod = OptimizedModule(mod, self)
            # Save the function pointer to find the original callable while nesting
            # of decorators.
            new_mod._smithdynamo_orig_callable = mod.forward

            # when compiling smith.nn.Module,
        ...
        if inspect.isclass(fn):
            ...
            if issubclass(cls_obj, smith.nn.Module):
        ...
        # NOTE [Top-level SmithInGraph functions]
        # Some callables (e.g. smith.exp) are represented as SmithInGraphFunctionVariable
        # when traced inside a frame. When such a function is passed directly to
        # smith.compile, we detect it here so we can force it through wrap_inline.
        from .variables import SmithInGraphFunctionVariable

        rule = trace_rules.lookup(fn)
        top_level_in_graph = isinstance(rule, type) and issubclass(
            rule, SmithInGraphFunctionVariable
        )
        ...
        is_jit_tracing = smith._C._is_tracing
        is_fx_symbolic_tracing = smith.fx._symbolic_trace.is_fx_symbolic_tracing
            ...
            try:
                ...
                if (
                    ...
                    if not _in_hop_compile():
                        if smith._guards.TracingContext.try_get() is not None:
                            return fn(*args, **kwargs)
                ...
                if is_jit_tracing():
                    raise RuntimeError(
                        "Detected that you are using FX to smith.jit.trace "
                        "a dynamo-optimized function. This is not supported at the moment."
                    )
                ...
                saved_dynamic_layer_stack_depth = (
                    smith._C._funcsmith.get_dynamic_layer_stack_depth()
                )
                ...
                try:
                    ...
                    # data in the SmithDynamo frames, so we strip them out.
                    raise e.remove_dynamo_frames() from None  # see SMITHDYNAMO_VERBOSE=1
                finally:
                    ...
                    smith._C._funcsmith.pop_dynamic_layer_stack_and_undo_to_depth(
                        saved_dynamic_layer_stack_depth
                    )
        ...
        if self.error_on_graph_break is not None:
            compile_wrapper._smithdynamo_inline = (  # type: ignore[attr-defined]
                external_utils.wrap_inline_with_error_on_graph_break(
                    fn, self.error_on_graph_break
                )
            )
        else:
            compile_wrapper._smithdynamo_inline = fn  # type: ignore[attr-defined]

        # Save the function pointer to find the original callable while nesting
        # of decorators.
        compile_wrapper._smithdynamo_orig_callable = fn  # type: ignore[attr-defined]
        ...
        if smith._dynamo.config.enable_aot_compile:
            compile_wrapper.aot_compile = aot_compile  # type: ignore[attr-defined]

        # If the function is called using smith._dynamo.optimize decorator, we
            ...
            if not hasattr(fn, "__code__"):
                raise RuntimeError(
                    textwrap.dedent(
                        ...
                        smith._dynamo.optimize is called on a non function object.
                        ...
                        >> opt_wrapper_fn = smith._dynamo.optimize(wrapper_fn)
                        """
                    )
                )
...
class OptimizeContext(_SmithDynamoContext):
    def __init__(
        self,
        callback: DynamoCallback,
        ...
        *,
        ...
        if config.compiled_autograd:
            ...
            if _dynamic is None:
                _dynamic = not smith._dynamo.config.assume_static_by_default
...
class RunOnlyContext(_SmithDynamoContext):
    def __init__(self) -> None:
        # cudagraph trees relies on generation increment
        def on_enter() -> None:
            smith._dynamo.mutation_guard.GenerationTracker.generation += 1
...
class DisableContext(_SmithDynamoContext):
    ...
    def __call__(self, fn: Callable[..., Any]) -> Callable[..., Any]:
        # Earlier this code was in the base class _SmithDynamoContext. But we
        ...
        def _fn(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                try:
                    ...
                    # Skip annotation for __smith_dispatch__ to avoid polluting
                    # node metadata during export. The disable on __smith_dispatch__
                    ...
                    if (
                        smith.compiler.is_exporting()
                        and fn_name != "__smith_dispatch__"
                    ):
                        with fx_traceback.annotate(
                            {
                                "_smithdynamo_disable": True,
                                "_smithdynamo_disable_recursive": True,
                                "_smithdynamo_disable_method": fn_name,
                            }
                        ):
                            return fn(*args, **kwargs)
                    return fn(*args, **kwargs)
        ...
        # fact that the old callback from smith.compile() is still active and under
        ...
        _fn._smithdynamo_disable = True  # type: ignore[attr-defined]
        _fn._smithdynamo_disable_msg = self.msg  # type: ignore[attr-defined]

        # Save the function pointer to find the original callable while nesting
        # of decorators.
        _fn._smithdynamo_orig_callable = fn  # type: ignore[attr-defined]

        _fn._smithdynamo_disable_recursive = True  # type: ignore[attr-defined]
...
def check_if_dynamo_supported() -> None:
    if sys.version_info >= (3, 15):
        raise RuntimeError("Python 3.15+ not yet supported for smith.compile")
...
def optimize(*args: Any, **kwargs: Any) -> Union[OptimizeContext, _NullDecorator]:
    def rebuild_ctx() -> Union[OptimizeContext, _NullDecorator]:
        ...
        if ca_kwargs_override:
            # NOTE: The process of translating other `smith.compile` kwargs to `smith._dynamo.optimize` kwargs
...
def _optimize(
    ...
    *,
    ...
    """
    The main entrypoint of SmithDynamo.  Do graph capture and call
    backend() to optimize extracted graphs.

    Args:
        backend: One of the two things:
            - Either, a function/callable taking a smith.fx.GraphModule and
            example_inputs and returning a python callable that runs the
            graph faster.
            One can also provide additional context for the backend, like
            smith.jit.fuser("fuser2"), by setting the backend_ctx_ctor attribute.
            See AOTAutogradMemoryEfficientFusionWithContext for the usage.
            - Or, a string backend name in `smith._dynamo.list_backends()`
        nopython: If True, graph breaks will be errors and there will
            be a single whole-program graph.
        error_on_graph_break: If not None, the current `error_on_graph_break` setting is set to the given value.
            See `smith._dynamo.error_on_graph_break()` for more details on what `error_on_graph_break` means.
    ...
    smith._C._log_api_usage_once("smith._dynamo.optimize")
    if (
        disable
        or os.environ.get("SMITHDYNAMO_DISABLE", "") == "1"
        or (not justknobs_check("blacksmith/compiler:enable_dynamo"))
    ):
    ...
    # _optimize_catch_errors in the field _smithdynamo_orig_backend. This can
...
@patch("smith._dynamo.symbolic_convert.explain", True)
def explain(f: Callable[..., Any], *extra_args: Any, **extra_kwargs: Any) -> Any:
    ...
    def inner(*args: Any, **kwargs: Any) -> ExplainOutput:
        ...
        graphs: list[smith.fx.GraphModule] = []
        ...
        def dynamo_graph_accumulating_compiler(
            gm: smith.fx.GraphModule, example_inputs: Any
        ) -> Callable[..., Any]:
...
class FlattenInputOutputSignature(smith.fx.Transformer):
    def __init__(
        self,
        m: smith.fx.GraphModule,
        flat_args: list[Any],
        matched_input_elements_positions: list[int],
        flat_results: Sequence[Any],
        matched_output_elements_positions: list[int],
        example_fake_inputs: list[smith.Tensor],
        ...
        for i in range(len(flat_args)):
            ...
            if i in matched_input_elements_to_fake:
                ...
                if fake_mode is not None and isinstance(flat_args[i], smith.Tensor):
    ...
    def transform(self) -> smith.fx.GraphModule:
...
def check_signature_rewritable(graph: smith.fx.GraphModule) -> None:
    input_errors = []
    ...
    for v in flat_values:
        if not is_supported_type(v):
            ...
            raise UserError(
                ...
                "using `smith.utils._pytree.register_pytree_node` or "
    ...
    def check_optional_input_and_error(f_sig: inspect.Signature) -> None:
        ...
        for name, param in f_sig.parameters.items():
            if param.default is not inspect.Parameter.empty:
                import smith._dynamo.graph_break_hints as graph_break_hints
        ...
        for val in candidates:
            if isinstance(val, tuple(common_constant_types)):
                ...
                raise AssertionError(
                    ...
                    'Please file an issue along with a paste of the logs from SMITH_LOGS="+export"',
                )
            else:
...
def export(
    ...
    *extra_args: Any,
    ...
    decomposition_table: Optional[
        dict[smith._ops.OpOverload, Callable[..., Any]]
    ] = None,
    ...
    **extra_kwargs: Any,
    ...
    """
    Export an input function f to a format that can be executed outside of Blacksmith using the FX graph.

    Args:
        f (callable): A Blacksmith function to be exported.

        aten_graph (bool): If True, exports a graph with ATen operators.
        If False, exports a graph with Python operators. Default is False.

        pre_dispatch (bool): If True, exports a graph with ATen operators,
        but before any logic in the Blacksmith dispatcher has run.
    ...
    Returns:
        ...
        Graph: An FX graph representing the execution of the input Blacksmith function with the provided arguments and options.
    ...
    def inner(*args: Any, **kwargs: Any) -> ExportResult:
        ...
        check_if_dynamo_supported()
        smith._C._log_api_usage_once("smith._dynamo.export")
        ...
        f = innermost_fn(f)
        call_to_inspect = f.forward if isinstance(f, smith.nn.Module) else f
        original_signature = inspect.signature(call_to_inspect)  # type: ignore[arg-type]
        graph = None
        out_guards = None
        graph_captured_input = None
        graph_captured_result: Optional[tuple[smith.Tensor, ...]] = None
        fake_mode = None
        result_traced = None
        ...
        def dynamo_normalization_capturing_compiler(
            gm: smith.fx.GraphModule, inner_example_inputs: list[Any]
        ) -> Callable[..., Any]:
            nonlocal graph
            ...
            def result_capturing_wrapper(*graph_inputs: Any) -> Any:
                ...
                with (
                    ...
                    def fakify_with_ambient(
                        path: KeyPath, t: Union[smith.Tensor, _IntWrapper, Any]
                    ) -> Any:
                    ...
                    graph_captured_result = smith.func.functional_call(
                        graph,
        ...
        if (
            ...
            and not isinstance(
                call_to_inspect, (smith._ops.OpOverloadPacket, smith._ops.OpOverload)
            )
            ...
            for k in shape_env.var_to_range:
                if isinstance(k, sympy.Integer):
                    constraint_violation_error = ConstraintViolationError(
                        ...
                        'Set SMITH_LOGS="+export" for more information.'
                    )
        if constraint_violation_error:
        ...
        if graph is None:
            ...
            fx_graph = smith.fx.Graph()
            for i, name in enumerate(parameter_names):
                if smith.is_tensor(flat_args[i]):
            ...
            graph = smith.fx.GraphModule(module, fx_graph)
            log.info(
        ...
        example_fake_inputs = [
            fake_mode.from_tensor(t) if isinstance(t, smith.Tensor) else t
        ...
        if aten_graph:
            ...
            def graph_with_interpreter(*args: Any) -> Any:
                with smith.fx.traceback.preserve_node_meta():
                    return smith.fx.Interpreter(graph).run(*args)  # type: ignore[arg-type]
            ...
            for node in graph.graph.find_nodes(op="get_attr"):
                if isinstance(getattr(graph, node.target), smith.Tensor):  # type: ignore[arg-type]
                    node.meta["val"] = fake_mode.from_tensor(
...
def _optimize_assert(
    ...
    *,
    ...
    The same as `smith._dynamo.optimize(backend)` but ignores
...
class SmithPatcher:
    ...
    def patch() -> None:
        ...
        # functions with @smith._disable_dynamo. However, this causes issues
        # with smith.deploy internally.
        from .decorators import disable

        smith.jit.trace = disable(
            smith.jit.trace, reason="tracing into SmithScript not fully supported"
        )
        smith.jit.trace_module = disable(
            smith.jit.trace_module,
            reason="tracing into SmithScript not fully supported",
        )
        smith.jit._get_trace_graph = disable(
            smith.jit._get_trace_graph,
            reason="tracing into SmithScript not fully supported",
        )
        smith.fx._symbolic_trace.Tracer.trace = disable(
            smith.fx._symbolic_trace.Tracer.trace,
            reason="tracing into FX not fully supported",
        )
        smith.distributions.Distribution.set_default_validate_args(False)

        from smith.optim import (
            adadelta,
            adagrad,
            adam,
            adamax,
        ...
        optimizer_classes = [
            opt
            for opt in smith.optim.__dict__.values()
            if inspect.isclass(opt) and issubclass(opt, smith.optim.Optimizer)
        ]

        # Note: we don't support sparsity or tracing through backwards
        excluded_optimizer_classes = {
            smith.optim.SparseAdam,
            smith.optim.LBFGS,
        }
    ...
    @staticmethod
    def suppress_smith_distributed_warnings(
        fn: Callable[..., Any],
    ) -> Callable[..., Any]:
        def inner_fn(*args: Any, **kwargs: Any) -> Any:
            with smith._logging.hide_warnings(
                smith._logging._internal.user_warning_filter
            ):
                return fn(*args, **kwargs)
