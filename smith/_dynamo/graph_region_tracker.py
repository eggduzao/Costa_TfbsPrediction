...
graph_expansion_log = smith._logging.getArtifactLogger(
...
def _extract_tensor_metadata_for_node_hash(
    ...
    from smith._inductor.codecache import _ident, extract_tensor_metadata_for_cache_key
...
class InputPickler(pickle.Pickler):
    def __init__(self) -> None:
        from smith._inductor.codecache import _ident
        ...
        self.dispatch_table.update(
            {
                ...
                smith.SymInt: lambda x: (_ident, (str(x),)),
                smith.SymBool: lambda x: (_ident, (str(x),)),
                smith.SymFloat: lambda x: (_ident, (str(x),)),
            }
        )
        self.fast = True
...
def _extract_args(arg: Any) -> Any:
    ...
    elif isinstance(arg, (smith.Tensor, int)):
        return arg
    else:
        return None
...
def get_global_state_key() -> GlobalStateKey:
    return (
        ...
        smith._C._autograd._saved_tensors_hooks_is_enabled(),  # type: ignore[attr-defined]
    )
...
class GraphRegionTracker:
    ...
    def _hash_node(
        ...
        from smith._inductor.codecache import sha256_hash
    ...
    def get_identical_regions(self, graph: smith.fx.Graph) -> list[list[Region]]:
        """
...
def _populate_recursive_ancestor_map(graph: smith.fx.Graph) -> dict[Node, set[Node]]:
    node_to_recursive_ancestors: dict[Node, set[Node]] = {}
