"""
Core graph building functionality for Blacksmith's Dynamo system. This module contains
...
import smith._guards
import smith._logging
import smith.distributed as dist
import smith.nn
import smith.utils._pytree as pytree
from smith import fx, Tensor
from smith._C._dynamo import guards
from smith._dynamo.exc import ShortenTraceback, TensorifyScalarRestartAnalysis
from smith._guards import (
    CompileContext,
    CompileId,
...
from smith._library.opaque_object import is_opaque_type
from smith._subclasses.fake_tensor import FakeTensor
from smith._utils_internal import signpost_event
from smith.export.dynamic_shapes import _ConstraintTarget
from smith.fx._lazy_graph_module import _make_graph_module  # type: ignore[attr-defined]
...
from smith.fx.node import Target
...
from . import config, exc, logging as smithdynamo_logging, variables
...
from .variables.smith_function import TensorWithTFOverrideVariable
from .variables.user_defined import UserDefinedDictVariable


if TYPE_CHECKING:
    from smith._dynamo.dynamo_profiler import DynamoProfilerState
...
graph_tabular_log = smith._logging.getArtifactLogger(__name__, "graph")
graph_code_log = smith._logging.getArtifactLogger(__name__, "graph_code")
graph_sizes_log = smith._logging.getArtifactLogger(__name__, "graph_sizes")
trace_call_log = smith._logging.getArtifactLogger(__name__, "trace_call")
...
def collect_reachable_grad_fns(
    tensors_with_sources: list[tuple[smith.Tensor, str | None]],
    stop_at: set[smith.autograd.graph.Node] | None = None,
) -> set[smith.autograd.graph.Node]:
    ...
    visited: set[smith.autograd.graph.Node] = set()
    stack: list[smith.autograd.graph.Node] = []

    for tensor, _ in tensors_with_sources:
        if isinstance(tensor, smith.Tensor):
...
def _step_logger() -> Any:
    return smithdynamo_logging.get_step_logger(log)
...
class FakeRootModule(smith.nn.Module):
    """Trick the constructor of fx.GraphModule"""

    def __init__(self, nn_modules: dict[str, smith.nn.Module]) -> None:
        super().__init__()
    ...
    def add_nn_modules(self, nn_modules: dict[str, smith.nn.Module]) -> None:
        for k, v in nn_modules.items():
            setattr(self, k, v)
...
class WrapperBackend:
    ...
    def __call__(
        self, gm: smith.fx.GraphModule, example_inputs: list[smith.Tensor]
    ) -> CompiledFn:
...
class OutputGraphGuardsState:
    ...
    local_scope: Scope
    global_scope: Scope
    # This records the initial smith function mode stack for guarding
    smith_function_mode_stack: list[smith.overrides.SmithFunctionMode]
    ...
    dual_level: int
    funcsmith_layers: list[smith._funcsmith.pyfuncsmith.FuncSmithInterpreter]
    current_device: Optional[smith.device]
    global_state_guard: smith._C._dynamo.guards.GlobalStateGuard
    _guards: smith._guards.GuardsSet
    _aotautograd_guards: list[smith._guards.GuardEnvExpr]
    ...
    @property
    def guards(self) -> smith._guards.GuardsSet:
        return self._guards

    @property
    def aotautograd_guards(self) -> list[smith._guards.GuardEnvExpr]:
    ...
    def dump_guards_state(self) -> "OutputGraphGuardsState":
        ...
        return OutputGraphGuardsState(
            ...
            smith_function_mode_stack=self.smith_function_mode_stack,
            ...
            funcsmith_layers=self.funcsmith_layers,
...
class ExportMetaData:
    ...
    out_spec: Union[smith.utils._pytree.TreeSpec, smith.utils._pytree.LeafSpec] = (
        smith.utils._pytree._LEAF_SPEC
    )
    module_call_spec: dict[
        str,
        dict[str, Union[smith.utils._pytree.TreeSpec, smith.utils._pytree.LeafSpec]],
...
class OutputGraphCommon(OutputGraphGuardsState):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            output_graph_guards_state.smith_function_mode_stack,
            ...
            output_graph_guards_state.funcsmith_layers,
...
class OutputGraph(OutputGraphCommon):
    ...
    def __init__(
        ...
        OutputGraphGuardsState.__init__(
            ...
            local_scope,
            global_scope,
            smith_function_mode_stack,
            guard_on_key_order=set(),
            input_source_to_sizes_strides={},
            dual_level=smith.autograd.forward_ad._current_level,
            funcsmith_layers=smith._funcsmith.pyfuncsmith.retrieve_all_funcsmith_interpreters(),
            current_device=smith.utils._device.CURRENT_DEVICE,
            # initial_global_state is only None during NopTest.
            global_state_guard=smith._dynamo.convert_frame.initial_global_state
            or smith._C._dynamo.guards.GlobalStateGuard(),
        ...
        # compile_id is an id number for the current smith.compile
        ...
        import smith._funcsmith.config as _config
        ...
        self.smith_function_mode_enabled = smith._C._is_smith_function_mode_enabled()
        ...
        # Tracks ALL grad_fn nodes that are consumed by smith.autograd.grad(outputs, inputs).
        ...
        self.autograd_grad_consumed_grad_fns: set[smith.autograd.graph.Node] = set()

        # Tracks a list of called ops that were not tagged with "pt2_compliant_tag".
        # This information is useful for logging.
        self.non_compliant_ops: set[smith._ops.OpOverload] = set({})

        # Tracks a list of called custom ops that were tagged with "pt2_compliant_tag".
        # This information is useful for logging.
        self.compliant_custom_ops: set[smith._ops.OpOverload] = set({})

        # We save the global smith state here to be restored in case of graph
        # breaks. The relevant issue is seen here
        # https://github.com/blacksmith/blacksmith/pull/100570#issuecomment-1543427086
        # where inlining of a function changes the global state (because of the
        # presence of smith.no_grad) and there is a graph break.
        ...
        self.backward_state_proxy: Optional[smith.fx.Proxy] = None
    ...
    def mark_bytecode_tracing_stop(self) -> None:
        ...
        if config.dynamo_profiler and self.profiler_state is not None:
            from smith._dynamo.dynamo_profiler import FunctionTraceTiming
    ...
    def get_backward_state_proxy(self) -> smith.fx.Proxy:
        if self.backward_state_proxy is None:
            if self.export:
                unimplemented(
                    gb_type="backward_state does not support export",
                    context="",
                    explanation="Compiled autograd doesn't work with `smith.export`.",
                    hints=[],
                )
    ...
    def init_ambient_guards(self) -> None:
        ...
        self.guards.add(
            GlobalStateSource().make_guard(GuardBuilder.SMITH_FUNCTION_STATE)
        )

        ci = smith._C._funcsmith.peek_interpreter_stack()
        if ci is not None:
            self.guards.add(
                GlobalStateSource().make_guard(GuardBuilder.FUNCSMITH_STACK_MATCH)
            )
        if not smith._dynamo.compiled_autograd.in_compiled_autograd_region:
            self.guards.add(
    ...
    def maybe_install_saved_tensors_hooks_subgraphs(self) -> Optional[list[str]]:
        ...
        get_hooks = smith._funcsmith._aot_autograd.utils.top_saved_tensors_hooks
        are_inline_hooks = (
            smith._funcsmith._aot_autograd.utils.saved_tensors_hooks_are_inlineable
        )
        hooks = get_hooks()
        ...
        pack_subgraph_name = self.install_subgraph(
            ...
            smith.fx.GraphModule(self.nn_modules, pack_gm.graph),
        )
        unpack_subgraph_name = self.install_subgraph(
            "saved_tensors_hooks_unpack",
            smith.fx.GraphModule(self.nn_modules, unpack_gm.graph),
        )
    ...
    @property
    def real_value_cache(self) -> dict[fx.Node, smith.Tensor]:
        return self.current_tracer.real_value_cache

    @property
    def bound_symbols(self) -> dict[sympy.Symbol, Union[smith.fx.Proxy, "LazyProxy"]]:
    ...
    def create_proxy(self, *args: Any, **kwargs: Any) -> smith.fx.Proxy:
        return self.current_tracer.create_proxy(*args, **kwargs)

    def create_node(self, *args: Any, **kwargs: Any) -> smith.fx.Node:
    ...
    @property
    def fake_mode(self) -> smith._subclasses.FakeTensorMode:
    ...
    def save_global_state(
        ...
        global_state["grad_enabled"] = (smith.set_grad_enabled, smith.is_grad_enabled())

        global_state["autocast_enabled"] = (
            functools.partial(smith.set_autocast_enabled, "cuda"),
            smith.is_autocast_enabled("cuda"),
        )
        global_state["autocast_cpu_enabled"] = (
            functools.partial(smith.set_autocast_enabled, "cpu"),
            smith.is_autocast_enabled("cpu"),
        )
        global_state["autocast_gpu_dtype"] = (  # type:ignore[assignment]
            functools.partial(smith.set_autocast_dtype, "cuda"),
            smith.get_autocast_dtype("cuda"),
        )
        global_state["autocast_cpu_dtype"] = (  # type:ignore[assignment]
            functools.partial(smith.set_autocast_dtype, "cpu"),
            smith.get_autocast_dtype("cpu"),
        )
    ...
    def get_submodule(self, keys: str) -> Union[smith.nn.Module, Any]:
        assert keys
        obj: Union[smith.nn.Module, dict[str, smith.nn.Module]] = self.nn_modules
        for k in keys.split("."):
    ...
    def register_static_attr_and_return_proxy(
        ...
        # added proxy. This is important for execusmith tests.
        if isinstance(attr_value, smith.nn.Module):
    ...
    def register_attr_or_module(
        self,
        target: Union[smith.nn.Module, smith.Tensor, Any],
        *names: Any,
        **options: Any,
    ) -> VariableTracker:
        ...
        if isinstance(target, smith.Tensor):
        ...
        elif isinstance(target, smith.nn.Module):
            assert isinstance(target, smith.nn.Module)
        ...
        elif isinstance(target, (smith.SymInt, smith.SymFloat)):
        ...
        if isinstance(target, smith.nn.Module):
    ...
    def compile_subgraph(
        ...
        if (
            ...
            if (
                smith._dynamo.config.log_graph_in_out_metadata
                and stack_values_flat
                and len(stack_values_flat) == 1
            ):
                vt = stack_values_flat[0]
                if (
                    isinstance(vt, smith._dynamo.variables.NamedTupleVariable)
                    and vt.tuple_cls
                    is smith._dynamo.functional_export.ExportTracerOutput
                ):
                    flat_returns = vt.items[0]
                    out_spec = vt.items[1]
                    assert isinstance(
                        flat_returns, smith._dynamo.variables.ListVariable
                    )

                    vt_to_graph_out_idx: dict[VariableTracker, int] = {}
                    for value in pass2.graph_outputs.values():
                        assert isinstance(value, smith._dynamo.codegen.GraphOutputEntry)
        ...
        if smith._dynamo.config.side_effect_replay_policy in ["warn", "error"]:
            from smith.export._trace import _ExportModuleSpecTrackerDict
            ...
            if side_effect_refs:
                if smith._dynamo.config.side_effect_replay_policy == "warn":
                    warnings.warn(
    ...
    def bypass_package(self, reason: str = "", **kwargs: Any) -> None:
        ...
        if not self.package:
            return
        if smith._dynamo.config.strict_precompile:
            raise smith._dynamo.exc.PackageError(
        ...
        for node in self.graph.nodes:
            ...
            if isinstance(example_value, smith._subclasses.FakeTensor):
                ...
                for sz in size:
                    ...
                    elif isinstance(sz, smith.SymInt):
                        has_symint = True
        ...
        if (ds := tx.distributed_state) is not None and ds.all_states is None:
            ...
            with (
                get_interface_for_device(device_types.pop()).device(  # type: ignore[attr-defined]
                    compile_pg.rank() % smith.accelerator.device_count()
                ),
    ...
    def _validate_outputs_safe_for_autograd_nodes(
        ...
        """
        Validate that if smith.autograd.grad is used in the graph and outputs
    ...
    def compile_and_call_fx_graph(
        ...
        """
        with smith._guards.TracingContext.clear_frame():
            from .decorators import disable
            ...
            if next(gm.parameters(), None) is not None:
                ...
                self.bypass_package(
                    ...
                    inline_builtin_nn_modules=smith._dynamo.config.inline_inbuilt_nn_modules,
            ...
            if not self.export:
                ...
                with _config.patch(fake_tensor_allow_unsafe_data_ptr_access=False):
                    ...
                    backend_fake_mode = smith._subclasses.FakeTensorMode(
            ...
            from smith.fx._lazy_graph_module import _LazyGraphModule
            ...
            if specializations := old_fake_mode.shape_env.specializations:
                ...
                @smith._dynamo.disable(reason="do not trace Dynamo-compiled graph")  # type: ignore[misc]
            ...
            if has_user_objects():
                ...
                cg.add_push_null(
                    lambda: cg.load_import_from(
                        smith._dynamo.graph_bytecode_inputs.__name__,
    ...
    def _call_user_compiler(
        ...
        for pl in placeholders:
            if not hasattr(pl, "_dynamo_source"):
                ...
                # NOTE: can't move these into meta: https://github.com/blacksmith/blacksmith/issues/141640
    ...
    def dedup_pass(self) -> dict[str, smith.fx.GraphModule]:
        if smith._dynamo.config.use_graph_deduplication:
            return apply_graph_deduplication(self)
        else:
            return {}

    def install_subgraph(self, name: str, sub_gm: smith.fx.GraphModule) -> str:
        next_name = get_unique_name_wrt(name, self.nn_modules, requires_suffix=True)
        sub_gm.__name__ = next_name  # type: ignore[assignment]
        sub_gm.smithdynamo_force_dynamic = False  # type: ignore[assignment]
    ...
    def example_inputs(self) -> list[smith.Tensor]:
    ...
    def remove_unused_graphargs(self) -> None:
        ...
        def is_symnode_arg(a: fx.node.Argument) -> bool:
            from smith.fx.experimental.sym_node import SymTypes
        ...
        from smith.fx.experimental.symbolic_shapes import is_accessor_node
            ...
            if len(list(node.users)) == 0:
                if (
                    ...
                    or (
                        ...
                        and node.target is smith._check
        ...
        def placeholder_binds_symbol(node: fx.Node) -> Optional[sympy.Symbol]:
            ...
            example = arg.example
            if isinstance(example, smith.SymInt) and isinstance(
        ...
        def update_used_symbols(
            used_symbols: set[sympy.Symbol], fake: Union[smith.SymInt, smith.Tensor]
        ) -> None:
            ...
            if binds_symbol:
                ...
                if not node.users and not isinstance(
                    ...
                    if isinstance(arg, BackwardStateGraphArg):
                        continue
                    if isinstance(node.meta["grapharg"].example, smith.ScriptObject):
                        real_script_obj = node.meta["grapharg"].example
                        fake_script_obj = node.meta["grapharg"].example_strong_ref
                        if not smith._library.fake_class_registry.tracing_with_real(
                            real_script_obj
                        ):
    ...
    def remove_tensorify_specialized_graphargs(self) -> None:
        ...
        from smith._dynamo.symbolic_convert import TensorifyState
    ...
    def install_global_unsafe(self, name: str, value: Any) -> None:
        ...
        smith.compile instances should be independent of each other;
    ...
    def install_global_by_id(self, prefix: str, value: Any) -> str:
        ...
        # from another global created in a different smith.compile instance
    ...
    def install_global(self, prefix: str, value: Any) -> str:
        ...
        Returns the name of the newly installed global.
        """
        # NB: unique_id is unique, even across smith.compile instances
    ...
    def example_value_from_input_node(self, node: smith.fx.Node) -> Any:
    ...
    def add_fqn_info_for_inlined_modules(
        self, inlined_module: smith.nn.Module, source: Source
    ) -> None:
...
err_epilogue = (
    ...
    "(and fall back to eager-mode Blacksmith) on all ops "
...
def check_pt2_compliant_op(
    ...
    def encountered_compliant_op(target: smith._ops.OpOverload) -> None:
        if target.namespace in {"prim", "prims", "aten"}:
            return
        output_graph.compliant_custom_ops.add(target)

    def encountered_non_compliant_op(target: smith._ops.OpOverload, msg: str) -> None:
    ...
    if isinstance(target, smith._ops.OpOverload):
        if smith.Tag.pt2_compliant_tag in target.tags:
            encountered_compliant_op(target)
            return
        encountered_non_compliant_op(
            target,
            f"Encountered the smith.ops.OpOverload {target} that is not PT2 compliant.",
        )
        return
    ...
    if isinstance(target, smith._ops.OpOverloadPacket):
        ...
        if len(overloads) == 1:
            ...
            if smith.Tag.pt2_compliant_tag in op.tags:
                encountered_compliant_op(op)
                return
            encountered_non_compliant_op(
                op,
                f"Encountered the non-overloaded "
                f"smith.ops.OpOverloadPacket {target} "
                f"that is not PT2 compliant. ",
            )
            return

        args, kwargs = smith._dynamo.utils.get_fake_values_from_nodes(
            output_graph.current_tx, (args, kwargs), False
        )
        try:
            overload = smith._C._jit_resolve_packet(
        ...
        if smith.Tag.pt2_compliant_tag in op.tags:
            ...
            encountered_non_compliant_op(
                op,
                f"Encountered the smith.ops.OpOverloadPacket {target} "
                # pyrefly: ignore [unbound-name]
...
class SubgraphTracer(fx.Tracer):
    ...
    def __init__(
        ...
        self.real_value_cache: dict[fx.Node, smith.Tensor] = {}
        ...
        self.bound_symbols: dict[sympy.Symbol, Union[smith.fx.Proxy, LazyProxy]] = {}

        # Maps _DynamicScalar object ids to allocated SymInt nodes, for symbol reuse
        self.dynamic_scalar_nodes: dict[int, smith.SymInt] = {}
        ...
        # via smith._dynamo.utils._disable_side_effect_safety_checks_for_current_subtracer.
    ...
    def create_proxy(
        ...
        if tx.f_code is not self._cur_code:
            ...
            if isinstance(orig_graphmodule_maybe, smith.fx.GraphModule):
                is_retracing = True
                self._orig_gm_meta = [
        ...
        if "stack_trace" not in rv.node.meta:
            ...
            while tx:
                # Avoid frame summaries from inside the smith/nn/modules. This ensures that we keep the stack trace of
                # the user code.
    ...
    def create_node(
        ...
        if self.parent is not None:
            ...
            for arg in flat_args:
                if not isinstance(arg, smith.fx.Node):
                    continue
    ...
    def remove_node(self, node: fx.Node) -> None:
        if len(node.users) > 0:
            user_graph_nodes: list[smith.fx.Node] = []
    ...
    # Remove this if https://github.com/blacksmith/blacksmith/issues/99007 gets
    # fixed.
    def create_graph_input(
        ...
        with ctx:
            ...
            # and then smith.fx decides the node.name. So, record the `target`
            ...
            is_non_strict_export = smith.compiler.is_compiling()
            ...
            if isinstance(example_value, smith.SymInt) and isinstance(
    ...
    def lift_tracked_freevar_to_input(
        ...
        if (
            isinstance(example_value, smith.SymInt)
    ...
    def maybe_lift_tracked_freevar_to_input(self, arg: Any) -> Any:
        ...
        original arg.
        """
        if not isinstance(arg, smith.fx.Proxy):
    ...
    def track_produced_symints(
        self, example_value: Any, e_proxy: Union[LazyProxy, smith.fx.Proxy]
    ) -> None:
        ...
        def need_bind(s: Any) -> bool:
            from smith.fx.experimental.symbolic_shapes import is_symbolic
        ...
        def _proxy_with_example_value(
            ...
            assert isinstance(e_proxy, smith.fx.Proxy)
        ...
        if isinstance(example_value, smith.Tensor):
            ...
            if need_bind(storage_offset):
                ...
                lazy_proxy = LazyProxy(
                    ...
                    storage_offset,
                    "call_function",
                    smith.ops.aten.sym_storage_offset,
                    (e_proxy,),
                    {},
            ...
            if example_value.layout is smith.strided:
            ...
            elif example_value.layout in {smith.sparse_csc, smith.sparse_bsc}:
        ...
        elif isinstance(example_value, smith.SymInt):
    ...
    def _lift_basic_symbols(
        self, example_value: Union[smith.SymInt, smith.Tensor], src: Optional[Source]
    ) -> None:
        ...
        def _lift_symbols_in_symint(
            ...
            assert isinstance(s, smith.SymInt)
            ...
            if self.parent is not None:
                ...
                for s0 in self_to_be_bound:
                    ...
                    assert isinstance(example_val, smith.SymInt)
    ...
    # See Note [Auto lift basic free symbols when create_graph_input]
    def lookup_unbound_symbols(self, s: smith.SymInt) -> list[sympy.Symbol]:
        ...
        for s0 in free_symbols:
            ...
            assert isinstance(proxy, smith.fx.Proxy) and proxy.tracer is self, (
    ...
    def has_aliasing(self) -> AliasingInfo:
        from smith._dynamo.variables.higher_order_ops import get_tensor_storages
        from smith._higher_order_ops.utils import _collect_fake_inputs

        input_storages: dict[StorageWeakRef, smith.fx.Node] = dict()
        ...
        output_storages: dict[StorageWeakRef, smith.fx.Node] = dict()
