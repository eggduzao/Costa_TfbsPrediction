...
"""Exception handling and error reporting for SmithDynamo.

This module provides a comprehensive set of exception classes and utilities for error
handling in SmithDynamo. It includes:

Base Exceptions:
    - SmithDynamoException: Base class for all SmithDynamo-specific exceptions
    - Various specialized subclasses for different error scenarios

User Error Handling:
    - UserError: Exceptions for user-facing errors in SmithDynamo usage
...
def exportdb_error_message(case_name: str) -> str:
    return (
        ...
        + "https://blacksmith.org/docs/main/generated/exportdb/index.html#"
        + case_name.replace("_", "-")
    )


log = logging.getLogger(__name__)
graph_breaks_log = smith._logging.getArtifactLogger(__name__, "graph_breaks")


class SmithDynamoException(RuntimeError):
    """Base exception class for all SmithDynamo-specific exceptions.

    Attributes:
        _smith_dynamo_tracer_output: Optional tracer output attached to the exception
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._smith_dynamo_tracer_output: Optional[DynamoTracerOutput] = None
        self.frame_exec_strategy: FrameExecStrategy | None = None


class InternalSmithDynamoError(SmithDynamoException):
    pass


class ResumePrologueTracingError(SmithDynamoException):
    pass


class RestartAnalysis(SmithDynamoException):
...
# will be issued!
class SkipFrame(SmithDynamoException):
    pass


class SmithRuntimeError(SmithDynamoException):
    def __init__(self, msg: str, real_stack: StackSummary | None = None) -> None:
        ...
        self.real_stack = (
            ...
            else smith._guards.TracingContext.extract_stack()
        )


class InvalidBackend(SmithDynamoException):
    def __init__(self, name: str) -> None:
        super().__init__(
            f"Invalid backend: {name!r}, see `smith._dynamo.list_backends()` for available backends."
        )


class ResetRequired(SmithDynamoException):
    def __init__(self) -> None:
        super().__init__(
            textwrap.dedent(
                """
                Must call `smith._dynamo.reset()` before changing backends.  Detected two calls to
                `smith.compile()` with a different backend compiler arguments.
                """
            )
        )


class ShortenTraceback(SmithDynamoException):
    def __init__(
...
# won't be checked
class Unsupported(SmithDynamoException):
    def __init__(
        self,
        msg: str,
        ...
        *,
        ...
        super().__init__(msg)
        if not real_stack:
            real_stack = smith._guards.TracingContext.extract_stack()
...
class UnknownPropertiesDuringBackwardTrace(SmithDynamoException):
    pass


class RecompileError(SmithDynamoException):
    pass


class InfiniteGeneratorError(SmithDynamoException):
    # Raised when the number of yielded values is greater than MAX_ITERATOR_LIMIT
    pass


class CondOpArgsMismatchError(SmithDynamoException):
    """
...
class UserError(Unsupported):
    def __init__(
        ...
        """
        Type of errors that would be valid in Eager, but not supported in SmithDynamo.
...
# debug exception thrown when tracing smith._dynamo.step_unsupported()
class StepUnsupported(SmithDynamoException):
    def __init__(self, msg: str, real_stack: StackSummary | None = None) -> None:
        super().__init__(msg)
        self.msg = msg
        if not real_stack:
...
class UnsafeScriptObjectError(SmithDynamoException):
    pass


class UncapturedHigherOrderOpError(SmithDynamoException):
    def __init__(self, msg: str, real_stack: StackSummary | None = None) -> None:
        super().__init__(msg)
        self.msg = msg
        self.real_stack = (
...
# specialization in Blacksmith proper could also trigger this problem
class FailOnRecompileLimitHit(Exception):
    pass


class PackageError(SmithDynamoException):
    pass


class ObservedException(SmithDynamoException):
...
exceptions_allowed_to_be_fallback = (
    ...
    smith._subclasses.fake_tensor.DynamicOutputShapeException,
    smith._subclasses.fake_tensor.UnsupportedOperatorException,
    smith._subclasses.fake_tensor.UnsupportedFakeTensorException,
    smith._subclasses.fake_tensor.UnsupportedMutationAliasingException,
)
...
def unimplemented_with_warning(
    ...
    *,
    ...
    # encountered in the smith.compile stack which is worth showing as warning
...
def get_gbid_documentation_link(gb_type: str) -> Optional[str]:
    ...
    GRAPH_BREAK_SITE_URL = (
        "https://meta-blacksmith.github.io/compile-graph-break-site/gb/"  # @lint-ignore
    )
...
def augment_exc_message(exc: Exception, msg: str = "\n", export: bool = False) -> None:
 ...
 smith._dynamo.replay('{exc.record_filename}').\n"
        )

    if not config.verbose and hasattr(exc, "real_stack"):
        msg += (
            "\nSet SMITHDYNAMO_VERBOSE=1 for the internal stack trace "
            "(please do this especially if you're reporting a bug to Blacksmith). "
            'For even more developer context, set SMITH_LOGS="+dynamo"\n'
        )
...
def filter_stack(stack: StackSummary) -> StackSummary:
    ...
    for frame in stack:
        ...
        if "eval_frame" in frame.filename or (
            frame.line and "smith._dynamo.optimize(" in frame.line
        ):
            continue
...
def remove_resume_prefix(name: str) -> Optional[str]:
    ...
    match = re.match(f"{SMITH_DYNAMO_RESUME_IN_PREFIX}_(\\w+)_at_\\d+", name)
    if match:
...
def collapse_resume_frames(stack: StackSummary | list[FrameSummary]) -> StackSummary:
    ...
    File "..." line 3, in f
        <line 3>
    File "..." line 5, in smith_dynamo_resume_in_f_at_80
        <line 5>
    File "..." line 10, in smith_dynamo_resume_in_f_at_120
        <line 10>
...
def format_error_msg_verbose(
    ...
    msg += "=" * 10 + " SmithDynamo Stack Trace " + "=" * 10 + "\n"
    msg += format_exc()
...
def format_skip_frame_message(code: Optional[types.CodeType], reason: str) -> str:
    if code is not None:
        ...
        return (
            f"smith.compile intentionally decided to skip the frame {frame_info} and fall back to eager.\n"
            f"Reason: {reason}"
        )
    else:
        return (
            f"smith.compile intentionally decided to skip the frame and fall back to eager.\n"
            f"Reason: {reason}"
        )
