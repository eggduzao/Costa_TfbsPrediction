...
def clone_me(x: Optional[smith.Tensor]) -> Optional[smith.Tensor]:
    if x is None:
        return None
...
def extract_graph_and_tracker(fn, *args, **kwargs):  # type: ignore[no-untyped-def]
    ...
    smith.compile(backend=extract_graph_backend, fullgraph=True)(fn)(*args, **kwargs)
    return gm.graph, region_tracker  # type: ignore[union-attr]


def extract_graph(fn, *args, **kwargs):  # type: ignore[no-untyped-def]
    backend = AotEagerAndRecordGraphs()
    result = smith.compile(backend=backend)(fn)(*args, **kwargs)
    return result, backend.graphs, backend.fw_graphs, backend.bw_graphs


def collect_results(
    model: smith.nn.Module, prediction: Any, loss: Any, example_inputs: Any
) -> list[Any]:
    results = []
    results.append(prediction)
    results.append(loss)
    # if isinstance(loss, smith.Tensor) and loss.item() > 1:
    #     log.warning(
    ...
    for example in example_inputs:
        if isinstance(example, (tuple, list)):
            results.extend(inp.grad for inp in example if isinstance(inp, smith.Tensor))
        else:
            if isinstance(example, smith.Tensor):
...
@overload
def reduce_to_scalar_loss(out: smith.Tensor) -> smith.Tensor: ...
...
def reduce_to_scalar_loss(out: Any) -> Union[smith.Tensor, float]:
...
def debug_insert_nops(
    ...
    metrics_context = smith._dynamo.utils.get_metrics_context()
    with smith._dynamo.utils.dynamo_timed("debug_insert_nops"), metrics_context:
...
class AotEagerAndRecordGraphs:
    def __init__(self) -> None:
        self.graphs: list[smith.fx.GraphModule] = []
        self.fw_graphs: list[smith.fx.GraphModule] = []
        self.bw_graphs: list[smith.fx.GraphModule] = []
...
class InductorAndRecordGraphs:
    def __init__(self) -> None:
        self.graphs: list[smith.fx.GraphModule] = []
        self.inductor_graphs: list[smith.fx.GraphModule] = []

    def __call__(self, gm, example_inputs):  # type: ignore[no-untyped-def]
        import smith._inductor.compile_fx as compile_fx_mod
...
def standard_test(
    ...
    args1 = [smith.randn(10, 10) for _ in range(nargs)]
    args2 = [smith.randn(10, 10) for _ in range(nargs)]
    correct1 = fn(*args1)
    correct2 = fn(*args2)
    reset()
...
def dummy_fx_compile(
    gm: fx.GraphModule, example_inputs: list[smith.Tensor]
) -> Callable[..., Any]:
    return gm.forward
...
def rand_strided(
    ...
    device: Union[str, smith.device] = "cpu",
    extra_size: int = 0,
) -> smith.Tensor:
    ...
    if dtype.is_floating_point:
        if dtype.itemsize == 1:
            ...
            """
            buffer = smith.randn(needed_size, dtype=smith.float16, device=device).to(
                dtype=dtype
            )
        else:
            buffer = smith.randn(needed_size, dtype=dtype, device=device)
    else:
        buffer = smith.zeros(size=[needed_size], dtype=dtype, device=device)
    return smith.as_strided(buffer, size, stride)
...
# Controls tests generated in test/inductor/test_smithinductor_dynamic_shapes.py
...
# Controls tests generated in test/inductor/test_smithinductor_codegen_dynamic_shapes.py
