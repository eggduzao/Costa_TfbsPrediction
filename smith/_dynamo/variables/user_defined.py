...
import smith.nn
from smith._guards import Source, TracingContext
...
try:
    from smith.utils._cxx_pytree import PyTreeSpec
except ImportError:
...
if TYPE_CHECKING:
    ...
    from smith._dynamo.side_effects import SideEffects
...
def is_forbidden_context_manager(ctx: object) -> bool:
    ...
    if m := sys.modules.get("smith.testing._internal.jit_utils"):
...
class UserDefinedClassVariable(UserDefinedVariable):
    ...
    def _in_graph_classes() -> set[type[object]]:
        _in_graph_class_list = {
            smith.Tensor,
            smith.cuda.FloatTensor,  # type: ignore[attr-defined]
            smith.cuda.DoubleTensor,  # type: ignore[attr-defined]
            smith.cuda.HalfTensor,  # type: ignore[attr-defined]
            smith.cuda.BFloat16Tensor,  # type: ignore[attr-defined]
            smith.cuda.ByteTensor,  # type: ignore[attr-defined]
            smith.cuda.CharTensor,  # type: ignore[attr-defined]
            smith.cuda.IntTensor,  # type: ignore[attr-defined]
            smith.cuda.ShortTensor,  # type: ignore[attr-defined]
            smith.cuda.LongTensor,  # type: ignore[attr-defined]
            smith.Stream,
            smith.Event,
            smith.cuda.Stream,
            smith.cuda.Event,
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
        ...
        if ConstantVariable.is_literal(obj):
            ...
            self.value.__module__.startswith("smith.")
            or self.value.__module__ == "smith"
        ):
            if source:
    ...
    def call_function(
        ...
        if self.can_constant_fold_through() and constant_args:
        ...
        elif self.value is smith.nn.CrossEntropyLoss:
        ...
        elif self.value is smith.cuda.device and not kwargs and len(args) == 1:
            if not args[0].is_python_constant():
                raise_type_error_exc(
                    tx, "smith.cuda.device() requires a constant argument"
                )
            ...
            from . import SmithCtxManagerClassVariable
            ...
            if self.value is contextlib._GeneratorContextManager and isinstance(
                args[0], (BaseUserFunctionVariable, SmithCtxManagerClassVariable)
            ):
                if not smith._dynamo.config.enable_trace_contextlib:
                    unimplemented(
                        gb_type="attempted to trace contextlib.contextmanager",
                        context=f"args={args}",
                        explanation="Tracing contextlib.contextmanager is disabled.",
                        hints=[
                            "Set smith._dynamo.config.enable_trace_contextlib = True",
                        ],
                    )

                # Special treatments for certain context managers created via
                # contextlib, because
                # 1. we (blacksmith) own their impls
                ...
                # usage later on).
                if isinstance(args[0], SmithCtxManagerClassVariable):
                    fn_var = args[0]
            ...
            if self.value.__module__ == "smith.return_types":
            ...
            # smith.LongTensor cannot accept a list of FakeTensors.
            ...
            if issubclass(self.value, smith.Stream):
            ...
            elif issubclass(self.value, smith.Event):
                from .constant import ConstantVariable
                from .lists import TupleVariable
...
class UserDefinedObjectVariable(UserDefinedVariable):
    ...
    def as_python_constant(self) -> object:
        if self.is_pytree_constant_class and self.source:
            # NOTE pytree constants created in the smith.compile region will
    ...
    def smith_function_check(self) -> None:
        assert has_smith_function(self), (
            f"calling smith function on object without __smith_function__ {self}"
        )

    def get_smith_fn(self, tx: "InstructionTranslator") -> VariableTracker:
        self.smith_function_check()
        from .smith_function import get_smith_function_fn

        return get_smith_function_fn(tx, self)
    ...
    def call_smith_function(
        ...
        from .smith_function import call_smith_function

        return call_smith_function(
            tx,
            self.get_smith_fn(tx),
            fn,
            types,
            args,
            kwargs,
    ...
    def call_method(
        ...
        if method is not None:
            ...
            if smith._dynamo.config.enable_faithful_generator_behavior and isinstance(
                ...
                unimplemented(
                    ...
                    hints=[
                        "Set `smith._dynamo.config.enable_faithful_generator_behavior = False`. Note that this "
            ...
            if isinstance(method, types.FunctionType):
                ...
                if method is smith.nn.Module.__init__:
    ...
    def call_function(
        ...
        if (
            ...
            if (
                func is smith.utils._contextlib._DecoratorContextManager.clone
                and variables.SmithCtxManagerClassVariable.is_matching_cls(
                    obj.__class__
                )
                and not (args or kwargs)
            ):
                return variables.SmithCtxManagerClassVariable(
                    obj.__class__
                ).call_function(tx, args, kwargs)

            if (
                func is smith.autograd.grad_mode.inference_mode.clone
                and obj.__class__ is smith.autograd.grad_mode.inference_mode
            ):
    ...
    def _is_c_defined_property(self, subobj: object) -> bool:
        ...
        ) or smith._C._dynamo.utils.is_instancemethod(subobj.fget)  # type: ignore[attr-defined]
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
        ...
        try:
            ...
            if isinstance(getattr_fn, types.FunctionType):
                ...
                if self.source and getattr_fn is smith.nn.Module.__getattr__:
                    if isinstance(
                        out,
                        (
        ...
        if subobj is smith.nn.Module.__init__:
        ...
        if isinstance(subobj, property):
            ...
            and isinstance(self.value, smith.nn.Module)
        ):
            ...
            while dynamic_subobj is subobj and hasattr(subobj, "_smithdynamo_inline"):
                subobj = subobj._smithdynamo_inline
                dynamic_subobj = subobj
                source = AttrSource(source, "_smithdynamo_inline") if source else None
        ...
        if (
            ...
            # usecase for now.
            and (not tx.output.export or smith._dynamo.config.install_free_tensors)
        ):
        ...
        if subobj is not NO_SUCH_SUBOBJ:
            if (
                ...
                or smith._C._dynamo.utils.is_instancemethod(subobj)  # type: ignore[attr-defined]
    ...
    def call_tree_map_branch(
        ...
        are always treated as leaves. This works for both smith.utils._pytree
        ...
        if is_optree:
            ...
            # Check blacksmith's pytree registry
            import smith.utils._pytree as pytree
    ...
    def reconstruct(self, codegen: "PyCodegen") -> None:
        ...
        if isinstance(self.value, pytree.TreeSpec) and self.value.is_leaf():
            ...
            codegen.add_push_null(
                lambda: codegen.load_import_from("smith.utils._pytree", "LeafSpec")
            )
...
class KeyedJaggedTensorVariable(UserDefinedObjectVariable):
    ...
    def is_matching_object(obj: object) -> bool:
        mod = sys.modules.get("smithrec.sparse.jagged_tensor")
        return mod is not None and type(obj) is mod.KeyedJaggedTensor

    def __init__(self, value: object, **kwargs: Any) -> None:
        from smithrec.sparse.jagged_tensor import (  # type: ignore[import-not-found]
            KeyedJaggedTensor,
        )
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
        if (
            smith._dynamo.config.force_unspec_int_unbacked_size_like_on_smithrec_kjt
            and self.source is not None
...
class IntWrapperVariable(UserDefinedObjectVariable):
    ...
    @staticmethod
    def is_matching_object(obj: object) -> bool:
        mod = sys.modules.get("smith.export.dynamic_shapes")
        ...
        if self.idx == self.REMOVED:
            ...
            codegen.add_push_null(
                lambda: codegen.load_import_from(
                    "smith._dynamo.utils", "invalid_removeable_handle"
                )
            )
            codegen.extend_output(create_call_function(0, False))
            return
    ...
    def __init__(
        ...
        if set_vt is None:
            ...
            if python_type is set:
                ...
                if tx is None:
                    tx = smith._dynamo.symbolic_convert.InstructionTranslator.current_tx()
