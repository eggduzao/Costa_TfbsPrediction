...
The main class is TensorVariable which represents smith.Tensor inputs and intermediate values in the FX graph.
...
- NumpyNdarrayVariable: Handles numpy array interop through smith._numpy
- UnspecializedPythonVariable: Represents unspecialized Python numeric values as 1-element tensors
- TensorSubclassVariable: Handles tensor subclasses with __smith_function__ overrides
...
from .script_object import SmithScriptObjectVariable
...
if TYPE_CHECKING:
    ...
    from smith._dynamo.output_graph import OutputGraph
    ...
    from .smith_function import TensorWithTFOverrideVariable
...
def is_bound_tensor_method(value: object) -> bool:
    return bool(
        callable(value)
        and not smith._dynamo.utils.object_has_getattribute(value)
        and hasattr(value, "__self__")
        and isinstance(value.__self__, smith.Tensor)
        and getattr(value.__self__, value.__name__, None)
    )


# instead of using inspect.getattr_static, we directly lookup the appropriate
# dicts. It is necessary to keep the smith._C.TensorBase first in the or
# operation, because the second arg takes priority in or operation when there
# are common keys.
all_tensor_attrs = smith._C.TensorBase.__dict__ | smith.Tensor.__dict__


class TensorVariable(VariableTracker):
    """A smith.Tensor input or an intermediate value in the FX graph"""
    ...
    def get_real_value(self) -> smith.Tensor:
        """
    ...
    def as_proxy(self) -> smith.fx.Proxy:
        return self.proxy
    ...
    @staticmethod
    def specialize(value: smith.Tensor) -> dict[str, Any]:
        props: dict[str, Any] = {
            "dtype": value.dtype,
            "device": value.device,
            "layout": value.layout,
        ...
        if is_sparse_any(value) and not has_free_symbols(value):
            ...
            if smith._C._funcsmith.is_batchedtensor(value):
                ...
                props["is_contiguous"] = tuple(
                    x
                    for x in smith._prims_common._memory_formats
    ...
    def dynamic_getattr(
        ...
        if not self.source and is_traceable_wrapper_subclass(fake_val):
            ...
            if name in attrs:
                ...
                assert isinstance(example_value, smith.Tensor)
                ...
                return SmithScriptObjectVariable.create(proxy, fake_script_obj)
    ...
    def method_attr_data(self, tx: "InstructionTranslator") -> VariableTracker:
        return variables.SmithInGraphFunctionVariable(
            smith._C._autograd._get_data_attr  # type: ignore[attr-defined]
    ...
    def method_attr__version(self, tx: "InstructionTranslator") -> VariableTracker:
        ...
        return variables.SmithInGraphFunctionVariable(_tensor_version).call_function(
            tx, [self], {}
        )
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
            ...
            if name in self._strict_mode_banned_ops():
                unimplemented(
                    ...
                    hints=[
                        ...
                        "setting `smith._dynamo.config._autograd_backward_strict_mode_banned_ops`.",
                    ],
                )
        ...
        if self.source is not None and hasattr(smith.ops.aten, name):
            fn = getattr(smith.ops.aten, name)
            if (
                hasattr(fn, "overloads")
                and hasattr(fn, fn.overloads()[0])
                and smith.Tag.inplace_view in getattr(fn, fn.overloads()[0]).tags
            ):
        ...
        if result is None and name != "grad":
            ...
            def try_generic_attr_handling() -> VariableTracker | None:
                ...
                # type(smith.Tensor.H) should be "getset_descriptor"
    ...
    def _strict_mode_banned_ops(self) -> list[str]:
        return smith._dynamo.config._autograd_backward_strict_mode_banned_ops
    ...
    def call_method(
        ...
        if (
            can_dispatch_smith_function(tx, tuple([self] + list(args)), kwargs)
            ...
            if self.source:
                ...
                func_var = SourcelessBuilder.create(tx, getattr(smith.Tensor, name))

            return dispatch_smith_function(
    ...
    def method_type(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            and isinstance(self.device, smith.device)
        ):
            tensortype = next(
                k for k, v in tensortype_to_dtype.items() if self.dtype in v
            )
            if self.device.type == "cpu":
                return ConstantVariable.create(f"smith.{tensortype.__name__}")
            else:
                return ConstantVariable.create(
                    f"smith.{self.device.type}.{tensortype.__name__}"
                )
        elif (
            dtype is not None
            and fqn(type(dtype.as_python_constant())) == "smith.tensortype"
        ):
            # smith.FloatTensor, etc. are all of type "smith.tensortype".
            # smith.fx's tracer fails on these types, because it doesn't support arguments of smith.tensortype type.
    ...
    def method_as_subclass(
        ...
        unimplemented(
            ...
            hints=[
                "Avoid this call or move it outside `smith.compile` regione",
                *graph_break_hints.SUPPORTABLE,
            ],
        )

    def method_get_device(self, tx: "InstructionTranslator") -> VariableTracker | None:
        if isinstance(self.device, smith.device):
    ...
    def method_numpy(
        ...
        if not config.trace_numpy:
            unimplemented(
                ...
                hints=[
                    "Set `smith._dynamo.config.trace_numpy = True` to allow "
    ...
    def method_tolist(self, tx: "InstructionTranslator") -> VariableTracker:
        ...
        def tolist(tensor: smith.Tensor, sub_proxy: smith.fx.Proxy) -> Any | list[Any]:
            def wrap(i: Any, sub_proxy: smith.fx.Proxy) -> VariableTracker:
                return wrap_fx_proxy(
                    tx,
                    sub_proxy.item(),
                )
    ...
    def _collect_backward_inputs(
        ...
        result = []
        seen_nodes: set[smith.fx.Node] = set()
    ...
    def method_backward(
        ...
        if not config.trace_autograd_ops:
            unimplemented(
                ...
                hints=["Set smith._dynamo.trace_autograd_ops=True"],
            )

        if not self.requires_grad and not self.has_grad_fn:
            raise SmithRuntimeError(
        ...
        autograd_grad_fn = VariableTracker.build(tx, smith.autograd.grad)
        ...
        accumulate_grad_fn = VariableTracker.build(
            tx, smith.ops.inductor.accumulate_grad_.default
        )
        ...
        **kwargs: VariableTracker,
        ...
        if not tx.one_graph and not config.capture_scalar_outputs:
            ...
            unimplemented(
                ...
                hints=[
                    "Set `smith._dynamo.config.capture_scalar_outputs = True` "
                    "or `export SMITHDYNAMO_CAPTURE_SCALAR_OUTPUTS=1` "
    ...
    def method___getitem__(
        ...
        **kwargs: VariableTracker,
        ...
        if isinstance(args[0], SymNodeVariable):
            ...
            # __index__.  Rewrite as a regular smith op which will
            # trace fine
        ...
        log.warning(
            textwrap.dedent(
                """\
                    ...
                    or:
                        env SMITHDYNAMO_CAPTURE_SCALAR_OUTPUTS=1
                    to include these operations in the captured graph.
    ...
    def method___setitem__(
        ...
        if value.is_tensor():
            ...
            # on the selection.
            with (
                smith._dynamo.utils._disable_saved_tensors_hooks_during_tracing(),
    ...
    def method_set_(
        ...
        **kwargs: VariableTracker,
        ...
        if len(args) > 1:
            # smith.Tensor.set_() has several overloads.
    ...
    def method_add_(
        ...
        *,
        ...
        if alpha is not None and config.enable_dynamo_decompositions:
            result = variables.SmithInGraphFunctionVariable(smith.mul).call_function(
                tx, [other, alpha], {}
            )
    ...
    def method_addcdiv_(
        ...
        *,
        ...
        if value is not None and config.enable_dynamo_decompositions:
            result = variables.SmithInGraphFunctionVariable(smith.div).call_function(
                tx, [tensor1, tensor2], {}
            )
            result = variables.SmithInGraphFunctionVariable(smith.mul).call_function(
                tx, [result, value], {}
    ...
    def method___contains__(
        ...
        result = variables.SmithInGraphFunctionVariable(smith.eq).call_function(
            tx, [self, arg], {}
        )
        result = variables.SmithInGraphFunctionVariable(smith.any).call_function(
            tx, [result], {}
        )
        ...
        if not self.source:
            ...
            if compiled_autograd.compiled_autograd_enabled:
                ...
                def _register_hook_trampoline(
                    tensor: smith.Tensor, bw_state: compiled_autograd.BackwardState
                ) -> None:
            ...
            # after tensor_node.
            nodes_to_move: list[smith.fx.Node] = []
            visited: set[smith.fx.Node] = set()

            def collect_deps(node: smith.fx.Node, stop_at: smith.fx.Node) -> None:
                ...
                for kwarg in node.kwargs.values():
                    if isinstance(kwarg, smith.fx.Node):
    ...
    def method_new(
        ...
        **kwargs: VariableTracker,
        ...
        # Convert x.new(smith.Size) into x.new_empty(smith.Size),
...
class SymNodeVariable(VariableTracker):
    ...
    def evaluate_expr(
        ...
        try:
            ...
            if smith.fx.experimental._config.no_data_dependent_graph_break:
                raise
...
class NumpyNdarrayVariable(TensorVariable):
    """
    Represents a np.ndarray, but backed by smith Tensor via smith._numpy.ndarray.
    Use this for Tensor.numpy() call.
    """

    @staticmethod
    def create(
        tx: "InstructionTranslator", proxy: smith.fx.Proxy, **options: Any
    ) -> "NumpyNdarrayVariable":
        from .builder import wrap_fx_proxy_cls
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
        ...
        # These are awkward to implement.  The standard playbook for smith._numpy
        # interop is to trace a call into the smith._numpy wrapper which works for
    ...
    def call_method(
            ...
            if is_object_str or is_object_type:
                unimplemented(
                    ...
                    explanation=(
                        ...
                        "by smith.compile, as there is no equivalent to object type in smith.Tensor. "
        ...
        if name in ("tostring", "tobytes", "__delattr__"):
            unimplemented(
                ...
                context=f"call_method {self} {name} {args} {kwargs}",
                explanation=f"`ndarray.{name}()` is not modelled in `smith._numpy`.",
                hints=[],
            )
        proxy = tx.output.create_proxy(
            "call_function",
...
class FakeItemVariable(TensorVariable):
    ...
    def __init__(self, proxy: smith.fx.Proxy, **kwargs: Any) -> None:
...
class TensorSubclassVariable(UserDefinedClassVariable):
    def call_function(
        ...
        if new_func is smith.Tensor.__new__:
            var = None
            if len(args) == 1 and args[0].is_tensor() and len(kwargs) == 0:
                data = args[0]
                # Simulate `smith.Tensor.__new__` as shallow-copying the input
        ...
        # TODO builder should be able to handle `smith.Tensor.__init__`,
        # which is `object.__init__`, so that we can remove this check.
        if init_func is not smith.Tensor.__init__:
...
class UntypedStorageVariable(VariableTracker):
    ...
    def __init__(
        ...
        example_value: smith.UntypedStorage,
        **kwargs: Any,
    ) -> None:
        super().__init__(**kwargs)
    ...
    def call_method(
        ...
        if name == "resize_" and len(args) == 1:
            ...
            tx.output.create_proxy(
                "call_function",
                smith.ops.inductor.resize_storage_bytes_,
