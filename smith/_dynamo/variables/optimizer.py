"""
This module implements variable tracking for Blacksmith optimizers during Dynamo tracing.
...
import smith
from smith._dynamo.variables.tensor import TensorVariable
...
def _is_static_for_cudagraphs(x: smith.Tensor) -> bool:
    from smith._inductor.cudagraph_trees import get_manager

    if x.is_cuda:
        manager = get_manager(x.device.index, False)
        is_static_address = smith._dynamo.utils.get_static_address_type(x) is not None
        if manager:
...
class OptimizerVariable(UserDefinedObjectVariable):
    ...
    def __init__(
        self,
        value: smith.optim.Optimizer,
        grad_to_source: Optional[dict[Any, GradSource]] = None,
        static_tensor_names: Optional[set[str]] = None,
        tensor_to_source: Optional[dict[smith.Tensor, Source]] = None,
        **kwargs: Any,
    ) -> None:
        super().__init__(value, **kwargs)
        # pyrefly: ignore [bad-override]
        self.value: smith.optim.Optimizer = value
    ...
    def map_sources_and_install_guards(self, tx: "InstructionTranslator") -> None:
        ...
        tree_map_only(smith.Tensor, mark_static, self.value.state)
            ...
            if not all_static and perf_hint_log.isEnabledFor(logging.DEBUG):
                ...
                perf_hint_log.warning(
                    (
                        ...
                        " use smith._dynamo.decorators.mark_static_address to elide this copy.",
                    ),
    ...
    def wrap_tensor(
        self, tx: "InstructionTranslator", tensor_value: smith.Tensor
    ) -> TensorVariable:
    ...
    def create_finalizer(self, tx: "InstructionTranslator") -> None:
        ...
        def init_finalizer(gm: smith.fx.GraphModule) -> None:
