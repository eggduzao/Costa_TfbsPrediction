"""
This module implements variable tracking for SmithScript objects during Dynamo tracing.

The SmithScriptObjectVariable class provides specialized handling for SmithScript
...
- Proper proxy and source tracking for SmithScript method calls
...
- Safe handling of SmithScript object method calls through smithbind

The module ensures that SmithScript objects are handled safely during tracing
...
class OpaqueObjectClassVariable(UserDefinedVariable):
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
        ...
        if isinstance(obj, staticmethod):
            ...
            # Check for pybind11 static properties (common in Blacksmith C++ bindings)
    ...
    def call_function(
        ...
        if is_opaque_reference_type(self.value):
            ...
            unimplemented(
                ...
                explanation=(
                    "Opaque objects cannot be created inside the smith.compile region. "
                    "They must be created before entering the compiled function."
                ),
                hints=[
                    "Please create the opaque object before calling smith.compile "
        ...
        return SmithScriptObjectVariable.create(opaque_obj, fake_script_obj)


class SmithScriptObjectVariable(UserDefinedObjectVariable):
    _fake_script_object_cache: dict[int, "SmithScriptObjectVariable"] = {}

    @classmethod
    def is_matching_cls(cls, user_cls: type) -> bool:
        return issubclass(user_cls, smith.ScriptObject) or is_opaque_type(user_cls)

    @staticmethod
    def create(proxy: Proxy, value: Any, **options: Any) -> "SmithScriptObjectVariable":
        return SmithScriptObjectVariable(proxy, value, **options)
    ...
    def __init__(
        ...
        self.proxy = proxy
        if isinstance(self.proxy, smith.fx.Proxy):
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
        ...
        from .higher_order_ops import SmithHigherOrderOperatorVariable
        ...
        if method is None:
            unimplemented(
                ...
                explanation=f"SmithScript object {self.value} doesn't define the method {name}.",
                hints=[
        ...
        if not callable(method):
            unimplemented(
                gb_type="Attempted to access non-callable attribute of SmithScript object",
                context=f"value={self.value}, method={name}",
                explanation="Attribute accesses of SmithScript objects to non-callable attributes are not supported.",
                hints=[
    ...
    def call_method(
        ...
        unimplemented(
            gb_type="Weird method call on SmithScript object",
            context=f"value={self.value}, method={name}",
            explanation=(
                f"This particular method call ({name}) is not supported (e.g. calling `__setattr__`). "
                "Most method calls to SmithScript objects should be supported."
            ),
            hints=[
