...
conditions (smith.cond), loops (smith.while_loop), maps (smith.ops.higher_order.map),
...
from smith._dynamo.utils import get_fake_value
from smith._dynamo.variables.constant import ConstantVariable
from smith._dynamo.variables.ctx_manager import RepararametrizeModuleContextVariable
from smith._dynamo.variables.functions import UserFunctionVariable
from smith._dynamo.variables.nn_module import UnspecializedNNModuleVariable
from smith._dynamo.variables.tensor import SymNodeVariable, TensorVariable
from smith._guards import Source
from smith._higher_order_ops.invoke_subgraph import NestedCompileRegionOptions
from smith._ops import HigherOrderOperator
from smith.fx.graph_module import GraphModule
from smith.fx.passes.shape_prop import _extract_tensor_metadata
from smith.fx.proxy import Proxy
...
if TYPE_CHECKING:
    from smith._dynamo.output_graph import SubgraphTracer
    from smith._dynamo.symbolic_convert import InstructionTranslator
    from smith._subclasses.fake_tensor import FakeTensor, FakeTensorMode
...
P = ParamSpec("P")
R = TypeVar("R")
HOP_VT_Alias = TypeVar("HOP_VT_Alias", bound="SmithHigherOrderOperatorVariable")

log = logging.getLogger(__name__)
hc_log = smith._logging.getArtifactLogger(__name__, "hierarchical_compile")
...
def check_meta_consistency_vt(
    ...
    from smith._higher_order_ops.utils import check_meta_consistency
...
def dynamo_enable_grad(
    ...
    org_value = smith.is_grad_enabled()
    try:
...
def _make_inlined(
    ...
    def inline_call(
        *args: VariableTracker, **kwargs: VariableTracker
        ...
        from smith._dynamo.trace_rules import _force_inline
...
def add_call_function(
    ...
    if config is not None:
        ...
        assert proxy.node.target == smith._higher_order_ops.invoke_subgraph
...
def _call_function_and_unflatten_output(
    ...
    if ret_spec.masks_to_filter_const_values:
        from smith._dynamo.external_utils import insert_const_values_with_mask
...
def _assert_tensors_nonaliasing(inputs: Any, outputs: Any) -> None:
    input_tensor_ids = {
        id(t) for t in pytree.tree_leaves(inputs) if isinstance(t, smith.Tensor)
    }
    output_tensor_ids = {
        id(t) for t in pytree.tree_leaves(outputs) if isinstance(t, smith.Tensor)
    }
...
def get_tensor_storages(tensor: smith.Tensor) -> set[StorageWeakRef]:
    """
    ...
    if not isinstance(tensor, smith.Tensor):
        return storages
...
class StorageAliasingTracker:
    ...
    def _collect_storages_from_tensor(self, example_value: smith.Tensor) -> None:
    ...
    def collect_from_inputs(self, tx: "InstructionTranslator") -> None:
        ...
        from smith._higher_order_ops.utils import _collect_fake_inputs

        for node in tx.output.graph.nodes:
            if node.op == "placeholder":
                example_value = _collect_fake_inputs([node])[0]
                if isinstance(example_value, smith.Tensor):
    ...
    def check_and_track(self, proxy_node: Proxy) -> bool:
        ...
        if not isinstance(example_value, smith.Tensor):
            return True
...
def _check_all_tensorvariable(args: Sequence[VariableTracker]) -> None:
    if not all(type(a.realize()) is TensorVariable for a in args):
        unimplemented(
            gb_type="HOP: non smith.Tensor leaf",
            context=f"args types: {[type(a.realize()) for a in args]}",
            explanation="Expected all leaves to be of smith.Tensor type.",
            hints=[],
        )
...
def _call_while_loop(
    ...
    from smith._higher_order_ops.while_loop import _create_unbacked_symint
    ...
    if kwargs or len(args) != 4:
        unimplemented(
            gb_type="smith.while_loop: improper args/kwargs",
            context=f"args: {args}, kwargs: {kwargs}",
            explanation=f"smith.while_loop expects 4 positional arguments (got {len(args)}) "
    ...
    if not isinstance(additional_inputs, (ListVariable, TupleVariable)):
        unimplemented(
            gb_type="smith.while_loop: improper additional_inputs",
    ...
    (
        ...
        #   return new_a, new_b
        #
        # a, b = smith.randn(3), smith.randn(3)
    ...
    if cond_r.is_tensor():
        ...
        if cond_r_meta.dtype != smith.bool or cond_r_meta.shape != smith.Size([]):
            unimplemented(
                gb_type="smith.while_loop: unsupported cond_fn return type",
                context=str(cond_r),
        ...
        if pred:
            unimplemented(
                gb_type="smith.while_loop: infinite loop detected",
                context=str(cond_r),
    ...
    # smith.contiguous_format is not yet implemented". This is okay because stride
    # is still checked.
    ...
    cond_gm = smith.fx.GraphModule(cond_nn_modules, cond_graph)
    body_gm = smith.fx.GraphModule(body_nn_modules, body_graph)
...
def are_same_graph_modules(
    ...
    from smith._subclasses._fake_tensor_utils import _CacheKeyState
    ...
    def check_all_args(a_nodes: Iterable[Any], b_nodes: Iterable[Any]) -> bool:
        for arg_a, arg_b in zip(a_nodes, b_nodes):
            if isinstance(arg_a, smith.fx.Node):
    ...
    for a_node, b_node in zip(a_mod.graph.nodes, b_mod.graph.nodes):
        ...
        if a_node.op == "placeholder":
            ...
            if isinstance(a_value, smith.Tensor):
                if not isinstance(b_value, smith.Tensor):
                    return False
                ...
                if a_result != b_result:
                    return False
            elif isinstance(a_value, smith.SymInt):
                if not isinstance(b_value, smith.SymInt):
                    return False
            ...
            if isinstance(a_attr, smith.fx.GraphModule):
                if not isinstance(b_attr, smith.fx.GraphModule):
                    return False
...
# in smith.cond, two branches might lift different set of tensors as inputs. This function helps to
# dedup the inputs and modify the graphs to take the same set of inputs.
def _merge_graph_inputs(
    l_graph: smith.fx.Graph,
    l_lifted_freevars: dict[Proxy, Proxy],
    l_name: str,
    r_graph: smith.fx.Graph,
    r_lifted_freevars: dict[Proxy, Proxy],
    r_name: str,
) -> tuple[
    smith.fx.Graph, smith.fx.Graph, list[Proxy], list[Proxy], list[Proxy], list[Proxy]
]:
...
def move_lifted_freevars_phs_to_end(
    graph: smith.fx.Graph, lifted_freevars: dict[Proxy, Proxy]
) -> None:
...
def speculate_subgraph_with_auto_output_flattening(
    ...
    *,
    ...
    smith.fx.Graph,  # graph: The FX graph representing the subgraph computation
    dict[
        smith.fx.Proxy, smith.fx.Proxy
    ...
    For example:
    - invoke_subgraph
    - activation checkpointing (smith.utils.checkpoint.checkpoint)
    - autograd.Function
    - nested_compile_region

    This is in contrast to control flow HOPs which do not follow this desugaring:
    - smith.cond (conditional execution based on predicate)
    - smith.while_loop (iterative execution)
    - smith.map (parallel execution over batch dimension)
    ...
    Example:
        ...
        def gn(x):
            return Foo(smith.sin(x), smith.cos(x))
    ...
    try:
        ...
        with tx.output.subtracer(source_target, tracer, description) as subtracer:
            ...
            #     out = x.sin()
            #     z.append(out)
            #     return smith.cos(smith.sin(o))

            # def fn(x):
            #     z = []
            #     out1 = smith.utils.checkpoint.checkpoint(
            #         gn,
            #         x,
            #         z,
            #         use_reentrant=False,
        ...
        msg = (
            ...
            f"fall back to eager-mode Blacksmith, which could lead to a slowdown."
        )
        log.info(msg)
...
def speculate_subgraph(
    ...
    *,
    ...
    # across fwd-bwd for autograd.Function
    tracer: Optional["SubgraphTracer"] = None,
) -> tuple[tuple[VariableTracker, OutputSpec], smith.fx.Graph, dict[Proxy, Proxy]]:
    if sub_kwargs is None:
        sub_kwargs = {}
    ...
    try:
        ...
        with tx.output.subtracer(source_target, tracer, description) as subtracer:
            ...
            if should_flatten_outputs:
                from smith._dynamo.external_utils import filter_out_const_values
                ...
                if remove_consts_from_outputs:
                    ...
                    masks_to_filter_const_values = pytree.tree_map(
                        lambda x: not isinstance(x, smith.fx.Proxy), output_proxies
                    )
                    const_values = pytree.tree_map(
                        lambda x: None if isinstance(x, smith.fx.Proxy) else x,
                        output_proxies,
                    )
        ...
        try:
            ...
            if self._ALLOW_FALLBACK_TO_EAGER:
                ...
                full_msg = (
                    ...
                    "captured completely with smith.compile. Got graph break/error:"
                    f"\n\n{str(e)}"
                )
...
class SmithHigherOrderOperatorVariable(VariableTracker):
    ...
    def make(
    ...
    ) -> "SmithHigherOrderOperatorVariable":
        ...
        from smith._higher_order_ops import BaseHOP
    ...
    def call_function(
        ...
        from .smith_function import can_dispatch_smith_function, dispatch_smith_function

        if can_dispatch_smith_function(tx, args, kwargs):
...
class CustomFunctionHigherOrderOperatorVariable(SmithHigherOrderOperatorVariable):
    """
    Wraps smith._funcsmith.autograd_function.custom_function_call
    """

    _HOP_NAME = "smith.ops.higher_order.custom_function_call"
    ...
    def _call_function(
        ...
        return smith._dynamo.variables.UserMethodVariable(
            self.value.__call__.__func__,
            smith._dynamo.variables.UserDefinedObjectVariable(
...
class CondHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.cond"
    ...
    def _call_function(
        ...
        # ops - see smith/dispatch/_dispatcher.py
        if len(args) != 4 or kwargs:
            unimplemented(
                gb_type="smith.cond: improper args/kwargs",
                context=f"args: {args}, kwargs: {kwargs}",
                explanation=f"smith.cond expects 4 positional arguments (got {len(args)}) "
                f"and no keyword arguments (got {len(kwargs)}) "
        ...
        if type(args[0]) is ConstantVariable:
            warnings.warn(
                "Pred is a Python constant. When used with smith.cond, it specializes on one of the branches."
                " If you want smith.cond to preserve two branches, please make the predicate a boolean tensor or a SymBool.",
                UserWarning,
            )
        ...
        if type(pred.realize()) not in (
            ...
            unimplemented(
                gb_type="smith.cond: improper predicate",
                context=str(pred),
        ...
        if not isinstance(operands, (ListVariable, TupleVariable)):
            unimplemented(
                gb_type="smith.cond: improper operands",
        ...
        if not only_consist_of(
            ...
            unimplemented(
                gb_type="smith.cond: improper operands contents",
                context=str(operands),
        ...
        def speculate_branch(
            branch: bool,
        ) -> tuple[VariableTracker, OutputSpec, smith.fx.Graph, dict[Proxy, Proxy]]:
            # NB: 0 is predicate
            ...
            if not only_consist_of(ret_val, (TensorVariable, ConstantVariable)):
                unimplemented(
                    gb_type="smith.cond: unsupported branch return type",
                    context=str(ret_val),
            ...
            for ret in ret_val.unpack_var_sequence(tx):
                if ret.is_python_constant() and not isinstance(
                    ...
                    unimplemented(
                        gb_type="smith.cond: unsupported branch return type (constant non-int)",
                        context=str(ret_val),
        ...
        if same_spec is not NotImplemented and not same_spec:
            unimplemented(
                gb_type="smith.cond: differing branch outputs",
        ...
        true_name = tx.output.install_subgraph(
            "cond_true",
            smith.fx.GraphModule(true_nn_modules, true_graph),
        )
        false_name = tx.output.install_subgraph(
            "cond_false",
            smith.fx.GraphModule(false_nn_modules, false_graph),
        )
...
class CallSmithbindHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.call_smithbind"
...
class WhileLoopHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.while_loop"
    _ALLOW_FALLBACK_TO_EAGER = False
    supports_input_mutation = False
    supports_aliasing = False
...
class WhileLoopStackOutputHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.while_loop"
    _ALLOW_FALLBACK_TO_EAGER = False
    supports_input_mutation = False
    supports_aliasing = False
...
class AssociativeScanHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.associative_scan"
    _ALLOW_FALLBACK_TO_EAGER = False
    supports_input_mutation = False
    supports_aliasing = False
    ...
    def _call_function(
        ...
        from smith._higher_order_ops.utils import first_slice_copy
        ...
        if args[0].python_type() is functools.partial:
            ...
            if len(args) != 2:
                unimplemented(
                    gb_type="smith.associative_scan: improper args",
                    context=f"args: {args}",
                    explanation=f"smith.associative_scan expects 2 positional arguments (got {len(args)}) "
        ...
        if not isinstance(xs, (ListVariable, TupleVariable)):
            unimplemented(
                gb_type="smith.associative_scan: improper xs",
                context=str(xs),
        ...
        if not isinstance(additional_inputs, (ListVariable, TupleVariable)):
            unimplemented(
                gb_type="smith.associative_scan: improper additional_inputs",
                context=str(additional_inputs),
                explanation=f"Expected additional_inputs to be a list/tuple but got {additional_inputs.python_type()}",
                hints=[
                    *graph_break_hints.DYNAMO_BUG,
        ...
        if scan_length == 0:
            unimplemented(
                gb_type="smith.associative_scan: zero-sized tensor",
        ...
        if _combine_treespec.as_python_constant().num_leaves < 1:
            unimplemented(
                gb_type="smith.associative_scan: combine_fn improper number of leaves",
        ...
        if (
            ...
            unimplemented(
                gb_type="smith.associative_scan: mismatched input/output tree structure",
        ...
        combine_gm = smith.fx.GraphModule(dict(tx.output.nn_modules), combine_graph)
        ...
        from smith._higher_order_ops.utils import _maybe_fake_tracing
        from smith._inductor.utils import is_pointwise_use
        ...
        return _call_function_and_unflatten_output(
            tx,
            smith.ops.higher_order.associative_scan,
            p_args,
            {},
            None,
...
class ScanHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.scan"
    _ALLOW_FALLBACK_TO_EAGER = False
    supports_input_mutation = False
    supports_aliasing = False
    ...
    def _call_function(
        ...
        from smith._higher_order_ops.scan import _extract_carry_and_out
        from smith._higher_order_ops.utils import first_slice_copy
        ...
        def _check_combine_fn_is_normalized(combine_fn_var: VariableTracker) -> bool:
            if not isinstance(
                ...
                unimplemented(
                    gb_type="smith.scan: improper combine_fn",
        ...
        if combine_fn_is_normalized:
            ...
            assert isinstance(combine_gm, smith.fx.GraphModule), (
                combine_fn,
                combine_gm,
            )
        else:
        ...
        if not isinstance(xs, (ListVariable, TupleVariable)):
            unimplemented(
                gb_type="smith.scan: improper xs",
                context=str(xs),
                explanation=f"Expected xs to be a list/tuple but got {xs.python_type()}",
                hints=[
                    *graph_break_hints.DYNAMO_BUG,
        ...
        if not isinstance(init, (ListVariable, TupleVariable)):
            unimplemented(
                gb_type="smith.scan: improper init",
                context=str(init),
        ...
        if len(init_vars) == 0:
            unimplemented(
                gb_type="smith.scan: no init leaves",
                context="",
        ...
        if not isinstance(additional_inputs, (ListVariable, TupleVariable)):
            unimplemented(
                gb_type="smith.scan: improper additional_inputs",
                context=str(additional_inputs),
                explanation=f"Expected additional_inputs to be a list/tuple but got {additional_inputs.python_type()}",
                hints=[
                    *graph_break_hints.DYNAMO_BUG,
        ...
        if scan_length == 0:
            unimplemented(
                gb_type="smith.scan: zero-sized tensor",
                context=str(xs_vars[0]),
                explanation="associative_scan() operator doesn't support zero-sized tensors during tracing.",
                hints=[
                    *graph_break_hints.USER_ERROR,
        ...
        if combine_fn_is_normalized:
            ...
            if len(combine_result_vars) != 2:
                unimplemented(
                    gb_type="smith.scan: improper combine_fn number of returns",
...
class MapHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.map_impl"
    _ALLOW_FALLBACK_TO_EAGER = False
    supports_input_mutation = False
    supports_aliasing = False
    ...
    def _call_function(
        ...
        if len(kwargs) > 0:
            unimplemented(
                gb_type="smith.map: kwargs not supported",
                context=f"args: {args}, kwargs: {kwargs}",
                explanation=f"smith.map expects no keyword arguments (got {len(kwargs)})",
                hints=[
                    *graph_break_hints.USER_ERROR,
                ],
            )
        ...
        if len(sample_shape) < 1 or sample_shape[0] == 0:
            unimplemented(
                gb_type="smith.map: improper inputs",
                context=str(sample_shape),
                explanation="smith.map doesn't support scalar or non-zero sized tensors during tracing.",
                hints=[
                    *graph_break_hints.USER_ERROR,
                ],
            )
        ...
        body_name = tx.output.install_subgraph(
            "map_body",
            smith.fx.GraphModule(body_nn_modules, body_graph),
        )
        ...
        return _call_function_and_unflatten_output(
            tx, smith.ops.higher_order.map_impl, p_args, {}, None, body_spec, body_r
        )


class PrintHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.print"
...
class ExecusmithCallDelegateHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.execusmith_call_delegate"
    ...
    def _call_function(
        ...
        # This is operator for delegation within Execusmith which calls a
        # specific function in the given lowered module with the given
        # operators. The actual operator is defined in the Execusmith codebase.
        # This is a bad hierarchical violation since
        # execusmith_call_delegate sits at a higher level than dynamo, but
        # there's no real solution to this issue yet.
        if len(kwargs) > 0:
            unimplemented(
                gb_type="execusmith_call_delegate: kwargs not supported",
                context=f"args: {args}, kwargs: {kwargs}",
                explanation=f"execusmith_call_delegate expects no keyword arguments (got {len(kwargs)})",
                hints=[],
            )
        ...
        if isinstance(args[0], variables.NNModuleVariable):
            ...
            # This nn module is special sa delegated by execusmith. Just
            ...
            unimplemented(
                gb_type="execusmith_call_delegate: first arg not supported",
                context=f"args: {args}, kwargs: {kwargs}",
                explanation=f"execusmith_call_delegate expects the first argument to be a nn.Module (got {args[0]})",
                hints=[],
            )

        p_args = tuple(arg.as_proxy() for arg in args[1:])
        real_sub_args = pytree.tree_map_only(
            smith.fx.Proxy, lambda a: get_fake_value(a.node, tx), p_args
        )
        assert tx.fake_mode is not None
        with tx.fake_mode:
        ...
        # execusmith modules promise not to alias inputs and outputs.
...
class FuncsmithHigherOrderVariable(UserFunctionVariable):
    def call_function(
        self,
        tx: "InstructionTranslator",
        args: Sequence[VariableTracker],
...
class FunctionalCallVariable(FuncsmithHigherOrderVariable):
    def call_function(
        self,
        tx: "InstructionTranslator",
        args: Sequence[VariableTracker],
        ...
        if not smith._dynamo.config.inline_inbuilt_nn_modules:
            unimplemented(
                gb_type="smith.func.functional_call capture is disabled",
                context="",
                explanation="smith.func.functional_call capture is disabled",
                hints=[
                    "Set `smith._dynamo.config.inline_inbuilt_nn_modules=True` to enable.",
                ],
            )
        return super().call_function(tx, args, kwargs)


class ReparametrizeModuleCallVariable(FuncsmithHigherOrderVariable):
...
class WrapHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.wrap"
    ...
    def create_wrapped_node(
        ...
        *,
        ...
        body_gmod = smith.fx.GraphModule(tx.output.nn_modules, body_graph)
...
class WrapWithSetGradEnabledHigherOrderVariable(SmithHigherOrderOperatorVariable):
    """
    This hop is not exposed to users but is inserted into the graph
    after export as a post-processing step.
    """

    _HOP_NAME = "smith.ops.higher_order.wrap_with_set_grad_enabled"
    ...
    def call_function(
        ...
        with smith.set_grad_enabled(grad_enabled.as_python_constant()):
            assert self._HOP_NAME is not None
            (
                (body_r, treespec),
                body_graph,
...
class WrapWithAutocastHigherOrderVariable(SmithHigherOrderOperatorVariable):
    """
    This hop is not exposed to users but is inserted into the graph
    after export as a post-processing step.
    """

    _HOP_NAME = "smith.ops.higher_order.wrap_with_autocast"
    ...
    def call_function(
        ...
        with smith.autocast(*python_constants):
            assert self._HOP_NAME is not None
            (
                (body_r, treespec),
                body_graph,
...
class HintsWrapperHigherOrderVariable(WrapHigherOrderVariable):
    _HOP_NAME = "smith.ops.higher_order.hints_wrapper"
    _ALLOW_FALLBACK_TO_EAGER = False
...
class OutDtypeHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.out_dtype"
    ...
    def _call_function(
        ...
        fake_sub_args = pytree.tree_map_only(
            smith.fx.Proxy, lambda a: a.node.meta["example_value"], p_args[2:]
        )
...
class StrictModeHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.strict_mode"
    _ALLOW_FALLBACK_TO_EAGER = False
    ...
    def _call_function(
        ...
        strict_mode_name = tx.output.install_subgraph(
            "strict_mode_body",
            smith.fx.GraphModule(strict_mode_nn_modules, ret_graph),
        )
        ...
        return _call_function_and_unflatten_output(
            tx,
            smith.ops.higher_order.strict_mode,
            p_args,
            {},
            flat_example_value,
            ret_spec,
...
class CheckpointHigherOrderVariable(WrapHigherOrderVariable):
    _HOP_NAME = "smith.utils.checkpoint.checkpoint"

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
        self.allow_side_effects = (
            smith._dynamo.config.skip_fwd_side_effects_in_bwd_under_checkpoint
        )
    ...
    def _call_function(
        ...
        from smith._higher_order_ops.wrap import TagActivationCheckpoint
        from smith.utils.checkpoint import noop_context_fn

        context_fn = None
        if "context_fn" in kwargs and kwargs["context_fn"] is not noop_context_fn:
            ctx = kwargs.pop("context_fn")
            if isinstance(ctx, smith._dynamo.variables.UserFunctionVariable):
                context_fn = ctx.fn
            elif isinstance(
                ctx, smith._dynamo.variables.functions.FunctoolsPartialVariable
            ):
...
class DynamoBypassingWrapperHigherOrderVariable(WrapHigherOrderVariable):
    _HOP_NAME = "smith.ops.higher_order.dynamo_bypassing_wrapper"
    ...
    def _call_function(
        ...
        if isinstance(func_var, smith._dynamo.variables.UserFunctionVariable):
            func = func_var.fn
        elif isinstance(
            func_var, smith._dynamo.variables.functions.FunctoolsPartialVariable
        ):
...
class ExportTracepointHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order._export_tracepoint"
...
class RunWithRNGStateHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.run_with_rng_state"
...
class AutoFunctionalizeHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.auto_functionalized"
...
class FlexAttentionBackwardHighOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.flex_attention_backward"
...
class TraceWrappedHigherOrderOperatorVariable(SmithHigherOrderOperatorVariable):
    """
    Handles smith._dynamo._trace_wrapped_higher_order_op.inner_trace
    ...
    _HOP_NAME = "smith._dynamo._trace_wrapped_higher_order_op.inner_trace"
...
class FlexAttentionHigherOrderVariable(SmithHigherOrderOperatorVariable):
    _HOP_NAME = "smith.ops.higher_order.flex_attention"
    ...
    def create_wrapped_node(
        ...
        def create_scalar() -> VariableTracker:
            return query.call_method(
                ...
                {
                    "dtype": VariableTracker.build(tx, smith.int32),
                },
            )
        ...
        body_name = tx.output.install_subgraph(
            fn_name,
            smith.fx.GraphModule(tx.output.nn_modules, body_graph),
        )
    ...
    def _call_function(
        ...
        if mask_fn.is_python_constant() and mask_fn.as_python_constant() is None:
            mask_fn = VariableTracker.build(
                tx,
                smith.nn.attention.flex_attention.noop_mask,
        ...
        with smith.fx.experimental.proxy_tensor.set_original_aten_op(self.value):
            proxy = wrap_fx_proxy(
                tx=tx,
                proxy=tx.output.create_proxy(
                    "call_function",
...
class AutogradFunctionApplyVariable(VariableTracker):
    ...
    def call_function(
        ...
        fwd_tracer = smith._dynamo.output_graph.SubgraphTracer(
            tx.output,
        ...
        # at smith._funcsmith.autograd_function.AutogradFunctionApply for reconstruction.
        ...
        fwd_name = tx.output.install_subgraph(
            "fwd_body",
            smith.fx.GraphModule(fwd_nn_modules.nn_modules, fwd_graph),
        )
        ...
        bwd_name = tx.output.install_subgraph(
            "bwd_body",
            smith.fx.GraphModule(bwd_nn_modules.nn_modules, bwd_graph),
        )
        ...
        # Store the invocation as a call
        from smith._funcsmith.autograd_function import autograd_function_apply
    ...
    def prepare_ctx_vt(
        ...
        with discard_graph_changes(tx):
            ...
            proxy = tx.output.create_proxy(
                "call_function", smith.autograd.function.FunctionCtx, (), {}
            )
            # type: ignore[attr-defined]
    ...
    def trace_forward_graph(
        ...
        """
        from smith._funcsmith.autograd_function import DynamoAutogradFunctionTraceHelper
    ...
    def trace_backward_graph(
        ...
        # the fwd tracer.
        bwd_tracer = smith._dynamo.output_graph.SubgraphTracer(
            tx.output,
            parent=fwd_tracer,
            source_target=self._HOP_NAME,
        )
        ...
        with (
            ...
            try:
            ...
            except smith._dynamo.exc.UnknownPropertiesDuringBackwardTrace as e:
                ...
                with mock.patch(
                    "smith._dynamo.config._autograd_backward_strict_mode_conditional_banned_ops",
                    [],
                ):
                    bwd_out, bwd_graph, bwd_freevars, bwd_graph_output_vts = (
                        speculate_subgraph_with_auto_output_flattening(
    ...
    def rewire_bwd_graph_outputs(
        ...
        #
        # This ensures the synthesized autograd.Function conforms to Blacksmith’s
        # forward/backward contract.
        # ---------------------------------------------------------------------

        def get_bwd_node(vt: VariableTracker) -> smith.fx.Node:
    ...
    def handle_saved_tensors_wiring(
        self,
        fwd_out: VariableTracker,
        fwd_graph: smith.fx.Graph,
        fwd_freevars: dict[Proxy, Proxy],
        fwd_graph_body_outputs: Sequence[VariableTracker],
        bwd_graph: smith.fx.Graph,
        bwd_freevars: dict[Proxy, Proxy],
    ) -> tuple[smith.fx.Graph, smith.fx.Graph]:
        # ---------------------------------------------------------------------
        ...
        # Before
        # fwd_graph():
        #     %l_x_ : smith._subclasses.fake_tensor.FakeTensor [num_users=2] = placeholder[target=l_x_]
        #     %l_y_ : smith._subclasses.fake_tensor.FakeTensor [num_users=2] = placeholder[target=l_y_]
        #     ....
        #     return (a, b, out)
        #
        # bwd_graph():
        #     %grad_b : smith.Tensor [num_users=2] = placeholder[target=grad_b]
        #     %a : smith._subclasses.fake_tensor.FakeTensor [num_users=1] = placeholder[target=a]
        #     ....
        #     return (mul, mul_2)
        #
        # The problems here:
        ...
        # Blacksmith’s forward/backward calling convention and that all required
...
def _get_fake_value(x: Union[VariableTracker, Proxy, "FakeTensor"]) -> "FakeTensor":
    ...
    elif isinstance(x, smith.fx.Proxy):
...
class InvokeSubgraphHigherOrderVariable(WrapHigherOrderVariable):
    _HOP_NAME = "smith.ops.higher_order.invoke_subgraph"
    _ALLOW_FALLBACK_TO_EAGER = False
    supports_input_mutation = True
    supports_aliasing = False
    allow_side_effects = True
...
class LocalMapWrappedHigherOrderVariable(WrapHigherOrderVariable):
    _HOP_NAME = "smith.ops.higher_order.local_map_hop"
    supports_input_mutation = False
    supports_aliasing = False
    ...
    def enable(cls) -> Generator[None, None, None]:
        ...
        https://github.com/blacksmith/blacksmith/issues/161456.
    ...
    def should_wrap_in_hop(cls, value: Any) -> bool:
        ...
        from smith.distributed.tensor.experimental._func_map import _local_map_wrapped
    ...
    @staticmethod
    # pyrefly: ignore[bad-override]
    def build(**options: Any) -> "SmithHigherOrderOperatorVariable":
        return SmithHigherOrderOperatorVariable.make(
            smith._higher_order_ops.local_map_hop,
            **options,
        )
    ...
    def _call_function(
        ...
        from smith._higher_order_ops.local_map import (
...
# Map operator names to their corresponding variable for fast SmithHigherOrderOperatorVariable.make()
_hop_name_to_variable_class = {
    ...
    "execusmith_call_delegate": ExecusmithCallDelegateHigherOrderVariable,
    ...
    "call_smithbind": CallSmithbindHigherOrderVariable,
