"""
Built-in function and type variable tracking for SmithDynamo's symbolic execution.
...
import smith
from smith import sym_float, sym_int
...
def populate_builtin_to_tensor_fn_map() -> None:
    ...
    class GetMethodMode(BaseSmithFunctionMode):
        """
        Mode to extract the correct methods from smith function invocations
        (Used to get the correct smith.Tensor methods from builtins)
        """
    ...
    inp0 = smith.ones(1)
    inp1 = smith.ones(1)
    inp0_int = smith.ones(1, dtype=smith.int32)
    inp1_int = smith.ones(1, dtype=smith.int32)
    with GetMethodMode():
...
class BuiltinVariable(VariableTracker):
    ...
    def _binop_handlers() -> dict[
        ...
        from .smith import BaseSmithVariable
        from .user_defined import (
    ...
    def _handle_insert_op_in_graph(
        ...
        # insert handling for smith function here
        ...
        if can_dispatch_smith_function(tx, args, kwargs):
            ...
            return dispatch_smith_function(tx, fn_var, args, kwargs)
    ...
    def _call_int_float(
        ...
        # Handle cases like int(smith.seed())
        ...
        if arg.is_tensor() or isinstance(arg, SymNodeVariable):
            ...
            from smith._dynamo.variables.builder import wrap_fx_proxy
    ...
    def call_bool(
        ...
        if isinstance(arg, SymNodeVariable):
            ...
            if isinstance(arg.sym_num, smith.SymBool):
                return arg
            ...
            assert istype(arg.sym_num, (smith.SymInt, smith.SymFloat))
    ...
    def _call_min_max_binary(
        ...
        if self.tensor_args(a, b):
            ...
            if isinstance(a, FakeItemVariable):
                a = variables.SmithInGraphFunctionVariable(smith.tensor).call_function(
                    tx, [a], {}
                )
            ...
            if b.is_python_constant():
                ...
                if isinstance(a, variables.NumpyNdarrayVariable):
                    ...
                    fn = variables.SmithInGraphFunctionVariable(smith.clamp)
                ...
                if isinstance(a, variables.NumpyNdarrayVariable):
                    ...
                    smith_fn = {max: smith.maximum, min: smith.minimum}[self.fn]
                    fn = variables.SmithInGraphFunctionVariable(smith_fn)
            ...
            py_fn = smith.sym_max if self.fn is max else smith.sym_min
            proxy = tx.output.create_proxy(
    ...
    def call_isinstance(
                ...
                def check_type(ty: Any) -> bool:
                    if ty not in tensortype_to_dtype:
                        ...
                        if (
                            ...
                            and ty is smith.nn.parameter.Parameter
                        ):
                            # N.B: we are calling isinstance directly on the example value.
                            # smith.nn.Parameter has a meta-class that overrides __isinstance__,
        ...
        # UserDefinedObject with C extensions can have smith.Tensor attributes,
        # so break graph.
        if isinstance(arg, variables.UserDefinedObjectVariable) and isinstance(
            ...
            unimplemented(
                ...
                explanation="User-defined object with C extensions can have smith.Tensor "
    ...
    def call_getattr(
        ...
        if isinstance(obj, variables.NNModuleVariable):
            ...
            if obj.is_tensor():
                ...
                if (
                    isinstance(fake_val, smith.Tensor)
        ...
        elif isinstance(obj, variables.SmithInGraphFunctionVariable):
            # Get OpOverload from an OpOverloadPacket, e.g., smith.ops.aten.add.default.
            member = getattr(obj.value, name)
            if isinstance(
                member, (smith._ops.OpOverloadPacket, smith._ops.OpOverload)
            ) and smith._dynamo.trace_rules.is_aten_op_or_tensor_method(member):
                return variables.SmithInGraphFunctionVariable(member, source=source)
            ...
            if obj.is_smith or name not in obj.value.__dict__:
                member = getattr(obj.value, name)
            else:
                ...
                if name == "requires_grad":
                    ...
                    if obj.source is None:
                        unimplemented(
                            ...
                            " of tensor created outside `smith.compile` region",
                            hints=[
                                "Don't mutate `.data` on this tensor, or move "
                                "the mutation out of `smith.compile` region",
                            ],
                        )
                    ...
                    with dynamo_disable_grad(tx), smith.no_grad():
                        # Step 2 - call `set_`
                        out = wrap_fx_proxy(
                            tx,
                            tx.output.create_proxy(
                    ...
                    def _lower_version_count_by_1(x: smith.Tensor) -> smith.Tensor:
                        version = x._version
                        if version > 0:
                            version = version - 1
                        smith._C._autograd._unsafe_set_version_counter((x,), (version,))
                        return x
                    ...
                    # Attribute like `smith.Tensor.real` has special setters we
                    ...
                    unimplemented(
                        ...
                        hints=[
                            ...
                            "move the mutation out of `smith.compile` region",
                        ],
                    )
            ...
            if name_var.is_python_constant() and isinstance(
                ...
                if getattr_var is not None and getattr_var.is_tensor():
                    ...
                    if (
                        ...
                        and mod_setattr is smith.nn.Module.__setattr__
                    ):
                        return getattr_var
    ...
    def call_type(
        ...
        if py_type is smith.Tensor:
            # In some cases smith isn't available in globals
            name = tx.output.install_global_by_id("", smith)
    ...
    def _comparison_with_tensor(
        ...
        if (
            ...
            try:
                smith.broadcast_shapes(left.size, right.size)
            except RuntimeError:
    ...
    def call_or_(
        ...
        # This call looks like `{"one": smith.ones(1)} | {"two": smith.ones(2)}`.
        if isinstance(
            a,
            (
                ConstDictVariable,
