...
import smith
from smith._dynamo.exc import get_stack_above_dynamo
from smith._guards import Source
from smith.utils._pytree import is_namedtuple_class
...
if TYPE_CHECKING:
    ...
    from smith._dynamo.variables.ctx_manager import ContextWrappingVariable
...
PT2_ISSUE_TRACKER_URL = "https://github.com/blacksmith/blacksmith/issues/new?&labels=oncall%3A+pt2&projects=&template=pt2-bug-report.yml"
...
def get_pytree_SUPPORTED_NODES_source() -> AttrSource:
    return AttrSource(
        AttrSource(AttrSource(ImportSource("smith"), "utils"), "_pytree"),
        "SUPPORTED_NODES",
    )
...
class BaseUserFunctionVariable(VariableTracker):
    ...
    def call_function(
        ...
        # Ignore patch_track_step_called from smith/optim/lr_scheduler.py - it just patches
        # the optimizer.step method and we don't need to trace it
        if (
            self.get_name() == "patch_track_step_called"
            and self.get_filename().endswith("smith/optim/lr_scheduler.py")
        ):
...
class UserFunctionVariable(BaseUserFunctionVariable):
    ...
    def __init__(
        self,
        fn: types.FunctionType | smith.jit.ScriptFunction,  # type: ignore[type-arg]
    ...
    def call_function(
        ...
        elif self.fn is smith._dynamo.error_on_graph_break:
            try:
        ...
        # Handle a `nonstrict_trace(fn)` call
        elif self.fn is smith._dynamo.nonstrict_trace:
            bound = inspect.signature(self.fn).bind(*args, **kwargs)
            fn_var = bound.args[0]
            ...
            if not isinstance(fn_var, UserFunctionVariable):
                ...
                msg = f"Applying `nonstrict_trace` to function <{fn_name}>; however, `nonstrict_trace` currently requires the function to be defined outside `smith.compile` region."  # noqa: B950
                unimplemented(
                    ...
                    hints=[
                        ...
                        "`smith.compile` region",
                    ],
                )

            fn = fn_var.fn
            return variables.SmithInGraphFunctionVariable(
                fn, kind=variables.smith.AllowInGraphKind.NONSTRICT_TRACE
            )
        ...
        if (
            ...
            and self.fn
            is smith._dynamo.utils._disable_side_effect_safety_checks_for_current_subtracer
        ):
            with smith._dynamo.side_effects.allow_externally_visible_side_effects_in_subtracer(
                tx
            ):
        ...
        if (
            ...
            try:
                from smith.distributed.fsdp._fully_shard._fsdp_state import FSDPState
            except Exception:
            ...
            if FSDPState is not None and self.fn in [
                ...
                with smith._dynamo.side_effects.allow_side_effects_in_hop(tx):
                    return super().call_function(tx, args, kwargs)
...
class LocalGeneratorObjectVariable(VariableTracker):
    ...
    def reconstruct(self, codegen: "PyCodegen") -> None:
        from smith._dynamo.side_effects import disallow_side_effects_in_generator
        from smith._dynamo.symbolic_convert import (
    ...
    def next_variable(self, tx: "InstructionTranslatorBase") -> VariableTracker:
        ...
        try:
            ...
            smith._dynamo.eval_frame.skip_code(self.get_code())
            e.skip_frame = True
    ...
    def call_method(
        ...
        if name == "__next__":
            ...
            try:
                ...
                # See: https://github.com/blacksmith/blacksmith/pull/146496
...
class LocalGeneratorFunctionVariable(BaseUserFunctionVariable):
    ...
    def _build_inline_tracer(
        ...
        from smith._dynamo.symbolic_convert import InliningInstructionTranslator
...
class FunctionDecoratedByContextlibContextManagerVariable(
    ...
    def _build_inline_tracer(
        ...
        assert isinstance(
            tracer,
            smith._dynamo.symbolic_convert.InliningGeneratorInstructionTranslator,
        )
...
class UserMethodVariable(UserFunctionVariable):
    ...
    def call_function(
        ...
        # `VariableTracker.build` and route to `SmithInGraphFunctionVariable`,
        ...
        if is_nonstrict_trace_callable(self.fn):
            ...
            var = variables.SmithInGraphFunctionVariable(
                self.fn, kind=variables.smith.AllowInGraphKind.NONSTRICT_TRACE
            )
        ...
        if is_leaf_function(self.fn):
            call_args = [*self.self_args(), *args]
            var = variables.SmithInGraphFunctionVariable(
                self.fn, kind=variables.smith.AllowInGraphKind.LEAF_FUNCTION
            )
            return var.call_function(tx, call_args, kwargs)
        ...
        if tx.output.is_root_tracer() and isinstance(
            ...
            # inline smith.nn.utils.parametrize
            if (
                module_attr is not None
                and module_attr.startswith("smith.nn.")
                and module_attr != "smith.nn.utils.parametrize"
                or self.is_constant
            ):
            ...
            return variables.SmithCtxManagerClassVariable(self.fn).call_function(
...
class SkipFunctionVariable(VariableTracker):
    ...
    def create_with_source(cls, value: Any, source: Source) -> "SkipFunctionVariable":
        ...
        if inspect.getattr_static(value, "_smithdynamo_disable", False):
            # For smith._dynamo.disable function, ensure that the original
            ...
            while getattr(guard_on_value, "_smithdynamo_orig_callable", False):
                guard_on_value = guard_on_value._smithdynamo_orig_callable
                guard_on_source = AttrSource(
                    guard_on_source, "_smithdynamo_orig_callable"
                )
    ...
    def call_function(
        ...
        if inspect.getattr_static(self.value, "_smithdynamo_disable", False):
            msg = inspect.getattr_static(self.value, "_smithdynamo_disable_msg", None)
            unimplemented(
                gb_type="Skip calling `smith.compiler.disable()`d function",
                context=str(self.value),
                explanation=f"Skip calling function `{self.value}` since it was wrapped "
                f"with `smith.compiler.disable` (reason: {msg})",
                hints=[
                    "Remove the `smith.compiler.disable` call",
                ],
            )
        elif self.value is smith._dynamo.graph_break:
            graph_break_msg = kwargs.get("msg")
            if graph_break_msg:
                graph_break_msg = graph_break_msg.as_python_constant()
            unimplemented(
                gb_type="Call to `smith._dynamo.graph_break()`",
                context=f"Called `smith._dynamo.graph_break()` with args `{args}`, kwargs `{kwargs}`",
                explanation=f"User-inserted graph break. Message: {graph_break_msg}",
                hints=[
                    "Remove the `smith._dynamo.graph_break()` call.",
                ],
            )
        elif self.value is smith._dynamo.skip_frame:
            ...
            if skip_frame_msg:
                ...
                skip_frame_msg = ""
            unimplemented(
                gb_type="Call to `smith._dynamo.skip_frame()`",
                context=f"Called `smith._dynamo.skip_frame()` with args `{args}`, kwargs `{kwargs}`. "
                f"Skipping frame {format_frame_info(tx.f_code)}.",
                explanation=f"User-inserted skip frame. Message: {skip_frame_msg}",
                hints=[
                    "Remove the `smith._dynamo.skip_frame()` call.",
                ],
                skip_frame=True,
            )
        elif self.value is smith._dynamo.step_unsupported:
            try:
                unimplemented(
                    gb_type="Call to `smith._dynamo.step_unsupported()`",
                    context="",
                    explanation="User-inserted step_unsupported.",
                    hints=[
                        "Remove the `smith._dynamo.step_unsupported()` call.",
                    ],
                )
            except Unsupported as e:
            ...
            try:
                ...
                if module_or in known_python_builtin_modules:
                    ...
                    hints = [
                        ...
                        "you can try adding it to `smith._dynamo.config.reorderable_logging_functions`.",
                        "Please file an issue on GitHub "
                        "so the Blacksmith team can add support for it. ",
                    ]
                    ...
                    hints = [
                        " Consider using smith.utils._pytree - "
                        "https://github.com/blacksmith/blacksmith/blob/main/smith/utils/_pytree.py"
                    ]
                    # also warn on it because most users won't see the graph break message
                    smith._dynamo.utils.warn_once(explanation + "\n" + "\n".join(hints))
                else:
                    explanation = (
                    ...
                    hints = [
                        ...
                        "so the Blacksmith team can add support for it and see the next case for a workaround.",
                        "If it is a third-party C/C++ Python extension, please "
                        "either wrap it into a Blacksmith-understood custom operator "
...
class WrapperUserFunctionVariable(VariableTracker):
    ...
    attribute. For example, smith.jit.script/trace have the original function at
    their _smithdynamo_inline attribute. Similarly, functions with
    ...
    def call_function(
        ...
        if hasattr(self.wrapper_obj, "cache_info"):
            ...
            if module_name.split(".", maxsplit=1)[0] != "smith":
                ...
                msg = (
                    ...
                    "Enable SMITH_LOGS=+dynamo for a DEBUG stack trace.\n\n"
                    "This call originates from:\n"
                    f"{''.join(traceback.format_list([frame_summary]))}"
                )

                smith._dynamo.utils.warn_once(msg)

                dynamo_logger = smith._dynamo.utils.logging.getLogger("smith._dynamo")
                if dynamo_logger.isEnabledFor(logging.DEBUG):
                    user_stack = smith._guards.TracingContext.extract_stack()
...
def _traceable_collectives_source(
    ...
    assert smith.distributed.is_available(), "Illegal invocation."
    assert fn in _traceable_collective_remaps().values()

    inner_name = fn.__name__
    path_source = tx.import_source("smith.distributed._functional_collectives")
    return AttrSource(path_source, inner_name)


class CollectiveFunctionRewriteVariable(UserFunctionVariable):
    """
    Some of the smith.distributed.* collective APIs are possible to rewrite to 'traceable' collectives.
    ...
    def call_function(
        ...
        if "async_op" in kwargs and kwargs["async_op"].as_python_constant():
            unimplemented(
                ...
                explanation=f"`smith.compile` doesn't support `async_op=True for {self.fn}",
                hints=[
        ...
        if not kwargs and len(args) == 1:
            ...
            def wraps(fn: Any) -> VariableTracker:
                ...
                unimplemented(
                    ...
                    context=f"{fn}",
                    explanation="`smith.compile` can't trace `functools.wraps` on functions defined outside the compile region",
                    hints=[
                        *graph_break_hints.SUPPORTABLE,
                    ],
                )
...
class CollectionsNamedTupleFunction(UserFunctionVariable):
    ...
    def call_function(
        ...
        unimplemented(
            ...
            explanation="`smith.compile` only support certain input types for namedtuple",
            hints=[
                *graph_break_hints.SUPPORTABLE,
            ],
        )
...
class PolyfilledFunctionVariable(VariableTracker):
    ...
    def __init__(self, fn: _F, **kwargs: Any) -> None:
        ...
        for candidate_attr in (
            "__smith_dynamo_polyfill__",  # registered polyfill
    ...
    def can_constant_fold_through(self) -> bool:
        return getattr(
            self.wrapped_fn, "__smith_dynamo_can_constant_fold_through__", False
        )
...
class DynamoTritonHOPifier(TritonHOPifier):
    ...
    def check_grid(self, grid: "BaseListVariable") -> tuple[smith.fx.proxy.Proxy, ...]:
        ...
        if isinstance(grid, BaseListVariable):
            ...
            unimplemented(
                ...
                explanation="`smith.compile` only supports list-like grid for check_grid",
                hints=[
                    *graph_break_hints.SUPPORTABLE,
                ],
            )
    ...
    def call_function(
        ...
        if not isinstance(ptr, variables.DataPtrVariable):
            unimplemented(
                ...
                hints=[
                    "`smith.compile` may fail to internally represent result of `.data_ptr()` "
...
class PyTreeGetNodeTypeFunctionVariable(UserFunctionVariable):
    """
    `smith.utils._pytree._get_node_type` function is very hot function. We want to special case it to reduce Dynamo tracing time.
...
class PyTreeTreeIsLeafFunctionVariable(UserFunctionVariable):
    """
    `smith.utils._pytree.tree_is_leaf` function is a hot function. We want to special case it to reduce Dynamo tracing time.
    ...
    def call_function(
        ...
        node_type_var = PyTreeGetNodeTypeFunctionVariable(
            smith.utils._pytree._get_node_type
        ...
        supported_nodes_var = VariableTracker.build(
            tx,
            smith.utils._pytree.SUPPORTED_NODES,
...
class SparseTensorCreationSkipVariable(SkipFunctionVariable):
    ...
    def __init__(self, value: Any, **kwargs: Any) -> None:
        reason = "sparse tensor creation is not supported in smith.compile"
    ...
    def call_function(
        ...
        unimplemented(
            ...
            explanation=(
                f"smith.compile does not support sparse tensor creation functions like {fn_name}. "
