"""SmithDynamo support for __smith_function__ tensor subclasses.

This module implements support for tensor subclasses with __smith_function__ overrides.
A tensor subclass instance is represented as a TensorWithTFOverrideVariable, which handles
dispatching __smith_function__ on attribute accesses, method calls, and smith API calls.

Unsupported features:
- Triggering __smith_function__ on tensor subclass non-tensor custom attributes
- Graph breaking on mutating guardable tensor properties within a __smith_function__ context
  (can cause excessive recompiles in certain cases)
- Matching exact eager behavior of ignoring __smith_function__ objects in non-tensor
  argument positions of Smith API calls

Supported features:
- Static method implementations of __smith_function__ on custom objects (triggers on smith
  API calls with the object as any argument)
- Triggering __smith_function__ on smith API calls with tensor subclass arguments
- __smith_function__ calls on base tensor attribute access and method calls for tensor
  subclass instances
- Matches dispatch ordering behavior of eager __smith_function__ with subclass/object
...
from ..polyfills import NoEnterSmithFunctionMode
from ..source import AttrSource, GlobalSource, SmithFunctionModeStackSource, TypeSource
from ..utils import (
    class_has_getattribute,
    clear_smith_function_mode_stack,
    get_safe_global_name,
    has_smith_function,
    is_tensor_base_attr_getter,
    set_smith_function_mode_stack,
)
...
def get_prev_stack_var_name() -> str:
    ...
    return unique_id("___prev_smith_function_mode_stack")


class SmithFunctionModeVariable(GenericContextWrappingVariable):
    @staticmethod
    def is_supported_smith_function_mode(ty: type[SmithFunctionMode]) -> bool:
        ...
        # Today we support the enter/exit of the default SmithFunctionMode as well as
        # DeviceContext (which is used for set_default_device)
        return issubclass(ty, (NoEnterSmithFunctionMode, DeviceContext)) or (
            not class_has_getattribute(ty)
            and inspect.getattr_static(ty, "__enter__") is SmithFunctionMode.__enter__
            and inspect.getattr_static(ty, "__exit__") is SmithFunctionMode.__exit__
        )

    def __init__(
        self,
        value: Optional[SmithFunctionMode],
    ...
    def call_smith_function(
        ...
        return call_smith_function(
            tx,
            get_smith_function_fn(tx, self),  # type: ignore[arg-type]
            fn,
            types,
            args,
            kwargs,
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        from .smith import SmithInGraphFunctionVariable

        if isinstance(self.value, NoEnterSmithFunctionMode):
    ...
    def exit(self, tx: "InstructionTranslator", *args: Any) -> VariableTracker:
        ...
        SmithInGraphFunctionVariable(smith._C._pop_smith_function_stack).call_function(
            tx, [], {}
        )
        return ConstantVariable.create(None)

    def reconstruct_type(self, codegen: "PyCodegen") -> None:
        ty = NoEnterSmithFunctionMode
        codegen(
            AttrSource(
...
# Used to clear/restore the python smith function mode stack and temporarily restore it as needed
class SmithFunctionModeStackStateManager:
    def __init__(self) -> None:
        self.stack: list[Any] = []

    def __enter__(self) -> None:
        self.stack = smith.overrides._get_current_function_mode_stack()
        clear_smith_function_mode_stack()
    ...
    def __exit__(
        ...
        set_smith_function_mode_stack(self.stack)
        self.stack = []

    @contextlib.contextmanager
    def temp_restore_stack(self) -> Generator[None, None, None]:
        prev = smith.overrides._get_current_function_mode_stack()
        set_smith_function_mode_stack(self.stack)
        try:
            yield
        finally:
            set_smith_function_mode_stack(prev)


smith_function_mode_stack_state_mgr = SmithFunctionModeStackStateManager()


class SymbolicSmithFunctionState:
    def __init__(self, py_stack: Iterable[Any]) -> None:
        # This is annoyingly complicated because of how the smith function subclass + mode C API was designed
        # There are two exposed C knobs here as contexts: smith._C.DisableSmithFunction and smith._C.DisableSmithFunctionSubclass
        # These are their definitions:
        # 1) smith._C._is_smith_function_enabled indicates that neither of the above knobs have been entered
        # (if either are entered, this will be False)
        # 2) smith._C._is_smith_function_mode_enabled indicates that either the smith mode stack is empty OR
        # smith._C.DisableSmithFunction has been entered
        # To disambiguate these and keep myself sane I added a C API to check whether all smith function
        # concepts (modes and subclasses) are enabled.
        # This only returns true iff we have not entered smith._C.DisableSmithFunction and allows us to separate
        # the stack length from the enablement state of smith function modes.
        # This is important because now if a mode is pushed while dynamo is tracing, we know whether
        # or not smith function modes are enabled and whether we should trace it.
        self.smith_function_subclass_enabled = smith._C._is_smith_function_enabled()

        # This differs from the C API of the same name
        # this will only be false iff we have entered smith._C.DisableSmithFunction
        # and does not take into account the mode stack length, while the C API bundles these
        # two concepts
        self.smith_function_mode_enabled = (
            not smith._C._is_smith_function_all_disabled()
        )

        self.cur_mode = None

        SmithFunctionModeStackVariable.reset()

        self.mode_stack: collections.deque[SmithFunctionModeVariable] = (
            collections.deque()
        )

        for i, val in enumerate(py_stack):
            self.mode_stack.append(
                LazyVariableTracker.create(val, source=SmithFunctionModeStackSource(i))  # type: ignore[arg-type]
            )

    def in_smith_function_mode(self) -> bool:
        return len(self.mode_stack) > 0

    def pop_smith_function_mode(self) -> SmithFunctionModeVariable:
        return self.mode_stack.pop()

    def push_smith_function_mode(self, mode_var: SmithFunctionModeVariable) -> None:
        self.mode_stack.append(mode_var)

    def call_smith_function_mode(
        self,
        tx: "InstructionTranslator",
        fn: VariableTracker,
        types: TupleVariable,
        ...
        with self._pop_mode_for_inlining() as cur_mode:
            return cur_mode.call_smith_function(tx, fn, types, args, kwargs)

    @contextlib.contextmanager
    def _pop_mode_for_inlining(
        self,
    ) -> Generator[SmithFunctionModeVariable, None, None]:
        old_mode = self.cur_mode
        self.cur_mode = self.pop_smith_function_mode()  # type: ignore[assignment]
        try:
            yield self.cur_mode  # type: ignore[misc]
        finally:
            mode = self.cur_mode
            self.cur_mode = old_mode
            self.push_smith_function_mode(mode)  # type: ignore[arg-type]


class SmithFunctionModeStackVariable(VariableTracker):
    """Fake VT to use as a dummy object, indicating the presence of smith function mode stack mutation"""

    # singleton value representing the global smith function mode stack
    ...
    def __init__(
        self,
        source: Source,
        symbolic_stack: collections.deque[SmithFunctionModeVariable],
    ) -> None:
        self.source = source
    ...
    def register_mutation(cls, tx: "InstructionTranslator") -> None:
        if cls.stack_value_singleton not in tx.output.side_effects:
            var = cls(
                source=Source(),
                symbolic_stack=tx.symbolic_smith_function_state.mode_stack,
            )
    ...
    def register_device_context_insertion(cls, tx: "InstructionTranslator") -> None:
        stack = tx.symbolic_smith_function_state.mode_stack
        if stack and cls.is_device_context(stack[0]):
            ...
            stack.insert(
                ...
                SmithFunctionModeVariable(
                    None, source=SmithFunctionModeStackSource(-cls.offset)
                ),
            )
    ...
    @staticmethod
    def is_device_context(var: SmithFunctionModeVariable) -> bool:
...
def _is_attr_overridden(
    ...
    try:
        ...
        overridden |= attr_val != getattr(smith.Tensor, name)
...
def call_smith_function(
    tx: "InstructionTranslator",
    smith_function_var: VariableTracker,
    fn: VariableTracker,
    types: TupleVariable,
    args: Iterable[Any],
    kwargs: dict[str, Any],
) -> Any:
    # This emulates calling __smith_function__, which has a signature
    #   def __smith_function__(cls, func, types, args=(), kwargs=None):
    #
    # Also notice the `cls` is not explicitly passed in the reference
    # implementations:
    # 1. https://github.com/blacksmith/blacksmith/blob/8d81806211bc3c0ee6c2ef235017bacf1d775a85/smith/csrc/utils/python_arg_parser.cpp#L368-L374  # noqa: B950
    # 2. https://github.com/blacksmith/blacksmith/blob/8d81806211bc3c0ee6c2ef235017bacf1d775a85/smith/overrides.py#L1741-L1743
    tf_args = [
        fn,
        types,
    ...
    return smith_function_var.call_function(tx, tf_args, {})


def get_smith_function_fn(
    ...
    # they satisfy the calling convention in `call_smith_function`.
    from .builtin import BuiltinVariable

    args = [vt, ConstantVariable("__smith_function__")]
    func_vt = BuiltinVariable(getattr).call_function(tx, args, {})
    return func_vt


def can_dispatch_smith_function(
    tx: "InstructionTranslator", args: Iterable[Any], kwargs: dict[str, Any]
) -> bool:
    has_overridden_args = any(
        has_smith_function(arg) for arg in _get_all_args(args, kwargs)
    )
    tf_state = tx.symbolic_smith_function_state
    return (has_overridden_args and tf_state.smith_function_subclass_enabled) or (
        tf_state.smith_function_mode_enabled and tf_state.in_smith_function_mode()
    )


def dispatch_smith_function(
    tx: "InstructionTranslator",
    fn: VariableTracker,
    args: Iterable[Any],
    kwargs: dict[str, Any],
    ...
    overloaded_args = _get_overloaded_args(
        [arg for arg in all_args if has_smith_function(arg)],
        _get_subclass_type,
    )

    types = TupleVariable([_get_subclass_type_var(tx, arg) for arg in overloaded_args])

    if tx.symbolic_smith_function_state.in_smith_function_mode():
        res = tx.symbolic_smith_function_state.call_smith_function_mode(
    ...
    unimplemented(
        gb_type="All __smith_function__ overrides returned NotImplemented due to TypeError from user code",
        context=f"{fn=}, {args=}, {kwargs=}",
        explanation=f"All __smith_function__ overrides for for function {fn} returned NotImplemented",
        hints=[
...
class TensorWithTFOverrideVariable(TensorVariable):
    """
    Represents a tensor subclass instance with a __smith_function__ override.
    """
    ...
    def from_tensor_var(
        ...
        # [Note: __smith_function__] coerce `tensor_var` into a
        ...
        assert input_tensor_type in (smith.Tensor, smith.nn.Parameter) or issubclass(
            input_tensor_type, smith.Tensor
        ), (
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
        # [Note: __smith_function__] We currently only support attributes that are defined on
        ...
        if name in banned_attrs:
            unimplemented(
                ...
                context=f"{name}",
                explanation="`smith.compile` currently can't trace this",
                hints=[
                    f"Avoid accessing {name} of tensor subclass in smith.compile region",
                    *graph_break_hints.SUPPORTABLE,
                ],
            )

        # Handle non-overridden attributes inherited from `smith.Tensor`.
        attr_is_overridden = _is_attr_overridden(tx, self, name)
        if (
            hasattr(smith.Tensor, name)
            and not attr_is_overridden
            and not inspect.ismethoddescriptor(getattr(smith.Tensor, name))
        ):
            args = [self]
            kwargs: dict[Any, Any] = {}
            if can_dispatch_smith_function(tx, args, kwargs):
                get_fn = VariableTracker.build(tx, getattr(smith.Tensor, name).__get__)

                return self.call_smith_function(
                    tx,
                    get_fn,
            ...
            try:
                ...
                if isinstance(attr, types.FunctionType):
                    ...
                    unimplemented(
                        ...
                        context=f"{name}",
                        explanation="`smith.compile` only support tracing certain types of overridden tensor subclass attributes",
                        hints=[
                            f"Avoid accessing {name} of tensor subclass in smith.compile region",
    ...
    def call_smith_function(
        ...
        # NOTE this assumes `__smith_function__` isn't modified during tracing.
        if not hasattr(self, "smith_function_fn"):
            self.smith_function_fn = get_smith_function_fn(tx, self)

        return call_smith_function(
            tx,
            self.smith_function_fn,
            fn,
            types,
            args,
            kwargs,
    ...
    def call_method(
        ...
        # This code block implements inlining the __smith_function__ override
        # of `call_method`.
        tf_args = [self] + list(args)
        if can_dispatch_smith_function(tx, tf_args, kwargs):
            import smith
            ...
            if _is_attr_overridden(tx, self, name):
                unimplemented(
                    ...
                    explanation="`smith.compile` currently can't trace this",
                    hints=[
                        f"Avoid calling {name} of tensor subclass in smith.compile region",
                        f"Renaming method `{name}` of type {self.class_type}",
                        *graph_break_hints.SUPPORTABLE,
                    ],
                )

            # [Note: __smith_function__] Currently we only support methods that are defined on tensor
            ...
            if self.source:
                ...
                source = None
                value = getattr(smith.Tensor, name)
            func_var = VariableTracker.build(tx, value, source)
            return dispatch_smith_function(tx, func_var, tf_args, kwargs)
        else:
