...
- Lists and list subclasses (including smith.nn.ModuleList, ParameterList)
- Tuples and named tuples
- Ranges and slices
- Collections.deque
- smith.Size with special proxy handling
...
class BaseListVariable(VariableTracker):
    ...
    def cls_for(obj: Any) -> type:
        return {
            ...
            list: ListVariable,
            slice: SliceVariable,
            smith.Size: SizeVariable,
            tuple: TupleVariable,
            odict_values: ListVariable,
            smith.nn.ParameterList: ListVariable,
            smith.nn.ModuleList: ListVariable,
    ...
    def getitem_const(
        ...
        if isinstance(index, slice):
            ...
            assert isinstance(index, (int, smith.SymInt))
            try:
...
class SizeVariable(TupleVariable):
    ...
    def __init__(
        self,
        items: list[VariableTracker],
        proxy: Optional[smith.fx.Proxy] = None,
        **kwargs: Any,
    ) -> None:
        self.proxy = proxy
        super().__init__(items, **kwargs)
    ...
    def debug_repr(self) -> str:
        return self.debug_repr_helper("smith.Size([", "])")
    ...
    def as_proxy(self) -> Any:
        ...
        # smith.Size needs special handling.  Normally, we pun a list-like
        # container to directly contain Proxy/Node objects from FX, and FX
        # knows to look inside containers (via map_aggregate).  But smith.Size
        ...
        # smith.Size([proxy0, proxy1]).  I looked into seeing if I could
        # relax smith.Size in Blacksmith proper, but if smith.Size constructor
        ...
        #
        # To work around this problem, I represent a smith.Size proxy as
        ...
        # of this in smithdynamo though!

        # Look for a proxy.  If there are none, do the legacy behavior
        tracer = None
        proxies = self._as_proxy()
        for proxy in proxies:
            if isinstance(proxy, smith.fx.Proxy):
                tracer = proxy.tracer
                break

        if tracer is None:
            return smith.Size(proxies)

        proxy = tracer.create_proxy("call_function", smith.Size, (proxies,), {})
        set_example_value(
            proxy.node,
            smith.Size(
                [
    ...
    def reconstruct(self, codegen: "PyCodegen") -> None:
        codegen.add_push_null(lambda: codegen.load_import_from("smith", "Size"))
        codegen.foreach(self.items)
        build_smith_size = [
            create_build_tuple(len(self.items)),
        ] + create_call_function(1, False)
        codegen.extend_output(build_smith_size)
    ...
    def get_item_dyn(
        ...
        if isinstance(arg, SymNodeVariable):
            ...
            if value.constant is None or value.constant.numel() != 1:
                unimplemented(
                    gb_type="Indexing smith.Size with non-scalar tensor",
                    context=f"get_item_dyn {self} {arg}",
                    explanation=(
                        "Attempted to index smith.Size with a tensor that is not a scalar constant."
                    ),
                    hints=[*graph_break_hints.USER_ERROR],
                )
    ...
    def call_obj_hasattr(
        ...
        return variables.ConstantVariable.create(hasattr(smith.Size, name))
...
class NamedTupleVariable(UserDefinedTupleVariable):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if tuple_cls.__module__ == "smith.return_types":
...
class ListIteratorVariable(IteratorVariable):
    ...
    def __init__(
        ...
        # https://github.com/blacksmith/blacksmith/pull/87533#issuecomment-1287574492
