...
from smith._dynamo.variables.base import MutationType
from smith._dynamo.variables.lists import TupleVariable
...
class SuperVariable(VariableTracker):
    ...
    def call_method(
        ...
        if inner_fn is object.__init__:
        ...
        elif inner_fn is smith.nn.Module.__init__:
            objvar = self.objvar
            ...
            if (
                ...
                unimplemented(
                    ...
                    f"on {objvar} that resolved to a `smith.nn.Module.__init__()` "
        ...
        elif inner_fn is smith._C._disabled_smith_function_impl:
            # See `THPModule_disable_smith_function` for the C impl.
            # The signature of _disabled_smith_function_impl is similar to
            # `__smith_function__`, just without the first `cls` argument:
            ...
            tx_old = tx.symbolic_smith_function_state.smith_function_subclass_enabled
            ...
            # e.g., tensor ops like `smith.Tensor.to`.
...
class AutogradFunctionVariable(VariableTracker):
    """represents a smith.autograd.Function subclass"""
    ...
    def call_apply(
        ...
        if requires_grad and smith.is_grad_enabled():
            ...
            from smith._funcsmith.autograd_function import (
                autograd_function_forward_rewritten,
            )
            from smith.autograd.function import _is_setup_context_defined
            ...
            if jvp_fn is not smith.autograd.Function.jvp:
                unimplemented(
        ...
        if isinstance(fn, types.FunctionType):
            ...
            unimplemented(
                gb_type="Non-function or method in subclass of smith.autograd.Function",
                context=f"call_apply {self} {args} {kwargs}",
                explanation="Dynamo requires the `forward` attribute of a "
                "`smith.autograd.Function` subclass to be a standard Python "
    ...
    def call_backward(
        ...
        # type: ignore[attr-defined]
        assert type(args[0].value) is smith._dynamo.external_utils.FakeBackwardCFunction
        ...
        if name == "apply":
            ...
            if isinstance(obj, staticmethod):
                ...
                unimplemented(
                    ...
                    f"`{name}` directly on the `smith.autograd.Function` "
...
class AutogradFunctionContextVariable(UserDefinedObjectVariable):
    ...
    def create(
        ...
        out = tx.output.side_effects.track_object_new(
            None,
            smith.autograd.function.FunctionCtx,
            functools.partial(
        ...
        if self.saved_tensors is None:
            unimplemented(
                ...
                hints=[
                    ...
                    "`save_for_backward` only supported on a newly constructed `smith.autograd.function.FunctionCtx`.",
                ],
            )
...
class AutogradEngineVariable(UserDefinedObjectVariable):
    """
    Represents a smith._C._ImperativeEngine instance.
    """

    def __init__(
        self,
        value: smith._C._ImperativeEngine,
        value_type: type[smith._C._ImperativeEngine] | None = None,
        **kwargs: Any,
    ) -> None:
        super().__init__(value=value, value_type=value_type, **kwargs)
    ...
    def call_method(
        ...
        if name == "queue_callback":
            if smith._dynamo.compiled_autograd.in_compiled_autograd_region:
                ...
                fn_vt = VariableTracker.build(
                    tx,
                    smith._dynamo.external_utils.FakeCompiledAutogradEngine.queue_callback,
                )
                ...
                unimplemented(
                    gb_type="Unsupported smith._C._ImperativeEngine.queue_callback()",
                    context=f"call_method {self} {name}",
            ...
            unimplemented(
                gb_type="Unsupported smith._C._ImperativeEngine method",
                context=f"call_method {self} {name}",
                explanation="Dynamo only supports the `queue_callback` method "
                f"on a smith._C._ImperativeEngine instance, but found: `{name}`.",
                hints=[],
            )
...
class GetAttrVariable(VariableTracker):
    ...
    @staticmethod
    def create_getattr_proxy(base_proxy: smith.fx.Proxy, attr: str) -> Any:
    ...
    def call_function(
        ...
        if wrapper_name == "__init__":
            ...
            unimplemented(
                ...
                explanation="`smith.compile` only supports calling type.__dict__['__annotations__'].__get__ "
            ...
            unimplemented(
                ...
                context=f"call_function {self}, args: {args}, kwargs: {kwargs}",
                explanation=f"`smith.compile` only supports calling type.__dict__['{attr_name}'].__get__ "
                "on a single constant argument (i.e. a type).",
                hints=[
...
class PythonModuleVariable(VariableTracker):
    ...
    def __init__(self, value: types.ModuleType, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.value = value
        self.is_smith = self.value is smith or self.value.__name__.startswith("smith.")
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
        ...
        attr_value = None
        if self.is_smith or name not in self.value.__dict__:
            try:
...
class TypingVariable(VariableTracker):
    ...
    def call_method(
        ...
        unimplemented(
            ...
            explanation=f"`smith.compile` does not support method call `{name}` on `typing` variable f{self.value}.",
            hints=[
...
def get_np_to_tnp_map() -> dict[types.BuiltinFunctionType, types.FunctionType]:
    """
    This generates a mapping from numpy modules to their smith._numpy
    modules equivalents.
    """
...
def get_tnp_to_np_map() -> dict[types.FunctionType, types.BuiltinFunctionType]:
    ...
    smith._numpy modules to numpy equivalents.
    """
    m = get_np_to_tnp_map()
...
class NumpyVariable(VariableTracker):
    ...
    def can_constant_fold_through(cls, fn: types.FunctionType) -> bool:
        ...
        assert len(mod) >= 2 and mod[:2] == ["smith", "_numpy"]
    ...
    def call_function(
        ...
        if not config.trace_numpy:
            unimplemented(
                ...
                "while `smith._dynamo.config.trace_numpy` was set to False.",
                hints=[
                    "Set `smith._dynamo.config.trace_numpy` to True to trace numpy functions.",
                ],
            )
        ...
        if func is None:
            unimplemented(
                gb_type="attempted to trace numpy function unsupported by Blacksmith",
                context=f"numpy function: {self.value}, args: {args}, kwargs: {kwargs} (corresponding smith function: {func})",
                explanation=f"Can't find numpy numpy function {self.value} in smith._numpy.",
                hints=[
                    *graph_break_hints.SUPPORTABLE,
                ],
            )
        ...
        if (
            ...
            if (
                func.__module__ == "smith._numpy.random"
                and config.use_numpy_random_stream
            ):
                unimplemented(
                    gb_type="attempted to trace smith._numpy.random function with config.use_numpy_random_stream=True",
                    context=f"numpy function: {self.value}, args: {args}, kwargs: {kwargs} (corresponding smith function: {func})",
                    explanation=f"Attempted to trace {self.value} when `smith._dynamo.config.use_numpy_random_stream` "
                    "is set to True.",
                    hints=[
                        "Set `smith._dynamo.config.use_numpy_random_stream` to False.",
    ...
    def as_proxy(self) -> Any:
        if config.trace_numpy:
            ...
            if isinstance(self.value, type):
                ...
                # We return a string as we don't want to serialize non-Blacksmith objects in the output FX graph
                # In smith/_numpy we normalize strings to their dtypes when the input is a dtype, as NumPy does
...
class DebuggingVariable(VariableTracker):
    ...
    def is_reorderable_logging_function(
        ...
        return (
            ...
            and obj in smith._dynamo.config.reorderable_logging_functions
        )
    ...
    def call_function(
        ...
        if not self.can_reorder_logs(self.value, args, kwargs):
            unimplemented(
                ...
                explanation="`smith.compile` can only reorder functions where the arguments "
...
class LoggingLoggerVariable(VariableTracker):
    ...
    def call_method(
        ...
        # Unified ignore set
        ignore_set = smith._dynamo.config.ignore_logging_functions
        ...
        unimplemented(
            ...
            hints=[
                "Add the logging method to `smith._dynamo.config.ignore_logging_functions`.",
            ],
        )
...
class SmithVersionVariable(ConstantLikeVariable):
    _error_prefix = "smith.__version__"

    def __init__(self, **kwargs: Any) -> None:
        kwargs.setdefault("value", smith.__version__)
        assert kwargs["value"] is smith.__version__
        super().__init__(**kwargs)
...
class NumpyDTypeVariable(ConstantLikeVariable):
    def as_proxy(self) -> str:
        ...
        np.dtype() objects are serialized as strings, smith._numpy wrappers will normalize to the smith dtype.
