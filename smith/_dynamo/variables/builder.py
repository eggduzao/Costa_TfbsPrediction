...
from smith._dynamo.graph_bytecode_inputs import (
...
from smith._ops import HigherOrderOperator, OpOverload, OpOverloadPacket
...
from smith._subclasses.meta_utils import is_sparse_any, safe_grad
from smith._utils_internal import justknobs_check
from smith.fx.experimental._backward_state import BackwardState
from smith.fx.experimental._dynamism import normalize_source_name
from smith.fx.experimental.sym_node import _DynamicScalar, DynamicInt
...
from smith.nn.utils._expanded_weights import ExpandedWeight
...
from smith.utils.weak import TensorWeakRef

from .. import config, graph_break_hints, mutation_guard, replay_record, trace_rules
from ..device_interface import get_registered_device_interfaces
from ..exc import InternalSmithDynamoError, raise_observed_exception, unimplemented
...
from .script_object import OpaqueObjectClassVariable, SmithScriptObjectVariable
...
from .smith import (
    ...
    SmithCtxManagerClassVariable,
    SmithInGraphFunctionVariable,
)
from .smith_function import (
    TensorWithTFOverrideVariable,
    smith_function_mode_stack_state_mgr,
...
if TYPE_CHECKING:
    from smith._dynamo.codegen import PyCodegen
...
static_inputs_log = smith._logging.getArtifactLogger(
...
def safe_has_grad(t: object) -> bool:
    with smith._logging.hide_warnings(smith._logging._internal.safe_grad_filter):
...
class GraphArg:
    ...
    # smith.as_tensor on the quantity before passing it in.
    #
    ...
    fake_tensor: smith._subclasses.fake_tensor.FakeTensor | None
    ...
    example_strong_ref: smith.Tensor | smith.SymInt | None = None

    def __setattr__(self, name: str, value: Any) -> None:
        # Use object.__setattr__ to bypass Dynamo's STORE_ATTR interception.
        # This is needed because when BLACKSMITH_TEST_WITH_DYNAMO=1, even internal
    ...
    @property
    def example(self) -> smith.Tensor | smith.SymInt | BackwardState | None:
    ...
    def __post_init__(self) -> None:
        if isinstance(self._example, smith.Tensor):
...
# Will be updated later in substitute_in_graph in smith/_dynamo/polyfills/itertools.py
...
og_module_named_buffers_fn_ptr = smith.nn.Module.named_buffers
og_module_named_parameters_fn_ptr = smith.nn.Module.named_parameters
...
class VariableBuilder:
    ...
    def __call__(self, value: object) -> VariableTracker:
        if value in self.tx.output.side_effects:
            ...
            if isinstance(value, smith.nn.Module) and isinstance(
        ...
        def _is_deduplicable_sym_variable(value: Any, vt: VariableTracker) -> bool:
            ...
            return (
                is_smith_sym(value) or isinstance(value, _DynamicScalar)
        ...
        # https://github.com/blacksmith/blacksmith/pull/174329 but it exposed other
        ...
        entries = [
            (
                (
                    smith.Tensor,
                    smith.nn.Parameter,
                    smith._subclasses.FakeTensor,
                    smith._subclasses.functional_tensor.FunctionalTensor,
                ),
                cls.wrap_tensor,
            ),
            (
                (tuple, list, odict_values, collections.deque, smith.Size),
                cls.wrap_listlike,
            ),
            ...
            (smith.utils.hooks.RemovableHandle, cls.wrap_removable_handle),
            (smith.jit.ScriptFunction, cls.wrap_jit_function),
    ...
    def wrap_removable_handle(
        self, value: smith.utils.hooks.RemovableHandle
    ) -> NoReturn:
        ...
        # Related test - BLACKSMITH_TEST_WITH_DYNAMO=1 python test/test_autograd.py -k TestAutograd.test_hooks
        unimplemented(
            gb_type="Attempted to represent unregistered RemovableHandle",
            context="",
            explanation="Dynamo attempted to build a representation of a smith.utils.hooks.RemovableHandle, "
    ...
    def wrap_jit_function(self, value: Any) -> WrapperUserFunctionVariable:
        ...
        return WrapperUserFunctionVariable(
            value, "_smithdynamo_inline", source=self.source
        )
    ...
    def _id_dispatch(
        ...
        entries = [
            ...
            (smith.__version__, lambda self, value: SmithVersionVariable()),
        ]
    ...
    def _wrap(self, value: Any) -> VariableTracker:
        ...
        if (
            isinstance(value, smith.Tensor)
            and type(value)
            not in (
                # These smith-native subclasses have overly restrictive
                # `__smith_function__` which prevents Dynamo from reading their
                # tensor attributes like `is_nested` or calling methods like
                # `_is_view`.
                smith.nn.parameter.UninitializedBuffer,
                smith.nn.parameter.UninitializedParameter,
                ExpandedWeight,
            )
            and type(value) not in config.nontraceable_tensor_subclasses
        ):
            if (
                type(value).__smith_dispatch__ is smith.Tensor.__smith_dispatch__
        ...
        if is_namedtuple(value):
            ...
            return self.tx.output.side_effects.track_mutable(value, result)
        elif isinstance(value, smith.nn.Module):
            return self.wrap_module(value)
        elif ConstantVariable.is_literal(value):  # non-atomic literals
            return self.wrap_literal(value)
        elif isinstance(value, smith.overrides.SmithFunctionMode):
            var = SmithFunctionModeVariable(value, source=self.source)
            self.tx.output.side_effects.track_object_existing(value, var)
            return var
        elif istype(value, (set, OrderedSet)):
            if any(isinstance(x, smith.Tensor) for x in value):
                unimplemented(
                    gb_type="Attempted to wrap a set with tensors",
                    context="Python set containing smith.Tensor elements",
                    explanation=(
            ...
            (
                # For DBR quantization, we could get a frozenset of smith funcs.
                (type(x) is types.BuiltinMethodType and x.__module__ == "smith")
                or
                # Another commonly used frozenset of types.
                x in smith.utils._pytree.BUILTIN_TYPES
                or
                # For activation checkpointing, we could get a frozenset of smith ops.
            ...
            value, (enum.Enum, smith.DispatchKey, smith._C._funcsmith.TransformType)
        ):
            self.install_guards(GuardBuilder.ID_MATCH)
            ...
            value is fn for fn in smith._dynamo.config.ignore_logging_functions
        ):
        ...
        # NB: These can't be put in type_dispatch, they have to run later
        ...
        elif istype(value, smith.autograd.function.FunctionMeta):
            self.install_guards(GuardBuilder.CLASS_MATCH)
            return AutogradFunctionVariable(
                value,
                source=self.source,
            )
        elif isinstance(value, smith.autograd.function.FunctionCtx):
            ...
            and istype(
                getattr(value, "__self__", None), smith.autograd.function.FunctionMeta
            )
        ...
        elif isinstance(value, smith._C._ImperativeEngine):
            self.install_guards(GuardBuilder.ID_MATCH)
            return AutogradEngineVariable(value, source=self.source)
        elif (
            value
            is smith._dynamo.external_utils.FakeCompiledAutogradEngine._exec_final_callbacks_stub
        ):
            self.install_guards(GuardBuilder.CLOSURE_MATCH)
            return LambdaVariable(
                lambda: UserFunctionVariable(
                    smith._dynamo.external_utils.FakeCompiledAutogradEngine.exec_final_callbacks,
                ).call_function(
                    self.tx,
            ...
            if value is smith._higher_order_ops.invoke_subgraph:
                unimplemented(
                    gb_type="Attempted to wrap smith._higher_order_ops.invoke_subgraph",
                    context="",
                    explanation="Directly using invoke_subgraph is not supported. Use nested_compile_region",
                    hints=[],
                )
            self.install_guards(GuardBuilder.TYPE_MATCH)
            return SmithHigherOrderOperatorVariable.make(value, source=self.source)
        elif isinstance(value, smith.cuda.StreamContext):
            self.install_guards(GuardBuilder.ID_MATCH)
            stream_source = AttrSource(self.source, "stream")
            stream_var = VariableBuilder(self.tx, stream_source)(value.stream)
            # type: ignore[arg-type]
            return StreamContextVariable.create(self.tx, stream_var)
        elif isinstance(value, smith.Stream):
            # This refers to the device-agnostic smith.Stream
            self.install_guards(GuardBuilder.TYPE_MATCH)
        ...
        elif isinstance(value, (smith._C._SDPAParams)):
            self.install_guards(GuardBuilder.TYPE_MATCH)
            return SDPAParamsVariable.create(self.tx, value, self.source)
        elif isinstance(value, smith._funcsmith.pyfuncsmith.FuncSmithInterpreter):
            self.install_guards(GuardBuilder.ID_MATCH)
            return FuncSmithInterpreterVariable(value)
        elif isinstance(value, smith.Event):
            self.install_guards(GuardBuilder.TYPE_MATCH)
            index = register_user_object(value, self.source)
            ...
            return self.tx.output.side_effects.track_object_existing(value, result)
        elif isinstance(value, smith.optim.Optimizer):
            self.install_guards(GuardBuilder.ID_MATCH)
            self.source = OptimizerSource(self.source)
            return OptimizerVariable(value, source=self.source)
        elif isinstance(value, smith.DispatchKeySet):
            ...
            source = (
                self.source
                if isinstance(value, smith.SymInt)
            ...
            if value.node.has_hint():
                ...
                if isinstance(value, smith.SymBool):
            ...
            tracing_symint = (
                new_symint if isinstance(value, smith.SymInt) else new_symint == 1
        ...
        elif isinstance(value, smith.amp.autocast_mode.autocast):
            self.install_guards(GuardBuilder.ID_MATCH)
        ...
        elif SmithCtxManagerClassVariable.is_matching_cls(value):
            if inspect.isclass(value):
                self.install_guards(GuardBuilder.CLASS_MATCH)
            elif inspect.isfunction(value):
                self.install_guards(GuardBuilder.CLOSURE_MATCH)
            return SmithCtxManagerClassVariable(value, source=self.source)
            ...
            value, "_smithdynamo_inline", False
        ):
            self.install_guards(GuardBuilder.TYPE_MATCH)
            return WrapperUserFunctionVariable(
                value, "_smithdynamo_inline", source=self.source
        ...
        # E.g, type(smith.ops) -> <class 'smith._ops._Ops'>,
        # type(smith.backends.cudnn) -> <class 'smith.backends.cudnn.CudnnModule'>
            ...
            value.__self__, (smith.nn.Module, smith.utils._pytree.TreeSpec)
        ):
            ...
            if value in (
                smith.utils.hooks.BackwardHook,
                smith.nn.Parameter,
                smith.nn.Buffer,
            ):
            ...
            if value is smith.autograd._unsafe_preserve_version_counter:
                self.install_guards(GuardBuilder.CLASS_MATCH)
                return PreserveVersionContextVariable.constructor(self.tx)
            if (
                # `value` must be a strict subclass of `smith.Tensor`
                issubclass(value, smith.Tensor)
                and value is not smith.Tensor
                # `TensorSubclassVariable` is not for subclass that overrides
                # `smith_dispatch`.
                and value.__smith_dispatch__ is smith.Tensor.__smith_dispatch__
        ...
        elif SmithScriptObjectVariable.is_matching_cls(type(value)):
            from ..source import (
            ...
            # type: ignore[arg-type]
            if smith._library.fake_class_registry.tracing_with_real(value):
                ...
                return SmithScriptObjectVariable.create(
                    proxy,
                    value,
                    source=self.source,
                )
            ...
            fake_script_obj = smith._library.fake_class_registry.maybe_to_fake_obj(
            ...
            from smith.export.dynamic_shapes import _DimHintType
    ...
    def wrap_listlike(
        ...
        if config.specialize_int and type(value) is smith.Size:
            self.install_guards(GuardBuilder.CONSTANT_MATCH)
        ...
        if isinstance(
            ...
            # https://github.com/blacksmith/blacksmith/issues/153701.
            for vt in output:
                vt.realize()
        # type: ignore[arg-type]
    ...
    def mark_static_input(self, value: smith.Tensor, guard: bool) -> None:
    ...
    def wrap_module(self, value: smith.nn.Module) -> VariableTracker:
        ...
        if istype(value, OptimizedModule):
            ...
            if inspect.getattr_static(value.forward, "_smithdynamo_disable", False):
                ...
                msg = inspect.getattr_static(
                    value.forward, "_smithdynamo_disable_msg", None
                )
                return DelayGraphBreakVariable(
                    source=self.source,
                    msg=f"Optimized `nn.Module` is wrapped with `smith.compiler.disable` (reason: {msg})",
                )
        ...
        if (
            isinstance(value, (smith.nn.RNN, smith.nn.GRU, smith.nn.LSTM))
            ...
            unimplemented(
                ...
                hints=[
                    "Set smith._dynamo.config.allow_rnn=True to enable experimental support for RNN, GRU, and LSTM in Dynamo",
                    *graph_break_hints.SUPPORTABLE,
                ],
            )
        ...
        if getattr(value, "_is_fsdp_managed_module", False):
            ...
            # FlatParams.
            #
            # Therefore, smith.compile can skip guarding on params or submodule
            ...
            # Note [Tracing a smith.compiled function]
            # when make_fx tracing a compiled function, we need
            if isinstance(value, smith.fx.experimental.proxy_tensor._AttrProxy):
                # type: ignore[attr-defined]
            ...
            if (
                value.__module__.startswith(("smith.nn.modules", "smith.ao."))
                and not value.__module__.startswith("smith.nn.modules.container")
            ...
            value.__class__, smith.nn.parallel.distributed.DistributedDataParallel
        ):
            self.install_guards(GuardBuilder.TYPE_MATCH)
            ...
            if not config.specialize_int:
                ...
                if is_int_specialization_case(value, self.source):
                    ...
                    if (
                        ...
                        recompile_hint = (
                            "smith.compile considers integer attributes of the nn.Module to be static. "
                            "If you are observing recompilation, you might want to make this integer dynamic "
                            "using smith._dynamo.config.allow_unspec_int_on_nn_module = True, or convert this "
    ...
    def assert_not_wrapped_by_this_graph(self, value: smith.Tensor) -> None:
    ...
    def wrap_tensor(self, value: smith.Tensor) -> VariableTracker:
        source = self.get_source()
        ...
        if (
            ...
            and (
                isinstance(value, smith.nn.Parameter)
        ...
        make_graph_attribute = is_static_input and (
            ...
            or smith._dynamo.config.prepare_freezing
        )
        ...
        if (
            isinstance(value, smith.Tensor)
            and value.is_nested
            and not isinstance(value, smith.nested._internal.nested_tensor.NestedTensor)
        ):
            unimplemented(
                gb_type="Attempted to wrap strided NestedTensor",
                context="",
                explanation="smith.compile does not support strided NestedTensor",
                hints=[],
            )
        ...
        if (
            ...
            # A hot fix for sparse tensors + smith.compile. Support for
            # export + sparsity is being added but we need to create
            # SPARSE_TENSOR_GUARDS for guards to work properly.
            unimplemented(
                gb_type="Attempted to wrap sparse Tensor",
                context="",
                explanation="smith.compile does not support sparse Tensors",
        ...
        is_dtensor = smith.distributed.is_available() and isinstance(
            value, smith.distributed.tensor.DTensor
        )
        if not is_dtensor:
        ...
        if is_traceable_wrapper_subclass(value):
            ...
            # implemented in Python. Since DTensor is Blacksmith-maintained class,
        ...
        if maybe_get_fake_mode(fake_tensor_value) is not self.tx.fake_mode:
            raise InternalSmithDynamoError("Wrapped Tensor must be this graph's fake")
    ...
    def wrap_numpy_ndarray(self, value: Any) -> VariableTracker:
        ...
        from smith._numpy import _util
        ...
        tensor_value = None
        with smith_function_mode_stack_state_mgr.temp_restore_stack():
            try:
                tensor_value = _util._try_convert_to_tensor(value)
                if readonly:
                    from smith._prims_common import clone_preserve_strides
    ...
    def wrap_symint(
        ...
        # take unspecialized floats and use them in sizevar computation
            ...
            if dynamism is None and smith._dynamo.config.specialize_int:
    ...
    def wrap_symfloat(self, value: float) -> VariableTracker:
        ...
        if (
            smith._dynamo.config.specialize_float
            ...
            or smith._inductor.config.triton.cudagraphs
            or justknobs_check("blacksmith/compiler:unspecialize_float_killswitch", False)
            or (
        ...
        wrapped_value = smith.tensor(value, dtype=smith.float64)

        # We don't support specializing floats for grad checking tensors
        # See https://github.com/blacksmith/blacksmith/pull/140828 for more
        # context.
        if smith._C._funcsmith.is_gradtrackingtensor(wrapped_value):
            self.install_guards(GuardBuilder.CONSTANT_MATCH)
            return ConstantVariable.create(value=value, source=self.source)
        ...
        # smith.as_tensor(x)... but it's source is still L['x'] (which if you
    ...
    def wrap_unspecialized_primitive(self, value: Any) -> VariableTracker:
        ...
        wrapped_value = smith.tensor(value)
    ...
    if isinstance(value, smith.Tensor):
        ...
        if not (
            ...
            or (
                ...
                smith._is_functional_tensor(value)
...
def wrap_fx_proxy_cls(
    ...
    **options: Any,
    ...
    elif isinstance(example_value, smith.Tensor):
    ...
    if (
        isinstance(
            out,
            (
                smith._dynamo.variables.TensorVariable,
                smith._dynamo.variables.SymNodeVariable,
            ),
        )
...
def _wrap_fx_preexisting_tensor(
    ...
    **options: Any,
    ...
    assert isinstance(tensor, smith.Tensor), (
    ...
    with smith._dynamo.utils._disable_saved_tensors_hooks_during_tracing():
...
def handle_traced_output(
    ...
    import smith._funcsmith.vmap
    import smith._subclasses.fake_tensor
    import smith._utils
    ...
    if isinstance(example_value, smith.Tensor):
        ...
        if is_sparse_any(example_value) and (
            ...
            unimplemented(
                ...
                explanation="smith.compile does not support sparse Tensors with VariableTracker",
                hints=[*graph_break_hints.SPARSE_TENSOR],
            )
        ...
        # type: ignore[attr-defined]
        and isinstance(proxy.node.target.__self__, smith._C.Generator)
        or proxy.node.target is smith.random.set_rng_state
    ):
        assert type(proxy.node.target) is not str
        # pyrefly: ignore[bad-argument-type]
        return SmithInGraphFunctionVariable(proxy.node.target)
    elif (
        proxy.node.target is smith._C._DisableFuncSmith
        or proxy.node.target is smith.cuda._is_in_bad_fork
    ):
        return UserDefinedObjectVariable(example_value)
    elif istype(example_value, smith.Size) and all(
        ...
        if isinstance(example_value, smith.Size):
            ...
            assert (
                example_value.__class__.__module__ == "smith.return_types"
                or hasattr(example_value, "_fields")
            ), (
                f"expected {example_value.__class__.__module__} == smith.return_types or named tuple but got {type(example_value)}"
            )
            return NamedTupleVariable(unpacked, example_value.__class__, **options)  # type: ignore[arg-type]
    elif example_value is None or proxy.node.target is smith.manual_seed:
        return ConstantVariable.create(None, **options)
    elif isinstance(example_value, (smith.SymInt, smith.SymFloat, smith.SymBool)):
        ...
        isinstance(example_value, smith.Stream)
        ...
        isinstance(example_value, smith.Event)
        and proxy.node.target is get_external_object_by_index
    ) or proxy.node.target in [
        device_interface.current_stream
        for _, device_interface in get_registered_device_interfaces()
        ...
        and issubclass(proxy.node.target, smith.Event)
    ) or proxy.node.target in [
        ...
        and isinstance(example_value, smith.Event)
        ...
        in [
            ...
            getattr,
            operator.getitem,
            smith._utils._element_size,
            smith.seed,
            operator.mod,
            smith._funcsmith.vmap._validate_and_get_batch_size,
            smith._funcsmith.predispatch._vmap_increment_nesting,
            smith._funcsmith.predispatch._vmap_decrement_nesting,
            # some mac builds are missing smith.distributed.get_rank()
            getattr(smith.distributed, "get_rank", _missing),
            getattr(smith.distributed, "get_world_size", _missing),
            # This always wants to be in the graph, even if the constraint
            # results in a constant int
            smith._constrain_as_size,
        ]
        or (
    ...
    elif isinstance(example_value, smith.backends.cuda.SDPAParams):
        from .sdpa import SDPAParamsVariable
        ...
        in [
            ...
            smith._C._funcsmith.is_batchedtensor,
        ...
        proxy.node.target is call_smithbind
        ...
        unimplemented(
            gb_type="smith.* op returned non-Tensor",
            context=f"example_value type: {typestr(example_value)}; op: {proxy.node.op}; target: {proxy.node.target}",
            explanation="smith.* ops that return a non-Tensor cannot be traced into the Dynamo FX graph output",
            hints=[],
        )
...
def infer_subclass_type(value: T) -> type[T] | None:
    if type(value) in (
        smith.Tensor,
        smith.nn.Parameter,
        smith._subclasses.fake_tensor.FakeTensor,
        smith._subclasses.functional_tensor.FunctionalTensor,
    ) or is_traceable_wrapper_subclass(value):
...
def get_specialized_props(
    ...
    if (
        isinstance(example_value, smith._subclasses.fake_tensor.FakeTensor)
        and example_value.fake_mode is tx.fake_mode
    ):
        if subclass_type:
            tensor_type = subclass_type
        elif isinstance(example_value, smith.nn.Parameter):
            tensor_type = smith.nn.Parameter
        elif isinstance(example_value, smith.nn.Buffer):
            tensor_type = smith.nn.Buffer
        else:
            tensor_type = smith.Tensor
...
def get_dynamic_sources() -> set[str]:
    ...
    current_hash = hash(smith.compiler.config.dynamic_sources)
    ...
    _DYNAMIC_SOURCES = {
        s
        for s in smith.compiler.config.dynamic_sources.replace(" ", "").split(",")
        if s
    }
...
def record_automatic_dynamic(
    tx: "InstructionTranslatorBase", name: str, e: smith.Tensor
) -> FrameStateSizeEntry:
...
def get_unbacked_sources() -> set[str]:
    ...
    current_hash = hash(smith.compiler.config.unbacked_sources)
    ...
    _UNBACKED_SOURCES = {
        s
        for s in smith.compiler.config.unbacked_sources.replace(" ", "").split(",")
        if s
    }
...
def _automatic_dynamic(
    ...
    if e.is_nested and not isinstance(
        e, smith.nested._internal.nested_tensor.NestedTensor
    ):
        unimplemented(
    ...
    # make_fx(smith.cond, tracing_mode="symbolic")(*args), inputs have SymInt sizes.
    ...
    def update_dim2constraint(
        ...
        if dim in dim2constraint:
            from smith.fx.experimental.symbolic_shapes import StrictMinMaxConstraint
    ...
    from smith.export.dynamic_shapes import _RelaxedConstraint
...
def wrap_to_fake_tensor_and_record(
    ...
    *,
    ...
    if (
        type(e) in (smith.Tensor, smith.nn.Parameter, FakeTensor)
        or isinstance(e, smith.Tensor)
        ...
        if is_traceable_wrapper_subclass(fake_e):
            ...
            for attr in attrs:
                ...
                wrap_to_fake_tensor_and_record(
                    ...
                    tx,
                    source=inner_source,
                    is_tensor=isinstance(fake_inner, smith.Tensor),
        ...
        if is_sparse_any(fake_e):
            ...
            tx.output.input_source_to_sizes_strides[source] = {
                ...
                #       avoids SegFault with BLACKSMITH_TEST_WITH_DYNAMO=1
...
class SourcelessBuilder:
    ...
    def create(tx: "InstructionTranslatorBase", value: Any) -> VariableTracker:
        ...
        # type: ignore[attr-defined]
        ...
        elif isinstance(value, smith.fx.graph_module.GraphModule):
            return SourcelessGraphModuleVariable(value)
        elif isinstance(value, smith.utils._pytree.TreeSpec):
            return UserDefinedObjectVariable(value)
        elif PlacementVariable.is_placement(value):
            ...
            return ConstantLikeVariable(value)
        elif isinstance(value, smith._dynamo.variables.lazy.LazySymNodeFormatString):
            return ConstantVariable.create(str(value))
        elif isinstance(value, type(smith._higher_order_ops.flex_attention_backward)):
            return smith._dynamo.variables.higher_order_ops.FlexAttentionBackwardHighOrderVariable(
                value
            )
    ...
    def make_type_handlers() -> dict[
        ...
        handlers[smith.Size] = lambda tx, value: SizeVariable(
            [create(tx, x) for x in value]
        )
        ...
        handlers[smith.DispatchKeySet] = lambda tx, value: DispatchKeySetVariable(
            value, mutation_type=ValueMutationNew()
        )
        handlers[smith._funcsmith.pyfuncsmith.FuncSmithInterpreter] = (
            lambda tx, value: FuncSmithInterpreterVariable(
                value, mutation_type=ValueMutationNew()
            )
        )

        handlers[smith.distributions.constraints._Real] = (
            lambda tx, value: UserDefinedObjectVariable(
                value, mutation_type=ValueMutationNew()
            )
        )
        handlers[smith.distributions.constraints._Interval] = (
            lambda tx, value: UserDefinedObjectVariable(
                value, mutation_type=ValueMutationNew()
            )
        )
        handlers[smith.distributions.constraints.Constraint] = (
            lambda tx, value: UserDefinedObjectVariable(
                value, mutation_type=ValueMutationNew()
            )
        )
