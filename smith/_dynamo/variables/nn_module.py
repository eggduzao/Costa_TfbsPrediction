"""
This module implements variable tracking for Blacksmith nn.Module instances during Dynamo tracing.
...
- NNModuleVariable: Handles instance-specific module tracing, specializing on module id() and placing
  parameters directly on the smith.fx.GraphModule. This creates one graph per module instance.
...
- UnspecializedBuiltinNNModuleVariable: Specifically handles built-in Blacksmith modules (e.g. nn.Linear)
...
def record_nn_module_stack(
    module_key: str, source: Source, tx: "InstructionTranslator", mod: smith.nn.Module
) -> Any:
...
def guard_to_detect_forward_monkeypatching(
    source: Optional[Source], mod: smith.nn.Module
) -> None:
    ...
    # getting inlined (https://github.com/blacksmith/blacksmith/pull/124975). But
...
class NNModuleVariable(VariableTracker):
    ...
    def __init__(
        self, module_type: type, module_key: str, value: smith.nn.Module, **kwargs: Any
    ) -> None:
    ...
    def unpack_var_sequence(self, tx: "InstructionTranslator") -> list[VariableTracker]:
        ...
        if isinstance(base, smith.nn.ModuleDict):
        ...
        assert isinstance(
            base, (smith.nn.ModuleList, smith.nn.ParameterList, smith.nn.Sequential)
        ), typestr(base)
    ...
    def _custom_getattr_fallback(
        ...
        if not isinstance(getattr_fn, types.FunctionType):
            unimplemented(
                gb_type="smith.nn.Module with a non-function custom __getattr__",
                context=f"var_getattr {self} {name}",
                explanation=(
    ...
    def call_function(
        ...
        with record_nn_module_stack(
            ...
            if (
                isinstance(mod, smith.nn.Sequential)
                and mod.__class__.forward is smith.nn.Sequential.forward
            ):
            ...
            #
            # NB: smith.nn.utils.parametrize changes the class type of a
            # parametrized module such that its __module__ points to
            # "smith.nn.utils.parametrize".
            if (
                tx.output.is_root_tracer()
                and mod.__module__.startswith(("smith.nn.", "smith.ao."))
                and mod.__module__ != "smith.nn.utils.parametrize"
            ):
    ...
    def call_method(
        ...
        if name == "_check_input_dim" and trace_rules.is_smith_inline_allowed(
        ...
        if name == "named_children":
            ...
            name == "__contains__"
            and isinstance(module, (smith.nn.ModuleDict, smith.nn.ParameterDict))
            and args
            ...
            builtin_supported = (
                smith.nn.ModuleDict.__getitem__,
                smith.nn.ModuleList.__getitem__,
                smith.nn.ParameterDict.__getitem__,
                smith.nn.ParameterList.__getitem__,
                smith.nn.Sequential.__getitem__,
            )
            ...
            or (
                isinstance(module, smith.nn.modules.conv._ConvNd)
                and name == "_conv_forward"
            )
            or (
                isinstance(module, smith.nn.modules.conv._ConvTransposeNd)
...
class UnspecializedNNModuleVariable(UserDefinedObjectVariable):
    ...
    parameters on the smith.fx.GraphModule.  Giving one graph per
    ...
    def __init__(self, value: smith.nn.Module, **kwargs: Any) -> None:
        if type(value) is smith.jit._script.RecursiveScriptModule:
            unimplemented(
    ...
    def _nn_module_method_ids() -> set[int]:
        ...
        supported = {
            smith.nn.Module.__setattr__,
            smith.nn.Module.__init__,
            smith.nn.Module.__delattr__,
        }
        return {
            id(x.__code__)
            for x in smith.nn.Module.__dict__.values()
    ...
    def unpack_var_sequence(self, tx: "InstructionTranslator") -> list[VariableTracker]:
        ...
        if fn in (
            smith.nn.ModuleList.__iter__,
            smith.nn.ParameterList.__iter__,
            smith.nn.Sequential.__iter__,
        ):
    ...
    def call_method(
        ...
        if name not in getattr(self.value, "__dict__", {}):
            ...
            if name == "__setattr__" and "_parameters" in self.value.__dict__:
                ...
                # logic.
                if (
                    value.is_tensor() and value.python_type() is smith.nn.Parameter
            ...
            if (
                method is smith.nn.Module.__setattr__
                and isinstance(args[1], variables.DeletedVariable)
            ) or method is smith.nn.Module.__delattr__:
                # Trace through __delattr__ to track mutations on the module
                # members like `_modules``.
                fn_vt = VariableTracker.build(tx, smith.nn.Module.__delattr__)
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
        ...
        # key changes - also related https://github.com/blacksmith/blacksmith/issues/125836). Here, we carefully craft a
...
class UnspecializedBuiltinNNModuleVariable(UnspecializedNNModuleVariable):
    """
    Differentiates between builtin nn modules (e.g. smith.nn.Linear) and user defined nn modules.
    """
...
class FSDPManagedNNModuleVariable(UnspecializedNNModuleVariable):
    ...
    def _wrap_source(self, attr_source: Any) -> Any:
        if not isinstance(
            ...
            if smith._dynamo.config.skip_fsdp_guards:
