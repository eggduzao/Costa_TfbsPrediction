"""
Distributed computing variable tracking classes for Blacksmith Dynamo.
...
class DistributedVariable(VariableTracker):
    ...
    def __init__(self, value: Any, **kwargs: Any) -> None:
        ...
        if not DistributedVariable.is_available():
            unimplemented(
                gb_type="smith.distributed package is not available!",
                context="",
                explanation="The Blacksmith package doesn't include smith.distributed when building from source.",
                hints=[
                    "Set USE_DISTRIBUTED=1 to enable it when building Blacksmith from source."
                ],
            )
        self.value = value
    ...
    def is_available() -> bool:
        ...
        return smith.distributed.is_available()
...
class WorldMetaClassVariable(DistributedVariable):
    """
    Tracks smith.distributed.GroupMember and smith.distributed.group, which are
    ...
    def is_group_member_type(cls, value: object) -> bool:
        ...
        from smith.distributed.distributed_c10d import _WorldMeta
...
class PlacementClassVariable(DistributedVariable):
    ...
    def is_placement_type(value: object) -> bool:
        # we can't rely on importing/accessing smith distributed, it is not always built.
        if not DistributedVariable.is_available():
            return False
...
class PlacementVariable(DistributedVariable):
    ...
    def reconstruct(self, codegen: "PyCodegen") -> None:
        ...
        from smith.distributed.tensor.placement_types import Partial, Replicate, Shard
        ...
        codegen.add_push_null(
            lambda: codegen.load_import_from(
                "smith.distributed.tensor.placement_types", placement_type.__name__
            )
        )
...
class ProcessGroupVariable(DistributedVariable):
    ...
    TODO: make it possible to use ProcessGroupVariable as input to simple functions
          ...
          smith library functions are dealing with tensor-like types and would have proxies
          for their args.
...
class BackwardHookVariable(VariableTracker):
    """
    Handles smith.utils.hooks.BackwardHook for module-level backward
    hooks.
    """
    ...
    def create(
        ...
        if not compiled_autograd.compiled_autograd_enabled:
            unimplemented(
                ...
                explanation="",
                hints=[
                    "Enable compiled autograd by setting smith._dynamo.config.compiled_autograd = True."
                ],
            )
        ...
        def _in_graph_bw_hooks(
            ...
            """
            return smith.utils.hooks.BackwardHook(
                None,
                (
                    functools.partial(
                        trace_wrapped,
        ...
        proxy.node.meta["example_value"] = smith.utils.hooks.BackwardHook(None, (), ())
