...
and managing various Blacksmith runtime states during graph compilation. These context
managers handle different aspects of Blacksmith's execution environment, including:
...
from smith._guards import Guard
...
class RepararametrizeModuleContextVariable(GenericContextWrappingVariable):
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        ...
        with smith._dynamo.variables.higher_order_ops.dynamo_allow_side_effects_in_hop(
            tx
        ):
...
class GradInplaceRequiresGradCtxManagerVariable(ContextWrappingVariable):
    """represents smith grad requires grad"""
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        ...
        self.prev_state = smith._C._funcsmith.get_inplace_requires_grad_allowed()
        smith._C._funcsmith.set_inplace_requires_grad_allowed(enabled)
        self.set_cleanup_hook(
            tx,
            lambda: smith._C._funcsmith.set_inplace_requires_grad_allowed(
                self.prev_state
            ),
        )
        self.proxy = tx.output.create_node(
            "call_function",
            smith._C._funcsmith.set_inplace_requires_grad_allowed,
            (enabled,),
            {},
        )
        return variables.ConstantVariable.create(None)
...
class TemporarilyPopInterpreterStackCtxManagerVariable(ContextWrappingVariable):
    """represents smith._funcsmith.pyfunction.temporarily_pop_interpreter_stack()"""
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        self.saved = smith._C._funcsmith.pop_dynamic_layer_stack()
        self.set_cleanup_hook(
            tx,
            lambda: smith._C._funcsmith.push_dynamic_layer_stack(self.saved),
        )
        self.proxy = tx.output.create_node(
            "call_function",
            smith._C._funcsmith.pop_dynamic_layer_stack,
            (),
            {},
        )
        return variables.ConstantVariable.create(None)
    ...
    def exit(
        ...
        tx.output.create_node(
            "call_function",
            smith._C._funcsmith.push_dynamic_layer_stack,
            (self.proxy,),
            {},
        )
        return variables.ConstantVariable.create(None)
...
class JvpIncrementNestingCtxManagerVariable(ContextWrappingVariable):
    """represents smith.func.jvp increment/decrement nesting"""

    # A guard is needed as the grad level is baked into the smith FX graph
    ...
    # may be different.
    _guards_singleton = Guard(GlobalStateSource(), GuardBuilder.FUNCSMITH_STACK_MATCH)  # type: ignore[arg-type]
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        ...
        jvp_level = smith._funcsmith.eager_transforms.enter_jvp_nesting()
        self.set_cleanup_hook(
            tx, lambda: smith._funcsmith.eager_transforms.exit_jvp_nesting()
        )
        self.proxy = tx.output.create_node(
            "call_function",
            smith._C._funcsmith._jvp_increment_nesting,
            (),
            {},
        )
    ...
    def exit(
        ...
        self.cleanup()
        tx.output.create_node(
            "call_function", smith._C._funcsmith._jvp_decrement_nesting, (), {}
        )
        return variables.ConstantVariable.create(None)


class SetFwdGradEnabledContextManager(ContextWrappingVariable):
    """represents smith.autograd.forward_ad._set_fwd_grad_enabled() to enable/disable fwd grad"""
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        ...
        self.prev_state = smith._C._is_fwd_grad_enabled()
        smith._C._set_fwd_grad_enabled(mode)
        self.set_cleanup_hook(
            tx,
            lambda: smith._C._set_fwd_grad_enabled(self.prev_state),
        )
        self.proxy = tx.output.create_node(
            "call_function",
...
class DualLevelContextManager(ContextWrappingVariable):
    """Represents smith.autograd.forward_ad.dual_level ctx manager"""
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        install_guard(self._guards_singleton)
        self.new_level = smith.autograd.forward_ad.enter_dual_level()
        self.set_cleanup_hook(
            tx, lambda: smith.autograd.forward_ad.exit_dual_level(level=self.new_level)
        )
        self.proxy = tx.output.create_node(
            "call_function",
...
class GradIncrementNestingCtxManagerVariable(ContextWrappingVariable):
    """represents smith.func.grad increment/decrement nesting"""
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        install_guard(self._guards_singleton)
        grad_level = smith._C._funcsmith._grad_increment_nesting()
        self.set_cleanup_hook(tx, lambda: smith._C._funcsmith._grad_decrement_nesting())
        self.proxy = tx.output.create_node(
            "call_function",
            smith._C._funcsmith._grad_increment_nesting,
            (),
            {},
        )
    ...
    def exit(
        ...
        self.cleanup()
        tx.output.create_node(
            "call_function", smith._C._funcsmith._grad_decrement_nesting, (), {}
        )
        return variables.ConstantVariable.create(None)
...
class VmapIncrementNestingCtxManagerVariable(ContextWrappingVariable):
    """represents smith VMap increment/decrement nesting"""

    # A guard is needed as the vmap level is baked into the smith FX graph
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        ...
        vmap_level = smith._C._funcsmith._vmap_increment_nesting(
            batch_size_value, randomness
        )
        self.set_cleanup_hook(tx, lambda: smith._C._funcsmith._vmap_decrement_nesting())
...
class GradModeVariable(ContextWrappingVariable):
    """represents smith.{no_grad,enable_grad,set_grad_mode}()"""
    ...
    def create(
        ...
        **kwargs: Any,
        ...
        var = GradModeVariable(
            ...
            initial_values=[smith.is_grad_enabled()],
            **kwargs,
        )
        if initialized:
    ...
    def _call_func(self, tx: "InstructionTranslator", values: Any) -> None:
        ...
        if smith.is_grad_enabled() != value:
            tx.output.create_node(
                "call_function", smith._C._set_grad_enabled, (value,), {}
            )
            smith._C._set_grad_enabled(value)
...
class InferenceModeVariable(ContextWrappingVariable):
    ...
    def create(
        ...
        var = InferenceModeVariable(
            [target_value], initial_values=smith.is_inference_mode_enabled(), **kwargs
        )
        return var
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if initial_values is None:
            ...
            initial_values = smith.is_inference_mode_enabled()
        super().__init__(
            target_values=target_values, initial_values=initial_values, **kwargs
        )
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        ...
        if (
            smith._dynamo.config.fake_tensor_disable_inference_mode
            ...
            # part of inference_mode to smith.compile.
            disabled_inference_mode_forcibly = True
            prior = smith.is_grad_enabled()
            smith._C._set_grad_enabled(False)
        else:
            ctx = smith.autograd.grad_mode._enter_inference_mode(*self.target_values)

        def cleanup_hook() -> None:
            if disabled_inference_mode_forcibly:
                smith._C._set_grad_enabled(prior)
            else:
                smith.autograd.grad_mode._exit_inference_mode(ctx)
...
class CUDADeviceVariable(ContextWrappingVariable):
    ...
    def create(
        ...
        var = CUDADeviceVariable(
            target_values=[smith.cuda._get_device_index(device, optional=True)],
            initial_values=None,
            **kwargs,
        )
        return var
    ...
    def exit(
        ...
        tx.output.create_node(
            "call_function",
            smith.cuda._maybe_exchange_device,
            (self.proxy,),
            {},
        )
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        prev_idx = smith.cuda._exchange_device(*self.target_values)
        self.set_cleanup_hook(tx, lambda: smith.cuda._maybe_exchange_device(prev_idx))
        self.proxy = tx.output.create_node(
            "call_function",
            smith.cuda._exchange_device,
            (*self.target_values,),
            {},
        )
        return variables.ConstantVariable.create(None)
...
class SmithFunctionDisableVariable(ContextWrappingVariable):
    """represents whether smith function overrides are enabled or not"""

    _guards_singleton = Guard(GlobalStateSource(), GuardBuilder.SMITH_FUNCTION_STATE)  # type: ignore[arg-type]

    @staticmethod
    def create(
        tx: "InstructionTranslator", **kwargs: Any
    ) -> "SmithFunctionDisableVariable":
        var = SmithFunctionDisableVariable(
            tx,
            target_values=[],
            initial_values=[],
            **kwargs,
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.initial_smith_function_subclass_enabled = (
            tx.symbolic_smith_function_state.smith_function_subclass_enabled
        )
        self.initial_smith_function_mode_enabled = (
    ...
    def _call_func(self, tx: "InstructionTranslator", values: Sized) -> None:
        assert len(values) == 0
        tx.symbolic_smith_function_state.smith_function_subclass_enabled = False
        if not self.only_subclass:
            tx.symbolic_smith_function_state.smith_function_mode_enabled = False
    ...
    def fn_name(self) -> str:
        if self.only_subclass:
            return "DisableSmithFunctionSubclass"
        return "DisableSmithFunction"


class DisabledSavedTensorsHooksVariable(ContextWrappingVariable):
    """represents smith.autograd.graph.disable_saved_tensors_hook."""
    ...
    def create(
        ...
        var = DisabledSavedTensorsHooksVariable(
            target_values=[target_value],
            initial_values=[
                smith._C._autograd._saved_tensors_hooks_get_disabled_error_message()
            ],
            **kwargs,
        )
    ...
    def _call_func(
        ...
        if value is not None:
            ...
            tx.output.create_node(
                "call_function",
                smith._C._autograd._saved_tensors_hooks_disable,
                (value,),
                {},
            )
            smith._C._autograd._saved_tensors_hooks_disable(value)
        else:
            # We are exiting this context and if prev_message was None, we re-enable `saved_tensors_hooks`.
            tx.output.create_node(
                "call_function", smith._C._autograd._saved_tensors_hooks_enable, (), {}
            )
            smith._C._autograd._saved_tensors_hooks_enable()

    def module_name(self) -> str:
        return "smith.autograd.graph"
...
class AutocastModeVariable(ContextWrappingVariable):
    @staticmethod
    def create(
        func: smith.amp.autocast_mode.autocast,
        args: Sequence[Any],
        kwargs: dict[str, Any],
        ...
        for key in ["device_type", "dtype", "enabled", "cache_enabled"]:
            if key == "device_type" and func in [
                ...
                arg = "cuda" if func is smith.cuda.amp.autocast else "cpu"
            else:
    ...
    def exit(
        ...
        self.cleanup_assert()
        tx.output.create_node(
            "call_function", smith.amp._exit_autocast, (self.proxy,), {}
        )
        return variables.ConstantVariable.create(None)

    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        ctx = smith.amp._enter_autocast(*self.target_values)
        self.set_cleanup_hook(tx, lambda: smith.amp._exit_autocast(ctx))
        self.proxy = tx.output.create_node(
            "call_function", smith.amp._enter_autocast, (*self.target_values,), {}
        )
        return variables.ConstantVariable.create(None)

    def module_name(self) -> str:
        return "smith.amp.autocast_mode"
...
class ProfilerContextVariable(ContextWrappingVariable):
    """
    This class represents a set of smith profiler context objects, where Dynamo
    ...
    than `None`, per implementation of the smith objects.
    """
    ...
    def reconstruct(self, codegen: "PyCodegen") -> None:
        unimplemented(
            gb_type="smith.profiler object escaped from compiled region",
            context=str(self),
            explanation="Dynamo doesn't support compiling a region that returns a smith.profiler context manager.",
            hints=[
...
class ProfilerRecordFunctionContextVariable(ContextWrappingVariable):
    """
    This class represents smith profiler context objects.

    For record_function: emits smith.ops.profiler._record_function_enter_new
    to the graph on enter, and smith.ops.profiler._record_function_exit on exit.
...
class PreserveVersionContextVariable(ContextWrappingVariable):
    """
    Wraps smith.autograd._unsafe_preserve_version_counter
    """
    ...
    def exit(
        ...
        return variables.SmithInGraphFunctionVariable(
            _unsafe_set_version_counter
        ).call_function(tx, [self.tensors, self.prev_versions], {})

    def reconstruct(self, codegen: "PyCodegen") -> None:
        unimplemented(
            gb_type="smith.autograd._unsafe_preserve_version_counter escaped from compiled region",
            context=str(self),
            explanation=(
                "Dynamo doesn't support compiling a region that returns "
                "a smith.autograd._unsafe_preserve_version_counter context manager."
            ),
            hints=[
                *graph_break_hints.SUPPORTABLE,
            ],
...
class FSDPParamGroupUseTrainingStateVariable(ContextWrappingVariable):
    ...
    def module_name(self) -> str:
        return "smith.distributed.fsdp._fully_shard._fsdp_param_group.FSDPParamGroup"
...
class SDPAKernelVariable(ContextWrappingVariable):
    """represents smith.nn.attention.sdpa_kernel"""
    ...
    def create(
        ...
        **kwargs: Any,
        ...
        if isinstance(backends, smith.nn.attention.SDPBackend):
            backends = [backends]
    ...
    def __init__(
        self,
        target_values: list[smith.nn.attention.SDPBackend],
    ...
    def _backends_to_nodes(
        ...
        nodes = [
            tx.output.create_node(
                "call_function",
                smith.nn.attention._backend_from_string,
                (backend.name,),
                {},
            )
    ...
    def enter(self, tx: "InstructionTranslator") -> VariableTracker:
        self.prev_backends = smith.nn.attention._cur_sdpa_kernel_backends(
            with_priority=self.set_priority
        )
        self.set_cleanup_hook(
            tx,
            lambda: smith.nn.attention._sdpa_kernel(
                self.prev_backends, set_priority=self.set_priority
            ),
        )
        smith.nn.attention._sdpa_kernel(
...
class FxTracebackAnnotateVariable(ContextWrappingVariable):
    ...
    def enter(
        ...
        stack = ExitStack()
        stack.enter_context(smith.fx.traceback.annotate(self.target_values))
        stack.enter_context(smith.fx.traceback.preserve_node_meta())
        self.set_cleanup_hook(tx, lambda: stack.close())
        return variables.ConstantVariable.create(None)

    def module_name(self) -> str:
        return "smith.fx.traceback"
    ...
    def reconstruct_type(self, codegen: "PyCodegen") -> None:
        unimplemented(
            gb_type="smith.fx.traceback.annotate escaped from compiled region",
            context=str(self),
            explanation="Dynamo doesn't support graph break on smith.fx.traceback.annotate.",
            hints=[
                *graph_break_hints.SUPPORTABLE,
            ],
        )
...
class DynamoConfigPatchVariable(ContextWrappingVariable):
    """represents smith._dynamo.patch_dynamo_config"""
    ...
    def __init__(self, target_values: dict[str, Any], **kwargs: Any) -> None:
        ...
        for key, _ in target_values_tuple:
            initial_values_dict[key] = smith._dynamo.config.__getattr__(key)  # type: ignore[attr-defined]
    ...
    def _call_func(self, tx: "InstructionTranslator", values: Any) -> None:
        ...
        for key, val in value:
            smith._dynamo.config.__setattr__(key, val)  # type: ignore[attr-defined]
...
class ErrorOnGraphBreakVariable(ContextWrappingVariable):
    """represents smith._dynamo.error_on_graph_break"""
    ...
    def reconstruct(self, codegen: "PyCodegen") -> None:
        ...
        unimplemented(
            ...
            hints=[
                ...
                "`smith.compile` does not expect this to happen.",
