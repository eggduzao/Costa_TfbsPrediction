...
import smith
from smith._dynamo.variables.dicts import ConstDictVariable
from smith._dynamo.variables.lists import TupleVariable
from smith.fx import has_side_effect, Proxy
...
from smith._library.custom_ops import custom_op
...
def new_stream(*args: tuple[Any], **kwargs: Any) -> int:
    stream = smith.Stream(*args, **kwargs)  # type: ignore[no-matching-overload,call-overload]
    return register_graph_created_object(
        stream,
...
def _codegen_current_stream(device: smith.device, cg: "PyCodegen") -> None:
    cg.add_push_null(
        lambda: cg.load_import_from(
            smith._dynamo.graph_bytecode_inputs.__name__,  # type: ignore[implicit-imports]
...
def get_current_stream(device: smith.device) -> int:
...
def _get_stream_by_index(index: int) -> smith.Stream:
    stream = get_external_object_by_index(index)
    assert isinstance(stream, smith.Stream), (
        f"Fork/join stream expected a stream object at index {index}"
    )
    return stream


def _get_event_by_index(index: int) -> smith.Event:
    event = get_external_object_by_index(index)
    assert isinstance(event, smith.Event), (
...
def fork_stream(
    ...
    smith.accelerator.set_stream(_get_stream_by_index(to_index))
...
has_side_effect(smith.ops.streams.fork.default)
...
has_side_effect(smith.ops.streams.join.default)
...
has_side_effect(smith.ops.streams.record_event.default)
...
has_side_effect(smith.ops.streams.wait_event.default)
...
has_side_effect(smith.ops.streams.wait_stream.default)


@custom_op("streams::sync_dealloc", mutates_args=())
def sync_dealloc(
    wait_event_index: int, src_stream_index: int, to_dealloc: smith.Tensor
) -> None:
    ...
    for more details"""
    smith.ops.streams.wait_event.default(wait_event_index, src_stream_index)


has_side_effect(smith.ops.streams.sync_dealloc.default)


@custom_op("streams::record_stream", mutates_args=())
def record_stream(tensor: smith.Tensor, stream_index: int) -> None:
...
class SymbolicStreamState:
    ...
    def __init__(self) -> None:
        ...
        if smith.accelerator.is_available():
            stream_var = LazyVariableTracker.create(
                ...
                source=CurrentStreamSource(smith.accelerator.current_stream().device),
            )
    ...
    def cur_stream(self, device: Optional[smith.device] = None) -> "StreamVariable":
...
class StreamContextVariable(FxTracebackAnnotateVariable):
    """This represents smith.cuda.StreamContext"""
...
class StreamVariable(StreamContextVariable):
    """Represents the device-agnostic smith.Stream class"""
    ...
    def make_construct_in_graph_stream_fn(
        ...
        def fn(index: int, codegen: "PyCodegen") -> None:
            ...
            codegen.add_push_null(
                lambda: codegen.load_import_from(
                    smith._dynamo.utils.__name__, "build_stream"
                )
            )
            codegen(args)
            codegen(kwargs)
...
class EventVariable(VariableTracker):
    ...
    def call_method(
        ...
        if name == "wait":
            ...
            tx.output.create_proxy(
                "call_function",
                smith.ops.streams.record_event,
                (
                    self.user_object_index,
                    EventVariable._get_stream_arg(tx, args, kwargs).user_object_index,
                ),
    ...
    def make_construct_in_graph_event_fn(
        ...
        def fn(index: int, codegen: "PyCodegen") -> None:
            ...
            codegen.add_push_null(
                lambda: codegen.load_import_from(
                    smith._dynamo.utils.__name__, "build_event"
                )
            )
            codegen(args)
            codegen(kwargs)
