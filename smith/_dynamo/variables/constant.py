...
import smith
from smith._dynamo.source import AttrSource, GetItemSource
...
This failure likely due to Blacksmith-internal use of `ConstantVariable` on
...
"""
    ...
    def is_literal(obj: object) -> bool:
        if type(obj) in (list, tuple, set, frozenset, smith.Size):
    ...
    def call_method(
        ...
        if isinstance(self.value, str) and name in str.__dict__:
            ...
            if (
                ...
                if isinstance(
                    add_target, (smith.SymBool, smith.SymFloat, smith.SymInt)
                ):
    ...
    def call_tree_map(
        ...
        if self.value is None:
            ...
            if none_is_leaf_var is not None:
                ...
                # smith.utils._pytree and smith.utils._cxx_pytree treat None as a leaf
            ...
            if none_is_leaf:
                ...
                return self.clone()
        if isinstance(self.value, (int, float, bool, complex, str, bytes, smith.dtype)):
            return map_fn.call_function(tx, [self, *rest], {})
