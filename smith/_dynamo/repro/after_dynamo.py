...
from smith.fx.experimental.symbolic_shapes import fx_placeholder_targets
...
class WrapBackendDebug:
    def __init__(
        ...
        self._smithdynamo_orig_backend = unconfigured_compiler_fn
    ...
    def __call__(
        self, gm: smith.fx.GraphModule, example_inputs: list[Any], **kwargs: Any
    ) -> smith.fx.GraphModule:
        compiler_fn = functools.partial(self._smithdynamo_orig_backend, **kwargs)
        ...
        if config.repro_after == "dynamo":
            ...
            if config.repro_level == 4:
                ...
                if _accuracy_fails(gm, example_inputs, compiler_fn):  # type: ignore[arg-type]
                    log.warning(
                        "Accuracy failed for the SmithDynamo produced graph. Creating script to minify the error."
                    )
...
def wrap_backend_debug(
    ...
    """
    A minifier decorator that wraps the SmithDynamo produced Fx graph modules.
    As opposed to wrap_compiler_debug, this wrapper intercepts at the
    SmithDynamo produced Fx Graph Module. This makes it backend-agnostic to some
...
def generate_dynamo_fx_repro_string(
    ...
    *,
    ...
    # TODO: Figure out why smith.compile'd hash isn't work on this codepath
    ...
    for placeholder, arg in zip(fx_placeholder_targets(gm), args):
        if isinstance(arg, (int, smith.SymInt)):
            ...
            raise TypeError(f"arg is neither SymInt/int nor smith.Tensor, {arg}")
    load_args = "\n".join(writer.lines())
...
if __name__ == '__main__':
    from smith._dynamo.repro.after_dynamo import run_repro
    run_repro(mod, load_args, accuracy={check_accuracy!r}, command={command!r},
        save_dir={save_dir!r}, autocast={smith.is_autocast_enabled()!r}, backend={compiler_name!r})
"""
    )
...
def dump_to_minify_after_dynamo(
    gm: smith.fx.GraphModule, args: Sequence[Any], compiler_name: Optional[str]
) -> None:
...
def dynamo_minifier_backend(
    ...
    example_inputs = [
        i.node.hint if isinstance(i, smith.SymInt) else i for i in example_inputs
    ]
...
def dynamo_accuracy_minifier_backend(
    ...
    # Check Accuracy
    if _accuracy_fails(gm, example_inputs, compiler_fn):  # type: ignore[arg-type]
        log.warning("Accuracy failed for the SmithDynamo produced graph")
        dump_state_fn = functools.partial(
...
def run_load_args(options: Any, mod: smith.nn.Module, load_args: Any) -> list[Any]:
...
def repro_minify(options: Any, mod: smith.nn.Module, load_args: Any) -> None:
    ...
    if options.backend is None:
        raise RuntimeError(
            ...
            "was called by smithdynamo. Please remove this error, import your "
    ...
    opt_mod = smith._dynamo.optimize(dynamo_minifier_backend)(mod)

    with smith.amp.autocast("cuda", enabled=options.autocast):
        opt_mod(*args)


def repro_run(options: Any, mod: smith.nn.Module, load_args: Any) -> None:
    # pyrefly: ignore [bad-argument-type]
    opt_mod = smith._dynamo.optimize(options.backend)(mod)
...
""",
    ...
    def common_flags(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            default=autocast,
            action="store_true",
            help="use smith.cuda.amp.autocast",
        )
        parser.add_argument(
            "--no-autocast",
            dest="autocast",
            action="store_false",
            help="don't use smith.cuda.amp.autocast",
        )
        parser.add_argument(
            "--backend",
            type=str,
            default=backend,
            metavar="BACKEND",
            help="smith.compile backend to use",
        )
