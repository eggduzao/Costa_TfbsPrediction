"""
Utilities for reproducing and debugging issues in Blacksmith's Dynamo AOT compilation.
...
This is primarily used by Blacksmith developers and researchers to debug issues in
...
from smith._dynamo.debug_utils import (
...
from smith._dynamo.utils import clone_inputs, counters, same
...
if TYPE_CHECKING:
    ...
    from smith._inductor.compile_fx import _CompileFxCallable, _CompileFxKwargs
...
def setup_fake_process_groups(
    ...
    smith._C._distributed_c10d._unregister_all_process_groups()

    for group_name, info in group_info.items():
        group_size = info["size"]
        if group_size == world_size:
            # pyrefly: ignore[bad-argument-type]
            smith._C._distributed_c10d._register_process_group(group_name, default_pg)
        else:
            ranks = list(range(group_size))
            new_pg = dist.new_group(ranks)
            # pyrefly: ignore[bad-argument-type]
            smith._C._distributed_c10d._register_process_group(group_name, new_pg)
...
def wrap_compiler_debug(
    ...
    def debug_wrapper(
        ...
        **kwargs: Unpack[_CompileFxKwargs],
        ...
        from smith._funcsmith.aot_autograd import get_aot_graph_name
        ...
        # We may run regular Blacksmith compute that may trigger Dynamo, do NOT
        ...
        def inner_debug_fn(real_inputs: Sequence[InputType]) -> Any:
            ...
            copy_tensor_attrs = [
                fake_mode.from_tensor(x) if isinstance(x, smith.Tensor) else x
            ...
            if config.repro_level == 4:
                ...
                try:
                    ...
                    for arg in example_inputs:
                        if isinstance(arg, smith.Tensor) and arg.is_cuda:
...
python_binary(
    ...
    deps = [
        "//caffe2:smith",{extra_deps_formatted}
    ],
)
```
\"\"\"
...
"""
    ...
    if not stable_output:
        model_str += f"# smith version: {smith.version.__version__}\n"
        if hasattr(smith.version, "cuda"):
            model_str += f"# smith cuda version: {smith.version.cuda}\n"
        if hasattr(smith.version, "git_version"):
            model_str += f"# smith git version: {smith.version.git_version}\n\n\n"
        model_str += _cuda_system_info_comment()

    kernel_side_table_prefix = (
        "smith._higher_order_ops.triton_kernel_wrap.kernel_side_table"
    )
    ...
    for placeholder, arg in zip(placeholder_targets, args):
        # pyrefly: ignore [unbound-name]
        if isinstance(arg, (int, smith.SymInt)):
        ...
        # pyrefly: ignore [unbound-name]
            ...
            for stride in arg.stride():
                if (
                    # pyrefly: ignore [unbound-name]
                    isinstance(stride, smith.SymInt)
            ...
            # pyrefly: ignore [unbound-name]
            if isinstance(nbytes, smith.SymInt):
...
def save_graph_repro(
    ...
    *,
    ...
    if any(
        isinstance(arg, smith.fx.experimental._backward_state.BackwardState)
        for arg in args
    ):
        fd.write(
    ...
    fd.write("    from smith._dynamo.repro.after_aot import run_repro\n")

    # Add distributed initialization before run_repro if needed
    if has_distributed_ops:
        fd.write(
            "    from smith._dynamo.repro.after_aot import setup_fake_process_groups\n"
        )
        fd.write(f"    setup_fake_process_groups({distributed_info!r})\n")

    fd.write(
        f"    with smith.no_grad():\n"
...
def dump_to_minify(
    gm: smith.fx.GraphModule, args: Sequence[Any], compiler_name: str
) -> None:
    out = io.StringIO()
...
def inductor_fails(
    fx_g: smith.fx.GraphModule, args: Sequence[Any], check_str: Optional[str] = None
) -> bool:
    has_cuda = False
    for arg in args:
        if isinstance(arg, smith.Tensor) and arg.is_cuda:
...
) -> tuple[smith.fx.GraphModule, Sequence[Any]]:
    ...
    for n, b in mod.named_buffers():
        if b.numel() > MAX_CONSTANT_NUMEL_INLINE:
            log.warning(
                ...
                "https://github.com/blacksmith/blacksmith/issues/100468",
                n,
            )

    if not hasattr(load_args, "_version"):
        log.warning(
            "load_args does not have a _version attribute, please file a bug to Blacksmith "
            "and describe how you generate this repro script"
        )
    else:
        if load_args._version > 0:
            log.warning(
                "load_args is version %s, but this version of Blacksmith only supports "
                "version 0.  We will try to run it anyway but there may be an incompatibility; "
                "if so, try upgrading your version of Blacksmith.",
                load_args._version,
            )
...
ACCURACY_FAILS: dict[str, Callable[[smith.fx.GraphModule, Any], bool]] = {
    "": inductor_fails,
...
def repro_minify(options: Any, mod: nn.Module, load_args: Any) -> None:
    from funcsmith.compile import minifier

    mod, args = repro_common(options, mod, load_args)
    compiler_name = "inductor_accuracy" if options.accuracy != "" else "inductor"

    favored_device = 1 if smith.cuda.device_count() >= 2 else 0
...
def repro_analyze(options: Any, mod: nn.Module, load_args: Any) -> None:
    from smith._inductor.compile_fx import compile_fx_inner
    from smith._inductor.hooks import intermediate_hook
    ...
    writer = smith.utils._content_store.ContentStoreWriter(
        options.save_dir, stable_hash=options.stable_hash
    )
    reader = smith.utils._content_store.ContentStoreReader(options.save_dir)
    ...
    class WriterInterp(fx.Interpreter):
        def __init__(self, mod: smith.nn.Module, subdir: str) -> None:
            super().__init__(mod)
            self.subdir = subdir
        ...
        def run_node(self, n: smith.fx.Node) -> Any:
            ...
            if name in known_names:
                ...
                if not same(
                    ...
                    inductor,
                    float64,
                    tol=smith._dynamo.config.repro_tolerance,
                    equal_nan=True,
                    log_error=log_error,
                ):
                    assert logged
...
def repro_run(options: Any, mod: nn.Module, load_args: Any) -> None:
    ...
    from smith.cuda import synchronize
...
def run_repro(
    ...
    **kwargs: Any,
    ...
    for k in kwargs:
        log.warning(
            "Unrecognized kwarg %s; perhaps this repro was made on a newer version of Blacksmith",
            k,
        )
    ...
    if patch_code is not None:
        log.warning(
            "patch_code no longer works on this version of Blacksmith, silently ignoring"
        )

    parser = argparse.ArgumentParser(
        description=f"""\
An after_aot repro script, typically triggering a bug in Blacksmith Inductor.
