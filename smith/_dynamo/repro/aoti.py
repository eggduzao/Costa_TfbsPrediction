...
class AOTIMinifierError(Exception):
    def __init__(self, original_exception: Union[str, Exception]) -> None:
        additional_message = "This error is caused by a bug in the AOTI minifier, please report a bug to Blacksmith"
...
def get_module_string(gm: smith.fx.GraphModule) -> str:
...
def save_graph_repro_ep(
    ...
    *,
    ...
    gm: Optional[smith.nn.Module] = None,
    ...
    if exported_program is None:
        ...
        exported_program = smith.export.export(gm, args, strict=strict)
    elif gm is None:
    ...
    fd.write("    from smith._dynamo.repro.aoti import run_repro\n")
    fd.write(
...
{extra_imports}
    ...
    smith.export.save(exported_program, ep_path)

    model_str += f"exported_program = smith.export.load('{ep_path}')\n"
...
) -> tuple[smith.fx.GraphModule, Any, Any]:
...
def repro_run(
    ...
    from smith._inductor import _aoti_compile_and_package_inner
...
def export_for_aoti_minifier(
    ...
    try:
        ep = smith.export.export(gm, tuple_inputs, strict=strict)
...
def run_repro(
    ...
    **more_kwargs: Any,
    ...
    parser = argparse.ArgumentParser(
        description=f"""\
An AOTI repro script, typically triggering a bug in Blacksmith AOTInductor.
