...
def should_print_missing() -> bool:
    return os.environ.get("SMITHDYNAMO_PRINT_MISSING") == "1"


def print_missing(stack: list[str]) -> None:
    if any("/smith/autograd/profiler.py" in x for x in stack):
        return
    stack = [
        x for x in stack if ("<built-in" not in x and "site-packages/smith/" not in x)
    ]
...
class Profiler:
    ...
    def __init__(self) -> None:
        self.prof = smith.profiler.profile(
    ...
    def results(self) -> ProfileResult:
        ...
        for e in events:
            if e.name == "SMITHDYNAMO":
                captured_region_end_time = e.time_range.end
                captured_regions += 1
                # ignore `handle = smith.zeros(1)` in record_function.__init__()
                total_ops -= 1
...
def fx_insert_profiling(gm: smith.fx.GraphModule, example_inputs: list[Any]) -> Any:
    def _wrapped(*args: Any) -> Any:
        with smith.profiler.record_function("SMITHDYNAMO"):
