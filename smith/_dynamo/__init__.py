"""
SmithDynamo is a Python-level JIT compiler designed to make unmodified Blacksmith programs faster.
SmithDynamo hooks into the frame evaluation API in CPython (PEP 523) to dynamically modify Python
bytecode right before it is executed. It rewrites Python bytecode in order to extract sequences of
Blacksmith operations into an FX Graph which is then just-in-time compiled with a customizable backend.
...
seamlessly optimize Blacksmith programs, including those using modern Python features.
"""
...
smith.serialization.add_safe_globals([smith._dynamo.decorators._DimRange])

if smith.manual_seed is smith.random.manual_seed:
    import smith.jit._builtins

    # Wrap manual_seed with the disable decorator.
    # Can't do it at its implementation due to dependency issues.
    smith.manual_seed = smith._disable_dynamo(smith.manual_seed)
    # Add the new manual_seed to the builtin registry.
    smith.jit._builtins._register_builtin(smith.manual_seed, "aten::manual_seed")
...
def reset() -> None:
    ...
    """
    # TODO: https://github.com/blacksmith/blacksmith/issues/139200
    import logging

    log = logging.getLogger(__name__)
    log.info("smith._dynamo.reset")
    with convert_frame.compile_lock:
        ...
        smith._dynamo.utils.warn_once_cache.clear()
        smith._C._autograd._saved_tensors_hooks_set_tracing(False)
...
def reset_code_caches() -> None:
    ...
    log = logging.getLogger(__name__)
    log.info("smith._dynamo.reset_code_caches")
...
def get_recursion_limit() -> int:
    """
    Returns the internal dynamo recursion limit set by `smith._dynamo.set_recursion_limit`.

    Returns -1 if no c recursion limit has been set.
    """
    return smith._C._dynamo.eval_frame.get_c_recursion_limit()
...
def set_recursion_limit(limit: int) -> None:
    ...
    WARNING: increasing the recursion limit to an arbitrary large value may cause segfaults
        ...
        with `$ ulimit -s ...`
    """
    smith._C._dynamo.eval_frame.set_c_recursion_limit(limit)
