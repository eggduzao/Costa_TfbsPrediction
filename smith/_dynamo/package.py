...
for smith.compile. We mainly have two abstractions here:
  - CompilePackage: Overarching data structure for store and lookup a list of compiled codes.
  - CodeCacheEntry: Data structure for a single code being compiled by smith.compile.
...
from smith._dynamo.graph_utils import _graph_device_type
...
def load_guards_state(guards_state: bytes) -> Any:
    try:
        import smith.distributed.fsdp._fully_shard._fully_shard as _fully_shard
...
def load_guard_manager(
    ...
    return smith._dynamo.guards.CheckFunctionManager(
        target_code,
...
class SystemInfo:
    """
    System information including Python, Blacksmith, and GPU details.
    ...
    def current(cls) -> "SystemInfo":
        ...
        gpu_name = None
        from smith.utils._triton import get_triton_version

        gpu_name, toolkit_version = None, None
        for device_type in cls.CHECK_GPUS:
            if getattr(smith, device_type).is_available():
                try:
                    gpu_name = getattr(smith, device_type).get_device_name()
                    toolkit_version = getattr(smith.version, device_type)
                    break
                except Exception:
                    pass
    ...
    def check_compatibility(
        ...
        if self.smith_version != other.smith_version:
            raise RuntimeError(
                f"Compile package was created with a different Blacksmith version: {self.smith_version}"
            )
        if device_type in self.CHECK_GPUS:
            if not getattr(smith, device_type).is_available():
...
class PrecompileCacheEntry:
    """
    A full cache entry for caching precompile, for a toplevel smith.compile.
...
def _compile_frame_context(
    ...
    from smith._dynamo.convert_frame import get_compile_id, log_dynamo_start
    from smith._guards import compile_context, CompileContext
    ...
    def _ctx() -> Iterator[None]:
        ...
        with (
            ...
            dynamo_timed(
                ...
                metadata={
                    "frame_key": str(smith._dynamo.utils.curr_frame),
...
class CompilePackage:
    ...
    def update_device_type(self, graph: Optional[smith.fx.Graph]) -> None:
    ...
    def uninstall(self) -> None:
        from smith._C._dynamo.eval_frame import _reset_precompile_entries
        ...
        for code, entry in self._codes.items():
            ...
            with context:
                ...
                for guarded_code in entry.guarded_codes:
                    ...
                    assert isinstance(guards_state, smith._dynamo.guards.GuardsState)
...
class DynamoStore(abc.ABC):
    ...
    def record_eager_backend(self, backend_id: _BackendId, backend: Any) -> None:
        ...
        """
        from smith._dynamo.precompile_context import (
            EagerCacheArtifact,
            PrecompileContext,
        )
