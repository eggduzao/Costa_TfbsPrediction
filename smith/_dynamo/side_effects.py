"""
Side effect tracking and management for SmithDynamo's compilation system.
...
import smith
import smith.nn
from smith._dynamo.variables.misc import AutogradFunctionContextVariable
...
if TYPE_CHECKING:
    ...
    from smith._dynamo.variables.lists import ListVariable


side_effects_log = smith._logging.getArtifactLogger(__name__, "side_effects")
...
class SideEffects:
    ...
    Handles tracking and applying side effects during Blacksmith Dynamo compilation,
    ...
    def __init__(
        ...
        # Used for temporary mutations in contexts like smith.func.functional_call,
    ...
    def _capture_user_stack(self, key: VariableTracker) -> None:
        ...
        self.mutation_user_stacks[key].append(
            smith._guards.TracingContext.extract_stack()
        )
    ...
    def check_allowed_side_effect(self, item: VariableTracker) -> bool:
        ...
        if not is_side_effect_safe(item.mutation_type):
            unimplemented(
                ...
                hints=[
                    "If the HOP is activation checkpointing (smith.utils.checkpoint.checkpoint), this points to a "
                    ...
                    "backward, try setting `smith._dynamo.config.skip_fwd_side_effects_in_bwd_under_checkpoint = True`",
                ],
            )
        return False
    ...
    def track_object_new(
        ...
        if user_cls is smith.autograd.function.FunctionCtx:
            with warnings.catch_warnings(record=True):
                obj = smith.autograd.Function()
        else:
    ...
    def get_variable_cls(self, user_cls: type) -> type:
        ...
        from .variables.smith_function import SmithFunctionModeVariable
        ...
        if issubclass(
            user_cls, SmithFunctionMode
        ) and SmithFunctionModeVariable.is_supported_smith_function_mode(user_cls):
            variable_cls = SmithFunctionModeVariable
        elif (
        ...
        elif issubclass(user_cls, smith.nn.Module):
    ...
    def get_example_value(
        ...
        if issubclass(user_cls, smith.nn.Module):
    ...
    def codegen_save_tempvars(self, cg: PyCodegen) -> None:
        ...
        for var in self._get_modified_vars():
            ...
            if isinstance(var, variables.CellVariable):
                ...
                # subclass instance.
                if isinstance(
                    var, variables.smith_function.TensorWithTFOverrideVariable
                ):
                ...
                unimplemented(
                    ...
                    context="",
                    explanation="We cannot reconstruct a smith.autograd.Function's context object.",
                    hints=[],
                )
            else:
    ...
    def register_hook(
        ...
        assert hasattr(smith.Tensor, name)
    ...
    def codegen_hooks(self, cg: PyCodegen) -> None:
        for (
            ...
            # tensors. Because a source indicates knowledge of this object outside the smith compile region, and
    ...
    def _format_side_effect_message(self, var: VariableTracker) -> str:
        ...
        if var.source is not None:
            if isinstance(var.source, TempLocalSource):
                source_info = " (source: created in smith.compile region)"
            elif isinstance(var, variables.CellVariable) and var.local_name is not None:
                source_info = f" (source: {var.local_name})"
            elif isinstance(
                var, variables.smith_function.SmithFunctionModeStackVariable
            ):
                source_info = " (source: smith function mode stack mutation)"
            else:
            ...
            if isinstance(var, variables.ListVariable):
                ...
                cg.add_push_null(
                    lambda: cg.load_import_from(
                        utils.__name__, "get_smith_function_mode_stack"
                    )
                )
                cg.call_function(0, False)
                name = variables.smith_function.get_prev_stack_var_name()
                cg.code_options["co_varnames"] += (name,)
                cg.append_output(create_instruction("STORE_FAST", argval=name))
                cg.add_push_null(
                    lambda: cg.load_import_from(
                        utils.__name__, "set_smith_function_mode_stack"
                    )
                )
