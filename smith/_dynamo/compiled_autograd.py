"""
Provides functionality for compiling Blacksmith's autograd (automatic differentiation) system.
...
from smith._dynamo.external_utils import (
...
from smith._dynamo.source import GetItemSource, LocalSource
...
from smith._funcsmith._aot_autograd.runtime_wrappers import (
    AutogradLazyBackwardCompileInfo,
    CachedAutogradLazyBackwardCompileInfo,
)
from smith._guards import compile_context, CompileContext, CompileId, Source
from smith._logging import getArtifactLogger, trace_structured
...
from smith.fx.experimental.symbolic_shapes import DimDynamic, ShapeEnv
from smith.fx.traceback import preserve_node_meta, set_stack_trace
from smith.types import FloatLikeType, IntLikeType
...
1. Moving the unsupported autograd call outside of the smith.compile'd region.
2. Wrapping the unsupported autograd call in the smith._dynamo.compiled_autograd._disable() context manager.
3. Setting smith._dynamo.config.compiled_autograd=False for the smith.compile call containing the unsupported autograd call.
4. Setting smith._dynamo.config.compiled_autograd=False at the start of the program."""
...
def snapshot_verbose_logging_enabled() -> bool:
    return smith._logging._internal.log_state.is_artifact_enabled(
        "compiled_autograd_verbose"
    )


def snapshot_cudagraph_enabled() -> bool:
    return smith._inductor.config.triton.cudagraphs


def maybe_clone(x: Optional[smith.Tensor]) -> Optional[smith.Tensor]:
    if x is not None:
...
#
# With smith.compile, following eager semantics would require inserting
...
class NaNChecker:
    def __init__(self, accumulate_grad: bool) -> None:
        ...
        self.params_to_check: dict[str, smith.Tensor] = {}
        self.output_names: list[str] = []

    def prep_with_graph(self, graph: smith.fx.Graph) -> None:
    ...
    def prep_with_inputs(self, inputs: tuple[smith.Tensor, ...]) -> None:
        ...
        for idx in self.params_indices:
            ...
            if grad is not None:
                assert not smith.isnan(grad).any(), (
    ...
    def check(self, out: tuple[smith.Tensor, ...]) -> None:
...
# We lazily bind "functional backward" variants for Blacksmith built-in autograd
# nodes to this class. Example: smith._dynamo.compiled_autograd.ops.MulBackward0
...
class Op:
    def __init__(
        ...
        self.__name__ = name
        self.__module__ = "smith._dynamo.compiled_autograd.ops"
...
class AutogradCompilerInstance:
    def __init__(self, compiler_fn: Callable[..., Any]) -> None:
        ...
        self.proxy_mode = ProxySmithDispatchMode(self.fx_tracer, "symbolic")
        self.hooks_proxy: Optional[Proxy] = None

    def wrap_fake(self, x: smith.Tensor, source: Optional[Source]) -> FakeTensor:
    ...
    def begin_capture(
        ...
        check_nans: bool,
    ) -> tuple[str, list[smith.Tensor], list[IntLikeType], list[FloatLikeType]]:
        ...
        self.fx_tracer.root = smith.nn.Module()
        self.fx_tracer.graph = smith.fx.Graph(tracer_cls=PythonKeyTracer)
        ...
        self.stack.enter_context(
            smith.fx.experimental.symbolic_shapes._suppress_guards(env)
        )
    ...
    def proxy_call_aot_backward(
        ...
        psaved_tensors: Sequence[smith.Tensor],
        saved_tensors: Sequence[smith.Tensor],
        pctx: Any,
        ctx: Any,
        ...
        if smith.is_grad_enabled():
            for output_alias_info in metadata.output_info:
                if output_alias_info.requires_grad:
                    raise RuntimeError(
                        "smith.compile does not currently support higher order gradients."
                    )

        @smith._dynamo.allow_in_graph  # type: ignore[misc]
        def call_aot_bwd_prologue(
            ctx_saved_tensors: Sequence[smith.Tensor],
            ctx_symints: Sequence[IntLikeType],
            ctx_opaque_objs: Sequence[Any],
            *flat_args: Sequence[Any],
        ) -> Any:
            out = smith._funcsmith._aot_autograd.runtime_wrappers._backward_prologue_functional(
                ctx_saved_tensors,
                ctx_symints,
                ctx_opaque_objs,
                metadata,
        ...
        def copy_paste_aot_backward_graph() -> list[smith.Tensor]:
            def num_inputs(graph: smith.fx.Graph) -> int:
                num_args = 0
            ...
            args_idx = 0
            value_remap = {}
            poutputs: Optional[list[smith.fx.Proxy]] = None
            ...
            for node in bw_module.graph.nodes:  # type: ignore[attr-defined]
                if node.op == "placeholder":
                    ...
                    poutputs = [
                        smith.fx.Proxy(value_remap[n], self.fx_tracer)
                    ...
                    if node.target is smith.ops.aten.view.default:
                        ...
                        node.target = smith.ops.aten.reshape.default
            ...
            def dummy() -> smith.Tensor:
                with disable_proxy_modes_tracing():
                    return smith.zeros(0, 0, 0, 0, 123)

            outputs = [
                dummy() if isinstance(o, smith.fx.Proxy) else o for o in poutputs
            ]
        ...
        results = smith._funcsmith._aot_autograd.runtime_wrappers._backward_epilogue_functional(
            metadata,
            maybe_subclass_metadata,
            outputs,
    ...
    def proxy_call_backward(
        ...
        saved_tensors: Sequence[smith.Tensor],
        backward_idx: int,
        ctx: smith.autograd.function.BackwardCFunction,
        maybe_backward_state_idx: Optional[int],
        opaque_object_indices: list[int],
    ) -> tuple[Optional[smith.Tensor], ...]:
        ...
        with disable_proxy_modes_tracing():
            # create fake Tensors
            grad_ins: list[Optional[smith.Tensor]] = []
            for idx, output_metadata in enumerate(output_metadatas):
                ...
                grad_ins.append(
                    smith.empty(size=size, dtype=dtype, layout=layout, device=device)
                )
    ...
    def allocate_dummy(self) -> smith.Tensor:
        with disable_proxy_modes_tracing():
            # Weird quantity so it's easy to grep
            return smith.zeros([0, 123456789])
    ...
    def accumulate(self, old_var: Any, new_var: Any) -> smith.Tensor:
        old_var_proxy = self.to_proxy(old_var)
        new_var_proxy = self.to_proxy(new_var)
        proxy_out = self.fx_tracer.create_proxy(
            "call_function", smith.add, args=(old_var_proxy, new_var_proxy), kwargs={}
        )
    ...
    def accumulate_grad(
        self, variable: smith.Tensor, grad: smith.Tensor, has_post_hooks: bool
    ) -> None:
    ...
    def unpack_hook(self, hook_id: int, data_id: int) -> smith.Tensor:
        assert self.hooks_proxy is not None
    ...
    def tensor_pre_hook(
        self, inputs: list[smith.Tensor], hook_id: int, i: int
    ) -> list[smith.Tensor]:
        assert self.hooks_proxy is not None
        hook = self.hooks_proxy[hook_id]  # type: ignore[index]
        proxy = self.proxy_call_hook(
    ...
    def cpp_tensor_pre_hook(
        ...
        proxy = self.fx_tracer.create_proxy(
            "call_function",
            smith._C._dynamo.compiled_autograd.call_cpp_tensor_pre_hooks,
    ...
    def pre_hook(self, inputs: Sequence[Any], hook_id: int) -> list[smith.Tensor]:
        assert self.hooks_proxy is not None
        hook = self.hooks_proxy[hook_id]  # type: ignore[index]
    ...
    def post_hook(
        self, outputs: list[smith.Tensor], inputs: Sequence[smith.Tensor], hook_id: int
    ) -> list[smith.Tensor]:
        assert self.hooks_proxy is not None
        hook = self.hooks_proxy[hook_id]  # type: ignore[index]
        proxies = self.proxy_call_hook(
    ...
    def post_acc_grad_hook(
        self, input: smith.Tensor, hook_id: int
    ) -> list[smith.Tensor]:
        assert isinstance(input, smith.Tensor)
        assert self.hooks_proxy is not None
        hook = self.hooks_proxy[hook_id]  # type: ignore[index]
        proxy = self.proxy_call_hook(
            hook,
    ...
    def move_graph_nodes_to_cuda(self, graph: smith.fx.Graph) -> list[int]:
        to_move: dict[int, smith.fx.Node] = {}
        ...
        for i, node in enumerate(inputs_users):
            ...
            if is_cpu and is_scalar:
                ...
                if all(
                    (
                        isinstance(user.target, smith._ops.OpOverload)
    ...
    def is_sym_node(self, node: Any) -> bool:
        return (
            ...
            and node.target
            in [smith.ops.aten.sym_size.int, smith.ops.aten.sym_numel.default]
        )
    ...
    def dce(self) -> None:
        ...
        unpack_nodes: OrderedSet[smith.fx.Node] = OrderedSet()
        i: int | None = None
        ...
        def is_impure(node: smith.fx.Node) -> bool:
    ...
    def end_capture(self, outputs: Any) -> tuple[Callable[..., Any], Any]:
        ...
        # ```
        # sym_numel_default = smith.ops.aten.sym_numel.default(sum_109);  sum_109 = None
        # eq_115 = 16 == sym_numel_default;  sym_numel_default = eq_115 = None
        # sym_size_int_39 = smith.ops.aten.sym_size.int(getitem_112, 1);  getitem_112 = None
            ...
            try:
                ...
                for idx, integer in enumerate(sizes):
                    if idx in used_sizes_idx:
                        ...
                        if integer > 0:
                            filtered_sizes.append(smith.empty(0, integer))
                            smith._dynamo.maybe_mark_dynamic(filtered_sizes[-1], 1)
                        else:
    ...
    @staticmethod
    def get_all_nodes(args: Sequence[Any]) -> list[smith.fx.Node]:
        # filter out non-Node args, like None
        nodes = [n for n in args if type(n) is smith.fx.Node]
        return nodes

    @staticmethod
    def is_placeholder(node: smith.fx.Node) -> bool:
    ...
    def to_proxy(self, t: Any) -> Any:
        ...
        if isinstance(t, (smith.SymInt, smith.SymFloat)):
        ...
        assert isinstance(proxy_tensor, smith.fx.experimental.proxy_tensor._ProxyTensor)
    ...
    def bind_objects_to_proxies(
        ...
        if isinstance(proxies, smith.fx.Proxy):
            if origins:
    ...
    def set_node_origin(
        ...
        node_name: str,
        nodecall_index: int,
        pyobj: Optional[smith.autograd.Function],
    ) -> None:
        maybe_aot_id = ""
        if pyobj is not None:
            ...
            if hasattr(forward_cls, "_aot_id"):
                ...
                if forward_cls._lazy_backward_info is None:
                    ...
                    compiled autograd. Please turn off AOTAutogradCache using `SMITHINDUCTOR_AUTOGRAD_CACHE=0`."""
                    )
...
def _enable(
    ...
    # It is recommended to enable via `smith._dynamo.config.compiled_autograd = True` rather
    # than using this context manager directly. If you are smith.compiling the corresponding
    ...
    #
    # Inputs:
    # - compiler_fn: The wrapper that will consume the compiled autograd graph, e.g. `smith.compile`
    ...
    if not ignore_active_disable_ctx and active_disable_ctx:
        ...
        from smith._dynamo import eval_frame
        ...
        if eval_frame._stance.stance == "force_eager":
            ...
            if smith.cuda.is_available():
                from smith._inductor import cudagraph_trees  # noqa: F401

            (
                prior_compiler,
                prior_dynamic,
            ) = smith._C._dynamo.compiled_autograd.set_autograd_compiler(
                functools.partial(AutogradCompilerInstance, compiler_fn), dynamic
            )
            if snapshot_verbose_logging_enabled():
                smith._C._dynamo.compiled_autograd.set_verbose_logger(verbose_log)  # type:ignore[arg-type]
...
def reset() -> None:
    ...
    smith._C._dynamo.compiled_autograd.set_autograd_compiler(None, False)
    smith._C._dynamo.compiled_autograd.set_verbose_logger(None)
    smith._C._dynamo.compiled_autograd.clear_cache()
...
def copy_slices_prologue(
    ...
    return [result, grad_slice, grad_slice.clone(memory_format=smith.contiguous_format)]
...
def copy_slices_epilogue(
    needs_input_grad: Sequence[bool],
    result: smith.Tensor,
    res: Sequence[Optional[smith.Tensor]],
    grad_slice: smith.Tensor,
) -> list[Optional[smith.Tensor]]:
    grad_inputs: list[Optional[smith.Tensor]] = [None] * len(needs_input_grad)
