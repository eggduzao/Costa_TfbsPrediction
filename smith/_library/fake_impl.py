...
import smith
from smith._library.utils import Kernel, RegistrationHandle
...
class FakeImplHolder:
    ...
    def register(
        ...
        if not allow_override:
            ...
            if smith._C._dispatch_has_kernel_for_dispatch_key(self.qualname, "Meta"):
                raise RuntimeError(
                    f"register_fake(...): the operator {self.qualname} "
    ...
    def meta_kernel(*args, **kwargs):
        ...
        def error_on_ctx():
            raise RuntimeError(
                ...
                f"smith.library.get_ctx(). Otherwise, please use FakeTensors."
            )
...
class FakeImplCtx:
    ...
    def create_unbacked_symint(self, *, min=2, max=None) -> smith.SymInt:
        return self.new_dynamic_size(min=min, max=max)

    def new_dynamic_size(self, *, min=0, max=None) -> smith.SymInt:
        ...
        for smith.compile) for a CustomOp where an output Tensor has a size
        ...
        .. warning:
            ...
            smith.compile. The default value of ``min`` is 2 due to smith.compile
        ...
        Example::
            ...
            >>> lib = smith.library.Library("mymodule", "FRAGMENT")
            >>> lib.define("mymodule::custom_nonzero(Tensor x) -> Tensor")
            >>>
            >>> @smith.library.register_fake("mymodule::custom_nonzero")
            >>> def _(x):
            ...
            >>>     return result
            >>>
            >>> @smith.library.impl(lib, "custom_nonzero", "CPU")
            >>> def _(x):
            >>>     x_np = x.numpy()
        ...
        if (
            ...
            raise smith._subclasses.fake_tensor.DynamicOutputShapeException(self._op)

        if isinstance(min, smith.SymInt) or isinstance(max, smith.SymInt):
            raise ValueError(
...
def allocate_size(shape_env, min_val=0, max_val=None):
    ...
    smith.fx.experimental.symbolic_shapes._constrain_range_for_size(
