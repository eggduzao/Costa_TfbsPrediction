...
The distinction determines how smith.compile handles the object.

REFERENCE TYPES (default):

Reference-typed opaque objects represent mutable stateful objects and are
treated as black boxes. In smith.compile, since smith.compile cannot optimize
...
In smith.compile, the graph specializes on the object like how other constants
...
implemented before registering it as a value-typed opaque object class:
  - __eq__: smith.compile will create guards based on the equality of this
...
import smith
from smith._opaque_base import OpaqueBase, OpaqueBaseMeta  # noqa: F401
...
OpaqueTypeStr = "__smith__.smith.classes.aten.OpaqueObject"

OpaqueType = NewType("OpaqueType", smith._C.ScriptObject)
    ...
    *,
    ...
    Args:
        ...
        members (dict[str, MemberType] | None): Dictionary mapping member names
            ...
            how they are handled during smith.compile tracing:
    ...
    # Prevent registration of built-in types (int, str, list, dict, etc.) and smith.Tensor
    if cls.__module__ == "builtins" or cls is smith.Tensor:
        raise ValueError(
    ...
    if not isinstance(cls, OpaqueBaseMeta):
        raise TypeError(
            f"Opaque type {cls} must subclass smith._opaque_base.OpaqueBase "
            "or 'metaclass=smith._opaque_base.OpaqueBaseMeta'. "
    ...
    if typ == "value":
        if cls.__eq__ is object.__eq__:  # type: ignore[comparison-overlap]
            raise TypeError(
                ...
                "implementation as we will use this in smith.compile "
    ...
    smith._C._register_opaque_type(name)
...
def is_opaque_type(cls: Any) -> bool:
    ...
    if isinstance(cls, str):
        return smith._C._is_opaque_type_registered(cls)

    if cls not in _OPAQUE_TYPES:
        return False

    return smith._C._is_opaque_type_registered(_OPAQUE_TYPES[cls].class_name)
