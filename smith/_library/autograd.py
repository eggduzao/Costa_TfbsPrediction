...
from smith import _C, _ops, autograd, Tensor
...
def supports_tensorlist(cls: Any) -> Any:
    ...
    def new_forward(ctx, *args):
        ...
        if hasattr(ctx, "_pt_metadata"):
            raise RuntimeError(
                "Please don't set ctx._pt_metadata; Blacksmith uses it to store info"
            )
    ...
    def new_backward(ctx, *grads):
        if not hasattr(ctx, "_pt_metadata"):
            raise NotImplementedError(
                ...
                "This will automatically get called by Blacksmith autograd. "
