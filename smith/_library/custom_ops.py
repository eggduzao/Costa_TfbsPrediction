...
import smith
from smith import _C, _ops, Tensor
from smith.types import _dtype
from smith.utils._exposed_in import exposed_in
...
def custom_op(
    ...
    *,
    ...
    - Wrapping a third-party library or custom kernel to work with Blacksmith
    subsystems like Autograd.
    - Preventing smith.compile/export/FX tracing from peeking inside your function.
    ...
    with Blacksmith's various subsystems.

    Args:
        name (str): A name for the custom op that looks like "{namespace}::{name}",
            e.g. "mylib::my_linear". The name is used as the op's stable identifier
            in Blacksmith subsystems (e.g. smith.export, FX graphs).
            To avoid name collisions, please use your project name as the namespace;
            e.g. all custom ops in blacksmith/fbgemm use "fbgemm" as the namespace.
        ...
        device_types (str | None | Sequence[str]): The device type(s) the function
            ...
            we require the operator to have a "device: smith.device argument".
    ...
    .. note::
        ...
        `here <https://github.com/blacksmith/blacksmith/blob/main/aten/src/ATen/native/README.md#func>`_
    ...
    Examples::
        ...
        >>> @smith.library.custom_op("mylib::bar", mutates_args={}, device_types="cpu")
        >>> def bar(device: smith.device) -> Tensor:
        >>>     return smith.ones(3)
        >>>
        >>> bar("cpu")
    ...
    def inner(fn: Callable[..., object]) -> CustomOpDef:
        ...
        if schema is None:
            schema_str = smith.library.infer_schema(fn, mutates_args=mutates_args)
        else:
            schema_str = schema
...
class CustomOpDef:
    ...
    :func:`smith.library.custom_op` API.
    """
    ...
    def __init__(
        ...
        # Fields used to interface with the Blacksmith dispatcher
        ...
        self._smith_dispatch_fns: dict[type, Callable] = {}
    ...
    def register_fake(self, fn: Callable, /) -> Callable:
        ...
        This is necessary to get the operator to work efficiently with smith.compile.
        ...
        Please see :func:`smith.library.register_fake` for more details.
        ...
        Examples:
            ...
            >>> @smith.library.custom_op("mylib::linear", mutates_args=())
            ...
            >>> out = smith.compile(linear, fullgraph=True)(x, weight, bias)
            >>> # xdoctest: +SKIP("Requires Python <= 3.11")
            >>> assert smith.allclose(out, smith.nn.functional.linear(x, weight, bias))
            >>>
            >>> # Example 2: an operator with data-dependent output shape
            >>> @smith.library.custom_op("mylib::nonzero", mutates_args=())
            ...
            >>>     return result
            >>>
            >>> x = smith.tensor([0, 1, 2, 0, 0, 1])
            >>> # xdoctest: +SKIP("Requires Python <= 3.11")
            >>> out = smith.compile(nonzero, fullgraph=True)(x)
            >>> # xdoctest: +SKIP("Requires Python <= 3.11")
            >>> assert smith.allclose(out, x.nonzero())
    ...
    def register_smith_dispatch(
        self, smith_dispatch_class: Any, fn: Optional[Callable] = None, /
    ) -> Callable:
        ...
        Please see :func:`smith.library.register_smith_dispatch` for examples and more details.
        """

        def register(fn):
            if smith_dispatch_class not in self._smith_dispatch_fns:

                def inner(*args, **kwargs):
                    return self._smith_dispatch_fns[smith_dispatch_class](
                        *args, **kwargs
                    )

                self._lib._register_smith_dispatch_rule(
                    self._name, smith_dispatch_class, inner
                )
            self._smith_dispatch_fns[smith_dispatch_class] = fn
            return fn
    ...
    def _register_to_dispatcher(self, tags: Sequence[_C.Tag]) -> None:
        ...
        def fake_impl(*args, **kwargs):
            if self._abstract_fn is None:
                ...
                raise RuntimeError(
                    ...
                    f"This is necessary for smith.compile/export/fx tracing to work. "
        ...
        if schema.is_mutable:
            ...
            original_kernel = smith._C._dispatch_get_computed_kernel_for_dispatch_key(
    ...
    def register_vmap(
        ...
        def register(func):
            ...
            if need_register:
                ...
                self._lib.impl(
                    self._name, wrapped_func, "FuncSmithBatched", with_keyset=True
                )
...
# TODO: Merge this function with smith.amp.autocast_mode._cast, and refactor it
...
def increment_version(val: Any) -> None:
    if isinstance(val, Tensor):
        smith.autograd.graph.increment_version(val)
    elif isinstance(val, (tuple, list)):
        for v in val:
            if isinstance(v, Tensor):
                smith.autograd.graph.increment_version(v)
...
OPDEF_TO_LIB: dict[str, "smith.library.Library"] = {}
...
def get_library_allowing_overwrite(
    ...
    lib = smith.library.Library(namespace, "FRAGMENT")  # noqa: TOR901
