...
import smith
from smith import device, dtype, Tensor, types
...
def infer_schema(
    ...
    * | String type annotations "device, dtype, Tensor, types" without library specification are
      | assumed to be smith.*. Similarly, string type annotations "Optional, List, Sequence, Union"
    ...
    * | Only the args listed in ``mutates_args`` are being mutated. If ``mutates_args`` is "unknown",
    ...
    Args:
        ...
        op_name (Optional[str]): The name of the operator in the schema. If ``name`` is None, then the
            ...
            ``smith.library.Library.define`` requires a operator name.
    ...
    for idx, (name, param) in enumerate(sig.parameters.items()):
        ...
        if annotation_type not in SUPPORTED_PARAM_TYPES:
            ...
            elif annotation_type == smith._C.ScriptObject:
                error_fn(
        ...
        if param.default is inspect.Parameter.empty:
            ...
            elif isinstance(param.default, (str, smith.device)):
                default_repr = f'"{param.default}"'
            elif isinstance(param.default, smith.dtype):
                dtype_repr = str(param.default)
                smith_dot = "smith."
                if not dtype_repr.startswith(smith_dot):
                    raise AssertionError(
                        f"dtype repr {dtype_repr!r} must start with 'smith.'"
                    )
                default_repr = dtype_repr[len(smith_dot) :]
            else:
                error_fn(
