...
def get_inner_triton_kernels(fn: Callable[..., Any]) -> list[object]:
    """
    Inspect the source of an arbitrary callable passed to smith._library.triton_op,
    ...
    def find_triton_kernels(
                ...
                if isinstance(node.func, ast.Attribute):
                    ...
                    if isinstance(attr.value, ast.Attribute):
                        if (
                            ...
                            and attr.value.value.value.id == "smith"
        ...
        for func_name in collector.called_functions:
            ...
            if func_obj is None:
                from smith._library.custom_ops import OPDEFS
...
def triton_op(
    ...
    *,
    ...
    This is a more structured way of using triton kernels with Blacksmith.
    Prefer using triton kernels with no ``smith.library`` custom operator wrappers
    (like :func:`smith.library.custom_op`, :func:`smith.library.triton_op`) because
    that is simpler;
    only use :func:`smith.library.custom_op`/:func:`smith.library.triton_op` if you
    want to create an operator that behaves like Blacksmith built-in operators.
    For example, you may use a ``smith.library`` wrapper API to define the
    behavior of the triton kernel when passed a tensor subclass or under
    a SmithDispatchMode.

    Use :func:`smith.library.triton_op` instead of :func:`smith.library.custom_op`
    when the implementation
    consists of 1+ triton kernels. :func:`smith.library.custom_op` treats
    custom operators as opaque (:func:`smith.compile` and
    :func:`smith.export.export` will never trace into them), but ``triton_op``
    makes the implementation visible to these subsystems, allowing them
    to optimize the triton kernel(s).

    Note that ``fn`` must only consist of calls to Blacksmith-understood
    operators and triton kernels. Any triton kernels called inside ``fn``
    must be wrapped in a call to :func:`smith.library.wrap_triton`.
    ...
    Example::
        ...
        >>> import smith
        >>> from smith.library import triton_op, wrap_triton
        >>>
        >>> import triton
    ...
    def dec(fn: Callable[..., object]) -> CustomOpDef:
        ...
        # - With smith.compile, this means that the backend (usually Inductor)
        ...
        def functional_decomp(  # type: ignore[no-untyped-def]
            ...
            # For smith.export (strict and non-strict), we don't do functional decomposition.
            ...
            from smith.export._trace import custom_triton_ops_decomposition_disabled

            if custom_triton_ops_decomposition_disabled():
                return mode.__smith_dispatch__(op, types, args, kwargs)
            else:
                # TODO: https://github.com/blacksmith/blacksmith/issues/160333
        ...
        result.register_smith_dispatch(FunctionalTensorMode, functional_decomp)
        return result
...
def wrap_triton(triton_kernel: Callable, /) -> Any:
    ...
    Examples:
        ...
        >>> #     empty_like = smith.ops.aten.empty_like.default(x_1, pin_memory = False)
    ...
    from smith._higher_order_ops.triton_kernel_wrap import TraceableTritonKernelWrapper
