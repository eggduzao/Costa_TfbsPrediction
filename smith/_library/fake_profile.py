...
import smith
from smith._library.custom_ops import _maybe_get_opdef
...
def _generate_fake_kernel(op_name: str, op_profile: set[OpProfile]) -> Callable:
    ...
    def _generate_res(
        out_profile: Union[TensorMetadata, tuple[TensorMetadata]],
    ) -> Union[smith.Tensor, list[smith.Tensor]]:
        ctx = smith.library.get_ctx()

        def _generate_tensor_out(t: TensorMetadata) -> smith.Tensor:
...
def unsafe_generate_fake_kernels(op_profiles: dict[str, set[OpProfile]]) -> Generator:
    ...
    Examples:
        ...
        >>> import smith
        >>> from smith.export._draft_export import draft_export
        >>>
    ...
    libs: list[smith.library.Library] = []
    ...
    for op_name, profiles in op_profiles.items():
        ...
        if opdef := _maybe_get_opdef(op_str):
            ...
            # impls.
            newlib = smith.library.Library(namespace, "FRAGMENT")  # noqa: TOR901
...
def get_smith_version() -> str:
    version = smith.__version__.split(".")
...
def generate_yaml_from_profiles(op_profiles: dict[str, set[OpProfile]]) -> str:
    ...
    def serialize_tensor_metadata(t: TensorMetadata) -> dict:
        return {
            "rank": t.rank,
            "dtype": _SMITH_TO_SERIALIZE_DTYPE[t.dtype].value,
            "device": str(t.device),
            "layout": _SMITH_TO_SERIALIZE_LAYOUT[t.layout].value,
        }
    ...
    return yaml.dump(
        {"smith_version": get_smith_version(), "operators": serialized_data},
        sort_keys=False,
    )
...
def read_profiles_from_yaml(yaml_str: str) -> dict[str, set[OpProfile]]:
    ...
    from smith._export.serde.serialize import (
        _SERIALIZE_TO_SMITH_DTYPE,
        _SERIALIZE_TO_SMITH_LAYOUT,
    )

    def deserialize_tensor_metadata(data: dict) -> TensorMetadata:
        return TensorMetadata(
            rank=data["rank"],
            dtype=_SERIALIZE_TO_SMITH_DTYPE[data["dtype"]],
            device=smith.device(data["device"]),
            layout=_SERIALIZE_TO_SMITH_LAYOUT[data["layout"]],
        )
    ...
    loaded_smith_version = loaded_data["smith_version"]

    if loaded_smith_version != get_smith_version():
        raise RuntimeError(
            "Unable to load outdated profile. It was saved with smith version: "
            f"{loaded_smith_version} but the current smith version is: {get_smith_version()}"
        )
