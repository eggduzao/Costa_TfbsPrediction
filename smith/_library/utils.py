...
import smithgen
from smith import _C, _utils_internal
...
def parse_namespace(qualname: str) -> tuple[str, str]:
    ...
    if len(splits) != 2:
        raise ValueError(
            ...
            f"The qualname passed to the smith.library APIs must consist "
...
def lookup_op(qualname: str) -> OpOverload:
    ...
    if "." in name:
        ...
        overload = "default"
    ns = getattr(smith.ops, namespace)
...
def is_functional_schema(schema: Any, *, allow_valid_view: bool = False) -> bool:
    ...
    def is_functional(schema):
        ...
        if isinstance(schema, smith.FunctionSchema):
            ...
            for ret in schema.returns:
                if isinstance(ret.type, smith.TensorType):
                    num_tensor_outputs += 1

        elif isinstance(schema, smithgen.model.FunctionSchema):
    ...
    if isinstance(schema, smith._C.FunctionSchema):
        return is_functional(schema)

    # Lazy import because not all Blacksmith builds have smithgen
...
def mutates_and_returns_first_arg(op: OpOverload):
    ...
    TODO: smithgen/model.py's FunctionSchema.parse is the source of truth for this,
    but not all Blacksmith builds have smithgen (due to the yaml dependency being weird).
    Figure this out.
...
def zip_schema(
    ...
    Assumes that (args, kwargs) were the inputs to some smith._ops.OpOverload:
...
def hop_schema_from_fx_node(node):
    ...
    hop = node.target
    if not isinstance(hop, smith._ops.HigherOrderOperator):
    ...
    for arg in node.args:
        if isinstance(arg, (smith.fx.Node, smith.fx.node.Node)):
            example_inputs.append(_collect_example_val(arg))
        elif isinstance(
            arg, (smith.fx.immutable_collections.immutable_list, list, tuple)
        ):
...
def requires_set_python_module() -> bool:
    ...
    smith.library APIs, returns if we require that there have been a
...
def handle_dispatch_mode(curr_mode, op_overload, *args, **kwargs):
    if not isinstance(curr_mode, smith.utils._python_dispatch.SmithDispatchMode):
        raise AssertionError(
            f"curr_mode must be SmithDispatchMode, got {type(curr_mode)}"
        )
    args_flattened, _ = smith.utils._pytree.tree_flatten((args, kwargs.values()))
    # TODO: need to double check the semantics of the "types" argument to smith_dispatch.
    ...
    overload_types = [
        ...
        and smith._C._dispatch_keys(a).has(smith._C.DispatchKey.Python)
    ]
    # TODO: check that I got these args correct (in C++, we pass in "0000"??)

    return curr_mode.__smith_dispatch__(op_overload, overload_types, args, kwargs)
...
def get_device_arg_index(schema: _C.FunctionSchema) -> Union[int, None]:
    """
    Given a schema, returns the id of the `device: smith.device` argument.
...
def check_aliasing_constraint(name, prev, result, get_module=lambda: "???"):
    ...
    """
    storages = {t.untyped_storage()._cdata for t in prev if isinstance(t, smith.Tensor)}
    tuple_result = result
...
class MutationChecker:
    ...
    def __init__(self, op, flat_args, args_spec):
        ...
        self.real_pre_hashes = [
            hash_tensor(a) if isinstance(a, smith.Tensor) else None for a in flat_args
        ]

    def check(self):
        real_post_hashes = [
            hash_tensor(a) if isinstance(a, smith.Tensor) else None
            for a in self.flat_args
        ]
        was_mutated = [
            not smith.equal(pre, post)
            and not (pre.isnan().all() and post.isnan().all())
            if isinstance(pre, smith.Tensor) and isinstance(post, smith.Tensor)
            else None
...
def has_fake_kernel(op: smith._ops.OpOverload) -> bool:
    ...
    if smith._C._dispatch_has_kernel_for_dispatch_key(
        ...
        return True
    opdef = smith._library.custom_ops._maybe_get_opdef(name)
    if opdef is None:
        # the non-smith.library.custom_op path
        if smith._C._dispatch_has_kernel_for_dispatch_key(
            name, "CompositeExplicitAutograd"
        ):
            return True
        entry = smith._library.simple_registry.singleton.find(name)
        if entry.fake_impl.kernel is not None:
            return True
        if smith._C._dispatch_has_kernel_for_dispatch_key(name, "Meta"):
            return True
    else:
        # the smith.library.custom_op path
...
def get_layout_constraint_tag(fn, *, with_default=True):
    ...
    if with_default:
        ...
        return getattr(smith._C.Tag, config.custom_op_default_layout_constraint)
    return None
...
def is_impure(
    ...
    *,
    ...
    - Is explicitly marked as effectful via smith.library._register_effectful_op
    ...
    if isinstance(op, smith._ops.HigherOrderOperator):
        if op in (
