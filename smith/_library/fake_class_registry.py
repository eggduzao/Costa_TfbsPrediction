...
import smith
from smith._library.utils import parse_namespace
...
class FakeScriptObject:
    def __init__(
        self, wrapped_obj: Any, script_class_name: str, x: Optional[smith.ScriptObject]
    ):
    ...
    def __setattr__(self, name, value):
        raise AttributeError(
            ...
            "functionality, consider creating a custom SmithBind Object instead"
...
class FakeScriptMethod:
    def __init__(
        ...
        method_name: str,
        schema: Optional[smith.FunctionSchema],
    ):
    ...
    def __call__(self, *args, **kwargs):
        ...
        return call_smithbind(self.self_fake_obj, self.method_name, *args, **kwargs)


class HasStaticMethodFromReal(Protocol):
    @classmethod
    def from_real(cls, real_obj: smith.ScriptObject):
        pass
...
def tracing_with_real(x: smith.ScriptObject) -> bool:
...
def maybe_to_fake_obj(
    fake_mode,
    x: Any,
) -> Union[FakeScriptObject, smith.ScriptObject]:
    ...
    if is_opaque_type(x_type):
        ...
        with fake_mode:
            from smith._higher_order_ops.utils import _tensor_storage
            ...
            # This breaks the aliasing relationship among the tensors inside the smithbind object
    ...
    for name in x._method_names():  # type: ignore[attr-defined]
        ...
        if attr is not None:
            ...
            # real attr sometimes is not smith.ScriptMethod thus doesn't have schema e.g. __init___ or __eq__
            method_schema: Optional[smith.FunctionSchema] = None
            if isinstance(real_attr, smith.ScriptMethod):
...
def register_fake_class(qualname, fake_class: Optional[HasStaticMethodFromReal] = None):
    ...
    associates a fake class with the original smith bind class (registered
    with smith::class_). In this way, smith.compile can handle them properly
    ...
    __obj_unflatten__ as inputs to create a fake class. This protocol allows blacksmith to look
    ...
    def inner(fake_class: HasStaticMethodFromReal):
        ...
        # This also checks whether the referred smith::class_ exists.
        smith._C._get_custom_class_python_wrapper(ns, name)
...
def _ns_and_class_name(full_qualname: str) -> tuple[str, str]:
    ...
    _smith, _smith_ns, _classes, ns, class_name = splits
    return ns, class_name


def _find_fake_class_for_script_object(x: smith.ScriptObject) -> Any:
...
def _fake_obj_from_real(fake_mode, x) -> Any:
    ...
    ctx = smith._library.fake_impl.FakeImplCtx(fake_mode, None)
    with smith._library.fake_impl.set_ctx_getter(lambda: ctx):
