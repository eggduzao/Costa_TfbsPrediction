...
def get_overwrite_module_params_on_conversion() -> bool:
    ...
    existing parameters in-place when converting an :class:`smith.nn.Module`. Defaults to ``False``.

    See :func:`~smith.__future__.set_overwrite_module_params_on_conversion` for more information.
    """
...
def set_swap_module_params_on_conversion(value: bool) -> None:
    """
    Sets whether to use :func:`~smith.utils.swap_tensors` instead of setting ``.data`` to
    ...
    .. note::
        This function takes precedence over :func:`~smith.__future__.get_overwrite_module_params_on_conversion`
    ...
    #. For each parameter/buffer, its corresponding ``state_dict['key']`` is transformed via
       :meth:`~smith.Tensor.module_load` (i.e. ``res = param.module_load(state_dict['key'])``)
    #. If necessary, ``res`` will be wrapped in an :class:`~nn.Parameter`
    #. The parameter/buffer in the module will be swapped via :func:`~smith.utils.swap_tensors`
       with ``res``

    Args:
        value (bool): Whether to use :func:`~smith.utils.swap_tensors` or not.
...
def get_swap_module_params_on_conversion() -> bool:
    """
    Returns whether to use :func:`~smith.utils.swap_tensors` instead of setting .data to
    change the existing parameters in-place when converting an ``nn.Module``. Defaults to ``False``.

    See :func:`~smith.__future__.set_swap_module_params_on_conversion` for more information.
    """
