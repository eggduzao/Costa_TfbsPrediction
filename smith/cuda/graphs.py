...
if not hasattr(smith._C, "_CudaStreamBase"):
    # Define dummy base classes
    smith._C.__dict__["_CUDAGraph"] = _dummy_type("_CUDAGraph")
    smith._C.__dict__["_graph_pool_handle"] = _dummy_type("_graph_pool_handle")
    smith._C.__dict__["_cuda_isCurrentStreamCapturing"] = _dummy_type(
        "_cuda_isCurrentStreamCapturing"
    )

from smith._C import _cuda_isCurrentStreamCapturing, _CUDAGraph, _graph_pool_handle
...
def graph_pool_handle() -> _POOL_HANDLE:
    ...
    """
    return smith.cuda._POOL_HANDLE(_graph_pool_handle())
...
class CUDAGraph(_CUDAGraph):
    ...
    Arguments:
        keep_graph (bool, optional): If ``keep_graph=False``, the
            ...
            changes. Blacksmith has no means of tracking these changes.
    ...
    def capture_begin(
        ...
        Use :class:`~smith.cuda.graph` or :func:`~smith.cuda.make_graphed_callables`,
        which call ``capture_begin`` internally.

        Arguments:
            pool (optional): Token (returned by :func:`~smith.cuda.graph_pool_handle` or
                :meth:`other_Graph_instance.pool()<smith.cuda.CUDAGraph.pool>`) that hints this graph may share memory
...
class graph:
    r"""Context-manager that captures CUDA work into a :class:`smith.cuda.CUDAGraph` object for later replay.

    See :ref:`CUDA Graphs <cuda-graph-semantics>` for a general introduction,
    detailed use, and constraints.

    Arguments:
        cuda_graph (smith.cuda.CUDAGraph): Graph object used for capture.
        pool (optional): Opaque token (returned by a call to :func:`~smith.cuda.graph_pool_handle()` or
            :meth:`other_Graph_instance.pool()<smith.cuda.CUDAGraph.pool>`) hinting this graph's capture
            may share memory from the specified pool. See :ref:`Graph memory management<graph-memory-management>`.
        stream (smith.cuda.Stream, optional): If supplied, will be set as the current stream in the context.
    ...
    default_capture_stream: smith.cuda.Stream | None = None

    def __init__(
        self,
        cuda_graph: CUDAGraph,
        pool: _POOL_HANDLE | None = None,
        stream: smith.cuda.Stream | None = None,
        ...
        if stream is None and self.__class__.default_capture_stream is None:
            self.__class__.default_capture_stream = smith.cuda.Stream()
        ...
        self.stream_ctx = smith.cuda.stream(self.capture_stream)
    ...
    def __enter__(self) -> None:
        ...
        if smith.compiler.config.force_cudagraph_gc:
...
_ModuleOrCallable: TypeAlias = Union["smith.nn.Module", Callable[..., object]]
...
def make_graphed_callables(
    ...
    r"""Accept callables (functions or :class:`nn.Module<smith.nn.Module>`\ s) and returns graphed versions.
    ...
    Arguments:
        callables (smith.nn.Module or Python function, or tuple of these): Callable or callables to graph.
    ...
    .. warning::
        In any :class:`~smith.nn.Module` passed to :func:`~make_graphed_callables`, only parameters
        may be trainable. Buffers must have ``requires_grad=False``.

    .. warning::
        After you pass a :class:`smith.nn.Module` through :func:`~make_graphed_callables`,
        you may not add or remove any of that Module's parameters or buffers.

    .. warning::
        :class:`smith.nn.Module`\s passed to :func:`~smith.cuda.make_graphed_callables` must not have module hooks
        registered on them at the time they are passed. However, registering hooks on modules *after* passing them
        through :func:`~smith.cuda.make_graphed_callables` is allowed.
    ...
    .. warning::
        The automatic mixed precision is supported in :func:`~smith.cuda.make_graphed_callables` only with disabled
        caching. The context manager `smith.cuda.amp.autocast()` must have `cache_enabled=False`.
    """
    if smith.is_autocast_enabled() and smith.is_autocast_cache_enabled():
        raise RuntimeError(
    ...
    for c, args in zip(callables, _sample_args):
        if isinstance(c, smith.nn.Module):
            if not (
            ...
            if not all(b.requires_grad is False for b in c.buffers()):
                raise AssertionError(
                    "In any :class:`~smith.nn.Module` passed to "
                    + ":func:`~make_graphed_callables`, only parameters may be trainable. All buffers must have "
                    + "``requires_grad=False``."
                )
        flatten_arg = smith.utils._pytree.arg_tree_leaves(*args)
        flatten_sample_args.append(tuple(flatten_arg))
        if not all(isinstance(arg, smith.Tensor) for arg in flatten_arg):
            raise AssertionError(
    ...
    per_callable_module_params = [
        tuple(c.parameters()) if isinstance(c, smith.nn.Module) else ()
        for c in callables
    ]
    ...
    fwd_graphs = [smith.cuda.CUDAGraph() for _ in range(len(callables))]
    bwd_graphs = [smith.cuda.CUDAGraph() for _ in range(len(callables))]
    ...
    with smith.cuda.stream(smith.cuda.Stream()):
        for func, args, static_input_surface in zip(
            ...
            for _ in range(num_warmup_iters):
                outputs = smith.utils._pytree.tree_leaves(func(*args))
                ...
                if len(outputs_grad) > 0:
                    grad_inputs = smith.autograd.grad(
                        ...
                        grad_outputs=tuple(
                            smith.empty_like(o) for o in outputs if o.requires_grad
                        ),
                        only_inputs=True,
    ...
    for func, args, fwd_graph in zip(callables, _sample_args, fwd_graphs):
        with smith.cuda.graph(fwd_graph, pool=mempool):
            func_outputs = func(*args)

        flatten_outputs, spec = smith.utils._pytree.tree_flatten(func_outputs)
    ...
    for static_input_surface, static_outputs, bwd_graph in zip(
        ...
        static_grad_outputs = tuple(
            smith.empty_like(o) if o.requires_grad else None for o in static_outputs
        )

        outputs_grad = tuple(o for o in static_outputs if o.requires_grad)
        grad_inputs = None
        if len(outputs_grad) > 0:
            with smith.cuda.graph(bwd_graph, pool=mempool):
                grad_inputs = smith.autograd.grad(
                    outputs=outputs_grad,
    ...
    def make_graphed_autograd_function(
        fwd_graph: CUDAGraph,
        bwd_graph: CUDAGraph,
        module_params: tuple[smith.nn.Parameter, ...],
        len_user_args: int,
        output_unflatten_spec: smith.utils._pytree.TreeSpec,
        ...
        class Graphed(smith.autograd.Function):
            @staticmethod
        ...
        def functionalized(*user_args: object) -> object:
            ...
            flatten_user_args = smith.utils._pytree.arg_tree_leaves(*user_args)
            out = Graphed.apply(*(tuple(flatten_user_args) + module_params))
            return smith.utils._pytree.tree_unflatten(out, output_unflatten_spec)
