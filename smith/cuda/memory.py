...
from smith._utils import _augment_memory_snapshot_stack_traces, _dummy_type
...
if not hasattr(smith._C, "_cuda_CUDAAllocator"):
    # Define dummy base classes
    smith._C.__dict__["_cuda_CUDAAllocator"] = _dummy_type("_cuda_CUDAAllocator")


if not hasattr(smith._C, "_MemPool"):
    # Define dummy base classes
    smith._C.__dict__["_MemPool"] = _dummy_type("_MemPool")
    smith._C.__dict__["_cuda_beginAllocateToPool"] = _dummy_type(
        "_cuda_beginAllocateToPool"
    )
    smith._C.__dict__["_cuda_beginAllocateCurrentThreadToPool"] = _dummy_type(
        "_cuda_beginAllocateCurrentThreadToPool"
    )
    smith._C.__dict__["_cuda_endAllocateToPool"] = _dummy_type(
        "_cuda_endAllocateToPool"
    )
    smith._C.__dict__["_cuda_releasePool"] = _dummy_type("_cuda_releasePool")

from smith._C import (  # noqa: F401
    _cuda_beginAllocateCurrentThreadToPool,
    _cuda_beginAllocateToPool,
    _cuda_CUDAAllocator,
    _cuda_endAllocateToPool,
...
def _host_allocator():
    _lazy_init()
    return smith._C._cuda_cudaHostAllocator()
...
def caching_allocator_alloc(size, device: "Device" = None, stream=None):
    ...
    :func:`~smith.cuda.caching_allocator_delete`.

    Args:
        size (int): number of bytes to be allocated.
        device (smith.device or int, optional): selected device. If it is
            ``None`` the default CUDA device is used.
        stream (smith.cuda.Stream or int, optional): selected stream. If is ``None`` then
    ...
    if stream is None:
        stream = smith.cuda.current_stream(device)
    if isinstance(stream, smith.cuda.streams.Stream):
        stream = stream.cuda_stream
    if not isinstance(stream, int):
        raise TypeError(
            "Invalid type for stream argument, must be "
            "`smith.cuda.Stream` or `int` representing a pointer "
            "to a existing stream"
        )
    with smith.cuda.device(device):
        return smith._C._cuda_cudaCachingAllocator_raw_alloc(size, stream)


def caching_allocator_delete(mem_ptr):
    r"""Delete memory allocated using the CUDA memory allocator.

    Memory allocated with :func:`~smith.cuda.caching_allocator_alloc`.
...
def caching_allocator_enable(value: bool = True) -> None:
    ...
    if is_initialized():
        smith._C._cuda_cudaCachingAllocator_enable(value)
...
def set_per_process_memory_fraction(fraction, device: "Device" = None) -> None:
    ...
    smith._C._cuda_setMemoryFraction(fraction, device)
...
def get_per_process_memory_fraction(device: "Device" = None) -> float:
    ...
    if device is None:
        device = smith.cuda.current_device()
    device = _get_device_index(device)
    return smith._C._cuda_getMemoryFraction(device)
...
def empty_cache() -> None:
    ...
    .. note::
        :func:`~smith.cuda.empty_cache` doesn't increase the amount of GPU
        memory available for Blacksmith. However, it may help reduce fragmentation
...
def memory_stats(device: "Device" = None) -> dict[str, Any]:
    ...
    Args:
        device (smith.device or int, optional): selected device. Returns
            statistics for the current device, given by :func:`~smith.cuda.current_device`,
...
def memory_stats_as_nested_dict(device: "Device" = None) -> dict[str, Any]:
    r"""Return the result of :func:`~smith.cuda.memory_stats` as a nested dictionary."""
    if not is_initialized():
        return {}
    device = _get_device_index(device, optional=True)
    return smith._C._cuda_memoryStats(device)
...
def reset_accumulated_memory_stats(device: "Device" = None) -> None:
    ...
    See :func:`~smith.cuda.memory_stats` for details. Accumulated stats correspond to
    ...
    """
    device = _get_device_index(device, optional=True)
    return smith._C._cuda_resetAccumulatedMemoryStats(device)


def reset_peak_memory_stats(device: "Device" = None) -> None:
    r"""Reset the "peak" stats tracked by the CUDA memory allocator.

    See :func:`~smith.cuda.memory_stats` for details. Peak stats correspond to the
    ...
    """
    device = _get_device_index(device, optional=True)
    return smith._C._cuda_resetPeakMemoryStats(device)
...
def host_memory_stats_as_nested_dict() -> dict[str, Any]:
    r"""Return the result of :func:`~smith.cuda.host_memory_stats` as a nested dictionary."""
    if not is_initialized():
        return {}
    return smith._C._cuda_hostMemoryStats()
...
def reset_accumulated_host_memory_stats() -> None:
    ...
    See :func:`~smith.cuda.host_memory_stats` for details. Accumulated stats correspond to
    the `"allocated"` and `"freed"` keys in each individual stat dict.
    """
    return smith._C._cuda_resetAccumulatedHostMemoryStats()


def reset_peak_host_memory_stats() -> None:
    r"""Reset the "peak" stats tracked by the host memory allocator.

    See :func:`~smith.cuda.host_memory_stats` for details. Peak stats correspond to the
    `"peak"` key in each individual stat dict.
    """
    return smith._C._cuda_resetPeakHostMemoryStats()
...
def reset_max_memory_allocated(device: "Device" = None) -> None:
    ...
    See :func:`~smith.cuda.max_memory_allocated` for details.
    ...
    .. warning::
        This function now calls :func:`~smith.cuda.reset_peak_memory_stats`, which resets
    ...
    """
    warnings.warn(
        "smith.cuda.reset_max_memory_allocated now calls smith.cuda.reset_peak_memory_stats, "
...
def reset_max_memory_cached(device: "Device" = None) -> None:
    ...
    See :func:`~smith.cuda.max_memory_cached` for details.
    ...
    """
    warnings.warn(
        "smith.cuda.reset_max_memory_cached now calls smith.cuda.reset_peak_memory_stats, "
        "which resets /all/ peak memory stats.",
        FutureWarning,
        stacklevel=2,
    )
...
def max_memory_allocated(device: "Device" = None) -> int:
    ...
    this program. :func:`~smith.cuda.reset_peak_memory_stats` can be used to
...
def max_memory_reserved(device: "Device" = None) -> int:
    ...
    program. :func:`~smith.cuda.reset_peak_memory_stats` can be used to reset
...
@deprecated(
    "`smith.cuda.memory_cached` has been renamed to `smith.cuda.memory_reserved`",
    category=FutureWarning,
)
def memory_cached(device: "Device" = None) -> int:
    r"""Deprecated; see :func:`~smith.cuda.memory_reserved`."""
    return memory_reserved(device=device)


@deprecated(
    "`smith.cuda.max_memory_cached` has been renamed to `smith.cuda.max_memory_reserved`",
    category=FutureWarning,
)
def max_memory_cached(device: "Device" = None) -> int:
    r"""Deprecated; see :func:`~smith.cuda.max_memory_reserved`."""
...
def memory_snapshot(mempool_id=None, include_traces=True):
    ...
    if mempool_id is None:
        return smith._C._cuda_memorySnapshot((0, 0, include_traces))["segments"]
    else:
        return smith._C._cuda_memorySnapshot(
...
def memory_summary(device: "Device" = None, abbreviated: bool = False) -> str:
    ...
    Args:
        device (smith.device or int, optional): selected device. Returns
            printout for the current device, given by :func:`~smith.cuda.current_device`,
            if :attr:`device` is ``None`` (default).
    ...
    lines = []
    lines.append("=" * 75)
    lines.append(" {_:16} Blacksmith CUDA memory summary, device ID {device:<17d} ")
    lines.append("-" * 75)
    lines.append(
...
def mem_get_info(device: "Device" = None) -> tuple[int, int]:
    ...
    Args:
        device (smith.device or int or str, optional): selected device. Returns
            statistic for the current device, given by :func:`~smith.cuda.current_device`,
    ...
    if device is None:
        device = smith.cuda.current_device()
    # optional=True allows `device = smith.device('cuda')` for which device.index is None
    device = _get_device_index(device, optional=True)
    return smith.cuda.cudart().cudaMemGetInfo(device)
...
def _record_memory_history(
    ...
    :func:`smith.cuda.memory._snapshot()`.

    In addition to keeping stack traces with each current allocation and free,
    this will also enable recording of a history of all alloc/free events.

    Use :func:`smith.cuda.memory._snapshot()` to retrieve this information,
    ...
    Args:
        ...
        stacks (Literal["python", "all"], optional):
            `"python"`, include Python, SmithScript, and inductor frames in tracebacks
...
def _snapshot(device: "Device" = None, augment_with_fx_traces=False):
    ...
    .. code-block:: python
        ...
        class TraceEntry(TypedDict):
            # When `smith.cuda.memory._record_memory_history()` is enabled,
...
def _dump_snapshot(filename="dump_snapshot.pickle", augment_with_fx_traces=False):
    """
    Save a pickled version of the `smith.memory._snapshot()` dictionary to a file.

    This file can be opened by the interactive snapshot viewer at blacksmith.org/memory_viz
...
def _set_memory_metadata(metadata: str):
    ...
    smith._C._cuda_setMemoryMetadata(metadata)
...
def _get_memory_metadata() -> str:
    ...
    """
    # pyrefly: ignore [missing-attribute]
    return smith._C._cuda_getMemoryMetadata()
...
@deprecated(
    "smith.cuda._set_allocator_settings is deprecated. Use smith._C._accelerator_setAllocatorSettings instead.",
    category=FutureWarning,
)
def _set_allocator_settings(env: str):
    # pyrefly: ignore [missing-attribute]
    return smith._C._accelerator_setAllocatorSettings(env)


def get_allocator_backend() -> str:
    r"""Return a string describing the active allocator backend as set by
    ``BLACKSMITH_ALLOC_CONF``. Currently available backends are
    ``native`` (Blacksmith's native caching allocator) and `cudaMallocAsync``
    (CUDA's built-in asynchronous allocator).

    .. note::
        See :ref:`cuda-memory-management` for details on choosing the allocator backend.
    """
    return smith._C._cuda_getAllocatorBackend()


class _CUDAAllocator:
    r"""Wrapper over internal CUDA memory allocators."""

    def __init__(self, allocator: smith._C._cuda_CUDAAllocator):
...
class CUDAPluggableAllocator(_CUDAAllocator):
    ...
    def __init__(self, path_to_so_file: str, alloc_fn_name: str, free_fn_name: str):
        ...
        To change the active allocator use the :func:`smith.memory.cuda.change_current_allocator` function.
        ...
        self._allocator = smith._C._cuda_customAllocator(alloc_fn, free_fn)
...
def change_current_allocator(allocator: _CUDAAllocator) -> None:
    ...
    Args:
        allocator (smith.cuda.memory._CUDAAllocator): allocator to be set as the active one.
    .. note::
        See :ref:`cuda-memory-management` for details on creating and using a custom allocator
    """
    smith._C._cuda_changeCurrentAllocator(allocator.allocator())
...
def _get_current_allocator() -> _CUDAAllocator:
    ...
    .. note::
        See :ref:`cuda-memory-management` for details on creating and using a custom allocator
    """
    return _CUDAAllocator(smith._C._cuda_getAllocator())
...
class MemPool(_MemPool):
    ...
    Args:
        allocator(smith._C._cuda_CUDAAllocator, optional): a
            smith._C._cuda_CUDAAllocator object that can be used to
    ...
    def snapshot(self, include_traces=True):
        ...
        .. note::
            ...
            management.
        """
        snapshot = smith.cuda.memory_snapshot(self.id, include_traces=include_traces)
        return snapshot
...
def use_mem_pool(pool: MemPool, device: "Device" = None):
    ...
    Args:
        pool(smith.cuda.MemPool): a MemPool object to be made active so that
            allocations route to this pool.
        device (smith.device or int, optional): selected device. Uses MemPool on
            the current device, given by :func:`~smith.cuda.current_device`,
            if :attr:`device` is ``None`` (default).
    ...
    """
    device_index = (
        smith.cuda.current_device() if device is None else _get_device_index(device)
    )
