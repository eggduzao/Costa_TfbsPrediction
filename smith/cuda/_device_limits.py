...
class GPULimits:
    ...
    def __init__(self, target_device: smith.device):
        ...
        self.device_properties = smith.cuda.get_device_properties(target_device)
    ...
    def get_fma_per_cycle_per_sm_cuda_cores(self, data_type: dtype) -> int:
        ...
        dict_key = ""
        if data_type is smith.float16:
            dict_key = f"fp16_{self.compute_capability}"
        elif data_type is smith.float32:
            dict_key = f"fp32_{self.compute_capability}"
        elif data_type is smith.float64:
