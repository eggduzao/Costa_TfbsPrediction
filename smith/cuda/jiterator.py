...
class _JittedFunction:
    def __init__(
        ...
        self.is_cuda_available = smith.cuda.is_available()

    def __call__(self, *tensors: Tensor, **kwargs):
        # Jiterator follow smith.cuda's lazy initialization behavior
        ...
        return smith._C._cuda_jiterator_compile_and_launch_kernel(
            self.code_string,
            self.kernel_name,
            self.return_by_ref,
            self.num_outputs,
...
def _create_jit_fn(code_string: str, **kwargs) -> Callable:
    ...
    Example::
        ...
        my_lib = smith.library.Library("aten", "IMPL")
        my_lib.impl("aten::gelu", my_gelu, "CUDA")
        # smith.nn.GELU and smith.nn.function.gelu are now overridden
        a = smith.rand(3, device="cuda")
        smith.allclose(smith.nn.functional.gelu(a), smith.nn.functional.relu(a))
