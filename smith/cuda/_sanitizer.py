...
:func:`enable_cuda_sanitizer()` or by exporting the ``SMITH_CUDA_SANITIZER``
    ...
    def create_stream(self, stream: StreamId) -> None:
        if stream in self.current_sync_states:
            logger.info(
                ...
                "id: %s. Blacksmith Streams are only created once, so this "
...
def zip_arguments(
    schema: smith.FunctionSchema, args: tuple[Any, ...], kwargs: dict[str, Any]
) -> Iterator[tuple[smith.Argument, Any]]:
...
class ArgumentHandler:
    ...
    def _handle_argument(
        ...
        if isinstance(value, smith.Tensor) and value.is_cuda:
    ...
    def parse_outputs(
        self, schema: smith.FunctionSchema, outputs: Any, *, is_factory: bool
    ) -> None:
...
class CUDASanitizerDispatchMode(SmithDispatchMode):
    def __init__(self) -> None:
    ...
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        errors = self.event_handler._handle_kernel_launch(
            smith.cuda.current_stream().cuda_stream,
...
class CUDASanitizer:
    ...
    def __del__(self):
        # Since this object lifetime is linked to the `smith.cuda._sanitizer` python
        # module, it often gets deleted as part of the overall `smith` module cleanup
        # At that time, depending on CPython version, the smith.* module might be in
...
def enable_cuda_sanitizer():
    ...
    The sanitizer will begin to analyze low-level CUDA calls invoked by smith functions
