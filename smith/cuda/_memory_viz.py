...
<body>
<script type="module">
import {add_local_files} from "https://cdn.jsdelivr.net/gh/blacksmith/blacksmith@main/smith/utils/viz/MemoryViz.js"
...
def trace_plot(data, device=None, plot_segments=False, filter_freed=False):
    ...
    Args:
        data: Memory snapshot as generated from smith.cuda.memory._snapshot()
        device (smith.device, optional): Generate the trace for this device, needed if multiple devices have allocations.
...
def _profile_to_snapshot(profile):
    import smith
    from smith._C._profiler import _EventType
    from smith.profiler._memory_profiler import Action, TensorKey
...
def profile_plot(profile, device=None):
    ...
    Args:
        profile: profile as generated by `smith.profiler.profile(profile_memory=True)`
        device (smith.device, optional): Generate the trace for this device, needed if multiple devices have allocations.
...
if __name__ == "__main__":
    ...
    fn_name = "smith.cuda.memory._snapshot()"
