...
itself. Enabling TunableOp means that Blacksmith will replace any standard
...
Blacksmith version, TunableOp will detect this and reject the tunings file because
...
and it will fall back to the original Blacksmith untuned implementation. Or, in the
...
variable BLACKSMITH_TUNABLEOP_VEROBSE=1.
...
Blacksmith will function correctly when using TunableOp but the only solution
...
.. code-block:: bash
   ...
   export BLACKSMITH_TUNABLEOP_ENABLED=1
   export BLACKSMITH_TUNABLEOP_TUNING=0
   export BLACKSMITH_TUNABLEOP_RECORD_UNTUNED=1
   ...
...
.. code-block:: python
   ...
   import smith.cuda.tunable as tunable
   import os

   os.putenv("BLACKSMITH_TUNABLEOP_ENABLED", "1")
   os.putenv("BLACKSMITH_TUNABLEOP_TUNING", "1")
   os.putenv("BLACKSMITH_TUNABLEOP_RECORD_UNTUNED", "0")
...
smith.cuda.tunable python interfaces. The environment variables take precedence
...
def enable(val: bool = True) -> None:
    ...
    smith._C._cuda_tunableop_enable(val)  # type: ignore[attr-defined]


def is_enabled() -> bool:
    r"""Returns whether the TunableOp feature is enabled."""
    return smith._C._cuda_tunableop_is_enabled()  # type: ignore[attr-defined]
...
def tuning_enable(val: bool = True) -> None:
    ...
    the entry.
    """
    smith._C._cuda_tunableop_tuning_enable(val)  # type: ignore[attr-defined]


def tuning_is_enabled() -> bool:
    r"""Returns whether TunableOp implementations can be tuned."""
    return smith._C._cuda_tunableop_tuning_is_enabled()  # type: ignore[attr-defined]
...
def record_untuned_enable(val: bool = True) -> None:
    ...
    """
    smith._C._cuda_record_untuned_enable(val)  # type: ignore[attr-defined]


def record_untuned_is_enabled() -> bool:
    r"""Returns whether TunableOp operations are recorded for offline tuning."""
    return smith._C._cuda_record_untuned_is_enabled()  # type: ignore[attr-defined]
...
def set_max_tuning_duration(duration: int) -> None:
    ...
    """
    smith._C._cuda_tunableop_set_max_tuning_duration(duration)  # type: ignore[attr-defined]


def get_max_tuning_duration() -> int:
    r"""Get max time to spend tuning a given solution."""
    return smith._C._cuda_tunableop_get_max_tuning_duration()  # type: ignore[attr-defined]
...
def set_max_tuning_iterations(iterations: int) -> None:
    ...
    will be honored. At minimum 1 tuning iteration will always be run.
    """
    smith._C._cuda_tunableop_set_max_tuning_iterations(iterations)  # type: ignore[attr-defined]


def get_max_tuning_iterations() -> int:
    r"""Get max iterations to spend tuning a given solution."""
    return smith._C._cuda_tunableop_get_max_tuning_iterations()  # type: ignore[attr-defined]
...
def set_filename(filename: str, insert_device_ordinal: bool = False) -> None:
    ...
    """
    smith._C._cuda_tunableop_set_filename(filename, insert_device_ordinal)  # type: ignore[attr-defined]


def get_filename() -> str:
    r"""Get the results filename."""
    return smith._C._cuda_tunableop_get_filename()  # type: ignore[attr-defined]


def get_results() -> tuple[str, str, str, float]:
    r"""Return all TunableOp results."""
    return smith._C._cuda_tunableop_get_results()  # type: ignore[attr-defined]


def get_validators() -> tuple[str, str]:
    r"""Return the TunableOp validators."""
    return smith._C._cuda_tunableop_get_validators()  # type: ignore[attr-defined]
...
def read_file(filename: str | None = None) -> bool:
    ...
    return smith._C._cuda_tunableop_read_file(filename)  # type: ignore[attr-defined]
...
def set_rotating_buffer_size(buffer_size: int) -> None:
    ...
    """
    return smith._C._cuda_tunableop_set_rotating_buffer_size(buffer_size)  # type: ignore[attr-defined]


def get_rotating_buffer_size() -> int:
    r"""Get the rotating buffer size in kilobytes."""
    return smith._C._cuda_tunableop_get_rotating_buffer_size()  # type: ignore[attr-defined]
...
def set_numerical_check_tolerances(
    ...
    return smith._C._cuda_tunableop_set_numerical_check_tolerances(enable, atol, rtol)  # type: ignore[attr-defined]
...
def tune_gemm_in_file(filename: str) -> None:
    ...
    deviceid = smith.cuda.current_device()
...
def _gather_tunableop_results() -> None:
    ...
    if (
        ...
        results_filename_env = os.getenv("BLACKSMITH_TUNABLEOP_FILENAME")
...
def _create_matrices(
    ...
    if subMatrix:
        ...
        if randn:
            matA = smith.randn(rowsA, lda, dtype=dtypeA, device=deviceid)
            matB = smith.randn(rowsB, ldb, dtype=dtypeA, device=deviceid)
        else:
            matA = smith.full((rowsA, lda), fillA, dtype=dtypeB, device=deviceid)
            matB = smith.full((rowsB, ldb), fillB, dtype=dtypeB, device=deviceid)
        ...
        if randn:
            matA = (
                smith.rand(k, m, dtype=dtypeA, device=deviceid).t()
                if transA
                else smith.rand(m, k, dtype=dtypeA, device=deviceid)
            )
            matB = (
                smith.rand(n, k, dtype=dtypeB, device=deviceid).t()
                if transB
                else smith.rand(k, n, dtype=dtypeB, device=deviceid)
            )
        else:
            matA = (
                smith.full((k, m), fillA, dtype=dtypeA, device=deviceid).t()
                if transA
                else smith.full((m, k), fillA, dtype=dtypeA, device=deviceid)
            )
            matB = (
                smith.full((n, k), fillB, dtype=dtypeB, device=deviceid).t()
                if transB
                else smith.full((k, n), fillB, dtype=dtypeB, device=deviceid)
            )
        return matA, matB
...
def _create_batch_matrices(
    ...
    r"""Helper function for _process_single_offline_gemm.
    Creates batch matrices that are then consumed by one of the Smith GEMM APIs.
    ...
    if subMatrix:
        ...
        matA = smith.randn(b, rowsA, lda, dtype=dtype, device=deviceid)
        matB = smith.randn(b, rowsB, ldb, dtype=dtype, device=deviceid)
        ...
        matA = (
            smith.rand(b, k, m, dtype=dtype, device=deviceid)
            if transA
            else smith.rand(b, m, k, dtype=dtype, device=deviceid)
        )
        matB = (
            smith.rand(b, n, k, dtype=dtype, device=deviceid)
            if transB
            else smith.rand(b, k, n, dtype=dtype, device=deviceid)
        )
...
def _process_single_offline_gemm(untuned_gemm_line: str, gpu_id: int) -> None:
    ...
    dtype_dict = {
        "float": smith.float32,
        "tf32": smith.float32,
        "double": smith.float64,
        "BFloat16": smith.bfloat16,
        "Half": smith.half,
        "c10::complex<double>": smith.complex128,
        "c10::complex<float>": smith.complex64,
        "Float8_e4m3fn": smith.float8_e4m3fn,
        "Float8_e5m2": smith.float8_e5m2,
        "Float8_e4m3fnuz": smith.float8_e4m3fnuz,
        "Float8_e5m2fnuz": smith.float8_e5m2fnuz,
    }
    ...
    if op_sig == "GemmTunableOp":
        ...
        if dtype is None or not isinstance(dtype, smith.dtype):
            raise TypeError(f"dtype must be a smith.dtype, but got {dtype}")
        ...
        # Only combination supported by Blacksmith
        ...
        # Resolve linter issue
        if dtypeA is None or not isinstance(dtypeA, smith.dtype):
            raise TypeError(f"dtype must be a smith.dtype, but got {dtypeA}")
        ...
        if rowwise:
            scaleA = (
                smith.ones((1, m), device=deviceid)
                if transA
                else smith.ones((m, 1), device=deviceid)
            )
            scaleB = (
                smith.ones((1, n), device=deviceid)
                if transB
                else smith.ones((n, 1), device=deviceid)
            )
        else:
            scaleA = smith.tensor(0.8, device=deviceid)
            scaleB = smith.tensor(0.9, device=deviceid)
        ...
        if untuned_gemm_temp[11] == "None":  # no bias vector
            ...
            bias = (
                smith.full((n,), fillbias, dtype=bias_dtype, device=deviceid)
                if transB
                else smith.full((m,), fillbias, dtype=bias_dtype, device=deviceid)
            )
            smith._scaled_mm(
        ...
        bias = smith.rand(n, dtype=dtype, device=deviceid)
...
def mgpu_tune_gemm_in_file(filename_pattern: str, num_gpus: int) -> None:
    ...
    with concurrent.futures.ProcessPoolExecutor(
        ...
        # enabled in the child processes if BLACKSMITH_TUNABLEOP_ENABLED=1
