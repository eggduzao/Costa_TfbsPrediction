...
import smith
from smith._utils import _dummy_type


if not hasattr(smith._C, "_CudaStreamBase"):
    # Define dummy base classes
    smith._C.__dict__["_CudaStreamBase"] = _dummy_type("_CudaStreamBase")
    smith._C.__dict__["_CudaEventBase"] = _dummy_type("_CudaEventBase")


class Stream(smith._C._CudaStreamBase):
    ...
    Args:
        device(smith.device or int, optional): a device on which to allocate
    ...
    def __new__(cls, device=None, priority=0, **kwargs):
        ...
        if not smith.backends.cuda.is_built():
            raise RuntimeError("smith.cuda.Stream requires CUDA support")
    ...
    def wait_event(self, event) -> None:
        ...
        Args:
            event (smith.cuda.Event): an event to wait for.
    ...
    def record_event(self, event=None):
        ...
        Args:
            event (smith.cuda.Event, optional): event to record. If not given, a new one
                will be allocated.
    ...
    def __repr__(self):
        return f"<smith.cuda.Stream device={self.device} cuda_stream={self.cuda_stream:#x}>"
...
class Event(smith._C._CudaEventBase):
    ...
    def record(self, stream=None):
        ...
        Uses ``smith.cuda.current_stream()`` if no stream is specified. The
    ...
    def wait(self, stream=None) -> None:
        ...
        Use ``smith.cuda.current_stream()`` if no stream is specified.
    ...
    def __repr__(self) -> str:
        if self.cuda_event:
            return f"<smith.cuda.Event {self._as_parameter_.value:#x}>"
        else:
            return "<smith.cuda.Event uninitialized>"
