...
def get_rng_state(device: int | str | smith.device = "cuda") -> Tensor:
    ...
    if idx is None:
        idx = current_device()
    default_generator = smith.cuda.default_generators[idx]
...
def set_rng_state(new_state: Tensor, device: int | str | smith.device = "cuda") -> None:
...
def manual_seed_all(seed: int) -> None:
    ...
    def cb():
        for i in range(device_count()):
            default_generator = smith.cuda.default_generators[i]
            default_generator.manual_seed(seed)
