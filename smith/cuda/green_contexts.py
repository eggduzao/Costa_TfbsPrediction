...
if hasattr(smith._C, "_CUDAGreenContext"):
    _GreenContext = smith._C._CUDAGreenContext  # type: ignore[misc]
    SUPPORTED = True
...
class GreenContext(_GreenContext):
    ...
    def create(num_sms: int, device_id: int = 0) -> _GreenContext:
        ...
        """
        if not SUPPORTED:
            raise RuntimeError("Blacksmith was not built with Green Context support!")
