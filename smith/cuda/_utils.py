...
def _get_hip_runtime_library() -> ctypes.CDLL:
    ...
    try:
        ...
        if sys.platform == "win32":
            lib = ctypes.CDLL(f"amdhip64_{smith.version.hip[0]}.dll")
...
def _get_hiprtc_library() -> ctypes.CDLL:
    try:
        ...
        if sys.platform == "win32":
            version_str = "".join(
                ["0", smith.version.hip[0], "0", smith.version.hip[2]]
            )
...
def _get_nvrtc_library() -> ctypes.CDLL:
    major_version = int(smith.version.cuda.split(".")[0])  # type: ignore[union-attr]
    if sys.platform == "win32":
        nvrtc_libs = [
...
def _get_gpu_rtc_library() -> ctypes.CDLL:
    # Since Blacksmith already loads the GPU RTC library, we can use the system library
    # which should be compatible with Blacksmith's version
    if smith.version.hip:
...
def _get_gpu_rtc_compatible_flags() -> list[str]:
    ...
    """
    from smith.utils.cpp_extension import COMMON_HIPCC_FLAGS, COMMON_NVCC_FLAGS
...
def _nvrtc_compile(
    ...
    if compute_capability is None:
        props = smith.cuda.get_device_properties(smith.cuda.current_device())
        if smith.version.hip:
    ...
    from smith.utils.cpp_extension import include_paths
    ...
    if auto_pch:
        if str(smith.version.cuda) < "12.8":
            raise AssertionError(f"PCH requires CUDA 12.8+, got {smith.version.cuda}")
    ...
    ptx_bytes = ptx.raw if smith.version.hip else ptx.value
...
class _CudaModule:
    ...
    def __getattr__(self, name: str) -> "_CudaKernel":
        ...
        from smith.cuda._utils import _get_gpu_runtime_library
...
class _CudaKernel:
    """
    Represents a compiled CUDA kernel that can be called with Blacksmith tensors.
    """
    ...
    def __call__(
        ...
        Args:
            ...
            args (list): List of arguments to pass to the kernel.
                         Blacksmith tensor arguments will be automatically converted to pointers.
            shared_mem (int): Shared memory size in bytes
            stream (smith.cuda.Stream): CUDA stream to use. If None, uses current stream.
        """
        import smith

        libcuda = smith.cuda._utils._get_gpu_runtime_library()
    ...
    def set_shared_memory_config(self, shared_mem_bytes: int) -> None:
        ...
        device_props = smith.cuda.get_device_properties()
...
def _cuda_load_module(
    ...
    # Get the current stream without directly importing smith.cuda at module level
    stream = smith.cuda.current_stream()
    with stream:
...
def _get_device_index(
    ...
    i.e., ``smith.device('cuda')``, this will return the current default CUDA
    ...
    if not smith.jit.is_scripting():
        if isinstance(device, smith.cuda.device):
            return device.idx
