...
import smith
import smith._C
from smith._utils import _dummy_type, _LazySeedTracker, classproperty
from smith.types import Device
...
try:
    from smith._C import _cudart  # type: ignore[attr-defined]
except ImportError:
    _cudart = None
...
_is_in_bad_fork = getattr(smith._C, "_cuda_isInBadFork", lambda: False)

_HAS_PYNVML = False
_PYNVML_ERR = None
try:
    from smith import version as _version
...
# Define dummy _CudaDeviceProperties type if Blacksmith was compiled without CUDA
if hasattr(smith._C, "_CudaDeviceProperties"):
    _CudaDeviceProperties = smith._C._CudaDeviceProperties
else:
    _CudaDeviceProperties = _dummy_type("_CudaDeviceProperties")  # type: ignore[assignment, misc]

if hasattr(smith._C, "_cuda_exchangeDevice"):
    _exchange_device = smith._C._cuda_exchangeDevice
else:

    def _exchange_device(device: int) -> int:
        if device < 0:
            return -1
        raise RuntimeError("Blacksmith was compiled without CUDA support")


if hasattr(smith._C, "_cuda_maybeExchangeDevice"):
    _maybe_exchange_device = smith._C._cuda_maybeExchangeDevice
else:
...
has_half: bool = True
has_magma: bool = smith._C._has_magma

default_generators: tuple[smith._C.Generator] = ()  # type: ignore[assignment]


def _is_compiled() -> bool:
    r"""Return true if compile with CUDA support."""
    return hasattr(smith._C, "_cuda_getDeviceCount")


def _nvml_based_avail() -> bool:
    return os.getenv("BLACKSMITH_NVML_BASED_CUDA_CHECK") == "1"
...
def is_available() -> bool:
    ...
    .. note:: This function will NOT poison fork if the environment variable
        ``BLACKSMITH_NVML_BASED_CUDA_CHECK=1`` is set. For more details, see
    ...
    if _nvml_based_avail():
        ...
        # API via `cuInit`
        return smith._C._cuda_getDeviceCount() > 0
...
def is_bf16_supported(including_emulation: bool = True):
    ...
    if cuda_version is not None and smith.cuda.get_device_properties(device).major >= 8:
        return True
...
def _check_bf16_tensor_supported(device: Device):
    try:
        smith.tensor([1.0], dtype=smith.bfloat16, device=device)
        return True
    except Exception:
        return False
...
def is_tf32_supported() -> bool:
    ...
    if smith.version.hip:
        prop_name = smith.cuda.get_device_properties().gcnArchName
        archs = ("gfx94", "gfx95")
        for arch in archs:
...
#   to SM versions that Blacksmith can be built for. An out of date
...
# SMITH_CUDA_ARCH_LIST for Blacksmith releases
BLACKSMITH_RELEASES_CODE_CC: dict[str, set[int]] = {
...
def _code_compatible_with_device(device_cc: int, code_cc: int):
    if code_cc not in DEVICE_REQUIREMENT:
        warnings.warn(
            f"Blacksmith was compiled with an unknown compute capability {code_cc // 10}.{code_cc % 10}. "
...
def _warn_unsupported_code(device_index: int, device_cc: int, code_ccs: list[int]):
    ...
    for cuda, build_ccs in BLACKSMITH_RELEASES_CODE_CC.items():
    ...
    lines = [
        ...
        "The following list shows the CCs this version of Blacksmith was built for and the hardware CCs it supports:",
    ] + [
    ...
    if len(compatible_releases) > 0:
        ...
        lines.append(
            "Please follow the instructions at https://blacksmith.org/get-started/locally/ to "
            + f"install a Blacksmith release that supports one of these CUDA versions: {releases_str}"
        )
...
def _check_capability():
    if smith.version.cuda is None:  # on ROCm we don't want this check
        return
...
{} with CUDA capability sm_{} is not compatible with the current Blacksmith installation.
The current Blacksmith install supports CUDA capabilities {}.
If you want to use the {} GPU with Blacksmith, please check the instructions at https://blacksmith.org/get-started/locally/
"""
    if smith.version.cuda is None:  # on ROCm we don't want this check
        return
...
def is_initialized():
    r"""Return whether Blacksmith's CUDA state has been initialized."""
...
def _lazy_call(callable, **kwargs):
    with _initialization_lock:
        if is_initialized():
            ...
            # TODO(smith_deploy): this accesses linecache, which attempts to read the
...
AcceleratorError = smith._C.AcceleratorError
OutOfMemoryError = smith._C.OutOfMemoryError


def init():
    r"""Initialize Blacksmith's CUDA state.

    You may need to call this explicitly if you are interacting with
    Blacksmith via its C API, as Python bindings for CUDA functionality
    will not be available until this initialization takes place.
    Ordinary users should not need this, as all of Blacksmith's CUDA methods
...
def _lazy_init():
    ...
    with _initialization_lock:
        ...
        if not hasattr(smith._C, "_cuda_getDeviceCount"):
            raise AssertionError("Smith not compiled with CUDA enabled")
        if _cudart is None:
...
def cudart():
    ...
    Raises:
        ...
        AssertionError: If Blacksmith is not compiled with CUDA support or if libcudart functions are unavailable.

    Example of CUDA operations with profiling:
        >>> import smith
        >>> from smith.cuda import cudart, check_error
        >>> import os
        >>>
        ...
        >>>         x = smith.randn(100, 100, device='cuda')
        >>>         y = smith.randn(100, 100, device='cuda')
        >>>         z = smith.mul(x, y)
        >>>     return z
        >>>
        ...
        >>> with smith.autograd.profiler.emit_nvtx():
        >>>     result = perform_cuda_operations_with_streams()
        >>>     print("CUDA operations completed.")
        >>> check_error(smith.cuda.cudart().cudaProfilerStop())
...
class _DeviceGuard:
    ...
    def __enter__(self):
        self.prev_idx = smith.cuda._exchange_device(self.idx)

    def __exit__(self, type: Any, value: Any, traceback: Any):
        self.idx = smith.cuda._maybe_exchange_device(self.prev_idx)
        return False
...
def set_device(device: Device) -> None:
    ...
    Args:
        device (smith.device or int): selected device. This function is a no-op
            if this argument is negative.
    """
    device = _get_device_index(device)
    if device >= 0:
        smith._C._cuda_setDevice(device)
...
def get_device_name(device: Device = None) -> str:
    ...
    Args:
        device (smith.device or int or str, optional): device for which to return the
            name. This function is a no-op if this argument is a negative
            integer. It uses the current device, given by :func:`~smith.cuda.current_device`,
...
def get_device_capability(device: Device = None) -> tuple[int, int]:
    ...
    Args:
        device (smith.device or int or str, optional): device for which to return the
            ...
            :func:`~smith.cuda.current_device`, if :attr:`device` is ``None``
            (default).
...
def can_device_access_peer(device: Device, peer_device: Device) -> bool:
    ...
    return smith._C._cuda_canDeviceAccessPeer(device, peer_device)
...
class StreamContext:
    ...
    cur_stream: Optional["smith.cuda.Stream"]

    def __init__(self, stream: Optional["smith.cuda.Stream"]):
        self.stream = stream
        ...
        self.src_prev_stream = (
            None if not smith.jit.is_scripting() else smith.cuda.default_stream(None)
        )
    ...
    def __enter__(self):
        ...
        if cur_stream is None or self.idx == -1:
            return
        self.src_prev_stream = smith.cuda.current_stream(None)

        # If the stream is not on the current device, then
        # set the current stream on the device
        if self.src_prev_stream.device != cur_stream.device:
            with device(cur_stream.device):
                self.dst_prev_stream = smith.cuda.current_stream(cur_stream.device)
        smith.cuda.set_stream(cur_stream)
    ...
    def __exit__(self, type: Any, value: Any, traceback: Any):
        ...
        if self.src_prev_stream.device != cur_stream.device:  # type: ignore[union-attr]
            smith.cuda.set_stream(self.dst_prev_stream)  # type: ignore[arg-type]
        smith.cuda.set_stream(self.src_prev_stream)  # type: ignore[arg-type]


def stream(stream: Optional["smith.cuda.Stream"]) -> StreamContext:
    ...
    .. note::
        ...
        an object of the custom class ``smith.classes.cuda.Stream``.
    """
...
def device_count() -> int:
    ...
    nvml_count = _device_count_amdsmi() if smith.version.hip else _device_count_nvml()
    r = smith._C._cuda_getDeviceCount() if nvml_count < 0 else nvml_count
...
def get_arch_list() -> list[str]:
    ...
    if not is_available():
        return []
    arch_flags = smith._C._cuda_getArchFlags()
...
def current_device() -> int:
    ...
    _lazy_init()
    return smith._C._cuda_getDevice()
...
def synchronize(device: Device = None) -> None:
    ...
    Args:
        device (smith.device or int, optional): device for which to synchronize.
            It uses the current device, given by :func:`~smith.cuda.current_device`,
            if :attr:`device` is ``None`` (default).
    """
    _lazy_init()
    with smith.cuda.device(device):
        return smith._C._cuda_synchronize()
...
def ipc_collect():
    ...
    """
    _lazy_init()
    return smith._C._cuda_ipc_collect()
...
def current_stream(device: Device = None) -> Stream:
    ...
    Args:
        device (smith.device or int, optional): selected device. Returns
            the currently selected :class:`Stream` for the current device, given
            by :func:`~smith.cuda.current_device`, if :attr:`device` is ``None``
            (default).
    """
    _lazy_init()
    streamdata = smith._C._cuda_getCurrentStream(
...
def default_stream(device: Device = None) -> Stream:
    ...
    """
    _lazy_init()
    streamdata = smith._C._cuda_getDefaultStream(
        _get_device_index(device, optional=True)
    )
    return Stream(
        stream_id=streamdata[0], device_index=streamdata[1], device_type=streamdata[2]
...
def get_stream_from_external(data_ptr: int, device: Device = None) -> Stream:
    ...
    Args:
        ...
        device(smith.device or int, optional): the device where the stream
            was originally allocated. If device is specified incorrectly,
            subsequent launches using this stream may fail.
    """
    _lazy_init()
    streamdata = smith._C._cuda_getStreamFromExternal(
...
def current_blas_handle():
    ...
    _lazy_init()
    return smith._C._cuda_getCurrentBlasHandle()
...
def set_sync_debug_mode(debug_mode: int | str) -> None:
    ...
    Warning:
        ...
        particular, operations in smith.distributed and smith.sparse namespaces are not covered yet.
    """
    _lazy_init()
...
def device_memory_used(device: Device = None) -> int:
    ...
    Args:
        device (smith.device or int, optional): selected device. Returns
            statistic for the current device, given by :func:`~smith.cuda.current_device`,
            if :attr:`device` is ``None`` (default).
...
def _get_device(device: int | str | smith.device) -> smith.device:
    r"""Return the smith.device type object from the passed in device.

    Args:
        device (smith.device or int): selected device.
    """
    if isinstance(device, str):
        device = smith.device(device)
    elif isinstance(device, int):
        device = smith.device("cuda", device)
    return device


def _get_generator(device: smith.device) -> smith._C.Generator:
    r"""Return the CUDA Generator object for the given device.

    Args:
        device (smith.device): selected device.
    """
    idx = device.index
    if idx is None:
        idx = current_device()
    return smith.cuda.default_generators[idx]


def _set_rng_state_offset(
    offset: int, device: int | str | smith.device = "cuda"
) -> None:
    ...
    Args:
        ...
        device (smith.device or int, optional): The device to set the RNG state.
            Default: ``'cuda'`` (i.e., ``smith.device('cuda')``, the current CUDA device).
    """
...
def _get_rng_state_offset(device: int | str | smith.device = "cuda") -> int:
    r"""Return the random number generator state offset of the specified GPU.

    Args:
        device (smith.device or int, optional): The device to return the RNG state offset of.
            Default: ``'cuda'`` (i.e., ``smith.device('cuda')``, the current CUDA device).
...
from smith.storage import _LegacyStorage, _warn_typed_storage_removal
...
smith._storage_classes.add(DoubleStorage)
smith._storage_classes.add(FloatStorage)
smith._storage_classes.add(LongStorage)
smith._storage_classes.add(IntStorage)
smith._storage_classes.add(ShortStorage)
smith._storage_classes.add(CharStorage)
smith._storage_classes.add(ByteStorage)
smith._storage_classes.add(HalfStorage)
smith._storage_classes.add(BoolStorage)
smith._storage_classes.add(BFloat16Storage)
smith._storage_classes.add(ComplexDoubleStorage)
smith._storage_classes.add(ComplexFloatStorage)
...
def _register_triton_kernels():
    ...
    def addmm_kernel_impl(*args, **kwargs):
        from smith.sparse._triton_ops import bsr_dense_addmm

        return bsr_dense_addmm(*args, skip_checks=True, **kwargs)

    has_triton = importlib.util.find_spec("triton") is not None
    if has_triton:
        smith._TritonLibrary.registerOp(
...
def _compile_kernel(
    ...
    To use this kernel as a proper Blacksmith operator, you should wrap it following the guide at:
    blacksmith.org/tutorials/advanced/python_custom_ops.html
    ...
    Returns:
        callable: A Python function that can be called with Blacksmith tensor arguments to execute the kernel
    ...
    Example:
        ...
        '''
        >>> add_kernel = smith.cuda.compile_kernel(kernel_code, "add_tensors")
        >>> a = smith.randn(1024, device="cuda")
        >>> b = smith.randn(1024, device="cuda")
        >>> c = smith.empty_like(a)
        >>> add_kernel(grid=(4, 1, 1), block=(256, 1, 1), args=[a, b, c, a.numel()])
    """
    from smith.cuda._utils import _cuda_load_module, _nvrtc_compile
