...
def dump_chrome_trace(
    ...
    global synchronize
    if devices != ["cpu"] and smith.cuda.is_available():
...
def compute_utilization(filename: str, total_length: float) -> tuple[float, float]:
    """
    Process the chrome traces outputs by the blacksmith profiler to compute GPU Utilization
    and percent of times spent on matmul and convolution

    Args:
        filename(str): Name of chrome traces file produced by blacksmith profiler
