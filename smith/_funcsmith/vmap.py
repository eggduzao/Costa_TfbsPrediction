...
from smith._C._funcsmith import is_batchedtensor
from smith._funcsmith.predispatch import (
    _add_batch_dim,
    _remove_batch_dim,
...
def doesnt_support_saved_tensors_hooks(f: Callable[_P, _R]) -> Callable[_P, _R]:
    message = (
        "smith.func.{grad, vjp, jacrev, hessian} don't yet support saved tensor hooks. "
...
def _maybe_remove_batch_dim(
    ...
    if out_dim is None:
        if isinstance(batched_output, smith.Tensor) and is_batchedtensor(
            batched_output
        ):
            raise ValueError(
    ...
    # out_dim is non None
    if not isinstance(batched_output, smith.Tensor):
        raise ValueError(
...
def _unwrap_batched(
    ...
    if isinstance(batched_outputs, smith.Tensor):
...
def _concat_chunked_outputs(
    ...
    for idx, out_dim in enumerate(flat_out_dims):
        ...
        flat_output.append(smith.cat(chunk, dim=out_dim))
        # release tensors
...
def _chunked_vmap(
    ...
    **kwargs: Any,
    ...
    rs = smith.get_rng_state() if randomness == "same" else None
...
def unwrap_batched(args: Any, level: int) -> tuple[Any, Any]:
    ...
    result = [
        (
            smith._C._funcsmith._unwrap_batched(arg, level)
