...
def dp_knapsack(
    ...
    quantized_memory = smith.tensor(
        [round(m * S) for m in memory], dtype=smith.long, device="cpu"
    )
    runtimes = smith.tensor(runtime, dtype=smith.float32, device="cpu")
    ...
    dp = smith.zeros(
        (n + 1, quantized_max_memory + 1), dtype=smith.float32, device="cpu"
    )
    ...
    for i in range(1, n + 1):
        ...
        if current_memory == 0:
            ...
            dp[i, current_memory:] = smith.maximum(
...
def dp_knapsack_sliding_hirschberg(
    ...
    dp_profile = smith.zeros(dp_profile_size, dtype=smith.float32, device="cpu")
    # Store a candidate for next dp_profile - current dp row + item
    candidate_profile = smith.empty(dp_profile_size, dtype=smith.float32, device="cpu")
    left_profile = smith.empty(dp_profile_size, dtype=smith.float32, device="cpu")
    right_profile = smith.empty(dp_profile_size, dtype=smith.float32, device="cpu")
    ...
    while stack:
        ...
        for index in left_items:
            ...
            smith.add(dp_view[:-memory_item], runtime_item, out=dp_view_candidate)
        ...
        best_split = int(smith.argmax(left_dp_local).item())
