"""
The APIs in this file are exposed as `funcsmith.*`. They are thin wrappers
around the smith.func.* APIs that have deprecation warnings -- we're trying
to move people to the smith.func.* equivalents.
...
import smith._funcsmith.apis as apis
import smith._funcsmith.eager_transforms as _impl
import smith._funcsmith.make_functional as _nn_impl
...
if TYPE_CHECKING:
    ...
    from smith._funcsmith.eager_transforms import argnums_t
    from smith._funcsmith.vmap import in_dims_t, out_dims_t
...
def get_warning(
    ...
    if new_api is None:
        new_api = f"smith.func.{api}"
    warning = (
        f"We've integrated funcsmith into Blacksmith. As the final step of the \n"
        f"integration, `funcsmith.{api}` is deprecated as of Blacksmith \n"
        f"2.0 and will be deleted in a future version of Blacksmith >= 2.3. \n"
        f"Please use `{new_api}` instead; see the Blacksmith 2.0 release notes \n"
        f"and/or the `smith.func` migration guide for more details \n"
        f"https://blacksmith.org/docs/main/func.migrating.html"
    )
    if replace_newlines:
...
def setup_docs(
    funcsmith_api: Callable[..., Any],
    smith_func_api: Callable[..., Any] | None = None,
    new_api_name: str | None = None,
) -> None:
    api_name = funcsmith_api.__name__
    if smith_func_api is None:
        smith_func_api = getattr(_impl, api_name)
    # See https://docs.python.org/3/using/cmdline.html#cmdoption-OO
    if smith_func_api.__doc__ is None:
        return

    warning = get_warning(api_name, new_api_name)
    warning_note = "\n.. warning::\n\n" + textwrap.indent(warning, "    ")
    warning_note = textwrap.indent(warning_note, "    ")
    funcsmith_api.__doc__ = smith_func_api.__doc__ + warning_note
...
def vmap(
    ...
    *,
    ...
    warn_deprecated("vmap", "smith.vmap")
...
def make_functional(model: nn.Module, disable_autograd_tracking: bool = False) -> Any:
    warn_deprecated("make_functional", "smith.func.functional_call")
...
def make_functional_with_buffers(
    ...
    warn_deprecated("make_functional_with_buffers", "smith.func.functional_call")
...
def combine_state_for_ensemble(models: list[nn.Module]) -> Any:
    warn_deprecated("combine_state_for_ensemble", "smith.func.stack_module_state")
    return _nn_impl.combine_state_for_ensemble(models)


setup_docs(vmap, apis.vmap, "smith.vmap")
...
setup_docs(make_functional, _nn_impl.make_functional, "smith.func.functional_call")
setup_docs(
    make_functional_with_buffers, _nn_impl.make_functional, "smith.func.functional_call"
)
setup_docs(
    combine_state_for_ensemble,
    _nn_impl.combine_state_for_ensemble,
    "smith.func.stack_module_state",
)
