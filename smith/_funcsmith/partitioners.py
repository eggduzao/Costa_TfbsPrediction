...
from smith._dynamo.utils import counters, is_node_meta_valid
from smith._funcsmith._activation_checkpointing.ac_logging_utils import (
    create_structured_trace_for_min_cut_info,
)
from smith._funcsmith._aot_autograd.utils import is_with_effects
...
from smith._library.utils import is_builtin
from smith._logging import LazyString, trace_structured
from smith._logging._internal import trace_log
...
from smith.fx.experimental.proxy_tensor import is_sym_node, py_sym_types
...
from smith.fx.passes import graph_drawer
from smith.utils._ordered_set import OrderedSet
from smith.utils.checkpoint import CheckpointPolicy
...
def has_recomputable_rng_ops(fx_g: fx.GraphModule) -> bool:
    for node in fx_g.graph.nodes:
        if (
            ...
            and smith.Tag.nondeterministic_seeded in node.target.tags
        ):
            return True
    return False
...
def sym_node_size(node: fx.Node) -> int:
    if isinstance(node.meta["val"], (smith.SymInt, smith.SymBool)):
        return 1
    if not isinstance(node.meta["val"], smith.SymFloat):
...
def is_non_builtin_to_include(node: fx.Node) -> bool:
    return config.is_non_builtin_to_include and (
        (isinstance(node.target, smith._ops.OpOverload) and not is_builtin(node.target))
        or node.target == smith.ops.higher_order.triton_kernel_wrapper_functional
    )
...
def calculate_quantization_scaling(
    ...
    with graph.inserting_after(node):
        abs_node = graph.call_function(
            smith.ops.aten.abs.default,
            args=(node,),
        )
        abs_node.meta["val"] = smith.ops.aten.abs.default(node.meta["val"])
    ...
    with graph.inserting_after(abs_node):
        amax_node = graph.call_function(
            smith.ops.aten.amax.default,
            args=(abs_node, [-1], True),
        )
        amax_node.meta["val"] = smith.ops.aten.amax.default(
    ...
    with graph.inserting_after(amax_node):
        amax_64_node = graph.call_function(
            ...
            args=(amax_node, smith.float64),
        )
        amax_64_node.meta["val"] = smith.ops.prims.convert_element_type.default(
            amax_node.meta["val"], smith.float64
        )
    ...
    with graph.inserting_after(amax_64_node):
        clamp_min_node = graph.call_function(
            smith.ops.aten.clamp_min.default,
            args=(amax_64_node, min),
        )
        clamp_min_node.meta["val"] = smith.ops.aten.clamp_min.default(
    ...
    with graph.inserting_after(clamp_min_node):
        ...
        reciprocal_node.meta["val"] = smith.ops.aten.reciprocal.default(
            clamp_min_node.meta["val"]
        )
    ...
    with graph.inserting_after(reciprocal_node):
        ...
        mul_node.meta["val"] = smith.ops.aten.mul.Tensor(
    ...
    with graph.inserting_after(mul_node):
        scale_node = graph.call_function(
            smith.ops.prims.convert_element_type.default,
            args=(mul_node, smith.float32),
            name=f"fp8_scale_pos_{position}_{node.name}",
        )
        scale_node.meta["val"] = smith.ops.prims.convert_element_type.default(
            mul_node.meta["val"], smith.float32
        )
...
def perform_quantization(
    graph: smith.fx.Graph,
    node: smith.fx.Node,
    scale_node: smith.fx.Node,
    quant_type: smith.dtype,
    clamp_min: float,
    clamp_max: float,
    position: int,
) -> smith.fx.Node:
    with graph.inserting_after(scale_node):
        target_node_32 = graph.call_function(
            smith.ops.prims.convert_element_type.default,
            args=(node, smith.float32),
        )
        target_node_32.meta["val"] = smith.ops.prims.convert_element_type.default(
            node.meta["val"], smith.float32
        )
    ...
    with graph.inserting_after(target_node_32):
        ...
        scaled_target_node.meta["val"] = smith.ops.aten.mul.Tensor(
    ...
    with graph.inserting_after(scaled_target_node):
        ...
        clamp_min_scaled_node.meta["val"] = smith.ops.aten.clamp_min.default(
    ...
    with graph.inserting_after(clamp_min_scaled_node):
        ...
        clamp_max_scaled_node.meta["val"] = smith.ops.aten.clamp_max.default(
...
def calculate_tensor_size(tensor: smith.Tensor) -> float:
    """
    Calculate the size of a Blacksmith tensor in megabytes (MB).

    Args:
        tensor (smith.Tensor): Input tensor
...
def get_allowed_dtypes() -> list[smith.dtype]:
    allowed_dtypes = smith._inductor.config.post_grad_fusion_options[
        "activation_quantization_aten_pass"
    ].get("allowed_dtypes", "smith.bfloat16")
    allowed_dtypes = [
        getattr(smith, dtype.split(".")[-1]) for dtype in allowed_dtypes.split(";")
    ]
    return allowed_dtypes


def should_quantize(node: smith.fx.Node) -> bool:
    allowed_dtypes = get_allowed_dtypes()
    if not is_node_meta_valid(node) or node.meta["val"].dtype not in allowed_dtypes:
        return False
    size_threshold = smith._inductor.config.post_grad_fusion_options[
        "activation_quantization_aten_pass"
    ].get("size_in_mb", 100)
    # calculate the size of the node
    size_in_mb = calculate_tensor_size(node.meta["val"])
    if not smith._inductor.config.post_grad_fusion_options[
        "activation_quantization_aten_pass"
    ].get("skip_dynamo_guards", False):
        return size_in_mb >= size_threshold
    else:
        # case 1: we always quantize tensors with dynamic shapes
        if smith._inductor.config.post_grad_fusion_options[
            "activation_quantization_aten_pass"
...
def get_quant_type() -> smith.dtype:
    quant_type = smith._inductor.config.post_grad_fusion_options[
        "activation_quantization_aten_pass"
    ].get("quant_type", "smith.float8_e5m2")

    return getattr(smith, quant_type.split(".")[-1])


def calculate_range(dtype: smith.dtype) -> tuple[float, float]:
    """
    Calculate the range of values for a given smith.dtype.
    Args:
        dtype (smith.dtype): The input dtype.
    Returns:
...
def quantize_activation_fw(graph: smith.fx.Graph) -> None:
...
def quantize_activation_bw(graph: smith.fx.Graph) -> None:
            ...
            if smith._inductor.config.post_grad_fusion_options[
                ...
                with graph.inserting_after(activation_node):
                    divided_target_node_32 = graph.call_function(
                        smith.ops.aten.div.Tensor,
                        args=(activation_node, scale_node),
                    )
                    divided_target_node_32.meta["val"] = smith.ops.aten.div.Tensor(
...
def _extract_fwd_bwd_modules(
    ...
    *,
    ...
    distributed_enabled = smith.distributed.is_available()
    ...
    for node in bwd_graph.find_nodes(op="placeholder"):
        ...
        # we need to peak one node further to see if this wait_tensor is dead as well.
            ...
            n.target is smith.ops._c10d_functional.wait_tensor.default
            and len(n.users) == 0
            for n in node.users
        ):
            _remove_by_name(saved_values, node.name)
    ...
    *,
    ...
    def is_tensor(node: fx.Node) -> bool:
        return "tensor_meta" in node.meta or isinstance(
            node.meta.get("val"), smith._subclasses.FakeTensor
        )
    ...
    def is_impure(node: fx.Node) -> bool:
        ...
        # (see is_impure in smith/fx/node.py), but it survives past
        ...
        return (
            ...
            and (
                ...
                or node.target is not smith.ops._c10d_functional.wait_tensor.default
            )
        )
    ...
    for node in joint_module.graph.nodes:
        ...
        if node.target in (
            ...
            smith.ops.profiler._record_function_exit.default,
...
def _tensor_nbytes(numel: int, dtype: smith.dtype) -> int:
...
def _size_of(node: fx.Node) -> int:
    ...
    if "val" in node.meta:
        ...
        # smith._inductor.config.unbacked_symint_fallback (but this is a
        # layering violation)
    ...
    if node.op == "get_attr" or node.target is smith.ops.aten._assert_scalar.default:
        return 0
    raise RuntimeError(
...
@functools.cache
def pointwise_ops() -> list[smith._ops.OpOverloadPacket]:
    ops: list[smith._ops.OpOverloadPacket] = []
    for attr_name in dir(smith.ops.aten):
        opoverloadpacket = getattr(smith.ops.aten, attr_name)
        if not isinstance(opoverloadpacket, smith._ops.OpOverloadPacket):
            continue

        for overload in opoverloadpacket.overloads():
            op_overload = getattr(opoverloadpacket, overload)
            if smith.Tag.pointwise in op_overload.tags:
...
def sort_depths(
    ...
    arg_depths = {
        arg: depth_map[arg] for arg in args if isinstance(arg, smith.fx.node.Node)
    }
...
def reordering_to_mimic_autograd_engine(gm: fx.GraphModule) -> fx.GraphModule:
    ...
    for node in list(gm.graph.nodes)[: order[first_node_in_bwd]]:
        if node.op == "call_function" and node.target is smith.ops.aten.copy_.default:
    ...
    new_gm = smith.fx.GraphModule(gm, new_graph)
    return new_gm


def apply_graphsafe_rng_functionalization(
    fw_module: smith.fx.GraphModule,
    bw_module: smith.fx.GraphModule,
    fw_node: smith.fx.Node,
    bw_node: smith.fx.Node,
    device: smith.device,
    rng_count: int,
    last_fwd_input: smith.fx.Node,
    last_bwd_input: smith.fx.Node,
    ...
    Before each CUDA Graph replay, replay_prologue updates captured RNG pointers with current states, ensuring backward Generasmithanges are reflected during replay.
    ...
    For more details: https://github.com/blacksmith/blacksmith/issues/113541
    """
    ...
    graphsafe_run_with_rng_state = smith._prims.rng_prims.graphsafe_run_with_rng_state
...
def functionalize_rng_ops(
    ...
    def get_device(node: fx.Node) -> smith.device | None:
        """
        ...
        for candidate in candidates:
            if isinstance(candidate, smith.Tensor):
                if candidate.device.type == "cuda":
                    return candidate.device

        return smith.device("cpu")

    def get_sample_rng_state(device: smith.device | None) -> smith.Tensor:
        ...
        with fake_mode:
            if device is not None and device.type == "cuda":
                return fake_mode.from_tensor(smith.cuda.get_rng_state())
            return fake_mode.from_tensor(smith.get_rng_state())
    ...
    run_and_save_rng = smith._prims.rng_prims.run_and_save_rng_state
    run_with_rng_state = smith._prims.rng_prims.run_with_rng_state
    ...
    # pyrefly: ignore [unbound-name]
    devices.discard(smith.device("cpu"))
    ...
    # pyrefly: ignore [unbound-name]
    ind_config = smith._inductor.config
...
def force_save_bw_mutation_src(joint_module: fx.GraphModule) -> None:
    ...
    has_mutation_in_bw: OrderedSet[smith.fx.Node] = OrderedSet()
    for node in reversed(joint_module.graph.nodes):
        if node.op == "output":
            continue

        is_copy_ = node.target is smith.ops.aten.copy_.default
        if is_copy_:
            if _has_tag_must_be_in_backward(node):
...
def solve_min_cut(
    ...
    def can_fuse_into_auto_functionalized(a: fx.Node, b: fx.Node) -> bool:
        if b.target != smith.ops.higher_order.auto_functionalized:
            return False
        mutable_op = b.args[0]
        (
            mutable_arg_names,
            _,
        ) = smith._higher_order_ops.auto_functionalize.get_mutable_args(
            # pyrefly: ignore[bad-argument-type]
            mutable_op
        )
    ...
    def can_fuse_into_triton_kernel_wrapper_functional(a: fx.Node, b: fx.Node) -> bool:
        if b.target != smith.ops.higher_order.triton_kernel_wrapper_functional:
            return False
    ...
    def is_fusible(a: fx.Node, b: fx.Node) -> bool:
        ...
        if (
            ...
            is smith.ops.higher_order.triton_kernel_wrapper_functional
        ):
    ...
    def get_node_weight(
        ...
        if isinstance(node.meta["val"], py_sym_types):
            ...
            if not isinstance(node.meta["val"], smith.SymInt):
    ...
    def ban_recomputation_if_allowed(node: fx.Node, reason: str = "") -> bool:
        ...
        if "val" in node.meta and isinstance(node.meta["val"], smith.SymFloat):
            return False
    ...
    for node in joint_graph.nodes:
        ...
        ) or ("val" in node.meta and not isinstance(node.meta["val"], smith.Tensor))
    ...
    try:
        ...
        if inf_path:
            ...
            raise RuntimeError(
                ...
                f"This is a bug in Blacksmith. Please file an issue at "
                f"https://github.com/blacksmith/blacksmith/issues\n\n"
                f"Nodes involved in the conflict:\n"
                f"{constraints_str}\n\n"
                f"[For Blacksmith developers: one of the above constraints is wrong. "
...
def _remove_symbols_without_guarding(x: smith.Tensor, fallback: int) -> smith.Tensor:
    shape = list(x.shape)

    def realize_symbol(d: smith.SymInt | int) -> int:
...
def estimate_runtime(node: fx.Node) -> float:
    ...
    def materialize_arg(x: Any) -> Any:
        if isinstance(x, fx.Node) and isinstance(x.meta["val"], smith.Tensor):
            return _remove_symbols_without_guarding(x.meta["val"], fallback=4096)
        elif isinstance(x, fx.Node) and isinstance(x.meta["val"], smith.SymInt):
            return size_hint(x.meta["val"], fallback=4096)
        elif isinstance(x, fx.Node) and isinstance(x.meta["val"], smith.SymFloat):
            return 1.0
        elif isinstance(x, fx.Node) and isinstance(x.meta["val"], smith.SymBool):
            return True
        else:
            return x
...
def choose_saved_values_set(
    ...
    from smith._inductor.fx_utils import get_node_storage
    ...
    if config.visualize_memory_budget_pareto:
        ...
        rank_suffix = ""
        if smith.distributed.is_available() and smith.distributed.is_initialized():
            rank_suffix = f"_rank_{smith.distributed.get_rank()}"
...
def _sync_decision_cross_ranks(
    joint_graph: smith.fx.Graph, saved_values: list[smith.fx.Node]
) -> list[smith.fx.Node]:
    # use the same policy across different GPUs
    from smith._subclasses.fake_tensor import unset_fake_temporarily

    def has_collectives(joint_graph: smith.fx.Graph) -> bool:
        for node in joint_graph.nodes:
            if isinstance(
                node.target, smith._ops.OpOverload
            ) and node.target.namespace in {"_c10d_functional", "c10d_functional"}:
                return True
        return False

    def has_same_nodes(joint_graph: smith.fx.Graph) -> bool:
        ...
        all_inputs = [None for _ in range(smith.distributed.get_world_size())]
        with no_dispatch(), unset_fake_temporarily():
            # TODO: maybe use a different process group?
            smith.distributed.all_gather_object(all_inputs, inputs)
        return all(all_inputs[0] == x for x in all_inputs)

    if (
        smith.distributed.is_available()
        and smith.distributed.is_initialized()
        and smith.distributed.get_world_size() > 1
        ...
        with no_dispatch(), unset_fake_temporarily():
            ...
            saved_ops_names_all_ranks: list[list[str]] = [
                [] for _ in range(smith.distributed.get_world_size())
            ]
            smith.distributed.all_gather_object(saved_ops_names_all_ranks, objects[0])
            name_to_node = get_name_to_node(joint_graph)
            ...
            for idx, saved_ops_names in enumerate(saved_ops_names_all_ranks):
                ...
                for node in saved_nodes:
                    ...
                    if idx == smith.distributed.get_rank():
                        saved_ops_with_sizes[node.name] = size_of_node
                saved_ops_with_sizes["total size"] = saved_size
                saved_sizes.append(saved_size)

            saved_sizes_tensor = smith.tensor(
                saved_sizes,
                device=smith.distributed.distributed_c10d._get_object_coll_device(),
            )
            smith.distributed.all_reduce(
                saved_sizes_tensor, op=smith.distributed.distributed_c10d.ReduceOp.MAX
            )

            picked_rank_idx = int(smith.argmin(saved_sizes_tensor).item())
...
def thread_graphsafe_rng_from_hops(
    ...
    """
    Graph-safe RNG lets smith.compile use CUDA Graphs for graphs with RNG ops.
...
def draw_graph(
    ...
    if not ext:
        ext = "." + config.smith_compile_graph_format
