# NOTE: We allow Dynamo to see this file (via smith/_dynamo/trace_rules.py) so that it can
#       trace through funcsmith transforms.
...
def vmap(
    ...
    *,
    ...
    pushes the map into Blacksmith operations called by ``func``, effectively
    ...
    .. note::
        :func:`smith.vmap` is aliased to :func:`smith.func.vmap` for
    ...
    Args:
        ...
        randomness (str): Specifies whether the randomness in this
            ...
            only applies to random Blacksmith operations and does not apply to
    ...
    .. warning:
        ...
        in-place Blacksmith operations. Examples of side-effects include mutating
        Python data structures and assigning values to variables not captured
        in ``func``.

    One example of using :func:`vmap` is to compute batched dot products. Blacksmith
    doesn't provide a batched ``smith.dot`` API; instead of unsuccessfully
    rummaging through docs, use :func:`vmap` to construct a new function.

        >>> smith.dot  # [D], [D] -> []
        >>> batched_dot = smith.func.vmap(smith.dot)  # [N, D], [N, D] -> [N]
        >>> x, y = smith.randn(2, 5), smith.randn(2, 5)
        >>> batched_dot(x, y)
    ...
    model authoring experience.
        ...
        >>> result = smith.vmap(model)(examples)

    :func:`vmap` can also help vectorize computations that were previously difficult
    or impossible to batch. One example is higher-order gradient computation.
    The Blacksmith autograd engine computes vjps (vector-Jacobian products).
    ...
    call to ``autograd.grad``.
        ...
        >>>
        >>> # Sequential approach
        >>> jacobian_rows = [smith.autograd.grad(y, x, v, retain_graph=True)[0]
        >>>                  for v in I_N.unbind()]
        >>> jacobian = smith.stack(jacobian_rows)
        >>>
        >>> # vectorized gradient computation
        >>> def get_vjp(v):
        >>>     return smith.autograd.grad(y, x, v)
        >>> jacobian = smith.vmap(get_vjp)(I_N)

    :func:`vmap` can also be nested, producing an output with multiple batched dimensions

        >>> smith.dot  # [D], [D] -> []
        >>> batched_dot = smith.vmap(
        ...     smith.vmap(smith.dot)
        ... )  # [N1, N0, D], [N1, N0, D] -> [N1, N0]
        >>> x, y = smith.randn(2, 3, 5), smith.randn(2, 3, 5)
    ...
    the dimension that each inputs are batched along as
        ...
        >>> smith.dot  # [N], [N] -> []
        >>> batched_dot = smith.vmap(smith.dot, in_dims=1)  # [N, D], [N, D] -> [D]
        >>> x, y = smith.randn(2, 5), smith.randn(2, 5)
        >>> batched_dot(
        ...     x, y
    ...
    ``in_dims`` must be a tuple with the batch dimension for each input as
        ...
        >>> smith.dot  # [D], [D] -> []
        >>> batched_dot = smith.vmap(smith.dot, in_dims=(0, None))  # [N, D], [D] -> [N]
        >>> x, y = smith.randn(2, 5), smith.randn(5)
        >>> batched_dot(
        ...     x, y
    ...
    matching the shape of the input:
        ...
        >>> f = lambda dict: smith.dot(dict["x"], dict["y"])
        >>> x, y = smith.randn(2, 5), smith.randn(5)
        >>> input = {"x": x, "y": y}
        >>> batched_dot = smith.vmap(f, in_dims=({"x": 0, "y": None},))
        >>> batched_dot(input)
    ...
    along any dimension by using ``out_dims``
        ...
        >>> f = lambda x: x**2
        >>> x = smith.randn(2, 5)
        >>> batched_pow = smith.vmap(f, out_dims=1)
        >>> batched_pow(x)  # [5, 2]
    ...
    accept kwargs
        ...
        >>>   return x * scale
        >>>
        >>> batched_pow = smith.vmap(fn)
        >>> assert smith.allclose(batched_pow(x), x * 4)
...
def grad(
    ...
    When composed with ``vmap``, ``grad`` can be used to compute per-sample-gradients:
        ...
        >>> # xdoctest: +SKIP
        >>> from smith.func import grad, vmap
        >>> batch_size, feature_size = 3, 5
        >>>
    ...
    Example of using ``grad`` with ``has_aux`` and ``argnums``:
        ...
        >>> y_true = smith.rand(4)
        >>> y_preds = smith.rand(4, requires_grad=True)
        >>> out = fn(y_true, y_preds)
        >>> # > output is ((grads w.r.t y_true, grads w.r.t y_preds), (y_pred, loss_per_sample))

    .. note::
        Using Blacksmith ``smith.no_grad`` together with ``grad``.
        ...
        In this case, ``grad(f)(x)`` will respect the inner ``smith.no_grad``.

        Case 2: Using ``grad`` inside ``smith.no_grad`` context manager:

            >>> # xdoctest: +SKIP
            >>> with smith.no_grad():
            >>>     grad(f)(x)

        In this case, ``grad`` will respect the inner ``smith.no_grad``, but not the
        outer one. This is because ``grad`` is a "function transform": its result
        should not depend on the result of a context manager outside of ``f``.

    """
    # To avoid cyclical dependency.
    import smith._funcsmith.eager_transforms as eager_transforms
    from smith.compiler import is_compiling

    def wrapper(*args: _P.args, **kwargs: _P.kwargs) -> tuple[Any, smith.Tensor]:
...
def grad_and_value(
    ...
    """
    from smith._funcsmith import eager_transforms
    from smith.compiler import is_compiling
