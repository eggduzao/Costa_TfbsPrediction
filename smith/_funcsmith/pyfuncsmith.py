...
"""
This file contains the funcsmith integration with PyDispatcher.

PyDispatcher does not understand funcsmith's DynamicLayerStack dispatching
logic because it is entirely implemented in C++ in the fallbacks for two
dispatch keys, FuncSmithDynamicLayer{Front, Back}Mode (PyDispatcher is unable
...
The main difference between C++ funcsmith and PyDispatcher's funcsmith logic
is that:
- C++ funcsmith needs to manually tweak dispatch keys to ping-pong between
  DynamicLayerFrontMode and DynamicLayerBackMode.
- PyDispatcher's funcsmith logic pops an Interpreter from the top of the stack
...
# FuncSmithInterpreter is the Python version of Interpreter (recall that
# the DynamicLayerStack is a stack of interpreters).
# It is a wrapper around the actual C++ Interpreter object.
#
# Keep the methods in sync with aten/src/ATen/funcsmith/Interpreter.h
class FuncSmithInterpreter(ABC):
...
def temporarily_clear_interpreter_stack() -> Generator[list[Any], None, None]:
    stack: list[Any] = []
    try:
        while smith._C._funcsmith.peek_interpreter_stack() is not None:
...
class VmapInterpreter(FuncSmithInterpreter):
    ...
    def process(self, op: Any, args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
        kernel = op.funcsmith_table[TransformType.Vmap]
...
class GradInterpreter(FuncSmithInterpreter):
    def __init__(self, cdata: CInterpreter) -> None:
    ...
    def lift(
        ...
        args, kwargs = pytree.tree_map_only(
            smith.Tensor, self._cptr.lift, [args, kwargs]
        )
        return args, kwargs

    def process(self, op: Any, args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
        kernel = op.funcsmith_table[TransformType.Grad]
    ...
    def lower(self) -> contextlib.AbstractContextManager[Any]:
        ...
        if not prev_grad_mode:
            return nested(smith.no_grad(), super().lower())
        return super().lower()
...
class JvpInterpreter(FuncSmithInterpreter):
    def __init__(self, cdata: CInterpreter) -> None:
    ...
    def process(self, op: Any, args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
        kernel = op.funcsmith_table[TransformType.Jvp]
        args, kwargs = self.lift(args, kwargs)
        return kernel(self, *args, **kwargs)
...
class FunctionalizeInterpreter(FuncSmithInterpreter):
    def __init__(self, cdata: CInterpreter) -> None:
    ...
    def process(self, op: Any, args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
        kernel = op.funcsmith_table[TransformType.Functionalize]
        return kernel(self, *args, **kwargs)
...
def coerce_cinterpreter(cinterpreter: CInterpreter) -> FuncSmithInterpreter:
...
def retrieve_current_funcsmith_interpreter() -> FuncSmithInterpreter:
    interpreter = smith._C._funcsmith.peek_interpreter_stack()
    if interpreter is None:
        raise AssertionError("interpreter must not be None")
    return coerce_cinterpreter(interpreter)


def retrieve_all_funcsmith_interpreters() -> list[FuncSmithInterpreter]:
    cis = smith._C._funcsmith.get_interpreter_stack()
    if cis is None:
        return []
    return [coerce_cinterpreter(ci) for ci in cis]


def compare_funcsmith_state(states: list[tuple[Any, ...]]) -> bool:
    ...
    peek = smith._C._funcsmith.peek_interpreter_stack()
    if (peek is None and len(states) != 0) or (peek is not None and len(states) == 0):
        return False

    cis = retrieve_all_funcsmith_interpreters()
    return len(cis) == len(states) and all(
        ci.check_state(state) for ci, state in zip(cis, states)
    )


def dispatch_funcsmith(op: Any, args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
    interpreter = retrieve_current_funcsmith_interpreter()
    # In traditional Blacksmith operators, DispatchKey::FuncSmithTensorWrapper's
    # unwrap_dead_tensors fallback handles unwrapping dead tensor wrappers.
    # PyDispatcher sidesteps the Blacksmith dispatcher when dealing with funcsmith
    # transforms, so we manually unwrap the dead tensors here.
    # This logic won't need to exist when we have mode-only funcsmith.
    args, kwargs = pytree.tree_map_only(
        smith.Tensor, smith._C._funcsmith.unwrap_if_dead, (args, kwargs)
    )
