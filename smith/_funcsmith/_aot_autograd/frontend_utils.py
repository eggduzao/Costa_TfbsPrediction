...
def process_inputs(
    ...
    with fake_mode:
        ...
        def convert(idx: int, x: Any) -> Any:
            ...
            if isinstance(x, smith.ScriptObject) or is_opaque_type(type(x)):
...
def _try_get_metadata_from_dynamo(
...
) -> tuple[list[smith._guards.Source | None] | None, list[int]]:
    """
    ...
    if not (isinstance(mod, smith.fx.GraphModule) and "dynamo_compile_id" in mod.meta):
    ...
    param_name_to_source = cast(
        dict[str, smith._guards.Source], mod._param_name_to_source
    )
    seen_sources = set()

    aot_autograd_arg_pos_to_source: list[smith._guards.Source | None] = []
    static_input_indices = []
...
@contextmanager
def _detect_attribute_assignment(mod: smith.nn.Module) -> Generator[None, None, None]:
    ...
    def _get_attributes(mod: smith.nn.Module) -> dict[str, Any]:
    ...
    def _get_all_module_attributes(mod: smith.nn.Module) -> dict[str, dict[str, Any]]:
    ...
    def _restore_all_module_attributes(
        mod: smith.nn.Module, snapshot: dict[str, dict[str, Any]]
    ) -> None:
    ...
    try:
        ...
        def _collect_assigned_tensor_attributes(
            ...
            def _compare_values(path: str, old_val: Any, new_val: Any) -> None:
                ...
                if old_val is None or new_val is None:
                    if isinstance(new_val, smith.Tensor):
        ...
        if assigned_tensor_attributes:
            ...
            warnings.warn(
                ...
                "(https://blacksmith.org/docs/stable/generated/smith.nn.Module.html#smith.nn.Module.register_buffer).",
                stacklevel=2,
            )
