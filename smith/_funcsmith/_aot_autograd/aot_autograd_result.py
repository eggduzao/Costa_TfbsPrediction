...
Similar to how smith._inductor.output_code provides OutputCode classes for inductor
...
import smith
from smith._dynamo.precompile_context import BackendCacheArtifact
...
from smith._inductor.utils import should_use_remote_fx_graph_cache
...
class CompiledBackward(GenericCompiledBackward[CompiledFxGraph], FxGraphCacheLoadable):
    ...
    def post_compile(
        ...
        # This is done by _wrapped_bw_compiler in smith/_dynamo/backends/common.py
        # But since on cache hit we do not call the bw_compiler, we need to reapply the disable
        return smith._dynamo.disable(  # type: ignore[return-value]
...
class SerializedGraphModule:
    fn: Callable[[dict[Any, Any], str], smith.nn.Module]
    args: tuple[Any, ...]

    def __init__(self, gm: smith.fx.GraphModule) -> None:
        self.fn, self.args = gm.__reduce__()

    def deserialize(self) -> smith.fx.GraphModule:
...
def serialize_graph_module(gm: smith.fx.GraphModule) -> SerializedGraphModule:
...
class GenericAOTAutogradResult(Generic[TForward, TBackward]):
    ...
    def wrap_post_compile(
        ...
        """
        from smith._dynamo.utils import CompileEventLogger, dynamo_timed
