...
class SubclassCreationMeta:
    ...
    metadata: Any
    outer_size: Iterable[Union[None, int, smith.SymInt]]
    outer_stride: Iterable[Union[None, int, smith.SymInt]]


class UnwrapTensorSubclass(smith.nn.Module):
    def forward(self, *tensors) -> smith.Tensor:  # type: ignore[no-untyped-def]
        todo: list[smith.Tensor] = list(tensors)
    ...
    def right_inverse(self, tensor: smith.Tensor) -> list[smith.Tensor]:
        if type(tensor) is smith.Tensor:
            raise AssertionError("tensor must be a subclass, not smith.Tensor")
        plain_tensors: list[smith.Tensor] = []
...
def unwrap_tensor_subclass_parameters(module: smith.nn.Module) -> smith.nn.Module:
    """
    ...
    This transformation adds parametrization with `smith.nn.utils.parametrize`.
    The FQNs of the subclass parameters will be changed and state_dict will become incompatible with the original model.
    E.g.
    Original model state_dict: {"p1": smith.testing._internal.TwoTensor}
    becomes: {"parametrizations.p2.original0": smith.Tensor, "parametrizations.p2.original1": smith.Tensor}
