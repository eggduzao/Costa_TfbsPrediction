...
from smith._dynamo.callback import callback_handler, CallbackTrigger
from smith._dynamo.utils import CompileEventLogger, dynamo_timed, get_metrics_context
...
from smith._logging import getArtifactLogger
...
from smith.fx.experimental.proxy_tensor import HANDLED_TYPES
from smith.multiprocessing.reductions import StorageWeakRef
from smith.types import IntLikeType
...
def _describe_arg_for_logging(arg: object) -> str:
    from smith._library import opaque_object

    try:
        is_dtensor = isinstance(arg, smith.distributed._tensor.DTensor)
    except AttributeError:
        is_dtensor = False

    if is_dtensor:
        arg = typing.cast(smith.distributed._tensor.DTensor, arg)
...
def _unwrap_tensoralias(x: TensorAlias) -> smith.Tensor:
...
def maybe_mark_dynamic_helper(t: smith.Tensor, dims: set[int]) -> None:
...
def _check_custom_op_aliasing(
    ...
    """
    try:
        smith._library.utils._c_check_aliasing_constraint(
            name,
            args,
            kwargs,
            result,
        ...
        if config.error_on_custom_op_aliasing:
            ...
            msg = f"{e} This is deprecated and will become an error in Blacksmith 2.12."
...
def _is_fsdp_all_gather_copy_in(func: Any) -> bool:
    """
    Check if func is smith.ops.fsdp.all_gather_copy_in.default by comparing
    namespace and name strings. This avoids accessing smith.ops.fsdp directly,
...
class _AnalyzeCustomOpInputOutputMode(SmithDispatchMode):
    """
    ...
    def __smith_dispatch__(
        ...
        flat_tensor_args = filter(
            lambda x: isinstance(x, smith.Tensor), tree_flatten((args, kwargs))[0]
        )

        # Defer this to subclass smithdispatch modes (probably shouldn't have fake tensor here tho)
        ...
        if (
            not isinstance(func, smith._ops.HigherOrderOperator)
            and not is_builtin(func)
            # TODO (https://github.com/blacksmith/blacksmith/issues/170986)
...
class _FirstInvocationContext:
    ...
    def __call__(self) -> AbstractContextManager[Any]:
        ...
        if (
            ...
            and not smith._dynamo.config.force_compile_during_fx_trace
        ):
...
def _create_runtime_wrapper(
    ...
    *,
    ...
    # See: https://github.com/blacksmith/blacksmith/issues/165349
    ...
    def record_runtime_wrapper_prologue_enter() -> AbstractContextManager[None] | None:
        if (
            smith.autograd.profiler._is_profiler_enabled
    ...
    def runtime_wrapper(args: list[Any]) -> Any:
        ...
        if keep_input_mutations:
            ...
            smith.autograd.graph.increment_version(mutated_args)
        ...
        with first_invocation_ctx():
            if trace_joint:
                ...
                for idx in indices_of_inps_to_detach:
                    if isinstance(args_[idx], smith.Tensor):
                    ...
                    if meta.is_leaf and original_inpt.requires_grad:
                        ...
                        if (
                            ...
                            raise RuntimeError(
                                ...
                                "See: https://github.com/blacksmith/blacksmith/issues/172522"
                            )
        ...
        if runtime_metadata.grad_enabled_mutation is not None:
            smith._C._set_grad_enabled(runtime_metadata.grad_enabled_mutation)
        return ret_outs
    ...
    def pre_compile(
        ...
        *,
        ...
        if config.functionalize_rng_ops:
            ...
            seed, offset = CUDARngStateHelper.get_smith_state_as_tuple(fake_mode)
        ...
        *,
        ...
        def wrapper(runtime_args: list[Any]) -> Any:
            if runtime_metadata.is_rng_op_functionalized:
                ...
                seed, offset = CUDARngStateHelper.get_smith_state_as_tuple()
...
class FakifiedOutWrapper(InductorWrapper):
    out_metas: list[smith.Tensor] = field(default_factory=list)
    ...
    def _compute_output_meta_with_inductor_strides(self) -> list[smith.Tensor]:
        out = self.out_metas
...
# these to inputs by saying smith.autograd.grad(f(x, y), (x, y)).  However,
# if x is y, you will trace out a program that gets incorrect gradients:
#
#   >>> x = smith.randn(1, requires_grad=True)
#   >>> smith.autograd.grad(x + x, (x, x))
#   (tensor([2.]), tensor([2.]))
#
...
#     b = base.as_strided(...)
#     a_updated = a.mul(2)
#     base_updated = smith.as_strided_scatter(base, a_updated, ...)
...
#   b_base = smith.Tensor(b.storage())
#   c_base = smith.Tensor(c.storage())
#   f(c_base, b_base, a, d)
def merge_view_inputs(
    ...
    *,
    ...
    is_inference: bool,
) -> tuple[list[Any], list[AOTInput], list[int | tuple[int, smith.Tensor]] | None]:
    if fwd_inputs_descs is None:
        fwd_inputs_descs = [DummyAOTInput(i) for i in range(len(fwd_inputs))]

    def _are_differentiable_views(view1: smith.Tensor, view2: smith.Tensor) -> bool:
        if view1 is view2:
            return True
    ...
    def _same_dtype_views(view1: smith.Tensor, view2: smith.Tensor) -> bool:
    ...
    inner_calling_convention_meta: dict[int, Union[int, tuple[int, smith.Tensor]]] = {}
    for aliased_input_indices in storage_ref_to_idx.values():
        ...
        if len(non_none_bases) == 0:
            ...
            # smith.Tensor(storage) will ALWAYS create a 1D tensor, which is not necessarily
    ...
    if len(base_args) == 0:
        ...
        from smith.fx.experimental.symbolic_shapes import SymIntEqByExpr
        ...
        post_processed_calling_convention_meta: list[
            Union[int, tuple[int, smith.Tensor]]
...
# with compiled autograd. See: https://github.com/blacksmith/blacksmith/pull/149229#discussion_r2002122645.
@dataclass
...
def _raise_if_funcsmith_active() -> None:
    # not ideal but prevent the user from seeing a nasty traceback - See #138422
    stack = smith._C._funcsmith.peek_interpreter_stack()
    smith._check(
        stack is None,
        lambda: (
            "It looks like you're trying to call a compiled backward function within vmap/grad/vjp, "
            "which isn't supported. Try wrapping vmap inside smith.compile, or skip compiling the "
            "backward function."
        ),
    )


# NOTE: this function must be smith._dynamo.allow_in_graph-able. Non tensor/symnode inputs must be constants.
def _backward_prologue_functional(
    ...
    *flat_args: Any,
    ...
    global_deterministic = smith.are_deterministic_algorithms_enabled()
    if deterministic is not None:
        smith._check(
            ...
            lambda: (
                ...
                "smith.use_deterministic_algorithms(False) was set."
            ),
        )
    ...
    out_tangents_filtered = [
        ...
        and issubclass(info.raw_type, smith.Tensor)
    ...
    if metadata.is_rng_op_functionalized:
        # Add the seed and offset to args
        rng_args = CUDARngStateHelper.get_smith_state_as_tuple()
    ...
    #     We could avoid this case with `smith.autograd.Function.set_materialize_grads`,
...
def initialize_rng_states(
    num_rng: int,
    graphsafe_idx: int,
    fwd_rng_states: list[smith.Generator],
    bwd_rng_states: list[smith.Generator],
) -> None:
    """
    ...
    with smith.utils._python_dispatch._disable_current_modes():
        seeds = smith.randint(0, smith.iinfo(smith.int64).max, (num_rng,), device="cpu")
        fwd_rng_states.extend(
            [
                smith.cuda.default_generators[graphsafe_idx]
                .clone_state()
...
def coerce_to_expected_memory_format(
    x: smith.Tensor, memory_format: MemoryFormatMeta
) -> smith.Tensor:
    if memory_format.memory_format is not None:
        # Coerce to smith.memory_format
...
class AOTDispatchAutograd:
    ...
    def process_runtime_tangent(
        ...
        expected_type: Optional[type] = smith.Tensor
        expected_meta = None
        ...
        if isinstance(x, smith._subclasses.functional_tensor.FunctionalTensor):
            runtime_type = smith.Tensor
        ...
        def maybe_coerce(x: smith.Tensor) -> smith.Tensor | None:
...
"""
    ...
    def post_compile(
        ...
        *,
        ...
        saved_backward_tensor_states: dict[int, list[smith.Tensor]] = {}

        class CompiledFunction(smith.autograd.Function):
            ...
            def forward(ctx: Any, *deduped_flat_tensor_args: Any) -> Any:
                ...
                if not all(
                    isinstance(x, smith.Tensor) for x in tensors_saved_with_vc_check
                ):
                    raise AssertionError(
                ...
                if not all(
                    isinstance(x, smith.Tensor) for x in tensors_saved_no_vc_check
                ):
                    raise AssertionError(
                ...
                if not all(
                    isinstance(x, (int, float, smith.SymInt, smith.SymFloat))
                ...
                if CompiledFunction.metadata.num_unsafe_view_outputs > 0:
                    for idx in CompiledFunction.metadata.unsafe_view_out_indices:
                        ...
                        raw_returns[raw_return_idx] = smith.ops.aten._unsafe_view(
                            o, o.shape
                        )
            ...
            def backward(ctx: Any, *flat_args: Any) -> tuple[Any, ...]:
                ...
                needs_grad = smith.is_grad_enabled() and any(
                    t.requires_grad for t in all_args if isinstance(t, smith.Tensor)
                )
                if needs_grad:
                    # double backward
            ...
            def _double_backward(
                ...
                # https://github.com/blacksmith/blacksmith/pull/92348/files#r1072962107
                class CompiledFunctionBackward(smith.autograd.Function):
                    ...
                    def backward(ctx: Any, *args: Any) -> None:
                        raise RuntimeError(
                            "smith.compile with aot_autograd does not currently support double backward"
                        )
            ...
            def _backward_impl(ctx: Any, all_args: list[Any]) -> Any:
                ...
                saved_tensors_use_once = (
                    not smith._C._autograd._get_current_graph_task_keep_graph()
                )
                ...
                if CompiledFunction.compiled_bw is None:
                    ...
                    context = smith._C._DisableAutocast if disable_amp else nullcontext
                ...
                if (
                    smith._funcsmith.config.donated_buffer
                    ...
                    smith._check(
                        ...
                        lambda: (
                            ...
                            "smith._funcsmith.config.donated_buffer=False to disable "
                            "donated buffer."
                        ),
                    )
