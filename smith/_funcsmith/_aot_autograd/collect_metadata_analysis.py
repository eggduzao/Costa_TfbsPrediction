...
from smith._subclasses.meta_utils import safe_is_leaf
from smith.fx.experimental.proxy_tensor import disable_autocast_cache
from smith.fx.experimental.symbolic_shapes import is_concrete_int
...
static_input_logger = getArtifactLogger("smith._dynamo", "cudagraph_static_inputs")
...
#
# Unlike funcsmith's variant, this doesn't use the funcsmith level system,
# instead it directly uses Blacksmith's conventional dispatcher to hit the
...
def run_functionalized_fw_and_collect_metadata(
    ...
    *,
    ...
    def inner(*flat_args: Any) -> ViewAndMutationMeta:
        ...
        prior_grad_enabled = smith.is_grad_enabled()
        ...
        if prior_autocast_states != _get_autocast_states():
            raise RuntimeError(
                ...
                "Dynamo will only insert autocast context managers (e.g. with smith.autocast(..)) into the graph, "
                "which will unwind all of their mutations to autocast state before the graph exits. "
                "If you encounter this error while using smith.compile, please file a bug."
            )
        ...
        for arg, f_arg in zip(flat_args, flat_f_args):
            ...
            requires_grad = isinstance(f_arg, smith.Tensor) and f_arg.requires_grad
        ...
        out_storage_to_metadata_key_to_tensors: collections.defaultdict[
            ...
            collections.defaultdict[MetadataKey, set[smith.Tensor]],
        ...
        for o in flat_f_outs:
            if isinstance(o, smith.Tensor):
                ...
                # without causing an error in eager mode, we will simple hide the aliasing from autograd during smith.compile
                ...
                # in eager but fail to during smith.compile, but it has the benefit that this code has much better performance.
                ...
                is_cur_tensor_multi_out_view = isinstance(
                    o, FunctionalTensor
                ) and smith._functionalize_is_multi_output_view(  # type: ignore[attr-defined]
                    o.elem
                )
        ...
        intermediate_bases: list[smith.Tensor] = []
        ...
        #     non_leaf_tensor = smith.ones(4, requires_grad=True).clone()
        #
        #     # Using no_grad() and _unsafe_preserve_version_counter to simulate the .data = operation
        #     with smith.no_grad(), smith.autograd._unsafe_preserve_version_counter(x):
        ...
        for o, desc in zip(flat_f_outs, flat_f_outs_descs):
            functional_tensor_storage_changed = isinstance(
                o, FunctionalTensor
            ) and smith._functionalize_was_storage_changed(  # type: ignore[attr-defined]
                o.elem
            )
            curr_storage = (
                None
            ...
            outs_with_identical_metadata_that_require_grad: list[smith.Tensor] = (
                []
            ...
            if isinstance(grad_fn, smith.autograd.function.BackwardCFunction):
                is_result_of_custom_autograd_fn = True
...
from a multi-output view call"
                ...
                # See https://github.com/blacksmith/blacksmith/issues/100348 for this case.
            ...
            out_info = OutputAliasInfo(
                ...
                requires_grad=isinstance(o, smith.Tensor) and o.requires_grad,
        ...
        def _plain_fake_tensor_like_subclass(x: Any) -> smith.Tensor:
        ...
        def _is_subclass_mutated_input_tangent_always_subclass(inp: object) -> bool:
            return (
                isinstance(inp, smith.nested._internal.nested_tensor.NestedTensor)
                or smith._funcsmith.config.disable_guess_zero_tangent_for_mutated_input_subclass
            )
        ...
        f_input_tangents_pairs = [
            ...
            # See smith/csrc/autograd/python_function.cpp, use_zeros_like.
            (
                (
        ...
        if smith._dynamo.compiled_autograd.in_compiled_autograd_region:
            ...
            static_input_indices = [
                ...
                if (isinstance(arg, smith.nn.Parameter) or i in passed_indices)
            ]
        ...
        if smith.is_grad_enabled() != prior_grad_enabled:
            grad_enabled_mutation = smith.is_grad_enabled()
