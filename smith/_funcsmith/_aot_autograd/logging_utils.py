...
def setup_stacktrace_preservation_hooks(roots: list[smith.autograd.graph.Node]) -> None:
    def iter_graph(
        roots: list[smith.autograd.graph.Node],
    ) -> Iterator[smith.autograd.graph.Node]:
        if not roots:
            return
        seen = set()
        q = collections.deque()
    ...
    def get_prehook(stack_: list[str], seq_nr: int) -> Callable[[Any], None]:
        def prehook(grad_output: Any) -> None:
            ...
            if not callback_set:
                smith.autograd.variable.Variable._execution_engine.queue_callback(  # type: ignore[attr-defined]
...
def format_guard_bug_msg(aot_config: AOTConfig, expected: str) -> str:
    return (
        ...
        "This indicates a guard bug in AOTAutograd or Dynamo, please file a bug to Blacksmith."
    )
