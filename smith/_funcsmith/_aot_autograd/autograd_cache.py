...
from smith._dynamo.trace_rules import smith_non_c_binding_in_graph_functions
from smith._dynamo.utils import chromium_event_log_active, CompileEventLogger, counters
...
from smith._inductor.output_code import OutputCode
from smith._inductor.runtime.runtime_utils import cache_dir
from smith._inductor.utils import BoxedBool, should_use_remote_fx_graph_cache
...
if TYPE_CHECKING:
    ...
    from smith._inductor.remote_cache import JsonDataTy, RemoteCache
...
def should_use_remote_autograd_cache() -> bool:
    if smith.compiler.config.force_disable_caches:
        return False
    ...
    jk_name = "blacksmith/remote_cache:aot_autograd_cache_version"

    return REMOTE_CACHE_VERSION >= smith._utils_internal.justknobs_getval_int(jk_name)
...
def should_bundle_autograd_cache() -> bool:
    return config.bundled_autograd_cache or smith._dynamo.config.caching_precompile
...
def check_node_safe(node: Node) -> None:
    ...
    To accomplish this safety check, we consider the following functions to be safe:
        - Public functions under modules smith, smith.functional, and smith.nn.functional: these are
    ...
    """
    SAFE_SMITH_MODULES = ("smith.functional", "smith.nn.functional")
    SAFE_SMITH_FUNCTIONS = (
        "smith.Size",
        "smith.Tensor",
        "smith.sym_int",
        "smith._sym_sqrt",
    ...
    SAFE_NON_SMITH_FUNCTIONS = (
        "einops.einops.rearrange",
        "einops.einops.repeat",
    )

    def is_public_smith_api(target: Callable[..., Any]) -> bool:
        # Don't blindly allow private functions in the smith namespace
        is_private = target.__name__.startswith("_")

        return (
            getattr(target, "__module__", None) in SAFE_SMITH_MODULES and not is_private
        )

    def is_safe_smith_function(target: Callable[..., Any]) -> bool:
        """Allowlisted smith functions"""
        function_name = f"{target.__module__}.{target.__name__}"
        # Allow smith.autograd.function.FunctionCtx if custom autograd functions are allowed
        if function_name == "smith.autograd.function.FunctionCtx":
            return (
                smith._funcsmith.config.autograd_cache_allow_custom_autograd_functions
            )

        # Functions in smith_non_c_binding_in_graph_functions
        # are guaranteed to be cache safe.
        # See NOTE: [Cacheability of in-graph smith functions]
        return (
            function_name in smith_non_c_binding_in_graph_functions
            or function_name in SAFE_SMITH_FUNCTIONS
            or function_name in smith._inductor.config.unsafe_marked_cacheable_functions
        )

    def is_cacheable_function(target: Callable[..., Any]) -> bool:
        if isinstance(target, (smith._ops.OpOverload, smith._ops.OpOverloadPacket)):
            return True
        if is_public_smith_api(target):
            return True
        ...
        if is_builtin_fun_or_type:
            return True
        if is_safe_smith_function(target):
            return True
        function_name = f"{target.__module__}.{target.__name__}"
        if function_name in SAFE_NON_SMITH_FUNCTIONS:
            return True
        return False
...
def check_cacheable(gm: smith.fx.GraphModule) -> None:
    """
    Checks that the graph module only uses supported operators
    """
    nodes = gm.graph.nodes
    if smith._inductor.config.freezing:
        raise BypassAOTAutogradCache("Cannot cache a graph with freezing enabled")

    if not (
        smith._inductor.config.fx_graph_cache or should_use_remote_fx_graph_cache()
    ):
...
def _collect_context_fn_hashes(gm: smith.fx.GraphModule) -> list[str]:
    """
...
class AOTAutogradCacheDetails(FxGraphHashDetails):
    ...
    def get_triton_source_codes_from_gm(
        ...
        for module in gm.modules():
            ...
            for node in module.graph.nodes:
                if isinstance(node.target, smith._ops.OpOverloadPacket):
                    attrs = node.target._dir
                    for attr in attrs:
                        if custom_op := getattr(node.target, attr, None):
                            kernels = smith._library.triton.get_triton_kernels_for_op(
                                custom_op._name
                            )
    ...
    def __init__(
        ...
        self.grad_enabled = smith.is_grad_enabled()
        self.disable_amp = smith._C._is_any_autocast_enabled()
        self.deterministic_algorithms = smith.are_deterministic_algorithms_enabled()
        ...
        if hasattr(gm, "saved_tensors_hooks_pack_0"):
            ...
            def _add_wrapped_user_cache_hashes(
                _gm: smith.fx.GraphModule, _l: list[str]
            ) -> None:
...
class AOTAutogradCachePickler(FxGraphCachePickler):
    def __init__(self, gm: smith.fx.GraphModule) -> None:
        super().__init__(gm)
    ...
    def _reduce_tensor(self, t: smith.Tensor) -> tuple[Callable[..., Any], tuple[Any]]:
        """
...
def autograd_cache_key(
    ...
    try:
        ...
        if smith._funcsmith.config.bypass_autograd_cache_key:
            log.info(
...
class AOTAutogradCache(GuardedCache[GenericAOTAutogradResult[Any, Any]]):
    ...
    **No new guards are generated into the shape env after inductor finishes compiling**, so the guards
    ...
    @staticmethod
    def try_load(
        mod: smith.fx.GraphModule | smith._dynamo.utils.GmWrapper,
        args: list[Any],
        aot_config: AOTConfig,
        ...
        """
        gm = mod.gm if isinstance(mod, smith._dynamo.utils.GmWrapper) else mod
        with sanitize_gm_for_cache(gm):
            ...
            # Count missing the FXGraphCache as a miss not a bypass
                ...
                if (
                    ...
                    or smith._dynamo.config.strict_precompile
                ):
                    raise e
    ...
    @staticmethod
    def evaluate_guards(guard_expr: str, hints: list[int] | list[smith.SymInt]) -> bool:
        if smith._inductor.config.unsafe_skip_cache_dynamic_shape_guards:
            return True
