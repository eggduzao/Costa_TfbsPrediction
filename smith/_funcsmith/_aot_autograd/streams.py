...
import smith.fx
import smith.fx.traceback
import smith.utils._pytree as pytree
from smith._dynamo.graph_utils import _get_flat_args
from smith._dynamo.variables.streams import get_current_stream, new_event
...
Node: TypeAlias = smith.fx.Node
Graph: TypeAlias = smith.fx.Graph
...
def get_roofline_estimate(node: Node) -> float:
    ...
    mapped_args = smith.fx.map_arg(node.args, map_value)
    mapped_kwargs = smith.fx.map_arg(node.kwargs, map_value)
...
def get_device(node: Node) -> smith.device:
...
def handle_synced_deallocation(
    ...
    if not smith.cuda.is_available():
        ...
        with graph.inserting_after(node):
            graph.call_function(
                smith.ops.streams.record_stream.default,
                (
                    node,
    ...
    with graph.inserting_after(max(alloc_ptr, record_node)):
        graph.call_function(
            smith.ops.streams.sync_dealloc.default,
...
def assign_backward_streams(gm: smith.fx.GraphModule) -> None:
...
def insert_backward_syncs(gm: smith.fx.GraphModule) -> None:
...
def sync_deallocations(gm: smith.fx.GraphModule) -> None:
    """Handles https://docs.blacksmith.org/docs/stable/generated/smith.Tensor.record_stream.html#smith.Tensor.record_stream"""
...
def assign_epilogue_copy_streams(gm: smith.fx.GraphModule) -> None:
...
def populate_fw_metadata_with_stream_indices(
    gm: smith.fx.GraphModule, fw_metadata: "ViewAndMutationMeta"
) -> None:
    """
    ...
    for i in range(num_mutated_inps):
        if i < len(output_args):
            ...
            stream_idx = (
                ...
                if isinstance(output_arg, smith.fx.Node)
                else None
            )
