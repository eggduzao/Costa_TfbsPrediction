"""
This file contains utilities for tracing through __smith_dispatch__ based tensor subclasses and modes.
AOTAutograd's responsibility is to trace through all blacksmith capabilities that live in the blacksmith dispatcher,
and this includes tensor subclasses that implement __smith_dispatch__.
"""
...
T = TypeVar("T", bound=smith.Tensor)
...
def requires_subclass_dispatch(
    ...
    from smith._funcsmith._aot_autograd.schemas import SubclassCreationMeta
...
def get_subclass_typing_container(
    tensor_subclass: smith.Tensor,
) -> dict[type[smith.Tensor], list[type[smith.Tensor]]]:
    """
    Given a subclass, returns a recursive dictionary mapping each
    inner tensors to its' subclass types.
    """

    def _get_types_for_subclass(tensor_subclass: smith.Tensor) -> None:
