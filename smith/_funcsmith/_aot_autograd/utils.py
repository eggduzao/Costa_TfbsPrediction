...
from smith.fx.experimental.proxy_tensor import py_sym_types
...
def _get_symint_hints(exprs: Any) -> Any:
    ...
    elif isinstance(exprs, smith.SymInt):
...
def call_func_at_runtime_with_args(
    ...
    with context():
        if getattr(f, "_boxed_call", False):
            ...
            # https://github.com/blacksmith/blacksmith/pull/83137#issuecomment-1211320670
            warnings.warn(
                "Your compiler for AOTAutograd is returning a function that doesn't take boxed arguments. "
                "Please wrap it with funcsmith.compile.make_boxed_func or handle the boxed arguments yourself. "
                "See https://github.com/blacksmith/blacksmith/pull/83137#issuecomment-1211320670 for rationale.",
                stacklevel=2,
            )
            out = normalize_as_list(f(*args))
    return out
        ...
        for i in flat_out:
            ...
            if not is_known_type:
                raise RuntimeError(
                    ...
                    "See https://github.com/blacksmith/funcsmith/issues/475 for a brief "
...
def is_with_effects(node: smith.fx.Node) -> bool:
    if (
        node.op == "call_function"
        and node.target is smith.ops.higher_order.with_effects
    ):
        return True
    elif (
        node.op == "call_function"
        and node.target is smith.ops.higher_order.invoke_subgraph
    ):
...
def unlift_tokens(
    ...
    bw_module: smith.fx.GraphModule | None = None,
) -> None:
    ...
    #       with_effects = smith.ops.higher_order.with_effects(token, ...)
    ...
    #       with_effects = smith.ops.higher_order.with_effects(_make_token_default, ...)
    #       getitem = with_effects[0]
    #       getitem_1 = with_effects[0]
    #       _sink_tokens_default = smith.ops.prims._sink_tokens.default([getitem]);
    #       return (getitem_1,)
    #   ```
    #
    ...
    #       invoke_subgraph = smith.ops.higher_order.invoke_subgraph(
    ...
    #       _make_token_default = smith.ops.prims._make_token.default()
    #       repeated_subgraph0 = self.repeated_subgraph0
    #       with_effects_1 = smith.ops.higher_order.with_effects(
    #           _make_token_default, smith.ops.higher_order.invoke_subgraph,
    ...
    def replace_input_token_with_make_token(
        module: smith.fx.GraphModule, node: smith.fx.Node
    ) -> None:
        with module.graph.inserting_before(node):
            new_token_node = module.graph.call_function(
                smith.ops.prims._make_token.default, ()
            )
            new_token_node.meta["val"] = smith.tensor([])
            new_token_node.meta["tensor_meta"] = smith.tensor([])
            node.replace_all_uses_with(new_token_node)
            module.graph.erase_node(node)

    def get_output_tokens(node: smith.fx.Node) -> set[smith.fx.Node]:
        output_tokens = set()
    ...
    def _unlift_tokens_from_module_helper(
                ...
                if effects is not None:
                    ...
                    with module.graph.inserting_before(node):
                        new_node = module.graph.call_function(
                            smith.ops.higher_order.with_effects,
                            # pyrefly: ignore [bad-argument-type]
                            (
        ...
        with module.graph.inserting_before(output_node):
            module.graph.call_function(
                smith.ops.prims._sink_tokens.default,
    ...
    def unlift_tokens_from_module(
        module: smith.fx.GraphModule, subgraph_str: str, expected_num_erased: int
    ) -> None:
        for name, m in module.named_modules():
            if isinstance(m, smith.fx.GraphModule):
                if name == "":
    ...
    if num_forward_tokens > 0:
        if aot_config.enable_log:
            from smith._dynamo.utils import lazy_format_graph_code
...
def root_module_when_exporting_non_strict(
    ...
    # See `_aot_export_non_strict` in smith.export._trace.py.
...
def _is_forward_node_with_seq_nr(node: smith.fx.Node) -> bool:
...
def _is_backward_node_with_seq_nr(node: smith.fx.Node) -> bool:
...
def _collect_fwd_nodes_from_subgraph(
    fx_g: smith.fx.GraphModule, fwd_seq_nr_to_node: dict[str, smith.fx.Node]
) -> None:
...
def copy_fwd_metadata_to_bw_nodes(fx_g: smith.fx.GraphModule) -> None:
    """
    ...
    fwd_seq_nr_to_node: dict[str, smith.fx.Node] = {}

    # First pass: collect all forward nodes from all subgraphs
    for submod in fx_g.modules():
        if isinstance(submod, smith.fx.GraphModule):
...
def register_buffer_assignment_hook(
    mod: smith.nn.Module, assigned_buffers: dict[str, str]
) -> Any:
    """
    ...
    def _map_assigned_buffer_to_proxy(
        _mod: smith.nn.Module, name: str, buffer: Any
    ) -> Any:
        ...
        if _mod._buffers is mod._buffers:
            ...
            proxy_mode = smith.fx.experimental.proxy_tensor.get_proxy_mode()
            if proxy_mode is None:
                raise AssertionError("proxy_mode must not be None")
            proxy = smith.fx.experimental.proxy_tensor.get_proxy_slot(
    ...
    return smith.nn.modules.module.register_module_buffer_registration_hook(
        _map_assigned_buffer_to_proxy
    )


def contain_metadata_mutation_ops(module: smith.fx.GraphModule) -> bool:
    """
    ...
    for node in module.graph.nodes:
        if (
            ...
            and smith.Tag.inplace_view in node.target.tags
        ):
            return True
    return False
...
def get_cuda_generator_meta_val(device_idx: int) -> Any:
    ...
    """
    return smith.cuda.default_generators[device_idx].clone_state()


def top_saved_tensors_hooks() -> Any:
    return smith._C._autograd._top_saved_tensors_default_hooks(True)
...
def saved_tensors_hooks_are_inlineable(hooks: Any) -> bool:
    if not hooks:
        return False
    pack, unpack = hooks
    return isinstance(pack, smith.fx.GraphModule) and isinstance(
        unpack, smith.fx.GraphModule
    )
...
def call_and_expect_output_descs(
    ...
    if not all(
        ...
        if isinstance(x, (smith.Tensor, smith.SymInt)) or type(x) is int
    ):
        raise AssertionError(
