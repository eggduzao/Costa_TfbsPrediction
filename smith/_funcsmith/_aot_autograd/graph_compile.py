...
from smith._guards import CompileContext, TracingContext
...
from smith.fx.experimental.symbolic_shapes import fx_placeholder_vals, guard_or_true
from smith.fx.graph_module import GraphModule
from smith.fx.passes._tensorify_python_scalars import tensorify_python_scalars
...
def aot_stage1_graph_capture(
    ...
    aot_state.fw_metadata.deterministic = smith.are_deterministic_algorithms_enabled()
    ...
    with maybe_skip_decompose(aot_config):
        ...
        if aot_state.needs_autograd and not aot_config.pre_dispatch:
            ...
            if smith._funcsmith.config.remat_using_tags_for_fwd_loss_bwd_graph:
                from smith._funcsmith._activation_checkpointing.remat_using_tags_for_fwd_loss_bwd_graph_pass import (
    ...
    if config.selective_decompose:
        from smith.fx.experimental.proxy_tensor import selective_decompose
...
def aot_stage2_export(
    ...
    if not isinstance(compiled_fn, smith.fx.GraphModule):
...
def _apply_tensorify_python_scalars(module: smith.fx.GraphModule) -> None:
    """
...
def _log_inference_graph(
    ...
    if aot_config.enable_log:
        trace_structured(
            "artifact",
            metadata_fn=lambda: {
                "name": "smith._funcsmith.config",
                "encoding": "string",
            },
            payload_fn=lambda: smith._funcsmith.config.get_serializable_config_copy(),
        )
...
def collect_bw_donated_buffer_idxs(
    ...
    # See: https://github.com/blacksmith/blacksmith/pull/141308#issuecomment-2495798947
    #
...
class InvokeSubgraphHopGraphs:
    ...
    new_fw_hop_gm: Optional[smith.fx.GraphModule] = None
    new_bw_hop_gm: Optional[smith.fx.GraphModule] = None
    new_num_sym_nodes: Optional[int] = None
    new_num_saved_nodes: Optional[int] = None


def prepare_for_partitioner(
    mod: smith.fx.GraphModule, num_primals: int, num_fw_outputs: int
    ...
    out = smith.fx.GraphModule(mod, new_graph)
    return out


def _get_partition_fn(
    fw_hop_node: smith.fx.Node, aot_config: AOTConfig
) -> tuple[bool, Callable[..., tuple[smith.fx.GraphModule, smith.fx.GraphModule]]]:
    """
    ...
    partition_fn: Callable[..., tuple[smith.fx.GraphModule, smith.fx.GraphModule]] = (
        aot_config.partition_fn
    )
    if (
        fw_hop_node.target == smith._higher_order_ops.invoke_subgraph
        ...
        if callable(hop_partition_fn):
            ...
            match hop_partition_fn:
                case "default_partition":
                    partition_fn = smith._funcsmith.partitioners.default_partition
                case "min_cut_rematerialization_partition":
                    partition_fn = smith._funcsmith.partitioners.min_cut_rematerialization_partition
                case _:
                    raise ValueError(
...
def run_joint_graph_passes_on_hops(
    joint_gm: smith.fx.GraphModule,
    joint_inputs: Any,
    aot_config: AOTConfig,
) -> smith.fx.GraphModule:
    """
    This pass runs the joint graph passes on the HOP graph. In smith.compile, we
    ...
    def num_outputs(mod: smith.fx.GraphModule) -> int:
        return len(mod.graph.find_nodes(op="output")[0].args[0])

    def num_inputs(mod: smith.fx.GraphModule) -> int:
    ...
    for node in bw_hop_nodes:
        ...
        if not isinstance(fw_hop_gm, smith.fx.GraphModule):
            raise AssertionError(
        ...
        if not isinstance(joint_hop_gm, smith.fx.GraphModule):
            raise AssertionError(
    ...
    def add_new_hop_gm(new_subgraph_mod: smith.fx.GraphModule, name: str) -> str:
    ...
    def propagate_meta_info(
        new_hop_gm: smith.fx.GraphModule,
        new_call_function_node: smith.fx.Node,
        old_call_function_node: smith.fx.Node,
    ) -> None:
...
def create_wrap_fn(
    ...
    from smith.fx.experimental.proxy_tensor import maybe_enable_thunkify
...
def prepare_hook_gm(
    ...
    from smith._funcsmith._aot_autograd.graph_capture import _create_graph
...
def maybe_inline_graph_saved_tensors_hooks(
    ...
    mode = smith._funcsmith.config.saved_tensors_hooks_filtering_mode
    allow_set = None
    exclude_set = None
    ...
    for saved in fw_outs_saved_for_bw:
        if ((allow_set is not None) and (saved.name not in allow_set)) or (  # type: ignore[union-attr]
            ...
            if isinstance(saved.meta["val"], smith.Tensor):  # type: ignore[union-attr]
                fw_outs_packed_tensors.append(saved)
            continue

        val = saved.meta["val"]  # type: ignore[union-attr]
        if not isinstance(val, smith.Tensor):
            continue
        ...
        for out_idx, _n in enumerate(pytree.tree_leaves(fw_pack_out_args)):
            if not isinstance(_n, smith.fx.Node):
            ...
            if _n.op == "placeholder" or _n in fw_outs_inner_set:
                ...
                fw_g.erase_node(_n)
            if isinstance(n.meta["val"], smith.Tensor):
        ...
        for out_idx, (unp_in_n, out_n, val) in enumerate(
            ...
            if isinstance(val, smith.Tensor) or is_sym:
...
def _aot_stage2a_partition(
    ...
    fw_metadata: ViewAndMutationMeta,
    aot_config: AOTConfig,
) -> tuple[smith.fx.GraphModule, smith.fx.GraphModule, int, int, list[int], list[Any]]:
    """
    ...
    with smith.no_grad():
        ...
        with context(), track_graph_compiling(aot_config, "joint"):
            ...
            if callable(smith._funcsmith.config.joint_custom_pass):
                # pyrefly: ignore [bad-assignment]
                fx_g = smith._funcsmith.config.joint_custom_pass(fx_g, joint_inputs)
            ...
            for idx, node in enumerate(fw_outs_saved_for_bw):
                ...
                elif isinstance(node, smith.fx.Node) and "val" in getattr(
                    node, "meta", {}
                ):
            ...
            for node in fw_outs_saved_for_bw:
                if isinstance(node, smith.fx.Node) and node.meta.get(
            ...
            if smith._funcsmith.config.donated_buffer:
        ...
        bw_outs: Sequence[smith.fx.Node] = bw_output.args[0]  # type: ignore[assignment]
...
def _aot_stage2b_bw_compile(
    ...
    with smith.no_grad():
        ...
        # forwards
        with track_graph_compiling(aot_config, "backward"), smith._C._DisableAutocast():
            ...
            for i in range(len(placeholder_list)):
                ...
                if not isinstance(ph_arg, smith.Tensor):
                    continue
...
def aot_stage2_autograd(
    ...
    and returns a wrapped smith.autograd.Function with a forward and backward.
    """
...
def _cache_autograd_info(
    ...
    _indices_of_inps_to_detach: list[int],
    num_symints_saved_for_bw: int,
    bw_module: Optional[smith.fx.GraphModule],
) -> tuple[
    Optional[GenericAOTAutogradResult[Any, Any]],
    Callable[..., Any],
]:
...
def _aot_stage2b_compile_forward_or_inference(
    ...
    *,
    ...
    if is_inference:
        ...
        grad_ctx = smith.no_grad
        autocast_ctx = smith._C._DisableAutocast
