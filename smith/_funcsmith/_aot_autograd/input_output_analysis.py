...
from smith._funcsmith._aot_autograd.schemas import PlainTensorMeta
from smith._guards import StorageOverlap
...
def remove_dupe_metadata(
    ...
    subclass_tangent_meta = [
        PlainTensorMeta(
            0, memory_format=MemoryFormatMeta(memory_format=smith.contiguous_format)
        )
...
def create_synthetic_base_metadata(
    ...
    synthetic_base_info: list[int | tuple[int, smith.Tensor]],
    outer_args: list[Any],
    inner_args: list[Any],
...
def compute_overlapping_inputs(
    ...
    if smith._inductor.config.is_fbcode():
        if symbolic and num_aliases > 400:
            ...
            if justknobs_check(
                "blacksmith/compiler:aliased_inputs_with_mutation_and_dyn_shapes_killswitch",
                False,
            ):
...
def _graph_input_names(gm: smith.fx.GraphModule) -> list[str]:
    return [node.name for node in gm.graph.find_nodes(op="placeholder")]


def _graph_output_names(gm: smith.fx.GraphModule) -> list[Any]:
