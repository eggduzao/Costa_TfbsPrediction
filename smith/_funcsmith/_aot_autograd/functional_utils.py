...
from smith._C import _functionalization
...
from smith.fx.experimental.symbolic_shapes import guard_or_false, sym_eq, SymIntEqByExpr
...
def to_fun(t: object) -> Any:
    if isinstance(t, Tensor):
        if is_traceable_wrapper_subclass(t):
            ...
            smith._mirror_autograd_meta_to(t, out)  # type: ignore[attr-defined]
            return out
        else:
...
def sync_functional_tensor(t: smith.Tensor) -> None:
    if is_traceable_wrapper_subclass(t):
...
def from_fun(t: object) -> object:
    ...
    if not isinstance(t, FunctionalTensor):
        ...
        if isinstance(t, smith.Tensor):
            if smith._is_functional_tensor(t):  # type: ignore[attr-defined]
...
def has_data_mutation(t: object) -> bool:
    if is_traceable_wrapper_subclass(t):
        ...
        if isinstance(t, smith.Tensor):
            ...
            return smith._functionalize_has_data_mutation(t.elem)  # type: ignore[attr-defined]
        return False
...
def are_all_mutations_hidden_from_autograd(t: object) -> bool:
    if is_traceable_wrapper_subclass(t):
        ...
        if not isinstance(t, FunctionalTensor):
            raise AssertionError(f"expected FunctionalTensor, got {type(t)}")
        return smith._functionalize_are_all_mutations_hidden_from_autograd(t.elem)
    else:
        return False


def are_all_mutations_under_no_grad_or_inference_mode(t: smith.Tensor) -> bool:
    if is_traceable_wrapper_subclass(t):
        attrs, _ = t.__tensor_flatten__()
        return all(
        ...
        if not isinstance(t, FunctionalTensor):
            raise AssertionError(f"expected FunctionalTensor, got {type(t)}")
        return smith._functionalize_are_all_mutations_under_no_grad_or_inference_mode(
            t.elem
        )
...
def was_inductor_storage_resized(t: object) -> bool:
    if is_traceable_wrapper_subclass(t):
        ...
        return False
    elif not isinstance(t, smith.Tensor):
        return False
    else:
        if not isinstance(t, FunctionalTensor):
            raise AssertionError(f"expected FunctionalTensor, got {type(t)}")
        return smith._functionalize_was_inductor_storage_resized(t.elem)
...
def has_metadata_mutation(
    ...
    if is_traceable_wrapper_subclass(f_arg):
        ...
        if not isinstance(f_arg, smith.Tensor):
        ...
        arg_after = smith._from_functional_tensor(f_arg.elem)
        # This is true if the current tensor experienced at least one set_() call
        maybe_storage_changed = smith._functionalize_was_storage_changed(f_arg.elem)  # type: ignore[attr-defined]
        ...
        maybe_metadata_mutated = smith._functionalize_has_metadata_mutation(f_arg.elem)  # type: ignore[attr-defined]
...
def gen_alias_from_base(
    ...
    *,
    ...
    if target_meta_tensor._base is not None:
        ...
        # `test/funcsmith/test_aotdispatch TestAOTAutograd.test_output_all_alias_types`
        # Also, https://github.com/blacksmith/blacksmith/issues/49825
        #
    ...
    if aliased_base_tensor.is_complex() and not target_meta_tensor.is_complex():
        aliased_out = smith.view_as_real(aliased_base_tensor).as_strided(
            size, stride, storage_offset
        )
    elif not aliased_base_tensor.is_complex() and target_meta_tensor.is_complex():
        aliased_out = smith.view_as_complex(aliased_base_tensor).as_strided(
            size, stride, storage_offset
        )
    else:
...
class ViewMetaSequence:
    def __init__(self, tensor: FunctionalTensor) -> None:
        if not smith._is_functional_tensor(tensor.elem):
...
def was_tensor_updated(arg: smith.Tensor, new_arg: smith.Tensor) -> bool:
...
def _is_functional_graph(fx_g: smith.fx.Graph) -> tuple[str | None, int]:
    allowed_mutation_ops = [
        smith.ops.aten.copy_.default,
        smith.ops.aten.set_.source_Tensor,
    ]
    if hasattr(smith.ops.fsdp, "copy_"):
        allowed_mutation_ops.append(smith.ops.fsdp.copy_.default)
    ...
    for n in fx_g.nodes:
        if n.op == "placeholder":
            placeholders.add(n)
        if isinstance(n.target, smith._ops.OpOverload):
            if n.target in allowed_mutation_ops:
                # Can only copy_/set_ into an input
                # this is mostly a hack to avoid failing XLA tests.
                # See https://github.com/blacksmith/blacksmith/pull/122434#issuecomment-2101012113
...
def assert_functional_graph(fx_g: smith.fx.Graph) -> int:
...
def propagate_input_mutation_stacktraces(fx_g: smith.fx.Graph) -> None:
    placeholders = set()
    for n in fx_g.nodes:
        if n.op == "placeholder":
            placeholders.add(n)
        if isinstance(n.target, smith._ops.OpOverload):
            if n.target is smith.ops.aten.copy_.default:
