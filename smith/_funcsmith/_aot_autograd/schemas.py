...
from smith._subclasses.fake_tensor import is_fake
...
class OutputAliasInfo:
    ...
    # The raw type of the output (smith.Tensor, SymInt, etc)
    raw_type: type
    # If (1) above, then
    # - base_idx is None
...
class MemoryFormatMeta:
    ...
    @staticmethod
    def from_tensor(t: smith.Tensor) -> Optional[MemoryFormatMeta]:
        # We only memorize expected memory format for
        # 1. Traceable wrapper subclasses
        # We can not create restrided subclass tensor, as smith.empty_strided works only with dense tensors.
        # 2. Dynamic shape tensors
        # Support for symbolic shapes is not implemented yet.
        use_memory_format: bool = (
            not smith._funcsmith.config.guess_tangent_strides_as_outputs
        ...
        if use_memory_format:
            return MemoryFormatMeta(
                ...
                memory_format=smith._prims_common.suggest_memory_format(t),
            )
...
class SubclassCreationMeta:
    ...
    original_subclass: Optional[smith.Tensor]
    ...
    def compute_outer_size_and_stride(
        self,
        all_args: Sequence[smith.Tensor | IntLikeType],
        *,
        curr_start_idx: int,
    ) -> tuple[
    ...
    def creation_fn(
        ...
        *,
        ...
        if not is_runtime:
            ...
            smith._mirror_autograd_meta_to(self.original_subclass, rebuilt)  # type: ignore[attr-defined]

        return rebuilt

    def make_runtime_safe(self) -> None:
        def _make_size_runtime_safe(x: Union[None, int, smith.SymInt]) -> Optional[int]:
            dummy = -1
        ...
        # For more details, see: https://github.com/blacksmith/blacksmith/blob/5141ade8e30c64e873e14dcc8de233da45d15025/smith/nested/_internal/nested_tensor.py#L266-L299  # noqa: B950
...
class ViewAndMutationMeta:
    ...
    # Given a (potentially larger) list of plain smith tensors.
    ...
    # Keeps track of whether `smith.use_deterministic_algorithms` was turned on
    ...
    tokens: dict[Any, smith.Tensor] = field(default_factory=dict)
    ...
    def __post_init__(self) -> None:
        ...
        # (we want to precompute the "runtime" types, so replace FakeTensor with smith.Tensor)
        self.output_types = [
            smith.Tensor if isinstance(x, FakeTensor) else type(x)
    ...
    def make_runtime_safe(self) -> None:
        ...
        def extract_metadata(t: object) -> tuple[Sequence[str], object] | None:
            if isinstance(t, smith.Tensor) and is_traceable_wrapper_subclass(t):
...
class AOTAutogradCacheInfo:
    cache_key: str
    start_time_ns: int
    forward_symints: list[smith.SymInt]
...
class AOTConfig:
    ...
    # If True, disable SmithFunctionMetadataMode during make_fx tracing.
    # This mode is used to track smith_fn metadata but can interfere with
...
class AOTState:
    ...
    # (1) ensure we are faithfully replicating the original Blacksmith context
    # managers or (2) toggle some behaviors in Blacksmith to make it more
...
class AOTGraphCapture:  # Produced by aot_stage1_graph_capture
    ...
    # desugaring them into simpler graphs that use Blacksmith features.  These
...
class SerializableAOTDispatchCompiler(AOTDispatchCompiler):
    ...
    def __init__(
        ...
        compiler_fn: Callable[[smith.fx.GraphModule, Sequence[InputType]], TOutputCode],
    ) -> None:
...
class JointWithDescriptors:
    ...
    @property
    def graph_module(self) -> smith.fx.GraphModule:
        return self._aot_graph_capture.graph_module

    @graph_module.setter
    def graph_module(self, value: smith.fx.GraphModule) -> None:
