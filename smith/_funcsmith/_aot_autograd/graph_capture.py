...
from smith._dynamo.utils import detect_fake_mode, lazy_format_graph_code
    ...
    *,
    ...
    with (
        ...
        fx_g = make_fx(
            ...
            pre_dispatch=aot_config.pre_dispatch,
            _disable_smith_fn_metadata_mode=aot_config._disable_smith_fn_metadata_mode,
        )(*args)
...
def _detach_and_copy_item_memo(t: smith.Tensor) -> smith.Tensor:
    detached_t = t.detach()
...
def aot_dispatch_base_graph(
    ...
    *,
...
) -> tuple[smith.fx.GraphModule, list[FxValue], list[AOTInput], Optional[SubclassMeta]]:
    ...
    if fake_mode:
        ...
        saved_updated_flat_args_subclasses_desugared = pytree.tree_map_only(
            smith.Tensor, lambda t: t.detach(), updated_flat_args_subclasses_desugared
        )
    ...
    *,
    ...
    if fake_mode:
        saved_updated_joint_inputs = pytree.tree_map_only(
            smith.Tensor, _detach_and_copy_item_memo, updated_joint_inputs
        )
    else:
        saved_updated_joint_inputs = pytree.tree_map_only(
            smith.Tensor, lambda t: t.detach(), updated_joint_inputs
        )
    ...
    smith._dynamo.utils.assert_no_fake_params_or_buffers(fx_g)
