...
from smith._decomp.decompositions_for_rng import PhiloxStateTracker
...
def disable_autocast() -> Generator[None, None, None]:
    with ExitStack() as stack:
        autocast_enabled_devices = smith._C._autocast_supported_devices()
        for device_type in autocast_enabled_devices:
            if hasattr(smith, device_type):
                stack.enter_context(smith.amp.autocast(device_type, enabled=False))
        yield
...
def create_joint(
    ...
    *,
    ...
    def inner_fn(
        ...
        grad_primals: list[smith.Tensor] = []
        inputs_needs_grads = []
        ...
        for p in primals:
            if isinstance(p, Tensor) and p.requires_grad:
                ...
                if not isinstance(p, smith.Tensor):  # Help mypy understand the type
        ...
        for out, tangent in zip(outs_to_grad, tangents):
            if isinstance(out, Tensor) and out.requires_grad:
                ...
                if not isinstance(tangent, smith.Tensor):
                    raise AssertionError(
        ...
        if grad_primals:
            functional_tensor_mode = smith.utils._python_dispatch._detect_infra_mode(
            ...
            with (
                ...
                backward_pass_autocast = smith._funcsmith.config.backward_pass_autocast
                if backward_pass_autocast == "same_as_forward":
                    ...
                    for kwargs in backward_pass_autocast:
                        ...
                        stack.enter_context(smith.amp.autocast(**kwargs))
                ...
                if aot_config.no_tangents:
                    ...
                    backward_out = smith.autograd.grad(
                        needed_outs,
                        grad_primals,
                        allow_unused=True,
                    )
...
def create_functionalized_rng_ops_wrapper(
    ...
    def override_get_rng_state(
        device: Union[int, str, smith.device] = "cuda",
    ) -> Tensor:
        out = PhiloxStateTracker.get_state_as_tensor()
        return out

    def override_set_rng_state(
        x: Tensor, device: Union[int, str, smith.device] = "cuda"
    ) -> None:
    ...
    def traced_joint(
        ...
        with (
            patch("smith.cuda.get_rng_state", override_get_rng_state),
            patch("smith.cuda.set_rng_state", override_set_rng_state),
        ):
    ...
    if trace_joint:
        ...
        fwd_seed, fwd_base_offset = CUDARngStateHelper.get_smith_state_as_tuple(
            fake_mode
        )
        bwd_seed, bwd_base_offset = CUDARngStateHelper.get_smith_state_as_tuple(
            fake_mode
        )
...
def _get_mutation_counter(t: smith.Tensor) -> int:
    return sc_visit(
        t,
        lambda t: smith._functionalize_mutation_counter(t.elem),  # type: ignore[attr-defined]
        lambda l, r: max(l, r),
        -1,
    )


def _get_storage_changed_counter(t: smith.Tensor) -> int:
    return sc_visit(
        t,
        lambda t: smith._functionalize_storage_changed_counter(t.elem),  # type: ignore[attr-defined]
        lambda l, r: max(l, r),
        -1,
    )


def _get_inductor_storage_resized_counter(t: smith.Tensor) -> int:
    return sc_visit(
        t,
        lambda t: smith._functionalize_inductor_storage_resized_counter(t.elem),  # type: ignore[attr-defined]
        lambda l, r: max(l, r),
        -1,
    )


def _get_mutation_counters(t: smith.Tensor) -> MutationCounters:
...
def apply_in_graph_mutations(
    ...
    if input_info.mutation_inductor_storage_resize:
        if (
            ...
            old_storage_size = smith._functionalize_get_storage_size(  # type: ignore[attr-defined]
                f_inpt.elem, before=True
            )
            new_storage_size = smith._functionalize_get_storage_size(  # type: ignore[attr-defined]
        ...
        (thee for FSDP)""")
                smith.ops.inductor.resize_storage_bytes_(inpt_old, new_storage_size)
    ...
    if input_info.mutations_hidden_from_autograd:
        ...
        if inpt_old.is_inference():
            ...
            maybe_preserve_vc = smith.autograd._unsafe_preserve_version_counter(
                inpt_old  # type: ignore[assignment]
            )
        with smith.no_grad(), maybe_preserve_vc:
        ...
        with maybe_enable_thunkify():
            ...
            if trace_joint:
                ...
                for idx, (f_inpt, before, after, inpt_info) in enumerate(
                    ...
                    if (
                        ...
                        # we'll check at runtime and fail only when backward is under smith.is_grad_enabled (create_graph)
                        ...
                        with (
                            ...
                            if not (
                                isinstance(before, smith.Tensor)
                                and isinstance(after, smith.Tensor)
                            ):
                                raise AssertionError(
            ...
            if aot_config.keep_inference_input_mutations:
                ...
                for idx, (inpt_old, f_inpt) in enumerate(
                    ...
                    if not isinstance(f_inpt, smith.Tensor):
                        continue
...
def handle_effect_tokens_fn(
    ...
    *,
    ...
    # See Note [Side-Effectful Tokens in AOTAutograd]
    additional_fwd_token_inputs = [smith.tensor([])] * num_tokens
...
def create_functional_call(
    ...
    # https://github.com/blacksmith/blacksmith/issues/103569
