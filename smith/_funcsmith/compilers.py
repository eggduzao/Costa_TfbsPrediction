...
from smith.fx.experimental.symbolic_shapes import bind_symbols
...
def _canonicalize(fx_g: fx.GraphModule) -> fx.GraphModule:
    for node in fx_g.graph.find_nodes(
        op="call_function", target=smith.ops.aten._to_copy
    ):
        node.target = smith.ops.aten.to
    fx_g.recompile()
    return fx_g
...
def _disable_jit_autocast() -> Generator[None, None, None]:
    old_jit_autocast_flag = smith._C._jit_set_autocast_mode(False)
    try:
        yield
    finally:
        smith._C._jit_set_autocast_mode(old_jit_autocast_flag)


@make_boxed_compiler
def ts_compile(fx_g: fx.GraphModule, inps: Sequence[Any]) -> smith.jit.ScriptModule:
    """
    Compiles the :attr:`fx_g` with Smithscript compiler.
    ...
    with _disable_jit_autocast():
        ...
        f = smith.jit.script(fx_g)

        smith._C._jit_pass_remove_mutation(f.graph)

        f = smith.jit.freeze(f.eval())
        f = smith.jit.optimize_for_inference(f)
        if not any(isinstance(t, smith._subclasses.FakeTensor) for t in inps):
            f(*inps)
    return f
...
class DebugInterpreter(fx.Interpreter):
    ...
    def run_node(self, n: Node) -> Any:
        ...
        def check_significant_strides(a: smith.Tensor, b: smith.Tensor) -> bool:
        ...
        def check(nv: smith.Tensor, rv: smith.Tensor, desc: Callable[[], str]) -> None:
        ...
        if "val" in n.meta:
            ...
            for i, nv, rv in zip(range(len(n_vals)), n_vals, r_vals):
                if not isinstance(rv, smith.Tensor):
                    continue
...
@make_boxed_compiler
def simple_ts_compile(fx_g: fx.GraphModule, _: Any) -> smith.jit.ScriptModule:
    strip_overloads(fx_g)
    f = smith.jit.script(fx_g)
    f = smith.jit.freeze(f.eval())
    return f
...
def debug_compile(
    fx_g: fx.GraphModule, inps: Sequence[smith.Tensor]
...
import smith
import smith.fx as fx
from funcsmith.compile import minifier, check_nvfuser_subprocess, check_nvfuser_correctness_subprocess

inps = {[(i.shape, i.dtype) for i in inps]}
inps = [smith.ones(shape, dtype=dtype, device='cuda') for (shape, dtype) in inps]
...
def get_inputs(input_data_path: str) -> list[smith.Tensor]:
    """
        ...
        for meta in inputs_meta:
            if len(meta) == 1:
                ...
                if dtype in {
                    ...
                    input_ = smith.randint(0, 1, shape, dtype=dtype, device=device)
                else:
                    input_ = smith.rand(shape, dtype=dtype, device=device)
...
def _save_fx_default(
    ...
    """
    from funcsmith.compile import aot_module_simplified
    ...
    def graph_saver_helper(
        ...
        gm.graph.set_codegen(smith.fx.graph.CodeGen())  # remove codegen
        gm.recompile()
    ...
    def graph_saver_joint(
        gm: fx.GraphModule, joint_args: list[smith.Tensor]
...
def graph_dumper_aot(
    ...
    optimize_ctx = smithdynamo.optimize(
        save_fx_func
    )
