...
from smith.nn.utils._named_member_accessor import NamedMemberAccessor
...
def raise_parameter_tying_error() -> NoReturn:
    raise RuntimeError(
        ...
        "https://github.com/blacksmith/funcsmith/issues/446"
    )
...
def _extract_members(
    ...
    for name, p in all_named_members:
        if p not in memo:
            memo[p] = subclass(smith.empty_like(p, device="meta"))
        replacement = memo[p]
...
def make_functional(
    ...
    Given a ``smith.nn.Module``, :func:`make_functional` extracts the state
    ...
    .. code-block:: python
        ...
        import smith
        import smith.nn as nn
        from funcsmith import make_functional, grad
    ...
    Args:
        model (smith.nn.Module): Input model.
        disable_autograd_tracking (bool): Flag to disable gradients tracking for output parameters.
            ...
            Blacksmith autograd), matching the requires_grad-ness of the params from the original model.
            Otherwise, the returned params will have ``requires_grad=False``. Default, False.
            If you plan on using regular Blacksmith autograd (e.g., if you want to call ``.backward()`` or
            ``smith.autograd.grad()``, then set ``disable_autograd_tracking=False``.
            Otherwise, if you're only planning on using funcsmith's gradient transforms,
            then please set ``disable_autograd_tracking=True`` to avoid unnecessarily tracking
            history with Blacksmith autograd.
...
def make_functional_with_buffers(
    ...
    Given a ``smith.nn.Module``, make_functional_with_buffers extracts the
    ...
    .. code-block:: python
        ...
        import smith
        import smith.nn as nn
        from funcsmith import make_functional_with_buffers
    ...
    .. code-block:: python
        ...
        import smith
        import smith.nn as nn
        from funcsmith import make_functional_with_buffers, grad
...
def transpose_stack(
    ...
    results = tuple(
        smith.stack(shards).detach() for shards in tuple_of_tuple_of_tensors
    )
    return results
...
def combine_state_for_ensemble(
    ...
    Prepares a list of smith.nn.Modules for ensembling with :func:`vmap`.
...
def functional_init(
    ...
    def wrapped(
        *args: Any, **kwargs: Any
        ...
        weights = tuple(smith.stack(shards).detach() for shards in weights)
...
def functional_init_with_buffers(
    ...
    def wrapped(
        *args: Any, **kwargs: Any
        ...
        buffers = tuple(smith.stack(shards).detach() for shards in buffers)
