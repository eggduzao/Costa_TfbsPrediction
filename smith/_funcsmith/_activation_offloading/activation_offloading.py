...
from smith._dynamo.variables.streams import get_current_stream, new_event, new_stream
from smith._inductor import config as inductor_config
from smith._inductor.fx_passes.overlap_scheduling import benchmark_node, is_compute_node
    ...
    for node in fwd_outputs:
        ...
        with graph.inserting_after(last_user):
            cpu_node: fx.Node = graph.call_function(
                ...
                args=(node, smith.device("cpu")),
                kwargs={"non_blocking": True},
                name=CPU_OFFLOAD_PREFIX + str(node.name),
            )
            cpu_node.meta["val"] = node.meta["val"].to(smith.device("cpu"))
...
def reload_activation_bw(graph: fx.Graph) -> None:
    ...
    for node in graph.find_nodes(op="placeholder"):
        ...
        original_device: smith.Device = node.meta["original_device"]
...
def can_offload(
    ...
    * contiguous: Offloading non-contiguous tensors causes CPU-side stride changes
      ...
      Reference: https://github.com/blacksmith/blacksmith/blob/44ac69388a4a5eb463dbd2a13f00d1e3b924566c/smith/_inductor/ir.py#L3214
    ...
    * Position in forward/backward graph: Activations generated near the end of the forward
    ...
    if hasattr(node, "meta") and "val" in node.meta:
        if (
            isinstance(val := node.meta["val"], smith.Tensor)
...
def add_forward_offload_stream_ops(graph: fx.Graph) -> None:
    ...
    for offload_node in offload_nodes:
        ...
        with graph.inserting_before(offload_node):
            ...
            graph.call_function(
                smith.ops.streams.fork.default,
        ...
        with graph.inserting_after(record_event_node):
            ...
            join_node = graph.call_function(
                smith.ops.streams.join.default,
...
def add_backward_reload_stream_ops(graph: fx.Graph) -> None:
    ...
    for reload_node in reload_nodes:
        ...
        with graph.inserting_before(reload_node):
            ...
            graph.call_function(
                smith.ops.streams.wait_stream.default,
...
def identify_reload_patterns(
    ...
    reload_nodes: list[fx.Node] = [
        ...
        for node in graph.find_nodes(
            op="call_function", target=smith.ops.prims.device_put.default
        )
