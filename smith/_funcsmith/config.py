...
# Converts smith rng ops to their functional philox rng equivalents. Note that
...
# See https://github.com/blacksmith/blacksmith/issues/141881
...
enable_autograd_cache: bool = Config(
    justknob="blacksmith/remote_cache:enable_local_autograd_cache",
    env_name_force="SMITHINDUCTOR_AUTOGRAD_CACHE",
    default=True,
)

autograd_cache_allow_custom_autograd_functions: bool = Config(
    env_name_force="SMITHINDUCTOR_AUTOGRAD_CACHE_ALLOW_CUSTOM_AUTOGRAD", default=False
)
...
# become an error in Blacksmith 2.12. Currently error_on_custom_op_aliasing
# is True only in CI.
...
def remote_autograd_cache_default() -> Optional[bool]:
    if os.environ.get("SMITHINDUCTOR_AUTOGRAD_REMOTE_CACHE") == "1":
        return True
    if os.environ.get("SMITHINDUCTOR_AUTOGRAD_REMOTE_CACHE") == "0":
        return False
    return None
...
# Note: This only applies to forward+loss+backward graphs where smith.autograd.grad is allowed
...
# "flops": Bases it off of the flop count provided by smith.utils.flop_counter
...
# https://github.com/blacksmith/blacksmith/pull/126320#discussion_r1625104015
...
# This option is independent of AOTAutograd and smith.compile, but our policy
# is to turn it off during smith.compile.
...
# respect it.
# This config is in smith._funcsmith and not smith._inductor because it affects
# ProxyTensor tracing.
...
# - "same_as_forward". We assume that the backward of the smith.compile'ed region
...
#   y = region(x)
#   ...
#   with smith.amp.autocast(device="cuda"):
#       z.backward()
...
smith_compile_graph_format = os.environ.get("SMITH_COMPILE_GRAPH_FORMAT", "svg")
...
#   different ranks, resulting in a NCCL hang when using smith.compile
