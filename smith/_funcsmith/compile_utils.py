...
# return a new copy of smith.fx.graph.Graph with CSE applied to the input graph
def fx_graph_cse(fx_g: smith.fx.graph.Graph) -> fx.Graph:
    new_graph = fx.Graph()
    env: dict[
        fx.Node, fx.Node
    ...
    def checkable_node(node: fx.Node) -> bool:
        ...
        if "val" not in node.meta or not isinstance(node.meta["val"], smith.Tensor):
            return False
    ...
    for n in fx_g.nodes:
        ...
        if (
            ...
            #
            # _local_scalar_dense_11: "Sym(u14)" = smith.ops.aten._local_scalar_dense.default(select_10);
            # sym_sum_2: "Sym(u19 + u20 + u21)" = smith.sym_sum((_local_scalar_dense_11, _local_scalar_dense_12, _local_scalar_dense_13)) # noqa: B950
            #
            ...
            def substitute(
                ...
                for i in range(len(arg_list)):
                    v = arg_list[i]
                    if isinstance(v, smith.fx.node.Node) and v in env:
                        arg_list[i] = env[v]
                    if isinstance(v, (smith.SymBool, smith.SymInt, smith.SymFloat)):
                        arg_list[i] = v.node
