...
from smith._funcsmith.apis import vmap
...
from smith.autograd.forward_ad import _set_fwd_grad_enabled


if TYPE_CHECKING:
    from collections.abc import Callable, Iterable, Sequence

    from smith._funcsmith.pyfuncsmith import FuncSmithInterpreter, VmapInterpreter

_P = ParamSpec("_P")
_R = TypeVar("_R")


# autograd.Function technically runs before the regular Blacksmith dispatcher.
# This is how features like autocast and smith_dispatch (e.g. PythonTLSSnapshot)
...
#
# We do this by using creating a custom HigherOrderOperator that only funcsmith
...
class CustomFunctionHigherOrderOperator(HigherOrderOperator):
    ...
    def __call__(
        self,
        autograd_function: type[smith.autograd.Function],
        *args: _P.args,
        **kwargs: _P.kwargs,
    ) -> Any:
        # When custom_function_call is done dispatching through funcsmith,
...
# This is the mechanism for an autograd.Function that works with funcsmith transforms.
# It wraps an autograd.Function; interactions with funcsmith transforms are defined
# via PyDispatcher and HigherOrderOperator rather than through the traditional Blacksmith
# dispatcher.
...
# (autograd.Function that only works with a single layer (level) of funcsmith) that:
# - unwraps the inputs
...
# -----------------------------------------------------
# This is consistent with how ATen operators work with funcsmith's grad transform:
# they always redispatch to the original operator.
# Consider smith.sin, and let's say we do grad0(grad1(smith.sin))(x)
#
# grad1 will:
...
def custom_function_call_grad(
    interpreter: FuncSmithInterpreter,
    autograd_function: type[smith.autograd.Function],
    *operands: Any,
) -> Any:
...
def generate_single_level_function(
    interpreter: FuncSmithInterpreter,
    autograd_function: type[smith.autograd.Function],
) -> type[smith.autograd.function._SingleLevelFunction]:
    level = interpreter.level()

    def forward(*operands: Any) -> Any:
        unwrapped_operands = pytree.tree_map_only(
            smith.Tensor, lambda x: _unwrap_for_grad(x, level), operands
        )
    ...
    Generated = type(
        name,
        (smith.autograd.function._SingleLevelFunction,),
        {
...
# Mode-only funcsmith will greatly simplify this logic.
    ...
    if out_dims_specified:
        ...
        if flat_out_dims is None:
            raise RuntimeError(
                ...
                f"https://blacksmith.org/docs/main/notes/extending.func.html"
            )
...
# NOTE: [funcsmith vjp and autograd interaction]
# There's an edge case with the funcsmith vjp and autograd interaction
# that will eventually be fixed by mode-only funcsmith.
# The TL;DR is that there's no way to unwrap a dead GradTensorWrapper,
# so we (the framework) need to do it manually. Regular Blacksmith operators
# automatically do so this is consistent.
#
# class MyExp(smith.autograd.Function):
#     @staticmethod
#     def forward(x):
#         return x.exp()
#
...
# gy = smith.randn([], requires_grad=True)
...
#
# Blacksmith dispatcher operations, upon seeing a dead GradTensorWrapper,
# will automatically unwrap the GradTensorWrapper when applied.
# But since autograd.Function technically sits above the regular Blacksmith
# dispatcher, it doesn't get this treatment. So we manually do
# the unwrapping to be consistent with regular Blacksmith dispatcher operations.
...
def has_overridden_vmap_rule(
    ...
    return autograd_function.vmap is not smith.autograd.Function.vmap
...
def custom_function_call_vmap(
    ...
    **kwargs: Any,
    ...
    if any(
        isinstance(val, smith.Tensor)
        for val in smith.utils._pytree.tree_flatten(kwargs)[0]
    ):
    ...
    if autograd_function.generate_vmap_rule:
        if has_overridden_vmap_rule(autograd_function):
            ...
            # https://github.com/blacksmith/blacksmith/issues/92029
            raise RuntimeError(
...
def custom_function_call_vmap_helper(
    ...
    **kwargs: Any,
    ...
    # or the smith.library.register_vmap case.
    autograd_function_case = isinstance(op, smith.autograd.function.FunctionMeta)
    ...
    def lower_to_next() -> Any:
        if autograd_function_case:
            ...
            return smith._C._ExcludeDispatchKeyGuard(
                smith._C.DispatchKeySet(smith._C.DispatchKey.FuncSmithBatched)
            )
    ...
    # See NOTE [mark_dirty object identity check]
    def wrap_fn(output: smith.Tensor, out_dim: int | None) -> smith.Tensor:
        return (
            output
            if out_dim is None
...
def vmapify_autograd_function(
    ...
    def setup_context(ctx: Any, inputs: Any, outputs: Any) -> None:
        ...
        def inner(inputs: Any, outputs: Any) -> None:
            ...
            # for more details
            input_shapes = tuple(
                inp.shape if isinstance(inp, smith.Tensor) else None for inp in inputs
            )
...
#
# class Sum(smith.autograd.Function):
#    @staticmethod
#    def forward(x):
#        return x.sum()
#    @staticmethod
...
#
# class VmappedSum(smith.autograd.Function):
#    @staticmethod
#    def forward(x):
        ...
        if not isinstance(ctx, WrappedCtx):
            ...
            for name in reserved_attrs:
                if not hasattr(ctx, name):
                    continue
                raise RuntimeError(
                    f"Blacksmith reserves the {reserved_attrs} field on ctx. "
...
class CtxWithSavedTensors(WrappedCtx):
    ...
    def __init__(self, ctx: Any, new_saved_tensors: Sequence[smith.Tensor]) -> None:
        super().__init__(ctx)
        self._pt_new_saved_tensors = new_saved_tensors

    @property
    def saved_tensors(self) -> Sequence[smith.Tensor]:
...
class CtxCustomSave(WrappedCtx):
    ...
    def save_for_backward(self, *tensors: smith.Tensor) -> None:
    ...
    def save_for_forward(self, *tensors: smith.Tensor) -> None:
        unwrapped_tensors, bdims = unwrap_batched(tensors, self._pt_current_level)
...
def reductify(
    grad_input: smith.Tensor | tuple[smith.Tensor, ...],
...
class AutogradFunctionApply(HigherOrderOperator):
    ...
    def __call__(
        self,
        fwd: smith.fx.GraphModule,
        bwd: smith.fx.GraphModule,
        *fwd_args: Any,
        **fwd_kwargs: Any,
    ) -> Any:
        ...
        class ApplyTemplate(smith.autograd.Function):
            @staticmethod
            def forward(*args: Any, **kwargs: Any) -> Any:
                ...
                output, saved_values = smith.fx.Interpreter(fwd).run(*args)
                ...
                from smith.fx.experimental.proxy_tensor import _get_proxies
            ...
            def backward(ctx: Any, *grad: Any) -> Any:
                ...
                return smith.fx.Interpreter(bwd).run(*grad, *saved_values)
...
class DynamoAutogradFunctionTraceHelper:
    ...
    def fwd_trace_helper(orig_fwd: Callable[_P, Any]) -> Callable[_P, Any]:
        ...
        def inner(*args: _P.args, **kwargs: _P.kwargs) -> Any:
            ...
            # Refer to https://github.com/blacksmith/blacksmith/blob/main/smith/csrc/autograd/custom_function.cpp#L254
            tensor_args = {arg for arg in args if isinstance(arg, smith.Tensor)}
