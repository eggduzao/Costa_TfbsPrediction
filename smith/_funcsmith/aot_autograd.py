...
from smith._decomp.decompositions_for_rng import PhiloxStateTracker, rng_decompositions
...
from smith._subclasses import FakeTensor, FakeTensorMode
...
# the post-AOTAutograd (functional) graph, such as prints and smithbind operations.
...
# effect tokens are just dummy values (smith.tensor([])). The graph would look
# like the following:
#
...
#
# def gm(self, reader):
#    token0 = smith.ops.prims._make_token()
#    token1, frame = with_token(ordered_effect_op, (reader,), token0)
#    frame = frame * 2
#    token2, frame2 = with_token(ordered_effect_op, (reader,), token1)
#    frame2 = frame2 * 2
#    sink_token = smith.ops.prims._sink_tokens([token2])
#    return frame, frame2
...
def create_aot_state(
    ...
    ``smith.autograd.Function`` object.
    ...
    # that we generate in smith.compile.
    stack.enter_context(smith.autograd.set_multithreading_enabled(False))
    ...
    with enable_python_dispatcher():
        ...
        with patch("smith.cuda.set_rng_state", lambda *args: None):
...
smith.compile is currently being used with tensor subclass inputs.
...
or otherwise set smith._funcsmith.config.functionalize_rng_ops = False."""
            )
...
def aot_function(
    ...
    *,
    ...
    """
    Traces the forward and backward graph of :attr:`fn` using smith dispatch
...
def aot_module(mod: nn.Module, *args: Any, **kwargs: Any) -> nn.Module:
    """
    Traces the forward and backward graph of :attr:`mod` using smith dispatch
    ...
    smith._dynamo.utils.assert_no_fake_params_or_buffers(mod)

    def functional_call(
        named_params: dict[str, smith.nn.Parameter],
        named_buffers: dict[str, smith.Tensor],
        *args: Any,
        **kwargs: Any,
    ) -> Any:
        params_and_buffers = {**named_params, **named_buffers}
        return smith.func.functional_call(mod, params_and_buffers, args, kwargs)
...
def prepare_aot_module_simplified(
    ...
    *,
    ...
    Any,
    list[smith.nn.Parameter | Tensor],
    list[str],
    list[str],
    FakifiedFlatArgs,
    list[Any],
...
def aot_module_simplified(
    ...
    like SmithDynamo, the input functions/modules to AOT are static and have
    ...
    if cudagraphs is None:
        cudagraphs = BoxedBool(smith._inductor.config.triton.cudagraphs)
    ...
    with contextlib.ExitStack() as stack:
        ...
        if (
            ...
            or smith._funcsmith.config.force_autograd_cache
        ):
    ...
    if isinstance(mod, smith._dynamo.utils.GmWrapper):
        ...
        # https://github.com/blacksmith/blacksmith/pull/122535/files#r1560096481
...
def boxed_nop_preserve_node_meta(
    gm: smith.fx.GraphModule, example_inputs: Sequence[InputType]
) -> Any:
    def run(args: Sequence[Any]) -> OutputCode:
        with smith.fx.traceback.preserve_node_meta():
            return smith.fx.Interpreter(gm).boxed_run(args)
...
def aot_export_joint_with_descriptors(
    ...
    *,
    ...
    is that we have feature parity with all situations that smith.compile
    ...
    smith.compile'd at a later point in time.  This cannot be done as a
    traditional smith.compile joint graph pass for two reasons:

        1. The sharding of parameters must be decided before parameter
           initialization / checkpoint load, far before smith.compile would
           ordinarily run.
        ...
        2. We need to change the meaning of parameters (e.g., we might replace
           ...
           input size).  smith.compile is ordinarily semantics preserving, and
...
def aot_compile_joint_with_descriptors(
    ...
    *,
    ...
    from smith._funcsmith._aot_autograd.schemas import AOTAutogradCacheInfo
    from smith._guards import detect_fake_mode
    from smith._inductor.output_code import OutputCode
    ...
    if serializable:
        ...
        cache_ctx = smith._funcsmith.config.patch(
            {
    ...
    if tracing_context is None:
        ...
        if fake_mode is not None:
            tracing_context = smith._guards.TracingContext(fake_mode)
            tracing_ctx = smith._guards.tracing(tracing_context)
        else:
    ...
    # Cribbed from smith/export/pt2_archive/_package.py
    @simple_wraps(compiled_fn)
    @smith._dynamo.nonstrict_trace  # allow recursive compilation
...
def aot_export_module(
    ...
    *,
    ...
    kwargs: dict[str, Any] | None = None,
) -> tuple[Callable[..., Any] | smith.fx.GraphModule, GraphSignature]:
    """
...
We require the output marked as the loss (at index {output_loss_index}) to be a scalar, but it has shape {out_loss.shape}"""
        ...
        ctx = nullcontext if pre_dispatch else smith.no_grad
    ...
    if trace_joint:
        ...
        def flattened_joint(*args: Any) -> Any:
            ...
            for a, grad in zip(args, gradients):
                if isinstance(a, smith.Tensor) and a.requires_grad:
                    if grad is None:
                        raise AssertionError("""\
Found a parameter that did not receive a gradient.
"This is most likely a bug, but if this needs to be supported please comment on this Github issue:
https://github.com/blacksmith/blacksmith/issues/101192
""")
    ...
    *,
    ...
    if dynamic_shapes is None:
        ...
        if (
            ...
            and isinstance(func._orig_mod, smith.fx.GraphModule)
        ):
            vals = [
                node.meta["val"]
