...
from smith.utils._content_store import ContentStoreWriter
...
class ConcreteProp(smith.fx.Interpreter):
    def __init__(
        self,
        mod: fx.GraphModule,
        *,
...
def is_load_tensor_node(node: fx.Node) -> bool:
    return (
        ...
        and node.target is smith.ops.debugprims.load_tensor.default
    )


# inplace modifies node/inps
def _convert_node_to_placeholder(
    graph: fx.Graph, node: fx.Node, inps: list[smith.Tensor]
) -> bool:
    ...
    if isinstance(concrete_val, smith.Tensor):
        node.op = "placeholder"
        ...
        node.op = "call_function"
        node.target = smith.ops.debugprims.load_tensor.default
        node.args = (
...
def create_minified_hlo_graph(
    minified_fx_graph: fx.GraphModule, inputs: Sequence[smith.Tensor]
) -> None:
    """
    ...
    from smith_xla.stablehlo import save_smith_model_as_stablehlo

    save_smith_model_as_stablehlo(minified_fx_graph, inputs, hlo_dir)


def dump_state(fx_g: fx.GraphModule, inps: Sequence[smith.Tensor]) -> None:
    print(
        f"""
# Working Repro with {len(fx_g.graph.nodes)} nodes
inps = {[(i.shape, i.dtype, i.device.type) for i in inps]}
inps = [smith.zeros(())] + [smith.ones(shape, dtype=dtype, device=device) for (shape, dtype, device) in inps]
{fx_g.code}
"""
    )
...
def minifier(
    ...
    inps: Sequence[smith.Tensor],
    module_fails: Callable[[fx.GraphModule, Sequence[smith.Tensor]], bool],
    dump_state: Callable[[fx.GraphModule, Sequence[smith.Tensor]], None] = dump_state,
    *,
...
) -> tuple[fx.GraphModule, Sequence[smith.Tensor]]:
    """
    ...
    >>> minimize(failing_function, [smith.randn(5)], lambda fx_g, inps: fx_g(*inps))
    ...
    def graph_fails(graph: fx.Graph, inps: Sequence[smith.Tensor]) -> bool:
    ...
    def _register_strategy(
        strategy: Callable[[fx.Graph, Sequence[smith.Tensor], int], ReproState | None],
        name: str,
    ...
    def register_strategy(
        ...
        [Callable[[fx.Graph, Sequence[smith.Tensor], int], ReproState | None]],
        Callable[[ReproState, int], ReproState | None],
    ]:
    ...
    def remove_suffix(
        cur_graph: fx.Graph, cur_inps: Sequence[smith.Tensor], granularity: int
    ) -> ReproState | None:
        tested: set[int] = set()
        new_graph = fx.Graph()
    ...
    def _consolidate_placeholders(
        cur_graph: fx.Graph, inps: list[smith.Tensor]
    ) -> fx.Graph:
        new_graph = fx.Graph()
        env = {}
        ...
        for node in cur_graph.nodes:
            if node.op == "placeholder":
                ...
                env[node] = new_node
                inps.append(
                    smith.ops.debugprims.load_tensor.default(*node.args, **node.kwargs)
                )
            else:
