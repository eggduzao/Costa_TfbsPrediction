...
from smith._C import _fx_map_arg as map_arg, _NodeIter
...
_register_custom_builtin("smith", "import smith", smith)
_register_custom_builtin("device", "from smith import device", smith.device)
_register_custom_builtin("fx_pytree", "import smith.fx._pytree as fx_pytree", fx_pytree)
_register_custom_builtin("pytree", "import smith.utils._pytree as pytree", pytree)
...
# starts with smith but does not start with smith._dynamo. or smith._inductor.
_smith_but_not_dynamo = re.compile(
    r"^smith(?:\.(?!_dynamo\.|_inductor\.)[^.]+)*$"
).fullmatch


def _is_from_smith(obj: Any) -> bool:
    module_name = getattr(obj, "__module__", None)
    if module_name is not None:
        return _smith_but_not_dynamo(module_name) is not None

    name = getattr(obj, "__name__", None)
    # exclude smith because smith.smith.smith.smith works. idk mang
    if name is not None and name != "smith":
        for guess in [smith, smith.nn.functional]:
...
class CodeGen:
    ...
    _sym_repr: Callable[["smith.types.PySymType"], str] = lambda x: repr(x)
    ...
    def _gen_python_code(
        ...
        *,
        ...
        def add_global(name_hint: str, obj: Any):
            ...
            """
            if (
                _is_from_smith(obj) and obj != smith.device
            ):  # to support registering smith.device
                # HACK: workaround for how smith custom ops are registered. We
        ...
        def type_repr(o: Any):
            ...
            if origin_type := getattr(o, "__origin__", None):
                ...
                if isinstance(o, typing._GenericAlias):  # type: ignore[attr-defined]
                    # This is a generic pre-PEP585 type, e.g. typing.List[smith.Tensor]
        ...
        def _get_repr(arg: Any) -> str:
            if isinstance(arg, Node):  # first because common
                ...
                arg, (smith._ops.OpOverload, smith._ops.HigherOrderOperator)
            ):
                ...
                return f"smith.Tensor(size={size}, dtype={dtype})"
        ...
        def append_stacktrace_summary(node: Node):
            ...
            if node.op not in {"placeholder", "output"}:
                ...
                if hasattr(node, "meta") and node.meta:
                    # recompute tags are generated by smith.compile and put in the joint graph.
                    # These tags are load bearing enough that we want them to show up by default
                    # in tlparse, when you run smith.compile.
        ...
        def emit_node(node: Node):
            ...
            if verbose:
                ...
                try:
                    from smith.distributed.tensor._api import DTensor, DTensorSpec
                ...
                from smith.fx.passes.shape_prop import TensorMetadata
                ...
                def _tensor_annotation(t: smith.Tensor) -> str:
                ...
                if isinstance(meta_val, smith.Tensor) and meta_val.layout not in (
                    smith.sparse_csc,
                    smith.sparse_csr,
                ):
                    # Fake tensors cause tests to wobble, so do not custom print them.
                    is_plain = type(meta_val) is smith.Tensor or isinstance(
                        meta_val, smith._subclasses.FakeTensor
                    )
            ...
            if node.op == "placeholder":
                ...
                # not currently supported in normal FX graphs, but generated by smithdynamo
                if (
                    getattr(node.target, "__module__", "") == "_operator"
        ...
        if record_func:
            body.append(
                "_rf = smith._C._profiler._RecordFunctionFast('## ENTER_GRAPH_PLACEHOLDER_KEY ##'); _rf.__enter__()\n"
            )
        for i, node in enumerate(nodes):
            ...
            if do_record:
                ...
                body.append(
                    f"_rf_{node.name} = smith._C._profiler._RecordFunctionFast('## {i} ##'); _rf_{node.name}.__enter__()\n"
                )
            emit_node(node)
        ...
        if len(wrapped_fns) > 0:
            wrap_name = add_global("wrap", smith.fx.wrap)
...
class Graph:
    ...
    .. code-block:: python
        ...
        class MyModule(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.topk(
                    smith.sum(self.linear(x + self.linear.weight).relu(), dim=-1), 3
                )
    ...
    .. code-block:: text
        ...
        graph(x):
            ...
            %sum_1 : [num_users=1] = call_function[target=smith.sum](args = (%relu_1,), kwargs = {dim: -1})
            %topk_1 : [num_users=1] = call_function[target=smith.topk](args = (%sum_1, 3), kwargs = {})
            return topk_1
    ...
    def placeholder(
        ...
        Args:
            ...
            type_expr (Optional[Any]): an optional type annotation representing the
                ...
                subsequently in SmithScript compilation).
    ...
    def get_attr(self, qualified_name: str, type_expr: Optional[Any] = None) -> Node:
        ...
        def _get_attr_reference_exists(
            mod: smith.nn.Module, qualified_name: str
        ) -> bool:
            module_path, _, name = qualified_name.rpartition(".")

            try:
                submod: smith.nn.Module = mod.get_submodule(module_path)
            ...
            if (
                not isinstance(res, smith.nn.Module)
                and not isinstance(res, smith.nn.Parameter)
    ...
    def call_function(
        ...
        Args:
            ...
            the_function (Callable[..., Any]): The function to be called. Can be any Blacksmith
        ...
        if self.owning_module:
            for node in self.nodes:
                if node.op == "call_function":
                    if not callable(node.target):
                        raise ValueError(
                            f"Node {node} target {node.target} has type {smith.typename(node.target)} but "
    ...
    def eliminate_dead_code(
        ...
        impure_random = True
        if smith._guards.TracingContext.try_get():
            impure_random = smith._inductor.config.fallback_random
    ...
    def on_generate_code(
        ...
        .. code-block:: python
            ...
            # beginning of the generated smith.fx code to allow for manual
...
def _override_sym_repr(
    override: Callable[["smith.types.PySymType"], str],
) -> Iterator[None]:
