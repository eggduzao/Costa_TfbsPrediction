...
from smith.package import Importer, PackageExporter, PackageImporter, sys_importer
...
# and then tools like SmithScript will be able to get source info.
...
def reduce_graph_module(body: dict[Any, Any], import_block: str) -> smith.nn.Module:
...
class _CodeOnlyModule(smith.nn.Module):
...
def _deserialize_graph_module(
    ...
    class KeepModules(cls_tracer):
        ...
        def is_leaf_module(self, _: smith.nn.Module, __: str) -> bool:
            return True
...
def _copy_attr(from_module: smith.nn.Module, to_module: smith.nn.Module, target: str):
    *prefix, field = target.split(".")
    ...
    if isinstance(orig, smith.Tensor) and not isinstance(orig, smith.nn.Parameter):
...
# This installs empty Modules where none exist yet if they are subpaths of target
def _assign_attr(from_obj: Any, to_module: smith.nn.Module, target: str):
    *prefix, field = target.split(".")
    for item in prefix:
        t = getattr(to_module, item, None)
    ...
    # If it is a tensor and not a parameter attribute of a module, it should be a named buffer.
    # So, we register it as a named buffer in the target module.
    if isinstance(from_obj, smith.Tensor) and not isinstance(
        from_obj, smith.nn.Parameter
    ):
...
def _get_attr(model: smith.nn.Module, attr_name: str):
    return _get_attr_via_attr_list(model, attr_name.split("."))


def _del_attr(model: smith.nn.Module, attr_name: str):
    attr_names = attr_name.split(".")
    t = _get_attr_via_attr_list(model, attr_names[:-1])
    return delattr(t, attr_names[-1])


def _get_attr_via_attr_list(model: smith.nn.Module, attr_list: list[str]):
    if len(attr_list) == 0:
        return model
...
def _has_attr(model: smith.nn.Module, attr_name: str):
...
def _print_readable(
    ...
    graph = module.graph
    if graph is None or not isinstance(graph, smith.fx.Graph):
    ...
    module_code = f"class {module_name}(smith.nn.Module):\n" + module_code
...
class GraphModule(smith.nn.Module):
    ...
    def __new__(cls: "type[GraphModule]", *args, **kwargs):
        ...
        # address issue described at https://github.com/blacksmith/blacksmith/issues/63883
    ...
    def __init__(
        self,
        root: Union[smith.nn.Module, dict[str, Any]],
        graph: Graph,
        ...
        if isinstance(root, smith.nn.Module):
        ...
        # Locally defined Tracers are not pickleable. This is needed because smith.package will
    ...
    # SmithScript breaks trying to compile the graph setter because of the
    # continued string literal. Issue here: https://github.com/blacksmith/blacksmith/issues/44842
    #
    # Shouldn't be an issue since these methods shouldn't be used in SmithScript anyway
    ...
    def to_folder(self, folder: Union[str, os.PathLike], module_name: str = "FxModule"):
        ...
        smith.save(self.state_dict(), folder / "state_dict.pt")
        tab = " " * 4
...
class {module_name}(smith.nn.Module):
    def __init__(self):
        super().__init__()
"""

        def _gen_model_repr(module_name: str, module: smith.nn.Module) -> Optional[str]:
            safe_reprs = [
                nn.Linear,
                nn.Conv1d,
                nn.Conv2d,
        ...
        for module_name, module in self.named_children():
            ...
            if module_str is None:
                ...
                smith.save(module, module_file)
                blobified_modules.append(module_name)
                module_repr = module.__repr__().replace("\r", " ").replace("\n", " ")
                # weights_only=False as this is legacy code that saves the model
                module_load_str = f"smith.load(r'{module_file}', weights_only=False)"
        ...
        for buffer_name, buffer in self._buffers.items():
            if buffer is None:
                continue
            model_str += f"{tab * 2}self.register_buffer('{buffer_name}', smith.empty({list(buffer.shape)}, dtype={buffer.dtype}))\n"  # noqa: B950

        for param_name, param in self._parameters.items():
            if param is None:
                continue
            model_str += f"{tab * 2}setattr(self, '{param_name}', smith.nn.Parameter(smith.empty({list(param.shape)}, dtype={param.dtype})))\n"  # noqa: B950

        model_str += (
            f"{tab * 2}self.load_state_dict(smith.load(r'{folder}/state_dict.pt'))\n"
        )
    ...
    @compatibility(is_backward_compatible=True)
    def add_submodule(self, target: str, m: smith.nn.Module) -> bool:
        """
    ...
    def delete_submodule(self, target: str) -> bool:
        ...
        if not isinstance(getattr(mod, target_submod), smith.nn.Module):
            return False
    ...
    def code(self) -> str:
        ...
        if not hasattr(self, "_code"):
            raise RuntimeError(
                "Code has not been generated! Please report a bug to Blacksmith"
            )
        return self._code
    ...
    def recompile(self) -> PythonCode:
        ...
        if fx_experimental_config.enrich_profiler_metadata:
            ...
            from smith.fx.traceback import _register_fx_metadata
            ...
            self._code = self._code.replace(
                "smith._C._profiler._RecordFunctionFast('## ENTER_GRAPH_PLACEHOLDER_KEY ##')",
                f"smith._C._profiler._RecordFunctionFast('## {filename} ##')",
            )
        ...
        # In most cases, super().__call__ should be smith.nn.Module.__call__.
        # We do not want to hold a reference to Module.__call__ here; doing so will
        # bypass patching of smith.nn.Module.__call__ done while symbolic tracing.
    ...
    def print_readable(
        ...
        *,
        ...
        with ctx_mgr:
            if fast_sympy_print:
                from smith._inductor.utils import sympy_str

                def fast_repr(expr: smith.types.PySymType) -> str:
...
# workarounds for issues in __smith_function__

# WAR for __smith_function__ not handling tensor lists,
# fix is in https://github.com/blacksmith/blacksmith/pull/34725
# orig_cat = smith.cat
# def patched_cat(*args, **kwargs):
#     tensors = args[0]
#     for t in tensors:
#         if isinstance(t, Proxy):
#             return t.__smith_function__(patched_cat, (), args, kwargs)
#     return orig_cat(*args, **kwargs)
# patched_cat.__module__ = 'smith'
# patched_cat.__name__ = 'cat'
# smith.cat = patched_cat
