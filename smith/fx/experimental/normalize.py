...
from smith.fx import Proxy, Transformer
from smith.fx.node import Argument, map_aggregate, Node, Target
...
class NormalizeArgs(Transformer):
    ...
    Example usage:
        m = smithvision.models.resnet18()
        traced = smith.fx.symbolic_trace(m)
        traced = NormalizeArgs(traced).transform()
    """

    def __init__(
        self, module: smith.fx.GraphModule, normalize_to_only_use_kwargs: bool = True
    ):
...
class NormalizeOperators(AnnotateTypesWithSchema):
    ...
    1. Normalize operators (e.g. operator.add) to the `smith` ops they
       ultimately invoke (e.g. smith.add) when it is possible to statically
       reason that
    ...
    binary_magic_method_remap: dict[
        ...
        smith.add: operator.add,
        smith.mul: operator.mul,
        smith.sub: operator.sub,
        smith.div: operator.truediv,
        smith.floor_divide: operator.floordiv,
        smith.remainder: operator.mod,
        smith.eq: operator.eq,
        smith.ne: operator.ne,
        smith.lt: operator.lt,
        smith.le: operator.le,
        smith.gt: operator.gt,
        smith.ge: operator.ge,
    }
    ...
    def call_function(
        ...
        # https://github.com/blacksmith/blacksmith/blob/28c5d90b679c6b38bf4183ec99f16d933c2f1bcd/tools/autograd/templates/python_variable_methods.cpp#L1137 # noqa: B950
