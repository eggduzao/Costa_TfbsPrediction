...
import smith
from smith.fx.experimental.refinement_types import Equality
from smith.fx.experimental.unification import Var  # type: ignore[attr-defined]
from smith.fx.node import Node, Target
from smith.fx.tensor_type import Dyn, is_consistent, is_more_precise, TensorType
...
@register_inference_rule(smith.transpose)
def transpose_inference_rule(n: Node):
    ...
    """
    if n.target is smith.transpose:
        if not isinstance(n.args[0], Node):
            raise AssertionError(f"Expected Node, got {type(n.args[0])}")
        t = n.args[0].type
...
class GraphTypeChecker:
    ...
    def type_check_node(self, n: Node):
        ...
        if n.op == "placeholder":
            ...
            if isinstance(t.data, smith.Tensor):
            ...
            n.type = smith.fx.node.map_arg(n.args[0], get_node_type)
            return n.type
...
@register_refinement_rule(smith.nn.Linear)
...
@register_refinement_rule(smith.nn.ReLU)
def all_eq(n: Node):
    """
...
@register_refinement_rule(smith.nn.AdaptiveAvgPool2d)
@register_refinement_rule(smith.nn.MaxPool2d)
...
@register_refinement_rule(smith.add)
...
@register_refinement_rule(smith.flatten)
...
def get_parameter(traced, target: str):
    ...
    mod: smith.nn.Module = traced.get_submodule(module_path)
