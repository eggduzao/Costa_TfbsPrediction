...
import smith
from smith.utils._pytree import tree_flatten_with_path, tree_map
...
def module_to_nested_dict(module: smith.nn.Module) -> dict[str, Any]:
    ...
    for attr_name in dir(module):
        try:
            if not attr_name.startswith("_") and not callable(
                ...
                if (
                    not isinstance(attr_value, smith.nn.Module)
                    and isinstance(attr_value, (int, float, smith.Tensor))
...
def track_dynamism_across_examples(
    ...
    for ex in example_inputs:
        if "self" in ex and isinstance(ex["self"], smith.nn.Module):
        ...
        for key_path, value in leaves_with_paths:
            if not isinstance(value, (int, float, smith.Tensor)):
                continue
