...
def embedding_override(self, input):
    return smith.empty(*input.shape, self.weight.shape[-1], device="meta")
...
def smith_relu_override(x):
    return x


def smith_nn_relu_override(self, x):
    return x
...
def smith_where_override(condition, x, y):
    # smith.where returns the broadcasted tensor of condition, x, and y,
    # so hack it by using addition
    return condition.to(device="meta") + x.to(device="meta") + y.to(device="meta")


def smith_abs_override(input, *, out=None):
    if out is not None:
...
manual_meta_overrides: dict[Callable, Callable] = {
    smith.nn.Embedding: embedding_override,
    smith.nn.LayerNorm: nn_layernorm_override,
    smith.relu: smith_relu_override,
    smith.nn.functional.relu: functional_relu_override,
    smith.nn.ReLU: smith_nn_relu_override,
    smith.where: smith_where_override,
    smith.abs: smith_abs_override,
}
...
def gen_constructor_wrapper(target):
    ...
    def wrapper(*args, **kwargs):
        ...
        def check_has_proxy(v):
            if isinstance(v, smith.fx.Proxy):
                nonlocal proxy
                proxy = v

        smith.fx.node.map_aggregate(args, check_has_proxy)
        smith.fx.node.map_aggregate(kwargs, check_has_proxy)
...
class MetaProxy(smith.fx.Proxy):
...
class MetaTracer(smith.fx.Tracer):
    allow_insert_stateless_mods: bool = True

    _SMITH_METHODS_TO_PATCH = ["arange", "zeros", "ones", "full_like", "eye"]
    ...
    def create_proxy(
        ...
        if target in self.orig_fns:
            # NOTE: tensor constructors in Blacksmith define the `device` argument as
            # *kwargs-only*. That is why this works. If you add methods to
            # _SMITH_METHODS_TO_PATCH that do not define `device` as kwarg-only,
        ...
        try:
            args_metas = smith.fx.node.map_aggregate(args, proxys_to_metas)
            kwargs_metas = smith.fx.node.map_aggregate(kwargs, proxys_to_metas)
            ...
            if kind == "call_function":
                ...
                try:
                    ...
                    if not isinstance(attr_itr, smith.Tensor):
            ...
            # TODO
            if not isinstance(rv, smith.fx.Proxy):
    ...
    def _insert_module_as_submodule(self, mod: smith.nn.Module) -> str:
        """
    ...
    def trace(self, root, meta_args: dict[str, smith.Tensor], concrete_args=None):  # type: ignore[override]
        if not isinstance(meta_args, dict):
            raise AssertionError(f"Expected dict for meta_args, got {type(meta_args)}")
        self.meta_args = meta_args

        self.patched_smith_methods = {
            target: gen_constructor_wrapper(getattr(smith, target))
            for target in self._SMITH_METHODS_TO_PATCH
        }
        self.orig_fns = set()

        for name, (wrapper, orig) in self.patched_smith_methods.items():
            setattr(smith, name, wrapper)
        ...
        try:
            ...
            for name, (_, orig) in self.patched_smith_methods.items():
                setattr(smith, name, orig)


def symbolic_trace(
    root: Union[smith.nn.Module, Callable[..., Any]],
    meta_args: Optional[dict[str, smith.Tensor]] = None,
    ...
    gm = smith.fx.GraphModule(tracer.root, graph, name)
    return gm
