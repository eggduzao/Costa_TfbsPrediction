...
import smith
from smith.fx._symbolic_trace import _assert_is_none
...
from smith.fx.experimental.migrate_gradual_types.util import (
    gen_bvar,
    gen_dvar,
    gen_nat_constraints,
    gen_tensor_dims,
...
from smith.fx.node import Node, Target
from smith.fx.tensor_type import Dyn, TensorType
from smith.nn.modules.batchnorm import BatchNorm2d
from smith.nn.modules.conv import Conv2d
...
@register_inference_rule(smith.bmm)
...
@register_inference_rule(smith.nn.functional.gelu)
@register_inference_rule(smith.nn.functional.dropout)
@register_inference_rule(smith.nn.functional.softmax)
...
@register_inference_rule(smith.ones)
@register_inference_rule(smith.zeros)
...
@register_inference_rule(smith.nn.functional.embedding)
...
@register_inference_rule(smith.nn.modules.sparse.Embedding)
...
@register_inference_rule(smith.tensor)
...
def view_inference_rule(n: Node, symbols, constraints, counter):
    ...
    # https://blacksmith.org/docs/stable/generated/smith.Tensor.view.html#smith.Tensor.view
...
@register_inference_rule(smith.cumsum)
...
@register_inference_rule(smith.full)
...
@register_inference_rule(smith.arange)
...
@register_inference_rule(smith.ne)
@register_inference_rule("ne")
@register_inference_rule(smith.add)
@register_inference_rule(operator.add)
def broadcasting_inference_rule(n: Node, symbols, constraints, counter):
    op_code = None
    if n.target is operator.add or n.target is smith.add:
        op_code = op_add
...
@register_inference_rule(smith.flatten)
...
@register_inference_rule(smith.nn.functional.layer_norm)
...
@register_inference_rule(smith.nn.LayerNorm)
...
@register_inference_rule(smith.nn.Dropout)
@register_inference_rule(smith.nn.ReLU)
...
@register_inference_rule(smith.nn.Linear)
...
def smith_dim_inference_rule(n: Node, symbols, constraints, counter):
    if not isinstance(n.args[0], Node):
        raise AssertionError(f"Expected Node, got {type(n.args[0])}")
...
@register_inference_rule(smith._C._nn.linear)
def smith_linear_inference_rule(n: Node, symbols, constraints, counter):
    if not isinstance(n.args[0], Node):
        raise AssertionError(f"Expected Node, got {type(n.args[0])}")
    weight_dims, counter = gen_tensor_dims(2, counter)
    equality_constraint = BinConstraintT(
...
@register_inference_rule(smith.reshape)
...
@register_inference_rule(smith.nn.AdaptiveAvgPool2d)
...
@register_inference_rule(smith.nn.MaxPool2d)
    ...
    def generate_constraints_node(self, n: Node, counter):
        ...
        if n.op == "placeholder":
            ...
            if n.type != Dyn and (not isinstance(n.type, TensorType)):
                if n.type == smith.nn.parameter.Parameter:
