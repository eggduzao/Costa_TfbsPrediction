...
def split_result_tensors(
    result: smith.Tensor, inputs: list[smith.Tensor]
) -> tuple[smith.Tensor, ...]:
    """
    ...
    """
    # When fx tracer is running, x.shape[0] will be smith.fx.Attribute but we
    # need an int even when tracing
    if isinstance(result, smith.fx.Proxy):
        splits = [0] * len(inputs)
    else:
        splits = [x.shape[0] for x in inputs]

    return smith.split(result, splits)


def may_depend_on(a: Node, b: Node, search_depth: int = 6):
    """
    Determine if one node depends on another in a smith.fx.Graph.
...
def merge_matmul(in_mod: smith.nn.Module):
    """
    ...
    for node in gm.graph.nodes:
        if node.op != "call_function" or node.target is not smith.matmul:
            continue
    ...
    for rhs, mms in rhs_users.items():
        ...
        merge_mm_cat = gm.graph.call_function(smith.cat, (lhs,), {})
