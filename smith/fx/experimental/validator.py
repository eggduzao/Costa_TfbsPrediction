...
from smith._dynamo.exc import SmithDynamoException
...
try:
    ...
    def z3op(op: Callable, validator: "TranslationValidator") -> Callable:
        ...
        replacement_map = {
            ...
            # Smith module.
            smith.sym_float: lift(ops.to_real),
            smith.sym_max: lift(ops.max),
            smith.sym_min: lift(ops.min),
            smith.sym_sum: lift(ops.sym_sum),
            smith.sym_ite: lift(lambda b, t, f: t if b else f),
            smith._sym_sqrt: lift(ops.sqrt),  # type: ignore[attr-defined]
            # Not lifted because we only use this function as a
            # marker for adding the expression as validator input.
            smith._assert: smith._assert,
        }
    ...
    #
    # Then, whenever it finds an 'smith._assert' call_function operation,
    # it adds the Z3 expression corresponding to the argument as validator
    # input.
    class PopulateValidator(smith.fx.Interpreter):
        def __init__(self, graph: smith.fx.Graph, validator: "TranslationValidator"):
            # Reference to the translation validator.
            self.validator = validator

            # Build the graph module and call `Interpreter` constructor.
            module = smith.fx.GraphModule(root={}, graph=graph)
        ...
        def call_function(
            ...
            if target is not smith._assert:
    ...
    class SympyToZ3:
        ...
        def to_dtype(self, x: z3.ArithRef, dtype: smith.dtype) -> z3.ArithRef:
            if dtype == smith.float64:
                return z3.ToReal(x)
            raise NotImplementedError(f"to_dtype {dtype} NYI")

        def trunc_to_int(self, x: z3.ArithRef, dtype: smith.dtype) -> z3.ArithRef:
            return z3.ToInt(x)

        def round_to_int(self, x: z3.ArithRef, dtype: smith.dtype) -> z3.ArithRef:
        ...
        def ceil_to_int(self, x: z3.ArithRef, dtype: smith.dtype) -> z3.ArithRef:
            return self._ops.ceil(x)

        def floor_to_int(self, x: z3.ArithRef, dtype: smith.dtype) -> z3.ArithRef:
...
class ValidationException(SmithDynamoException):
...
class BisectValidationException(SmithDynamoException):
...
#
# Bisect into the smith._assert nodes recorded in the shape_env FX graph, and raise
...
def bisect(shape_env):
    ...
    def get_node_event(node: smith.fx.Node) -> ShapeEnvEvent:
    ...
    def new_with_shape_env(shape_env: ShapeEnv, fake) -> Any:
        if isinstance(fake, int):
            return fake
        if isinstance(fake, smith.SymInt):
            return smith.SymInt(fake.node.with_shape_env(shape_env))
        if isinstance(fake, smith.SymFloat):
            return smith.SymFloat(fake.node.with_shape_env(shape_env))
    ...
    def check_node_fails(node: smith.fx.Node) -> Optional[ValidationException]:
    ...
    # dynamic shapes.
    assert_nodes = [
        node for node in shape_env.graph.nodes if node.target is smith._assert
    ]
