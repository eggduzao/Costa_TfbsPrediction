...
class FoldedGraphModule(smith.fx.GraphModule):
    """
    ...
    def __init__(
        ...
        const_subgraph: Optional[smith.fx.Graph] = None,
        ...
        self.const_subgraph_module = (
            ...
            else smith.fx.GraphModule(root, const_subgraph)
        )
    ...
    def run_folding(self):
        ...
        def _create_param(i):
            return smith.nn.Parameter(
                i.detach().clone()
                if not isinstance(i, int)
                else smith.Tensor([i]).to(device=self.device_for_folded_attrs),
                requires_grad=i.requires_grad if isinstance(i, smith.Tensor) else False,
            )

        params = (
            smith.nn.ParameterList([_create_param(i) for i in folded_attrs])
...
def _inline_module(
    gm: smith.fx.GraphModule, inline_mod_name: str, run_dce: bool = True
) -> dict[smith.fx.Node, smith.fx.Node]:
    """
    ...
    if not isinstance(inline_mod, smith.fx.GraphModule):
    ...
    replacement_mapping: dict[smith.fx.Node, smith.fx.Node] = {}
    ph_count = 0
    ...
    for inline_node in inline_mod.graph.nodes:
        ...
        if inline_node.op == "output":
            ...
            getitem_users: list[smith.fx.Node] = []
...
def get_unique_attr_name_in_module(mod_traced: smith.fx.GraphModule, name: str) -> str:
    """
...
def split_const_subgraphs(
    module: Union[smith.nn.Module, smith.fx.GraphModule],
    ...
    if not isinstance(module, smith.fx.GraphModule):
        mod_traced = smith.fx.symbolic_trace(module)
    else:
        mod_traced = module

    def _subgraph_has_impure_ops(module: smith.fx.GraphModule) -> bool:
        """
        ...
        for node in module.graph.nodes:
            ...
            if (
                ...
                and isinstance(submodule, smith.fx.GraphModule)
            ):
    ...
    const_nodes: set[smith.fx.Node] = set()
    ...
    for node in mod_traced.graph.nodes:
        ...
        if (
            ...
            and isinstance(target_mod, smith.fx.GraphModule)
    ...
    def mod_partition(node: smith.fx.Node):
    ...
    #    return add
    root_const_gm = smith.fx.GraphModule(split, const_gm.graph)
    ...
    setattr(
        ...
        smith.nn.ParameterList() if multiple_outputs else smith.nn.Parameter(),  # type: ignore[possibly-undefined]
    )
    for node in split.graph.nodes:
