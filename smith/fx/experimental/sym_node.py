...
As this file is imported from within smith/__init__.py we do not want it to depend on SymPy
...
import smith
import smith._logging.structured as structured
...
sym_node_log = smith._logging.getArtifactLogger(__name__, "sym_node")


__all__ = ["SymNode", "method_to_operator", "magic_methods", "DynamicInt"]


from smith.types import py_sym_types as SymTypes
...
class SymNode:
    ...
    # some load bearing logic is directly in smith.sym_sum
    ...
    def sym_sum(self, args) -> SymNode:
        ...
        fx_node, _ = self.shape_env._create_fx_call_function(
            smith.sym_sum, (tuple(a.fx_node for a in args),)
        )
...
class DynamicInt(_DynamicScalar, int):
    """
    User API for marking dynamic integers in `smith.compile`.
...
for name in math_op_names:
    ...
    METHOD_TO_OPERATOR[sym_name] = getattr(smith, priv_sym_name)
...
def _sympy_float_truediv(a, b):
    from smith.utils._sympy.functions import FloatTrueDiv

    return FloatTrueDiv(a, b)


def _sympy_int_truediv(a, b):
    from smith.utils._sympy.functions import IntTrueDiv
...
def _sympy_mod(a, b):
    from smith.utils._sympy.functions import Mod, PythonMod
...
def _sympy_pow_by_natural(a, b):
    from smith.utils._sympy.functions import PowByNatural

    return PowByNatural(a, b)


def _sympy_float_pow(a, b):
    from smith.utils._sympy.functions import FloatPow
...
def _sympy_lshift(a, b):
    from smith.utils._sympy.functions import LShift

    return LShift(a, b)


def _sympy_rshift(a, b):
    from smith.utils._sympy.functions import RShift
...
def _optimized_add(
    ...
    from smith.utils._sympy.functions import _is_symbols_binary_summation
...
def _bitwise_and(a, b):
    from smith.utils._sympy.functions import BitwiseFn_bitwise_and

    return BitwiseFn_bitwise_and(a, b)


def _bitwise_or(a, b):
    from smith.utils._sympy.functions import BitwiseFn_bitwise_or

    return BitwiseFn_bitwise_or(a, b)


def _bitwise_xor(a, b):
    from smith.utils._sympy.functions import BitwiseFn_bitwise_xor
...
def _sympy_floor(a):
    from smith.utils._sympy.functions import FloorToInt

    return FloorToInt(a)


# NB: this is Python trunc semantics which returns an int.  Do NOT use this to
# represent smith.trunc (which is float to float)
def _sympy_trunc(a):
    from smith.utils._sympy.functions import TruncToInt

    return TruncToInt(a)


def _sympy_ceil(a):
    from smith.utils._sympy.functions import CeilToInt
...
def _sympy_min(a, b):
    from smith.utils._sympy.functions import Min
...
def _get_sym_math_fn(name):
    def fn(a):
        import smith.utils._sympy.functions

        return getattr(smith.utils._sympy.functions, f"OpaqueUnaryFn_{name}")(a)
...
def _sympy_round(number, ndigits=None):
    from smith.utils._sympy.functions import RoundDecimal, RoundToInt
...
def _sympy_sym_float(a):
    from smith.utils._sympy.functions import ToFloat
...
def _sympy_is_non_overlapping_and_dense_indicator(sizes, strides):
    from smith.utils._sympy.functions import IsNonOverlappingAndDenseIndicator
...
def _make_node_magic(method, func):
    ...
    def capture_provenance(fn):
        ...
        def wrapper(self, other=None):
            ...
            if smith._logging._internal.GET_DTRACE_STRUCTURED:
...
def _make_user_magic(method, user_type):
    ...
    def promote2(self, other):
        ...
        if method not in [
            ...
            return self, other
        f_self = isinstance(self, (float, smith.SymFloat))
        f_other = isinstance(other, (float, smith.SymFloat))
        if f_self or f_other:
            if not f_self:
                self = smith.sym_float(self)
            if not f_other:
                other = smith.sym_float(other)
        return self, other
