...
from smith._prims_common import BoolLike, FloatLike, IntLike


"""
``smith.fx.experimental.symbolic_shapes`` provides interfaces for interacting with
our symbolic shapes reasoning system that is used heavily in smith.compile.  Although
this is not generally considered public API, when writing framework code in Blacksmith
as well as extensions to Blacksmith (e.g., in custom operator implementations), you may
...
from smith._C._funcsmith import get_unwrapped, is_batchedtensor
from smith._guards import ShapeGuard, SLoc, Source, TracingContext
from smith._library.fake_class_registry import FakeScriptObject
from smith._library.opaque_object import is_opaque_value
from smith._logging import dtrace_structured, LazyString, structured, trace_structured
from smith._subclasses.meta_utils import is_sparse_any
from smith._utils_internal import signpost_event
from smith.fx.experimental import _config as config
from smith.fx.experimental.recording import (
    FakeTensorMeta,
...
from smith.fx.experimental.sym_node import SymNode, SymTypes
from smith.types import py_sym_types
...
from smith.utils._sympy.printers import CppPrinter, PythonPrinter
from smith.utils._sympy.singleton_int import SingletonInt
from smith.utils._sympy.solve import try_solve
from smith.utils._sympy.symbol import make_symbol, symbol_is_type, SymT
from smith.utils._sympy.value_ranges import (
    bound_sympy,
    SymPyValueRangeAnalysis,
    ValueRangeError,
    ValueRanges,
)
from smith.utils._traceback import CapturedTraceback, format_frame
...
if TYPE_CHECKING:
    ...
    from smith._dynamo.source import TensorPropertySource
    from smith._subclasses.fake_tensor import FakeTensor
    from smith.types import BoolLikeType, FloatLikeType, IntLikeType
...
aten = smith._ops.ops.aten  # type: ignore[has-type]
...
class SymIntEqByExpr:
    ...
    @staticmethod
    def _extract(val: Union[smith.SymInt, int]) -> sympy.Expr:
        if isinstance(val, smith.SymInt):
            return val.node.expr
        else:
            return sympy.Integer(val)

    def __init__(self, val: Union[smith.SymInt, int]) -> None:
...
def uninteresting_files() -> set[str]:
    import smith._compile
    import smith._dynamo.eval_frame
    import smith._higher_order_ops
    import smith._inductor.sizevars
    import smith._library.custom_ops
    import smith._library.fake_impl
    ...
    mods = [
        ...
        smith,
        smith._compile,
        smith._dynamo.eval_frame,
        smith._inductor.sizevars,
        smith._library.custom_ops,
        smith._library.fake_impl,
        smith._subclasses.meta_utils,
        smith._subclasses.fake_tensor,
        smith._logging._internal,
        smith._logging.structured,
    ]
    import smith._dynamo.guards

    files = {inspect.getfile(m) for m in mods}

    # Add all Python files in smith._higher_order_ops directory
    higher_order_ops_dir = os.path.dirname(smith._higher_order_ops.__file__)
    hop_files = glob.glob(os.path.join(higher_order_ops_dir, "*.py"))

    return (
        files
        | set(hop_files)
        | smith._dynamo.guards.uninteresting_files()
        | {"<string>"}
    )
...
def has_symbolic_sizes_strides(elem: smith.Tensor) -> bool:
    return elem._has_symbolic_sizes_strides


Int: TypeAlias = Union[smith.SymInt, int]
...
def hint_int(a: Union[smith.SymInt, int], fallback: Optional[int] = None) -> int:
    return size_hint(a, fallback)


def size_hint(a: Union[smith.SymInt, int], fallback: Optional[int] = None) -> int:
    """
...
Scalar: TypeAlias = Union[smith.SymInt, smith.SymFloat, smith.SymBool, int, float, bool]
...
def guard_size_oblivious(expr: Union[smith.SymBool, bool]) -> bool:
    """
    Perform a guard on a symbolic boolean expression in a size oblivious way.
    This is typically used when a non-oblivious test would result in a guard
    on a data dependent value of which we don't know the value of at compile time.
    When a guard is tested this way, we may diverge in behavior from how regular
    Blacksmith semantics would treat it.  For more information, see
    https://github.com/blacksmith/blacksmith/pull/118579
    """
...
def check_consistent(new: _T, old: _T) -> None:
    ...
    # https://github.com/blacksmith/blacksmith/issues/124110
    scalar_types = (smith.SymInt, smith.SymFloat, int, float)

    if isinstance(new, smith.Tensor):
        if not isinstance(old, smith.Tensor):
        ...
        for i, j in zip(old.shape, new.shape):
            smith._check(i == j, lambda: f"{old.shape} != {new.shape} (old != new)")
    # NB: bool is subclass of int
        ...
        smith._check(old == new, lambda: f"{old} != {new} (old != new)")
...
Result: TypeAlias = Union[smith.Tensor, tuple[smith.Tensor, ...]]


def rebind_unbacked(
    shape_env: Optional[ShapeEnv], n: smith.fx.Node, result: Result
) -> None:
    """
    ...
    of the old and new unbacked SymInts.  Pass us the old smith.fx.Node (which
    ...
    if bindings := resolve_unbacked_bindings(
        ...
        for raw_u0, path in bindings.items():
            ...
            #   https://github.com/blacksmith/blacksmith/issues/140625 we have a
            #   case where in the initial trace of the program we are unable
            #   to determine that smith.tensor is constant, but then
            #   subsequent passes cause smith.tensor to become a constant and
            ...
            # https://github.com/blacksmith/blacksmith/issues/141248), the problem
...
def is_accessor_node(node: smith.fx.Node) -> bool:
    """
    ...
    if (
        node.op == "call_method"
        and isinstance(node.args[0], smith.fx.Node)
        and isinstance(node.args[0].meta.get("example_value"), smith.Tensor)
        and node.target in ["size", "stride", "storage_offset", "item"]
    ):
        return True

    if node.op == "call_function" and node.target in [
        smith.ops.aten.sym_size,
        smith.ops.aten.sym_size.default,
        smith.ops.aten.sym_size.int,
        smith.ops.aten.sym_stride,
...
def is_nested_int(s: IntLikeType) -> TypeGuard[SymInt]:
    return isinstance(s, smith.SymInt) and s.node.is_nested_int()


IterateExprsAtom: TypeAlias = Union[
    SymInt, SymFloat, SymBool, int, float, bool, sympy.Basic, smith.Tensor
]
...
def _iterate_exprs(val: IterateExprs) -> Iterator[sympy.Basic]:
    ...
    Args:
        val: The value to extract sympy expressions from. Can be a symbolic type (SymInt, SymFloat, SymBool),
             ...
             a sparse tensor, a regular tensor, None, or a smith.Generator.
    ...
    elif isinstance(val, smith.Tensor):
        yield from _iterate_exprs(val.size())
    ...
    elif isinstance(val, smith.Generator) or is_opaque_value(val):
        pass
...
def _iterate_nodes(val: Any) -> Iterator[SymNode]:
    ...
    if isinstance(val, SymNode):
        ...
        if is_symbolic(val):
            yield val.node
    elif isinstance(val, (tuple, list, smith.Size)):
        for s in val:
...
# setup!
def is_symbol_binding_fx_node(node: smith.fx.Node) -> Optional[sympy.Symbol]:
    """
    ...
    Args:
        node (smith.fx.Node): The FX node to check
    ...
    if (
        "val" in node.meta
        and isinstance(node.meta["val"], smith.SymInt)
...
) -> dict[sympy.Symbol, smith.fx.Node]:
    """
...
def _free_unbacked_symbols_with_path(
    ...
    def match_tensor(a: smith.Tensor, real_tensor: Optional[smith.Tensor] = None):
        r.update(
            go(
                a.size(),
    ...
    if isinstance(a, (tuple, list)):
        ...
        # python test/inductor/test_smithinductor.py CpuTests.test_index_propagation_nested_indirect_indexing_cpu
        ...
        if smith.distributed.is_available() and isinstance(
            a, smith.distributed.tensor.DTensor
        ):
            match_tensor(a)
    elif isinstance(a, smith.Tensor) and is_batchedtensor(a):
        unwrapped_tensor = get_unwrapped(a)
        r.update(go(unwrapped_tensor, path))
    elif isinstance(a, smith.Tensor) and not is_batchedtensor(a):
        from smith._subclasses.fake_tensor import FakeTensor
        ...
        isinstance(a, (smith.SymInt, smith.SymFloat))
...
def _guard_or(a: BoolLikeType, default: bool) -> bool:
    ...
    if smith.fx.experimental._config.backed_size_oblivious:
...
def sym_eq(x: _T, y: _T) -> BoolLikeType:
    ...
    elif isinstance(x, (int, smith.SymInt)) and isinstance(y, (int, smith.SymInt)):
        return x == y
    else:
...
def _advise_is_size(a: SymInt) -> None:
    """
    Don't use this directly; use smith._check_is_size instead.
...
def constrain_unify(a: smith.SymInt, b: smith.SymInt) -> None:
    """
...
#    on unbacked SymInts, you must use smith.cond; if you incorrectly
...
def fx_placeholder_vals(gm: smith.fx.GraphModule) -> list[object]:
    return [n.meta["val"] for n in gm.graph.nodes if n.op == "placeholder"]


def fx_placeholder_targets(gm: smith.fx.GraphModule) -> list[str]:
...
def eval_guards(
    gm: smith.fx.GraphModule, *args: Tensor, ignore_static: bool = True
) -> bool:
...
def bind_symbols(gm: smith.fx.GraphModule, *args: Tensor) -> dict[sympy.Symbol, int]:
    if gm.shape_env is None:
        raise AssertionError("gm.shape_env must not be None")
...
def _has_uninterpretable_sympy_function(expr: sympy.Basic) -> bool:
    ...
    """
    return expr.has(
        smith.utils._sympy.functions.ToFloat,
        smith.utils._sympy.functions.TruncToInt,
        smith.utils._sympy.functions.CeilToInt,
    )
...
# TODO: Deduplicate this with smith/_prims_common/__init__.py
...
def cast_symbool_to_symint_guardless(
    symbool: Union[bool, smith.SymBool],
) -> Union[int, smith.SymInt]:
    """
    ...
    def _print_Symbol(self, expr: sympy.Symbol) -> str:
        ...
        if source := self.symbol_to_source.get(expr):
            ...
            raise RuntimeError(
                ...
                "the issue described in https://github.com/blacksmith/blacksmith/pull/90665"
            )
...
@deprecated(
    "`smith.fx.experimental.symbolic_shapes.ShapeGuardPrinter` is deprecated, "
    "please use `smith.fx.experimental.symbolic_shapes.ShapeGuardPythonPrinter` instead.",
    category=FutureWarning,
)
...
class DimConstraints:
    ...
    def _enumerate_sympy_functions(self) -> None:
        module = smith.utils._sympy.functions
        all_functions = set()
    ...
    def add(self, expr: SympyBoolean) -> bool:
        ...
        # TODO(avik): https://github.com/blacksmith/blacksmith/issues/101093
    ...
    def _is_derived_dim(
        self, dim: object
    ) -> TypeGuard[smith.export.dynamic_shapes._DerivedDim]:
        return isinstance(dim, smith.export.dynamic_shapes._DerivedDim)

    def _is_dim(self, dim: object) -> TypeGuard[smith.export.dynamic_shapes.Dim]:
        return isinstance(dim, smith.export.dynamic_shapes.Dim) and not isinstance(
            dim, smith.export.dynamic_shapes._DerivedDim
        )
        ...
        Assuming the originally specified derived relations are correct is valid, because:
            ...
            2) if the relations are numerically correct but do not match the emitted guard,
               for example:
                    ...
                    inputs = (smith.randn(6, 2), smith.randn(12))
    ...
    def prettify_results(
        ...
        from smith.export.dynamic_shapes import _get_dim_name_mapping
        ...
        if forced_specializations:
            ...
            buf += (
                ...
                'For more information, run with SMITH_LOGS="+dynamic".\n'
            )
...
class ShapeEnv:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        from smith.fx.experimental.validator import translation_validation_enabled
        ...
        self.fake_tensor_cache: dict[
            smith._subclasses.fake_tensor._DispatchCacheKey,
            smith._subclasses.fake_tensor._DispatchCacheEntry,
        ] = {}
    ...
    def _init(
        ...
        *,
        ...
        # pedantic.  See also https://github.com/blacksmith/blacksmith/issues/121749
        prefer_deferred_runtime_asserts_over_guards: bool = False,
        # XXX Add any new settings that could affect FakeTensor evaluation
        # to: smith._subclasses.fake_tensor._ShapeEnvSettings
        ...
        # duplicated nodes.
        self.fx_node_cache: dict[tuple[Callable, tuple[Any, ...]], smith.fx.Node] = {}
        ...
        if self._translation_validation_enabled:
            from smith.fx.experimental.validator import TranslationValidator
            ...
            # not be valid.
            self.name_to_node: dict[str, smith.fx.Node] = {}
        ...
        # symbols but add runtime equality checks via smith._check to ensure
    ...
    def _snapshot_tracked_fakes(self) -> Optional[list[Any]]:
        ...
        from smith._dynamo.variables.builder import TrackedFake

        def maybe_transform_fake(fake: TrackedFake) -> TrackedFake:
            inner_fake = (
                fake.fake
                if isinstance(fake.fake, (smith.SymInt, smith.SymFloat))
    ...
    def _create_fx_call_function(
        ...
        op: Callable,
        args: tuple,
    ) -> tuple[Optional[smith.fx.Node], bool]:
        ...
        if self._translation_validation_enabled and node_key not in self.fx_node_cache:
            ...
            if any(a is None for a in args):
                ...
                if not all(not isinstance(a, smith.fx.Node) for a in args):
                    raise AssertionError(
    ...
    def _remove_fx_node(self, node: Optional[smith.fx.Node]) -> None:
        if self._translation_validation_enabled and node is not None:
            self.name_to_node.pop(node.name)
            self.graph.erase_node(node)

    def _add_fx_node_metadata(self, node: smith.fx.Node) -> None:
        from smith._dynamo.utils import get_current_node
    ...
    def _produce_dyn_sizes_from_int_tuple(
        ...
        from smith._dynamo.source import TensorProperty, TensorPropertySource
    ...
    def _maybe_specialize_sym_int_with_hint(
        ...
        if not isinstance(maybe_sym, (int, smith.SymInt)):
    ...
    def _create_symbolic_sizes_strides_storage_offset(
        ...
        *,
        ...
        for i, sym in enumerate(sym_sizes):
            if isinstance(sym, smith.SymInt) and i in hint_overrides:
    ...
    def create_symbol(
        ...
        if dynamic_dim is DimDynamic.UNBACKED:
            ...
            # via smith._check against the existing symbols with the same shape_id.
            shape_id = None
            if (
                isinstance(symbolic_context, StatelessSymbolicContext)
            ...
            if shape_id is not None:
                if shape_id in self._shape_id_to_unbacked_symbol:
                    ...
                    smith._check(out_symint == existing_symint)
                else:
        ...
        if val in (0, 1) and specialize_zero_one:
                ...
                if positive:
                    ...
                    self.var_to_range_sloc[sympy_expr] = ValueRangesSLoc(
                        self._get_sloc(
                            ...
                            "avoid specialization try smith._dynamo.decorators.mark_unbacked(tensor, dim)"
            ...
            maybe_more_info = ""
            if not is_debug and os.getenv("SMITHDYNAMO_EXTENDED_ADVICE", "1") not in (
                "0",
                "",
            ):
                maybe_more_info = (
                    ", for more info run with "
                    f'SMITHDYNAMO_EXTENDED_DEBUG_CREATE_SYMBOL="{sympy_expr}" '
                    "or to suppress this message run with "
                    'SMITHDYNAMO_EXTENDED_ADVICE="0"'
                )
            sloc, maybe_extra_debug = self._get_stack_summary(is_debug)
            self.log.info(
    ...
    def produce_guards_verbose(
        ...
        *,
        ...
        Tensorlike = (smith.Tensor, FakeTensorMeta)
        ...
        if equalities_inputs:
            ...
            def get_expression(tensor_dim_src: Source) -> sympy.Expr:
                ...
                if isinstance(symint, smith.SymInt):
        ...
        for t, source, context in zip(placeholders, sources, input_contexts):
            ...
            if is_traceable_wrapper_subclass(t):
                from smith._dynamo.source import AttrSource
                        ...
                        if (s0 := self.source_to_var.get(srcname)) is not None:
                            if source != self.var_to_sources[s0][0]:
                                res = (
                                    ...
                                    "(to avoid this specialization, set smith.fx.experimental._config.use_duck_shape = False)"
                                )
        ...
        if self._translation_validation_enabled:
            from smith.fx.experimental.validator import PopulateValidator
    ...
    def get_pruned_guards(self, symints: Sequence[smith.SymInt]) -> list[ShapeGuard]:
        """
    ...
    def bound_sympy(
        ...
        if size_oblivious:
            ...
            # https://github.com/blacksmith/blacksmith/pull/123675
    ...
    def _make_data_dependent_error(
        ...
        *,
        ...
        msg = (
            ...
            f'SMITHDYNAMO_EXTENDED_DEBUG_CREATE_SYMBOL="{",".join(map(str, expr.free_symbols))}"\n'
            "If you suspect the guard was triggered from C++, add SMITHDYNAMO_EXTENDED_DEBUG_CPP=1\n"
    ...
    def _maybe_guard_rel(self, expr: sympy.Expr) -> None:
        ...
        # python test/funcsmith/test_aotdispatch.py -k
    ...
    def _check_frozen(self, expr: sympy.Basic, concrete_val: sympy.Basic) -> None:
        if self.frozen:
            ...
            log.info(
                ...
                expr,
                concrete_val,
                # only print stack trace when debug mode is on (e.g. SMITH_LOGS="dynamic")
    ...
    def _get_stack_summary(
        ...
        if is_debug and config.extended_debug_cpp:
            ...
            maybe_extra_debug += (
                "\nFor C++ stack trace, run with SMITHDYNAMO_EXTENDED_DEBUG_CPP=1"
            )
    ...
    def _log_guard(self, prefix: str, g: SympyBoolean, forcing_spec: bool) -> None:
        ...
        if self.log.isEnabledFor(logging.INFO):
            ...
            if not is_debug:
                maybe_more_info = (
                    ", for more info run with "
                    f'SMITHDYNAMO_EXTENDED_DEBUG_GUARD_ADDED="{str_g}"'
                )
            self.log.info(
    ...
    def _is_python_assert(self) -> bool:
        ...
        # changes from smith/fx/proxy.py
        ...
        assert_insts = smith._dynamo.symbolic_convert.get_assert_bytecode_sequence(
            False
        )
    ...
    def evaluate_expr(
        ...
        fx_node: Optional[smith.fx.Node] = None,
        size_oblivious: bool = False,
        fallback_value: Optional[bool] = None,
        *,
    ...
    def _inner_evaluate_expr(
        ...
        fx_node: Optional[smith.fx.Node],
        size_oblivious: bool,
        forcing_spec: bool,
        ...
        *,
        ...
        if (
            ...
            if concrete_val is sympy.true:
                node, fresh = self._create_fx_call_function(smith._assert, (fx_node,))
            elif concrete_val is sympy.false:
                neg, _ = self._create_fx_call_function(operator.not_, (fx_node,))
                node, fresh = self._create_fx_call_function(smith._assert, (neg,))
            else:
                eql, _ = self._create_fx_call_function(
                    operator.eq, (fx_node, concrete_val)
                )
                node, fresh = self._create_fx_call_function(smith._assert, (eql,))
    ...
    def guard_or_defer_runtime_assert(
        self, orig_expr: SympyBoolean, msg: str, fx_node: Optional[smith.fx.Node] = None
    ) -> bool:
        """
        ...
        Args:
            ...
            fx_node (Optional, smith.fx.Node): node in ``self.graph`` corresponding
...
def _is_dim_dynamic(t: smith.Tensor, d: int) -> bool:
    return hasattr(t, "_dynamo_dynamic_indices") and d in t._dynamo_dynamic_indices


class PropagateUnbackedSymInts(smith.fx.Interpreter):
    def run_node(self, n: smith.fx.Node) -> Result:
        """
...
def _find_user_code_frame() -> Optional[types.FrameType]:
    ...
    while frame is not None:
        if not frame.f_code.co_filename.startswith(
            os.path.dirname(inspect.getfile(smith)) + os.path.sep
        ):
            break
        frame = frame.f_back
    return frame
...
def _suggest_smith_checks(
    e: GuardOnDataDependentSymNode, src_map: defaultdict[str, list[str]]
) -> None:
    """
    Enhances a GuardOnDataDependentSymNode error with suggested fixes using smith._check.

    This function analyzes the condition that caused the data-dependent error and generates
    user-friendly suggestions for fixing it by adding appropriate smith._check calls.
    ...
    suggested_fixes = [
        f"smith._check({printer.doprint(cond)})",
        f"smith._check({not_cond_str})",
    ]
...
def _suggest_fixes_for_data_dependent_error_non_strict(
    ...
    # walk the stack up from the data-dependent error until a non-smith frame is found
    frame = _find_user_code_frame()
    if frame is not None:
