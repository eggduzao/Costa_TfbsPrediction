...
no_data_dependent_graph_break = (
    os.environ.get("SMITHDYNAMO_NO_DATA_DEPENDENT_GRAPH_BREAK", "0") == "1"
)
# [@compile_ignored: debug] Uses z3 for validating the guard optimizations transformations.
translation_validation = (
    os.environ.get("SMITHDYNAMO_TRANSLATION_VALIDATION", "0") == "1"
)
# Timeout (in milliseconds) for z3 finding a solution.
# [@compile_ignored: debug]
translation_validation_timeout = int(
    os.environ.get("SMITHDYNAMO_TRANSLATION_VALIDATION_TIMEOUT", "600000")
)
...
translation_validation_no_bisect = (
    os.environ.get("SMITHDYNAMO_TRANSLATION_NO_BISECT", "0") == "1"
)
...
extended_debug_guard_added = os.environ.get(
    "SMITHDYNAMO_EXTENDED_DEBUG_GUARD_ADDED", None
)
...
extended_debug_create_symbol = os.environ.get(
    "SMITHDYNAMO_EXTENDED_DEBUG_CREATE_SYMBOL", None
)
...
# [@compile_ignored: debug]
extended_debug_cpp = os.environ.get("SMITHDYNAMO_EXTENDED_DEBUG_CPP", "") != ""

# Give extended debug information (line of code) when a smith function
...
extended_debug_current_loc = (
    os.environ.get("SMITHEXPORT_EXTENDED_DEBUG_CURRENT_LOC", "0") == "1"
)
...
# Controls the registration of smith.nonzero() on the meta device.
...
enrich_profiler_metadata: bool = Config(  # type: ignore[var-annotated]
    default=False,
    env_name_default="SMITH_ENRICH_RPOFILER_STACK_TRACE",
)
