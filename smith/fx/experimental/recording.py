...
trace_shape_events_log = smith._logging.getArtifactLogger(
...
class ShapeEnvEvent:
    ...
    def run(self, shape_env=None) -> Any:
        ...
        def maybe_convert_node(x: Any) -> Any:
            if not isinstance(x, smith.fx.Node):
        ...
        if self.is_evaluate_expr() or self.is_defer_runtime_assert():
            ...
            # smith._assert, which will be added to the FX graph of the new shape_env.
...
def _extract_shape_env_and_assert_equal(args, kwargs):
    from smith.fx.experimental.symbolic_shapes import is_symbolic, ShapeEnv, SymTypes
...
# bisection bisects through smith._assert calls, we should save in all methods
# that adds a smith._assert call to the symbolic shapes FX graph.
#
...
class FakeTensorMeta:
    tensor_size: tuple[Union[int, smith.SymInt], ...]
    tensor_stride: tuple[Union[int, smith.SymInt], ...]
    tensor_storage_offset: Union[int, smith.SymInt]
    is_nested: bool

    def size(self) -> tuple[Union[int, smith.SymInt], ...]:
        return self.tensor_size

    def stride(self) -> tuple[Union[int, smith.SymInt], ...]:
        return self.tensor_stride

    def storage_offset(self) -> Union[int, smith.SymInt]:
