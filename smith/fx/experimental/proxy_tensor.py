...
from smith import SymBool, SymInt, Tensor
...
from smith._library.opaque_object import is_opaque_value, OpaqueType
from smith._logging import trace_structured
from smith._ops import HigherOrderOperator
from smith._subclasses.fake_impls import fast_detach
...
from smith.utils._thunk import Thunk
from smith.utils.weak import _WeakHashRef, WeakIdKeyDictionary, WeakTensorKeyDictionary
...
if TYPE_CHECKING:
    ...
    from smith.fx._symbolic_trace import PHBase
...
_AnyScriptObject = (smith.ScriptObject, FakeScriptObject)
_AnyScriptObjectType = Union[smith.ScriptObject, FakeScriptObject]
...
pytree.register_pytree_node(
    ...
    serialized_type_name="smith.Size",
)
# Ideally unflattening should not lose info, but we unflatten
# smith.Size to tuple (see above). This is necessary because the
# smith.Size constructor only accepts ints whereas our infra often
...
_pytree_subclasses_that_lose_info = {smith.Size: tuple}
...
from smith.types import py_sym_types, PySymType
...
_FAKE_TENSOR_ID_TO_PROXY_MAP_FOR_EXPORT: dict[int, smith.fx.Node] = {}
...
def set_proxy_slot(
    ...
    if isinstance(obj, Tensor):
        ...
        if obj not in tracer.symnode_tracker:
            ...
            # https://github.com/blacksmith/blacksmith/pull/125941/
            import sympy
...
def _get_proxies(t: smith.Tensor) -> list[Proxy]:
    proxies = []
    mode = smith._C._get_dispatch_mode(smith._C._SmithDispatchModeKey.PROXY)
    if mode is None:
        return proxies
    if not isinstance(mode, ProxySmithDispatchMode):
        raise AssertionError(f"Expected ProxySmithDispatchMode, got {type(mode)}")
    tracer = mode.tracer
    for t_inner in get_plain_tensors(t, out=[]):
        if isinstance(t_inner, FunctionalTensor):
            t_inner = smith._from_functional_tensor(t_inner.elem)
        if not isinstance(t_inner, smith.Tensor):
            continue
...
def _sympy_handlers() -> dict[type[sympy.Expr], Callable[..., Any]]:
    ...
    """
    import smith.utils._sympy.interp

    handlers = {}
    for k, v in smith.utils._sympy.interp.handlers().items():
...
def _build_proxy_for_sym_expr(
    ...
    This function is used when the ProxySmithDispatchMode sees a SymNode
...
def snapshot_fake(val: Tensor, include_real: bool = False) -> Optional[Tensor]:
    ...
    # but this saves us a full trip into __smith_dispatch__
...
# (most notably autograd, but also vmap, funcsmith transforms, etc).  This means
...
def track_tensor(
    ...
    for i, s in enumerate(tensor.shape):
        try_set_proxy_slot(
            ...
            lambda x, i: set_meta(
                tracer.create_proxy(
                    "call_function", smith.ops.aten.sym_size.int, (proxy, i), {}
                ),
                x,
            ),
            i,
    ...
    if not is_sparse_any(tensor):
        for i, s in enumerate(tensor.stride()):
            try_set_proxy_slot(
                ...
                lambda x, i: set_meta(
                    tracer.create_proxy(
                        "call_function", smith.ops.aten.sym_stride.int, (proxy, i), {}
                    ),
                    x,
                ),
                i,
    ...
    try_set_proxy_slot(
        ...
        lambda x: set_meta(
            tracer.create_proxy(
                "call_function", smith.ops.aten.sym_numel.default, (proxy,), {}
            ),
            x,
        ),
    )
...
def track_tensor_tree(
    ...
    *,
    ...
    # working on https://github.com/blacksmith/blacksmith/pull/133585 because I
...
HANDLED_TYPES = (Tensor, smith.nn.Parameter, FakeTensor)


def _maybe_record_pointwise_barrier(
    func: object, proxy_mode: ProxySmithDispatchMode
) -> None:
    """
    ...
    output_low_precision = isinstance(t, smith.Tensor) and t.dtype in low_pr_fp
    ...
    if not output_low_precision:
        for input_node in last_node.all_input_nodes:
            ...
            if isinstance(val, smith.Tensor) and val.dtype in low_pr_fp:
...
def proxy_call(
    proxy_mode: ProxySmithDispatchMode,
    func: OpOverload,
    pre_dispatch: bool,
    args: tuple[object, ...],
    ...
    def can_handle_tensor(x: Tensor) -> bool:
        ...
        if proxy_mode._allow_fake_constant:
            r = r or type(x) is smith._subclasses.FakeTensor
        if not r:
    ...
    if (
        ...
        not in [
            ...
            smith.ops.aten.storage_offset.default,
        ]
    ...
    if func is smith.ops.aten.is_nonzero.default:
        with proxy_mode:
            smith._check(
    ...
    # When we trace through a smith.tensor invocation, you never actually
    # see a smith.ops.aten.tensor call. Instead, the way this function is
    ...
    #
    #       t = self._tensor_constant0  # initialized to smith.tensor(1)
    #       x = smith.ops.aten.lift_fresh(t)
    #       x.add_(2)
    #
    ...
    #
    #       t = self._tensor_constant0  # initialized to smith.tensor(1)
    #       x = smith.ops.aten.lift_fresh_copy(t)
    #       x.add_(2)
    #
    # This is what the overload modification does.
    if func is smith.ops.aten.lift_fresh.default:
        func = smith.ops.aten.lift_fresh_copy.default
    ...
    # you run smith.tensor; deterministic factory functions like smith.arange
    ...
    if (
        func is smith.ops.aten.lift_fresh_copy.default
        ...
        smith.Tag.nondeterministic_seeded not in func.tags
        and all_constant
        and any_constant
...
class PythonKeyTracer(Tracer):
    ...
    smith_fn_counts: dict[OpOverload, int]
    ...
    def __init__(self) -> None:
        ...
        # Stores the smith function that was called during tracing
        self.smith_fn_metadata = None
        # Stores the counts for every smith function called. This is to help
        # distinguish between different calls to the same smith function.
        self.smith_fn_counts = {}
    ...
    def create_arg(self, a: object) -> fx.node.Node:
        if isinstance(a, smith.nn.Parameter):
    ...
    def create_node(
        ...
        def map_fn(v: Any) -> Optional[_ExtractValType]:
            if not isinstance(v, smith.fx.Node) or "val" not in v.meta:
                return None
            val = v.meta["val"]
            ...
            if isinstance(val, smith.Tensor) and not isinstance(val, FakeTensor):
                return None
        ...
        if _should_save_eager_input_vals(target, (args, kwargs)):
            ...
            arg_inp, kwarg_inp = smith.fx.node.map_aggregate((args, kwargs), map_fn)  # type: ignore[misc, arg-type]
...
def _should_save_eager_input_vals(
    ...
    from smith._higher_order_ops.invoke_subgraph import InvokeSubgraphHOP
    ...
    if isinstance(
        target,
        (
            smith._higher_order_ops.triton_kernel_wrap.TritonKernelWrapperFunctional,
            smith._higher_order_ops.triton_kernel_wrap.TritonKernelWrapperMutation,
            InvokeSubgraphHOP,
        ),
    ):
        return True
    if args_kwargs is not None and (
        target is smith.ops.higher_order.auto_functionalized
        or target is smith.ops.higher_order.auto_functionalized_v2
    ):
        args = args_kwargs[0]
        if not isinstance(
            args[0], (smith._ops.OpOverload, smith._ops.HigherOrderOperator)
        ):
            raise AssertionError(
                f"Expected OpOverload or HigherOrderOperator, got {type(args[0])}"
            )
        return _should_save_eager_input_vals(args[0], None)
    if target is smith.ops.higher_order.with_effects:
    ...
    if isinstance(target, smith._ops.OpOverload):
        ...
        return get_layout_constraint_tag(target) == smith._C.Tag.needs_exact_strides
    return False


def _make_temp_remove_mode_context_manager(
    mode_ty: type[SmithFunctionMode],
) -> Callable[[], _GeneratorContextManager[Optional[SmithFunctionMode]]]:
    @contextmanager
    def context_manager_fn() -> Generator[Optional[SmithFunctionMode], None, None]:
        from smith.overrides import _len_smith_function_stack, _pop_mode, _push_mode

        temp_elements = []
        removed_mode = None

        while _len_smith_function_stack() > 0:
            mode = _pop_mode()
...
def dispatch_trace(
    ...
    from smith._inductor.fx_passes.dedupe_symint_uses import dedupe_symints
...
def wrap_key(
    ...
    def wrapped(*proxies: _P.args, **_unused: _P.kwargs) -> R:
        ...
        with disable_proxy_modes_tracing() as m:
            if not isinstance(m, ProxySmithDispatchMode):
                raise AssertionError(f"Expected ProxySmithDispatchMode, got {type(m)}")
            track_tensor_tree(flat_tensors, flat_proxies, constant=None, tracer=tracer)

        if getattr(tracer, "proxy_module_inputs", False):
            tensors = [  # type: ignore[assignment, var-annotated]
                p if isinstance(t, smith.nn.Module) else t
...
def set_original_aten_op(
    func: OpOverload | smith._ops.HigherOrderOperator,
) -> Generator[None, None, None]:
...
class SmithFunctionMetadataMode(SmithFunctionMode):
    ...
    def __smith_function__(
        ...
        self.tracer.smith_fn_metadata = func
        self.tracer.smith_fn_counts[func] = self.tracer.smith_fn_counts.get(func, 0) + 1
        return func(*args, **kwargs)


_temp_remove_metadata_smith_function_mode = _make_temp_remove_mode_context_manager(
    SmithFunctionMetadataMode
)
...
class PreDispatchSmithFunctionMode(SmithFunctionMode):
    def __init__(self, tracer: _ProxyTracer) -> None:
        self.tracer = tracer
        # The input to smith.amp.autocast_mode._exit_autocast graph node should be the
        # enter_autocast node. So we have to save the enter autocast node here, and assign it
        # to the exit_autocast call_function node.
        self.enter_autocast_nodes: list[smith.fx.Node] = []
    ...
    def __smith_function__(
        ...
        if func in _side_effectful_need_to_be_preserved_pre_dispatch:
            ...
            # here.
            if func is smith._C._set_grad_enabled:
                # pyrefly: ignore [bad-argument-type]
                func(*args, **kwargs)
            return node
        ...
        if func in [
            ...
            smith._funcsmith.vmap.lazy_load_decompositions,
        ]:
...
_temp_remove_pre_dispatch_smith_function_mode = _make_temp_remove_mode_context_manager(
    PreDispatchSmithFunctionMode
)


class ProxySmithDispatchMode(SmithDispatchMode):
    ...
    def __init__(
        ...
        dk = smith._C.DispatchKey.PreDispatch if pre_dispatch else None
        super().__init__(dk)
        self.tracer = tracer
    ...
    def __enter__(self) -> Self:
        ...
        maybe_prev_proxy_mode = _unset_infra_mode(smith._C._SmithDispatchModeKey.PROXY)
...
def _compute_proxy(
    ...
    # func doesn't have a __smith_function__ that Proxy can interpose, so
...
class _GraphAppendingTracerEx(fx.proxy.GraphAppendingTracer):
    ...
    tensor_tracker: MutableMapping[Tensor, _ProxyTensor]
    sympy_expr_tracker: dict[sympy.Symbol, _SympyExprTrackerValue]
    smith_fn_metadata: Optional[OpOverload]
    smith_fn_counts: dict[OpOverload, int]
    enable_thunkify: bool = False
...
class DecompositionInterpreter(fx.Interpreter):
    def __init__(
        ...
        **kwargs: object,
        ...
        self.mode = ProxySmithDispatchMode(self.tracer, tracing_mode="real")
    ...
    def run(self, *args: object, **kwargs: object) -> object:
        ...
        # See: https://github.com/blacksmith/blacksmith/pull/82549#discussion_r934782025
...
def disable_autocast_cache() -> Generator[None, None, None]:
    old_value = smith.is_autocast_cache_enabled()
    smith.set_autocast_cache_enabled(False)
    try:
        yield
    finally:
        smith.set_autocast_cache_enabled(old_value)
...
class _ModuleStackTracer(PythonKeyTracer):
    ...
    def __init__(self, scope_root: GraphModule) -> None:
        ...
        class AttrProxy(_AttrProxy):
            def __init__(self, base: Union[Module, _AttrProxy], path: str) -> None:
                ...
                # Class is modified to be a subclass of smith.nn.Module
            ...
            def __getattr__(self, name: str) -> AttrProxy:
                ...
                # Calling into smith.nn.Module.__getattr__ with super(),
            ...
            def __getitem__(self, idx: Union[int, slice]) -> AttrProxy:
                if isinstance(idx, slice):
                    if isinstance(self, smith.nn.Sequential):
                        # Copied from nn/modules/container.py
                        res = smith.nn.Sequential(
                            OrderedDict(list(self._modules.items())[idx])
                        )

                        return AttrProxy(res, f"{tracer.proxy_paths[self]}.{idx}")
                    elif isinstance(self, smith.nn.ModuleList):
                        # Copied from nn/modules/container.py
                        res = smith.nn.ModuleList(list(self._modules.values())[idx])
    ...
    def call_module(
        ...
        """
        from smith._dynamo import OptimizedModule

        # FIXME (tmanlaibaatar)
        # When we call smith.compile inside HOO, we will end up
    ...
    def create_node(self, *args: object, **kwargs: object) -> fx.node.Node:
        ...
        Add smith_fn by looking at smith_fn_metadata and smith_fn_counts.
        ...
        if (
            ...
            and self.smith_fn_metadata is not None
            and "smith_fn" not in node.meta
        ):
            node.meta["smith_fn"] = (
                f"{self.smith_fn_metadata.__name__}_{self.smith_fn_counts[self.smith_fn_metadata]}",
                f"{self.smith_fn_metadata.__class__.__name__}.{self.smith_fn_metadata.__name__}",
            )
...
class _MakefxTracer:
    def __init__(
        ...
        _disable_smith_fn_metadata_mode: bool = False,
    ) -> None:
        ...
        self.decomposition_table.setdefault(
            smith.ops.aten.sym_numel.default, smith._decomp.decompositions.sym_numel
        )
        ...
        self.proxy_mode: Union[nullcontext, ProxySmithDispatchMode] = nullcontext()
        self.proxy_function_mode: Union[nullcontext, PreDispatchSmithFunctionMode] = (
            nullcontext()
        )
        self.fx_tracer: Optional[PythonKeyTracer] = None
        self.python_dispatcher_mode: Union[nullcontext, Any] = nullcontext()
        self.smith_fn_metadata_mode: Union[nullcontext, SmithFunctionMetadataMode] = (
            nullcontext()
        )
        ...
        self._disable_smith_fn_metadata_mode = _disable_smith_fn_metadata_mode
    ...
    def _checkpoint_modes(self) -> list[Any]:
        return [
            ...
            self.smith_fn_metadata_mode,
        ]

    def _restore_modes(
        self,
        prev_fake_tensor_mode: Optional[FakeTensorMode],
        prev_proxy_mode: Union[nullcontext, ProxySmithDispatchMode],
        prev_proxy_function_mode: Union[nullcontext, PreDispatchSmithFunctionMode],
        prev_fx_tracer: Optional[PythonKeyTracer],
        prev_python_dispatcher_mode: Union[nullcontext, Any],
        prev_smith_fn_metadata_mode: Union[nullcontext, SmithFunctionMetadataMode],
    ) -> None:
        ...
        self.smith_fn_metadata_mode = prev_smith_fn_metadata_mode
    ...
    def _init_modes_from_inputs(
        ...
        try:
            ...
            if self.tracing_mode == "fake":
                ...
                fake_tensor_mode = smith._dynamo.utils.detect_fake_mode(args)
    ...
    def _construct_modes_with_fx_tracer(self, fx_tracer: _ProxyTracer) -> None:
        self.proxy_mode = ProxySmithDispatchMode(
            fx_tracer,
            self.tracing_mode,
        ...
        if self.pre_dispatch:
            self.proxy_function_mode = PreDispatchSmithFunctionMode(fx_tracer)
        ...
        if not self._disable_smith_fn_metadata_mode:
            self.smith_fn_metadata_mode = SmithFunctionMetadataMode(fx_tracer)
    ...
    def _trace_inner(self, f: Callable, *args: object) -> GraphModule:
        # TODO: We need to explicitly import smith._dynamo before calling dispatch_trace,
        # because dispatch_trace will introduce the lazy import of smith._dynamo,
        # and some contexts set before calling dispatch_trace will cause problems with the import of smith._dynamo,
        # such as some smith API(smith.ones and so on) in populate_builtin_to_tensor_fn_map() will be affected
        # by the context set before dispatch_trace.
        import smith._dynamo

        phs = pytree.tree_map(lambda _: smith.fx._symbolic_trace.PH, args)
        ...
        def _wrap_fake(args: T) -> T:
            ...
            def inner_wrap_fake(x: object) -> object:
                ...
                elif isinstance(x, smith.ScriptObject) or is_opaque_value(x):
                    return smith._library.fake_class_registry.maybe_to_fake_obj(
        ...
        proxy_mode: ProxySmithDispatchMode = typing.cast(
            ProxySmithDispatchMode, self.proxy_mode
        )
        with ExitStack() as stack:
            ...
            stack.enter_context(self.smith_fn_metadata_mode)
        ...
        if (
            ...
            and (fake_mode := smith._guards.detect_fake_mode(args))
...
def make_fx(
    ...
    *,
    ...
    make_fx_tracer = _MakefxTracer(
        ...
        _disable_smith_fn_metadata_mode=_disable_smith_fn_metadata_mode,
    )
...
def get_smith_dispatch_modes() -> list[SmithDispatchMode]:
    return smith.utils._python_dispatch._get_current_dispatch_mode_stack()


# TODO: this is a legacy name, there is only ever one proxy mode as it's an
# infra mode
def get_innermost_proxy_mode() -> Optional[ProxySmithDispatchMode]:
    return get_proxy_mode()


def get_proxy_mode() -> Optional[ProxySmithDispatchMode]:
    """
    Current the currently active proxy tracing mode, or None if
    we are not currently tracing.  This includes pre-dispatch proxy
    tracing.
    """
    pre_dispatch_mode = smith._ops._get_dispatch_mode_pre_dispatch(
        smith._C._SmithDispatchModeKey.PROXY
    )
    mode = smith._C._get_dispatch_mode(smith._C._SmithDispatchModeKey.PROXY)
...
def handle_sym_dispatch(
    ...
    # Have to do it manually, because we're not doing the normal smith
...
@contextmanager
def disable_proxy_modes_tracing() -> Generator[ProxySmithDispatchMode, None, None]:
    return _disable_infra_mode(smith._C._SmithDispatchModeKey.PROXY)
...
def _set_unbacked_bindings(out: object, out_proxy: _NestedProxys) -> None:
    ...
    fake_mode = smith._C._get_dispatch_mode(smith._C._SmithDispatchModeKey.FAKE)
