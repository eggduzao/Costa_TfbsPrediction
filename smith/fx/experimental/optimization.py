...
import smith.utils.mkldnn as th_mkldnn
from smith.fx.node import Argument, Target
from smith.fx.passes.shape_prop import ShapeProp
from smith.nn.utils.fusion import fuse_conv_bn_eval, fuse_linear_bn_eval
...
def replace_node_module(
    node: fx.Node, modules: dict[str, Any], new_module: smith.nn.Module
):
...
def fuse(model: smith.nn.Module, inplace=False, no_trace=False) -> smith.nn.Module:
    """
    ...
    if not no_trace or not isinstance(model, smith.fx.GraphModule):
...
def remove_dropout(model: nn.Module) -> nn.Module:
    ...
    class DropoutRemover(smith.fx.Transformer):
        def call_module(
    ...
    for node in nodes:
        if node.op == "call_module":
            ...
            if type(cur_module) in mkldnn_map:
                ...
                new_module = mkldnn_map[type(cur_module)](cur_module, smith.float)
...
def gen_mkl_autotuner(example_inputs, iters=10, warmup=1):
    ...
    def use_mkl_heuristic(graph: MklSubgraph) -> bool:
        ...
        sample_inputs = [smith.randn(node.shape) for node in input_nodes]  # type: ignore[attr-defined]
        ...
        if node.op == "call_module":
            ...
            if type(cur_module) in mkldnn_supported:
                ...
                if sample_parameter is not None:
                    if sample_parameter.dtype != smith.float:
                        raise AssertionError(
                            "this pass is only for smith.float modules"
                        )
                    if sample_parameter.device != smith.device("cpu"):
