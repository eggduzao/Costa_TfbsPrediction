...
from smith._subclasses.fake_tensor import FakeTensor, FakeTensorMode, Tensor
...
_SymNodeT = TypeVar("_SymNodeT", smith.SymInt, smith.SymFloat)


def _ops_filter_safe(name: str) -> bool:
    """
    An ops filter which allows pickle-safe ops. Pickle-safe ops are built-in
    ones where it will be possible to unpickle on any machine which has Blacksmith.
    """
...
class GraphPickler(pickle.Pickler):
    ...
    def reducer_override(
        ...
        elif isinstance(obj, smith.fx.GraphModule):
            return _GraphModulePickleData.reduce_helper(self, obj)
        elif isinstance(obj, (smith._ops.OperatorBase, smith._ops.OpOverloadPacket)):
...
class _SymNodePickleData:
    ...
    def reduce_helper(
        ...
        if isinstance(obj, smith.SymInt):
    ...
    def unpickle_sym_int(self, unpickle_state: _UnpickleState) -> smith.SymInt:
        return smith.SymInt(self._to_sym_node())
...
class _TensorPickleData:
    ...
    def __init__(self, describer: MetaTensorDescriber, t: Tensor) -> None:
        ...
        if metadata.view_func and not isinstance(
            metadata.view_func, smith._subclasses.meta_utils._FakeTensorViewFunc
        ):
    ...
    def unpickle(self, unpickle_state: _UnpickleState) -> FakeTensor:
        ...
        def with_fake(
            make_meta_t: Callable[[], smith.Tensor], device: Union[smith.device, str]
        ) -> FakeTensor:
            with no_dispatch():
                return FakeTensor(
...
class _SmithNumpyPickleData:
    ...
    def unpickle(self, unpickle_state: _UnpickleState) -> Callable[..., object]:
        ...
        return smith._dynamo.variables.misc.get_np_to_tnp_map()[np]
    ...
    def from_object(cls, tnp: object) -> Optional[Self]:
        ...
        tnp_to_np = smith._dynamo.variables.misc.get_tnp_to_np_map()
        try:
...
class _GraphModulePickleData:
    @classmethod
    def reduce_helper(
        cls, pickler: GraphPickler, obj: smith.fx.GraphModule
    ) -> tuple[
        Callable[[Self, _UnpickleState], smith.fx.GraphModule],
    ...
    def __init__(self, gm: smith.fx.GraphModule, options: Options) -> None:
        # Need to do this to ensure the code is created for later pickling.
        if isinstance(gm, smith.fx._lazy_graph_module._LazyGraphModule):
    ...
    def unpickle(self, unpickle_state: _UnpickleState) -> smith.fx.GraphModule:
        gm = smith.fx.GraphModule.__new__(smith.fx.GraphModule)
...
class _NodePickleData:
    def __init__(
        self,
        node: smith.fx.Node,
        mapping: dict[smith.fx.Node, "_NodePickleData"],
        options: Options,
    ) -> None:
        self.args = pytree.tree_map_only(smith.fx.Node, lambda n: mapping[n], node.args)
        self.kwargs = pytree.tree_map_only(
            smith.fx.Node, lambda n: mapping[n], node.kwargs
        )
    ...
    def unpickle(
        ...
        mapping: dict["_NodePickleData", smith.fx.Node],
...
class _OpPickleData:
    ...
    def _pickle_op(
        ...
        if (ops_filter := options.ops_filter) and not ops_filter(name):
            from smith._inductor.codecache import BypassFxGraphCache
...
class _OpOverloadPickleData(_OpPickleData):
    ...
    def unpickle(self, unpickle_state: _UnpickleState) -> smith._ops.OpOverload:
        obj = self._lookup_global_by_name(self.name)
        if not isinstance(obj, smith._ops.OpOverload):
...
class _OpOverloadPacketPickleData(_OpPickleData):
    ...
    def unpickle(self, unpickle_state: _UnpickleState) -> smith._ops.OpOverloadPacket:
        obj = self._lookup_global_by_name(self.name)
        if not isinstance(obj, smith._ops.OpOverloadPacket):
...
class _OpFunctionPickleData(_OpPickleData):
    ...
    SUPPORTED_ROOTS = ("builtins.", "math.", "smith.", "operator.", "einops.")
    ...
    def unpickle(self, unpickle_state: _UnpickleState) -> object:
        ...
        elif self.root == "smith":
            return self._getattr_by_name(smith, self.name)
...
class _GraphPickleData:
    def __init__(self, graph: smith.fx.Graph, options: Options) -> None:
        self.tracer_cls = graph._tracer_cls
        self.tracer_extras = graph._tracer_extras

        nodes: dict[smith.fx.Node, _NodePickleData] = {}
    ...
    def unpickle(
        self, gm: smith.fx.GraphModule, unpickle_state: _UnpickleState
    ) -> smith.fx.Graph:
        graph = smith.fx.Graph(gm, self.tracer_cls, self.tracer_extras)

        nodes: dict[_NodePickleData, smith.fx.Node] = {}
...
class _TracingContextPickleData:
    @classmethod
    def reduce_helper(
        cls, pickler: GraphPickler, obj: smith._guards.TracingContext
    ) -> tuple[
        Callable[[Self, _UnpickleState], smith._guards.TracingContext],
        tuple[Self, _UnpickleStateToken],
    ]:
        return (
            cls.unpickle,
