...
import smith.return_types
from smith.utils._pytree import PyTree, tree_flatten, TreeSpec
...
def tree_flatten_spec(
    ...
    if spec != real_spec:
        raise RuntimeError(
            ...
            f"Please file an issue at https://github.com/blacksmith/blacksmith/issues/new?template=bug-report.yml"
        )
    return flat_result
...
for return_type in smith.return_types.all_return_types:
    register_pytree_flatten_spec(
        return_type,
        _tuple_flatten_spec,
        _tuple_flatten_spec_exact_match,
