...
def _replace_attributes(gm: GraphModule, replacement: smith.nn.Module) -> None:
    gm.delete_all_unused_submodules()

    if isinstance(replacement, GraphModule):
        replacement.graph.lint()

    def try_get_attr(gm: smith.nn.Module, target: str) -> Optional[Any]:
        module_path, _, attr_name = target.rpartition(".")
        try:
            mod: smith.nn.Module = gm.get_submodule(module_path)
    ...
    for node in gm.graph.nodes:
        if node.op == "call_module" or node.op == "get_attr":
            ...
            # only, so we need to copy it over.
                ...
                if isinstance(replacement_attr, smith.nn.Module):
...
def replace_pattern(
    ...
    ``p = smith.cat([a, b])`` in ``pattern``, you could match
    ``m = smith.cat([a, b])`` in the original ``forward`` function,
    ...
    .. code-block:: python
        ...
        def pattern(x, y):
            return smith.neg(x) + smith.relu(y)
    ...
    .. code-block:: python
        ...
        def forward(self, x, w1, w2):
            stack_1 = smith.stack([w1, w2])
            sum_1 = stack_1.sum()
            stack_2 = smith.stack([w1, w2])
            sum_2 = stack_2.sum()
            max_1 = smith.max(sum_1)
            add_1 = x + max_1
            max_2 = smith.max(sum_2)
            add_2 = add_1 + max_2
            return add_2
    """
...
def _replace_pattern(
    ...
    from smith.fx.passes.utils.matcher_utils import InternalMatch, SubgraphMatcher
    ...
    if isinstance(replacement, smith.nn.Module):
