...
from smith._C import _fx_map_aggregate as map_aggregate, _fx_map_arg as map_arg
from smith._library.opaque_object import is_opaque_value_type
...
class TracerBase:
    ...
    def _filter_traceback_frames(
        ...
        if self._record_forward_stack_traces_only:
            user_frames = [
                ...
                if (
                    frame.name == "forward"
                    or frame.filename.endswith("smith/__init__.py")
                )
            ]
        else:
            first_forward = -1
    ...
    def _find_user_frame(self):
        ...
        # record the first frame not in the blacksmith source. This is the frame executing
        ...
        pt_files = [
            ...
            "smith/fx/experimental/proxy_tensor.py",
            "smith/_ops.py",
            "smith/_tensor.py",
            "smith/utils/_python_dispatch.py",
            "smith/_prims_common/wrappers.py",
            "smith/_refs/__init__.py",
            "smith/_refs/nn/functional/__init__.py",
            "smith/utils/_stats.py",
        ]
        while frame:
            frame = frame.f_back
            if frame and all(
    ...
    def create_arg(self, a: Any) -> Argument:
        ...
        # the graph? Please Please Please contact someone on the Blacksmith Compiler team;
        # the considerations are subtle.
        #
        # 1) When you add a new type, all of the downstream consumers and pass writers
        # need to handle the new type. smith.fx is intended to be easy to write
        # passes for, so we will push back against new types.
        # 2) In smith.compile's IR, there are only specific operations that go
        ...
        elif isinstance(a, (smith._ops.OpOverload, smith._ops.HigherOrderOperator)):
            return a
    ...
    def iter(self, obj: "Proxy") -> Iterator:
        ...
        raise TraceError(
            ...
            "See the smith.fx docs on blacksmith.org for a "
...
def _get_seq_nr(node_name: str = ""):
    ...
    if current_meta.get("in_grad_fn", 0) > 0:
    ...
    elif smith.fx.traceback._is_preserving_node_seq_nr():
        ...
        new_seq_nr = smith.autograd._get_sequence_nr() - 1

    if not smith.fx.traceback._is_preserving_node_seq_nr():
        # See Note [Functionalization View Replay Annotation]
        # Overriding some node meta with the original node meta of the
        # regenerated node.
        replay_node: Node = fx_traceback.get_current_replay_node()
...
class Proxy:
    ...
    (``smith`` function calls, method calls, operators) that they touch
    ...
    the "Proxy" section in `smith/fx/README.md`
    """
    ...
    def __len__(self):
        raise RuntimeError(
            ...
            "this call to be recorded, please call smith.fx.wrap('len') at "
            "module scope"
        )

    @classmethod
    def __smith_function__(cls, orig_method, types, args=None, kwargs=None):
        ...
        if isinstance(orig_method, smith._C.ScriptMethod):
            args = (orig_method.owner,) + args
            return tracer.create_proxy("call_method", orig_method.name, args, kwargs)
        if smith.overrides.is_tensor_method_or_property(orig_method):
            return tracer.create_proxy(
                "call_method", orig_method.__name__, args, kwargs
            )
        else:
            if isinstance(orig_method, smith._ops.HigherOrderOperator):
...
class MetaProxy(Proxy):
    ...
    def __smith_function__(cls, orig_method, types, args=None, kwargs=None):
        ...
        proxy = super().__smith_function__(orig_method, types, args, kwargs)
...
_create_arg_bypass = {
    ...
    for t in [
        *base_types,
        ...
        smith._ops.HigherOrderOperator,
    ]
}
