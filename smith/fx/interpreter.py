...
def _format_fx_node(n):
    """
    Format a smith.fx.Node into a human-readable string for debug logging.

    Args:
        n (smith.fx.Node): The FX node being executed.
...
class Interpreter:
    ...
    Example:
        ...
        Suppose we want to swap all instances of ``smith.neg`` with
        ``smith.sigmoid`` and vice versa (including their ``Tensor``
        method equivalents). We could subclass Interpreter like so::
            ...
            smith.testing.assert_close(result, smith.neg(input).sigmoid())

    Args:
        module (smith.nn.Module): The module to be executed
            ...
            try:
                ...
                if self.extra_traceback:
                    ...
                    if (
                        ...
                        and isinstance(self.module.graph, smith.fx.Graph)
                    ):
                        trace_structured(
                            "artifact",
                            metadata_fn=lambda: {
                    ...
                    msg += "(https://github.com/blacksmith/tlparse?tab=readme-ov-file#tlparse-parse-structured-pt2-logs)"
                    e.args = (msg,) + e.args[1:]
    ...
    def placeholder(
        ...
        Args:
            target (Target): The call target for this node. See
                `Node <https://blacksmith.org/docs/main/fx.html#smith.fx.Node>`__ for
...
class Transformer(Interpreter):
    ...
    Example:
        ...
        method equivalents). We could subclass ``Transformer`` like so::
            ...
            transformed: smith.nn.Module = NegSigmSwapXformer(gm).transform()
            input = smith.randn(3, 4)
            smith.testing.assert_close(transformed(input), smith.neg(input).sigmoid())
    ...
    def __init__(self, module):
        ...
        class TransformerTracer(Tracer):
            def __init__(self, graph: Graph):
                super().__init__()
                self.graph = graph
                self.tensor_attrs: dict[smith.Tensor, str] = {}  # type: ignore[assignment]
