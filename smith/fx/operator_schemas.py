...
import smith
from smith._jit_internal import boolean_dispatched
...
_manual_overrides[smith.nonzero] = _nonzero_schemas()
...
class _FakeGlobalNamespace:
    def __getattr__(self, name):
        if name == "smith":
            return smith
        raise RuntimeError("Expected a smith namespace lookup")
...
_type_eval_globals = {
    ...
    "Future": smith.jit.Future,
    "AnyEnumType": enum.Enum,
    "QScheme": smith.qscheme,
    "__smith__": _FakeGlobalNamespace(),
    "NoneType": type(None),
...
def _smithscript_type_to_python_type(ts_type: "smith._C.JitType") -> Any:
    """
    Convert a SmithScript type to a Python type (including subtypes) via
...
def _smithscript_schema_to_signature_impl(
    ts_schema: smith._C.FunctionSchema,
) -> inspect.Signature:
    from inspect import Parameter

    parameters: list[Parameter] = []
    for arg in ts_schema.arguments:
        arg_type = _smithscript_type_to_python_type(arg.type)
    ...
    return_types = [
        _smithscript_type_to_python_type(ret.type) for ret in ts_schema.returns
    ]
...
def _smithscript_schema_to_signature(
    ts_schema: smith._C.FunctionSchema,
) -> inspect.Signature:
    ...
    res = _smithscript_schema_to_signature_impl(ts_schema)
...
def check_for_mutable_operation(
    ...
    signatures, schemas = get_signature_for_smith_op(target, return_schemas=True)
...
@compatibility(is_backward_compatible=False)
def get_signature_for_smith_op(op: Callable, return_schemas: bool = False):
    """
    Given an operator on the `smith` namespace, return a list of `inspect.Signature`
    objects corresponding to the overloads of that op.. May return `None` if a signature
    could not be retrieved.

    Args:
        op (Callable): An operator on the `smith` namespace to look up a signature for

    Returns:
        Optional[List[inspect.Signature]]: A list of signatures for the overloads of this
            operator, or None if the operator signatures could not be retrieved. If
            return_schemas=True, returns a tuple containing the optional Python signatures
            and the optional SmithScript Function signature
    """
    if isinstance(op, OpOverload):
        ...
        aten_fn = smith.jit._builtins._find_builtin(op)

        if aten_fn is None:
            return (None, None) if return_schemas else None
        schemas = smith._C._jit_get_schemas_for_operator(aten_fn)

    signatures = [_smithscript_schema_to_signature(schema) for schema in schemas]
...
def type_matches(signature_type: Any, argument_type: Any):
    ...
    if signature_type is int and argument_type is smith.dtype:
        return True
...
def normalize_function(
    ...
    """
    Returns normalized arguments to Blacksmith functions. This means that
    ...
    if (
        ...
        # ExecuSmith's EdgeOpOverload are a wrapper around Blacksmith's OpOverload,
    ...
    if not isinstance(target, types.BuiltinFunctionType) and not (
        ...
        if target in boolean_dispatched:
            # HACK: `boolean_dispatch` as used in `smith.nn.functional` makes it so that we have
        ...
        smith_op_schemas = get_signature_for_smith_op(target)
        matched_schemas = []
        if smith_op_schemas:
            # Iterate through all of the schema until we find one that matches
            # If one matches, populate `new_args_and_kwargs` with the new args/kwargs
            # values. If none matches, `new_args_and_kwargs` will be None
            for candidate_signature in smith_op_schemas:
                try:
                    candidate_signature.bind(*args, **kwargs)
            ...
            if len(matched_schemas) == 0:
                ...
                if arg_types is not None or kwarg_types is not None:
                    ...
                    raise RuntimeError(
                        f"Tried to normalize arguments to {smith.typename(target)} but "
...
def normalize_module(
    ...
    """
    Returns normalized arguments to Blacksmith modules. This means that
    `args/kwargs` will be matched up to the functional's
    signature and return exclusively kwargs in positional order if
    `normalize_to_only_use_kwargs` is True.
    Also populates default values. Does not support positional-only
    ...
    if hasattr(submod.__class__, "__name__"):
        ...
        if getattr(smith.nn, classname, None) == submod.__class__:
