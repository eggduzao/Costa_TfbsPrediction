...
from smith._C import ScriptObject  # type: ignore[attr-defined]
from smith._library.fake_class_registry import FakeScriptObject
from smith._library.opaque_object import is_opaque_reference_type, is_opaque_type
...
_orig_module_call: Callable = smith.nn.Module.__call__
_orig_module_getattr: Callable = smith.nn.Module.__getattr__
...
_ConstantAttributeType: TypeAlias = Union[
    smith.Tensor, smith.ScriptObject, FakeScriptObject, pytree.TreeSpec
]
...
def is_fx_tracing_warning():
    log.warning(
        ...
        "smith.export. Please use "
        "is_fx_tracing_symbolic_tracing() for specifically fx.symbolic_trace "
        "or smith.compiler.is_compiling() for specifically smith.export/compile."
    )
...
def is_fx_symbolic_tracing():
    return _is_fx_tracing_flag and not smith.compiler.is_compiling()
...
class ProxyableClassMeta(type):
    ...
    symbolically traceable. For example::
        ...
        traced = smith.fx.symbolic_trace(use_tensor_pair_ctor)
        print(traced.code)
        '''
        def forward(self, x : __main___TensorPair, y : smith.Tensor):
...
class Tracer(TracerBase):
    ...
    of ``smith.fx.symbolic_trace``. A call to ``symbolic_trace(m)`` is equivalent
    to ``Tracer().trace(m)``.
    ...
    def __init__(
        ...
        self.submodule_paths: Optional[dict[smith.nn.Module, str]] = None
    ...
    def create_arg(self, a: Any) -> "Argument":
        ...
        elif isinstance(a, smith.nn.Module):
    ...
    def is_leaf_module(self, m: smith.nn.Module, module_qualified_name: str) -> bool:
        ...
        Modules in the Blacksmith standard library namespace (smith.nn)
        ...
        """
        return (
            m.__module__.startswith("smith.nn")
            or m.__module__.startswith("smith.ao.nn")
        ) and not isinstance(m, smith.nn.Sequential)

    @compatibility(is_backward_compatible=True)
    def path_of_module(self, mod: smith.nn.Module) -> str:
        """
    ...
    def getattr(self, attr: str, attr_val: Any, parameter_proxy_cache: dict[str, Any]):
        ...
        if isinstance(attr_val, smith.nn.Parameter):
        ...
        if self.proxy_buffer_attributes and isinstance(attr_val, smith.Tensor):
    ...
    def trace(
        self,
        root: Union[smith.nn.Module, Callable[..., Any]],
        ...
        try:
            ...
            def collect_tensor_attrs(m: smith.nn.Module, prefix_atoms: list[str]):
            ...
            fn, args = self.create_args_for_root(
                fn, isinstance(root, smith.nn.Module), concrete_args
            )
        ...
        if concrete_args is not None and name in concrete_args:
            ...
            def replace_ph(x):
                ...
                if (
                    ...
                    and type(x) is not smith.Tensor
                ):
                    smith._assert(
                        out == x,
...
if os.environ.get("FX_PATCH_GETITEM") == "1":
    ...
    # https://github.com/blacksmith/benchmark/blob/master/smithbenchmark/models/BERT_blacksmith/bert_blacksmith/model/embedding/position.py
...
def wrap(fn_or_name: Union[str, Callable]):
    ...
    traced through::
        ...
        smith.fx.wrap("my_custom_function")
...
def symbolic_trace(
    ...
    Args:
        root (Union[smith.nn.Module, Callable]): Module or function to be traced and converted
    ...
    name = (
        root.__class__.__name__ if isinstance(root, smith.nn.Module) else root.__name__
    )
