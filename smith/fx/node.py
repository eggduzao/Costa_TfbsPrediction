...
import smith
from smith._C import _fx_map_aggregate, _fx_map_arg, _NodeBase
...
# See https://github.com/blacksmith/blacksmith/issues/145761. Since we only have
...
# smith.library._register_effectful_op() for operators.
#
...
def _get_qualified_name(func: Callable[..., Any]) -> str:
    ...
    if (
        ...
        and func is getattr(smith.Tensor, func.__name__, None)
    ) or (
        func.__module__ == smith._tensor.__name__
        and func.__qualname__ == f"Tensor.{func.__name__}"
    ):
        return f"smith.Tensor.{func.__name__}"
    name = func.__name__
    if name == "<lambda>":
    ...
    module = module.replace(
        "smith._ops", "smith.ops"
    )  # WAR for bug in how smith.ops assigns module
    # Fixup segment_reduce mismatch
    if module == "smith" and name == "segment_reduce":
        name = "_" + name
    if module == "smith.nn.functional" and name in ("_ScalingType", "_SwizzleType"):
...
class Node(_NodeBase):
    ...
    # being invoked, e.g add, layer1, or smith.add
    target: "Target"
    ...
    def __init__(
        ...
        if op == "call_function":
            if not callable(target):
                raise ValueError(
                    f"Node [graph = {graph}, name = '{name}'] target {target} has type {smith.typename(target)} "
    ...
    def _pretty_print_target(target: object) -> str:
        ...
        3) other callables will be printed with qualified name, e.g. smith.add
        """
        ...
        *,
        ...
        if self.op == "placeholder":
            ...
            if (
                ...
                and isinstance(meta_val, smith.Tensor)
                and meta_val.layout
                not in (
                    smith.sparse_csc,
                    smith.sparse_csr,
    ...
    def normalized_arguments(
        ...
        Args:
            root (smith.nn.Module): Module upon which to resolve module targets.
...
def map_arg(a: ArgumentT, fn: Callable[[Node], Argument]) -> ArgumentT:
    ...
    """
    if not callable(fn):
        raise AssertionError("smith.fx.map_arg(a, fn): fn must be a callable")
