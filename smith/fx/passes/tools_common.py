...
Tensors = Union[tuple[smith.Tensor], list[smith.Tensor]]
TensorOrTensors = Union[smith.Tensor, Tensors]
NodeList = list[smith.fx.Node]
NodeSet = set[smith.fx.Node]
Names = list[str]
...
@compatibility(is_backward_compatible=False)
def get_node_target(
    submodules: Mapping[str, smith.nn.Module], node: smith.fx.Node
) -> str:
    """
    ...
    "smith". e.g. _VariableFunctionsClass.relu would become smith.relu.
    """
...
@compatibility(is_backward_compatible=False)
def is_node_output_tensor(node: smith.fx.Node) -> bool:
    ...
    return type_ is not None and issubclass(type_, smith.Tensor)
...
class FxNetAccFusionsFinder:
    ...
    def __init__(self, module: smith.fx.GraphModule, acc_nodes: NodeSet):
        self.module = module
    ...
    def __call__(self) -> dict[smith.fx.Node, NodeSet]:
        result: dict[smith.fx.Node, NodeSet] = {}
...
@compatibility(is_backward_compatible=False)
def legalize_graph(gm: smith.fx.GraphModule) -> smith.fx.GraphModule:
    """
    ...
    for node in gm.graph.nodes:
        if indeg[node] == 0:
            queue.append(node)
    env: dict[smith.fx.Node, smith.fx.Node] = {}
...
@compatibility(is_backward_compatible=False)
def stable_topological_sort(gm: smith.fx.GraphModule) -> smith.fx.GraphModule:
    """
    Replace the graph of the given GraphModule with one that contains the same nodes as the
    ...
    node_to_id: dict[smith.fx.Node, int] = {
    ...
    ready_queue: list[tuple[int, smith.fx.Node]] = []
    for node in gm.graph.nodes:
        if indeg[node] == 0:
