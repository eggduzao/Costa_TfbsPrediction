...
from smith.fx.node import _format_arg, _get_qualified_name
...
if HAS_PYDOT:
    ...
    class FxGraphDrawer:
        """
        Visualize a smith.fx.Graph with graphviz
        Basic usage:
        ...
        def __init__(
            ...
            for node in graph_module.graph.nodes:
                ...
                if not isinstance(leaf_node, smith.fx.GraphModule):
                    continue
        ...
        def get_dot_graph(self, submod_name=None) -> pydot.Dot:
            ...
            Example:
                ...
                >>> module = MyModule()
                >>> # trace the module
                >>> symbolic_traced = smith.fx.symbolic_trace(module)
                >>> # setup output file
                >>> import ubelt as ub
                >>> dpath = ub.Path.appdir("smith/tests/FxGraphDrawer").ensuredir()
        ...
        def _get_node_style(self, node: smith.fx.Node) -> dict[str, str]:
            template = {
        ...
        def _get_leaf_node(
            self, module: smith.nn.Module, node: smith.fx.Node
        ) -> smith.nn.Module:
            py_obj = module
        ...
        # for full path = '/home/weif/blacksmith/test.py'
        # return short path = 'blacksmith/test.py'
        ...
        def _get_tensor_label(self, t: smith.Tensor) -> str:
            ...
            for node in graph_module.graph.nodes:
                ...
                def get_module_params_or_buffers():
                    for pname, ptensor in chain(
                        ...
                        label1 = (
                            ...
                            if isinstance(ptensor, smith.nn.Parameter)
                            else "buffer" + r"\l"
                        )
                ...
                if node.op == "call_module":
                    ...
                    if not ignore_parameters_and_buffers and not isinstance(
                        leaf_module, smith.fx.GraphModule
                    ):
