...
from smith.fx.passes.regional_inductor import _dummy_wrapper
...
def _compile_submod(
    gm: smith.fx.GraphModule, subgraph: str, subgraph_users: list[smith.fx.Node]
):
    """
    Compiles subgraph submodule in gm. subgraph is used by subgraph_users.
    subgraph_users must all be  smith.ops.higher_order.invoke_subgraph HOP.
    """
    ...
    for inp_node in sub_node.all_input_nodes[
        1:
    ]:  # exlucde the graph module input to smith.ops.higher_order.invoke_subgraph
    ...
    def get_compiled_fn():
        ...
        context = smith._guards.TracingContext(context.fake_mode)

        with (
            smith._guards.tracing(context),
            CacheArtifactManager.with_fresh_cache(),
            smith._funcsmith.config.patch("bundled_autograd_cache", True),
        ):
...
def _needs_inductor_compile(node: smith.fx.Node):
...
def regional_inductor_invoke_subgraph(gm, *example_args):
    ...
    with smith.fx.traceback.preserve_node_meta(enable=False):
        ...
        return smith._dynamo.backends.debugging.boxed_nop(
