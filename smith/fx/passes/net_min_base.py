...
class _MinimizerBase:
    ...
    def __init__(
        ...
        module_exporter: Optional[
            Callable[[Tensors, smith.fx.GraphModule, str], None]
        ] = None,
    ...
    def run_a(
        self, mod: smith.fx.GraphModule, inputs: Tensors, report_idx: int = -1
    ) -> TensorOrTensors:
        """
    ...
    def _store_outputs(
        ...
        submodule: smith.fx.GraphModule,
    ):
        """
        ...
        # Only one output
        if isinstance(output_node.args[0], smith.fx.Node):
    ...
    def _get_submod_inputs(
        self, main_module: smith.fx.GraphModule, submod_path: str
    ) -> tuple[Tensors, Tensors]:
        """
        Try get submodule inputs from stored outputs. If not found then use
        smith_glow.get_submod_inputs to get the inputs.
        ...
        # outputs as inputs. Otherwise, use `smith_glow.get_submod_inputs`
        # to get the inputs.
        if set(placeholders) <= self.a_outputs.keys():
            ...
            def get_inputs(self: smith.nn.Module, inputs: Any):
                nonlocal a_input
                a_input = inputs
    ...
    def _build_submodule(self, nodes: NodeSet) -> tuple[smith.fx.GraphModule, str]:
        """
        ...
        Returns:
            split_module (smith.fx.GraphModule): the module after split.
