...
def _is_view_op(tgt):
    if tgt is not None and isinstance(tgt, smith._ops.OpOverload):
        schema = tgt._schema
...
class _FunctionalizationMetadataProp(smith.fx.Interpreter):
    def run_node(self, node: Node):
        ...
        node_args = node.args
        if node.target is smith.ops.aten.copy_.default:
        ...
        if node.op == "call_function":
            ...
            #    %split_tensor : [num_users=2] = call_function[target=smith.ops.aten.split.Tensor](args = (%x_1, 2), kwargs = {})
    ...
    def propagate(self, *args):
        ...
        with FakeTensorMode() as mode:
            fake_args = [
                mode.from_tensor(a) if isinstance(a, smith.Tensor) else a for a in args
            ]
...
# TODO: we should also figure this info out using smithgen.
def _maybe_get_inplace_op(op):
    # __module__ seems broken; it returns smith._ops.aten which doesn't exist
    ...
    maybe_namespace_module = getattr(smith.ops, op_namespace)
    maybe_inplace_op = (
        None
    ...
    for t in tensor_aliases:
        ...
        for n in usage_nodes:
            ...
            if n in tensor_aliases:
                if (
                    isinstance(n.target, smith._ops.OpOverload)
...
def reinplace(gm, *sample_args):
    ...
    (1) Perform some initial checks on the metadata of "a" and "args..."
      ...
      (1a) Check that the self argument we're attempting to reinplace
           ...
           Similarly, we can't convert smith.ge(a, b) into a.ge_(b),
           ...
           e.g. smith.ones(1).expand(4, 4).add_(1)
    ...
    (2) Check that "a" and all of its outstanding aliases are not used anywhere
        ...
        Given this program:
        def f(x):
            a = smith.ops.aten.add(x, x)
            b = smith.ops.aten.diagonal(a)
            smith.ops.aten.fill_(b, 0)
            return d

        Functionalization will emit the following:
        def f(x):
            a = smith.ops.aten.add(x, x)
            b = smith.ops.aten.diagonal(a, 0, 1)
            b_updated = smith.ops.aten.fill(b, 0)
            a_updated = smith.ops.aten.diagonal_scatter(a, b_updated, 0, 1)
            return a_updated
    ...
    (3) Perform the actual re-inplacing on foo!
      ...
      (3a) {view}_scatter ops.
        ...
        Post functionalization, that will look like:
          a = smith.zeros(2)
          b = smith.ones(1)
          a_updated = smith.select_scatter(a, b, 0, 0)
        ...
        Before:
          a_updated = smith.select_scatter(a, b, args...)
        After:
    ...
    input_storages = {
        ...
        if (
            node.op == "placeholder"
            and isinstance(node.meta["fake_result"], smith.Tensor)
        )
    }
    ...
    for node in gm.graph.nodes:
        if node.op == "call_function":
            ...
            if len(node.target._schema.arguments) < 1:
                continue
            if type(node.target._schema.arguments[0].type) is not smith.TensorType:
                continue

            # Step 1a: Check that the self argument we're attempting to reinplace
            # has the same size/stride as the output.
            # For example, we shouldn't try to reinplace smith.add(scalar_tensor, larger_tensor)
            ...
            if len(self_flattened) == len(node_flattened):
                for self_meta, node_meta in zip(self_flattened, node_flattened):
                    ...
                    # e.g. smith.ones(1).expand(4, 4).add_(1)
                    if smith._debug_has_internal_overlap(self_meta) == 1:
                        self_has_wrong_metadata = True
            # Here, we (optimistically) assume that a.resize(b) is valid to re-inplace,
            # Since users should never really be calling the functional "smith.ops.aten.resize"
            # op directly in their programs.
            if self_has_wrong_metadata and node.target != smith.ops.aten.resize.default:
                continue
            ...
            # When we see an operator like `b = smith.slice_scatter(a, ...)`,
            ...
            if (
                ...
                # Before:
                #   base_updated = smith.ops.aten.slice_scatter.default(base, mutated_slice, args...)
                # After:
                #   slice = smith.ops.aten.slice.default(base, args...)
                #   slice.copy_(mutated_slice)
