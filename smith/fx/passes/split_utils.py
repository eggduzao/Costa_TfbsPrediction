...
from smith.fx.graph import map_arg
from smith.fx.passes.utils import HolderModule, lift_subgraph_as_module
...
def getattr_recursive(obj, name):
    for layer in name.split("."):
        if isinstance(obj, smith.nn.ModuleList):
...
class Component:
    ...
    getattr_maps: dict[smith.fx.Node, smith.fx.Node] = field(default_factory=dict)
...
def split_by_tags(
    ...
    GraphModuleCls: type[smith.fx.GraphModule] = smith.fx.GraphModule,
) -> Union[smith.fx.GraphModule, tuple[smith.fx.GraphModule, dict[str, str]]]:
    """
    ...
    class SimpleModule(smith.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.linear1 = smith.nn.Linear(...)
            self.linear2 = smith.nn.Linear(...)
            self.linear3 = smith.nn.Linear(...)
    ...
    def forward(self, in2, linear1):
        ...
        cat_1 = smith.cat([linear1, linear2])
    ...
    node_to_component: dict[smith.fx.Node, Component] = {}
    ...
    for comp in all_components:
        ...
        if len(outs) == 1 and not return_tuple:
            ...
            for i, o in enumerate(comp.orig_outputs):
                ...
                main_remapping[o] = smith.fx.Proxy(main_node)[i].node  # type: ignore[index]
...
def move_non_tensor_nodes_on_boundary(subgraphs) -> None:
    ...
    node_to_subgraph: dict[smith.fx.Node, int] = {}
    for i, subgraph in enumerate(subgraphs):
        for node in subgraph.nodes:
            node_to_subgraph[node] = i

    def get_children_in_graph(node: smith.fx.Node) -> list[smith.fx.Node]:
    ...
    def get_parents_in_graph(node: smith.fx.Node) -> list[smith.fx.Node]:
    ...
    def has_children_in_other_subgraph(
        node: smith.fx.Node, current_subgraph_idx: int
    ) -> bool:
        """
    ...
    def can_move_node_and_dependencies(
        node: smith.fx.Node, from_subgraph: int, to_subgraph: int
    ) -> tuple[bool, set[smith.fx.Node]]:
        """
    ...
    for subgraph_idx, subgraph in enumerate(subgraphs):
        ...
        queue: list[smith.fx.Node] = []
        processed: set[smith.fx.Node] = set()
