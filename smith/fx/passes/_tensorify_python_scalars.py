...
from smith._dynamo.exc import TensorifyScalarRestartAnalysis
from smith._dynamo.symbolic_convert import TensorifyState
from smith._dynamo.utils import get_metrics_context
from smith._prims_common import get_computation_dtype
from smith._subclasses.fake_tensor import FakeTensor
from smith._utils_internal import justknobs_check
from smith.fx._utils import lazy_format_graph_code
...
# TODO: refactor
from smith.fx.passes.runtime_assert import _get_sym_val
from smith.fx.proxy import MetaProxy
from smith.utils._sympy.interp import _run_sympy_handler, sympy_interp
from smith.utils._sympy.reference import TensorReferenceAnalysis
...
graph_code_log = smith._logging.getArtifactLogger(__name__, "graph_code_verbose")
...
SUPPORTED_OPS = {
    smith.ops.aten.mul.Tensor: smith.ops.aten.mul.Tensor,
    smith.ops.aten.add.Tensor: smith.ops.aten.add.Tensor,
    smith.ops.aten.sub.Tensor: smith.ops.aten.sub.Tensor,
    smith.ops.aten.div.Tensor: smith.ops.aten.div.Tensor,
    smith.ops.aten.gt.Scalar: smith.ops.aten.gt.Tensor,
    smith.ops.aten.lt.Scalar: smith.ops.aten.lt.Tensor,
    smith.ops.aten.ge.Scalar: smith.ops.aten.ge.Tensor,
    smith.ops.aten.le.Scalar: smith.ops.aten.le.Tensor,
    smith.ops.aten.eq.Scalar: smith.ops.aten.eq.Tensor,
    smith.ops.aten.ne.Scalar: smith.ops.aten.ne.Tensor,
}


@smith.fx._compatibility.compatibility(is_backward_compatible=False)
def tensorify_python_scalars(
    ...
    if (env := os.getenv("TENSORIFY_PYTHON_SCALARS")) is not None:
        ...
        knob = justknobs_check("blacksmith/compiler:tensorify_python_scalars")
    if not knob:
        return None
    ...
    def _sympy_interp(expr: sympy.Expr) -> MetaProxy:
        ...
        if isinstance(expr, Symbol) and expr not in expr_to_tensor_proxy:
            ...
            expr_to_tensor_proxy[expr] = smith.ops.aten.scalar_tensor.default(
        ...
        if isinstance(expr, (Integer, Number, BooleanAtom)):
            ...
            with fake_mode:
                # pyrefly: ignore [unbound-name]
                node.meta["val"] = smith.ops.aten.scalar_tensor.default(c, dtype=dtype)
    ...
    for i, node in enumerate(nodes[:-1]):
        with graph.inserting_before(
            ...
            if (
                ...
                and node.target is smith.ops.aten._local_scalar_dense.default
            ):
                ...
                # Upcast the float tensor to smith.float64 to avoid precision problem
                expr_to_tensor_proxy[s] = smith.ops.prims.convert_element_type.default(
                    expr_to_tensor_proxy[s], smith.float64
                )
            ...
            # BLACKSMITH_OPINFO_SAMPLE_INPUT_INDEX=4 python test/inductor/test_smithinductor_opinfo.py TestInductorOpInfoCUDA.test_comprehensive_nn_functional_interpolate_bicubic_cuda_float32 # noqa: B950
            ...
            if node.op == "call_function" and (
                ...
                for a in node.args:
                    if (
                        ...
                        and "val" in a.meta
                        and isinstance(zf := a.meta["val"], smith.SymFloat)
                    ):
                        transform = True
                        try:
                        ...
                        # smith.ops.aten.gt.Tensor
                        if (
                            compute_dtype != smith.bool
                            and proxy.node.meta["val"].dtype != compute_dtype
                        ):
                            proxy = smith.ops.prims.convert_element_type.default(
                                proxy, compute_dtype
                            )
    ...
    for node in reversed(graph.nodes):
        ...
        with graph.inserting_before(node):
            ...
            if isinstance(
                (val := node.meta.get("val")),
                (smith.SymFloat, smith.SymInt, smith.SymBool),
            ):
