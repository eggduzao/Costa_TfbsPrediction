...
class CSEPass(PassBase):
    ...
    def call(self, graph_module: GraphModule) -> PassResult:
        """
        Return a new copy of smith.fx.GraphModule with CSE applied to the input graph
        ...
        p = CSEPass()
        traced_graph = make_fx(f)(smith.tensor(1))
        print(traced_graph)
        ...
        hash_env: dict[
            tuple[smith._ops.OpOverload, int], Node
        ] = {}  # map from hash to a node in the new graph
        token_map: dict[
            tuple[smith._ops.OpOverload, int], dict[str, Any]
