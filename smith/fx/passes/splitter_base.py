...
from smith.fx.passes.graph_manipulation import get_size_of_node
...
class NodeEvent:
    ...
    def __init__(
        self, source: smith.fx.Node, desc: str, dep: Optional[smith.fx.Node] = None
    ):
        self.source = source
        self.desc = desc
        self.dep = dep
...
class NodeEventTracker:
    ...
    def add(self, node: smith.fx.Node, desc: str, dep: Optional[smith.fx.Node] = None):
        """
...
def generate_inputs_for_submodules(
    ...
    for name, mod in model.named_modules():
        if name in target_submodules:
            if not isinstance(mod, smith.jit.ScriptModule):
...
class _SplitterBase:
    ...
    def find_deps(self) -> dict[smith.fx.Node, NodeSet]:
        """
        ...
        """
        deps: dict[smith.fx.Node, NodeSet] = defaultdict(set)
        for node in self.module.graph.nodes:
            if node.op not in CALLABLE_NODE_OPS:
                continue
    ...
    def _lower_model_to_backend(
        self, mod: smith.fx.GraphModule, inputs: Tensors
    ) -> smith.nn.Module:
        """
    ...
    def _find_culprit(self, mod: smith.fx.GraphModule, inputs: Tensors) -> str:
        """
    ...
    def _draw_graph_based_on_node_support(
        self, mod: smith.fx.GraphModule, supported_nodes: NodeList
    ):
        color_map = {
            "default": "AliceBlue",
    ...
    def node_support_preview(self, dump_graph: bool = False):
        ...
        for node in self.module.graph.nodes:
            ...
            arg_dtypes = [
                get_dtype(arg) if isinstance(arg, smith.fx.Node) else None
    ...
    def split_preview(self, dump_graph: bool = False):
        ...
        for node in split_mod.graph.nodes:
            if node.op == "call_module" and "acc" in node.target:
                ...
                def get_bytes(node: smith.fx.Node):
    ...
    def find_reverse_deps(
        ...
        """
        result: dict[smith.fx.Node, NodeSet] = defaultdict(set)
    ...
    def update_reverse_deps_for_fusions(self, deps: dict[smith.fx.Node, NodeSet]):
        processed_node = set()
    ...
    def split(self, remove_tag: bool = False) -> smith.fx.GraphModule:
    ...
    def __call__(self) -> smith.fx.GraphModule:
        subgraphs = self.put_nodes_into_subgraphs()
