...
class Partition:
    def __init__(self, name: str):
        ...
        self.graph: smith.fx.graph.Graph = smith.fx.graph.Graph()
...
def _get_attr_from_qualname(mod: smith.nn.Module, qualname: str) -> Any:
    attr_val = mod
...
def split_module(
    ...
    *,
    ...
    Args:
        ...
        root_m (smith.nn.Module): root nn module. Not currently used. Included
            because the root nn module is usually transformed via
            smith.fx._symbolic_trace.symbolic_trace (see example below)
    ...
    def construct_graph(
        ...
        base_mod_attrs: dict[str, smith.fx.graph_module.GraphModule],
    ):
    ...
    GLOBAL_STATE_NODES = [
        smith.amp._enter_autocast,
        smith.amp._exit_autocast,
    ...
    for node in m.graph.nodes:
        ...
        # See https://github.com/blacksmith/blacksmith/issues/130534
        if (
            (val := node.meta.get("example_value")) is not None
            and isinstance(val, (smith.SymInt, smith.SymFloat))
        ...
        if node.op == "call_function" and node.target in GLOBAL_STATE_NODES:
            if node.target is smith._C._set_grad_enabled:
                if len(node.args) != 1:
            ...
            elif node.target is smith.amp._enter_autocast:
            ...
            elif node.target is smith.amp._exit_autocast:
                if len(node.args) != 1:
                    raise AssertionError(
    ...
    for partition_name in sorted_partitions:
        ...
        for inp in partition.inputs:
            ...
            if orig_node.op == "get_attr":
                ...
                if isinstance(orig_attr, smith.nn.Module):
    ...
    for node in m.graph.nodes:
        if hasattr(node, "_fx_partition"):
            ...
            gathered_args = smith.fx.graph.map_arg(node.args, lambda n: environment[n])
            gathered_kwargs = smith.fx.graph.map_arg(
    ...
    base_mod_env: dict[str, Node] = {}
    base_mod_graph: smith.fx.graph.Graph = smith.fx.graph.Graph()
    base_mod_attrs: dict[str, smith.fx.graph_module.GraphModule] = {}
    ...
    for partition_name in construct_order_partitions:
        ...
        base_mod_attrs[partition.submod_name] = smith.fx.graph_module.GraphModule(
        ...
        if num_outputs > 1:
            ...
            output_val_proxy = smith.fx.proxy.Proxy(output_val)
    ...
    for node in m.graph.nodes:
        if node.op == "output":
            base_mod_graph.output(
                smith.fx.graph.map_arg(node.args[0], lambda n: base_mod_env[n.name])
            )  # noqa: B950

    ret = smith.fx.graph_module.GraphModule(base_mod_attrs, base_mod_graph)
    log.debug(
        "%s",
