...
from smith.fx.experimental.proxy_tensor import py_sym_types, snapshot_fake
...
class FakeTensorProp(smith.fx.Interpreter):
    """
    ...
    def __init__(
        self, module: smith.fx.GraphModule, mode: Optional[FakeTensorMode] = None
    ):
    ...
    def run_node(self, n: Node):
        ...
        if (
            n.op == "call_function"
            and n.target is smith.ops.higher_order.invoke_subgraph
            ...
            if not (
                isinstance(n.args[0], smith.fx.Node)
            ...
            for operand in operands:
                if not (isinstance(operand, smith.fx.Node) and "val" in operand.meta):
    ...
    def propagate(self, *args):
        fake_args = [
            self._mode.from_tensor(a) if isinstance(a, smith.Tensor) else a
            for a in args
        ]
