...
import smith
import smith.fx
from smith.fx.passes.split_utils import move_non_tensor_nodes_on_boundary
from smith.fx.passes.splitter_base import Subgraph
...
class TestMoveNonTensorNodesOnBoundary(unittest.TestCase):
    ...
    def _create_mock_node(
        ...
        if op == "placeholder":
            ...
            node = self.graph.call_function(smith.add, args=())
            node.op = op

        node.name = name
        # Mock meta attribute for tensor type checking
        if is_tensor:
            node.meta = {"type": smith.Tensor}
        else:
    ...
    def test_move_non_tensor_nodes_basic_case(self) -> None:
        ...
        with patch(
            "smith.fx.passes.split_utils.is_node_output_tensor"
        ) as mock_is_tensor:
