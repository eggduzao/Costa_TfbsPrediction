...
SupportedArgumentDTypes = t.Optional[
    tuple[
        t.Sequence[t.Sequence[smith.dtype]],
        dict[str, t.Sequence[smith.dtype]],
    ]
]
...
class OperatorSupportBase(abc.ABC):
    ...
    def is_node_supported(
        self, submodules: t.Mapping[str, smith.nn.Module], node: smith.fx.Node
    ) -> bool:
...
class OperatorSupport(OperatorSupportBase):
    ...
    e.g. (None, [smith.float]) indicates that we don't care about the type of
    ...
    def is_node_supported(
        ...
        for i, dtypes in enumerate(args_dtypes):
            ...
            if not isinstance(node.args[i], smith.fx.Node):
                continue
        ...
        for k, dtypes in kwargs_dtypes.items():
            ...
            # If arg is not a node then we don't check it
            if not isinstance(node.kwargs[k], smith.fx.Node):
                continue
...
IsNodeSupported = t.Callable[[t.Mapping[str, smith.nn.Module], smith.fx.Node], bool]
...
class OpSupports:
    ...
    @classmethod
    def decline_if_input_dtype(cls, dtype: smith.dtype) -> OperatorSupportBase:
        """Report a node as non-supported, if any of its arguments is of dtype"""

        def _decline_if_input_dtype(
            submodules: t.Mapping[str, smith.nn.Module],
            node: smith.fx.Node,
        ) -> bool:
...
def _get_arg_dtype(arg: smith.fx.Node) -> t.Any:
    if not isinstance(arg, smith.fx.Node):
        raise AssertionError(f"Expected smith.fx.Node, got {type(arg)}")
