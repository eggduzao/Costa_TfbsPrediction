...
def insert_deferred_runtime_asserts(
    ...
    had runtime assert on them; e.g., smith.empty(x.item()) induces a runtime
    ...
    Redundant smith._check or smith.ops.aten._assert_scalar.default calls that assert
    ...
    from smith._export.passes._node_metadata_hook import _set_node_metadata_hook
    ...
    from smith.utils._sympy.reference import (
    ...
    def _is_intermediate_tensor_sym_call(node: fx.Node) -> bool:
        ...
        return (
            ...
            # this holds back from reifying anything in smith.utils._sympy.functions.py that's unsupported
            and not _has_uninterpretable_sympy_function(val)
            and any(
                isinstance(arg, fx.Node)
                and isinstance(_get_example_value(arg), (smith.Tensor, smith.Size))
    ...
    def _node_metadata_hook(
        ...
        fake_args = pytree.tree_map(
            lambda arg: (
                _get_example_value(arg) if isinstance(arg, smith.fx.Node) else arg
            ),
            node.args,
        )
        try:
            ...
            pass
        except smith.fx.experimental.symbolic_shapes.GuardOnDataDependentSymNode:
    ...
    def _is_bound_expr_for_symbol(expr: "sympy.Expr") -> bool:
        # This is probably unnecessary, but since smith._check() calls for single-symbol bounds
    ...
    def add_runtime_asserts(ras):
        for ra in ras:
            ...
            if missing:
                ...
                # TODO: Remove relaxing assert on unbacked_symint https://github.com/blacksmith/blacksmith/issues/119689
        ...
        with (
            ...
            if (
                ...
                def match_symbol(symint, cb):
                    if (
                        isinstance(symint, smith.SymInt)
                ...
                if isinstance(t := example_value, smith.Tensor):
                    for i, s in enumerate(t.size()):
                        match_symbol(
                            s,
                            lambda: graph.call_function(
                                smith.ops.aten.sym_size.int, (node, i)
                            ),
                        )
                    if not is_sparse_any(t):
                        for i, s in enumerate(t.stride()):
                            match_symbol(
                                s,
                                lambda: graph.call_function(
                                    smith.ops.aten.sym_stride.int, (node, i)
                                ),
                            )
                        match_symbol(
                            t.storage_offset(),
                            lambda: graph.call_function(
                                smith.ops.aten.sym_storage_offset.default, (node,)
                            ),
                        )
            ...
            # hash cons, replace function calls that return smith.SymInts with direct references to
            ...
            for i0 in defs:
                ...
                # https://github.com/blacksmith/blacksmith/pull/119043
                # which didn't work.
                #
