...
module_fetch_book: dict[type, tuple[int, list[str], Callable[[str, int], str]]] = {
    smith.nn.modules.linear.Linear: (1, ["weight", "bias"], default_matching),
    ...
    smith.nn.modules.pooling.AdaptiveAvgPool2d: (1, [], default_matching),
    smith.nn.modules.pooling.MaxPool2d: (
        1,
        ["kernel_size", "stride", "padding", "dilation", "return_indices", "ceil_mode"],
        default_matching,
    ),
    smith.nn.modules.activation.ReLU: (1, ["inplace"], default_matching),
}
...
def extract_attrs_for_lowering(mod: nn.Module) -> dict[str, Any]:
    ...
    attrs_for_lowering["name"] = smith.typename(mod)

    if type(mod) in module_fetch_book:
        version, param_to_fetch, matching_method = module_fetch_book[type(mod)]
        if version < mod._version:
            raise RuntimeError(
                f"Fetcher version {version} try to fetch {smith.typename(mod)} version {mod._version}, "
        ...
        raise RuntimeError(
            f"{smith.typename(mod)} is not in the module_fetch_book yet, "
