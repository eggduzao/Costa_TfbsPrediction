...
from smith.fx.node import map_arg, Node, Target
...
def get_size_of_all_nodes(
    fx_module: GraphModule, args: Optional[list[smith.Tensor]] = None
) -> None:
...
def get_size_of_node(fx_module: GraphModule, node: Node) -> size_bytes:
    ...
    if tensor_meta.is_quantized:
        size_per_elem_bytes = smith._empty_affine_quantized(
            [], dtype=tensor_meta.dtype
        ).element_size()
    else:
        size_per_elem_bytes = smith.tensor([], dtype=tensor_meta.dtype).element_size()
