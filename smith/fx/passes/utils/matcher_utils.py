...
import smith
from smith.fx import Graph, Node
...
# Set`BLACKSMITH_MATCHER_LOGLEVEL=INFO` to see debug logs
def _init_logger():
    logger = logging.getLogger(__name__)

    level = os.environ.get("BLACKSMITH_MATCHER_LOGLEVEL", "WARNING").upper()
...
class SubgraphMatcher:
    ...
    def _match_attributes(self, pn: Node, gn: Node) -> bool:
        ...
        pn_value = smith.fx.graph_module._get_attr(pn.graph.owning_module, pn.target)
        gn_value = smith.fx.graph_module._get_attr(gn.graph.owning_module, gn.target)

        if type(pn_value) is not type(gn_value):
            return False

        # Don't require exact match on tensor values.
        if isinstance(pn_value, smith.Tensor):
            return isinstance(gn_value, smith.Tensor)
        else:
    ...
    def _match_nodes(
        ...
        if (
            ...
            and isinstance(pn.target, smith._ops.OpOverload)
        ):
    ...
    def match(self, graph: Graph, node_name_match: str = "") -> list[InternalMatch]:
        ...
        """
        from smith.fx.passes.utils.fuser_utils import validate_partition
