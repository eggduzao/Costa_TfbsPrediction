...
from smith.fx.passes.tools_common import legalize_graph, NodeList, NodeSet  # noqa: F401
from smith.fx.passes.utils import lift_subgraph_as_module  # type: ignore[attr-defined]
    ...
    with gm.graph.inserting_before(next_node):
        if len(orig_outputs) == 1 and not isinstance(output_node.args[0], tuple):
            ...
            for i, orig_output in enumerate(orig_outputs):
                ...
                proxy_out = smith.fx.Proxy(module_node)[i].node  # type: ignore[index]
...
def fuse_by_partitions(
    ...
    smith.fx.passes.tools_common.stable_topological_sort(gm)
    gm.graph.lint()
