...
def get_source_partitions(
    ...
    Args:
        ...
        wanted_sources: List of sources of nodes that were decomposed from this
            source. This can be a function (ex. smith.nn.functional.linear) or a
            leaf module type (ex. smith.nn.Linear).
    ...
    for node in graph.nodes:
        ...
        # TODO: Bypass "smith_fn" when "source_fn_stack" because now "smith_fn" can
        ...
        # after we fix "smith_fn". T199561090
        if (source_fn_st := node.meta.get("source_fn_stack", None)) is None and (
            smith_fn := node.meta.get("smith_fn", None)
        ) is not None:
            node_fqn, source_fn = smith_fn
