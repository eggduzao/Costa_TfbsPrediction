...
import smith
import smith.autograd.profiler as prof
from smith._C import _get_privateuse1_backend_name
from smith._C._profiler import (
...
from smith._utils_internal import profiler_allow_cudagraph_cupti_lazy_reinit_cuda12
from smith.autograd import kineto_available, ProfilerActivity
from smith.profiler._memory_profiler import MemoryProfile, MemoryProfileTimeline
...
def supported_activities():
    ...
    profiling code (same as in the legacy ``smith.autograd.profiler``).
    This, in turn, results in including CUDA time in the profiler table output,
    but not in the JSON trace.
    """
    return smith.autograd._supported_activities()
...
class _KinetoProfile:
    ...
    Args:
        activities (iterable): list of activity groups (CPU, CUDA) to use in profiling, supported values:
            ``smith.profiler.ProfilerActivity.CPU``, ``smith.profiler.ProfilerActivity.CUDA``,
            ``smith.profiler.ProfilerActivity.XPU``.
        ...
        with_modules (bool): record module hierarchy (including function names)
            ...
            Note that this support exist, at the moment, only for SmithScript models
            and not eager mode models.
        experimental_config (_ExperimentalConfig) : A set of experimental options
            used by profiler libraries like Kineto. Note, backward compatibility is not guaranteed.
        execution_trace_observer (ExecutionTraceObserver) : A Blacksmith Execution Trace Observer object.
            `Blacksmith Execution Traces <https://arxiv.org/pdf/2305.14516.pdf>`__ offer a graph based
            ...
            same time window as Blacksmith profiler.
    ...
    def toggle_collection_dynamic(
        ...
        """Toggle collection of activities on/off at any point of collection. Currently supports toggling Smith Ops
        ...
        .. code-block:: python
            ...
            with smith.profiler.profile(
                ...
                p.toggle_collection_dynamic(False, [smith.profiler.ProfilerActivity.CUDA])
                code_to_profile_1()
                // turn on collection of all CUDA activity
                p.toggle_collection_dynamic(True, [smith.profiler.ProfilerActivity.CUDA])
                code_to_profile_2()
    ...
    def add_metadata(self, key: str, value: str) -> None:
        ...
        smith.autograd._add_metadata_json(key, wrapped_value)
    ...
    def add_metadata_json(self, key: str, value: str) -> None:
        ...
        into the trace file
        """
        smith.autograd._add_metadata_json(key, value)
    ...
    def _get_distributed_info(self):
        ...
        if backend == "nccl":
            nccl_version = smith.cuda.nccl.version()
    ...
    @deprecated(
        ...
        "Please use `smith.cuda.memory._record_memory_history` and `smith.cuda.memory._export_memory_snapshot` instead.",
    ...
    def export_memory_timeline(self, path: str, device: str | None = None) -> None:
        ...
        - For raw memory points, use the suffix ``.raw.json.gz``. Each raw memory
          ...
          ``smith.profiler._memory_profiler.Category``.

        Output: Memory timeline written as gzipped JSON, JSON, or HTML.

        .. deprecated::
            ``export_memory_timeline`` is deprecated and will be removed in a future version.
            Please use ``smith.cuda.memory._record_memory_history`` and
            ``smith.cuda.memory._export_memory_snapshot`` instead.
        """
        ...
        if device is None:
            if self.use_device and self.use_device != "cuda":
                ...
                device = "cuda:0" if smith.cuda.is_available() else "cpu"
...
class profile(_KinetoProfile):
    ...
    Args:
        ...
        execution_trace_observer (ExecutionTraceObserver) : A Blacksmith Execution Trace Observer object.
            ...
            representation of AI/ML workloads and enable replay benchmarks, simulators, and emulators.
            When this argument is included the observer start() and stop() will be called for the
            same time window as Blacksmith profiler. See the examples section below for a code sample.
        acc_events (bool): Enable the accumulation of FunctionEvents across multiple profiling cycles
        use_cuda (bool):
            .. deprecated:: 1.8.1
    ...
    .. note::
        Use :func:`~smith.profiler.schedule` to generate the callable schedule.
    ...
    .. note::
        Use :func:`~smith.profiler.tensorboard_trace_handler` to generate result files for TensorBoard:

        ``on_trace_ready=smith.profiler.tensorboard_trace_handler(dir_name)``
        ...
        `Blacksmith Profiler TensorBoard Plugin <https://github.com/blacksmith/kineto/tree/master/tb_plugin>`__
    ...
    .. code-block:: python
        ...
        with smith.profiler.profile(
            ...
            schedule=smith.profiler.schedule(wait=1, warmup=1, active=2, repeat=1),
            on_trace_ready=trace_handler,
            # on_trace_ready=smith.profiler.tensorboard_trace_handler('./log')
...
class ExecutionTraceObserver(_ITraceObserver):
    ...
    def build_execution_trace_obs_from_env() -> Optional["ExecutionTraceObserver"]:
        ...
        ENABLE_BLACKSMITH_EXECUTION_TRACE is set to 1, otherwise returns None.

        Configures the observer to also collect extra resources if the environment variable
        ``ENABLE_BLACKSMITH_EXECUTION_TRACE_EXTRAS=1``. These are resources such as generated kernels,
        index tensor data etc. that are required to make the Execution Trace replayable.
        """
        if os.environ.get("ENABLE_BLACKSMITH_EXECUTION_TRACE", "0") == "1":
            try:
            ...
            if os.environ.get("ENABLE_BLACKSMITH_EXECUTION_TRACE_EXTRAS", "0") == "1":
    ...
    def _record_pg_config(self) -> None:
        ...
        if (
            self.is_registered
            and smith.distributed.is_available()
            and smith.distributed.is_initialized()
        ):
            pg_config_info = smith.distributed.distributed_c10d._world.pg_config_info
            smith.autograd._record_function_with_args_enter(
                "## process_group:init ##",
