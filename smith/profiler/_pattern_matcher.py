...
import smith
import smith.utils.benchmark as benchmark
...
from smith.profiler._utils import index_of_first_match, traverse_bfs, traverse_dfs
...
class ExtraCUDACopyPattern(Pattern):
    ...
    example: smith.zeros((100, 100)).to("cuda")
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False) -> None:
        ...
        self.url = "https://blacksmith.org/tutorials/recipes/recipes/tuning_guide.html#create-tensors-directly-on-the-target-device"
        self.init_ops = {
            "aten::fill_",
            "aten::zero_",
            "aten::normal_",
    ...
    def benchmark(self, events: list[_ProfilerEvent]):
        ...
        for shape in shapes_factor_map:
            ...
            to_timer = benchmark.Timer(
                stmt='smith.ones(size).to("cuda")', globals={"size": size}
            )
            de_timer = benchmark.Timer(
                stmt='smith.ones(size, device="cuda")', globals={"size": size}
            )
...
class FP32MatMulPattern(Pattern):
    def __init__(self, prof: profile, should_benchmark: bool = False) -> None:
        ...
        self.description = (
            ...
            "Please enable TF32 by setting 'smith.backends.cuda.matmul.allow_tf32 = True'"
        )
        self.url = "https://blacksmith.org/docs/stable/notes/cuda.html#tensorfloat-32-tf32-on-ampere-devices"
    ...
    def skip(self):
        if smith.version.hip is not None:
            ...
            has_tf32 = all(
                ...
                for arch in smith.cuda.get_arch_list()
            )
    ...
    def match(self, event: _ProfilerEvent) -> bool:
        ...
        if event.tag != _EventType.SmithOp:
            return False
        if not isinstance(event.extra_fields, _ExtraFields_SmithOp):
            raise AssertionError(
                f"expected _ExtraFields_SmithOp, got {type(event.extra_fields).__name__}"
            )
    ...
    def benchmark(self, events: list[_ProfilerEvent]):
        shapes_factor_map = {input_shapes(event): 0.0 for event in events}
        for shape in shapes_factor_map:
            matrixA = smith.randn(shape[0], device="cuda", dtype=smith.float32)
            matrixB = smith.randn(shape[1], device="cuda", dtype=smith.float32)
            fp32_timer = benchmark.Timer(
                stmt="smith.mm(matrixA, matrixB)",
                globals={"matrixA": matrixA, "matrixB": matrixB},
            )
            tf32_timer = benchmark.Timer(
                stmt="smith.mm(matrixA, matrixB)",
                setup="smith.backends.cuda.matmul.allow_tf32 = True",
                globals={"matrixA": matrixA, "matrixB": matrixB},
            )
...
class SynchronizedDataLoaderPattern(Pattern):
    ...
    example:
    smith.utils.data.DataLoader(dataset, batch_size=batch_size)
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False) -> None:
        ...
        self.url = (
            "https://blacksmith.org/tutorials/recipes/recipes/tuning_guide.html"
    ...
    def match(self, event: _ProfilerEvent) -> bool:
        def is_dataloader_function(name: str, function_name: str):
            return name.startswith(
                os.path.join("smith", "utils", "data", "dataloader.py")
...
class MatMulDimInFP16Pattern(Pattern):
    def __init__(self, prof: profile, should_benchmark: bool = False) -> None:
        ...
        self.url = "https://blacksmith.org/tutorials/recipes/recipes/tuning_guide.html#use-mixed-precision-and-amp"
    ...
    def match(self, event: _ProfilerEvent) -> bool:
        ...
        if arg_dtype in (smith.bfloat16, smith.half) and not mutiple_of(
    ...
    def benchmark(self, events: list[_ProfilerEvent]):
        ...
        shapes_factor_map = {input_shapes(event): 0.0 for event in events}
        for shape in shapes_factor_map:
            matrixA = smith.randn(shape[0], device="cuda", dtype=smith.float16)
            matrixB = smith.randn(shape[1], device="cuda", dtype=smith.float16)
            not_aligned_dim_timer = benchmark.Timer(
                stmt="smith.mm(matrixA, matrixB)",
                globals={"matrixA": matrixA, "matrixB": matrixB},
            )
            matrixA = smith.randn(
                closest_multiple(shape[0], 8), device="cuda", dtype=smith.float16
            )
            matrixB = smith.randn(
                closest_multiple(shape[1], 8), device="cuda", dtype=smith.float16
            )
...
def source_code_location(event: _ProfilerEvent | None) -> str:
    while event:
        if event.tag == _EventType.PyCall or event.tag == _EventType.PyCCall:
            ...
            if not event.extra_fields.caller.file_name.startswith("smith" + os.sep):
...
def report_all_anti_patterns(
    ...
    reported = set()
    summaries = []
    message_list = [f"{'-' * 40}SmithTidy Report{'-' * 40}"]
    ...
    if json_report_dir is not None:
        json_report_path = os.path.join(json_report_dir, "smithtidy_report.json")
    ...
    message_list.append(f"{'-' * 40}SmithTidy Report{'-' * 40}")
    if print_enable:
        print("\n".join(message_list))
