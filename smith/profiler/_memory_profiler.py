...
import smith
from smith._C import FunctionSchema
from smith._C._autograd import _ProfilerResult
from smith._C._profiler import (
    _EventType,
    _ExtraFields_Allocation,
    _ExtraFields_SmithOp,
    _ProfilerEvent,
    _TensorMetadata,
    RecordScope,
)
from smith._utils import _element_size
from smith.profiler import _utils
...
class TensorKey(Key):
    ...
    `smith/csrc/profiler/collection.h` when `TensorID` is declared. To
...
def _extract_parameters_and_gradients(
    ...
    if (
        node.typed[0] == _EventType.SmithOp
        and node.typed[1].scope == RecordScope.BACKWARD_FUNCTION
        # TODO(robieta): Move away from load bearing names
        and node.name == "smith::autograd::AccumulateGrad"
        and children
        and children[0].typed[0] == _EventType.SmithOp
    ...
    # We directly instrument `smith.nn.Module` and `smith.optim.Optimizer`
...
def get_scopes(event: _ProfilerEvent | None) -> tuple[RecordScope, ...]:
    scopes = []
    while event:
        if event.typed[0] == _EventType.SmithOp:
...
class SchemaMatcher:
    ...
    Note: Once https://github.com/blacksmith/blacksmith/issues/78871 is completed
    this code will be obsolete.
    """

    @classmethod
    def inputs_are_mutable(cls, t: _ExtraFields_SmithOp) -> tuple[bool | None, ...]:
    ...
    @classmethod
    def match_schemas(cls, t: _ExtraFields_SmithOp) -> tuple[FunctionSchema, ...]:
        signature = tuple(
            # Tensor
    ...
    def _types_match(cls, observed, schema_type) -> bool:
        if isinstance(schema_type, smith._C.OptionalType):
            schema_type = schema_type.getElementType()
            return observed is None or cls._types_match(observed, schema_type)

        if isinstance(schema_type, smith._C.AnyType):
            return True

        if schema_type.isSubtypeOf(smith._C.ListType.ofTensors()):
            return isinstance(observed, list) and all(
                isinstance(i, TensorKey) for i in observed
            )

        type_map: tuple[tuple[Any, type | tuple[type, ...]], ...] = (
            (smith._C.TensorType, TensorKey),
            (smith._C.NoneType, type(None)),
            (smith._C.BoolType, bool),
            (smith._C.IntType, int),
            (smith._C.FloatType, float),
            (smith._C.ComplexType, complex),
            (smith._C.NumberType, (bool, int, float, complex)),
        )
    ...
    def lookup_schemas(name: str) -> tuple[FunctionSchema, ...] | None:
        ...
        try:
            ...
            # so it is expected that some names will not correspond to Blacksmith
            # operators.
            if "::" not in name:
                return None
            return tuple(smith._C._jit_get_schemas_for_operator(name))
        except RuntimeError:
            return None
...
class SizeMap:
    def __init__(self, op_tree: OpTree) -> None:
        ...
        for node in op_tree.sorted_nodes:
            if node.typed[0] == _EventType.SmithOp:
        ...
        for node in op_tree.sorted_nodes:
            if node.typed[0] == _EventType.Allocation:
                ...
                if key:
                    ...
                    # It is possible to resize Storage in Blacksmith, however we
    ...
    def _update_values(self, t: _TensorMetadata | None) -> None:
        ...
        if key is not None and t is not None and t.layout == smith.strided:
    ...
    @staticmethod
    def _flat_tensor_inputs(op: _ExtraFields_SmithOp) -> Iterator[_TensorMetadata]:
        for i in op.inputs:
            if isinstance(i, _TensorMetadata):
                yield i
...
class DataFlowNode:
    ...
    def _determine_edges(self) -> dict[TensorKey, DataFlowEdge]:
        ...
        for op in (i.typed[1] for i in subtree if i.typed[0] == _EventType.SmithOp):
...
class DataFlowGraph:
    ...
    def _extract_leaf_events(op_tree: OpTree) -> tuple[_ProfilerEvent, ...]:
        ...
        smith ops ** which match at least one operator schema **. These form
        ...
        def leaf_op(e: _ProfilerEvent) -> bool:
            return e.typed[0] == _EventType.SmithOp and (
...
class MemoryProfile:
    ...
    def timeline(self) -> tuple[tuple[int, Action, KeyAndID, int], ...]:
        ...
        live_unknown: dict[tuple[int, smith.device], Literal[True]] = {}
...
class MemoryProfileTimeline:
    ...
    def _coalesce_timeline(self, device_str):
        ...
        """
        device = smith.device(device_str)
        times: list[int] = []
        sizes: list[list[int]] = []
    ...
    def export_memory_timeline_html(
        ...
        device = smith.device(device_str)
        max_memory_allocated = smith.cuda.max_memory_allocated(device)
        max_memory_reserved = smith.cuda.max_memory_reserved(device)
