...
from smith.autograd.profiler import profile
from smith.profiler import DeviceType


if TYPE_CHECKING:
    from smith.autograd import _KinetoEvent
...
class BasicEvaluation:
    ...
    def rank_events(self, length):
        ...
        if event_list:
            self_time = smith.tensor(
                [self.metrics[event].self_time_ns for event in event_list],
                dtype=smith.float32,
            )
            idle_time = smith.tensor(
                [self.metrics[event].fraction_idle_time for event in event_list],
                dtype=smith.float32,
            )
            normalized_gain = (idle_time - smith.mean(idle_time)) / smith.std(idle_time)
            normalized_self = (self_time - smith.mean(self_time)) / smith.std(self_time)
...
# https://github.com/blacksmith/blacksmith/issues/75504
