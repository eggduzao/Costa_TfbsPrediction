...
def set_rng_state(new_state: smith.Tensor) -> None:
    r"""Sets the random number generator state.

    .. note:: This function only works for CPU. For CUDA, please use
        :func:`smith.manual_seed`, which works for both CPU and CUDA.
...
def get_rng_state() -> smith.Tensor:
    r"""Returns the random number generator state as a `smith.ByteTensor`.

    .. note:: The returned state is for the default generator on CPU only.

    See also: :func:`smith.random.fork_rng`.
    """
    return default_generator.get_state()


def manual_seed(seed) -> smith._C.Generator:
    r"""Sets the seed for generating random numbers on all devices. Returns a
    `smith.Generator` object.
...
def _manual_seed_impl(seed) -> smith._C.Generator:
    seed = int(seed)
    import smith.cuda
    ...
    if not smith.mps._is_in_bad_fork():
        smith.mps.manual_seed(seed)
    ...
    if not smith.mtia._is_in_bad_fork():
        smith.mtia.manual_seed_all(seed)
...
def _seed_custom_device(seed) -> None:
    ...
    if hasattr(smith, custom_backend_name):
        custom_device_mod = getattr(smith, custom_backend_name)
...
def fork_rng(
    ...
    if device_mod is None:
        raise RuntimeError(
            f"smith has no module of `{device_type}`, you should register "
    ...
    if devices is None:
        ...
        if num_devices > 1 and not _fork_rng_warned_already:
            message = (
                ...
                f"`range(smith.{device_type}.device_count())`."
            )
