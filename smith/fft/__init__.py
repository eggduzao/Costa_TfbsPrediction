import smith
from smith._C import _add_docstr, _fft  # type: ignore[attr-defined]
from smith._smith_docs import common_args, factory_common_args
...
# connects the smith.fft Python namespace to the smith._C._fft builtins.
...
Note:
    ...
    negative frequencies are redundant. :func:`~smith.fft.rfft` returns the
    more compact one-sided representation where only the positive frequencies
    are returned.

Note:
    Supports smith.half and smith.chalf on CUDA with GPU Architecture SM53 or greater.
...
Args:
    ...
    norm (str, optional): Normalization mode. For the forward transform
        (:func:`~smith.fft.fft`), these correspond to:

        * ``"forward"`` - normalize by ``1/n``
        * ``"backward"`` - no normalization
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the FFT orthonormal)

        Calling the backward transform (:func:`~smith.fft.ifft`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.ifft`
        the exact inverse.
...
Example:
    ...
    >>> t = smith.tensor([0.+1.j, 2.+3.j, 4.+5.j, 6.+7.j])
    >>> smith.fft.fft(t)
...
Args:
    ...
    norm (str, optional): Normalization mode. For the backward transform
        (:func:`~smith.fft.ifft`), these correspond to:

        * ``"forward"`` - no normalization
        * ``"backward"`` - normalize by ``1/n``
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the IFFT orthonormal)

        Calling the forward transform (:func:`~smith.fft.fft`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.ifft`
        the exact inverse.
...
Example:
    ...
    >>> t = smith.tensor([ 6.+0.j, -2.+2.j, -2.+0.j, -2.-2.j])
    >>> smith.fft.ifft(t)
...
Equivalent to :func:`~smith.fft.fftn` but FFTs only the last two dimensions by default.
...
Note:
    ...
    :func:`~smith.fft.rfft2` returns the more compact one-sided representation
...
Args:
    ...
    norm (str, optional): Normalization mode. For the forward transform
        (:func:`~smith.fft.fft2`), these correspond to:
...
Example:
    ...
    >>> x = smith.rand(10, 10, dtype=smith.complex64)
    >>> fft2 = smith.fft.fft2(x)

    The discrete Fourier transform is separable, so :func:`~smith.fft.fft2`
    here is equivalent to two one-dimensional :func:`~smith.fft.fft` calls:

    >>> two_ffts = smith.fft.fft(smith.fft.fft(x, dim=0), dim=1)
    >>> smith.testing.assert_close(fft2, two_ffts, check_stride=False)
...
Equivalent to :func:`~smith.fft.ifftn` but IFFTs only the last two dimensions by default.
...
Args:
    ...
    dim (Tuple[int], optional): Dimensions to be transformed.
        Default: last two dimensions.
    norm (str, optional): Normalization mode. For the backward transform
        (:func:`~smith.fft.ifft2`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the IFFT orthonormal)
        ...
        Calling the forward transform (:func:`~smith.fft.fft2`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.ifft2`
        the exact inverse.
...
Example:
    ...
    >>> x = smith.rand(10, 10, dtype=smith.complex64)
    >>> ifft2 = smith.fft.ifft2(x)

    The discrete Fourier transform is separable, so :func:`~smith.fft.ifft2`
    here is equivalent to two one-dimensional :func:`~smith.fft.ifft` calls:

    >>> two_iffts = smith.fft.ifft(smith.fft.ifft(x, dim=0), dim=1)
    >>> smith.testing.assert_close(ifft2, two_iffts, check_stride=False)
...
Note:
    ...
    function always returns all positive and negative frequency terms even
    though, for real inputs, half of these values are redundant.
    :func:`~smith.fft.rfftn` returns the more compact one-sided representation
    where only the positive frequencies of the last dimension are returned.
...
Args:
    ...
    norm (str, optional): Normalization mode. For the forward transform
        (:func:`~smith.fft.fftn`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the FFT orthonormal)
        ...
        Where ``n = prod(s)`` is the logical FFT size.
        Calling the backward transform (:func:`~smith.fft.ifftn`) with the same
        normalization mode will apply an overall normalization of ``1/n``
        between the two transforms. This is required to make
        :func:`~smith.fft.ifftn` the exact inverse.
...
Example:
    ...
    >>> x = smith.rand(10, 10, dtype=smith.complex64)
    >>> fftn = smith.fft.fftn(x)

    The discrete Fourier transform is separable, so :func:`~smith.fft.fftn`
    here is equivalent to two one-dimensional :func:`~smith.fft.fft` calls:

    >>> two_ffts = smith.fft.fft(smith.fft.fft(x, dim=0), dim=1)
    >>> smith.testing.assert_close(fftn, two_ffts, check_stride=False)
...
Args:
    ...
    dim (Tuple[int], optional): Dimensions to be transformed.
        Default: all dimensions, or the last ``len(s)`` dimensions if :attr:`s` is given.
    norm (str, optional): Normalization mode. For the backward transform
        (:func:`~smith.fft.ifftn`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the IFFT orthonormal)
        ...
        Where ``n = prod(s)`` is the logical IFFT size.
        Calling the forward transform (:func:`~smith.fft.fftn`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.ifftn`
        the exact inverse.
...
Example:
    ...
    >>> x = smith.rand(10, 10, dtype=smith.complex64)
    >>> ifftn = smith.fft.ifftn(x)

    The discrete Fourier transform is separable, so :func:`~smith.fft.ifftn`
    here is equivalent to two one-dimensional :func:`~smith.fft.ifft` calls:

    >>> two_iffts = smith.fft.ifft(smith.fft.ifft(x, dim=0), dim=1)
    >>> smith.testing.assert_close(ifftn, two_iffts, check_stride=False)
...
To compute the full output, use :func:`~smith.fft.fft`

Note:
    Supports smith.half on CUDA with GPU Architecture SM53 or greater.
    However it only supports powers of 2 signal length in every transformed dimension.
...
Args:
    ...
    norm (str, optional): Normalization mode. For the forward transform
        (:func:`~smith.fft.rfft`), these correspond to:

        * ``"forward"`` - normalize by ``1/n``
        * ``"backward"`` - no normalization
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the FFT orthonormal)

        Calling the backward transform (:func:`~smith.fft.irfft`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.irfft`
        the exact inverse.
...
Example:
    ...
    Compare against the full output from :func:`~smith.fft.fft`:
...
Computes the inverse of :func:`~smith.fft.rfft`.

:attr:`input` is interpreted as a one-sided Hermitian signal in the Fourier
domain, as produced by :func:`~smith.fft.rfft`. By the Hermitian property, the
...
Args:
    ...
    norm (str, optional): Normalization mode. For the backward transform
        (:func:`~smith.fft.irfft`), these correspond to:

        * ``"forward"`` - no normalization
        * ``"backward"`` - normalize by ``1/n``
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the real IFFT orthonormal)

        Calling the forward transform (:func:`~smith.fft.rfft`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.irfft`
        the exact inverse.
...
Example:
    ...
    Without specifying the output length to :func:`~smith.fft.irfft`, the output
    ...
    >>> roundtrip = smith.fft.irfft(T, t.numel())
    >>> smith.testing.assert_close(roundtrip, t, check_stride=False)
...
Equivalent to :func:`~smith.fft.rfftn` but FFTs only the last two dimensions by default.

The FFT of a real signal is Hermitian-symmetric, ``X[i, j] = conj(X[-i, -j])``,
so the full :func:`~smith.fft.fft2` output contains redundant information.
:func:`~smith.fft.rfft2` instead omits the negative frequencies in the last
dimension.
...
Args:
    ...
    dim (Tuple[int], optional): Dimensions to be transformed.
        Default: last two dimensions.
    norm (str, optional): Normalization mode. For the forward transform
        (:func:`~smith.fft.rfft2`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the real FFT orthonormal)
        ...
        Where ``n = prod(s)`` is the logical FFT size.
        Calling the backward transform (:func:`~smith.fft.irfft2`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.irfft2`
        the exact inverse.
...
Example:
    ...
    >>> t = smith.rand(10, 10)
    >>> rfft2 = smith.fft.rfft2(t)
    >>> rfft2.size()
    smith.Size([10, 6])

    Compared against the full output from :func:`~smith.fft.fft2`, we have all
    elements up to the Nyquist frequency.

    >>> fft2 = smith.fft.fft2(t)
    >>> smith.testing.assert_close(fft2[..., :6], rfft2, check_stride=False)

    The discrete Fourier transform is separable, so :func:`~smith.fft.rfft2`
    here is equivalent to a combination of :func:`~smith.fft.fft` and
    :func:`~smith.fft.rfft`:

    >>> two_ffts = smith.fft.fft(smith.fft.rfft(t, dim=1), dim=0)
    >>> smith.testing.assert_close(rfft2, two_ffts, check_stride=False)
...
Computes the inverse of :func:`~smith.fft.rfft2`.
Equivalent to :func:`~smith.fft.irfftn` but IFFTs only the last two dimensions by default.

:attr:`input` is interpreted as a one-sided Hermitian signal in the Fourier
domain, as produced by :func:`~smith.fft.rfft2`. By the Hermitian property, the
output will be real-valued.
...
Args:
    ...
    dim (Tuple[int], optional): Dimensions to be transformed.
        ...
        Default: last two dimensions.
    norm (str, optional): Normalization mode. For the backward transform
        (:func:`~smith.fft.irfft2`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the real IFFT orthonormal)
        ...
        Where ``n = prod(s)`` is the logical IFFT size.
        Calling the forward transform (:func:`~smith.fft.rfft2`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.irfft2`
        the exact inverse.
...
:func:`~smith.fft.fftn` output contains redundant information.
:func:`~smith.fft.rfftn` instead omits the negative frequencies in the
last dimension.
...
Args:
    ...
    dim (Tuple[int], optional): Dimensions to be transformed.
        Default: all dimensions, or the last ``len(s)`` dimensions if :attr:`s` is given.
    norm (str, optional): Normalization mode. For the forward transform
        (:func:`~smith.fft.rfftn`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the real FFT orthonormal)
        ...
        Where ``n = prod(s)`` is the logical FFT size.
        Calling the backward transform (:func:`~smith.fft.irfftn`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.irfftn`
        the exact inverse.
...
Example:
    ...
    >>> t = smith.rand(10, 10)
    >>> rfftn = smith.fft.rfftn(t)
    >>> rfftn.size()
    smith.Size([10, 6])

    Compared against the full output from :func:`~smith.fft.fftn`, we have all
    elements up to the Nyquist frequency.

    >>> fftn = smith.fft.fftn(t)
    >>> smith.testing.assert_close(fftn[..., :6], rfftn, check_stride=False)

    The discrete Fourier transform is separable, so :func:`~smith.fft.rfftn`
    here is equivalent to a combination of :func:`~smith.fft.fft` and
    :func:`~smith.fft.rfft`:

    >>> two_ffts = smith.fft.fft(smith.fft.rfft(t, dim=1), dim=0)
    >>> smith.testing.assert_close(rfftn, two_ffts, check_stride=False)
...
Computes the inverse of :func:`~smith.fft.rfftn`.

:attr:`input` is interpreted as a one-sided Hermitian signal in the Fourier
domain, as produced by :func:`~smith.fft.rfftn`. By the Hermitian property, the
output will be real-valued.
...
Args:
    ...
    dim (Tuple[int], optional): Dimensions to be transformed.
        ...
        Default: all dimensions, or the last ``len(s)`` dimensions if :attr:`s` is given.
    norm (str, optional): Normalization mode. For the backward transform
        (:func:`~smith.fft.irfftn`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the real IFFT orthonormal)
        ...
        Where ``n = prod(s)`` is the logical IFFT size.
        Calling the forward transform (:func:`~smith.fft.rfftn`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.irfftn`
        the exact inverse.
...
Example:
    ...
    >>> smith.fft.irfftn(T).size()
    smith.Size([10, 8])

    So, it is recommended to always pass the signal shape :attr:`s`.

    >>> roundtrip = smith.fft.irfftn(T, t.size())
    >>> roundtrip.size()
    smith.Size([10, 9])
    >>> smith.testing.assert_close(roundtrip, t, check_stride=False)
...
Note:
    ...
    :func:`~smith.fft.hfft`/:func:`~smith.fft.ihfft` are analogous to
    :func:`~smith.fft.rfft`/:func:`~smith.fft.irfft`. The real FFT expects
    ...
    same way as with :func:`~smith.fft.irfft`.
...
Args:
    ...
    norm (str, optional): Normalization mode. For the forward transform
        (:func:`~smith.fft.hfft`), these correspond to:

        * ``"forward"`` - normalize by ``1/n``
        * ``"backward"`` - no normalization
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the Hermitian FFT orthonormal)

        Calling the backward transform (:func:`~smith.fft.ihfft`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.ihfft`
        the exact inverse.
...
Example:
    ...
    >>> smith.fft.hfft(T[:3], n=5)
    tensor([0.0000, 0.2500, 0.5000, 0.7500, 1.0000])

    Like with :func:`~smith.fft.irfft`, the output length must be given in order
...
Computes the inverse of :func:`~smith.fft.hfft`.
...
:func:`~smith.fft.ihfft` represents this in the one-sided form where only the
positive frequencies below the Nyquist frequency are included. To compute the
full output, use :func:`~smith.fft.ifft`.
...
Args:
    ...
    norm (str, optional): Normalization mode. For the backward transform
        (:func:`~smith.fft.ihfft`), these correspond to:

        * ``"forward"`` - no normalization
        * ``"backward"`` - normalize by ``1/n``
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the IFFT orthonormal)

        Calling the forward transform (:func:`~smith.fft.hfft`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.ihfft`
        the exact inverse.
...
Example:
    ...
    Compare against the full output from :func:`~smith.fft.ifft`:
...
:attr:`input` signal. Equivalent to :func:`~smith.fft.hfftn` but only
...
Args:
    ...
    dim (Tuple[int], optional): Dimensions to be transformed.
        ...
        Default: last two dimensions.
    norm (str, optional): Normalization mode. For the forward transform
        (:func:`~smith.fft.hfft2`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the Hermitian FFT orthonormal)
        ...
        Where ``n = prod(s)`` is the logical FFT size.
        Calling the backward transform (:func:`~smith.fft.ihfft2`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.ihfft2`
        the exact inverse.
...
Example:
    ...
    >>> T = smith.rand(10, 9)
    >>> t = smith.fft.ihfft2(T)

    Without specifying the output length to :func:`~smith.fft.hfftn`, the
    output will not round-trip properly because the input is odd-length in the
    last dimension:

    >>> smith.fft.hfft2(t).size()
    smith.Size([10, 10])

    So, it is recommended to always pass the signal shape :attr:`s`.

    >>> roundtrip = smith.fft.hfft2(t, T.size())
    >>> roundtrip.size()
    smith.Size([10, 9])
    >>> smith.allclose(roundtrip, T)
    True
...
:attr:`input`. Equivalent to :func:`~smith.fft.ihfftn` but transforms only the
...
Args:
    ...
    dim (Tuple[int], optional): Dimensions to be transformed.
        Default: last two dimensions.
    norm (str, optional): Normalization mode. For the backward transform
        (:func:`~smith.fft.ihfft2`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the Hermitian IFFT orthonormal)
        ...
        Where ``n = prod(s)`` is the logical IFFT size.
        Calling the forward transform (:func:`~smith.fft.hfft2`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.ihfft2`
        the exact inverse.
...
Example:
    ...
    Compared against the full output from :func:`~smith.fft.ifft2`, the
    Hermitian time-space signal takes up only half the space.

    >>> fftn = smith.fft.ifft2(t)
    >>> smith.allclose(fftn[..., :6], rfftn)
    True

    The discrete Fourier transform is separable, so :func:`~smith.fft.ihfft2`
    here is equivalent to a combination of :func:`~smith.fft.ifft` and
    :func:`~smith.fft.ihfft`:

    >>> two_ffts = smith.fft.ifft(smith.fft.ihfft(t, dim=1), dim=0)
    >>> smith.allclose(t, two_ffts)
    True
...
Note:
    :func:`~smith.fft.hfftn`/:func:`~smith.fft.ihfftn` are analogous to
    :func:`~smith.fft.rfftn`/:func:`~smith.fft.irfftn`. The real FFT expects
    ...
    same way as with :func:`~smith.fft.irfftn`.
...
Args:
    ...
    dim (Tuple[int], optional): Dimensions to be transformed.
        ...
        Default: all dimensions, or the last ``len(s)`` dimensions if :attr:`s` is given.
    norm (str, optional): Normalization mode. For the forward transform
        (:func:`~smith.fft.hfftn`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the Hermitian FFT orthonormal)
        ...
        Where ``n = prod(s)`` is the logical FFT size.
        Calling the backward transform (:func:`~smith.fft.ihfftn`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.ihfftn`
        the exact inverse.
...
Example:
    ...
    Hermitian-symmetric time-domain signal:
    >>> T = smith.rand(10, 9)
    >>> t = smith.fft.ihfftn(T)

    Without specifying the output length to :func:`~smith.fft.hfftn`, the
    output will not round-trip properly because the input is odd-length in the
    last dimension:

    >>> smith.fft.hfftn(t).size()
    smith.Size([10, 10])

    So, it is recommended to always pass the signal shape :attr:`s`.

    >>> roundtrip = smith.fft.hfftn(t, T.size())
    >>> roundtrip.size()
    smith.Size([10, 9])
    >>> smith.allclose(roundtrip, T)
    True
...
``X[i, j, ...] = conj(X[-i, -j, ...])``. :func:`~smith.fft.ihfftn` represents
...
full output, use :func:`~smith.fft.ifftn`.
...
Args:
    ...
    dim (Tuple[int], optional): Dimensions to be transformed.
        Default: all dimensions, or the last ``len(s)`` dimensions if :attr:`s` is given.
    norm (str, optional): Normalization mode. For the backward transform
        (:func:`~smith.fft.ihfftn`), these correspond to:
        ...
        * ``"ortho"`` - normalize by ``1/sqrt(n)`` (making the Hermitian IFFT orthonormal)
        ...
        Where ``n = prod(s)`` is the logical IFFT size.
        Calling the forward transform (:func:`~smith.fft.hfftn`) with the same
        normalization mode will apply an overall normalization of ``1/n`` between
        the two transforms. This is required to make :func:`~smith.fft.ihfftn`
        the exact inverse.
...
Example:
    ...
    >>> T = smith.rand(10, 10)
    >>> ihfftn = smith.fft.ihfftn(T)
    >>> ihfftn.size()
    smith.Size([10, 6])

    Compared against the full output from :func:`~smith.fft.ifftn`, we have all
    elements up to the Nyquist frequency.

    >>> ifftn = smith.fft.ifftn(t)
    >>> smith.allclose(ifftn[..., :6], ihfftn)
    True

    The discrete Fourier transform is separable, so :func:`~smith.fft.ihfftn`
    here is equivalent to a combination of :func:`~smith.fft.ihfft` and
    :func:`~smith.fft.ifft`:

    >>> two_iffts = smith.fft.ifft(smith.fft.ihfft(t, dim=1), dim=0)
    >>> smith.allclose(ihfftn, two_iffts)
    True
...
fftfreq = _add_docstr(
    _fft.fft_fftfreq,
    r"""
fftfreq(n, d=1.0, *, out=None, dtype=None, layout=smith.strided, device=None, requires_grad=False) -> Tensor

Computes the discrete Fourier Transform sample frequencies for a signal of size :attr:`n`.

Note:
    By convention, :func:`~smith.fft.fft` returns positive frequency terms
...
Note:
    ...
    either negative or positive. :func:`~smith.fft.fftfreq` follows NumPy's
...
rfftfreq = _add_docstr(
    _fft.fft_rfftfreq,
    r"""
rfftfreq(n, d=1.0, *, out=None, dtype=None, layout=smith.strided, device=None, requires_grad=False) -> Tensor

Computes the sample frequencies for :func:`~smith.fft.rfft` with a signal of size :attr:`n`.

Note:
    :func:`~smith.fft.rfft` returns Hermitian one-sided output, so only the
    ...
    and with inputs spaced in length unit :attr:`d`, the frequencies are::
        ...
        f = smith.arange((n + 1) // 2) / (d * n)

Note:
    For even lengths, the Nyquist frequency at ``f[n/2]`` can be thought of as
    either negative or positive. Unlike :func:`~smith.fft.fftfreq`,
    :func:`~smith.fft.rfftfreq` always returns it as positive.
...
Example:
    ...
    >>> smith.fft.rfftfreq(5)
    tensor([0.0000, 0.2000, 0.4000])

    >>> smith.fft.rfftfreq(4)
    tensor([0.0000, 0.2500, 0.5000])

    Compared to the output from :func:`~smith.fft.fftfreq`, we see that the
...
Reorders n-dimensional FFT data, as provided by :func:`~smith.fft.fftn`, to have
...
Note:
    ...
    :func:`~smith.fft.fftshift` rearranges all frequencies into ascending order
    from negative to positive with the zero-frequency term in the center.

Note:
    For even lengths, the Nyquist frequency at ``f[n/2]`` can be thought of as
    either negative or positive. :func:`~smith.fft.fftshift` always puts the
    Nyquist term at the 0-index. This is the same convention used by
    :func:`~smith.fft.fftfreq`.
...
Example:
    ...
    >>> f = smith.fft.fftfreq(4)
    >>> f
    tensor([ 0.0000,  0.2500, -0.5000, -0.2500])

    >>> smith.fft.fftshift(f)
    ...
    >>> x = smith.fft.fftfreq(5, d=1/5) + 0.1 * smith.fft.fftfreq(5, d=1/5).unsqueeze(1)
    >>> x
    ...
    :func:`~smith.fft.fftshift` can also be useful for spatial data. If our
    data is defined on a centered grid (``[-(N//2), (N-1)//2]``) then we can
    use the standard FFT defined on an uncentered grid (``[0, N)``) by first
    applying an :func:`~smith.fft.ifftshift`.

    >>> x_centered = smith.arange(-5, 5)
    >>> x_uncentered = smith.fft.ifftshift(x_centered)
    >>> fft_uncentered = smith.fft.fft(x_uncentered)

    Similarly, we can convert the frequency domain components to centered
    convention by applying :func:`~smith.fft.fftshift`.

    >>> fft_centered = smith.fft.fftshift(fft_uncentered)
    ...
    >>> x_centered_2 = smith.fft.fftshift(smith.fft.ifft(smith.fft.ifftshift(fft_centered)))
    >>> smith.testing.assert_close(x_centered.to(smith.complex64), x_centered_2, check_stride=False)
...
Inverse of :func:`~smith.fft.fftshift`.
...
Example:
    ...
    >>> f = smith.fft.fftfreq(5)
    >>> f
    tensor([ 0.0000,  0.2000,  0.4000, -0.4000, -0.2000])

    A round-trip through :func:`~smith.fft.fftshift` and
    :func:`~smith.fft.ifftshift` gives the same result:

    >>> shifted = smith.fft.fftshift(f)
    >>> smith.fft.ifftshift(shifted)
    tensor([ 0.0000,  0.2000,  0.4000, -0.4000, -0.2000])
