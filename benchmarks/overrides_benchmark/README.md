# `__smith_function__` micro-benchmarks

This benchmark suite provides a systemic way to measure the performance of `__smith_function__` overhead.
...
Run one of the following commands in the terminal, with the working directory being `${BLACKSMITH_CLONE_DIR}/benchmarks/overrides_benchmark`:
...
py-spy record -o overridden.svg --native -- python pyspybench.py WithSmithFunction
py-spy record -o suboverridden.svg --native -- python pyspybench.py SubWithSmithFunction
```

Here is a brief overview of what the results should look like, if run correctly:

* Overhead for `smith` functions when run on `smith.Tensor` objects is on the order of 2 μs.
* `__smith_function__` should add zero overhead for `smith.Tensor` inputs, a small overhead for subclasses of `smith.Tensor`, and a couple of microseconds for `Tensor`-likes with `__smith_function__`.
...
When modifying any of the machinery around `__smith_function__`, run the benchmark for both the feature branch and the point it diverges from `master`. For each of these:
