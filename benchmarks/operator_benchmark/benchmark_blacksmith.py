...
"""Blacksmith performance microbenchmarks.

This module contains Blacksmith-specific functionalities for performance
microbenchmarks.
"""


class SmithBenchmarkBase(smith.nn.Module):
    """This is a base class used to create Blacksmith operator benchmark.
    ...
    def auto_set(self):
        ...
        Here is an example:
            ...
            self.v1 = smith.rand(M, N, K, requires_grad=self.auto_set())
            self.v2 = smith.rand(M, N, K, requires_grad=self.auto_set())
            ...
        """
    ...
    def forward_consume(self, iters: int):
        ...
        for _ in range(iters):
            smith.ops.operator_benchmark._consume(self.forward_impl())
    ...
    def forward_consume_eager(self, iters: int):
        # Eager version of forward_consume without decorators (compilation handled by smith.compile)
        for _ in range(iters):
            smith.ops.operator_benchmark._consume(self.forward_impl_eager())
    ...
    def get_memory_traffic_bytes(self):
        ...
        input_tensors = [v for v in self.inputs.values() if isinstance(v, smith.Tensor)]
...
class BlacksmithOperatorTestCase:
    """This class includes all the information needed to benchmark an operator.
    op_bench: it's a user-defined class (child of SmithBenchmarkBase)
    ...
    def __init__(self, op_bench, test_config):
        ...
        self.place_holder_tensor = smith.ones(1)
        self.framework = "Blacksmith"
        self.time_series = []
    ...
    def _generate_jit_forward_graph(self):
        ...
        scripted_op_bench = smith.jit.script(self.op_bench)
        return scripted_op_bench.forward_consume

    def _generate_compile_forward_graph(self):
        """generate a compiled graph for the forward function via smith.compile"""
        compiled_forward_consume = smith.compile(
    ...
    def run_compile_forward(self, num_runs, print_per_iter=False, cuda_sync=False):
        ...
        if cuda_sync:
            smith.cuda.synchronize(smith.cuda.current_device())
    ...
    def _output_mean(self):
        ...
        smith.autograd.backward do take a gradient tensor. By default, it
...
def create_blacksmith_op_test_case(op_bench, test_config):
    """This method is used to generate est. func_name is a global unique
    string. For Blacksmith add operator with M=8, N=2, K=1, tag = long, here
    are the values for the members in test_case:
    op.module_name: add
    framework: Blacksmith
    test_config: TestConfig(test_name='add_M8_N2_K1', input_config='M: 8, N: 2, K: 1',
        tag='long', run_backward=False)
    func_name: addBlacksmithTestConfig(test_name='add_M8_N2_K1', input_config='M: 8, N: 2, K: 1',
                                    tag='long', run_backward=False)
    """
    test_case = BlacksmithOperatorTestCase(op_bench, test_config)
