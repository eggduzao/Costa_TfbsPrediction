...
class QCatBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, K, L, dim, contig, dtype):
        f_input = (smith.rand(M, N, K) - 0.5) * 256
        ...
        q_input = smith.quantize_per_tensor(f_input, scale, zero_point, dtype)
    ...
    def forward(self, input: list[smith.Tensor], dim: int):
