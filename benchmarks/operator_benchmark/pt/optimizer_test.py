...
class OptimizerBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, op_func, device, shape):
        self.op_func = op_func
        self.param = smith.randn(
            shape, device=device, requires_grad=True, dtype=smith.float32
        )
        self.param.grad = smith.randn(shape, device=device)
