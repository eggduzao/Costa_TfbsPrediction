...
ternary_ops = op_bench.op_list(
    ...
    attrs=[
        ["addcmul", smith.addcmul],
        ["addcdiv", smith.addcdiv],
    ],
)
...
ternary_configs_short = op_bench.config_list(
    ...
    cross_product_configs={
        "device": ["cpu"],
        "dtype": [smith.float, smith.bfloat16],
    },
    tags=["short"],
)
...
class TernaryOpBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, device, dtype, op_func):
        self.inputs = {
            "input_": smith.rand((M, N), device=device).to(dtype=dtype),
            "tensor1": smith.rand((M, N), device=device).to(dtype=dtype),
            "tensor2": smith.rand((M, N), device=device).to(dtype=dtype),
        }
