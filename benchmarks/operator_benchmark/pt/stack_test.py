...
class StackBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, sizes, N, dim, device):
        ...
        for s in gen_sizes:
            inputs.append(smith.rand(s, device=device))
        result = smith.rand(gen_sizes[0], device=device)
        self.inputs = {"result": result, "inputs": inputs, "dim": dim}
        self.set_module_name("stack")

    def forward(self, result: smith.Tensor, inputs: list[smith.Tensor], dim: int):
        return smith.stack(inputs, dim=dim, out=result)
