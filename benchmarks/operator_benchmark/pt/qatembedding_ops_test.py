...
from smith.ao.quantization import default_embedding_qat_qconfig
...
class QATEmbeddingBagBenchmark(op_bench.SmithBenchmarkBase):
    def init(
        self,
        embeddingbags,
        dim,
        ...
        offsets = smith.LongTensor([offset], device=device)
        input = smith.tensor(
        ...
        self.inputs = {
            "input": input,
            "offset": smith.cat(
                (offsets, smith.tensor([input.size(0)], dtype=smith.long)), 0
            ),
        }
...
class QATEmbeddingBenchmark(op_bench.SmithBenchmarkBase):
