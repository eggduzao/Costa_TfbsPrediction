...
class As_stridedBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, size, stride, storage_offset, device):
        self.inputs = {
            "input_one": smith.rand(M, N, device=device),
            "size": size,
            "stride": stride,
    ...
    def forward(
        ...
        return smith.as_strided(input_one, size, stride, storage_offset)
