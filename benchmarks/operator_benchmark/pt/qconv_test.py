...
class QConv1dBenchmark(op_bench.SmithBenchmarkBase):
    ...
    def init(self, IC, OC, kernel, stride, N, L, device):
        ...
        self.scale = 1.0 / 255
        self.zero_point = 0
        X = smith.randn(N, IC, L, dtype=smith.float32)
        qX = smith.quantize_per_tensor(
            X, scale=self.scale, zero_point=self.zero_point, dtype=smith.quint8
        )
        # Convert the tensor to NHWC format
        W = smith.randn(OC, IC // G, kernel, dtype=smith.float32)
        self.qW = smith.quantize_per_tensor(
            W, scale=self.scale, zero_point=0, dtype=smith.qint8
        )
        ...
        self.qconv1d.scale = smith.tensor(self.scale, dtype=smith.double)
        self.qconv1d.zero_point = smith.tensor(self.zero_point, dtype=smith.int)
...
class QConv2dBenchmark(op_bench.SmithBenchmarkBase):
    ...
    def init(self, IC, OC, kernel, stride, N, H, W, G, pad, device):
        ...
        self.scale = 1.0 / 255
        self.zero_point = 0
        X = smith.randn(N, IC, H, W, dtype=smith.float32)
        qX = smith.quantize_per_tensor(
            X, scale=self.scale, zero_point=self.zero_point, dtype=smith.quint8
        )
        # Convert the tensor to NHWC format
        W = smith.randn(OC, IC // G, kernel, kernel, dtype=smith.float32)
        self.qW = smith.quantize_per_tensor(
            W, scale=self.scale, zero_point=0, dtype=smith.qint8
        )
        ...
        self.qconv2d.scale = smith.tensor(self.scale, dtype=smith.double)
        self.qconv2d.zero_point = smith.tensor(self.zero_point, dtype=smith.int)
