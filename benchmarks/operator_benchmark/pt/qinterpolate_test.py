...
qinterpolate_short_configs = op_bench.config_list(
    ...
    attrs=[
        [32, 32, 32, smith.quint8, "nearest", 0.5, True],  # Downsample
        [32, 32, 32, smith.quint8, "bilinear", 0.5, True],  # Downsample
        [32, 32, 32, smith.quint8, "nearest", 2.0, True],  # Upsample
        [32, 32, 32, smith.quint8, "bilinear", 2.0, True],  # Upsample
        [3, 720, 1280, smith.quint8, "bilinear", 0.83333, True],  # Downsample
    ],
    tags=["short"],
)


class QInterpolateBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, K, dtype, mode, scale, contig):
        f_input = (smith.rand(1, M, N, K) - 0.5) * 256
        scale = 0.1
        zero_point = 42
