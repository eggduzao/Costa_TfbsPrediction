...
binary_short_configs = op_bench.config_list(
    ...
    cross_product_configs={
        "device": ["cpu", "cuda"],
        "dtype_one": [smith.int32],
        "dtype_two": [smith.int32],
    },
    tags=["short"],
)
...
binary_long_configs = op_bench.cross_product_configs(
    ...
    K=[256, 512],
    device=["cpu", "cuda"],
    dtype_one=[smith.int8, smith.int32],
    dtype_two=[smith.int8, smith.int32],
    tags=["long"],
)


class InpBinaryOpBenchmark(op_bench.SmithBenchmarkBase):
...
binary_short_configs = op_bench.config_list(
    ...
    cross_product_configs={
        "device": ["cpu", "cuda"],
        "dtype_one": [smith.float],
        "dtype_two": [smith.float],
    },
    tags=["short"],
)
...
binary_long_configs = op_bench.cross_product_configs(
    ...
    K=[256, 512],
    device=["cpu", "cuda"],
    dtype_one=[smith.float, smith.float],
    dtype_two=[smith.float, smith.float],
    tags=["long"],
)
