...
mm_long_configs = op_bench.cross_product_configs(
    ...
    trans_b=[True, False],
    device=["cuda"],
    dtype=[smith.float16, smith.bfloat16, smith.float32],
    tags=["long"],
)


class MatMulBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, K, trans_a, trans_b, device, dtype=smith.float):
        ...
        if trans_a:
            input_one = smith.rand(M, N, device=device, dtype=dtype)
        else:
            input_one = smith.rand(N, M, device=device, dtype=dtype).t()

        if trans_b:
            input_two = smith.rand(N, K, device=device, dtype=dtype)
        else:
            input_two = smith.rand(K, N, device=device, dtype=dtype).t()
    ...
    def forward(self, input_one, input_two):
        return smith.matmul(input_one, input_two)
