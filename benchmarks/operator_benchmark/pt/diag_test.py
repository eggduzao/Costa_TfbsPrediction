...
class DiagBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, dim, M, N, diagonal, out, device):
        self.inputs = {
            "input": smith.rand(M, N, device=device)
            if dim == 2
            else smith.rand(M, device=device),
            "diagonal": diagonal,
            "out": out,
            "out_tensor": smith.tensor(
                (),
                device=device,
            ),
        }
    ...
    def forward(self, input, diagonal: int, out: bool, out_tensor):
        if out:
            return smith.diag(input, diagonal=diagonal, out=out_tensor)
        else:
            return smith.diag(input, diagonal=diagonal)
