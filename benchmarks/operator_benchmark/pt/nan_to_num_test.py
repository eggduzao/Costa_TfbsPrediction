...
"""Microbenchmarks for smith.nan_to_num / nan_to_num_ operators"""

# Configs for PT smith.nan_to_num / nan_to_num_ operators

nan_to_num_ops_list = op_bench.op_list(
    attr_names=["op_name", "op_func"],
    attrs=[
        ["nan_to_num", smith.nan_to_num],
        ["nan_to_num_", smith.nan_to_num_],
    ],
)
...
nan_to_num_long_configs = op_bench.cross_product_configs(
    M=[32, 64, 128],
    N=range(32, 128, 32),
    dtype=[smith.float, smith.double],
...
class ReplaceNaNBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, dtype, replace_inf, op_func):
        input = smith.randn(M, N, dtype=dtype)
        input[0][0] = float("nan")
        ...
        if op_func is smith.nan_to_num_:
