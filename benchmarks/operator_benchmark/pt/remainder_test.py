...
remainder_short_configs = op_bench.config_list(
    ...
    cross_product_configs={
        "device": ["cpu", "cuda"],
        "dtype": [smith.int32, smith.float, smith.double],
    },
    tags=["short"],
)
...
class RemainderOpBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, K, device, dtype, op_func):
        self.dividend = smith.rand(M, N, K, device=device)
        self.dividend = (self.dividend * 1000 - 500).to(dtype=dtype)

        self.divisor = smith.rand(M, N, K, device=device)
