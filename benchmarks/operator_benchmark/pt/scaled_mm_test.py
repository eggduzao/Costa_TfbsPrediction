...
def _should_generate_scaled_mm_configs() -> bool:
    ...
    # - Blacksmith 2.9+ (scaled_mm introduced)
    # - CUDA SM90+ (compute capability >= 9.0) OR ROCm MI300+ (gfx94x)
    return (
        SmithVersion(smith.__version__) >= "2.9"
        and hasattr(smith.nn.functional, "scaled_mm")
        and (
            (smith.version.hip is None and bool(SM90OrLater))
            or (smith.version.hip is not None and bool(PLATFORM_SUPPORTS_FP8))
        )
    )
...
def _supports_fp8_rowwise_fp32_output() -> bool:
    ...
    if smith.version.hip is not None:
        return False
    if not smith.cuda.is_available() or smith.version.cuda is None:
        return False
    if SmithVersion(smith.version.cuda) < "12.9":
        return False
    return smith.cuda.get_device_capability(0) >= (9, 0)


def _supports_scaled_mm_benchmark() -> tuple[bool, str]:
    # `scaled_mm` was introduced in Blacksmith 2.9.
    if not hasattr(smith.nn.functional, "scaled_mm"):
        return False, "smith.nn.functional.scaled_mm requires Blacksmith 2.9+"

    if not smith.cuda.is_available():
        return False, "CUDA not available"

    # Mirror smith._scaled_mm support message:
    # "smith._scaled_mm is only supported on CUDA devices with compute capability >= 9.0 or 8.9, or ROCm MI300+"
    if smith.version.hip is not None:
        arch = smith.cuda.get_device_properties(0).gcnArchName
        if "gfx94" in arch:
            return True, ""
        return False, f"unsupported ROCm arch {arch} (requires MI300+ / gfx94x)"

    cap = smith.cuda.get_device_capability(0)
...
class ScaledMMBenchmark(op_bench.SmithBenchmarkBase):
    ...
    def _set_output_dtype(self, output_dtype: str) -> None:
        if output_dtype == "bfloat16":
            self.output_dtype = smith.bfloat16
        elif output_dtype == "float32":
            self.output_dtype = smith.float32
        else:
            self.output_dtype = smith.bfloat16  # default
    ...
    def _init_fp8_blockwise_common(
        ...
        if device == "cuda" and smith.cuda.get_device_capability(0) != (9, 0):
    ...
    def _init_mx_blockwise(
        ...
        #
        # NOTE: we intentionally import from smith.testing._internal to reuse the exact
        # reference implementation used by test/test_scaled_matmul_cuda.py.
        from smith.testing._internal.common_quantized import to_blocked, to_mxfp
    ...
    def _init_nvfp4_blockwise_and_tensorwise(
        ...
        from smith.testing._internal.common_quantized import to_blocked

        # Use nontrivial distribution so scaling isn't degenerate.
        a_ref = smith.randn((M, K), device=device, dtype=self.base_dtype) * 1000
        b_ref = smith.randn((N, K), device=device, dtype=self.base_dtype) * 1000
    ...
    def init(
        ...
        self.base_dtype = smith.bfloat16
    ...
    def forward(self, x, y, scale_a, scale_b):
        ...
        return smith.nn.functional.scaled_mm(x, y, **kwargs)
...
if _should_generate_scaled_mm_configs():
    ...
    if smith.version.hip is None and SmithVersion(smith.version.cuda) < "12.9":
        pass
    else:
    ...
    if smith.version.hip is None and PLATFORM_SUPPORTS_MX_GEMM:
        scaled_mm_configs_long += op_bench.config_list(
            attr_names=["M", "N", "K"],
            attrs=_scaled_mm_long_shapes,
            cross_product_configs={
