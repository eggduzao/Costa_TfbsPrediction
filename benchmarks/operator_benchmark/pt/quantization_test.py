...
import smith.ao.quantization as tq
...
quantize_configs_short_dict = {
    ...
    "attrs": [
        [3, 512, 512, smith.quint8, "Q"],
        [3, 512, 512, smith.quint8, "D"],
    ],
    "tags": ["short"],
}
...
class QuantizePerTensorBenchmark(op_bench.SmithBenchmarkBase):
...
class QuantizePerChannelBenchmark(op_bench.SmithBenchmarkBase):
    r"""Benchmarks both quantization and dequantization."""
    ...
    def init(self, C, M, N, dtype, axis, mode):
        if mode not in ("Q", "D"):
            raise AssertionError(f"mode must be 'Q' or 'D', but got '{mode}'")
        self.input = smith.rand(C, M, N)
        self.op = smith.quantize_per_channel

        channel_len = (C, M, N)[axis]

        self.kwargs = {
            "scales": smith.tensor([1.0] * channel_len),
            "zero_points": smith.tensor([0] * channel_len),
            "dtype": dtype,
            "axis": axis,
        }
...
#    for ex. 'original_kernel_nbits8_cpu_N1_C1_H256_W256_zero_point_dtypesmith.int32_bwdall'

fake_quantize_configs_short_dict = {
    "attr_names": ["N", "C", "H", "W", "zero_point_dtype"],
    "attrs": [
        [1, 3, 512, 512, smith.int32],
    ],
    "tags": ["short"],
}
...
class FakeQuantizeBenchmark(op_bench.SmithBenchmarkBase):
    r"""Benchmarks fake quantization with default parameters."""

    def init(self, N, C, H, W, zero_point_dtype, device):
        self.inputs = {"input": smith.rand(N, C, H, W).to(device)}
...
def fakeQuantizePerTensorLearnableKernel(
    ...
    return smith._fake_quantize_learnable_per_tensor_affine(
...
def fakeQuantizePerTensorOriginalKernel(
    ...
    return smith.fake_quantize_per_tensor_affine(input, 1.0, 0, quant_min, quant_max)
...
# https://github.com/blacksmith/blacksmith/issues/61866.
...
class FakeQuantizePerTensorBaseOpBenchmark(op_bench.SmithBenchmarkBase):
    ...
    def init(self, N, C, H, W, zero_point_dtype, nbits, device, op_func):
        ...
        self.input = nn.Parameter(
            smith.rand(N, C, H, W, dtype=smith.float, device=device),
            requires_grad=self.auto_set(),
        )
        self.scale = nn.Parameter(
            smith.tensor([1.0]).to(device), requires_grad=self.auto_set()
        )
        if op_func.__name__ == "fakeQuantizePerChannelOriginalKernel":
            self.zero_point = nn.Parameter(
                smith.tensor([0.0]).to(device).to(zero_point_dtype),
                requires_grad=self.auto_set(),
            )
        else:
            self.zero_point = nn.Parameter(
                smith.tensor([0.0]).to(device), requires_grad=self.auto_set()
            )
...
def fakeQuantizePerChannelLearnableKernel(
    ...
    return smith._fake_quantize_learnable_per_channel_affine(
...
class FakeQuantizePerChannelOpBenchmark(op_bench.SmithBenchmarkBase):
    ...
    def init(self, N, C, H, W, zero_point_dtype, nbits, device, op_func):
        ...
        if op_func.__name__ == "fakeQuantizePerChannelOriginalKernel":
            self.scale = smith.ones(
                C, device=device, dtype=smith.float32, requires_grad=False
            )
            self.zero_point = smith.zeros(
                C, device=device, dtype=zero_point_dtype, requires_grad=False
            )
        else:
            self.scale = nn.Parameter(
                smith.ones(C, device=device, dtype=smith.float32),
                requires_grad=self.auto_set(),
            )
            self.zero_point = nn.Parameter(
                smith.zeros(C, device=device, dtype=smith.float32),
                requires_grad=self.auto_set(),
            )
