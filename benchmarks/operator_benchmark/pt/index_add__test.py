...
class IndexAddBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, K, dim, dtype, device):
        # creating the original tensor
        tensor = smith.rand(M, N, K, dtype=dtype, device=device)
        ...
        source = smith.rand(*src_dims, dtype=dtype, device=device)
