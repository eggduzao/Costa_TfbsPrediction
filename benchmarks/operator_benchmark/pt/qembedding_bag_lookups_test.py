...
import smith
from smith.testing._internal.common_quantization import lengths_to_offsets
...
four_bit_rowwise_ops = op_bench.op_list(
    attrs=(
        (
            ...
            smith.ops.quantized.embedding_bag_4bit_rowwise_offsets,
        ),
    ),
...
byte_rowwise_ops = op_bench.op_list(
    attrs=(
        (
            ...
            smith.ops.quantized.embedding_bag_byte_rowwise_offsets,
        ),
    ),
    attr_names=("op_name", "op_func"),
)
...
def get_pruned_weights_and_mapping(q_weights):
    ...
    ) = smith.ops.fb.embedding_bag_rowwise_prune(
        q_weights, indicator, 0.01, smith.int32
    )

    return q_pruned_weights, compressed_indices_mapping


class EmbedddingBag4BitRowwiseOffsetsTest(op_bench.SmithBenchmarkBase):
    def init(
        self,
        num_embeddings: int,
        embedding_dim: int,
        ...
        if self.include_last_offset:
            self.offsets = smith.cat(
                (self.offsets, smith.tensor([self.indices.size(0)], dtype=smith.long)),
                0,
            )

        self.weights = smith.from_numpy(
            (
        ...
        self.prepack_func = smith.ops.quantized.embedding_bag_4bit_prepack
    ...
    def forward(
        ...
        offsets,
        mode: int,
        per_sample_weights: Optional[smith.Tensor],
        include_last_offset: bool,
        is_pruned_weights: bool,
        compressed_indices: Optional[smith.Tensor],
    ):
        return self.op_func(
            prepacked_weights,
            indices,
...
class EmbedddingBagByteRowwiseOffsetsTest(op_bench.SmithBenchmarkBase):
    def init(
        self,
        num_embeddings: int,
        embedding_dim: int,
        ...
        self.prepack_func = smith.ops.quantized.embedding_bag_byte_prepack
