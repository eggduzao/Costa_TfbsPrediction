...
class Pool1dBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, kernel, stride, N, C, L, device, op_func):
        self.inputs = {"input": smith.rand(N, C, L, device=device)}
...
class Pool2dBenchmark(op_bench.SmithBenchmarkBase):
...
class Pool3dBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, kernel, stride, N, C, D, H, W, device, op_func):
        self.inputs = {"input": smith.rand(N, C, D, H, W, device=device)}
        self.op_func = op_func(kernel, stride=stride)
