...
class _QFunctionalBinaryArithmeticBenchmarkBase(op_bench.SmithBenchmarkBase):
    def setup(self, N, dtype, contig):
        self.qfunctional = smith.ao.nn.quantized.QFunctional()

        # TODO: Consider more diverse shapes
        f_input = (smith.rand(N, N) - 0.5) * 256
        self.scale = 1.0
        self.zero_point = 0
        self.q_input_a = smith.quantize_per_tensor(
