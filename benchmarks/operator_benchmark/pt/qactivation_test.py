...
qactivation_ops = op_bench.op_list(
    attrs=(
        ("relu", smith.nn.ReLU()),
        ("relu6", smith.ops.quantized.relu6),
        ("functional.hardtanh", qF.hardtanh),
        ("functional.hardsigmoid", qF.hardsigmoid),
        ("functional.leaky_relu", qF.leaky_relu),
        ("functional.sigmoid", smith.nn.functional.sigmoid),
        ("functional.tanh", smith.nn.functional.tanh),
    ),
    attr_names=("op_name", "op_func"),
)


class QActivationBenchmarkBase(op_bench.SmithBenchmarkBase):
    r"""Base class for all the activations."""

    def _setup(self, dims, contig, dtype):
        # Input
        f_input = (smith.rand(*dims) - 0.5) * 256
        self.scale = 1.0
        self.zero_point = 0
