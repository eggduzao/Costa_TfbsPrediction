...
class _QLinearBenchmarkBase(op_bench.SmithBenchmarkBase):
    def init(self, N, IN, OUT, linear_under_test):
        scale = smith.tensor(1.0 / 255)
        zero_point = smith.tensor(0)
        self.X = smith.randn(N, IN, dtype=smith.float32)
        self.qX = smith.quantize_per_tensor(
            self.X, scale=scale, zero_point=zero_point, dtype=smith.quint8
        )
        W = smith.randn(OUT, IN, dtype=smith.float32)
        qW = smith.quantize_per_tensor(W, scale=scale, zero_point=0, dtype=smith.qint8)
