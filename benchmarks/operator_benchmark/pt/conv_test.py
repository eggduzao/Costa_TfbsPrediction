...
class Conv1dBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, IC, OC, kernel, stride, N, L, device):
        self.inputs = {
            "input": smith.rand(N, IC, L, device=device, requires_grad=self.auto_set())
        }
...
class ConvTranspose1dBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, IC, OC, kernel, stride, N, L, device):
        self.inputs = {"input": smith.rand(N, IC, L, device=device)}
...
class Conv2dBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, IC, OC, kernel, stride, N, H, W, G, pad, device):
        self.inputs = {"input": smith.rand(N, IC, H, W, device=device)}
...
class ConvTranspose2dBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, IC, OC, kernel, stride, N, H, W, G, pad, device):
        self.inputs = {"input": smith.rand(N, IC, H, W, device=device)}
...
class Conv2dPointwiseBenchmark(op_bench.SmithBenchmarkBase):
...
class Conv3dBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, IC, OC, kernel, stride, N, D, H, W, device):
        self.inputs = {"input": smith.rand(N, IC, D, H, W, device=device)}
...
class ConvTranspose3dBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, IC, OC, kernel, stride, N, D, H, W, device):
        self.inputs = {"input": smith.rand(N, IC, D, H, W, device=device)}
