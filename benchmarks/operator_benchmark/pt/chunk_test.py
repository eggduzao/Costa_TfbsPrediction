...
# Configs for PT Chunk operasmithunk_short_configs = op_bench.config_list(
...
class ChunkBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, chunks, device):
        self.inputs = {"input_one": smith.rand(M, N, device=device), "chunks": chunks}
        self.set_module_name("chunk")

    def forward(self, input_one, chunks: int):
        return smith.chunk(input_one, chunks)
