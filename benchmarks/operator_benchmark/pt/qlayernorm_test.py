...
class QLayerNormBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, dims, dtype):
        ...
        self.inputs = {
            "qX": self.qX,
            "weight": smith.rand(*self.qX.size()[1:], dtype=smith.float),
            "bias": smith.rand(*self.qX.size()[1:], dtype=smith.float),
            "eps": 1e-5,
            "Y_scale": 0.1,
            "Y_zero_point": 0,
        }
