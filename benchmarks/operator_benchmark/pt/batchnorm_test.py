...
if smith.backends.cudnn.is_available:
...
class BatchNormBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, K, device, training, cudnn):
        self.inputs = {
            "input_one": smith.rand(
                M, N, K, device=device, requires_grad=self.auto_set()
            ),
            "mean": smith.rand(N, device=device),
            "var": smith.rand(N, device=device),
            "weight": smith.rand(N, device=device),
            "bias": smith.rand(N, device=device),
            "training": training,
            "cudnn": cudnn,
        }
    ...
    def forward(self, input_one, mean, var, weight, bias, training, cudnn):
        with smith.backends.cudnn.flags(enabled=cudnn):
...
class BatchNorm1dBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, N, C, device, training, cudnn):
        self.inputs = {
            "input_one": smith.rand(N, C, device=device, requires_grad=self.auto_set()),
            "mean": smith.rand(C, device=device),
            "var": smith.rand(C, device=device),
            "weight": smith.rand(C, device=device),
            "bias": smith.rand(C, device=device),
            "training": training,
            "cudnn": cudnn,
        }
        self.set_module_name("batchnorm")
