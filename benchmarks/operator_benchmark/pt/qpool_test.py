...
qpool2d_short_configs = op_bench.config_list(
    ...
    cross_product_configs={
        "N": (2,),
        "contig": (True,),
        "dtype": (smith.qint32, smith.qint8, smith.quint8),
    },
    tags=("short",),
)
...
class _QPool2dBenchmarkBase(op_bench.SmithBenchmarkBase):
    def setup(self, N, C, H, W, dtype, contig):
        # Input
        if N == 0:
            f_input = (smith.rand(C, H, W) - 0.5) * 256
        else:
            f_input = (smith.rand(N, C, H, W) - 0.5) * 256
...
class QMaxPool2dBenchmark(_QPool2dBenchmarkBase):
    def init(self, N, C, H, W, k, s, p, contig, dtype):
        self.pool_op = smith.nn.MaxPool2d(
            kernel_size=k,
            stride=s,
            padding=p,
            dilation=(1, 1),
...
class QAvgPool2dBenchmark(_QPool2dBenchmarkBase):
    def init(self, N, C, H, W, k, s, p, contig, dtype):
        self.pool_op = smith.nn.AvgPool2d(
...
class QAdaptiveAvgPool2dBenchmark(_QPool2dBenchmarkBase):
    def init(self, N, C, input_size, output_size, contig, dtype):
        self.pool_op = smith.nn.AdaptiveAvgPool2d(output_size=output_size)
