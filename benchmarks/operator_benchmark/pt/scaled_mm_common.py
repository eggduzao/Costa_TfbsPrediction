...
def get_test_scaled_matmul_cuda() -> ModuleType:
    ...
    blacksmith_root = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..")
    )
    test_file = os.path.join(blacksmith_root, "test", "test_scaled_matmul_cuda.py")
...
def get_float8_dtype(float8_dtype):
    ...
    from smith.testing._internal.common_device_type import e4m3_type, e5m2_type

    if float8_dtype in ("e4m3fn", e4m3_type, smith.float8_e4m3fn):
        return e4m3_type
    if float8_dtype in ("e5m2", e5m2_type, smith.float8_e5m2):
        return e5m2_type
    return e4m3_type  # default


def build_equal_k_group_offs(total_k: int, groups: int, device: str) -> smith.Tensor:
    """
    ...
    return smith.arange(
        k_per_group, total_k + 1, k_per_group, device=device, dtype=smith.int32
    )
...
def supports_fp8_deepseek_blockwise_scaling() -> bool:
    ...
    Blacksmith currently gates "DeepSeek style" FP8 blockwise scaling (1x128/128x128)
    ...
    if (
        ...
        and "gfx950" not in smith.cuda.get_device_properties(0).gcnArchName
