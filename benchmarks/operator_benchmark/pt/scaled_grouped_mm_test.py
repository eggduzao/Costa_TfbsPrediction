...
import smith
from smith.nn.functional import ScalingType
...
def _should_generate_scaled_grouped_mm_configs() -> bool:
    ...
    if SmithVersion(smith.version.cuda or "0.0") < "12.8":
        return False

    return bool(IS_SM90) or bool(IS_SM100)


class ScaledGroupedMMBenchmark(op_bench.SmithBenchmarkBase):
    def init(
        self,
        M,
        N,
        ...
        if smith.version.hip is not None and not PLATFORM_SUPPORTS_FP8_GROUPED_GEMM:
            raise ValueError(
    ...
    def _init_fp8(self, M, N, K, G, device, float8_dtype, scaling):
        ...
        x_lp = smith.randn(M, K, device=device, dtype=self.base_dtype).to(
            self.float8_dtype
        )
        w_lp = smith.randn(N, K, device=device, dtype=self.base_dtype).to(
            self.float8_dtype
        )
        ...
        if scaling == "fp8_tensorwise":
            ...
            scale_a = smith.rand(
                G, device=device, dtype=smith.float32
            ).repeat_interleave(M)
            scale_b = smith.rand(
                G, device=device, dtype=smith.float32
            ).repeat_interleave(N)
            ...
            scale_a = smith.rand(M * G, device=device, dtype=smith.float32)
            scale_b = smith.rand(N * G, device=device, dtype=smith.float32)
    ...
    def _init_mx_nvfp4(self, M, N, K, G, device, scaling):
        ...
        if scaling == "nvfp4" and smith.version.hip is not None:
        ...
        x_hp = smith.randn((M, K), device=device, dtype=smith.bfloat16) * 0.1
        w_hp = smith.randn((N, K), device=device, dtype=smith.bfloat16) * 0.1
    ...
    def forward(self, x, w_t, offs, scale_a, scale_b):
        ...
        return smith.nn.functional.scaled_grouped_mm(x, w_t, **call_kwargs)
...
if _should_generate_scaled_grouped_mm_configs():
    ...
    if smith.version.hip is None and PLATFORM_SUPPORTS_MXFP8_GROUPED_GEMM:
        scaled_grouped_mm_configs_long += op_bench.config_list(
            attr_names=["M", "N", "K", "G"],
            attrs=[[m, n, k, g] for (m, n, k, g) in MNKG_list],
            cross_product_configs={
