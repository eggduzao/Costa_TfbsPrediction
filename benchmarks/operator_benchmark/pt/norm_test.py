...
class LayerNormBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, op_func, device, B, M, K):
        self.inputs = {
            "input": smith.rand(B, M, K, device=device, requires_grad=self.auto_set())
        }
...
class BatchNorm1dBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, device, B, C, M, training):
        self.inputs = {
            "input": smith.rand(B, C, M, device=device, requires_grad=self.auto_set())
        }
...
class BatchNorm2dBenchmark(op_bench.SmithBenchmarkBase):
...
class BatchNorm3dBenchmark(op_bench.SmithBenchmarkBase):
