...
"""Microbenchmarks for linear_unpack_fp16_ operator. Supports both Caffe2/Blacksmith."""
...
class LinearUnpackFP16Benchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, K, device):
        # input to unpack operator must be what the output is for prepack operator
        self.inputs = {
            "input_one": smith.ops.quantized.linear_prepack_fp16(
                smith.rand(
                    M, N, K, device=device, requires_grad=False, dtype=smith.float32
                )
            )
        }
    ...
    def forward(self, input_one):
        return smith.ops.quantized.linear_unpack_fp16(input_one)
