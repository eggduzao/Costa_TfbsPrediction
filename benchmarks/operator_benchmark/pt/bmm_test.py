...
batched_binary_configs_long = op_bench.cross_product_configs(
    ...
    K=[64, 128],
    device=["cuda"],
    dtype=[smith.float32, smith.bfloat16, smith.float16],
    tags=["long"],
)


class BatchedBinaryOpBenchmark(op_bench.SmithBenchmarkBase):
...
batched_ternary_ops = op_bench.op_list(
    attr_names=["op_name", "op_func"],
    attrs=[["baddbmm", smith.baddbmm]],
)


class BatchedTernaryOpBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, B, M, N, K, device, dtype, op_func):
        self.inputs = {
            "input_": smith.rand(
