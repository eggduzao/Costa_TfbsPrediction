...
"""Microbenchmarks for boolean operators. Supports both Caffe2/Blacksmith."""
...
class AllBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, K, device):
        self.inputs = {
            "input_one": smith.randint(0, 2, (M, N, K), device=device, dtype=smith.bool)
        }
        self.set_module_name("all")

    def forward(self, input_one):
        return smith.all(input_one)
...
class AnyBenchmark(op_bench.SmithBenchmarkBase):
    def init(self, M, N, device):
        self.inputs = {
            "input_one": smith.randint(0, 2, (M, N), device=device, dtype=smith.bool)
        }
        self.set_module_name("any")

    def forward(self, input_one):
        return smith.any(input_one)
