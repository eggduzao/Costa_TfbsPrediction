...
from benchmark_blacksmith import create_blacksmith_op_test_case


def generate_pt_test(configs, pt_bench_op):
    """This function creates Blacksmith op test based on the given operator"""
    _register_test(configs, pt_bench_op, create_blacksmith_op_test_case, False)


def generate_pt_gradient_test(configs, pt_bench_op):
    """This function creates Blacksmith op test based on the given operator"""
    _register_test(configs, pt_bench_op, create_blacksmith_op_test_case, True)
...
def generate_pt_tests_from_op_list(ops_list, configs, pt_bench_op):
    ...
    """
    for op in ops_list:
        _register_test(configs, pt_bench_op, create_blacksmith_op_test_case, False, op)


def generate_pt_gradient_tests_from_op_list(ops_list, configs, pt_bench_op):
    for op in ops_list:
        _register_test(configs, pt_bench_op, create_blacksmith_op_test_case, True, op)
