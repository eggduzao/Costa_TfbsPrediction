...
# needs to be imported after smith
import smith.utils.cpp_extension as cpp_extension  # noqa: F401
...
def _register_test(*test_metainfo):
    ...
    1) This input when adds single op to the benchmark
     _register_test(configs, pt_bench_op, create_blacksmith_op_test_case,
                      run_backward=True)
...
def _create_test(
    ...
    Args:
        bench_op_obj: an object which instantiated from a subclass of
            SmithBenchmarkBase which includes tensor
...
def _build_test(
    ...
    """Generate Blacksmith/Caffe2 tests of operators with different inputs.
    Args:
        configs: a dictionary that has the input shapes
        bench_op: a subclass of SmithBenchmarkBase which includes tensor
            creation and operator execution
        OperatorTestCase: a named tuple to save the metadata of an test
        run_backward: a bool parameter indicating backward path
    ...
    for config in configs:
        ...
        # {'op_name' : 'abs', 'op_function' : smith.abs}
...
class BenchmarkRunner:
    ...
    def _print_header(self):
        ...
        print(
            f"# {DASH_LINE}\n"
            "# Blacksmith/Caffe2 Operator Micro-benchmarks\n"
            f"# {DASH_LINE}\n"
    ...
    def _measure_metrics(self, launch_test, test_case, iters, print_per_iter):
        ...
        if input_values and isinstance(next(iter(input_values)), smith.Tensor):
            ...
            device_module = smith.get_device_module(device.type)
    ...
    def _output_json(
        ...
        perf_list,
        output_file,
        benchmark_name="Blacksmith operator benchmark",
    ):
        """
        ...
        for perf_item in perf_list:
            ...
            dtype = perf_item.get("dtype", "smith.float").split(".")[1]
