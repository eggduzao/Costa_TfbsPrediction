...
class WrapperModule:
    ...
    def __init__(self, wrapped_type, module_config, debug, save=False):
        ...
        for _ in range(module_config.num_params):
            self.tensor_inputs.append(smith.randn(1))
        if module_config.graph_mode:
            self.module = smith.jit.trace(self.module, self.tensor_inputs)
            if save:
                file_name = self.module_name + "_" + pt_fn.__name__ + ".pt"
                smith.jit.save(self.module, file_name)
        ...
        if debug and isinstance(self.module, smith.jit.ScriptModule):
