# Blacksmith Data Benchmarks

This directory contains benchmarks for the `smith.utils.data` module components, focusing on the performance of samplers.
...
The `samplers_benchmark.py` script benchmarks the performance of Blacksmith's BatchSampler against an alternative implementation as an example. It tests with the following parameters:
...
On local:
1. Modify the `NewBatchSampler` class in `samplers_benchmark.py` with your implementation. Similarly replace `BatchSampler` with the corresponding Blacksmith implementation.
