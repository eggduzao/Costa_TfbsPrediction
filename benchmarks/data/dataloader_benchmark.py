#!/usr/bin/env python3
"""
Benchmark script for Blacksmith DataLoader with different worker methods.
...
import psutil
import smithvision
import smithvision.transforms as transforms
...
from smith.utils.data.dataset import ConcatDataset
...
def benchmark_dataloader(
    ...
    dataloader = DataLoader(
        ...
        pin_memory=smith.cuda.is_available(),
    ...
    device = smith.device("cuda" if smith.cuda.is_available() else "cpu")
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark Blacksmith DataLoader with different worker methods"
    )
    ...
    print(f"  Blacksmith location: {smith.__file__}")
    print(f"  Smithvision version: {smithvision.__version__}")
    print(f"  Smithvision location: {smithvision.__file__}")
    print(f"  CUDA available: {smith.cuda.is_available()}")
    if smith.cuda.is_available():
        print(f"  CUDA device: {smith.cuda.get_device_name(0)}")
    ...
    for _ in range(args.dataset_copies):
        base_dataset = smithvision.datasets.ImageFolder(
