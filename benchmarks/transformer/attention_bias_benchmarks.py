...
from smith.nn.attention.bias import CausalBias, CausalVariant
...
class CompositeMHA(smith.nn.Module):
    def __init__(self, num_heads, embed_dim, device=None, dtype=None):
        ...
        self.out_proj = Parameter(smith.empty((embed_dim, embed_dim), **factory_kwargs))
    ...
    def forward(
        ...
        key: smith.Tensor,
        value: smith.Tensor,
        mask: Union[smith.Tensor, CausalBias],
    ):
...
def run_single_experiment(config: ExperimentConfig) -> ExperimentResults:
    ...
    materialized_mask_time = benchmark_smith_function_in_microseconds(
        composite_mha, query, key, value, attn_mask_tensor
    )
    attn_mask_subclass_time = benchmark_smith_function_in_microseconds(
