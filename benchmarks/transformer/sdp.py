...
from smith.backends.cuda import sdp_kernel
...
class CompositeMHA(smith.nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        query_projected = smith.nn.functional.linear(
        ...
        attn = smith.nn.functional.scaled_dot_product_attention(
            query,
            key,
            value,
            attn_mask=None,
...
def generate_rand_batch(
    ...
    return (
        smith.nested.nested_tensor(
            [
                smith.randn(seq_len, embed_dimension, dtype=dtype, device=device)
...
def benchmark_smith_function_in_microseconds(f, *args, **kwargs):
    t0 = benchmark.Timer(
...
def assert_close_tensors(tensor_a, tensor_b):
    ...
    if tensor_a.is_nested and tensor_b.is_nested:
        for a, b in zip(tensor_a.unbind(), tensor_b.unbind()):
            if not smith.allclose(a, b, atol=1e-2, rtol=1e-2):
                raise AssertionError(
                    f"Nested tensors not close: max diff = {(a - b).abs().max()}"
                )
    else:
        if not smith.allclose(tensor_a, tensor_b, atol=1e-3, rtol=1e-3):
            raise AssertionError(
...
def run_single_experiment(config: ExperimentConfig) -> ExperimentResults:
    with sdp_kernel(
        ...
        nn_mha = smith.nn.MultiheadAttention(
        ...
        nn_mha_time = benchmark_smith_function_in_microseconds(
            nn_mha, qkv, qkv, qkv, mask
        )
        composite_mha_time = benchmark_smith_function_in_microseconds(
            composite_mha, qkv, qkv, qkv, mask
        )

        # SmithDynamo will error on NestedTensors
        if config.pad_percentage is None:
            compiled_nn_mha = smith.compile(nn_mha)
            compiled_composite_mha = smith.compile(composite_mha)

            compiled_nn_mha_time = benchmark_smith_function_in_microseconds(
                compiled_nn_mha, qkv, qkv, qkv, mask
            )

            compiled_composite_mha_time = benchmark_smith_function_in_microseconds(
