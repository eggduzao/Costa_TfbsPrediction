...
smith._dynamo.config.recompile_limit = 1000
...
def benchmark_smith_function_in_microseconds(func: Callable, *args, **kwargs) -> float:
    # warmup
    for _ in range(5):
        func(*args, **kwargs)
...
def generate_inputs(
    ...
    make_q = partial(
        smith.rand, q_shape, device=device, dtype=dtype, requires_grad=requires_grad
    )
    make_kv = partial(
        smith.rand, kv_shape, device=device, dtype=dtype, requires_grad=requires_grad
    )
...
def generate_jagged_inputs(
    ...
    def offsets_to_lengths(
        offsets: smith.Tensor, device: Union[str, smith.device]
    ) -> smith.tensor:
    ...
    q = smith.nested.as_nested_tensor(q_list, device=query.device)

    k_list = [
        flatten_k[offsets[i] : offsets[i + 1]].clone().detach().to(key.dtype)
        for i in range(len(offsets) - 1)
    ]
    k = smith.nested.as_nested_tensor(k_list, device=key.device)
    v_list = [
        flatten_v[offsets[i] : offsets[i + 1]].clone().detach().to(value.dtype)
        for i in range(len(offsets) - 1)
    ]
    v = smith.nested.as_nested_tensor(v_list, device=value.device)
    ...
    with backend_context:
        ...
        if eager_sdpa:
            ...
            if config.attn_type in ["document_mask"]:
                flatten_o_eager = smith.cat(smith.unbind(out_eager.transpose(1, 2)))
                ...
                and config.dtype in [smith.float16, smith.bfloat16]
            ):  # rel has accuracy issue with 16bit floats
                smith.testing.assert_close(out_eager, out_compile, atol=1e-2, rtol=1e-2)

        if eager_sdpa:
            forward_eager_time = benchmark_smith_function_in_microseconds(
        ...
        if config.calculate_bwd_time:
            ...
            if eager_sdpa and config.attn_type != "document_mask":
                d_out = smith.randn_like(out_eager.transpose(1, 2)).transpose(1, 2)
                backward_eager_time = benchmark_smith_function_in_microseconds(
...
def run_single_backend_FA(
    ...
    if FA:
        forward_FA_time = benchmark_smith_function_in_microseconds(
    ...
    if config.calculate_bwd_time:
        if FA:
            d_out = smith.randn_like(out_FA)
            backward_FA_time = benchmark_smith_function_in_microseconds(
...
def run_single_experiment(
    ...
    if config.max_autotune:
        ...
        compiled_sdpa = smith.compile(flex_attention, dynamic=dynamic)
    ...
    forward_compiled_time = benchmark_smith_function_in_microseconds(
        compiled_sdpa,
        query,
        key,
        value,
    ...
    if config.calculate_bwd_time:
        d_out = smith.randn_like(out_compile)
        backward_compile_time = benchmark_smith_function_in_microseconds(
...
def generate_score_mod(attn_type: str, shape: tuple[int, ...]) -> Callable | None:
    ...
    if is_decoding and score_mod:
        offset = smith.tensor(N // 2).to("cuda")
...
def generate_block_mask(attn_type: str, shape: tuple[int, ...]):
    ...
    if is_decoding and mask_mod:
        cached_seq_len = smith.tensor(N // 2).to("cuda")
    ...
    compiled_block_mask = smith.compile(create_block_mask)
    if new_mask_mod:
...
def get_kernel_options(attn_type: str, shape: tuple[int, ...]):
    ...
    kernel_opt_training_dict = {
        ...
        if smith.cuda.get_device_capability() >= (8, 0) and D <= 128
        else None,
        "prefix_lm": None,
        "softcap": None,
    }
    ...
    def get_default_split_k(B: int, H: int, Mk: int) -> int:
        num_SM = smith.cuda.get_device_properties("cuda").multi_processor_count
...
def generate_FA_callable(
    attn_type: str, shape: tuple[int, ...], dtype: smith.dtype, backend: str, **kwargs
) -> Callable | None:
    if dtype not in [smith.float16, smith.bfloat16]:
        return None
    if backend == "fav3":
        try:
    ...
    FA_kwargs = {}
    if attn_type == "alibi":
        h = smith.arange(Hq, dtype=smith.float32, device="cuda")
        alibi_slopes = smith.exp2(-((h + 1) * 8.0 / Hq))
        FA_kwargs = dict(alibi_slopes=alibi_slopes)
    elif attn_type == "document_mask":
        FA_kwargs["cu_seqlens_q"] = kwargs["offsets"].to(smith.int32)
        FA_kwargs["cu_seqlens_k"] = kwargs["offsets"].to(smith.int32)
...
def generate_FD_callable(
    attn_type: str, shape: tuple[int, ...], dtype: smith.dtype
) -> Callable | None:
    if dtype not in [smith.float16, smith.bfloat16]:
        return None
    try:
...
def generate_attn_mask_linear_score_mod(
    ...
    if block_mask is None and score_mod is None:
        return None
    b = smith.arange(B, dtype=int, device="cuda")
    h = smith.arange(Hq, dtype=int, device="cuda")
    m = smith.arange(M, dtype=int, device="cuda")
    n = smith.arange(N, dtype=int, device="cuda")

    score = smith.zeros(B, Hq, M, N, dtype=dtype, device="cuda")
    bias = score_mod(
        score,
        b[:, None, None, None],
        h[None, :, None, None],
...
def generate_experiment_configs(
    ...
    for (
        ...
        if kv_cache_size:
            head_size_bytes = smith.finfo(dtype).bits / 8 * head_dim
            bsz = int(
...
def _output_json_for_dashboard(
    experiments,
    output_file,
    benchmark_name="Blacksmith operator microbenchmark",
):
    """
    Write the result into JSON format for Blacksmith OSS dashboard.
...
def main(
    ...
    benchmark_name: str = "Blacksmith operator microbenchmark",
) -> None:
    ...
    Args:
        ...
        output_json_for_dashboard: Path to save results in JSON format for Blacksmith OSS dashboard
        benchmark_name: Name of the benchmark for dashboard output
    """
    # Convert dtype string to smith dtype (if not already converted)
    import smith

    if isinstance(dtype, str):
        dtype = getattr(smith, dtype)
...
""",
    ...
    parser.add_argument(
        ...
        type=str,
        help="Path to save results in JSON format for Blacksmith OSS dashboard",
        default=None,
    )
    parser.add_argument(
        "--benchmark-name",
        ...
        default="Blacksmith operator microbenchmark",
    )
    parser.add_argument(
        "--print-config",
        type=str,
    ...
    # Convert dtype string to smith dtype (only if it's still a string)
    if isinstance(args.dtype, str):
        args.dtype = getattr(smith, args.dtype)
