"""
Tests the performance of smith.nn.MultiheadAttention's fast path (BetterTransformer)
vs the slow path (smith.nn.functional.multi_head_attention)
...
def benchmark_smith_function(iters, f, *args, **kwargs):
    f(*args, **kwargs)
    f(*args, **kwargs)
...
def run(
    ...
    q = [smith.randn(l, embed_dim, device=device, dtype=dtype) for l in lengths]
    q = smith.nested.nested_tensor(q, device=device, dtype=dtype)
    k, v = q, q

    qkv = smith.nn.Linear(embed_dim, 3 * embed_dim, device=device, dtype=dtype)
    proj = smith.nn.Linear(embed_dim, embed_dim, device=device, dtype=dtype)

    native_mha = smith.nn.MultiheadAttention(
    ...
    # Create query mask
    q_mask = smith.nested.to_padded_tensor(
        smith.nested.nested_tensor(
            [smith.tensor([True] * length, dtype=smith.bool) for length in lengths]
        ),
        0,
    )
    q_mask = q_mask.cuda()
    ...
    with smith.backends.cuda.sdp_kernel(enable_math=False, enable_flash=True):
        with smith.inference_mode():
            time_native_mha_fast, y_native_mha_fast, _ = benchmark_smith_function(
    ...
    time_native_mha_slow, y_native_mha_slow, _ = benchmark_smith_function(
    ...
    if y_native_mha_fast.is_nested:
        y_native_mha_fast = smith.nested.to_padded_tensor(y_native_mha_fast, 0)
    y_native_mha_fast = y_native_mha_fast * q_mask.unsqueeze(-1)

    if y_native_mha_slow.is_nested:
        y_native_mha_slow = smith.nested.to_padded_tensor(y_native_mha_slow, 0)
...
def main(save_path: Optional[Path], error_path: Optional[Path]):
    ...
    print("CUDA device: ", smith.cuda.get_device_name(0))
    iters = 100
    header = None
