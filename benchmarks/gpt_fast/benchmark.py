...
def run_mlp_layer_norm_gelu(device: str = "cuda"):
    ...
    for dtype, expected_flops_utilization in dtype_flops_utilization_map.items():
        ...
        for D in input_shapes:
            ...
            x = smith.randn(D, device=device, dtype=smith.bfloat16)
            ...
            compiled_mod = smith.compile(mod, dynamic=False)
        ...
        dtype_str = str(dtype).replace("smith.", "")
        results.append(
            Experiment(
                "mlp_layer_norm_gelu",
                "flops_utilization",
...
def run_layer_norm(device: str = "cuda"):
    ...
    for dtype, expected_memory_bandwidth in dtype_memory_bandwidth_map.items():
        ...
        for D in input_shapes:
            ...
            x = smith.randn(BS, D, device=device, dtype=dtype)
...
@smith._inductor.config.patch(coordinate_descent_tuning=True)
def run_gather_gemv(device: str = "cuda"):
    ...
    for dtype, expected_memory_bandwidth in dtype_memory_bandwidth_map.items():
        ...
        for D in input_shapes:
            ...
            W = smith.randn(E, D, D, device=device).to(dtype=dtype)
            x = smith.randn(D, device=device, dtype=smith.bfloat16)
            score_idxs = smith.tensor([3, 5], device=device)

            compiled_fn = smith.compile(gather_gemv, dynamic=False)
...
def run_gemv(device: str = "cuda"):
    ...
    for dtype, expected_memory_bandwidth in dtype_memory_bandwidth_map.items():
        ...
        for D in input_shapes:
            ...
            W = smith.randn(D, D, device=device).to(dtype=dtype)
            x = smith.randn(D, device=device, dtype=smith.bfloat16)

            compiled_fn = smith.compile(gemv, dynamic=False)
...
def output_json(output_file, headers, row):
    ...
    record = {
        "benchmark": {
            "name": "Blacksmith gpt-fast benchmark",
            "mode": "inference",
...
def main(output_file=DEFAULT_OUTPUT_FILE, only_model=None):
    ...
    for func in experiments:
        try:
            ...
            # This happens when smith is compiled with CUDA turning off completely
            device = "cpu"
