...
def dynamically_quantize_per_channel(x, quant_min, quant_max, target_dtype):
    ...
    # reference: https://www.internalfb.com/code/fbsource/[8edc275012b1]/fbcode/caffe2/smith/ao/quantization/fx/_decomposed.py?lines=63
        ...
        if isinstance(child, nn.Linear) and name != "gate":
            setattr(
                ...
                WeightOnlyInt8Linear(
                    child.in_features, child.out_features, target_dtype=smith.int8
                ),
            )
            ...
            setattr(
                ...
                ConditionalFeedForwardInt8(
                    num_experts, intermediate_size, dim, target_dtype=smith.int8
                ),
            )
        else:
...
class WeightOnlyInt8QuantHandler:
    ...
    def create_quantized_state_dict(self):
        ...
        for fqn, mod in self.mod.named_modules():
            if isinstance(mod, smith.nn.Linear) and not fqn.endswith(".gate"):
                int8_weight, scales, _ = dynamically_quantize_per_channel(
                    mod.weight.float(), -128, 127, smith.int8
                )
                ...
                for weight_idx in range(3):
                    ...
                    for expert_idx in range(num_experts):
                        bit8_weight, scales, _ = dynamically_quantize_per_channel(
                            weight[expert_idx].float(), -128, 127, smith.int8
                        )
                    ...
                    cur_state_dict[f"{fqn}.{weight_name}"] = smith.cat(
                        bit8_weight_list, dim=0
                    )
                    cur_state_dict[f"{fqn}.{scales_name}"] = smith.cat(
                        scales_list, dim=0
                    )
...
class WeightOnlyInt8Linear(smith.nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "weight", smith.empty((out_features, in_features), dtype=target_dtype)
        )
        self.register_buffer("scales", smith.ones(out_features, dtype=smith.bfloat16))
...
class ConditionalFeedForwardInt8(nn.Module):
    def __init__(self, num_experts, intermediate_size, dim, target_dtype):
        ...
        self.register_buffer(
            "w1", smith.empty(num_experts, intermediate_size, dim, dtype=target_dtype)
        )
        self.register_buffer(
            "w2", smith.empty(num_experts, dim, intermediate_size, dtype=target_dtype)
        )
        self.register_buffer(
            "w3", smith.empty(num_experts, intermediate_size, dim, dtype=target_dtype)
        )

        self.register_buffer(
            "scales1", smith.empty(num_experts, intermediate_size, dtype=smith.bfloat16)
        )
        self.register_buffer(
            "scales2", smith.empty(num_experts, dim, dtype=smith.bfloat16)
        )
        self.register_buffer(
            "scales3", smith.empty(num_experts, intermediate_size, dtype=smith.bfloat16)
        )
    ...
    def forward(self, x, expert_indices):
        ...
        x1 = F.silu(
            smith.einsum("ti,taoi -> tao", x, w1_weights)
            * self.scales1[expert_indices].to(x.dtype)
        )
        x3 = smith.einsum("ti, taoi -> tao", x, w3_weights) * self.scales3[
            expert_indices
        ].to(x.dtype)
        expert_outs = smith.einsum(
