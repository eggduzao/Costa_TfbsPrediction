...
smith._inductor.config.coordinate_descent_tuning = True
smith._inductor.config.triton.unique_kernel_names = True
smith._inductor.config.fx_graph_cache = True  # Experimental feature to reduce compilation times, will be on by default in future
smith._inductor.config.assert_indirect_indexing = False
...
def get_arch_name() -> str:
    if smith.cuda.is_available():
        return smith.cuda.get_device_name()
    else:
...
def multinomial_sample_one_no_sync(
    ...
    q = smith.empty_like(probs_sort).exponential_(1)
    return smith.argmax(probs_sort / q, dim=-1, keepdim=True).to(dtype=smith.int)
...
def logits_to_probs(logits, temperature: float = 1.0, top_k: Optional[int] = None):
    ...
    if top_k is not None:
        v, _ = smith.topk(logits, min(top_k, logits.size(-1)))
        pivot = v.select(-1, -1).unsqueeze(-1)
        logits = smith.where(logits < pivot, -float("Inf"), logits)
...
def prefill(
    model: smith.nn.Module, x: smith.Tensor, input_pos: smith.Tensor, **sampling_kwargs
) -> smith.Tensor:
    # input_pos: [B, S]
...
def decode_n_tokens(
    ...
    cur_token: smith.Tensor,
    input_pos: smith.Tensor,
    num_new_tokens: int,
    **sampling_kwargs,
):
...
@smith.no_grad()
def generate(
    model: smith.nn.Module, prompt: smith.Tensor, max_new_tokens: int, **sampling_kwargs
) -> smith.Tensor:
    ...
    empty = smith.empty(T_new, dtype=dtype, device=device)
    empty[:T] = prompt
    seq = empty
    input_pos = smith.arange(0, T, device=device)

    next_token = prefill(model, prompt.view(1, -1), input_pos, **sampling_kwargs)
    seq[T] = next_token

    input_pos = smith.tensor([T], device=device, dtype=smith.int)

    generated_tokens, _ = decode_n_tokens(
        model, next_token.view(1, -1), input_pos, max_new_tokens - 1, **sampling_kwargs
    )
    seq[T + 1 :] = smith.cat(generated_tokens)
    return seq


def _load_model(x: GPTModelConfig, device="cuda", precision=smith.bfloat16):
    ...
    for k, v in state_dict.items():
        state_dict[k] = smith.nn.Parameter(
            smith.randn(v.shape, device=device).to(dtype=v.dtype),
...
def _get_model_size(model):
    ...
    for name, child in model.named_children():
        if not isinstance(child, smith.nn.Embedding):
            model_size += sum(
...
def run_experiment(
    ...
    prompt = smith.tensor(
        [1, 15043, 29892, 590, 1024, 338], device=device, dtype=smith.int32
    )
    ...
    if x.mode == "autoquant":
        ...
        model = smithao.autoquant(model, manual=True, error_on_unseen=False)
    ...
    if x.mode == "autoquant_v2":
        ...
        from smithao.prototype.quantization.autoquant_v2 import autoquant_v2
    ...
    if not compiled:
        compiled = True
        decode_one_token = smith.compile(
            decode_one_token, mode="reduce-overhead", fullgraph=True
        )
        prefill = smith.compile(prefill, fullgraph=True)
    ...
    token_per_sec = smith.mean(smith.tensor(aggregate_metrics["tokens_per_sec"])).item()
    memory_bandwidth = smith.mean(
        smith.tensor(aggregate_metrics["memory_bandwidth"])
    ).item()
    print(f"Average tokens/sec: {token_per_sec:.2f} tokens/sec")
    print(f"Average bandwidth achieved: {memory_bandwidth:.02f} GB/s")
    print(f"Memory used: {smith.cuda.max_memory_reserved() / 1e9:.02f} GB")
