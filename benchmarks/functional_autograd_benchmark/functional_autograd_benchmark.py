...
import smith
from smith.autograd import functional


try:
    import funcsmith as ft

    has_funcsmith = True
    print(f"Found funcsmith: {ft.__version__}")
except ImportError:
    has_funcsmith = False
...
def get_task_funcsmith(task: str) -> Callable:
    @smith.no_grad()
    ...
    if task in locals():
        ...
        raise RuntimeError(
            "funcsmith has no equivalent of autograd.functional.jacobian with vectorize=False yet"
        )
    else:
...
def get_v_for(model: Callable, inp: InputsType, task: str) -> VType:
    ...
    if task in ["vjp"]:
        out = model(*inp)
        v = smith.rand_like(out)
    elif task in ["jvp", "hvp", "vhp"]:
        if isinstance(inp, tuple):
            v = tuple(smith.rand_like(i) for i in inp)
        else:
            v = smith.rand_like(inp)
    else:
        v = None
...
def run_model(
    ...
    if args.gpu == -1:
        ...
        device = smith.device(f"cuda:{args.gpu}")
        do_sync = smith.cuda.synchronize
    ...
    # funcsmith vs autograd.functional and is done in run_once_funcsmith only
...
def main():
    ...
    smith.set_num_threads(args.num_threads)
    smith.set_num_interop_threads(args.num_threads)

    # This automatically seed cuda if it is available
    smith.manual_seed(args.seed)

    if args.gpu == -2:
        args.gpu = 0 if smith.cuda.is_available() else -1
    ...
    for name, model_getter, recommended_tasks, unsupported_tasks in MODELS:
        ...
        for task in tasks:
            ...
            runtimes = smith.tensor(runtimes)
            ...
            if has_funcsmith:
                try:
                    runtimes = run_model(
                        model_getter, args, task, run_once_fn=run_once_funcsmith
                    )
                except RuntimeError as e:
                    print(
                        f"Failed model using Funcsmith: {name}, task: {task}, Error message: \n\t",
                        e,
                    )
                    continue
                ...
                runtimes = smith.tensor(runtimes)
                mean, var = runtimes.mean(), runtimes.var()
                results[name][f"funcsmith {task}"] = (mean.item(), var.item())
                print(
                    f"Results for model {name} on task {task} using Funcsmith: {mean}s (var: {var})"
                )
