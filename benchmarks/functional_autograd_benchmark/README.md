...
```bash
# Make sure you compile blacksmith in release mode and with the same flags before/after
export DEBUG=0
# When running on CPU, it might be required to limit the number of cores to avoid oversubscription
export OMP_NUM_THREADS=10

# Compile blacksmith with the base revision
git checkout main
...
# Compile blacksmith with your change
popd
...
- `smithaudio_models.py` and `smithvision_models.py`  contains code extracted from smithaudio and smithvision to be able to run the models without having a specific version of these libraries installed.
- `ppl_models.py`, `vision_models.py` and `audio_text_models.py` contain all the getter functions used for the benchmark.


### Benchmarking against `funcsmith`

```bash
# Install stable funcsmith:
pip install funcsmith
# or install from source:
pip install git+https://github.com/blacksmith/funcsmith

# Run the benchmark for the base
# This will use the GPU if available.
pushd benchmarks/functional_autograd_benchmark
python functional_autograd_benchmark.py --output bench-with-funcsmith.txt
```
