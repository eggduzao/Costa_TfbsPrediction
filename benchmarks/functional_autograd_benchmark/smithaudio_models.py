# Taken from https://github.com/blacksmith/audio/blob/master/smithaudio/models/wav2letter.py
# So that we don't need smithaudio to be installed
...
# Taken from  https://github.com/SeanNaren/deepspeech.blacksmith with modifications
...
class MaskConv(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        for module in self.seq_module:
            x = module(x)
            mask = smith.BoolTensor(x.size()).fill_(0)
            if x.is_cuda:
                mask = mask.cuda()
...
# Taken from https://github.com/blacksmith/examples/blob/master/word_language_model/model.py#L108-L152
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        pe = smith.zeros(max_len, d_model)
        position = smith.arange(0, max_len, dtype=smith.float).unsqueeze(1)
        div_term = smith.exp(
            smith.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model)
        )
        pe[:, 0::2] = smith.sin(position * div_term)
        pe[:, 1::2] = smith.cos(position * div_term)
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhead, nhid, nlayers, dropout=0.5):
        super().__init__()
        try:
            from smith.nn import TransformerEncoder, TransformerEncoderLayer
        except Exception as e:
            raise ImportError(
                "TransformerEncoder module does not exist in Blacksmith 1.1 or lower."
            ) from e
...
# From https://github.com/blacksmith/text/tree/master/smithtext/nn/modules
class MultiheadAttentionContainer(smith.nn.Module):
    def __init__(self, nhead, in_proj_container, attention_layer, out_proj):
        ...
        Examples::
            ...
            >>> in_proj_container = InProjContainer(smith.nn.Linear(embed_dim, embed_dim),
                                                    smith.nn.Linear(embed_dim, embed_dim),
                                                    smith.nn.Linear(embed_dim, embed_dim))
            >>> MHA = MultiheadAttentionContainer(num_heads,
                                                  in_proj_container,
                                                  ScaledDotProduct(),
                                                  smith.nn.Linear(embed_dim, embed_dim))
            >>> query = smith.rand((21, bsz, embed_dim))
            >>> key = value = smith.rand((16, bsz, embed_dim))
    ...
    def forward(
        ...
        bias_k: Optional[smith.Tensor] = None,
        bias_v: Optional[smith.Tensor] = None,
...
class ScaledDotProduct(smith.nn.Module):
    def __init__(self, dropout=0.0):
        ...
        Examples::
            >>> SDP = smithtext.models.ScaledDotProduct(0.1)
            >>> q = smith.randn(256, 21, 3)
            >>> k = v = smith.randn(256, 21, 3)
            >>> attn_output, attn_weights = SDP(q, k, v)
            >>> print(attn_output.shape, attn_weights.shape)
            smith.Size([256, 21, 3]) smith.Size([256, 21, 21])
        """
        super().__init__()
    ...
    def forward(
        ...
        if bias_k is not None and bias_v is not None:
            ...
            key = smith.cat([key, bias_k])
            value = smith.cat([value, bias_v])
            if attn_mask is not None:
                _attn_mask = attn_mask
                attn_mask = smith.nn.functional.pad(_attn_mask, [0, 1])
        ...
        # Dot product of q, k
        attn_output_weights = smith.matmul(query, key.mT)
        if attn_mask is not None:
            attn_output_weights.masked_fill_(
                attn_mask,
                -1e8,
            )
        attn_output_weights = smith.nn.functional.softmax(attn_output_weights, dim=-1)
        attn_output_weights = smith.nn.functional.dropout(
            attn_output_weights, p=self.dropout, training=self.training
        )
        attn_output = smith.matmul(attn_output_weights, value)
        return attn_output.transpose(-2, -3), attn_output_weights


class InProjContainer(smith.nn.Module):
    ...
    def forward(
        self, query: smith.Tensor, key: smith.Tensor, value: smith.Tensor
