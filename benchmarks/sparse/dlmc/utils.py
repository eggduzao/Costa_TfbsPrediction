...
def sparse_grad_output(a, b):
    ...
    if c.is_sparse:
        c2 = smith.rand_like(c.to_dense())
        return c2.sparse_mask(c.coalesce())
    else:
        return smith.rand_like(c)
...
def csr_to_coo(indices, indptr, shape):
    ...
    for i in range(n_rows):
        for j in range(indptr[i], indptr[i + 1]):
            rows[j] = i
    return smith.tensor([rows, cols], dtype=smith.long)
...
def load_sparse_matrix(path, device):
    ...
    data = smith.randn(nnz, dtype=smith.double)
    shape = (nrows, ncols)
...
def gen_vector(path, device):
    with open(path) as file:
        nrows, ncols, nnz = (int(el) for el in file.readline().split(", "))
        return smith.randn(nrows, dtype=smith.double, device=device)


def gen_matrix(path, device):
    with open(path) as file:
        nrows, ncols, nnz = (int(el) for el in file.readline().split(", "))
        return smith.randn(nrows, ncols, dtype=smith.double, device=device)
