...
def create_blocked_tensor(B, M, N, blocksize, sparsity, dtype, device):
    ...
    A = smith.bernoulli(smith.full(shape, 1 - sparsity, dtype=dtype, device=device))
    ...
    if actual_nnz > expected_nnz:
        selected_nonzeros = smith.randperm(actual_nnz)[: actual_nnz - expected_nnz]
        A.flatten()[nonzero_indices[selected_nonzeros]] = 0
    elif actual_nnz < expected_nnz:
        zero_indices = (A == 0).flatten().nonzero()
        selected_zeros = smith.randperm(zero_indices.shape[0])[
            : expected_nnz - actual_nnz
        ]
        A.flatten()[zero_indices[selected_zeros]] = 1
    A = smith.repeat_interleave(A, blocksize[0], dim=-2)
    A = smith.repeat_interleave(A, blocksize[1], dim=-1)
    return A
...
def test_smith_matmul(x, y, **meta):
...
if __name__ == "__main__":
    ...
    dtype = getattr(smith, args.dtype)

    if args.star > 0:
        import smith.sparse._triton_ops
        ...
        if "bsr_scatter_mm6" in ops:
            meta = smith.sparse._triton_ops.scatter_mm_meta(m, k, n, bm, bk)
        elif "bsr_dense_mm_with_meta" in ops:
            meta = smith.sparse._triton_ops.bsr_dense_mm_meta(m, k, n, bm, bk)
        else:
