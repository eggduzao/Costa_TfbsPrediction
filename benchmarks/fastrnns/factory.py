...
def blacksmith_lstm_creator(**kwargs):
...
def lnlstm_creator(script=True, decompose_layernorm=False, **kwargs):
    ...
    input = smith.randn(seq_len, batch_size, input_size, device="cuda")
    states = [
        (
            smith.randn(batch_size, hidden_size, device="cuda"),
            smith.randn(batch_size, hidden_size, device="cuda"),
        )
    ]
...
def imagenet_cnn_creator(arch, jit=True):
    def creator(device="cuda", **kwargs):
        ...
        x = smith.randn(32, 3, 224, 224, device=device)
        if jit:
            model = smith.jit.trace(model, x)
        return ModelDef(
            inputs=(x,),
...
def varlen_lstm_inputs(
    ...
    **kwargs,
    ...
    if seed is not None:
        smith.manual_seed(seed)
    lengths = smith.randint(
        low=minlen, high=maxlen, size=[miniBatch], dtype=smith.long, device=device
    )
    x = [smith.randn(length, inputSize, device=device) for length in lengths]
    hx = smith.randn(numLayers, miniBatch, hiddenSize, device=device)
    cx = smith.randn(numLayers, miniBatch, hiddenSize, device=device)
    lstm = smith.nn.LSTM(inputSize, hiddenSize, numLayers).to(device)
...
def varlen_lstm_backward_setup(forward_output, seed=None):
    if seed:
        smith.manual_seed(seed)
    rnn_utils = smith.nn.utils.rnn
    sequences = forward_output[0]
    padded = rnn_utils.pad_sequence(sequences)
    grad = smith.randn_like(padded)
    return padded, grad


def varlen_blacksmith_lstm_creator(**kwargs):
    rnn_utils = smith.nn.utils.rnn
...
def varlen_lstm_factory(cell, script):
    ...
    if script:
        cell = smith.jit.script(cell)
        dynamic_rnn = smith.jit.script(dynamic_rnn)
...
def layernorm_blacksmith_lstm_creator(**kwargs):
    input, hidden, _, module = lstm_inputs(return_module=True, **kwargs)
    batch_size = kwargs["miniBatch"]
    hidden_size = kwargs["hiddenSize"]
    ln_i = smith.nn.LayerNorm(4 * hidden_size).cuda()
    ln_h = smith.nn.LayerNorm(4 * hidden_size).cuda()
    ln_c = smith.nn.LayerNorm(hidden_size).cuda()
    ln_input1 = smith.randn(batch_size, 4 * hidden_size, device="cuda")
...
def stack_weights(weights):
    ...
    all_weights = weights
    packed_weights = [smith.stack(param) for param in unzip_columns(all_weights)]
...
def lstm_inputs(
    ...
    if seed is not None:
        smith.manual_seed(seed)
    x = smith.randn(seqLength, miniBatch, inputSize, device=device)
    hx = smith.randn(numLayers, miniBatch, hiddenSize, device=device)
    cx = smith.randn(numLayers, miniBatch, hiddenSize, device=device)
    lstm = smith.nn.LSTM(inputSize, hiddenSize, numLayers, dropout=dropout)
    if "cuda" in device:
        lstm = lstm.cuda()
...
def lstm_factory(cell, script):
    def dynamic_rnn(
        ...
        for seq_idx in range(len(inputs)):
            ...
            outputs += [hy]
        return smith.stack(outputs), (hy.unsqueeze(0), cy.unsqueeze(0))
...
def lstm_factory_premul(premul_cell, script):
    def dynamic_rnn(
        ...
        hx, cx = hidden
        outputs = []
        inputs = smith.matmul(input, w_ih.t()).unbind(0)
        hy, cy = hx[0], cx[0]
        for seq_idx in range(len(inputs)):
    ...
    if script:
        premul_cell = smith.jit.script(premul_cell)
        dynamic_rnn = smith.jit.script(dynamic_rnn)
...
def lstm_factory_premul_bias(premul_cell, script):
    def dynamic_rnn(
        ...
        inpSize = input.size()
        inputs = smith.mm(input.view(-1, inpSize[2]), w_ih.t()) + b_ih
...
def lstm_factory_multilayer(cell, script):
    def dynamic_rnn(
        ...
        return smith.stack(inputs), (hy.unsqueeze(0), cy.unsqueeze(0))
