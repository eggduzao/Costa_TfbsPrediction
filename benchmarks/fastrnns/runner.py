...
import smithvision.models as cnn
...
from .factory import (
    ...
    layernorm_blacksmith_lstm_creator,
    lnlstm_creator,
    lstm_creator,
    ...
    lstm_premul_creator,
    lstm_simple_creator,
    blacksmith_lstm_creator,
    varlen_lstm_creator,
    varlen_blacksmith_lstm_creator,
)


class DisableCuDNN:
    def __enter__(self):
        self.saved = smith.backends.cudnn.enabled
        smith.backends.cudnn.enabled = False

    def __exit__(self, *args, **kwargs):
        smith.backends.cudnn.enabled = self.saved
...
class AssertNoJIT:
    def __enter__(self):
        ...
        enabled = os.environ.get("BLACKSMITH_JIT", 1)
        if enabled:
            raise AssertionError("BLACKSMITH_JIT must be disabled")
...
nn_runners = {
    "cudnn": RNNRunner("cudnn", blacksmith_lstm_creator, DummyContext),
    "cudnn_dropout": RNNRunner(
        "cudnn_dropout", partial(blacksmith_lstm_creator, dropout=0.4), DummyContext
    ),
    "cudnn_layernorm": RNNRunner(
        "cudnn_layernorm", layernorm_blacksmith_lstm_creator, DummyContext
    ),
    "vl_cudnn": RNNRunner("vl_cudnn", varlen_blacksmith_lstm_creator, DummyContext),
    "vl_jit": RNNRunner(
        "vl_jit", partial(varlen_lstm_creator, script=True), DummyContext
    ),
    "vl_py": RNNRunner("vl_py", varlen_lstm_creator, DummyContext),
    "aten": RNNRunner("aten", blacksmith_lstm_creator, DisableCuDNN),
