...
# shim for smith.cuda.Event when running on cpu
class Event:
...
def trainbench(
    ...
    fwd_times = smith.tensor(fwd_times)
    bwd_times = smith.tensor(bwd_times)
    return BenchResult(
        name=name,
...
if __name__ == "__main__":
    ...
    if args.cuda_pointwise_loop_level:
        smith._C._jit_set_te_cuda_pointwise_loop_levels(args.cuda_pointwise_loop_level)
    if args.cuda_pointwise_block_count:
        smith._C._jit_set_te_cuda_pointwise_block_count(args.cuda_pointwise_block_count)
    if args.cuda_pointwise_block_size:
        smith._C._jit_set_te_cuda_pointwise_block_size(args.cuda_pointwise_block_size)
