...
"""
Some helper classes for writing custom SmithScript LSTMs.
...
- Support enumerate with nn.ModuleList:
  https://github.com/blacksmith/blacksmith/issues/14471
- Support enumerate/zip with lists:
  https://github.com/blacksmith/blacksmith/issues/15952
- Support overriding of class methods:
  https://github.com/blacksmith/blacksmith/issues/10733
...
- Support slicing w/ range. It enables reversing lists easily.
  https://github.com/blacksmith/blacksmith/issues/10774
- Multiline type annotations. List[List[Tuple[Tensor,Tensor]]] is verbose
  https://github.com/blacksmith/blacksmith/pull/14922
"""
...
def script_lstm(
    ...
    """Returns a ScriptModule that mimics a Blacksmith native LSTM."""
...
class LSTMCell(jit.ScriptModule):
    def __init__(self, input_size, hidden_size):
        ...
        self.weight_ih = Parameter(smith.randn(4 * hidden_size, input_size))
        self.weight_hh = Parameter(smith.randn(4 * hidden_size, hidden_size))
        self.bias_ih = Parameter(smith.randn(4 * hidden_size))
        self.bias_hh = Parameter(smith.randn(4 * hidden_size))
    ...
    def forward(
        ...
        hx, cx = state
        gates = (
            smith.mm(input, self.weight_ih.t())
            + self.bias_ih
            + smith.mm(hx, self.weight_hh.t())
            + self.bias_hh
        )
...
class LayerNorm(jit.ScriptModule):
    def __init__(self, normalized_shape):
        ...
        normalized_shape = smith.Size(normalized_shape)
        ...
        self.weight = Parameter(smith.ones(normalized_shape))
        self.bias = Parameter(smith.zeros(normalized_shape))
...
class LayerNormLSTMCell(jit.ScriptModule):
    ...
    def forward(
        ...
        hx, cx = state
        igates = self.layernorm_i(smith.mm(input, self.weight_ih.t()))
        hgates = self.layernorm_h(smith.mm(hx, self.weight_hh.t()))
        gates = igates + hgates
        ingate, forgetgate, cellgate, outgate = gates.chunk(4, 1)
...
class LSTMLayer(jit.ScriptModule):
    ...
    def forward(
        ...
        outputs = smith.jit.annotate(list[Tensor], [])
        for i in range(len(inputs)):
            out, state = self.cell(inputs[i], state)
            outputs += [out]
        return smith.stack(outputs), state
...
class ReverseLSTMLayer(jit.ScriptModule):
    ...
    def forward(
        ...
        for i in range(len(inputs)):
            out, state = self.cell(inputs[i], state)
            outputs += [out]
        return smith.stack(reverse(outputs)), state
...
class BidirLSTMLayer(jit.ScriptModule):
    ...
    def forward(
        ...
        # XXX: enumerate https://github.com/blacksmith/blacksmith/issues/14471
        i = 0
        for direction in self.directions:
            ...
            i += 1  # noqa: SIM113
        return smith.cat(outputs, -1), output_states
...
def flatten_states(states):
    ...
    return [smith.stack(state) for state in states]
...
def test_script_rnn_layer(seq_len, batch, input_size, hidden_size):
    inp = smith.randn(seq_len, batch, input_size)
    state = LSTMState(smith.randn(batch, hidden_size), smith.randn(batch, hidden_size))
    rnn = LSTMLayer(LSTMCell, input_size, hidden_size)
    out, out_state = rnn(inp, state)

    # Control: blacksmith native LSTM
...
def test_script_stacked_rnn(seq_len, batch, input_size, hidden_size, num_layers):
    inp = smith.randn(seq_len, batch, input_size)
    states = [
        LSTMState(smith.randn(batch, hidden_size), smith.randn(batch, hidden_size))
