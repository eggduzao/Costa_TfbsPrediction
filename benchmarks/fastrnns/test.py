...
from .factory import blacksmith_lstm_creator, varlen_blacksmith_lstm_creator
...
def test_vl_py(**test_args):
    ...
    control_creator = varlen_blacksmith_lstm_creator
    ...
    with context():
        ...
        assertEqual(smith.cat(experim_hx, dim=1), control_hx)
        assertEqual(smith.cat(experim_cx, dim=1), control_cx)
...
if __name__ == "__main__":
    ...
    for name, creator, context in rnn_runners:
        with context():
            print(f"testing {name}...")
            test_rnns(creator, blacksmith_lstm_creator, **test_args)
