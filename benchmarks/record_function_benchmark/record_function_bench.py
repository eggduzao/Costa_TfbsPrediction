...
def prepare_resnet50_jit(bench_args):
    model = resnet50()
    inputs = (smith.randn(32, 3, 224, 224),)
    model = smith.jit.trace(model, inputs)
    return inputs, model
    ...
    for model_name in model_names:
        ...
        for num_threads in NUM_THREADS:
            for with_rec_fn in [True, False]:
                smith.autograd._enable_record_function(with_rec_fn)
                smith.autograd._clear_callbacks()
                if with_rec_fn:
                    smith.autograd._set_empty_test_observer(True, 0.0001)
