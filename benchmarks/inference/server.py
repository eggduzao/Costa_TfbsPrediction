...
class FrontendWorker(mp.Process):
    ...
    def _run_gpu_utilization(self, metrics_lock):
        ...
        with metrics_lock:
            self.metrics_dict["gpu_util"] = smith.tensor(gpu_utilizations).mean().item()
    ...
    def _send_requests(self):
        ...
        fake_data = smith.randn(self.batch_size, 3, 250, 250, requires_grad=False)
        other_data = [
            smith.randn(self.batch_size, 3, 250, 250, requires_grad=False)
...
class BackendWorker:
    ...
    def __init__(
        ...
        self.h2d_stream = smith.cuda.Stream()
        self.d2h_stream = smith.cuda.Stream()
    ...
    def _setup(self):
        ...
        from smithvision.models.resnet import BasicBlock, ResNet
        ...
        state_dict = smith.load(
            f"{self.model_dir}/resnet18-f37072fd.pth",
            mmap=True,
            map_location=self.device,
        )
        self.metrics_dict["smith_load_time"] = time.time() - start_load_time
    ...
    def model_predict(
        ...
        with smith.cuda.stream(self.stream_map[threading.get_native_id()]):
            with smith.no_grad():
    ...
    def copy_data(self, input_buffer, data, copy_event, copy_sem):
        ...
        with smith.cuda.stream(self.h2d_stream):
    ...
    def respond(self, compute_event, compute_sem, response_list, request_time):
        ...
        with smith.cuda.stream(self.d2h_stream):
            self.response_queue.put((response_list[0].cpu(), request_time))

    async def run(self):
        def worker_initializer():
            self.stream_map[threading.get_native_id()] = smith.cuda.Stream()
        ...
        while True:
            ...
            copy_event = smith.cuda.Event()
            compute_event = smith.cuda.Event()
            response_list = []
            input_buffer = smith.empty(
                [self.batch_size, 3, 250, 250], dtype=smith.float32, device="cuda"
            )
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(f"{args.model_dir}/resnet18-f37072fd.pth"):
        p = subprocess.run(
            [
                "wget",
                "https://download.blacksmith.org/models/resnet18-f37072fd.pth",
            ]
        )
        if p.returncode == 0:
    ...
    try:
        ...
        if args.profile:
            ...
            with smith.profiler.profile(on_trace_ready=trace_handler) as prof:
