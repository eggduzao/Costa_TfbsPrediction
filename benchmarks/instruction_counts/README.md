...
`smith.utils.benchmark.Timer`
* `GroupedStmts`: Benchmark a snippet. (Python, C++, or both) Can automatically
generate SmithScript and autograd variants.
...
benchmark = GroupedStmts(
    ...
    setup=GroupedSetup(
        py_setup="""
            x = smith.ones((4, 4))
            w = smith.ones((4, 4), requires_grad=True)
        """,
        cpp_setup="""
            auto x = smith::ones((4, 4));
            auto w = smith::ones((4, 4));
...
As a result, we can derive SmithScript'd and AutoGrad variants as well. We can
deduce that a SmithScript model will take the form:
...
disk and load it as needed. At this point Timers for SmithScript become:
...
Timer(
    stmt="""
        std::vector<smith::jit::IValue> ivalue_inputs(
            smith::jit::IValue({x}),
            smith::jit::IValue({w})
        );
...
and we make sure to assign it when calling SmithScript models, testing AutoGrad
...
generating `stmt` based on language, Eager/SmithScript, with or without AutoGrad)
...
is a one-to-one and direct mapping to the `smith.utils.benchmark.Timer` instance
