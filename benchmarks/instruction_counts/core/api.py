...
class GroupedBenchmark:
    ...
    There are also two programming idioms in Blacksmith. One is to write free form
    code (so-called "NumPy with gradients"), and the other is to organize code
    using `smith.nn.Module`s. (This is how common neural network layers are
    exposed through the Blacksmith API.) To support easy definition two simple
    ...
    # `cpp_model_setup`, but if SmithScript is requested it will generate
    # `py_model_setup` using `smith.jit.script`.
    ...
    def init_from_model(
        ...
        """Create a set of benchmarks using smith.nn Modules.
        ...
        if runtime == RuntimeMode.EAGER:
            ...
            cpp_invocation = textwrap.dedent(
                ...
                std::vector<smith::jit::IValue> ivalue_inputs({{
                    {", ".join([f"smith::jit::IValue({a})" for a in signature_args])}
                }});
