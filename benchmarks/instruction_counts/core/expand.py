...
if TYPE_CHECKING:
    ...
    from smith.utils.benchmark.utils.timer import Language
else:
    from smith.utils.benchmark import Language
...
def _generate_smithscript_file(model_src: str, name: str) -> Optional[str]:
    """Returns the path a saved model if one can be constructed from `spec`.

    Because SmithScript requires actual source code in order to script a
    ...
    the SmithScript model from that temporary file.
    ...
    # `smith.utils.benchmark.Timer` will automatically import smith, so we
    # need to match that convention.
    model_src = f"import smith\n{model_src}"

    model_root = os.path.join(get_temp_dir(), "SmithScript_models")
    os.makedirs(model_root, exist_ok=True)
    module_path = os.path.join(model_root, f"smithscript_{name}.py")
    artifact_path = os.path.join(model_root, f"smithscript_{name}.pt")
    ...
    module_spec = importlib.util.spec_from_file_location(
        f"smithscript__{name}", module_path
    )
    ...
    if not isinstance(jit_model, (smith.jit.ScriptFunction, smith.jit.ScriptModule)):
...
def _get_setup(
    ...
    if language == Language.PYTHON:
        setup_template: str = textwrap.dedent(
            f"""
            jit_model = smith.jit.load("{model_path}")
        ...
        setup_template = textwrap.dedent(
            ...
            auto jit_model = smith::jit::load(fpath);
...
def materialize(benchmarks: FlatIntermediateDefinition) -> FlatDefinition:
    ...
    This entails generation of SmithScript model artifacts, splitting
    ...
    for label, args in benchmarks.items():
        if isinstance(args, TimerArgs):
            ...
            if args.py_model_setup and args.smithscript:
                model_setup = (
                    f"{args.py_model_setup}\njit_model = smith.jit.script(model)"
                )
                ...
                model_path = _generate_smithscript_file(model_setup, name=name)
            ...
            for (runtime, autograd, language), num_threads in it.product(
                ...
                if runtime == RuntimeMode.JIT and not args.smithscript:
                    continue
