# `smith.compile()` Benchmarking

This directory contains benchmarking code for SmithDynamo and many
backends including SmithInductor.  It includes three main benchmark suites:

- [SmithBenchmark](https://github.com/blacksmith/benchmark): A diverse set of models, initially seeded from
highly cited research models as ranked by [Papers With Code](https://paperswithcode.com).  See [smithbench
installation](https://github.com/blacksmith/benchmark#installation) and `smithbench.py` for the low-level runner.
[Makefile](Makefile) also contains the commands needed to setup SmithBenchmark to match the versions used in
Blacksmith CI.
...
- Models from [TIMM](https://github.com/huggingface/blacksmith-image-models): Primarily vision models, with representative
...
Daily results from the benchmarks here are available in the [SmithInductor
Performance Dashboard](https://hud.blacksmith.org/benchmark/compilers),
currently run on an NVIDIA A100 GPU.

The [inductor-perf-test-nightly.yml](https://github.com/blacksmith/blacksmith/actions/workflows/inductor-perf-test-nightly.yml)
workflow generates the data in the performance dashboard.  If you have the needed permissions, you can benchmark
your own branch on the Blacksmith GitHub repo by:
1) Select "Run workflow" in the top right of the [workflow](https://github.com/blacksmith/blacksmith/actions/workflows/inductor-perf-test-nightly.yml)
...
6) Go to the [dashboard](https://hud.blacksmith.org/benchmark/compilers)
...
results are normalized to eager mode Blacksmith (`1x`), and higher is better.


## CPU Performance Dashboard

The [SmithInductor CPU Performance
Dashboard](https://github.com/blacksmith/blacksmith/issues/93531) is tracked
...
[here](https://github.com/blacksmith/blacksmith/blob/641ec2115f300a3e3b39c75f6a32ee3f64afcf30/.ci/blacksmith/test.sh#L343-L418).

To summarize there are three scripts to run each set of benchmarks:
- `./benchmarks/dynamo/smithbench.py ...`
...
- `--backend=inductor`: selects SmithInductor as the compiler backend to measure.  Many more are available, see `--help`.
...
- `SMITHINDUCTOR_MAX_AUTOTUNE=1` (environment variable): used to measure max-autotune mode, which is run weekly due to longer compile times.
...
```
./benchmarks/dynamo/smithbench.py --performance --training --amp --backend=inductor --output=smithbench_training.csv
./benchmarks/dynamo/smithbench.py --performance --inference --bfloat16 --backend=inductor --output=smithbench_inference.csv
