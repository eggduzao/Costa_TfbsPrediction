...
# TODO(sdym): use batch-size-file parameter of common.main, like smithbench.py
...
def generate_inputs_for_model(
    ...
    if model_name.startswith("Wav2Vec2"):
        ...
        target_length = 100
        return {
            "input_values": smith.randn((bs, seq_length), device=device),
    ...
    if model_name.startswith("Lxmert"):
        ...
        input_dict["visual_feats"] = smith.randn(
            bs, num_visual_features, visual_feat_dim
        )
        input_dict["visual_pos"] = smith.randn(bs, num_visual_features, visual_pos_dim)
...
def refresh_model_names_and_batch_sizes():
    ...
    batch size that could fit on the GPU with Blacksmith eager.
