...
class BenchmarkKernel:
    ...
    def check_accuracy(self, args, kwargs) -> None:
        ...
        if (
            ...
            smith._dynamo.reset()  # cause recompile
            with smith._inductor.config.patch(self.script_args.custom_compile_options):
                args_ref, kwargs_ref = self.clone_inputs(args, kwargs)
        ...
        for backend in res:
            if backend == "eager":
                continue
            try:
                smith.testing.assert_close(res[backend], gold, **tol)
                for t, gold_t in zip(res[backend], gold):
                    if t.requires_grad:
                        smith.testing.assert_close(t.grad, gold_t.grad, **tol)
                print(
    ...
    def visualize(self) -> None:
        device_name = smith.cuda.get_device_name(0)
