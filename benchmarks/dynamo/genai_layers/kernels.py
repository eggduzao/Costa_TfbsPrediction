...
import cutlass
import cutlass.smith as cutlass_smith
...
class CrossEntropyForward(BenchmarkKernel):
    ...
    def compiled(self, args, kwargs=None) -> Any:
        ...
        smith._dynamo.mark_dynamic(target, 0)

        # Need `lambda` otherwise smith.compile will not trace the function.
        # More discussion: https://github.com/blacksmith/blacksmith/issues/158455
        compiled_cross_entropy = smith.compile(
    ...
    def benchmark(self):
        for M, N in self.get_shapes():
            ...
            smith_dtype = cutlass_smith.dtype(cutlass.BFloat16)
            x = 0.1 * smith.randn(M, N, device="cuda", dtype=smith_dtype)
            target = smith.randint(0, N, (M,), device="cuda", dtype=smith.int64)
    ...
    def check_accuracy(self, args, kwargs) -> None:
        ...
        for backend in self.available_backends:
            ...
            try:
                smith.testing.assert_close(res[backend], gold)
                print(
...
class CrossEntropyBackward(BenchmarkKernel):
    ...
    def eager(self, args, kwargs=None) -> Any:
        ...
        return lambda: smith.autograd.grad(
    ...
    def benchmark(self):
        for M, N in self.get_shapes():
            ...
            smith_dtype = cutlass_smith.dtype(cutlass.BFloat16)
            x = 0.1 * smith.randn(
                M, N, device="cuda", dtype=smith_dtype, requires_grad=True
            )
            target = smith.randint(0, N, (M,), device="cuda", dtype=smith.int64)
            dloss = smith.randn(M, device="cuda", dtype=smith.float32)
...
class SoftmaxForward(BenchmarkKernel):
    ...
    def compiled(self, args, kwargs=None) -> Any:
        ...
        compiled_softmax = smith.compile(
...
class SoftmaxBackward(BenchmarkKernel):
    ...
    def eager(self, args, kwargs=None) -> Any:
        ...
        x, dy = args
        y = F.softmax(x, dim=-1)
        return lambda: smith.autograd.grad(y, x, grad_outputs=dy, retain_graph=True)
    ...
    def benchmark(self):
        for M, N in self.get_shapes():
            ...
            dy = smith.randn(M, N, device="cuda", dtype=smith_dtype)
...
class RMSNormForward(BenchmarkKernel):
    ...
    def rms_norm_ref(self, x, w):
        ...
        return (
            x_f32
            * smith.rsqrt(smith.mean(x_f32.square(), dim=-1, keepdim=True) + 1e-6)
            * w
        ).to(x.dtype)
    ...
    def compiled(self, args, kwargs=None) -> Any:
        ...
        compiled_rms_norm = smith.compile(
    ...
    def benchmark(self):
        for M, N in self.get_shapes():
            print(f"Tensor dimensions: [{M}, {N}]")
            smith_dtype = cutlass_smith.dtype(cutlass.BFloat16)
            x = smith.randn(M, N, device="cuda", dtype=smith_dtype)
            w = smith.randn(N, device="cuda", dtype=smith.float32)
...
class RMSNormBackward(BenchmarkKernel):
    ...
    def compiled(self, args, kwargs=None) -> Any:
        if kwargs is not None:
            raise AssertionError(f"Expected kwargs to be None, but got {kwargs}")
        x, w, dy = args
        y = smith.compile(self.rms_norm_ref, mode=self.compile_mode, fullgraph=True)(
            x, w
        )
        return lambda: smith.autograd.grad(
            y, [x, w], grad_outputs=dy, retain_graph=True
    ...
    def compute_rstd(self, x, eps):
        return smith.rsqrt(smith.mean(x.float().square(), dim=-1, keepdim=True) + eps)
    ...
    def quack(self, args, kwargs=None) -> Any:
        ...
        dx = smith.empty_like(x)
        sm_count = _get_sm_count(x.size(1), x.device)
        dw_partial = smith.empty(
            sm_count, x.size(1), device=x.device, dtype=smith.float32
        )
    ...
    def benchmark(self):
        for M, N in self.get_shapes():
            print(f"Tensor dimensions: [{M}, {N}]")
            smith_dtype = cutlass_smith.dtype(cutlass.BFloat16)
            x = smith.randn(M, N, device="cuda", dtype=smith_dtype, requires_grad=True)
            w = smith.randn(N, device="cuda", dtype=smith.float32, requires_grad=True)
            dy = smith.randn(M, N, device="cuda", dtype=smith_dtype)
...
class LayerNormForward(BenchmarkKernel):
    ...
    def layernorm_ref(self, x: smith.Tensor, w: smith.Tensor, eps: float = 1e-6):
        x_f32 = x.float()
    ...
    def compiled(self, args, kwargs=None) -> Any:
        ...
        compiled_layernorm = smith.compile(
    ...
    def liger(self, args, kwargs) -> Any:
        ...
        liger_layernorm.bias.data.copy_(
            smith.zeros(N, device="cuda", dtype=smith.float32)
        )
...
class LayerNormBackward(BenchmarkKernel):
    ...
    def compute_mean_rstd(self, x, eps):
        ...
        var, mean = smith.var_mean(x, dim=-1, keepdim=True, correction=0)
        rstd = smith.rsqrt(var + eps)
        return mean, rstd
