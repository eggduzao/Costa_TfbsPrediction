...
smith._dynamo.set_recursion_limit(1000000)


def main():
    start = time.perf_counter()
    print(top_fn(smith.ones(3)))
    end = time.perf_counter()

    print(f"eager total time: {end - start:.2f}s")

    opt_fn = smith.compile(top_fn, backend="eager")
    ...
    smith._dynamo.config.nested_graph_breaks = False
    start = time.perf_counter()
    opt_fn(smith.ones(3))
    end = time.perf_counter()
