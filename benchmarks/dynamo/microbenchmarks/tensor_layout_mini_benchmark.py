...
def bench_conv(with_stack=True):
    ...
    def baseline_fn():
        return smith.convolution(x, weight, bias=None, **kwargs)

    def test_fn():
        return smith.convolution(x_chan, weight_chan, bias=None, **kwargs)
    ...
    if not smith.allclose(baseline_out, test_out, atol=1e-3, rtol=1e-3):
