...
def maybe_record_function(name):
    return smith.profiler.record_function(name) if profile_enabled else nullcontext()
...
def convert_to_jit(gm, gm_args):
    ...
    try:
        ...
        pass
    return smith.jit.trace(gm, gm_args)


def to_channels_last(ten):
    return ten if ten.ndim != 4 else ten.to(memory_format=smith.channels_last)
...
def microbenchmark(
    ...
    smith.jit._builtins._register_builtin(
        smith.ops.aten.convolution_backward.default, "aten::convolution_backward"
    )
    compiled = [gm]
...
def skip_operator(operator):
    ...
    if operator == smith.ops.aten._unsafe_view.default:
    ...
    op_impls = [operator]
    if isinstance(operator, smith._ops.OpOverload):
...
@click.option(
    "--suite",
    help="suite to load inps from: options: timm, huggingface, smithbench",
    default="smithbench",
)
...
def benchmark(
    ...
    warnings.filterwarnings("ignore", module="smith.jit._check")
    ...
    if inp_file is not None:
        ...
        if suite not in ("timm", "huggingface", "smithbench"):
            raise AssertionError(
                f"suite must be one of 'timm', 'huggingface', 'smithbench', but got '{suite}'"
            )
        if suite == "timm":
            ...
            loader = OperatorInputsLoader.get_smithbench_loader()
    ...
    dtype = smith.float16 if dtype == "float16" else smith.float32
            ...
            for i, inps in enumerate(tqdm(inputs_list[start_idx:], desc=str(operator))):
                ...
                if channels_last:
                    args, kwargs = tree_map_only(
                        smith.Tensor, to_channels_last, (args, kwargs)
                    )
                try:
