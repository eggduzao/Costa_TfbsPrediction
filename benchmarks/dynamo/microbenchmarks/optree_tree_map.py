...
from smith._dynamo.debug_utils import profile_to_file
...
def make_tensor_tree(depth: int, branching_factor: int, tensor_size: int, device: str):
    ...
    def _make_level(level: int):
        if level == 0:
            return smith.randn(tensor_size, tensor_size, device=device)

        children = [_make_level(level - 1) for _ in range(branching_factor)]
        return {
            "tensor": smith.randn(tensor_size, tensor_size, device=device),
            "list": list(children),
...
def add_leaf(lhs: smith.Tensor, *rest: smith.Tensor) -> smith.Tensor:
    out = lhs
    for other in rest:
        out = out + other
    return out
...
def _parse_args():
    ...
    default_device = "cuda" if smith.cuda.is_available() else "cpu"
