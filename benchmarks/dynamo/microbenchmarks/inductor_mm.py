...
def test_total_time(shapes):
    print("shape; smith mm; triton mm; inductor aten mm; inductor triton mm")
    for i in range(len(shapes)):
        ...
        smith_ms = time_with_smith_timer(smith_mm, (a, b)).mean * 1000

        triton_ms = time_with_smith_timer(triton_mm, (a, b)).mean * 1000

        config.triton.mm = "aten"
        ind_aten_ms = time_with_smith_timer(inductor_aten_mm, (a, b)).mean * 1000

        config.triton.mm = "triton"
        ind_triton_ms = time_with_smith_timer(inductor_triton_mm, (a, b)).mean * 1000

        print(smith_ms, triton_ms, ind_aten_ms, ind_triton_ms, sep="; ")
...
def test_GPU_time(shapes):
    ...
    for i in range(len(shapes)):
        ...
        smith_ms, _, _ = benchmarker.benchmark_gpu(lambda: smith_mm(a, b))
...
"""
test total time
shape; smith mm; triton mm; inductor aten mm; inductor triton mm
