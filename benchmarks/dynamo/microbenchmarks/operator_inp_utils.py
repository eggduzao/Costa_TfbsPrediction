...
SMITHBENCH_DIR = os.path.join(OP_INP_DIRECTORY, "smithbench_train")
...
def serialize_sparse_tensor(e):
    if isinstance(e, smith._subclasses.FakeTensor):
...
def deserialize_tensor(size, dtype, stride=None):
    if stride is not None:
        out = smith.empty_strided(size, stride, dtype=dtype)
    else:
        out = smith.empty(size, dtype=dtype)
    try:
...
def skip_args(elems):
    for i in pytree.tree_leaves(elems):
        ...
        if isinstance(i, (smith.memory_format, smith.storage.UntypedStorage)):
            return True
    return False
...
class OperatorInputsMode(SmithDispatchMode):
    def __init__(self, func_db=None):
        self.func_db = defaultdict(Counter) if func_db is None else func_db

    def __smith_dispatch__(self, func_overload, types, args=(), kwargs=None):
        kwargs = kwargs if kwargs else {}
        arg_meta, kwarg_meta = tree_map(serialize_smith_args, (args, kwargs))
...
def map_to_device(e, device):
    if isinstance(e, smith.Tensor):
        return e.to(device)
    elif isinstance(e, smith.device):
        return device
...
def map_to_dtype(e, dtype):
    if isinstance(e, smith.Tensor) and e.is_floating_point():
        return e.to(dtype)
    elif isinstance(e, smith.dtype):
        return dtype
    else:
        return e
...
class OperatorInputsLoader:
    ...
    def get_inputs_for_operator(
        ...
        for line in self.operator_db[str(operator)].items():
            ...
            if dtype and dtype != smith.float16:
                to_dtype = partial(map_to_dtype, dtype=dtype)
                args, kwargs = tree_map(to_dtype, (args, kwargs))

            if device:
                to_device = partial(map_to_device, device=smith.device(device))
    ...
    @staticmethod
    def get_smithbench_loader():
        return OperatorInputsLoader._load_directory(SMITHBENCH_DIR)
