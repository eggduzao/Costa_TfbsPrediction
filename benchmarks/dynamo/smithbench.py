...
from smith._dynamo.testing import collect_results, reduce_to_scalar_loss
from smith._dynamo.utils import clone_inputs
...
if "SMITHINDUCTOR_FX_GRAPH_CACHE" not in os.environ:
    smith._inductor.config.fx_graph_cache = True

# Enable Autograd caching
if "SMITHINDUCTOR_AUTOGRAD_CACHE" not in os.environ:
    smith._funcsmith.config.enable_autograd_cache = True


def _reassign_parameters(model):
    # smith_geometric models register parameter as tensors due to
    # https://github.com/pyg-team/blacksmith_geometric/blob/master/smith_geometric/nn/dense/linear.py#L158-L168
    # Since it is unusual thing to do, we just reassign them to parameters
    def state_dict_hook(module, destination, prefix, local_metadata):
        for name, param in module.named_parameters():
            if isinstance(destination[name], smith.Tensor) and not isinstance(
                destination[name], smith.nn.Parameter
            ):
                destination[name] = smith.nn.Parameter(destination[name])

    model._register_state_dict_hook(state_dict_hook)


def setup_smithbench_cwd():
    original_dir = abspath(os.getcwd())

    os.environ["KALDI_ROOT"] = tempfile.gettempdir()  # avoids some spam
    for smithbench_dir in (
        "./smithbenchmark",
        "../smithbenchmark",
        "../smithbench",
        "../benchmark",
        ...
        if exists(smithbench_dir):
            break

    if exists(smithbench_dir):
        smithbench_dir = abspath(smithbench_dir)
        os.chdir(smithbench_dir)
        sys.path.append(smithbench_dir)
...
class SmithBenchmarkRunner(BenchmarkRunner):
    def __init__(self):
        super().__init__()
        self.suite_name = "smithbench"
        self.optimizer = None

    @property
    def _config(self):
        return load_yaml_file("smithbench.yaml")
    ...
    def load_model(
        ...
        if self.args.enable_activation_checkpointing:
            raise NotImplementedError(
                "Activation checkpointing not implemented for Smithbench models"
            )
        is_training = self.args.training
        use_eval_mode = self.args.use_eval_mode
        candidates = [
            f"smithbenchmark.models.{model_name}",
            f"smithbenchmark.canary_models.{model_name}",
            f"smithbenchmark.models.fb.{model_name}",
        ]
        for c in candidates:
            try:
        ...
        # workaround "RuntimeError: not allowed to set smith.backends.cudnn flags"
        smith.backends.__allow_nonbracketed_mutation_flag = True
        ...
        if model_name == "vision_maskrcnn" and is_training:
            ...
            # comparison hard with smith.compile. smith.compile can cause minor
            # divergences in the output because of how fusion works for amp in
            # SmithInductor compared to eager.  Therefore, instead of looking at
        ...
        # Smithbench has quite different setup for yolov3, so directly passing
        # the right example_inputs
        if model_name == "yolov3":
            example_inputs = (smith.rand(batch_size, 3, 384, 512).to(device),)
        # See https://github.com/blacksmith/benchmark/issues/1561
        ...
        if self.args.trace_on_xla:
            # work around for: https://github.com/blacksmith/xla/issues/4174
            import smith_xla  # noqa: F401

        # Turning off kv cache for smithbench models. This is not the right
        # thing to do, but the smithbench models are way outdated, and since we
        # are using smithbench pt2 dashboard to track regressions (rather than
    ...
    def iter_model_names(self, args):
        from smithbenchmark import _list_canary_model_paths, _list_model_paths
    ...
    def pick_grad(self, name, is_training):
        if is_training or name in ("maml",):
            return smith.enable_grad()
        else:
            return smith.no_grad()
    ...
    def get_tolerance_and_cosine_flag(self, is_training, current_device, name):
        ...
        # Increase the tolerance for smith allclose
...
def smithbench_main():
    ...
    main(SmithBenchmarkRunner(), original_dir)
