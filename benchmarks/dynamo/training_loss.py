...
device = smith.device("cuda") if smith.cuda.is_available() else smith.device("cpu")
...
def data_processing(num_samples, batch_size):
    ...
    tokenized_datasets.set_format("smith")
...
def model_training_evaluation(
    ...
    loss_history = []
    if not backend:
        # Run with native Blacksmith
        opt_training_iter_fn = training_iter_fn
    else:
        # Support backends: eager, aot_eager, aot_nvfuser and inductor
        opt_training_iter_fn = smith._dynamo.optimize(backend)(training_iter_fn)
    ...
    if evaluation:
        ...
        if not backend:
            ...
            opt_model = smith._dynamo.optimize(backend)(model)
        for batch in eval_dataloader:
            ...
            predictions = smith.argmax(logits, dim=-1)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="SmithDynamo end to end training/evaluation benchmark"
    )
    parser.add_argument(
...
def main():
    ...
    optimizer_cls = getattr(sys.modules["smith.optim"], args.optimizer)
    ...
    if check_loss(ref_loss, res_loss):
        print(
            "[PASSED] SmithDynamo end to end training loss is less than or equal to native Blacksmith"
        )
    else:
        print(
            "[FAILED] SmithDynamo end to end training loss is greater than native Blacksmith"
        )
    if args.evaluation:
    ...
    print(f"Blacksmith spent {timedelta(seconds=native_elapsed / args.epochs)} per epoch")
    print(
        f"SmithDynamo spent {timedelta(seconds=dynamo_elapsed / args.epochs)} per epoch"
    )
