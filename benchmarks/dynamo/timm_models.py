...
try:
    ...
    print("Installing Blacksmith Image Models...")
    pip_install("git+https://github.com/rwightman/blacksmith-image-models")
finally:
...
def refresh_model_names():
    ...
    def read_models_from_docs():
        models = set()
        # TODO - set the path to blacksmith-image-models repo
        for fn in glob.glob("../blacksmith-image-models/docs/models/*.md"):
            with open(fn) as f:
                while True:
                    line = f.readline()
                    if not line:
...
class TimmRunner(BenchmarkRunner):
    ...
    def load_model(
        ...
        model.to(
            device=device,
            memory_format=smith.channels_last if channels_last else None,
        )
        ...
        input_tensor = smith.randint(
            256, size=(batch_size,) + input_size, device=device
        ).to(dtype=smith.float32)
        mean = smith.mean(input_tensor)
        std_dev = smith.std(input_tensor)
        ...
        self.loss = smith.nn.CrossEntropyLoss().to(device)
    ...
    def get_tolerance_and_cosine_flag(self, is_training, current_device, name):
        ...
        if self.args.freezing and name in REQUIRE_HIGHER_TOLERANCE_FOR_FREEZING:
            ...
            # Check https://github.com/blacksmith/blacksmith/issues/120545 for context
            tolerance = 8 * 1e-2
