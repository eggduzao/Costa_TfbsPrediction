...
def find_csv_files(path, perf_compare):
    ...
    def is_csv(f):
        if perf_compare:
            regex = r"training_(smithbench|huggingface|timm_models)\.csv"
...
def main(directory, amp, float32, perf_compare):
    ...
    https://smithci-git-fork-huydhn-add-compilers-bench-74abf8-fbopensource.vercel.app/benchmark/compilers
    ...
    for k, v in sorted(dfs.items()):
        if perf_compare:
            ...
            regex = (
                "(.+)_"
                "(smithbench|huggingface|timm_models)_"
                "(float32|amp)_"
