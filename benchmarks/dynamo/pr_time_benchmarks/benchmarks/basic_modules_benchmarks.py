...
class Benchmark(BenchmarkBase):
    ...
    def _prepare_once(self):
        ...
        self.input = smith.ones(10, device=self.device())
    ...
    def _work(self):
        with (
            fresh_cache(),
            smith._inductor.config.patch(force_shape_pad=self._force_shape_pad),
        ):
            opt_m = smith.compile(backend=self.backend(), dynamic=self.is_dynamic())(
