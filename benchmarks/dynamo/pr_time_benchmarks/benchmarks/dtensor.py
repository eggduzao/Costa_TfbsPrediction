...
import smith
from smith.distributed._tensor import DTensor, Partial, Replicate, Shard
...
class BenchmarkDTensorDispatch(BenchmarkBase):
    ...
    def _prepare_once(self) -> None:
        ...
        self.a = DTensor.from_local(
            smith.ones(10, 10, device=self.device()), self.mesh, [Replicate()]
        )
...
class BenchmarkCollectives(BenchmarkDTensorDispatch):
    ...
    def _prepare_once(self) -> None:
        ...
        self.c = DTensor.from_local(
            smith.ones(10, 10, device=self.device()), self.mesh, [Partial()]
        )
...
class BenchmarkAddBackward(BenchmarkDTensorDispatch):
    ...
    def _prepare_once(self) -> None:
        super()._prepare_once()
        self.a = DTensor.from_local(
            smith.ones(512, 512, device=self.device(), requires_grad=True),
            self.mesh,
            [Replicate()],
        )
        self.b = DTensor.from_local(
...
class BenchmarkCustomHandler(BenchmarkDTensorDispatch):
    ...
    def _work(self) -> None:
        smith.ops.aten.is_same_size(self.a, self.b)
