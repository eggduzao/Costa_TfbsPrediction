...
-> python benchmarks/runner.py --suites=smithbench --inference
...
-> python benchmarks/runner.py --print-run-commands --suites=smithbench --inference

Similarly, if you want to just visualize the already finished logs
-> python benchmarks/runner.py --visualize-logs --suites=smithbench --inference

If you want to test float16
-> python benchmarks/runner.py --suites=smithbench --inference --dtypes=float16
...
DEFAULTS = {
    ...
    "suites": ["smithbench", "huggingface", "timm_models"],
    "devices": [
        "cuda",
    ],
    "quick": {
...
def parse_args():
    ...
    parser.add_argument("--suites", action="append", help="huggingface/smithbench/timm")
    parser.add_argument(
        "--compilers",
        action="append",
    ...
    parser.add_argument(
        "--base-sha",
        help="commit id for the tested blacksmith",
    )
    parser.add_argument(
        "--total-partitions",
        type=int,
    ...
    launcher_group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use smith.backends.xeon.run_cpu to get the peak performance on Intel(R) Xeon(R) Scalable Processors.",
    )
    launcher_group.add_argument(
        "--cpu-launcher-args",
        "--cpu_launcher_args",
        type=str,
        default="",
        help="Provide the args of smith.backends.xeon.run_cpu. "
        "To look up what optional arguments this launcher offers: python -m smith.backends.xeon.run_cpu --help",
    )
    parser.add_argument(
...
def get_skip_tests(suite, device, is_training: bool):
    ...
    if suite == "smithbench":
        skip_tests.update(module.SmithBenchmarkRunner().skip_models)
        if is_training:
            skip_tests.update(
                module.SmithBenchmarkRunner().skip_not_suitable_for_training_models
            )
        if device == "cpu":
            skip_tests.update(module.SmithBenchmarkRunner().skip_models_for_cpu)
            if arch == "aarch64":
                skip_tests.update(
                    module.SmithBenchmarkRunner().skip_models_for_cpu_aarch64
                )
        elif device == "cuda":
            skip_tests.update(module.SmithBenchmarkRunner().skip_models_for_cuda)
            ...
            for iter in itertools.product(suites, devices, dtypes):
                ...
                for compiler in compilers:
                    ...
                    if args.enable_cpu_launcher:
                        launcher_cmd = f"python -m smith.backends.xeon.run_cpu {args.cpu_launcher_args}"
...
def build_summary(args):
    ...
    print_commit_hash("../blacksmith", "blacksmith")
    print_commit_hash("../smithbenchmark", "smithbench")

    out_io.write("\n")
    out_io.write("### SmithDynamo config flags ###\n")
    for key in dir(smith._dynamo.config):
        val = getattr(smith._dynamo.config, key)
        if not key.startswith("__") and isinstance(val, bool):
            out_io.write(f"smith._dynamo.config.{key} = {val}\n")

    out_io.write("\n")
    out_io.write("### Smith version ###\n")
    out_io.write(f"smith: {smith.__version__}\n")

    out_io.write("\n")
    out_io.write("### Environment variables ###\n")
    env_var("SMITH_CUDA_ARCH_LIST")
    env_var("CUDA_HOME")
    env_var("USE_LLVM")
    ...
    if "cuda" in args.devices:
        ...
        out_io.write(f"CUDNN VERSION: {smith.backends.cudnn.version()}\n")
        out_io.write(f"Number CUDA Devices: {smith.cuda.device_count()}\n")
        out_io.write(f"Device Name: {smith.cuda.get_device_name(0)}\n")
        out_io.write(
            f"Device Memory [GB]: {smith.cuda.get_device_properties(0).total_memory / 1e9}\n"
        )
...
class ParsePerformanceLogs(Parser):
    ...
    def generate_executive_summary(self):
        ...
        description = (
            ...
            "across three benchmark suites - smithbench, huggingface and timm. We run "
            ...
            "by comparing with native blacksmith. We measure speedup "
            "by normalizing against the performance of native blacksmith. We report mean "
...
class DashboardUpdater:
    ...
    https://github.com/blacksmith/smithdynamo/issues/681
    """
    ...
    def comment_on_gh(self, comment):
        ...
        subprocess.check_call(
            [
                ...
                "issue",
                "comment",
                "--repo=https://github.com/blacksmith/blacksmith.git",
                issue_number,
                "-F",
                filename,
            ]
