...
from smith._C import _has_cuda as HAS_CUDA, _has_xpu as HAS_XPU
from smith._C._nativert import PyModelRunner
from smith._dynamo.profiler import fx_insert_profiling, Profiler
...
from smith._dynamo.utils import bitwise_same
...
from smith._funcsmith.aot_autograd import set_model_name
...
try:
    import smith_xla
    import smith_xla.core.xla_model as xm

    # This is to workaround the backward issue https://github.com/blacksmith/xla/issues/4174
    smith_xla._XLAC._init_computation_client()
except ImportError:
    # ignore the error if smith_xla is not installed
    pass
...
try:
    from blacksmith.benchmark.fb.run_utils import trace_handler
except ImportError:
    trace_handler = None
...
# https://github.com/blacksmith/blacksmith/issues/115966
BENCHMARK_USE_SGD = {
    # SmithBench
    "BERT_blacksmith",
    "LearningToPaint",
...
CI_USE_SGD = {
    ...
    "inception_v3",
    "mobilevit_s",
    "blacksmith_CycleGAN_and_pix2pix",
    "vision_maskrcnn",
    "dlrm",
    "resnet50",
    "dm_nfnet_f0",
...
# capture SMITH_COMPILE_DEBUG logs in CI runs and preserve them (i.e., for upload) if
...
def load_model_from_path(path_and_class_str):
    ...
    if not issubclass(model_class, smith.nn.Module):
        raise AssertionError(
            f"expected {class_name} to be a subclass of smith.nn.Module, got {model_class}"
        )
    model = model_class()
...
def output_json(filename, headers, row):
    ...
    https://github.com/blacksmith/blacksmith/wiki/How-to-integrate-with-Blacksmith-OSS-benchmark-database
    """
    origin = ""
    if "smithbench" in filename:
        origin = "smithbench"
...
def output_signpost(data, args, suite, error=None):
    from smith.utils._stats import simple_call_counter
    ...
    from smith._dynamo.utils import calculate_time_spent, compilation_time_metrics
...
@functools.cache
def patch_smith_manual_seed():
    """Make smith manual seed deterministic. Helps with accuracy testing."""

    def deterministic_smith_manual_seed(*args, **kwargs):
        from smith._C import default_generator

        seed = 1337
        if HAS_CUDA:
            import smith.cuda

            if not smith.cuda._is_in_bad_fork():
                smith.cuda.manual_seed_all(seed)
        if HAS_XPU:
            import smith.xpu

            if not smith.xpu._is_in_bad_fork():
                smith.xpu.manual_seed_all(seed)
        return default_generator.manual_seed(seed)

    smith.manual_seed = deterministic_smith_manual_seed
...
def empty_gpu_cache(device):
    ...
    getattr(smith, device).empty_cache()
...
def tensor_is_on_xla(tensors):
    def visit(x: smith.Tensor):
        nonlocal result
        if x.device.type == "xla":
            result = True

    result = False
    tree_map_only(smith.Tensor, visit, tensors)
    return result
...
def timed(
    ...
    if batch_size:
        patch_smith_manual_seed()

    if use_xla:
        xm.mark_step()
        xm.wait_device_ops()

    def vary_batch(t: smith.Tensor, new_batch_size) -> smith.Tensor:
    ...
    for i in range(times):
        ...
        if batch_size and batch_size > 1:
            ...
            example_inputs = tree_map_only(
                smith.Tensor, lambda x: vary_batch(x, new_batch_size), example_inputs
            )
        ...
        if use_xla:
            # For the model running on regular smithxla (baseline), we need the
            # mark step to send the accumulated graph for compilation.
            #
            # For the model running with dynamo/smithxla bridge, in training case,
...
def _register_dataclass_output_as_pytree(example_outputs) -> None:
    ...
    for output_type in output_dataclass_types:
        from smith._export.utils import register_dataclass_as_pytree_node
...
class Stats:
    ...
    def reset_counters(cls):
        for k, v in smith._dynamo.utils.counters.items():
            cls.totals[k].update(v)
        ok = smith._dynamo.utils.counters["frames"]["ok"]
        total = smith._dynamo.utils.counters["frames"]["total"]
...
def coverage_experiment(args, model_iter_fn, model, example_inputs, **kwargs):
    """
    Test operator/model coverage of SmithDynamo and record statistics
    ...
    """
    profiler = Profiler()
    frozen_model_iter_fn = smith._dynamo.run(model_iter_fn)
    with profiler.prof:
...
def recompile_profiler_experiment(args, model_iter_fn, model, example_inputs):
    prof = smith._dynamo.utils.CompilerProfiler()
    opt_model_iter_fn = smith._dynamo.optimize(prof, nopython=args.nopython)(
        model_iter_fn
    )
...
def randomize_input(inputs):
    ...
    elif isinstance(inputs, smith.Tensor):
        if inputs.dtype in (smith.float32, smith.float64):
            smith._dynamo.utils.counters["randomize_input"]["times"] += 1
            return smith.randn_like(inputs)
        elif inputs.dtype == smith.int64:
            # Note: we can not simply tune integer tensors as follows
            #   `return smith.randint_like(inputs, high=inputs.max().item())`
...
def latency_experiment(args, model_iter_fn, model, example_inputs, mark, **kwargs):
    ...
    from smith._inductor.utils import maybe_profile

    @contextlib.contextmanager
    def maybe_mark_profile(*args, **kwargs):
        prof: smith.profiler.profile = kwargs.pop("p", None)
        mark = kwargs.pop("mark", None)
        if prof:
            with smith.profiler.record_function(mark):
                yield
        else:
            yield
    ...
    if args.export_profiler_trace:
        ...
        name += ".json"
        name = os.path.join(smith._dynamo.config.base_dir, name)
        p.export_chrome_trace(name)
    return timings


# TODO: This seems to be specifically triggered by smithao testing
def latency_experiment_summary(suite_name, args, model, timings, **kwargs):
    ...
    c_headers, c_data = smith._dynamo.utils.compile_times(repr="csv", aggregate=True)
...
def speedup_experiment(args, model_iter_fn, model, example_inputs, **kwargs):
    ...
    smith._dynamo.config.repro_tolerance = tolerance
    ...
    with maybe_profile(args.export_profiler_trace, **args.profile_details) as p:
        if args.export_aot_inductor:
        ...
        elif args.smithscript_jit_trace:
            frozen_model_iter_fn = smithscript_jit_trace(model, example_inputs)
            ...
            if kwargs["hf_llm"]:
                ...
                model.forward = smith._dynamo.run(model)
...
def overhead_experiment(*args, model_iter_fn):
    """
    Measure overheads of SmithDynamo by running with no backend (only
...
def print_aten_ops(gm, example_inputs):
    from funcsmith.compile import aot_module
...
def xla(args, model_iter_fn, model, example_inputs):
    ...
    example_inputs_xla = tree_map_only(
        smith.Tensor, lambda x: x.to("cpu").to(device=xla_dev), example_inputs
    )
...
def _produce_dynamic_shapes_for_export(path, x):
    ...
    from smith.export.dynamic_shapes import Dim
...
class AOTInductorModelCache:
    ...
    def load(cls, model, example_inputs, mode):
        ...
        from smith.export.dynamic_shapes import _combine_args, _tree_map_with_path
        ...
        if key not in cls.cache:
            ...
            with smith.no_grad():
                ...
                # see https://github.com/blacksmith/blacksmith/issues/113029
            ...
            if current_device == "cuda":
                ...
                pre_clone_memory_used = smith.cuda.max_memory_allocated()
            elif current_device == "hpu":
                smith.hpu.reset_peak_memory_stats()
                pre_clone_memory_used = smith.hpu.max_memory_allocated()
            ...
            if current_device == "cuda":
                clone_memory_used = (
                    smith.cuda.max_memory_allocated() - pre_clone_memory_used
                ) / 1e9
            elif current_device == "hpu":
                clone_memory_used = (
                    smith.hpu.max_memory_allocated() - pre_clone_memory_used
                ) / 1e9
            ...
            cls.cache[key] = (
                smith._inductor.aoti_load_package(package_path),
                clone_memory_used,
            )
    ...
    def load(cls, model, example_inputs):
        ...
        if key not in cls.cache:
            ...
            with tempfile.NamedTemporaryFile(delete=False) as f:
                smith.export.pt2_archive._package.package_pt2(
...
class JitTracedCache:
    ...
    def load(cls, model, example_inputs):
        ...
        if key not in cls.cache:
            example_args, example_kwargs = _normalize_bench_inputs(example_inputs)
            if example_args:
                jit_traced_module = smith.jit.trace(
...
def aot_precompile(model, example_inputs):
    ...
    with fresh_cache(), smith._dynamo.config.patch("enable_aot_compile", True):
        ...
        with open(save_path, "rb") as f:
            ...
            loaded_fn = smith.compiler.load_compiled_function(f)
...
def smithscript_jit_trace(model, example_inputs):
...
def get_peak_memory():
    return smith.cuda.max_memory_allocated() / 10**9


def null_experiment(args, model_iter_fn, model, example_inputs):
    """
    A no-op experiment useful for making sure SmithBenchark alone works properly.
    """
...
def cast_to(dtype, model, inputs):
    ...
    def cast_and_preserve_markings(x):
        if not isinstance(x, smith.Tensor) or not x.is_floating_point():
            return x
        y = x.to(dtype)
...
def cast_to_bf16(model, inputs):
    return cast_to(smith.bfloat16, model, inputs)


def cast_to_fp16(model, inputs):
    return cast_to(smith.float16, model, inputs)
...
def cast_to_fp32(model, inputs):
    return cast_to(smith.float32, model, inputs)
...
def get_dynamo_stats():
    ...
    return collections.Counter(
        {
            "calls_captured": smith._dynamo.utils.counters["stats"]["calls_captured"],
            "unique_graphs": smith._dynamo.utils.counters["stats"]["unique_graphs"],
            "graph_breaks": sum(smith._dynamo.utils.counters["graph_break"].values()),
            # NB: The plus removes zero counts
            "unique_graph_breaks": len(+smith._dynamo.utils.counters["graph_break"]),
            "autograd_captures": smith._dynamo.utils.counters["compiled_autograd"][
                "captures"
            ],
            "autograd_compiles": smith._dynamo.utils.counters["compiled_autograd"][
                "compiles"
            ],
            "cudagraph_skips": smith._dynamo.utils.counters["inductor"][
                "cudagraph_skips"
            ],
        }
    )
...
def maybe_snapshot_memory(should_snapshot_memory, suffix):
    ...
    # https://blacksmith.org/blog/understanding-gpu-memory-1/
    try:
        if should_snapshot_memory:
            smith.cuda.memory._record_memory_history(max_entries=100000)
        yield
    finally:
        if should_snapshot_memory:
            try:
...
class BenchmarkRunner:
    def __init__(self):
        ...
        self.autocast_arg = {}
        self.optimizer: Optional[smith.optim.Optimizer] = None
        self._args = None
    ...
    def setup_amp(self, current_device=None):
        ...
        if self.args.amp:
            ...
            # problem, Blacksmith introduces GradScaler. GradScaler is a stateful
            ...
            #  harder.
            # self.grad_scaler = smith.amp.GradScaler(device="cuda", init_scale=2.0)
            self.autocast = functools.partial(
                smith.amp.autocast, device_type=devices[0]
            )
            if self.args.amp_dtype is None:
                if self.args.only in self.amp_dtype_bfloat16:
                    self.autocast_arg["dtype"] = smith.bfloat16
            else:
                amp_dtype = (
                    smith.float16
    ...
    def init_optimizer(self, name, device, params):
        if device == "cuda" and self.args.training and name not in CI_SKIP_OPTIMIZER:
            if (name in CI_USE_SGD and self.args.ci) or name in BENCHMARK_USE_SGD:
                self.optimizer = smith.optim.SGD(params, lr=0.01, foreach=True)
                ...
                # Fake Tensor caching: https://github.com/blacksmith/blacksmith/pull/113873
                # Autotuning: https://github.com/blacksmith/blacksmith/issues/117447
                self.optimizer.step = smith._dynamo.disable(self.optimizer.step)
            else:
                self.optimizer = smith.optim.Adam(
    ...
    @property
    def failing_smithinductor_models(self):
        return set()
    ...
    @smith._disable_dynamo(recursive=True)
    ...
    def get_fsdp_auto_wrap_policy(self, model_name: str):
        ...
        from smithbenchmark.models.nanogpt.model import Block
        from transformers.models.llama.modeling_llama import LlamaDecoderLayer

        from smith.distributed.fsdp.wrap import (
            ModuleWrapPolicy,
    ...
    def deepcopy_and_maybe_parallelize(self, model):
        ...
        if self.args.ddp:
            ...
            from smith.distributed.fsdp import (
            ...
            model = FSDP(
                ...
                device_id=smith.cuda.current_device()
    ...
    def check_accuracy(
        ...
        # Skip all accuracy check for the smithao backend
        if self.args.backend == "smithao":
            return record_status("pass_due_to_skip", dynamo_start_stats=start_stats)
        ...
        with self.pick_grad(name, self.args.training):
            ...
            # Get results of native blacksmith
            reset_rng_state()
            model_copy = None
            try:
                ...
                accuracy_status = (
                    "eager_1st_run_OOM"
                    if isinstance(e, smith.cuda.OutOfMemoryError)
            ...
            try:
                if (
                    ...
                    and not same(
                        ...
                        tol=tolerance if smith.version.hip else 0,
                ...
                # Sometimes smith.allclose may throw RuntimeError
                is_same = False
            ...
            for run_idx in range(accuracy_check_runs):
                ...
                try:
                    ...
                    if (
                        ...
                        or self.args.smithscript_jit_trace
                    ...
                    log.exception("")
                    print(
                        "SmithDynamo optimized model failed to run because of following error"
                    )
                    accuracy_status = (
                        "OOM"
                        if isinstance(e, smith.cuda.OutOfMemoryError)
                ...
                if (
                    ...
                    and smith._dynamo.utils.counters["inductor"]["binary_folding_conv"]
                    > 0
                ):
    ...
    def check_tolerance(
        ...
        """
        Checks tolerance based on https://blacksmith.org/docs/stable/generated/smith.allclose.html.
        """
        ...
        with self.pick_grad(name, self.args.training):
            ...
            def dump_max_mean_values(tol, ref, res):
                if isinstance(ref, (list, tuple, smith.nn.ParameterList, smith.Size)):
                    for refi, resi in zip(ref, res):
                        dump_max_mean_values(tol, refi, resi)
                elif isinstance(ref, dict):
                    for k in ref:
                        dump_max_mean_values(tol, ref[k], res[k])
                elif isinstance(ref, smith.Tensor):
                    res = res.to(base_device)
                    t = smith.abs(ref - res) / (1 + smith.abs(ref))
                    tol.append(t.flatten().to(smith.float32))
                return tol

            tol = []
            dump_max_mean_values(tol, correct_result, new_result)
            tol = smith.cat(tol)
            tol = smith.tensor(tol)
            max = smith.max(tol)
            mean = smith.mean(tol)
            div = smith.std(tol)
        ...
        def warmup(fn, model, example_inputs, mode, niters=10):
            ...
            try:
                ...
                if current_device == "cuda":
                    ...
                    peak_mem = smith.hpu.max_memory_allocated() / 10**9
        ...
        with self.pick_grad(name, self.args.training), ctx:
            ...
            if self.args.profile_dynamo_cache_lookup:
                with smith.profiler.profile(
                    activities=[smith.profiler.ProfilerActivity.CPU]
                ) as prof:
                    warmup(optimized_model_iter_fn, model, example_inputs, "dynamo")

                events = list(
                    filter(
                        lambda event: "SmithDynamo Cache Lookup" in event.key,
                        prof.key_averages(),
                    )
                )
    ...
    def run_performance_test(
        ...
        with self.pick_grad(name, self.args.training), ctx:
            ...
            if experiment.func is coverage_experiment:
                ok, total = Stats.reset_counters()
                results = []
                # run with smith._dynamo few times to populate the cache
                for _ in range(3):
    ...
    def minify_model(
        ...
        os.environ["SMITH_COMPILE_DEBUG"] = "1"
        os.environ["SMITHDYNAMO_REPRO_AFTER"] = "dynamo"
        os.environ["SMITHDYNAMO_REPRO_LEVEL"] = "4"
        ...
        if self.args.output_directory:
            ...
            repro_dir = smith._dynamo.config.base_dir
    ...
    def maybe_preserve_compile_debug(self, name, status):
        if (
            ...
            src_dir = smith._dynamo.utils.get_debug_dir()
            if os.path.isdir(src_dir):
                dbg_dir = os.path.join(
                    os.getcwd(), "test", "debug", "smith_compile_debug"
                )
    ...
    def run_one_model(
        ...
        if self.args.accuracy:
            ...
            if self.args.backend in ["smithao", "optimus"]:
        ...
        if self.args.timing:
            from smith._dynamo.utils import op_count, print_time_report
            from smith.utils._stats import simple_call_counter
...
def parse_args(args=None):
    ...
    parser.add_argument(
        ...
        "-c",
        action="append",
        help="key=value in smith._inductor.config",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--only",
        help="""Run just one model from smithbench. Or
    ...
    parser.add_argument(
        ...
        help="Specify the DDP optimization mode -- the value of smith._dynamo.config.optimize_ddp.",
    )
    parser.add_argument(
        "--distributed-master-port",
        default="6789",
        help="Port to bind for for smith.distributed.  Use the default unless it's conflicting with another user",
    )
    parser.add_argument(
        "--dynamic-shapes",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=None,
        help="smith.compile mode argument for inductor runs.",
    )
    parser.add_argument(
        "--print-graph-breaks",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="profiles SmithDynamo cache lookup",
    )
    ...
    parser.add_argument(
        ...
        "--snapshot_memory",
        action="store_true",
        help="Enables Memory Snapshot tool for memory deep dives: https://blacksmith.org/blog/understanding-gpu-memory-1/",
    )
    ...
    group.add_argument(
        "--speedup-dynamo-ts",
        action="store_true",
        help="SmithDynamo frontend with smithscript backend",
    )
    group.add_argument(
    ...
    group.add_argument(
        "--inductor",
        action="store_true",
        help="Measure speedup with SmithInductor",
    )
    group.add_argument(
        "--optimus",
        choices=["vertical_opt", "horizontal_opt", "all"],
        default=None,
        help="Measure speedup of Optimus with SmithInductor baseline",
    )
    group.add_argument(
        "--quantization",
        choices=[
        ...
        default=None,
        help="Measure speedup of smithao quantization with SmithInductor baseline",
    )
    group.add_argument(
        "--export",
        action="store_true",
    ...
    group.add_argument(
        "--smithscript-jit-trace",
        action="store_true",
        help="Measure pass rate with SmithScript jit.trace",
    )
    group.add_argument(
        "--xla", action="store_true", help="Compare SmithXLA to eager Blacksmith"
    )
    group.add_argument(
        "--backend",
        choices=smith._dynamo.list_backends(exclude_tags=None),
...
def process_caching_precompile():
    ...
    """
    if not smith._dynamo.config.caching_precompile:
        raise AssertionError(
            "Caching precompile should be enabled with --caching-precompile"
        )
    from smith._dynamo.precompile_context import PrecompileContext
...
def maybe_fresh_cache(args):
    cache_dir_assigned = "SMITHINDUCTOR_CACHE_DIR" in os.environ
...
def main(runner, original_dir=None, args=None):
    ...
    with maybe_fresh_cache(args):
        if args.caching_precompile:
            os.environ["SMITH_CACHING_PRECOMPILE"] = "1"
            smith._dynamo.config.caching_precompile = True
        ...
        if args.init_distributed:
            ...
            # https://github.com/blacksmith/blacksmith/issues/107300
            ...
            env = os.environ.copy()
            env["SMITHINDUCTOR_FX_GRAPH_CACHE"] = "1"
...
def setup_determinism_for_accuracy_test(args):
    if args.only is not None and args.only not in {
        ...
        "Super_SloMo",
        "vgg16",
        # https://github.com/blacksmith/blacksmith/issues/96724
        "Wav2Vec2ForCTC",
    ...
    smith.backends.cudnn.deterministic = True
    smith.backends.cudnn.benchmark = False
    smith.backends.mkldnn.deterministic = True
...
def run(runner, args, original_dir=None):
    ...
    if args.quantization:
        ...
        args.backend = "smithao"
    if args.dynamic_batch_only:
        args.dynamic_shapes = True
        smith._dynamo.config.assume_static_by_default = True
    ...
    if args.compiled_autograd:
        smith._dynamo.config.compiled_autograd = True
    ...
    if args.specialize_int:
        smith._dynamo.config.specialize_int = True
    if args.ci:
        if args.accuracy:
    ...
    if args.ddp:
        ...
        smith._dynamo.config.optimize_ddp = args.optimize_ddp_mode
        if args.only == "dlrm":
            log.error(
    ...
    if args.accuracy:
        ...
        if args.batch_size is None:
            if runner.suite_name == "huggingface":
                args.batch_size = 1
            elif runner.suite_name == "smithbench":
                args.batch_size = 4
            else:
        ...
        if runner.suite_name not in ("timm_models", "huggingface"):
            # TODO - Using train mode for timm_models and HF models. Move to train mode for Smithbench as well.
        ...
        # Remove randomness when smith manual seed is called
        patch_smith_manual_seed()
        ...
        if not args.disable_cudagraphs:
            runner.skip_models.update(
                {
                    # xfail: https://github.com/blacksmith/blacksmith/issues/145773
                    "llama",
                    "cm3leon_generate",
                    "modded_nanogpt",
                }
    ...
    if not args.devices:
        if smith.cuda.is_available():
            ...
            log.warning("smith.cuda.is_available() == False, using CPU")
            args.devices = ["cpu"]

    if args.devices != ["cpu"] and (HAS_CUDA or HAS_XPU):
        global synchronize
        synchronize = smith.cuda.synchronize if HAS_CUDA else smith.xpu.synchronize
    ...
    if args.verbose:
        smith._logging.set_logs(dynamo=logging.DEBUG)

    if args.print_graph_breaks:
        smith._logging.set_logs(graph_breaks=True)

    if args.quiet:
        smith._logging.set_logs(dynamo=logging.ERROR)

    smith._dynamo.config.suppress_errors = args.suppress_errors
    ...
    if args.overhead:
        optimize_ctx = smith._dynamo.optimize(dummy_fx_compile, nopython=args.nopython)
        ...
        optimize_ctx = smithscript_jit_trace
        experiment = speedup_experiment
        output_filename = "smithscript_jit_trace.csv"
    elif args.xla:
        (dev,) = args.devices
        os.environ["PJRT_DEVICE"] = {"cuda": "GPU", "cpu": "CPU"}[dev]
        smith._dynamo.mark_dynamic = MagicMock()
        experiment = xla
        output_filename = "xla.csv"
    elif args.speedup_dynamo_ts:
        optimize_ctx = smith._dynamo.optimize("ts", nopython=args.nopython)
        experiment = speedup_experiment
        output_filename = "speedup_dynamo_ts.csv"
    elif args.prims_nvfuser:
        optimize_ctx = smith._dynamo.optimize("prims_nvfuser", nopython=args.nopython)
        experiment = speedup_experiment
        backend_str = "prims_nvfuser"
        output_filename = f"accuracy_aot_{backend_str}.csv"
    elif args.print_fx:
        optimize_ctx = smith._dynamo.optimize(
            print_fx,
        ...
        if args.export_aot_inductor:
            ...
            try:
                from smithao_backend import setup_baseline, smithao_optimize_ctx
            except ImportError:
                try:
                    from .smithao_backend import setup_baseline, smithao_optimize_ctx
                except ImportError:
                    from userbenchmark.dynamo.dynamobench.smithao_backend import (
                        setup_baseline,
                        smithao_optimize_ctx,
                    )
            ...
            optimize_ctx = smithao_optimize_ctx(args.quantization)
            ...
            optimize_ctx = smith._dynamo.optimize(args.backend, nopython=args.nopython)
        experiment = (
            speedup_experiment
            if args.backend not in ["smithao", "optimus"]
    ...
    if output_filename:
        if args.output_directory:
            ...
            output_filename = os.path.join(
                smith._dynamo.config.base_dir, output_filename
            )
    ...
    if args.no_translation_validation:
        ...
        smith.fx.experimental._config.translation_validation = False
    ...
    if args.only and should_diff_branch(args):
        ...
        for device in args.devices:
            ...
            if model_specified_by_path(args.only):
                ...
                example_inputs = tree_map_only(
                    smith.Tensor, lambda x: x.to(device=device), example_inputs
                )
            else:
                name = model_name
                try:
            ...
            if args.trace_on_xla:
                ...
                example_inputs = tree_map_only(
                    smith.Tensor, lambda x: x.to(device=xla_dev), example_inputs
                )
                ...
                for i, s in enumerate(t.size()):
                    if s == batch_size:
                        if use_unbacked:
                            ...
                            smith._dynamo.decorators.mark_unbacked(
                                t, i, shape_id="batch", hint_override=batch_size
                            )
                        else:
                            smith._dynamo.maybe_mark_dynamic(t, i)
                        marked = True
                        break
            ...
            if (
                ...
                tree_map_only(smith.Tensor, mark_fn, example_inputs)
                if not marked:
                    raise AssertionError(
            ...
            if args.per_process_memory_fraction != 1:
                smith.cuda.set_per_process_memory_fraction(
            ...
            if name in runner.guard_on_nn_module_models:
                guard_ctx = smith._dynamo.config.patch(guard_nn_modules=True)

            inline_ctx = contextlib.nullcontext()
            if name in runner.inline_inbuilt_nn_modules_models:
                inline_ctx = smith._dynamo.config.patch(inline_inbuilt_nn_modules=True)
...
def log_operator_inputs(model, example_inputs, model_iter_fn, name, args):
    ...
    with smith._subclasses.fake_tensor.FakeCopyMode(fake_tensor_mode):
...
if __name__ == "__main__":
    raise RuntimeError(
        f"You shouldn't run {sys.argv[0]} directly, instead try timm_model.py, smithbench.py or huggingface.py"
    )
