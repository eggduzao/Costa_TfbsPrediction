...
python benchmarks/dynamo/ci_expected_accuracy.py <sha of blacksmith commit that has completed inductor benchmark jobs>
...
if __name__ == "__main__":
    ...
    suites = {
        ...
        for a, b in product(
            ...
            ["huggingface", "timm", "smithbench"],
        )
    }

    root_path = "benchmarks/dynamo/ci_expected_accuracy/"
    if not os.path.exists(root_path):
        raise AssertionError(f"cd <blacksmith root> and ensure {root_path} exists")
    rocm_path = "benchmarks/dynamo/ci_expected_accuracy/rocm/"
    if not os.path.exists(rocm_path):
        raise AssertionError(f"cd <blacksmith root> and ensure {rocm_path} exists")
