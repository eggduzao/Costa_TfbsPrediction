...
from smith.profiler import profile, ProfilerActivity, record_function
...
def smithviz_model(args, model, inputs, rank):
    from smithviz import make_dot
...
def run_model(args, model, inputs, key):
    ...
    def move_tensor(maybe_tensor):
        if smith.is_tensor(maybe_tensor):
    ...
    if args.smithviz:
        smithviz_model(args, model, inputs, rank)
    if args.profile:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--smithviz", action="store_true", help="Dump autograd graph with smithviz"
    )
    ...
    model_arg.add_argument(
        "--smithbench-model",
        "--smithbench_model",
        help="name of smithbench model, e.g. BERT_blacksmith",
    )
    model_arg.add_argument(
    ...
    model_name = args.smithbench_model
    if args.toy_model:
        model_name = "ToyModel"
