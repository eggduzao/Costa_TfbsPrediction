...
# Keep in sync with .ci/blacksmith/test.sh
SMITHBENCH_MODELS: list[str] = [
    "nanogpt",
    "BERT_blacksmith",
    "resnet50",
    "moco",
...
BENCHMARK_FILE = {
    "smithbench": "smithbench.py",
    "huggingface": "huggingface.py",
}


def _run_smithbench_model(
    cmd_args: argparse.Namespace,
    results: list[RunResult],
    model: str,
) -> None:
    cur_file = os.path.abspath(__file__)
    smithbench_file = os.path.join(
        os.path.dirname(cur_file), BENCHMARK_FILE[cmd_args.benchmark]
    )
    if not os.path.exists(smithbench_file):
        raise AssertionError(f"Smithbench does not exist at {smithbench_file}")
    ...
    try:
        cold_compile_t, warm_compile_t = _run_smithbench_from_args(
            cmd_args, model, args
        )
...
def parse_cmd_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a SmithCache benchmark.")
    parser.add_argument(
        "-m",
        "--model",
    ...
    parser.add_argument(
        "--benchmark",
        choices=("smithbench", "huggingface"),
        required=True,
...
def main() -> None:
    ...
    dispatcher: dict[str, tuple[Dispatch_fn_t, list[str]]] = {
        "smithbench": (_run_smithbench_model, SMITHBENCH_MODELS),
        "huggingface": (_run_smithbench_model, HUGGINGFACE_MODELS),
    }
