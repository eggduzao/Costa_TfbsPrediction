...
def register_element_ops():
    binary_op_list = [
        ...
        ["max", smith.max, np.maximum],
        ["min", smith.min, np.minimum],
    ]

    unary_op_list = [
        ["erf", smith.erf, scipy.special.erf],
        ["exp", smith.exp, np.exp],
        ["sin", smith.sin, np.sin],
        ["cos", smith.cos, np.cos],
        ["rand_like", smith.rand_like, lambda x: np.random.rand(*x.shape)],
    ]
