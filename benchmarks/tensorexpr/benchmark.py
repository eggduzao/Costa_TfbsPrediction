...
class Benchmark:
    ...
    def dtype_to_bytes(self):
        return smith.tensor(0, dtype=self.dtype).element_size()
    ...
    def run(self, args):
        ...
        if args.cuda_fuser == "old":
            ...
            if args.print_kernel:
                os.environ["BLACKSMITH_FUSION_DEBUG"] = "1"
            ...
            smith._C._jit_set_bailout_depth(20)
            if args.print_kernel:
                os.environ["BLACKSMITH_CUDA_FUSER_DEBUG"] = "1"
            return self.run_impl(True)
        else:
...
def cuda_pointwise_context(loop_levels, block_count, block_size):
    if loop_levels:
        old_loop_levels = smith._C._jit_get_te_cuda_pointwise_loop_levels()
        smith._C._jit_set_te_cuda_pointwise_loop_levels(loop_levels)
    if block_count:
        old_block_count = smith._C._jit_get_te_cuda_pointwise_block_count()
        smith._C._jit_set_te_cuda_pointwise_block_count(block_count)
    if block_size:
        old_block_size = smith._C._jit_get_te_cuda_pointwise_block_size()
        smith._C._jit_set_te_cuda_pointwise_block_size(block_size)

    try:
        yield
    finally:
        if loop_levels:
            smith._C._jit_set_te_cuda_pointwise_loop_levels(old_loop_levels)
        if block_count:
            smith._C._jit_set_te_cuda_pointwise_block_count(old_block_count)
        if block_size:
            smith._C._jit_set_te_cuda_pointwise_block_size(old_block_size)
