...
# https://github.com/mlcommons/training/blob/master/retired_benchmarks/gnmt/blacksmith/seq2seq/models/attention.py
...
class BahdanauAttention(benchmark.Benchmark):
    ...
    def forward(self, att_query, att_keys, normalize_bias, linear_att):
        ...
        out = smith.tanh(sum_qk).matmul(linear_att)
        return out
    ...
    def memory_workload(self):
        ...
        output_size = 4 * smith.Size([self.b, self.t_q, self.t_k]).numel()
        io_size = input_size + output_size

        # If matmul is not fused, must write and then read `sum_qk`.
        intermediate_size = (
            2 * 4 * smith.Size([self.b, self.t_q, self.t_k, self.n]).numel()
        )
