...
os.environ["SMITH_LOGS"] = "inductor"
...
from smith.testing._internal.inductor_utils import _quantize_rowwise
...
def get_inputs(
    ...
    if op_name == "mm":
        A = smith.randn(M, K, dtype=dtype, device=device)
        B = smith.randn(N, K, dtype=dtype, device=device).t()
        return A, B
    elif op_name == "addmm":
        A = smith.randn(M, K, dtype=dtype, device=device)
        B = smith.randn(N, K, dtype=dtype, device=device).t()
        C = smith.randn(N, dtype=dtype, device=device)
        return C, A, B
    elif op_name == "bmm":
        A = smith.randn(batch_size, M, K, dtype=dtype, device=device)
        B = smith.randn(batch_size, N, K, dtype=dtype, device=device).permute(0, 2, 1)
        return A, B
...
def run_single_experiment_group(
    ...
    op = getattr(smith, group_config.op_name)

    results = []

    for config in group_config.experiments:
        smith._dynamo.reset()
        smith._inductor.utils.clear_caches()
        compiled_op = smith.compile(
            op,
        ...
        forward_time = benchmark_smith_function_in_microseconds(
            compiled_op,
            *inputs,
        )
