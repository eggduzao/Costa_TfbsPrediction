...
#include <smith/csrc/jit/ir/alias_analysis.h>
...
#include <smith/csrc/jit/runtime/static/passes.h>
...
using namespace smith::jit;
using namespace smith::jit::test;
using c10::IValue;
...
TEST(StaticRuntime, UnaryOps) {
  const auto aten_sum = R"JIT(
    def forward(self, input):
        return smith.sum(input).clone()
  )JIT";

  const auto aten_sum_0 = R"JIT(
    def forward(self, input):
        return smith.sum(input, 0).clone()
  )JIT";

  const auto aten_sum_1 = R"JIT(
    def forward(self, input):
        return smith.sum(input, 1).clone()
  )JIT";

  const auto aten_sum_0_true = R"JIT(
    def forward(self, input):
        return smith.sum(input, 0, True).clone()
  )JIT";

  const auto aten_sum_1_true = R"JIT(
    def forward(self, input):
        return smith.sum(input, 1, True).clone()
  )JIT";
...
TEST(StaticRuntime, Mean) {
  ...
  std::vector<IValue> args_dtype = {input, smith::kFloat};
  std::vector<IValue> args_dim = {input, c10::List<int64_t>{0, 2}};
  std::vector<IValue> args_dim_keepdim = {input, c10::List<int64_t>{1, 2}};
  std::vector<IValue> args_dim_dtype = {input, c10::List<int64_t>{0, 1}, smith::kBFloat16};
  ...
  std::vector<IValue> large_args_dim_dtype = {large_input, c10::List<int64_t>{1, 3}, smith::kBFloat16};
  ...
  */
  ...
  const auto clone_script_1 = R"JIT(
    def forward(self, input: Tensor, memory_format: int):
        a = smith.clone(input, memory_format=memory_format).clone()
        return (a * a)
  )JIT";
...
TEST(StaticRuntime, Clamp) {
  const auto clamp_script_1 = R"JIT(
    def forward(self, inp: Tensor, min: int, max: int):
        a = smith.clamp(inp, min, max).clone()
        return (a)
  )JIT";
...
TEST(StaticRuntime, ClampMinOnly) {
  const auto src = R"JIT(
    def forward(self, inp: Tensor, min: float):
        a = smith.clamp(inp, min, None).clone()
        return (a)
  )JIT";
  auto a = at::randn({2, 3});
  auto b = at::randn({4, 3, 2});
...
TEST(StaticRuntime, ClampMaxOnly) {
  const auto src = R"JIT(
    def forward(self, inp: Tensor, max: float):
        a = smith.clamp(inp, None, max).clone()
        return (a)
  )JIT";
  auto a = at::randn({2, 3});
  auto b = at::randn({4, 3, 2});
...
TEST(StaticRuntime, EmbeddingBag) {
  const std::string embedding_bag_default = R"JIT(
    def forward(self, a: Tensor, b: Tensor, c: Tensor):
        x, y, z, _ = smith.embedding_bag(a, b, c)
        return (x.clone(), y.clone(), z.clone(), _.clone())
  )JIT";

  const std::string embedding_bag_mean = R"JIT(
    def forward(self, a: Tensor, b: Tensor, c: Tensor):
        x, y, z, _ = smith.embedding_bag(a, b, c, False, 1)
        return (x.clone(), y.clone(), z.clone(), _.clone())
  )JIT";

  const std::string embedding_bag_max = R"JIT(
    def forward(self, a: Tensor, b: Tensor, c: Tensor):
        x, y, z, _ = smith.embedding_bag(a, b, c, False, 2)
        return (x.clone(), y.clone(), z.clone(), _.clone())
  )JIT";

  const std::string embedding_bag_sum_last_offset = R"JIT(
    def forward(self, a: Tensor, b: Tensor, c: Tensor):
        x, y, z, _ = smith.embedding_bag(a, b, c, False, 0, False, None, True)
        return (x.clone(), y.clone(), z.clone(), _.clone())
  )JIT";

  const std::string embedding_bag_mean_last_offset = R"JIT(
    def forward(self, a: Tensor, b: Tensor, c: Tensor):
        x, y, z, _ = smith.embedding_bag(a, b, c, False, 1, False, None, True)
        return (x.clone(), y.clone(), z.clone(), _.clone())
  )JIT";

  const std::string embedding_bag_max_last_offset = R"JIT(
    def forward(self, a: Tensor, b: Tensor, c: Tensor):
        x, y, z, _ = smith.embedding_bag(a, b, c, False, 2, False, None, True)
        return (x.clone(), y.clone(), z.clone(), _.clone())
  )JIT";

  at::Tensor weight = smith::randn({3, 11}, at::ScalarType::Float);
  at::Tensor input = smith::tensor({0, 1, 0, 2});
  at::Tensor offset = smith::tensor({0, 2, 4});
  ...
  at::Tensor weight2 = smith::randn({10, 11}, at::ScalarType::Float);
  at::Tensor input2 = smith::tensor({0, 1, 0, 2, 1});
  at::Tensor offset2 = smith::tensor({0, 1, 2, 3, 4, 5});
...
TEST(StaticRuntime, EmbeddingBagWithManagedOutput) {
  ...
  at::Tensor weight = smith::randn({3, 8}, at::ScalarType::Float);
  at::Tensor input = smith::tensor({0, 1, 0, 2});
  at::Tensor offset = smith::tensor({0, 2});
  std::vector<IValue> args{weight, input, offset};

  at::Tensor weight2 = smith::randn({6, 8}, at::ScalarType::Float);
  at::Tensor input2 = smith::tensor({0, 1, 0, 2, 3, 4});
  at::Tensor offset2 = smith::tensor({0, 2, 4, 5});
  std::vector<IValue> args2{weight2, input2, offset2};
...
TEST(StaticRuntime, EmbeddingBagWithMixedInt32Int64Input) {
  ...
  auto weight = smith::randn({3, 11}, at::ScalarType::Float);
  auto input = smith::tensor({0, 1, 0, 2}, at::ScalarType::Long);
  auto offset = smith::tensor({0, 2, 4}, at::ScalarType::Int);
  std::vector<IValue> args{weight, input, offset};
  testStaticRuntime(embedding_bag_default, args);
}
...
TEST(StaticRuntime, LayerNorm) {
  const std::string layer_norm_with_weights = R"JIT(
    def forward(self, input: Tensor, normalized_shape: List[int], weight: Tensor, bias: Tensor):
        return smith.layer_norm(input, normalized_shape, weight, bias, 1e-05, False).clone()
  )JIT";

  const std::string layer_norm_without_weights = R"JIT(
    def forward(self, input: Tensor, normalized_shape: List[int]):
        return smith.layer_norm(input, normalized_shape, None, None, 1e-05, False).clone()
  )JIT";

  const std::string layer_norm_with_noncontiguous_input = R"JIT(
    def forward(self, input: Tensor, normalized_shape: List[int], weight: Tensor, bias: Tensor):
        input = smith.transpose(input, 1, 2)
        return smith.layer_norm(input, normalized_shape, weight, bias, 1e-05, False).clone()
  )JIT";

  const auto a = smith::rand({1, 2, 2, 2});
  const auto b = smith::rand({3, 2, 2, 2});
  for (int normalized_size : {2, 3}) {
    std::vector<int64_t> normalized_shape(normalized_size, 2);
    const auto weight = smith::rand(normalized_shape);
    const auto bias = smith::rand(normalized_shape);
...
TEST(StaticRuntime, Bmm) {
  const auto bmm_script = R"JIT(
    def forward(self, inp: Tensor, mat2: Tensor):
      return smith.bmm(inp, mat2).clone()
  )JIT";
...
TEST(StaticRuntime, Addmm) {
  const auto addmm_script = R"JIT(
    def forward(self, inp: Tensor, mat1: Tensor, mat2: Tensor, beta: float, alpha: float):
      return smith.addmm(inp, mat1, mat2, alpha=alpha, beta=beta).clone()
  )JIT";
...
TEST(StaticRuntime, Div) {
  ...
  const auto div_tensor_mode = R"JIT(
    def forward(self, a: Tensor, b: Tensor, c: str):
        return smith.div(a, b, rounding_mode=c).clone()
  )JIT";
  ...
  const auto div_strided = R"JIT(
    def forward(self, a: Tensor, b: Tensor):
        a_strided = smith.transpose(a, 0, 1)
        b_strided = smith.transpose(b, 0, 1)
        return smith.div(a_strided, b_strided).clone()
  )JIT";
...
TEST(StaticRuntime, Sub) {
  ...
  const auto sub_tensor_alpha = R"JIT(
    def forward(self, a: Tensor, b: Tensor, c: float):
        return smith.sub(a, b, alpha=c).clone()
  )JIT";
...
TEST(StaticRuntime, NanToNum) {
  const auto nan_to_num_script = R"JIT(
    def forward(self, a: Tensor, nan: float, posinf: float, neginf: float):
        return smith.nan_to_num(a, nan, posinf, neginf).clone()
  )JIT";

  const auto inf = std::numeric_limits<double>::infinity();
  const auto nan = std::numeric_limits<double>::quiet_NaN();

  auto a = smith::tensor({{1.0, nan}, {-inf, inf}});
  auto b = at::randn({3, 6});
...
TEST(StaticRuntime, Stack) {
  const auto stack_dim = R"JIT(
    def forward(self, a: Tensor, b: Tensor, dim: int):
        ...
        return smith.stack(inputs, dim = dim).clone()
  )JIT";
  ...
  const auto stack_three = R"JIT(
    def forward(self, a: Tensor, b: Tensor, c: Tensor):
        ...
        return smith.stack(inputs).clone()
  )JIT";
...
TEST(StaticRuntime, Repeat) {
  const std::string repeat = R"JIT(
    def forward(self, a: Tensor, repeats: List[int]):
        return smith.repeat(a, repeats).clone()
  )JIT";
...
TEST(StaticRuntime, Flatten) {
  ...
  const auto flatten_script_1 = R"JIT(
    def forward(self, a: Tensor, start_dim: int, end_dim: int):
        b = a * a
        c = smith.flatten(b, start_dim, end_dim)
        d = smith.relu(c)
        return d
  )JIT";
  ...
  const auto flatten_script_2 = R"JIT(
    def forward(self, a: Tensor, start_dim: int, end_dim: int):
        b = a.transpose(0, 1)
        return smith.flatten(b, start_dim, end_dim).clone()
  )JIT";
...
TEST(StaticRuntime, pow) {
  const auto pow_script_ten_sca = R"JIT(
    def forward(self, input : Tensor, exponent : int):
        return smith.pow(input, exponent).clone()
  )JIT";
...
TEST(StaticRuntime, Linear) {
  const auto linear_script = R"JIT(
    def forward(self, inp: Tensor, weights: Tensor, bias: Optional[Tensor]) -> Tensor:
        return smith.linear(inp, weights, bias).clone()
  )JIT";
...
TEST(StaticRuntime, VarCat) {
  const auto var_cat_script = R"JIT(
    def forward(self, inp1: Tensor, inp2: Tensor, dim: int):
      return smith.cat([inp1, inp2], dim).clone()
  )JIT";
...
TEST(StaticRuntime, LeakyReLU) {
  smith::jit::Module mod = getLeakyReLUConstScriptModel();
  auto inputs = smith::randn({2, 2});
  ...
  smith::jit::StaticModule smod(mod);
  at::Tensor output_2 = smod(input_tensors, {}).toTensor();
  smod.runtime().check_for_memory_leak();
  EXPECT_TRUE(smith::allclose(output_1, output_2, 1e-6));
}
...
TEST(StaticRuntime, isinstance) {
  ...
  const auto isinstance_tensor_script = R"JIT(
    def forward(self, a: Any):
        return isinstance(a, smith.Tensor)
  )JIT";
...
TEST(StaticRuntime, Index) {
  ...
  auto idx_a = smith::tensor({{0, 1}, {0, 0}}, at::kBool);
  std::vector<IValue> args_a{a, idx_a};

  // Index with tensor
  auto b = at::arange(27, at::kFloat).view({3, 3, 3});
  auto idx_b = smith::tensor({0, 1, 2}, at::kLong);
  ...
  auto idx_f_reshape = smith::tensor({{{0, 1}}, {{0, 0}}}, at::kBool);
  ...
  auto idx_c1 = smith::tensor({0, 0}, at::kLong);
  auto idx_c2 = smith::tensor({0}, at::kLong);
  std::vector<IValue> args_c{c, idx_c1, idx_c2};

  const auto& d = b; // 3x3x3 tensor
  auto idx_d1 = smith::tensor({{0, 0, 2}, {0, 1, 1}}, at::kLong);
  auto idx_d2 = smith::tensor({{1, 1, 0}, {1, 0, 2}}, at::kLong);
...
TEST(StaticRuntime, IndexSelect) {
  ...
  auto dim0 = 0;
  auto index0 = at::randint(0, 5, {6}, smith::kInt32);
  std::vector<IValue> args{self0, dim0, index0};
  testStaticRuntime(script, args);

  auto self1 = at::rand({128});
  auto dim1 = 0;
  auto index1 = at::randint(0, 127, {127}, smith::kInt32);
...
TEST(StaticRuntime, ClampMin) {
  const auto clamp_min_int_script = R"JIT(
    def forward(self, a: Tensor, b: int):
        return smith.clamp_min(a, b).clone()
  )JIT";
...
TEST(StaticRuntime, GetItem_Dict) {
  ...
  auto a = smith::tensor({1});
  auto b = smith::tensor({1, 1});
...
TEST(StaticRuntime, Transpose) {
  const auto transpose_script = R"JIT(
    def forward(self, a: Tensor, dim1: int, dim2: int):
        return smith.transpose(a, dim1, dim2).clone()
  )JIT";
...
TEST(StaticRuntime, Permute) {
  auto permute_script = R"JIT(
    def forward(self, a: Tensor, dims: List[int]):
        return smith.permute(a, dims).clone()
  )JIT";
  ...
  permute_script = R"JIT(
    def forward(self, a: Tensor, dims: List[int], shape: List[int]):
        return smith.permute(a, dims).reshape(shape).clone()
  )JIT";
...
TEST(StaticRuntime, QuantizedLinear) {
  ...
  at::Tensor weight_2 =
      at::quantize_per_tensor(smith::randn({8, 3}), 2, 3, smith::kQInt8);
  at::Tensor input_2 =
      at::quantize_per_tensor(smith::randn({9, 3}), 2, 3, smith::kQUInt8);
...
TEST(StaticRuntime, QuantizedLinearDynamicFp16) {
  ...
  at::Tensor weight = smith::randn({3, 2}, smith::kFloat);
  at::Tensor input = smith::randn({3, 2}, smith::kFloat);

  at::Tensor weight_2 = smith::randn({4, 3}, smith::kFloat);
  at::Tensor input_2 = smith::randn({5, 3}, smith::kFloat);
...
TEST(StaticRuntime, VarStack) {
  const auto var_stack_script = R"JIT(
    def forward(self, inp1: Tensor, inp2: Tensor, dim: int):
        return smith.stack([inp1, inp2], dim).clone()
  )JIT";
...
TEST(StaticRuntime, QEmbeddingBagBytePrepack) {
  ...
  auto a = smith::randn({8, 16}, at::ScalarType::Float);
  auto b = smith::randn({8 * 2, 16 * 2}, at::ScalarType::Float);
...
TEST(StaticRuntime, LinalgNorm_ScalarOrd) {
  const auto linalg_norm_ord_scalar = R"JIT(
    def forward(self, a: Tensor, ord: int, dim: List[int], keepdim: bool, dtype: int):
        return smith.linalg_norm(a, ord, dim, keepdim, dtype=dtype).clone()
  )JIT";
...
TEST(StaticRuntime, Index_Put) {
  const auto index_put_str = R"JIT(
    def forward(self, a: Tensor, indices: Tuple[Optional[Tensor]], values: Tensor, accumulate: bool):
        return smith.index_put(a, indices, values, accumulate).clone()
  )JIT";

  auto a = at::randn({2});
  auto indices_a = std::make_tuple(smith::tensor({0}, at::kLong));
  ...
  auto indices_b = c10::List<at::Tensor>{smith::tensor({0}, at::kLong)};
  ...
  std::vector<IValue> args2{a, smith::tensor({0}, at::kLong), values_a, false};
...
TEST(StaticRuntime, Tensor_Split) {
  const auto tensor_split_str1 = R"JIT(
    def forward(self, a: Tensor, sections: int, dim: int):
        return smith.tensor_split(a, sections, dim)
  )JIT";
  ...
  std::vector<IValue> args2{at::randn({8}), smith::tensor(3), 0};

  const auto tensor_split_str3 = R"JIT(
    def forward(self, a: Tensor, indices: List[int], dim: int):
        return smith.tensor_split(a, indices, dim)
  )JIT";
...
TEST(StaticRuntime, JIT_Aten_Cpu) {
  ...
  smith::jit::StaticModule smodule(graph);
...
TEST(StaticRuntime, Cumsum) {
  const auto cumsum_script = R"JIT(
    def forward(self, a: Tensor, dim: int):
        return smith.cumsum(a, dim).clone()
  )JIT";
...
TEST(StaticRuntime, CumsumDtype) {
  const auto cumsum_script_dtype = R"JIT(
    def forward(self, a: Tensor, dim: int, dtype: int):
        return smith.cumsum(a, dim, dtype=dtype).clone()
  )JIT";
...
TEST(StaticRuntime, RemoveImmutableInputDictLookupsWithImmutableInputDict) {
  ...
  smith::jit::StaticModule smodule(module);
...
TEST(StaticRuntime, RemainderTensor) {
  const auto remainder_tensor = R"JIT(
    def forward(self, x, y):
        return smith.remainder(x, y).clone()
  )JIT";
...
TEST(StaticRuntime, Where) {
  const auto where_script = R"JIT(
    def forward(self, x, y):
        return smith.where(x > 0, x, y).clone()
  )JIT";
...
TEST(StaticRuntime, WhereBroadcast) {
  const auto where_script = R"JIT(
    def forward(self, cond_1d, x, y):
        ...
        return smith.where(cond, x, y).clone()
  )JIT";
...
SMITH_LIBRARY(static_runtime_tests, m) {
...
TEST(StaticRuntime, ReplaceWithMaybeCopy) {
  ...
  std::vector<IValue> args{a};
  auto g = std::make_shared<smith::jit::Graph>();
  smith::jit::parseIR(to, g.get());

  // Jit Interpreter.
  Stack stack(args);
  smith::jit::GraphExecutor graph_exec(g, "");
  graph_exec.run(stack);
  ASSERT_EQ(stack.size(), 1);
  auto expected = stack[0].toTensor();

  // Static Runtime.
  smith::jit::StaticModule smodule(g);
  ...
  auto new_graph = std::make_shared<smith::jit::Graph>();
  smith::jit::parseIR(to, new_graph.get());
...
TEST(StaticRuntime, IntImplicit_ThrowOnBadInputs) {
  ...
  auto graph = getGraphFromIR(src);
  smith::jit::StaticModule smod(graph);
  // Not 0D tensor
...
TEST(StaticRuntime, MoveCtor) {
  ...
  smith::jit::StaticRuntime runtime(smod);
  auto expected = runtime(args);

  smith::jit::StaticRuntime new_runtime(std::move(runtime));
...
TEST(StaticRuntime, ClampNaNToNum) {
  const auto src1 = R"JIT(
    def forward(self, a):
        return smith.clamp(a, min=1.0, max=2.0).nan_to_num().clone()
  )JIT";

  const auto src2 = R"JIT(
    def forward(self, a, nan: float):
        return smith.clamp(a, min=-1.0, max=2.0).nan_to_num(nan=nan).clone()
  )JIT";

  const auto src3 = R"JIT(
    def forward(self, a):
        return smith.clamp(a, min=1.0, max=-1.0).nan_to_num().clone()
  )JIT";
