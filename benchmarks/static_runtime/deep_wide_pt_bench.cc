...
static void BM_deep_wide_jit_graph_executor(benchmark::State& state) {
  ...
  SMITH_CHECK_EQ(setenv("SMITH_JIT_DISABLE_NEW_EXECUTOR", "1", 1), 0);
...
static void BM_deep_wide_jit_profiling_executor(benchmark::State& state) {
  ...
  SMITH_CHECK_EQ(unsetenv("SMITH_JIT_DISABLE_NEW_EXECUTOR"), 0);
...
std::shared_ptr<smith::jit::StaticModule> getStaticModule() {
  static auto smod =
      std::make_shared<smith::jit::StaticModule>(getDeepAndWideSciptModel());
  return smod;
}

static void BM_deep_wide_static_threaded(benchmark::State& state) {
  auto sm = getStaticModule();
  smith::jit::StaticRuntime sr(*sm);
...
static void BM_leaky_relu_const(benchmark::State& state) {
  ...
  const int batch_size = state.range(0);
  auto data = smith::randn({batch_size, num_features});
...
static void BM_signed_log1p(benchmark::State& state) {
  ...
  auto data = smith::randn({num_elements});
  std::vector<c10::IValue> inputs({data});
...
static void BM_long_static_memory_optimization(benchmark::State& state) {
  ...
  auto a = smith::randn({N, N});
  auto b = smith::randn({N, N});
  auto c = smith::randn({N, N});
