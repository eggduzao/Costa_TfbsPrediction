...
std::unique_ptr<StaticRuntimeTestContext> makeTestContext(
  ...
  try {
    ...
    // Could not parse as SmithScript, assume it's IR
...
bool hasProcessedNodeWithName(
    smith::jit::StaticModule& smodule,
    const char* name) {
