...
#include <smith/csrc/jit/runtime/static/fusion.h>
#include <smith/csrc/jit/runtime/static/impl.h>
#include <smith/csrc/jit/runtime/static/memory_planner.h>
...
const auto sigmoid_inplace_script = R"JIT(
  def forward(self, inp: Tensor):
      a = smith.sigmoid(inp, out=inp).clone()
      return (a)
)JIT";
...
TEST(StaticModule, ValueGroup) {
  ...
  auto input_graph = std::make_shared<smith::jit::Graph>();
  smith::jit::parseIR(src, input_graph.get());
  smith::jit::StaticModule sm(input_graph);
...
TEST(StaticRuntime, LongModel) {
  smith::jit::Module mod = getLongScriptModel();
  auto a = smith::randn({2, 2});
  auto b = smith::randn({2, 2});
  auto c = smith::randn({2, 2});
  ...
  EXPECT_TRUE(
      smith::allclose(output_1, output_2, /*rtol=*/1e-5, /*atol=*/1e-7));
}

TEST(StaticRuntime, TrivialModel) {
  smith::jit::Module mod = getTrivialScriptModel();
  auto a = smith::randn({2, 2});
  auto b = smith::randn({2, 2});
  auto c = smith::randn({2, 2});
...
TEST(StaticRuntime, DeepWide) {
  ...
  smith::jit::Module mod = getDeepAndWideSciptModel();
  smith::jit::StaticModule smod(mod);

  for (int batch_size : {1, 8, 32}) {
    for (int i = 0; i < 2; ++i) {
      auto ad_emb_packed = smith::randn({batch_size, 1, embedding_size});
      auto user_emb = smith::randn({batch_size, 1, embedding_size});
      auto wide = smith::randn({batch_size, num_features});
      ...
      smod.runtime().check_for_memory_leak();
      EXPECT_TRUE(
          smith::allclose(output_1, output_2, /*rtol=*/1e-5, /*atol=*/1e-5));
    }
  }
}
...
TEST(StaticRuntime, KWargsAPI_1) {
  ...
  smith::jit::StaticModule smod(module);
  ...
  for (auto enable_out_variant : {true, false}) {
    for (auto optimize_memory : {true, false}) {
      for (auto manage_output_tensors : {true, false}) {
        ...
        smith::jit::StaticModuleOptions opts{
            enable_out_variant, optimize_memory, manage_output_tensors};
        smith::jit::StaticModule smod(mod, false, opts);
...
TEST(
  ...
  smith::jit::parseIR(test_graph, g.get());
  smith::jit::StaticModuleOptions opts{
  ...
  auto a = at::randn({2, 2});
  std::vector<at::IValue> args{a};
  smith::jit::StaticModule smod(g, opts);
  smith::jit::StaticRuntime runtime(smod);
  // Profile run.
  {
...
TEST(StaticRuntime, FusionPass) {
  ...
  for (int batch_size : {1, 8, 32}) {
    for (int i = 0; i < 2; ++i) {
      smith::jit::Module module = getDeepAndWideSciptModel();
      auto ad_emb_packed = smith::randn({batch_size, 1, embedding_size});
      auto user_emb = smith::randn({batch_size, 1, embedding_size});
      auto wide = smith::randn({batch_size, num_features});
      ...
      for (const auto& n : module.get_method("forward").graph()->nodes()) {
        if (n->kind() == smith::jit::prim::StaticSubgraph) {
          hit = true;
        }
      }
      EXPECT_TRUE(hit);
...
TEST(
  ...
  std::array<IValue, 2> values = {smith::randn({2, 3}), smith::randn({3, 1})};
...
namespace {
SMITH_LIBRARY_FRAGMENT(static_runtime_tests, m) {
  m.def(smith::schema(
...
TEST(StaticModule, IncorrectTypesPassed) {
  ...
  const std::string kwargs_tensor_script = R"JIT(
    def forward(self, x: Tensor = smith.randn((1, ))):
        return x
  )JIT";
...
TEST(EliminateExtraPermuteOps, FusesSumCorrectly) {
  const auto src = R"JIT(
    def forward(self, x):
        y = smith.permute(x, (0, 2, 1))
        z = smith.sum(y, dim=-1)
        return z
  )JIT";
  smith::jit::Module mod("m");
  mod.define(src);
...
TEST(EliminateExtraPermuteOps, FusesSoftmaxCorrectly) {
  const auto src = R"JIT(
    def forward(self, x):
        a = smith.permute(x, [0, 2, 1])
        b = smith.softmax(a, 2)
        c = smith.permute(b, [0, 2, 1])
        return c.clone()
  )JIT";
  smith::jit::Module mod("m");
  mod.define(src);
...
TEST(EliminateExtraPermuteOps, DoesNotFuseSoftmaxWrongPermuteDim) {
  const auto src = R"JIT(
    def forward(self, x):
        a = smith.permute(x, [0, 1, 2])
        b = smith.softmax(a, 2)
        c = smith.permute(b, [0, 1, 2])
        return c.clone()
  )JIT";
  smith::jit::Module mod("m");
  mod.define(src);
...
TEST(FuseClampNaNToNum, FusionHappens) {
  const auto src = R"JIT(
    def forward(self, x):
        y = smith.clamp(x, min=0.0, max=1.0)
        z = y.nan_to_num()
        return z.clone()
  )JIT";
  smith::jit::Module mod("m");
