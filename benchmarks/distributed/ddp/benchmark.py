...
def benchmark_process_group(pg, benchmark, use_ddp_for_single_rank=True):
    smith.manual_seed(pg.rank())
    smith.cuda.manual_seed(pg.rank())
...
class SmithvisionBenchmark(Benchmark):
    ...
    def create_model(self):
        return smithvision.models.__dict__[self.model]().to(self.device)

    def generate_inputs(self):
        return [smith.rand([self.batch_size, 3, 224, 224], device=self.device)]

    def generate_target(self):
        return smith.tensor([1] * self.batch_size, dtype=smith.long, device=self.device)


def main():
    parser = argparse.ArgumentParser(description="Blacksmith distributed benchmark suite")
    ...
    num_gpus_per_node = smith.cuda.device_count()
    ...
    if args.rank == 0:
        print("-----------------------------------")
        print("Blacksmith distributed benchmark suite")
        print("-----------------------------------")
        print()
    ...
    smith.cuda.set_device(dist.get_rank() % 8)
    device = smith.device(f"cuda:{dist.get_rank() % 8:d}")

    benchmarks = []
    if args.model:
        benchmarks.append(
            SmithvisionBenchmark(
                device=device,
