...
def parallel_workload(x):
    ...
    for i in range(PARALLEL_TASKS_NUM):
        futs.append(smith.jit._fork(parallel_task, x))
    for i in range(PARALLEL_TASKS_NUM):
        smith.jit._wait(futs[i])
    return x
...
if __name__ == "__main__":
    ...
    if args.with_cuda and not smith.cuda.is_available():
    ...
    for profiling_enabled in [False, True]:
        ...
        input_x = smith.rand(args.profiling_tensor_size, args.profiling_tensor_size)
        ...
        if args.use_script:
            traced_workload = smith.jit.trace(workload, (input_x,))
